...
const log = debug('fcc:boot:donate-paypal');

export default function donatePayPal(app, done) {
  const api = app.loopback.Router();
  const hooks = app.loopback.Router();
  const donateRouter = app.loopback.Router();

  // -- API Endpoints -- //
  function apiAddDonation(req, res) {
    const { user, body } = req;
  ...
  function apihookUpdatePaypal(req, res) {
  ...
  if (paypalPublicInvalid || paypalSecretInvalid) {
    ...
    api.post('/add-donation', apiAddDonation);
    hooks.post('/update-paypal', apihookUpdatePaypal);
