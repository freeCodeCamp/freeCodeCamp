...
const LAB_FILE = path.resolve(
  __dirname,
  '..',
  'curriculum/challenges/english/blocks/lab-platformer-level-1-1/66faab00000000000000000b.md'
);

const LAB_URL =
  '/learn/certified-game-developer-with-phaser/lab-platformer-level-1-1/lab-platformer-level-1-1';

const readLabSolution = (): string => {
  const md = fs.readFileSync(LAB_FILE, 'utf8');
...
test.describe('Phaser Platformer Level 1-1 Lab', () => {
  test.skip(
  ...
  test('completes the platformer lab', async ({ page, browserName }) => {
