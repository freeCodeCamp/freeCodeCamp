...
test.describe('Challenge Title Component (signed out)', () => {
  ...
  test('should not display GreenPass after challenge completion', async ({
    ...
    await page.getByRole('radio', { name: '</p>' }).click({ force: true });
    ...
    await page.getByRole('radio', { name: 'src' }).click({ force: true });
    ...
    // After clicking 'submit-and-go' button, page redirects to courses list.
    ...
    await page.waitForURL(
      '/learn/responsive-web-design-v9/#lecture-understanding-html-attributes'
    );
