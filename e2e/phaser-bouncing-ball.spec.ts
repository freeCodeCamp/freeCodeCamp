...
const stepUrl = (n: number) =>
  `/learn/certified-game-developer-with-phaser/workshop-bouncing-ball-lab/step-${n}`;
    ...
    case 2:
      return [
        readStepJs(3).replace(
          /this\.add\.circle\(\s*240\s*,\s*60\s*,\s*12\s*,\s*0xffaa44\s*\)/,
          'this.add.circle(240, 60, 12, 0x000000)'
        ),
        readStepJs(3).replace(
          /this\.physics\.add\.existing\(\s*this\.ball\s*\)/,
    ...
    case 3:
      return [
        readStepJs(4).replace(
          /arcade\s*:\s*\{\s*gravity\s*:\s*\{\s*y\s*:\s*300\s*\}\s*\}/,
          'arcade: { gravity: { y: 0 } }'
        )
      ];
    case 5:
      return [
        readStepJs(6).replace(
          /this\.ball\.body\.setBounce\(\s*0\.7\s*\)/,
    ...
    case 6:
      return [
        readStepJs(7).replace(
          /this\.ball\.body\.setBounce\(\s*1\s*\)/,
...
test.describe('Phaser Bouncing Ball Lab', () => {
  test.skip(
