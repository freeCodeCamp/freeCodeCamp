...
test.describe('Mobile upper jaw interactions', () => {
  test.use({
  ...
  test.describe('Touch scrolling', () => {
    test('scrolls while touch is held on a challenge with example content', async ({
      page
    }) => {
      await openChallenge(page, challengeWithExampleUrl);
      await expect(
        page.locator('#description details.code-details')
      ).toHaveCount(1);

      await expectScrollWhileHolding(page);
    });

    test('scrolls while touch is held on long challenge content', async ({
      page
    }) => {
    ...
    test('scrolls when a drag starts on the upper jaw container', async ({
      page
    }) => {
      await openChallenge(page, challengeWithLongContentUrl);
    ...
    test('does not vertically scroll the upper jaw for a horizontal example-code gesture', async ({
      page
    }) => {
  ...
  test('keeps breadcrumb links and example code dropdown interactive', async ({
    page
  }) => {
    await openChallenge(page, challengeWithExampleUrl);
