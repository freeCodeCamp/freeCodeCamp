...
const stepIdHex = (n: number) =>
  `66faa80000000000000000${(0xa6 + n).toString(16).padStart(2, '0')}`;
...
const stepUrl = (n: number) =>
  `/learn/certified-game-developer-with-phaser/workshop-match-3-puzzle/step-${n}`;
...
const wrongCodesForStep = (n: number): string[] => {
  switch (n) {
    ...
    case 2:
      return [
        readStepJs(3).replace(/this\.cols\s*=\s*8/, 'this.cols = 4'),
        readStepJs(3).replace(/this\.cell\s*=\s*48/, 'this.cell = 16')
      ];
    case 5:
      return [
        readStepJs(6).replace(
          /this\.input\.on\(\s*"gameobjectdown"/,
          'this.input.on("pointerdown"'
        ),
        readStepJs(6).replace(/this\.busy\s*=\s*false/, 'this.busy = true')
      ];
    case 7:
      return [
        readStepJs(8).replace(
          /this\.findMatches\s*=\s*\(\)\s*=>/,
          'this.findMatch = () =>'
        )
      ];
    case 14:
      return [
        readStepJs(15).replace(
          /this\.events\.emit\(\s*"scoreUpdate"/,
    ...
    case 27:
      return [
        readStepJs(28).replace(
          /this\.scene\.restart\(\s*\{\s*level:\s*nextLevel\s*\}\s*\)/,
          'this.scene.restart()'
        )
      ];
    case 33:
      return [
        readStepJs(34).replace(
          /Math\.max\(\s*adx\s*,\s*ady\s*\)\s*<\s*20/,
          'Math.max(adx, ady) < 0'
        )
      ];
    case 35:
      return [
        readStepJs(36).replace(/this\.hintTimer\s*\?\.\s*remove\(\)/g, '')
      ];
    case 41:
      return [
        readStepJs(42).replace(
          /this\.sound\.mute\s*=\s*this\.muted/,
          'this.sound.mute = false'
        )
      ];
    case 49: {
      const sol = readStepSolution(50);
      return [sol.replace(/if\s*\(\s*this\.combo\s*>=\s*3\s*\)/, 'if (false)')];
    }
    default:
      return [];
  }
...
test.describe('Phaser Match-3 Puzzle', () => {
  test.skip(
  ...
  test('renders the canvas after the final solution loads', async ({
    page,
    browserName
  }) => {
