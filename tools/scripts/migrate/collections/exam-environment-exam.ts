...
export async function migrate(db: Db) {
  const child = log.child({ collection: 'ExamEnvironmentExam' });
  child.info('Starting migration for ExamEnvironmentExam collection');
  const collection = db.collection<ExamEnvironmentExam>('ExamEnvironmentExam');
