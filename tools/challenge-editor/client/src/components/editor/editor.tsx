...
const Editor = () => {
  ...
  return (
    <div>
      ...
      <p>
        <Link
          to={`${import.meta.env.CHALLENGE_EDITOR_LEARN_CLIENT_LOCATION || 'http://localhost:8000'}/learn/${superBlockNameMap[superblock || '']}/${block || ''}/${items.name.replace(/[\s]+/g, '-').toLowerCase() || ''}`}
          target='_blank'
        >
