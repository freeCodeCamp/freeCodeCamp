...
test('Current streak calculation', (t) => {
  t.plan(7);
...
test('Longest streak calculation', (t) => {
  t.plan(9);
  ...
  const count = 100;
  let cals = Array.apply(null, Array(count))
    .map((ele, index) => moment(moment().subtract(index, 'days')).valueOf())
    .sort();
  t.equal(calcLongestStreak(cals), count, 'should return correct longest streak when there is a very long period');
});
