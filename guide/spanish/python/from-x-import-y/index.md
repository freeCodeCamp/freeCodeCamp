...
Entonces nuestro código tendría `math.sqrt(x)` en lugar de `sqrt(x)` . Esto sucede porque cuando usamos `import x` , se crea un espacio de nombres `x` para evitar conflictos de nombre. Tienes que acceder a cada objeto del módulo como `x.<name>` . Pero cuando usamos `from x import y` aceptamos agregar `y` al espacio de nombres global principal. Entonces, al usar esto, debemos asegurarnos de que no tenemos un objeto con el mismo nombre en nuestro programa.
...
Incluso podemos usar `form x import *` , esto importaría todos los métodos, clases de ese módulo al espacio de nombres global del programa. Esta es una mala práctica de programación porque estarías incluyendo muchas funciones que ni usarás. Ademas, siempre es mejor usar `x.y()` porque es más facil entender lo que quiera decir. Por favor, evítelo.
