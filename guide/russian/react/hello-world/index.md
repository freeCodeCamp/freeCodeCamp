...
Каждое изучение языка начинается с традиционного примера Hello World. Здесь вы познакомитесь с такой же программой HelloWorld на React.

Все в react - это компонент.

Но перед этим нам нужно убедиться, что на компьютере установлены node.js и npm. По желанию мы можем использовать CRA (Create React App), который является инструментом, созданным разработчиками в Facebook, чтобы помочь вам создавать приложения React. Это избавит вас от длительной настройки и конфигурации. Вы просто запускаете одну команду и создаете приложение для реагирования, устанавливая инструменты, необходимые для запуска проекта React.
