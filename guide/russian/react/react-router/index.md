...
Теперь давайте добавим навигацию, чтобы перемещаться между страницами.

Для этого мы будем использовать компонент `<Link>`. `<Link>` похожа на использование тега привязки html.

Из документов:

Основной способ разрешить пользователям перемещаться по вашему приложению. <Link> сделает полностью доступным тег привязки с соответствующим href. 
Для этого сначала создадим компонент Nav. Наш компонент Nav будет содержать компоненты `<Link>` и будет выглядеть так:
