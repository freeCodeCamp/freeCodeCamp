...
Рамка Bootstrap предоставляет вам Glyphicons для значка. Bootstrap по умолчанию не включает библиотеку значков, но для вас есть несколько рекомендаций. Хотя большинство наборов иконок включают в себя несколько форматов файлов, мы предпочитаем реализацию SVG для их улучшенной доступности и векторной поддержки.

### Как пользоваться

Чтобы использовать значок Bootstrap, создайте тег span с `glyphicon` базового класса и отдельным классом значков. Используйте его только для элементов, которые не содержат текстового содержимого и не имеют дочерних элементов.
...
Чтобы использовать значки начальной загрузки, вы просто создаете `<span>` и применяете применимый класс CSS для значка. Пример кода приведен ниже.
