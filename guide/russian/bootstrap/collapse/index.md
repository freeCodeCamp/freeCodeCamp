...
Свернуть это плагин, который поможет вам скрыть или раскрыть элементы с помощью плавной анимации. Разработчики обычно используют развал bootstrap, чтобы скрыть или показать дополнительные детали раздела веб-страницы. Например, у вас может быть список предметов с очень длинными описаниями. При отображении всего будет потребляться много места, поэтому сбой бутстрапа можно использовать для скрытия и раскрытия описания.

Коллапс начальной загрузки предоставляется как отдельный плагин с именем collapse.js, что означает, что он может использоваться вне среды бутстрапа. Сам плагин можно найти здесь http://getbootstrap.com/2.0.4/javascript.html#collapse.
