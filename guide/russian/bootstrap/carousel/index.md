...
Карусель - это компонент слайд-шоу для циклического показа таких элементов, как изображения или слайды текста. Он обеспечивает динамический способ представления большого количества контента (текста или изображений) путем скольжения или циклического, плавного перемещения.
...
Объяснение атрибутов и классов, используемых в приведенном выше примере:

### 1) Атрибуты

a) `data-ride` : - `data-ride ="carousel"` позволяет начать анимацию после загрузки страницы.
...
c) `carousel-inner` : - Это обозначает часть кода, который будет содержать текст / изображение, отображаемое в слайдах.
...
f) `right carousal-control` : - добавляет кнопку «Вперед», позволяющую перейти к следующему слайду.
...
#### 1) Через Data атрибуты

Атрибут `data-ride="carousel"` используется для обозначения карусели как анимации, начинающейся после загрузки страницы. `data-slide` принимает ключевые слова `prev` или `next` , что изменяет положение слайда относительно его текущего положения.
...
Параметры могут передаваться через Data атрибуты или JavaScript. Для Data атрибутов добавьте имя параметра в `data-` , как в `data-interval=""` .
