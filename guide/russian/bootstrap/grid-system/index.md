...
Используйте `row` для группировки столбцов, это будет держать все в строгом порядке и помогать вам структурировать вашу сетку. Класс `row` по сути создаёт строку которая уже потом делится на столбцы.
```
<div class="row"></div> 
```
