...
React é uma biblioteca JavaScript para criar interfaces com o usuário. Ele foi eleito o mais amado na categoria "Frameworks, bibliotecas e outras tecnologias" da Pesquisa de desenvolvedores do Stack Overflow 2017. 
...
3.  Fluxo de dados unidirecional. A interface do usuário no React é, na verdade, a função do estado que significa que, à medida que o estado é atualizado, ele também atualiza a interface do usuário. Portanto, nossa interface de usuário progride conforme o estado muda.
...
1.  Rápido. Os aplicativos feitos no React podem lidar com atualizações complexas e ainda são rápidos e responsivos.
...
3.  Escalável. React apresenta o melhor desempenho em programas grandes onde muitos dados são alterados.
...
O React usa o DOM virtual para primeiramente renderizar uma árvore HTML virtualmente e, sempre que um estado é modificado, é obtido uma nova árvore HTML que precisa ser levada ao DOM do navegador, em vez de gravar toda a nova árvore, o React apenas grava diferença entre a nova árvore e a árvore anterior (desde que o React tenha ambas as árvores na memória). Esse processo é conhecido como Reconciliação de Árvore.
...
Você gostaria de começar a aprender os fundamentos do React, sem ficar atolado criando um ambiente de desenvolvimento? As chances são de que, se você é novo no desenvolvimento Web, configurar um ambiente de desenvolvimento pode deixá-lo um pouco intimidado quando estiver apenas tentando aprender React ou simplesmente aprender React pela primeira vez.
...
O primeiro argumento é o "o quê" do React. O segundo argumento é o "onde", a localização que você quer que seja colocado no DOM. Vamos começar chamando nosso método ReactDOM.render(). Nosso primeiro argumento será nosso JSX.
...
[O](http://www.youtube.com/watch?feature=player_embedded&v=100pKUE3OPI) 
```
A [documentação oficial do react diz](https://reactjs.org/docs/introducing-jsx.html) : “Esta sintaxe de tag engraçada não é nem uma string nem um HTML. É chamado de JSX e é uma extensão de sintaxe para JavaScript. Recomendamos usá-lo com o React para descrever como deve ser a interface do usuário. O JSX pode lembrá-lo de uma linguagem de modelo, mas ela vem com todo o poder do JavaScript. O JSX produz React "elements" ”.

Muitas vezes, JSX enlouquece as pessoas que foram desenvolvedores por um tempo, porque parece HTML. Porém, logo os desenvolvedores aprendem a diferenciar o JSX do HTML. O HTML tem o seu lugar, o CSS tem o seu lugar e o JavaScript tem o seu lugar. O JSX parece desfocar as linhas. Você está usando o que parece ser HTML, mas como o Facebook diz, vem com todo o poder do JavaScript.
...
É aqui que queremos que nosso conteúdo react seja renderizado ao dom. Você provavelmente já fez isso algumas vezes no passado. Vamos apenas digitar `document.getElementById()`. E vamos passar para o argumento do id do aplicativo. E é isso. Agora, segmentaremos o div com o ID do aplicativo para inserir nosso conteúdo de reação.
...
Se você clicar na tag head, poderemos ver nossas bibliotecas de scripts incluídas. Então podemos ir ao corpo do nosso documento. Vamos clicar na nossa div com o id de "app". E quando vemos a nossa tag `<h1>` com o conteúdo "Hello World".
