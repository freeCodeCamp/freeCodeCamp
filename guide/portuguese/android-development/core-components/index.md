...
*   serviços de _segundo plano_ : eles não estão diretamente relacionados às atividades do usuário, então eles podem ser encerrados se mais memória RAM for necessária.

## [Receptores de transmissão](https://developer.android.com/guide/components/broadcasts)

Um _receptor de trasmissão_ é outro componente sem interface de usuário (exceto uma notificação de barra de status opcional) que permite ao sistema entregar eventos de / para o aplicativo, mesmo quando este não foi lançado anteriormente.
