---
title: Intro to Matrix Multiplication
localeTitle: Introdução à multiplicação de matrizes
---
## Introdução à multiplicação de matrizes

Este é um esboço. [Ajude nossa comunidade a expandi-lo](https://github.com/freecodecamp/guides/tree/master/src/pages/mathematics/intro-to-matrix-multiplication/index.md) .

[Este guia de estilo rápido ajudará a garantir que sua solicitação de recebimento seja aceita](https://github.com/freecodecamp/guides/blob/master/README.md) .

#### Mais Informações: