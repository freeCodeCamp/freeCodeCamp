---
title: How to Find the Limit of a Function Algebraically
localeTitle: Como encontrar o limite de uma função algebricamente
---
## Como encontrar o limite de uma função algebricamente

Este é um esboço. [Ajude nossa comunidade a expandi-lo](https://github.com/freecodecamp/guides/tree/master/src/pages/mathematics/how-to-find-the-limit-of-a-function-algebraically/index.md) .

[Este guia de estilo rápido ajudará a garantir que sua solicitação de recebimento seja aceita](https://github.com/freecodecamp/guides/blob/master/README.md) .

#### Mais Informações: