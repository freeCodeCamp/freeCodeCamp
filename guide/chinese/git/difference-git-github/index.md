...
GitHub是一家提供Git存储库托管的公司。这意味着它们提供了一个交钥匙解决方案，可以在其服务器上托管Git存储库。这对于保留存储库的备份非常有用（Git只跟踪随着时间的推移对文件所做的更改，仍然需要备份repo），并且有一个集中的位置来保存和共享您的代码。

GitHub不仅仅是一个Git存储库托管服务，它还是一个[软件伪造](https://en.wikipedia.org/wiki/Forge_(software)) 。这意味着它还提供[问题跟踪器](https://en.wikipedia.org/wiki/Issue_tracking_system) ， [代码审查](https://en.wikipedia.org/wiki/Code_review)工具以及与其他人协作和创建软件的其他工具。
