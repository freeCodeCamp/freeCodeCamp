...
将所有变量更改为`let`或`const`并重命名`sentence` 。
...
通过在`sentence`上使用`const` ，我们可以将它设为只读，并且通过在for循环中使用`let` on `i` ，我们可以避免一起使用`var` 。为了增加代码清晰度，我们还可以将`sentence`更改为`SENTENCE`以显示它是常量。
