...
localeTitle: 分号
---
Javascript中不需要分号。这是因为Javascript有一个名为Automatic Semicolon Insertion “自动分号插入” 的功能或简称ASI。 ASI会为您的Javascript添加分号。默认情况下它始终处于活动状态，它是语言的一部分，无法禁用。

ASI有一套规则用于确定应该在哪里插入分号。如果已经存在分号，则不会改变任何内容。有关ASI如何工作的更多信息，请参阅[StackOverflow 答案](http://stackoverflow.com/a/2846298/3467946)。
...
一致的编码风格使代码更具可读性。决定你是否会使用分号，并在任何地方都这样做。最好不要有些地方有，而其他地方沒有使用分号。
