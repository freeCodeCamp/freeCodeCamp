...
When using different algorithms some questions have to be asked. How big is the collection being sorted? How much memory is at disposal to be used? Does the collection need to grow? The answers to these questions may determine what algorithm is going to work best for the situation. Some algoritms like merge sort may need a lot of space to run, insertion sort is not always the fastest but it doesn't require much resourses to run. It may need to be determined what the requirements of the system are and the limitations before deciding what algorithm to use.
