...
Algorithms are most commonly judged by their efficiency and the amount of computing resources they require to complete their task. A common way to evaluate an algorithm is to look at its time complexity. This shows how the running time of the algorithm grows as the input size grows. Since the algorithms today have to operate on large data inputs, it is essential for our algorithms to have a reasonably fast running time. One factor that plays into the running time (how fast the algorithm runs) is the amount of resources that are used. The trade off of speed is often the memory used to perform certain tasks faster.
