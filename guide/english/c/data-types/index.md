...
Since C is not interpreted language it is not allowed to declare variables dynamically so dynamic allocation of memory to variables is not possible .Variables should be declared at the begining or before using it.Also there should be provision by IDE to indicate the unused variable which is declared .
## Integer data types
