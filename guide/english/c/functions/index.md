...
Suppose a programme is divided into hundreds of mini programmes (each such mini programme is called 'function'). Now the programmer wants that certain functions are used only when they are called or told to run. But it is quiet confusing for the compiler to know which function to skip and which to run. So, the idea of <b>main()</b> function is thought of, in which only a single function is executed by the compiler and is the <b>main()</b> function. All the other function calls are to be made inside of the <b>main()</b> function wherever neccessary. Any other piece of code (funtions) is of no use unless it is called in inside the <b>main()</b> function.
