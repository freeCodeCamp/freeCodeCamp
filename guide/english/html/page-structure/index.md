...
```
1. The tag `<!DOCTYPE html>` statement must always be the first to appear on an `HTML` page to tell the browser which version of the language is being used. In this case, we are working with `HTML5`.
...
1. Between the `<body>` and `</ body>` tags is where the page content is placed, which is what is displayed in the browser.
