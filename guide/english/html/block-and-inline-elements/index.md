...
Common **block-level elements** are `<div>`,`<p>`,`<article>`,`<section>`,`<figure>`,`<footer>`,`<header>`,`<form>`,`<h1>`,`<main>` etc.
...
Some of the **inline elements** are `<a>`,`<span>`,`<img>`,`<code>`,`<cite>`,`<button>`,`<input>`,`<input>`,`<strong>`,`<label>`,`<object>`,`<script>` etc.
