...
**(Procedural approach)**
     ...
     function largestOfFour(arr) {
      let largestArr = [];
      for(let arrSet in arr){
       largestArr.push(Math.max(...arr[arrSet]));
      }
     return largestArr;
     }
...
*   Create an empty array to store the value of the maximum value in each of the subarrays
*   Create a for loop to access each sub-array in the given array
*   push the maximum value in each of the sub-arrays into the empty array
