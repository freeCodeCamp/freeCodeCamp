...
A For Loop is a repetitive statement that checks for a condition and then based upon the condition, a block of code is executed repeatedly until the specified condition is satisfied.
...
## Explanation
In the above example, the initialization condition is first set to a=10. Before initializing, The loop checks for this condition. It then checks for the condition expression- above, it checks if a<20, which holds true as 10<20(for the first case). Now the body of the loop is executed and we get the output "Value of a: 10". Then the update expression is executed, which adds the number 1 to 'a' and the value of 'a' gets updated to 11. The same steps are followed (as above) until the value of v reaches less than 20, such as 19.
