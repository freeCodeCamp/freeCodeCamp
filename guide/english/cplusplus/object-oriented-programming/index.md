...
Data abstraction refers to providing only needed information to the outside world and hiding implementation details. For example, consider a class Complex with public functions as getReal() and getImag(). We may implement the class as an array of size 2 or as two variables. The advantage of abstractions is, we can change implementation at any point, users of Complex class won't be affected as our method interface remains same. Had our implementation be public, we would not have been able to change it.Abstraction helps to protect data variables and member functions of class and accessability to vaiables and functions can be decided by using access specifiers:like 1.public:data members and functions declared public can be accessed by subclass of the class and functions belonging to class as well as other functions in friend class 
2.private:Data members and functions declared private can be accessed by member functions of class only.
3.protected:Data members and functions declared protected can be accessed by member functions of existing class and functions belonging to subclass.

### Inheritance:
Inheritance is the process by which objects of one class acquire the properties of objects of another class. It supports the concept of hierarchical classification. Inheritance provides reusability. This means that we can add additional features to an existing class without modifying it.Inheritance supports fact that subclass can add new features and methods in addition to existing features without changing it . Sometimes method can be override but by using certain keywords we can access methods of superclass.
Types of inheritance are:1.Single Inheritance2.Multilevel Inheritance3.Multiple Inheritance4.Hierarchical5.Hybridetc.
