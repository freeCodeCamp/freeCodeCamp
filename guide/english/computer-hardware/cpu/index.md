...
## CPU
  ...
  **The Central Processing Unit (CPU)** is hardware responsible for interpreting and performng system and application related software tasks. The CPU serves as the "brain" of a computer, allowing it to perform essential computational tasks.
...
Most modern CPUs use a 64-bit architecture, which means they use 64-bit long memory adresses. Older CPUs used 32-bit, 16-bit, and even 8-bit architectures. The largest number a 64-bit CPU can store is 18,446,744,073,709,552,000. A CPU needs memory adresses to get specified values from <a href='https://en.wikipedia.org/wiki/Random-access_memory'>**random access memory (RAM)**</a>. If we call the length of the memory adresses n, 2^n is the amount of memory cells a CPU can adress.

An instruction cycle for a CPU is called the <a href='https://en.wikipedia.org/wiki/Instruction_cycle'>fetch-decode-execute cycle</a>, where the computer retrieves a instruction from its memory, determines which instruction it fetched and what it does, and then carries out said instrucitons.

Perhaps the most common issue affecting the CPU is inadequate cooling, as CPUs, together with graphics cards (GPU), are one the main generators of heat in computer systems. Due to their nature, CPUs are typically included with a dedicated cooling fan. Consequently, there are various ways of reducing generated heat in a computer, with the two main methods being air fans and liquid cooling, commonly known as water cooling, systems. Proper heat maintenance, such as regular thermal grease application, computer interior cleanup and additional hardware, such as attachable fans or a better CPU cooler, can allow a properly configured CPU to perform better than rated by the chip manufacturer, also referred to as the process of <a href='https://en.wikipedia.org/wiki/Overclocking'>"Overclocking"</a>. Note that chips must be unlocked in order to have the ablility to be overclocked and increased cooling measures are recommended for successful overclocking. 
...
#### Additional sources:
<a href='https://en.wikipedia.org/wiki/Central_processing_unit' target='_blank' rel='nofollow'>Central processing unit, Wikipedia</a>

<a href='https://www.pcworld.com/article/3186812/computers/how-to-overclock-your-pcs-cpu.html'>How to overclock your PC's CPU, PCWorld </a>
