...
<a>Bash</a> (short for Bourne Again SHell) is a Unix shell, and a command language interpreter. A shell is simply a macro processor that executes commands. It's the most widely used shell packaged by default for most Linux distributions, and a successor of the Korn shell (ksh) and the C shell (csh).

Many things that can be done in the Linux operating system can be done via command line. Some examples are:
* Editing files
* Adjusting the volume of the operating system
* Fetching web pages from the internet
* Automating the work you do every day
...
You can also put all of your bash commands into a .sh file, and run them from the command line. Let's say you have a bash script with the following contents:
...
However, it is only executed if you run your script as an executable. For example, when you type `./scriptname.extension`, it will look at the top line to find out the interpreter; whereas running the script as `bash scriptname.sh` will result in the first line being ignored. 

Then you could run the script as follows:
For making the file executable, you should call this command under sudo chmod +x "filename".
```
