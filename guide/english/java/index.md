...
JAVA is one of the OBJECT ORIENTED PROGRAMMING LANGUAGE, unlike C,FORTAN etc. which are procedural language. OBJECT ORIENTED concept of programming makes it much efficient than procedural languages. Basically, object orientation breaks the programme(codes) into parts, which is decided by the programmer. Then the complete programme is stored in the permanent memory and when it is executed only that part is brought to dynamic memory which is called by the programmer. Each such part is called a 'class' and when it is called in the dynamic memory it is called 'object'.
