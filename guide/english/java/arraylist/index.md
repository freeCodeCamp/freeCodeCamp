...
**Compact way of accessing each element in the ArrayList **

```java
  variable_name.foreach(e -> { ... });
```

This function will go through all the elements in the ArrayList and do everything in the curly brackets.In this case each element has a name *e* which can be used inside the loop let's say for printing.

```java
  variable_name.foreach(e -> { System.out.prinln(e.toString()) });
```

Learn more in the [docs](https://docs.oracle.com/javase/8/docs/api/java/util/ArrayList.html#forEach-java.util.function.Consumer-)
