...
Vagrant is an automation tool that aids in creating and managing virtual machines that is simple to use and allows you to set up a  environment quickly. The enviroment's exact installation and configuration can then be shared by using a Vagrantfile, so all team members are using the same environment no matter what computer they are using.
