...
The peer to peer network, Torrent, for example is a decentralised system, where the user has complete control. There are many advantages to decentralized systems including increased security, reduced reliance on trusted intermediaries, and lower costs associated with malicious actors or accidental data loss.
...
When something is stored on a Distributed Ledger, multiple copies of it are made across the network at the same time. Unlike traditional databases, distributed ledger do not have a central database or administration functionality. This means that all participates in the 
network must come to a consensus on the current state of the ledger, including the entire history of transactions that preceding it.
...
BlockTech uses the concepts like Proof of Work and Hash encryption to ensure security and immutability. Proof of work involves several people around the world using computational algorithms to try and find the appropriate hash value that satisfies a predefined condition regarding the hash value. In order to tamper with data on the blockchain, a malicious actor would need to have more computational power than all the rest of the honest miners combined.
