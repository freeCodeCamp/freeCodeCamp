...
6. `pwd` Stands for "print working directory" . Where am I? Linux can be unforgiving, particularly when you delete something. So make sure you know where you are before you issue your commands.
...
Also, important commands used for System Administrators are the following:

1. `uptime` Command
In Linux uptime command shows since how long your system is running and the number of users that are currently logged in. It also displays load average for 1,5 and 15 minutes intervals.

2. `w` Command
It will displays users currently logged in and their processes along with load averages. also shows the login name, tty name, remote host, login time, idle time, JCPU, PCPU, command and processes.

3. `users` Command
Users command displays currently logged in users. This command doesnt have other parameters other than help and version.
...
7. `crontab` Command
List scheduled jobs for current user with crontab command and -l option.
