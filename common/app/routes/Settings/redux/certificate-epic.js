...
import { types, claimCertComplete, claimCertError } from '../redux';

function certificateEpic(actions$, { getState }) {
  const start = actions$
    ::ofType(types.claimCert.start)
  ...
  const complete = actions$
    ::ofType(types.claimCert.complete)
    .flatMap(({ meta: { message, success } }) =>
      Observable.if(
        () => success,
  ...
  const error = actions$::ofType(types.claimCert.error).flatMap(error => {
