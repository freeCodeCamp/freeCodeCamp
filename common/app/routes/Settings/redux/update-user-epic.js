...
function backendUserUpdateEpic(actions$, { getState }) {
  ...
  const server = start.flatMap(({ payload }) => {
  ...
  const optimistic = start.flatMap(({ payload }) => {
    const username = usernameSelector(getState());
    return Observable.of(updateMultipleUserFlags({ username, flags: payload }));
  });
  const complete = actions$
    ::ofType(types.updateUserBackend.complete)
...
function refetchChallengeMapEpic(actions$, { getState }) {
  return actions$::ofType(types.refetchChallengeMap.start).flatMap(() => {
    const { app: { csrfToken: _csrf } } = getState();
    const username = usernameSelector(getState());
...
function updateMyPortfolioEpic(actions$, { getState }) {
  ...
  const serverEdit = edit.flatMap(({ payload }) => {
    const { id } = payload;
    const { app: { csrfToken: _csrf, username } } = getState();
  ...
  const optimisticEdit = edit.map(({ payload }) => {
    const username = usernameSelector(getState());
    return optoUpdatePortfolio({ username, portfolio: payload });
  });
  const complete = actions$
    ::ofType(types.updateMyPortfolio.complete)
    .flatMap(({ payload: { message } }) =>
      Observable.of(makeToast({ message }))
    );

  const serverRemove = remove.flatMap(({ payload: { portfolio } }) => {
    const { app: { csrfToken: _csrf } } = getState();
  ...
  const optimisticRemove = remove.flatMap(
...
function updateUserEmailEpic(actions, { getState }) {
  return actions::ofType(types.updateMyEmail).flatMap(({ payload: email }) => {
    const {
    ...
    const optimisticUpdate = Observable.just(updateUserEmail(username, email));
    const ajaxUpdate = postJSON$('/update-my-email', body)
      .catch(
        doActionOnError(
          () => (oldEmail ? updateUserEmail(username, oldEmail) : null)
        )
      )
      .filter(Boolean);
...
export function updateUserLangEpic(actions, { getState }) {
  const updateLang = actions
    .filter(
      ({ type, payload }) => type === types.updateMyLang && !!langs[payload]
    )
    .map(({ payload }) => {
  ...
  const ajaxUpdate = updateLang.debounce(250).flatMap(({ lang, oldLang }) => {
  ...
  const optimistic = updateLang.map(({ lang }) => {
