...
class EmailForm extends PureComponent {
  ...
  render() {
    const { fields: { email, confirmEmail }, handleSubmit } = this.props;

    const disableForm =
      (email.pristine && confirmEmail.pristine) ||
      ...
      <form id='email-form' onSubmit={handleSubmit(this.handleSubmit)}>
        ...
        <FullWidthRow>
          {!email.pristine && email.error ? (
            <HelpBlock>
              <Alert bsStyle='danger'>{email.error}</Alert>
            </HelpBlock>
          ) : null}
        </FullWidthRow>
        <Row className='inline-form-field'>
          <Col sm={3} xs={12}>
            <ControlLabel htmlFor='confirm-email'>Confirm Email</ControlLabel>
          </Col>
          <Col sm={9} xs={12}>
            <FormControl
              bsSize='lg'
        ...
        <FullWidthRow>
          {!confirmEmail.pristine && confirmEmail.error ? (
            <HelpBlock>
              <Alert bsStyle='danger'>{confirmEmail.error}</Alert>
            </HelpBlock>
          ) : null}
        </FullWidthRow>
        <Spacer />
