...
const stepChallengeEpic = proxy('./step-challenge-epic', {
...
test(file, function(t) {
  t.test('does not respond to random actions', t => {
    ...
    let called = false;
    stepChallengeEpic(actions, () => {}).subscribe(
      () => {
        called = true;
      },
      e => t.fail(e),
  ...
  t.test('steps back', t => {
    ...
    stepChallengeEpic(actions, { getState: () => state }).subscribe(
      onNextSpy,
      e => {
        throw e;
      },
      () => {
        t.assert(onNextSpy.calledOnce, 'epic not called exactly once');
        t.assert(
