...
const fCCglyph =
  'https://raw.githubusercontent.com/freeCodeCamp/assets/' +
...
const mapStateToProps = createSelector(
  ...
  panesSelector,
  (isSignedIn, isDropdownOpen, showLoading, panes) => {
    return {
...
export class FCCNav extends React.Component {
  renderLink(isNavItem, { isReact, isDropdown, content, link, links, target }) {
    ...
    const { isDropdownOpen, openDropdown, closeDropdown } = this.props;
    ...
    if (isReact) {
      return (
        <Link
          ...
          to={link}
          >
          <Component target={target || null}>{content}</Component>
        </Link>
      );
    }
    return (
      ...
      <Navbar className='nav-height' id='navbar' staticTop={true}>
        <div className='nav-component-wrapper'>
          <Navbar.Header>
            ...
            <NavbarBrand>
              <a href='/challenges/current-challenge' onClick={clickOnLogo}>
                <img
          ...
          <Navbar.Collapse>
            <Nav navbar={true} pullRight={true}>
              ...
              {shouldShowMapButton ? (
                <BinButton content='Map' handleClick={clickOnMap} key='Map' />
              ) : null}
              {navLinks.map(this.renderLink.bind(this, true))}
              <SignUp showLoading={showLoading} showSignUp={!isSignedIn} />
            </Nav>
          </Navbar.Collapse>
        </div>
      </Navbar>
...
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(FCCNav);
