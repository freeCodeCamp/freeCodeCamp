...
export default function loadCurrentChallengeEpic(actions, { getState }) {
  return actions
    ::ofType(types.clickOnLogo, types.clickOnMap)
    .debounce(500)
    .map(getState)
    .map(state => {
      let finalChallenge;
      ...
      const { challenge: challengeMap, challengeIdToName } = entitiesSelector(
        state
      );
      ...
      if (!currentChallengeId) {
        ...
        finalChallenge = challengeMap[challengeIdToName[currentChallengeId]];
      }
      return {
    ...
    .filter(
      ({ id, isOnAChallenge, currentlyLoadedChallengeId }) =>
