...
var createTestDisplay = function() {
  ...
  for (var i = 0; i < userTests.length;i++) {
    ...
    if (test.err !== null) {
      $(testDoc)
        .html('<div class=\'row\'><div class=\'col-xs-1 text-center\'><i class=\'ion-close-circled big-error-icon\'></i>' +
        '</div><div class=\'col-xs-11 test-output wrappable\'>' + test.text + '</div>' +
        '<div class=\'col-xs-11 test-output wrappable\'>' + test.err + '</div></div><div class=\'ten-pixel-break\'/>')
        .prependTo($('#testSuite'));
    } else {
      $(testDoc)
        .html('<div class=\'row\'><div class=\'col-xs-1 text-center\'>' +
        '<i class=\'ion-checkmark-circled big-success-icon\'></i></div>' +
        '<div class=\'col-xs-11 test-output test-vertical-center wrappable\'>' + test.text + '</div></div>' +
        '<div class=\'ten-pixel-break\'/>')
        .appendTo($('#testSuite'));
    }
  }
};
// These are never used locally un
...
function showCompletion() {
  ...
  var $modal = $('#complete-bonfire-dialog');
  $modal.modal('show');
