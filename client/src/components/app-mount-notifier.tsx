...
type StoredSessionQualification = { deactivateAt: number };
...
export const AppMountNotifier = ({
  ...
  const attemptQualification = useCallback(async () => {
    ...
    try {
      ...
      // The next meaningful action retries; nothing is queued across tabs.
    } finally {
  ...
  useEffect(() => {
    ...
    if (storedTimer && !activityStreakRef.current?.activeSession) {
      timerState.current = { ...storedTimer, lastTickAt: Date.now() };
      writeTimerState(timerState.current);
    } else {
      clearTimerState();
      timerState.current = undefined;
