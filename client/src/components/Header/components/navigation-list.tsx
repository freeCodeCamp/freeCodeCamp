...
const NavigationList = ({
  ...
  const closeAndFocus = () => {
    innerRef.current?.classList.add('force-show');
    setShowList(false);
    setTimeout(() => {
      innerRef.current?.focus();
      innerRef.current?.classList.remove('force-show');
    }, 100);
  };
  ...
  return (
    <>
      ...
      <ul
        ...
        className={`nav-list${showList ? ' display-menu' : ''}`}
      >
        <DonateButton
...
export default connect(
  null,
  mapDispatchToProps
)(withTranslation()(NavigationList));
