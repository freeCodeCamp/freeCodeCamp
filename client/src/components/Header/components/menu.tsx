...
function Menu({ fetchState, t, toggleNightMode, user, navigate }: MenuProps) {
  const [showMenu, setShowMenu] = useState(false);
  const [isLanguageMenuDisplayed, setLanguageMenuDisplayed] = useState(false);
  ...
  const handleLanguageChange = (
    ...
    if (newLanguage === 'exit-lang-menu') {
      ...
      setLanguageMenuDisplayed(false);
      return;
    }
  ...
  const handleLanguageButtonClick = () => {
    if (isLanguageMenuDisplayed) {
      ...
      // setLanguageMenuDisplayed(firstLangOptionRef.current);
      setLanguageMenuDisplayed(true);
    }
  };
  ...
  const handleLanguageButtonKeyDown = (
    ...
    const DoKeyPress = new Map<string, { select: () => void }>([
      ...
      [
        ...
        {
          select: () => {
            // setLanguageMenuDisplayed(lastLangOptionRef.current);
            setLanguageMenuDisplayed(true);
            event.preventDefault();
          }
        }
    ...
    <>
      <button
        ...
        className={`exposed-button-nav${
          showMenu ? ' reverse-toggle-color' : ''
        }`}
        id='toggle-button-nav'
        onBlur={handleMenuButtonBlue}
        onClick={handleClick}
        ref={menuButtonRef}
      >
