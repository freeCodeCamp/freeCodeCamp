...
Lambdas are essentially functions - independent units of code that you can use anywhere you would normally use an object, like passing an argument to a method. In this case, instead of creating a class and then creating an object from the class, or directly using an anonymous class, you write your lambda expression and just pass it to the method instead of a class object.

A Lambda Expression is composed of three parts - the function parameters, the `->` symbol, and the function body.

- The first part, function parameters, indicate what arguments you expect your lambda function to take. For instance, if you are iterating over a list of `Person` objects, you can write the function parameter of the Lambda expression as `(Person p)`. If you expect to get two (or more) `Person` objects as arguments, you would separate them with a comma, `(Person p1, Person p2)`. If you don't expect any argument, you just use empty paranthesis `()`. The rule is you can omit the paranthesis in case your Lambda expression accepts only 1 argument. In all other cases, the paranthesis are mandatory.

- The function parameters are followed by the `->` symbol (dash followed by a greater-than/arrow symbol).

- The third component of the Lambda expression is the function body, which performs the actual work. You write the function body within braces `{ }`. However, if your function body is just one line, you can omit the `{ }` and write it directly after the `->`, like so: 

  ```java
   Person p -> System.out.println(p.getName());
  ```
  This is a complete Lambda Expression. It accepts a `Person` object as argument and prints it's name using the `getName()` getter.
...
Normally, the Stream API is used in conjuction with Lambda Expressions to produce concise code.
...
Now assume you're writing a method which has a list of all the students and you want to get a list of all the student names. 
...
This is only one use of the Streams API used in Java 8. There are many other applications of streams utilizing other operations as seen here in the [documentation](https://docs.oracle.com/javase/8/docs/api/java/util/stream/Stream.html).
