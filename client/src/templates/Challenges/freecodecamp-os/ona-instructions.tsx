...
export function OnaInstructions({
  ...
  title,
  userToken
}: OnaInstructionsProps) {
  const { t } = useTranslation();

  function openOna() {
    const repoUrl = `https://github.com/freeCodeCamp/back-end-development-and-apis`;
  ...
  return (
    <div className='ca-description'>
      <p>{t('learn.freecodecamp-os.ona.intro')}</p>
      <OnaContinueAlert course={title} />
      <ol>
        <li>
          <Trans i18nKey='learn.freecodecamp-os.ona.step-1'>
            <a
              href='https://app.ona.com/login'
        ...
        <li>{t('learn.freecodecamp-os.ona.step-2')}</li>
      ...
      <ol start={3}>
        <li>{t('learn.freecodecamp-os.ona.step-3')}</li>
        <li>
          <Trans i18nKey='learn.freecodecamp-os.ona.step-4'>
            <code>placeholder</code>
          </Trans>
        </li>
        <li>{t('learn.freecodecamp-os.ona.step-5')}</li>
        <li>{t('learn.freecodecamp-os.ona.step-6')}</li>
      </ol>
    </div>
  );
}

interface OnaContinueAlertProps {
  course: string;
}

function OnaContinueAlert({ course }: OnaContinueAlertProps): JSX.Element {
  const { t } = useTranslation();
  return (
    <Callout variant='note' label={t('misc.note')}>
      <Trans
        values={{ course }}
        i18nKey='learn.freecodecamp-os.ona.continue-project'
      >
