...
export const SuperBlockTreeView = ({
  ...
  const treeNodes = useMemo(() => {
    return {
      name: '', // the name of root node should be empty
      children: chapters.map(chapter => {
        ...
        return {
          name: t(`intro:front-end-development.chapters.${chapter}`),
          children: modules.map(mod => {
            ...
            return {
              name: t(`intro:front-end-development.modules.${mod}`),
  ...
  return (
    <TreeView
      aria-labelledby={ariaLabelledBy}
      data={flattenTree(treeNodes)}
      nodeRenderer={({ element, getNodeProps }) => (
        <div {...getNodeProps()}>
          {element.name}
          {/* <div>{element.metadata?.content && element.metadata.content}</div> */}
        </div>
      )}
    />
  );
