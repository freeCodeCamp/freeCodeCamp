...
title: Use column-gap to Control Spacing
challengeType: 0
videoUrl: ''
...
```yml
tests:
  - text: <code>item5</code> clase debe tener una <code>column-gap</code> propiedad que tiene el valor de <code>2 / 4</code> .
    testString: 'assert(code.match(/.item5\s*?{[\s\S]*column-gap\s*?:\s*?2\s*?\/\s*?4\s*?;[\s\S]*}/gi), "<code>item5</code> class should have a <code>column-gap</code> property that has the value of <code>2 / 4</code>.");'
```
