...
```yml
tests:
  - text: Crea un nuevo elemento de <code>button</code> con el texto &quot;Información&quot;.
    testString: 'assert(new RegExp("info","gi").test($("button").text()), "Create a new <code>button</code> element with the text "Info".");'
  - text: Ambos botones de Bootstrap deberían tener las clases <code>btn</code> y <code>btn-block</code> .
  ...
  - text: Tu nuevo botón debe tener la clase <code>btn-info</code> .
