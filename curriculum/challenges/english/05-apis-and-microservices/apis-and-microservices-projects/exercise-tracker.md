...
- Clone <a href='https://github.com/freeCodeCamp/boilerplate-project-exercisetracker/'>this GitHub repo</a> and complete your project locally.
...
tests:
  ...
  - text: You can make a `GET` request to `api/exercise/users` to get an array of all users. Each item in the array is an object containing a users `username` and `_id`.
    ...
    "
  - text: 'You can `POST` to `/api/exercise/add` with form data `userId(_id)`, `description`, `duration`, and optionally `date`. If no date supplied it will use current date. Returned will be the user object with the exercise fields added.'
    testString: "async getUserInput => {
      const url = getUserInput('url');
      const res = await fetch(url + '/api/exercise/new-user', {
        method: 'POST',
    ...
    "
  - text: You can make a `GET` request to `/api/exercise/log` with a parameter of `userId=_id` to retrieve a full exercise log of any user. Returned will be the user object with a `log` array of all the exercises added. Each log item has the `description`, `duration`, and `date`. Also included is a `count` key that has the total exercise count.
    testString: "async getUserInput => {
      const url = getUserInput('url');
      const res = await fetch(url + '/api/exercise/new-user', {
        method: 'POST',
