...
Notice how, unlike breadth-first search, every time a node is visited, it doesnt visit all its neighbors at the same depth. Instead, it first visits one of its children and continues down that path until there are no more nodes to be visited on that path.
