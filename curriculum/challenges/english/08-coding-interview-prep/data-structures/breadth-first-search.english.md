...
The grey shades represent a node getting added into the queue and the black shades represent a node getting removed from the queue. See how everytime a node gets removed from the queue all their neighbors get added into the queue.

To implement this algorithm, you'll need to input a graph structure and a node you want to start at.

First, you'll want to be aware of the distances from the start node. You'll want to start all your distances initially with some large number, like <code>Infinity</code>. This gives a reference for the case where a node may not be reachable from your start node.
