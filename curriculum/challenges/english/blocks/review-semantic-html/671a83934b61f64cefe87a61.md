...
- **Linking to an element on the same page**: You can navigate to a specific part of the page by using an element’s `id` inside the `href` attribute. This is commonly used for skip links, table of contents, or long pages with multiple sections.
