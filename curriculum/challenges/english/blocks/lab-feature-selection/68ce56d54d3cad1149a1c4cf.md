...
```js
const checkbox = document.querySelector('.feature-card input[type="checkbox"]');
...
await __helpers.retryingTest(() => {
  return window.getComputedStyle(checkbox).backgroundColor !== uncheckedBG
  
}, "backgroundColor has not changed");
```
...
await __helpers.retryingTest(() => {
  return window.getComputedStyle(checkbox).borderColor !== uncheckedBC
  
}, "borderColor has not changed");
