...
The `isLeapYear` function should have one parameter.
...
```js
assert.match(__helpers.removeJSComments(code), /isLeapYear\s*\(\s*year\s*\)/);
```
...
// Explorer cannot detect reassignments yet, so a `result` that is assigned
// after it is declared still needs a regex check.
if (!result.value.matches('isLeapYear(year)')) {
  assert.match(__helpers.removeJSComments(code), /result\s*=\s*isLeapYear\s*\(\s*year\s*\)/);
}
```
...
```js
assert.match(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*result\s*\)/);
```
