...
let called__test_code__ = false;
Math.random = function() {
  called__test_code__ = true;
  return originalRandom__test_code__();
};
try {
  new Function(`${code}; return randomNumber`)();
  assert.isTrue(called__test_code__);
} finally {
...
```js
const consoleLogSpy__test_code__ = __helpers.spyOn(console, 'log');
try {
  new Function(`${code}`)();
  const fortunes__test_code__ = [fortune1, fortune2, fortune3, fortune4, fortune5];
  assert.isAtLeast(consoleLogSpy__test_code__.calls.length, 1);
  assert.oneOf(consoleLogSpy__test_code__.calls[0][0], fortunes__test_code__);
} finally {
  consoleLogSpy__test_code__.restore();
}
```
