...
Then use string concatenation with the `+` operator to join the string `$tokens[0]` followed by the `bot` variable followed by a period (`.`). 
...
You should use string concatenation with the `+` operator to join the string `$tokens[0]` with the `bot` variable followed by a period (`.`). Be mindful of spaces. Assign this concatenated string to the `botIntroduction` variable.
