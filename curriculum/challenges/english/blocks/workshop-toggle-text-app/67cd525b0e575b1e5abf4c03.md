...
const handleToggleVisibility = explorer.allFunctions.ToggleApp.allFunctions.handleToggleVisibility.toString();
assert.match(handleToggleVisibility, /setIsVisible\s*\(\s*!isVisible\s*\)/);
```
