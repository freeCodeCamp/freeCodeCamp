...
const handleToggleVisibility = explorer.allFunctions.ToggleApp.allFunctions.handleToggleVisibility;
assert.exists(handleToggleVisibility);
// The parser recovers from invalid syntax, so check the function has a body.
assert.match(handleToggleVisibility.toString(), /\{[\s\S]*\}/);
```
