...
```js
assert.match(code, /class\s+Bee[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Your `Bee` constructor should call `super` with the `emojiElement` parameter.

```js
assert.match(code, /super\s*\(\s*emojiElement\s*\)/);
```

Your `Bee` constructor should set `this.emoji` to `"🐝"`.

```js
assert.match(code, /this\.emoji\s*=\s*["']🐝["']/);
```
