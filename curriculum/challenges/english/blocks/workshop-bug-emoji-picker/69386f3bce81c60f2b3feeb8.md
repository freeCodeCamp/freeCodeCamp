...
Get the `select` element with an `id` of `species` from the DOM and store it in a constant named `selectElement`. Use the `!` non-null assertion operator to assert that the element is not `null`
...
Your `selectElement` should query the element with an `id` of `species` using `document.querySelector` with a type argument of `HTMLSelectElement`.
