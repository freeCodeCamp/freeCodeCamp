...
Get the paragraph element with an `id` of `bug-emoji` and store it in a constant named `bugEmojiElement`. Use the `!` non-null assertion operator to assert that the element is not `null`.
...
Your `bugEmojiElement` should query the element with an `id` of `bug-emoji` using `document.querySelector` with a type argument of `HTMLParagraphElement`.
