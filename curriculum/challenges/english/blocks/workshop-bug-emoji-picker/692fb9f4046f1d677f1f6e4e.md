...
Create a function named `isSelect` that takes a single parameter `element` of type `EventTarget | null`. The return type of the function should be a type predicate that narrows the type of `element` to `HTMLSelectElement`. <!-- Give an example of a type predicate here. Note to self: Remember to remove this comment -->
