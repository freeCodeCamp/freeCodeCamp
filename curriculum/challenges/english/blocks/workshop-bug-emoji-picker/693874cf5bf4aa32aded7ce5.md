...
Add a `change` evenet listner to the `selectElement` variable. In the listner, check if the `event.target` is a `HTMLSelectElement` using the `isSelect` type guard function. If it is, get the selected bug species from the `value` property of the `selectElement`, use it to get the corresponding `Bug` instance from the `bugMap`, and call its `render` method to update the displayed emoji.
