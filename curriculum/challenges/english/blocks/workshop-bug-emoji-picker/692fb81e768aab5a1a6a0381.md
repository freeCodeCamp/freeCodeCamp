...
```js
assert.match(code, /class\s+Spider[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Your `Spider` constructor should call `super` with the `emojiElement` parameter.

```js
assert.match(code, /class\s+Spider[\s\S]*?super\s*\(\s*emojiElement\s*\)/);
```

Your `Spider` constructor should set `this.emoji` to `"🕷️"`.

```js
assert.match(code, /this\.emoji\s*=\s*["']🕷️["']/);
```
