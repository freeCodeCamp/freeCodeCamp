...
Now that the `if` check is in place, use `e.target.value` to look up the corresponding bug in `bugMap` and call its `render` method.
...
```js
assert.match(code, /bugMap\s*\[\s*e\.target\.value\s*\]\s*\.\s*render\s*\(\s*\)/);
```
