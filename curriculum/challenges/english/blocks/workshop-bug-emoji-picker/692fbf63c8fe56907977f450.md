...
Create a `bugMap` object that maps bug species values to their corresponding class constructors. The object should have two properties: `"bee"` mapped to the `Bee` class and `"spider"` mapped to the `Spider` class. <!-- Revisit this description and give an example here. Note to self: Remember to remove this comment -->
