...
# --description--
Time to honour accessibility. Add a `Motion: on` button that toggles `this.reduceMotion`. Once flipped, the existing shake, flash, and fade handlers (already gated on `this.reduceMotion`) silently no-op.
...
Click `Motion: on` then `Shake` — the camera no longer shakes. Same for `Flash` and `Fade`.

# --hints--
Initialise `this.reduceMotion = false`.

```js
assert.match(code, /this\.reduceMotion\s*=\s*false/);
```

Add a `Motion: on` button at `(328, 4)` stored on `this.reduceBtn`.

```js
assert.match(code, /this\.reduceBtn\s*=\s*this\.makeButton\(\s*328\s*,\s*4\s*,\s*["\']Motion: on["\']/);
```

The handler should flip `this.reduceMotion` and update the label between `"Motion: on"` and `"Motion: off"`.

```js
assert.match(code, /this\.reduceMotion\s*=\s*!\s*this\.reduceMotion[\s\S]*this\.reduceBtn\.setLabel\([\s\S]*this\.reduceMotion\s*\?\s*[\"\']Motion: off[\"\']\s*:\s*[\"\']Motion: on[\"\']/);
```
