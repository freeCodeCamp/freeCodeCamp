...
# --description--
From here on, every camera knob gets a clickable button. Build a `makeButton(x, y, label, onClick)` closure that paints a 50×16 rectangle plus a centered text label, anchors them to the screen (`setScrollFactor(0)`), wires up the click, registers them on `this.uiLayer`, and returns a `setLabel` setter for runtime updates. Then ship the first button: a smooth-follow toggle that flips between exact tracking (`lerp = 1`) and cinematic smoothing (`lerp = 0.1`).
...
```js
assert.match(code, /this\.makeButton\s*=\s*\(\s*x\s*,\s*y\s*,\s*label\s*,\s*onClick\s*\)\s*=>/);
```

Inside `makeButton`, create the background rectangle with `setScrollFactor(0)`.

```js
assert.match(code, /\.rectangle\(\s*x\s*,\s*y\s*,\s*w\s*,\s*h\s*,\s*0x222244\s*,\s*1\s*\)[\s\S]*setScrollFactor\(\s*0\s*\)/);
```

Inside `makeButton`, wire the click via `bg.setInteractive().on("pointerdown", onClick)`.

```js
assert.match(code, /bg\.setInteractive\(\s*\)\.on\(\s*["\']pointerdown["\']\s*,\s*onClick\s*\)/);
```

Add the smooth-follow toggle via `this.makeButton(4, 4, "Smooth: off", ...)`.

```js
assert.match(code, /this\.makeButton\(\s*4\s*,\s*4\s*,\s*["\']Smooth: off["\']/);
```

The smooth-toggle handler should call `this.cameras.main.setLerp(k, k)` based on `this.lerpOn`.

```js
assert.match(code, /this\.cameras\.main\.setLerp\(\s*k\s*,\s*k\s*\)/);
```
