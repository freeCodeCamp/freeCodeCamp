...
# --description--
Add a `BG` button on the top row that cycles the camera's background colour through four palettes — useful for previewing how the world reads against different vignettes.

After the deadzone-viz block (and before the shake or anywhere in the UI block — its order on the top row only matters visually). Insert it right after the deadzone visualizer for easy reading:
...
```js
assert.match(code, /this\.bgIndex\s*=\s*0[\s\S]*const\s+bgColors\s*=\s*\[\s*0x101020\s*,\s*0x1f3030\s*,\s*0x2a1a3a\s*,\s*0x000000\s*\]/);
```

Add a `BG` button at `(274, 4)`.

```js
assert.match(code, /this\.makeButton\(\s*274\s*,\s*4\s*,\s*["\']BG["\']/);
```
...
```js
assert.match(code, /this\.bgIndex\s*=\s*\(\s*this\.bgIndex\s*\+\s*1\s*\)\s*%\s*bgColors\.length[\s\S]*this\.cameras\.main\.setBackgroundColor\(\s*bgColors\[\s*this\.bgIndex\s*\]\s*\)/);
```
