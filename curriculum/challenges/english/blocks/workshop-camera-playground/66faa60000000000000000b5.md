...
# --description--
Time for juice. Add a `Shake` button that fires `cameras.main.shake(300, this.shakeIntensity)` — a 300 ms shake, default intensity `0.012`. Gate it behind `this.reduceMotion` so the accessibility toggle (step 22) silently disables it once wired.
...
```js
assert.match(code, /this\.shakeIntensity\s*=\s*0\.012/);
```

Add a `Shake` button at `(112, 4)`.

```js
assert.match(code, /this\.makeButton\(\s*112\s*,\s*4\s*,\s*["\']Shake["\']/);
```

The handler should guard with `if (!this.reduceMotion)` and call `this.cameras.main.shake(300, this.shakeIntensity)`.

```js
assert.match(code, /if\s*\(\s*!\s*this\.reduceMotion\s*\)[\s\S]*this\.cameras\.main\.shake\(\s*300\s*,\s*this\.shakeIntensity\s*\)/);
```
