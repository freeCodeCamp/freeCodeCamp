...
# --description--
Where flash is percussive, **fade** is for transitions. Add a `Fade` button that fades to black over 600 ms, then fades back in over 400 ms once the fade-out completes. Use the `camerafadeoutcomplete` event to chain the fade-in.

After the flash button:
...
The `once` listener auto-removes after one firing — exactly what you want for a chained fade.

# --hints--
Add a `Fade` button at `(220, 4)`.

```js
assert.match(code, /this\.makeButton\(\s*220\s*,\s*4\s*,\s*["\']Fade["\']/);
```

The handler should call `this.cameras.main.fade(600, 0, 0, 0)`.

```js
assert.match(code, /this\.cameras\.main\.fade\(\s*600\s*,\s*0\s*,\s*0\s*,\s*0\s*\)/);
```

Listen for `"camerafadeoutcomplete"` with `once`, then call `fadeIn(400, 0, 0, 0)`.

```js
assert.match(code, /this\.cameras\.main\.once\(\s*["\']camerafadeoutcomplete["\']\s*,\s*\(\s*\)\s*=>\s*\{\s*this\.cameras\.main\.fadeIn\(\s*400\s*,\s*0\s*,\s*0\s*,\s*0\s*\)/);
```
