...
# --description--
Add a `Flash` button: a 150 ms pure-white flash, perfect for damage-feedback prototyping. Same `reduceMotion` gate as shake.
...
```js
assert.match(code, /this\.makeButton\(\s*166\s*,\s*4\s*,\s*["\']Flash["\']/);
```

The handler should call `this.cameras.main.flash(150, 255, 255, 255)` behind a `reduceMotion` guard.

```js
assert.match(code, /if\s*\(\s*!\s*this\.reduceMotion\s*\)[\s\S]*this\.cameras\.main\.flash\(\s*150\s*,\s*255\s*,\s*255\s*,\s*255\s*\)/);
```
