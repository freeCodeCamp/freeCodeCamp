...
# --description--
Tuning a deadzone by feel is hard. Render a magenta-stroked rectangle that mirrors the deadzone's bounds, anchored to the screen so it stays put as the camera scrolls. Toggle visibility via `applyDeadzone` (already wired in step 10).

After the deadzone toggle:
...
The `applyDeadzone()` call at the end syncs the viz visibility with the current state.

# --hints--
Create the deadzone visualizer at `(240, 160)` sized 200×200 with zero fill alpha.

```js
assert.match(code, /this\.deadzoneViz\s*=\s*this\.add\s*\.rectangle\(\s*240\s*,\s*160\s*,\s*200\s*,\s*200\s*,\s*0x000000\s*,\s*0\s*\)/);
```

Give it a 2px magenta stroke via `.setStrokeStyle(2, 0xff66cc)`.

```js
assert.match(code, /\.setStrokeStyle\(\s*2\s*,\s*0xff66cc\s*\)/);
```

The visualizer should pin to the screen via `setScrollFactor(0)` and start hidden.

```js
assert.match(code, /\.setScrollFactor\(\s*0\s*\)[\s\S]*\.setVisible\(\s*false\s*\)/);
```

Add the visualizer to `this.uiLayer` and call `this.applyDeadzone()` to sync visibility.

```js
assert.match(code, /this\.uiLayer\.add\(\s*this\.deadzoneViz\s*\)\s*;\s*this\.applyDeadzone\(\s*\)/);
```
