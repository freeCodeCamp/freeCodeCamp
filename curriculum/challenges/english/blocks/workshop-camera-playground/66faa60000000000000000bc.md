...
# --description--
The minimap should be a tiny overview, not a full-zoom second view. Set its zoom to `0.06` (each world pixel renders as 0.06 minimap pixels) and tell it to ignore the UI layer and the player rectangle — the player is too small to see at this zoom anyway (step 20 adds a visible marker).

After the minimap init:
...
```js
assert.match(code, /this\.minimap\.setZoom\(\s*0\.06\s*\)/);
```

Call `this.minimap.ignore([this.uiLayer, this.player])`.

```js
assert.match(code, /this\.minimap\.ignore\(\s*\[\s*this\.uiLayer\s*,\s*this\.player\s*\]\s*\)/);
```
