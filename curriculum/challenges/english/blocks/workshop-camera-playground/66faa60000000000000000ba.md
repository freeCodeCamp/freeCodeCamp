...
# --description--
Instant zoom feels jarring. Replace `applyZoom`'s body so it tweens the camera zoom over 400 ms with a soft Sine ease — same result, much smoother feel.

Replace the `this.applyZoom` definition:
...
The `+` and `-` buttons keep working — they call `applyZoom`, and `applyZoom` now tweens instead of snapping.

# --hints--
Replace `applyZoom`'s body to call `this.tweens.add({ targets: this.cameras.main, zoom: target, ... })`.

```js
assert.match(code, /this\.tweens\.add\(\s*\{\s*targets:\s*this\.cameras\.main\s*,\s*zoom:\s*target\s*,/);
```

Use a 400 ms duration and `"Sine.InOut"` ease.

```js
assert.match(code, /duration:\s*400\s*,\s*ease:\s*["\']Sine\.InOut["\']/);
```

`setZoom` should no longer be called inside `applyZoom`.

```js
assert.match(code, /this\.applyZoom\s*=\s*\(\s*target\s*\)\s*=>\s*\{(?:(?!this\.cameras\.main\.setZoom)[\s\S])*?this\.zoomLabel\.setLabel/);
```
