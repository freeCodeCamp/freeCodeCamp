...
# --description--
The world is conceptually 4000×600, but Phaser's physics engine still thinks the world is the default canvas size. Walk far enough right and the player slides off into negative-coordinate territory. Set the world bounds explicitly and ask the player's body to respect them.
...
```js
assert.match(code, /this\.physics\.world\.setBounds\(\s*0\s*,\s*0\s*,\s*4000\s*,\s*600\s*\)/);
```
