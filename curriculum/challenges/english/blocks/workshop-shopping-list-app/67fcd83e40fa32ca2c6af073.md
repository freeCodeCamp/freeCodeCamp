...
`items` should be an array containing the strings `Apples`, `Bananas`, `Strawberries`, `Blueberries`, `Mangoes`, `Pineapple`, `Lettuce`, `Broccoli`, `Paper Towels`, and `Dish Soap`.

```js
const explorer = await __helpers.Explorer(code);
const { items } = explorer.variables;
const expected = [
  "Apples", "Bananas", "Strawberries", "Blueberries", "Mangoes",
  "Pineapple", "Lettuce", "Broccoli", "Paper Towels", "Dish Soap"
];
const itemsArr = eval(items?.value.toString());
assert.isTrue(
  itemsArr?.length === expected.length &&
  itemsArr.every((item) => expected.includes(item))
);
```
