...
You should create a state variable named `query` and a setter function named `setQuery`, and initialize the state with an empty string.

```js
const explorer = await __helpers.Explorer(code);
const { ShoppingList } = explorer.allFunctions;
const declarations = ["const", "let", "var"].map(
  keyword => `${keyword} [query, setQuery] = useState("")`
);
assert.isTrue(
