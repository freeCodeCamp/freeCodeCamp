...
const hookOrders = ["useState, useMemo", "useMemo, useState"];
const declarations = ["const", "let", "var"].flatMap(keyword =>
  hookOrders.map(hooks => `${keyword} { ${hooks} } = React`)
);
assert.isTrue(
  Object.values(explorer.variables).some(variable =>
