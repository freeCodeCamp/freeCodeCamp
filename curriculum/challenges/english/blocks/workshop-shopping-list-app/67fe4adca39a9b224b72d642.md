...
You should create a state variable named `selectedItems` and a setter function named `setSelectedItems`, and initialize the state with an empty array.

```js
const explorer = await __helpers.Explorer(code);
const { ShoppingList } = explorer.allFunctions;
const declarations = ["const", "let", "var"].map(
  keyword => `${keyword} [selectedItems, setSelectedItems] = useState([])`
);
assert.isTrue(
  Object.values(ShoppingList?.variables ?? {}).some(variable =>
    declarations.some(declaration => variable.matches(declaration))
  )
);
```
