...
const names = ['adjective', 'noun', 'noun2', 'place', 'verb', 'adjective2'];
const initialValues = {};
for (const name of names) {
  const raw = explorer.variables[name]?.value?.toString() ?? '';
  initialValues[name] = raw.slice(1, -1);
}

const expected = `Once upon a time, there was a(n) ${initialValues.adjective} ${initialValues.noun} who loved to eat ${initialValues.noun2}. The ${initialValues.noun} lived in a ${initialValues.place} and had ${initialValues.adjective2} nostrils that blew fire when it was ${initialValues.verb}.`;
...
```js
const explorer = await __helpers.Explorer(code);
const storyValue = explorer.variables.firstStory?.value.toString() ?? '';
const order = ['adjective', 'noun', 'noun2', 'noun', 'place', 'adjective2', 'verb'];
let lastIndex = -1;
const inOrder = order.every(name => {
  const idx = storyValue.indexOf(name, lastIndex + 1);
...
const loggedMessages = spy.calls.map(args => args.join(' '));
assert.include(loggedMessages, `First story: ${firstStory}`);
```
...
```js
const explorer = await __helpers.Explorer(code);
const storyValue = explorer.variables.secondStory?.value.toString() ?? '';
const order = ['adjective', 'noun', 'noun2', 'noun', 'place', 'adjective2', 'verb'];
let lastIndex = -1;
const inOrder = order.every(name => {
  const idx = storyValue.indexOf(name, lastIndex + 1);
...
const loggedMessages = spy.calls.map(args => args.join(' '));
assert.include(loggedMessages, `Second story: ${secondStory}`);
```
