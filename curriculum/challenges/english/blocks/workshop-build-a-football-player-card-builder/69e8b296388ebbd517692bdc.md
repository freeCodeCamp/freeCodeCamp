...
Add a `value` attribute of `player.sho` to the SHO `input`. Then set its `onChange` attribute to an arrow function that takes `e` as its parameter and calls `setPlayer` with a spread of `player` and `sho` set to `Number(e.target.value)`.
