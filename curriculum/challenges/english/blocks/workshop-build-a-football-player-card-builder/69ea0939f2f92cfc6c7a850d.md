...
Next, add a `useEffect` hook that runs whenever the `player` state changes. Inside it, use an arrow function and add a `try..catch` with `error` as the parameter of the `catch` block.
