...
Add a `value` attribute of `player.club` to the Club `input`. After that, add an `onChange` attribute set to an arrow function that takes `e` as its parameter and calls `setPlayer` with a spread of `player` and `club` set to `e.target.value`.
