...
After the closing `div` of `card-header` and still inside the `div` with `className` of `card`, add a `div` with the `className` of `card-image-wrap`. Leave it empty for now. You will add the image in the next step.
