...
Add a `value` attribute of `player.imageUrl` to the Image URL `input`. Then set the `onChange` attribute to an arrow function that takes `e` as its parameter and calls `setPlayer` with a spread of `player` and `imageUrl` set to `e.target.value`.
