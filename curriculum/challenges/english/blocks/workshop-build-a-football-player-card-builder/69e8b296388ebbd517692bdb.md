...
Add a `value` attribute of `player.pac` to the PAC `input`. Then add an `onChange` attribute set to an arrow function that takes `e` as its parameter and calls `setPlayer` with a spread of `player` and `pac` set to `Number(e.target.value)`.
