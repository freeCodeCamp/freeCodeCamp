...
The final hardcoded values are the six player stats. Replace each hardcoded number with the corresponding `player` property:
...
You should use `player.pac`, `player.sho`, and `player.pas` as the stat values in the first `.stat-col`.
...
You should use `player.dri`, `player.def`, and `player.phy` as the stat values in the second `.stat-col`.
