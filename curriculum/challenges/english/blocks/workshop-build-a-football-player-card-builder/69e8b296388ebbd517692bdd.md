...
Add a `value` attribute of `player.pas` to the PAS `input`. Then add an `onChange` attribute set to an arrow function that takes `e` as its parameter and calls `setPlayer` with a spread of `player` and `pas` set to `Number(e.target.value)`.
