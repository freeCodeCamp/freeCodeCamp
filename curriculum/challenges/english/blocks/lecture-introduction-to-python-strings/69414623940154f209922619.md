...
Note that concatenation only works with strings. If you try to concatenate a string with a number, you'll get a `TypeError`:
...
The `*` operator repeats a string, it does not join strings. Use `+` or an f-string for concatenation.
