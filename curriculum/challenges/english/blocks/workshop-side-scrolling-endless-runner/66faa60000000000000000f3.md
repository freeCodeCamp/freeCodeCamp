...
# --description--
Replace the basic `Game Over` text in `gameOverDeath` with a richer banner: the headline switches to **`New Best!`** in gold when the player crossed their previous record, and a hint line below tells them how to restart. Add both to `this.hud` so the UI camera renders them on top of the frozen world.

Replace the old banner block inside `gameOverDeath`:
...
Note the `>=` for `newBest` — the persist block already wrote the new best to `this.bestDistance` *before* the banner renders, so equality means "they tied or beat their record on this run."

# --hints--
You should compute `const newBest = Math.floor(this.distance) >= this.bestDistance`.

```js
assert.match(code, /const\s+newBest\s*=\s*Math\.floor\(\s*this\.distance\s*\)\s*>=\s*this\.bestDistance/);
```

The banner text should switch on `newBest`: `newBest ? "New Best!" : "Game Over"`.

```js
assert.match(code, /newBest\s*\?\s*["\']New Best!["\']\s*:\s*["\']Game Over["\']/);
```

You should colour the banner gold (`"#ffd866"`) on a new best, white otherwise.

```js
assert.match(code, /color:\s*newBest\s*\?\s*["\']#ffd866["\']\s*:\s*["\']#ffffff["\']/);
```

You should render the hint `"Press R to restart"` at `(240, 180)`.

```js
assert.match(code, /\.text\(\s*240\s*,\s*180\s*,\s*["\']Press R to restart["\']/);
```

You should add both texts to the HUD container with `this.hud.add([banner, hint])`.

```js
assert.match(code, /this\.hud\.add\(\s*\[\s*banner\s*,\s*hint\s*\]\s*\)/);
```
