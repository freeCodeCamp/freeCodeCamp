...
# --description--
A crash should hit the player physically as well as visually. Add a camera shake to `gameOverDeath` — short and sharp, applied between flipping the flag and stopping the timer.

Inside `gameOverDeath`, after `this.gameOver = true;`:

```js
this.cameras.main.shake(220, 0.012);
```

220 ms at 0.012 intensity reads as a meaty crash without vomit-inducing rattle. Anyone who's reduced-motion sensitive deserves an opt-out — that's step 38.

# --hints--
You should add `this.cameras.main.shake(220, 0.012)` inside `gameOverDeath`.

```js
assert.match(code, /this\.cameras\.main\.shake\(\s*220\s*,\s*0\.012\s*\)/);
```
