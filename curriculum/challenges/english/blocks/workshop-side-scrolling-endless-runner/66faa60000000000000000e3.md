...
# --description--
Time for collectibles. The coin spritesheet already exists from Chapter 2 — 8 frames at 16 × 16 each, in a 128 × 16 strip. Load it next to the cactus image in `preload`:
...
```js
assert.match(code, /this\.load\.spritesheet\(\s*["\']coin["\']\s*,\s*["\']\/curriculum-assets\/phaser\/spritesheets\/coin\.png["\']\s*,\s*\{\s*frameWidth:\s*16\s*,\s*frameHeight:\s*16\s*\}\s*\)/);
```
