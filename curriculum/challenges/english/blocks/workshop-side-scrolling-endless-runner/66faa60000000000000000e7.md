...
# --description--
Birds fly. The flying enemy spritesheet is a 96 × 16 strip — four 24 × 16 frames of wing flapping.
...
```js
assert.match(code, /this\.load\.spritesheet\(\s*["\']flyer["\']\s*,\s*["\']\/curriculum-assets\/phaser\/spritesheets\/flying-enemy\.png["\']\s*,\s*\{\s*frameWidth:\s*24\s*,\s*frameHeight:\s*16\s*\}\s*\)/);
```
