...
# --description--
Right now the world renders twice (main camera draws everything, UI camera draws everything on top). Fix that with a pair of `ignore` calls: tell the **main** camera to skip the HUD container, and tell the **UI** camera to skip every world game object. After this step the HUD only renders via the UI camera, and the world only renders via the main camera — that's the multi-camera UI pattern in five lines.
...
`ignore` accepts a single GameObject, an array of GameObjects, or a Group/Container/Layer — that's why one call covers the whole HUD and another array covers every world object.

# --hints--
You should call `this.cameras.main.ignore(this.hud)` so the main camera skips the HUD container.

```js
assert.match(code, /this\.cameras\.main\.ignore\(\s*this\.hud\s*\)/);
```
...
```js
assert.match(code, /this\.uiCam\.ignore\(\s*\[\s*this\.sky[\s\S]*this\.dust\s*\]\s*\)/);
```
