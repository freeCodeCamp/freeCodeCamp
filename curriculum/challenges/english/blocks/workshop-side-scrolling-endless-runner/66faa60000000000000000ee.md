...
# --description--
Reduced motion is an accessibility setting. Detect it via `window.matchMedia("(prefers-reduced-motion: reduce)")`, store the result on `this.reduceMotion`, and stop the dust emitter if the user opted out. Step 33's shake and step 34's flash will be wrapped in checks now.
...
Guard the shake call you wrote in step 33 to skip when reduced motion is set, and replace the bare `flash` line in the powerup overlap so it only fires when motion is allowed (both edits are pre-populated in the seed).

# --hints--
You should set `this.reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches`.

```js
assert.match(code, /this\.reduceMotion\s*=\s*window\.matchMedia\(\s*\n?\s*["\']\(prefers-reduced-motion:\s*reduce\)["\']\s*\n?\s*\)\.matches/);
```

You should call `this.dust.stop()` when `this.reduceMotion` is true.

```js
assert.match(code, /if\s*\(\s*this\.reduceMotion\s*&&\s*this\.dust\s*\)\s*this\.dust\.stop\(\s*\)/);
```
