...
# --description--
Add the HUD. Pin a `Dist: 0` label to the top-left with `setScrollFactor(0)` and `setDepth(120)` (above the world, below anything coming later).
...
The label exists but stays at `0` until you wire the live update — that's the next step.

# --hints--
You should create `this.distanceText` as `this.add.text(8, 8, "Dist: 0", { ... })` chained with `.setScrollFactor(0)` and `.setDepth(120)`.

```js
assert.match(code, /this\.distanceText\s*=\s*this\.add\s*\.text\(\s*8\s*,\s*8\s*,\s*["\']Dist: 0["\'][\s\S]*\.setScrollFactor\(\s*0\s*\)\s*\.setDepth\(\s*120\s*\)/);
```
