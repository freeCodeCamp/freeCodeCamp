...
# --description--
Spawn flying birds at head height. Touching one ends the run (same as a cactus). Register a 4-frame `"fly"` animation, a spawner that drops a flyer at `(520, 200)` (chest height — too high to jump over, you must duck), a 4500 ms timer, and the overlap callback.
...
```js
assert.match(code, /this\.flyers\s*=\s*this\.physics\.add\.group\(\s*\{\s*allowGravity:\s*false\s*\}\s*\)/);
```

You should register the `"fly"` animation with frames `[0, 1, 2, 3]` and `frameRate: 8`.

```js
assert.match(code, /this\.anims\.create\(\s*\{\s*key:\s*["\']fly["\'][\s\S]*frames:\s*\[\s*0\s*,\s*1\s*,\s*2\s*,\s*3\s*\][\s\S]*frameRate:\s*8/);
```

You should define `this.spawnFlyer` to create a flyer at `(520, 200)` with leftward velocity.

```js
assert.match(code, /this\.spawnFlyer\s*=[\s\S]*this\.flyers\.create\(\s*520\s*,\s*200\s*,\s*["\']flyer["\']\s*,\s*0\s*\)[\s\S]*setVelocityX\(\s*-\s*this\.scrollSpeed\s*\)/);
```

You should create `this.flyerTimer` with `delay: 4500` and `callback: this.spawnFlyer`.

```js
assert.match(code, /this\.flyerTimer\s*=\s*this\.time\.addEvent\(\s*\{\s*delay:\s*4500\s*,\s*loop:\s*true\s*,\s*callback:\s*this\.spawnFlyer/);
```

You should register an overlap with `this.flyers` that calls `this.gameOverDeath()`.

```js
assert.match(code, /this\.physics\.add\.overlap\(\s*this\.player\s*,\s*this\.flyers[\s\S]*this\.gameOverDeath\(\s*\)/);
```
