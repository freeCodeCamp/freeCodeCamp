...
# --description--
The distance score is the heart of the scoring loop. Every frame (while alive) it grows by `scrollSpeed * dt` — pixels of forward travel. `delta` is in milliseconds; divide by 1000.
...
The `if (!this.gameOver)` guard freezes the score when the run ends — no growth post-mortem.

# --hints--
You should accumulate `this.distance += (this.scrollSpeed * delta) / 1000` while `!this.gameOver`.

```js
assert.match(code, /if\s*\(\s*!\s*this\.gameOver\s*\)\s*\{[\s\S]*this\.distance\s*\+=\s*\(\s*this\.scrollSpeed\s*\*\s*delta\s*\)\s*\/\s*1000/);
```
