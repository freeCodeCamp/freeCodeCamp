...
`Math.floor` rounds the floating-point distance to a clean integer. Reload — the counter ticks up as you run. Hitting a cactus freezes the count because the score-update guard already returns early when `gameOver`.

# --hints--
You should call `this.distanceText.setText("Dist: " + Math.floor(this.distance))` in `update`.

```js
assert.match(code, /this\.distanceText\.setText\(\s*["\']Dist: ["\']\s*\+\s*Math\.floor\(\s*this\.distance\s*\)\s*\)/);
```
