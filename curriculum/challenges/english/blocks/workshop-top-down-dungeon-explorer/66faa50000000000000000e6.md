...
# --description--
Read the slime object from the SE room, render it as a green rectangle, give it an arcade body, and collide it with walls so it can't phase through stone.

After the loadGame call in `MainScene.create`:
...
The slime starts at the polyline's origin. Patrol logic comes in the next two steps.

# --hints--
You should read the slime object from the `"objects"` layer.

```js
assert.match(code, /const\s+slimeObj\s*=\s*map\.findObject\(\s*["\']objects["\']\s*,\s*\(\s*\w+\s*\)\s*=>\s*\w+\.type\s*===\s*["\']slime["\']\s*\)/);
```

You should render the slime as a 14×12 green rectangle (`0x88ee66`).

```js
assert.match(code, /this\.slime\s*=\s*this\.add\.rectangle\(\s*slimeObj\.x\s*,\s*slimeObj\.y\s*,\s*14\s*,\s*12\s*,\s*0x88ee66\s*\)/);
```

You should give the slime a dynamic arcade body and lock it to world bounds.

```js
assert.match(code, /this\.physics\.add\.existing\(\s*this\.slime\s*\)[\s\S]*this\.slime\.body\.setCollideWorldBounds\(\s*true\s*\)/);
```

You should collide the slime with the walls layer.

```js
assert.match(code, /this\.physics\.add\.collider\(\s*this\.slime\s*,\s*walls\s*\)/);
```
