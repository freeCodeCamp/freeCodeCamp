...
# --description--
Read every `torch` object and render it as a sprite using frame `25` of the tileset, with a yoyo'd alpha tween for that flickering-fire feel.
...
```js
assert.match(code, /map\.filterObjects\(\s*["\']objects["\']\s*,\s*\(\s*\w+\s*\)\s*=>\s*\w+\.type\s*===\s*["\']torch["\']\s*\)/);
```

Each torch should be a sprite using frame 25 of the `"tiles"` image.

```js
assert.match(code, /this\.add\s*\.sprite\(\s*\w+\.x\s*\+\s*8\s*,\s*\w+\.y\s*\+\s*8\s*,\s*["\']tiles["\']\s*,\s*25\s*\)/);
```

Each torch should yoyo its alpha from `1` to `0.6` indefinitely.

```js
assert.match(code, /alpha:\s*\{\s*from:\s*1\s*,\s*to:\s*0\.6\s*\}[\s\S]*yoyo:\s*true[\s\S]*repeat:\s*-1/);
```
