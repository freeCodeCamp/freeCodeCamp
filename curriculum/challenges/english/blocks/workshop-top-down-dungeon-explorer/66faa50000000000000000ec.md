...
# --description--
Combat. Bind the spacebar, track the last attack timestamp, and add a small white "swing" rectangle that's invisible until an attack fires.
...
```js
assert.match(code, /this\.spaceKey\s*=\s*this\.input\.keyboard\.addKey\(\s*["\']SPACE["\']\s*\)/);
```

You should initialize `this.lastAttackAt = 0`.

```js
assert.match(code, /this\.lastAttackAt\s*=\s*0/);
```

You should add a 16×16 white `this.swing` rectangle, hidden by default.

```js
assert.match(code, /this\.swing\s*=\s*this\.add\s*\.rectangle\(\s*0\s*,\s*0\s*,\s*16\s*,\s*16\s*,\s*0xffffff\s*,\s*0\.6\s*\)[\s\S]*\.setVisible\(\s*false\s*\)/);
```
