...
# --description--
On the UI side, add an HP bar — a graphics rectangle that redraws whenever MainScene emits `"ui:hp"`. Define the redraw as a closure (`this.drawHp`) so it stays in one editable region.

After the notice listener in `UIScene.create`:
...
```js
assert.match(code, /this\.hpBar\s*=\s*this\.add\.graphics\(\s*\)[\s\S]*this\.hpBar\.setScrollFactor\(\s*0\s*\)/);
```

You should assign `this.drawHp` to an arrow function that fills a 60×8 bar at `(8, 8)`.

```js
assert.match(code, /this\.drawHp\s*=\s*\(\s*\w+\s*\)\s*=>\s*\{[\s\S]*?fillRect\(\s*8\s*,\s*8\s*,\s*60\s*,\s*8\s*\)/);
```

`drawHp` should fill the foreground bar proportional to `hp / 3`.

```js
assert.match(code, /fillRect\(\s*8\s*,\s*8\s*,\s*\(\s*hp\s*\/\s*3\s*\)\s*\*\s*60\s*,\s*8\s*\)/);
```

You should subscribe to `"ui:hp"` and call `drawHp` with the new HP.

```js
assert.match(code, /main\.events\.on\(\s*["\']ui:hp["\']\s*,\s*\(\s*hp\s*\)\s*=>\s*this\.drawHp\(\s*hp\s*\)\s*\)/);
```
