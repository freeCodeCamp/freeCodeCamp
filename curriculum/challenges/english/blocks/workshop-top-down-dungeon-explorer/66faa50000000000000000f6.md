...
# --description--
A minimap is just a second camera. Add one at the top-right corner with a low zoom, bound to the map, following the player.
...
```js
assert.match(code, /this\.minimap\s*=\s*this\.cameras\s*\.add\(\s*252\s*,\s*4\s*,\s*60\s*,\s*40\s*\)\s*\.setZoom\(\s*0\.1\s*\)/);
```

You should give the minimap a black background and bind its bounds to the map.

```js
assert.match(code, /setBackgroundColor\(\s*0x000000\s*\)[\s\S]*this\.minimap\.setBounds\(\s*0\s*,\s*0\s*,\s*map\.widthInPixels\s*,\s*map\.heightInPixels\s*\)/);
```
