...
# --description--
Polish: while the camera fades between rooms, freeze the player. Modify the movement block in `MainScene.update` so it returns early when `time < this.transitionLockedUntil`. Without this, you can speed-clip past the fade and out the other side of a room.
...
```js
assert.match(code, /if\s*\(\s*time\s*<\s*this\.transitionLockedUntil\s*\)\s*\{\s*this\.player\.body\.setVelocity\(\s*0\s*\)\s*;\s*return\s*;\s*\}/);
```
