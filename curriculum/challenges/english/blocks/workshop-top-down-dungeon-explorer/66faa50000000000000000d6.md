...
# --description--
Talking to the NPC will fire on `E`. Two pieces of state need to exist before the talk handler can read them: the `E` key binding and a `lastTalkAt` timestamp used later to debounce the press.
...
```js
assert.match(code, /this\.eKey\s*=\s*this\.input\.keyboard\.addKey\(\s*["\']E["\']\s*\)/);
```

You should initialize `this.lastTalkAt` to `0`.

```js
assert.match(code, /this\.lastTalkAt\s*=\s*0/);
```
