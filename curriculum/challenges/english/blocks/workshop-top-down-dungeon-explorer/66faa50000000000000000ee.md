...
# --description--
Read the `goal` object (the stairs up out of the dungeon). Render it as a sprite using frame `31` of the tileset, give it a static body, and on overlap with the player set the `victory` flag, emit `"ui:victory"`, and fade the camera to black. The flag guard ensures the overlap fires exactly once.

After the attack init in `MainScene.create`:
...
```js
assert.match(code, /const\s+goalObj\s*=\s*map\.findObject\(\s*["\']objects["\']\s*,\s*\(\s*\w+\s*\)\s*=>\s*\w+\.type\s*===\s*["\']goal["\']\s*\)/);
```

You should render the goal as a sprite using frame 31 of the `"tiles"` image.

```js
assert.match(code, /this\.goal\s*=\s*this\.add\s*\.sprite\(\s*goalObj\.x\s*\+\s*8\s*,\s*goalObj\.y\s*\+\s*8\s*,\s*["\']tiles["\']\s*,\s*31\s*\)/);
```

You should add an overlap that fires exactly once via the `"victory"` flag.

```js
assert.match(code, /physics\.add\.overlap\(\s*this\.player\s*,\s*this\.goal[\s\S]*?if\s*\(\s*this\.flags\.has\(\s*["\']victory["\']\s*\)\s*\)\s*return/);
```

On victory you should emit `"ui:victory"` and fade the camera with `(800, 0, 0, 0)`.

```js
assert.match(code, /this\.events\.emit\(\s*["\']ui:victory["\']\s*\)[\s\S]*this\.cameras\.main\.fade\(\s*800\s*,\s*0\s*,\s*0\s*,\s*0\s*\)/);
```
