...
# --description--
Add a save point. Read the `save` object and render it as a small yellow square in the SW room.

After the chest block in `MainScene.create`:
...
```js
assert.match(code, /const\s+saveObj\s*=\s*map\.findObject\(\s*["\']objects["\']\s*,\s*\(\s*\w+\s*\)\s*=>\s*\w+\.type\s*===\s*["\']save["\']\s*\)/);
```

You should render the save point as a 12×12 rectangle with colour `0xffff66`.

```js
assert.match(code, /this\.savePoint\s*=\s*this\.add\.rectangle\(\s*saveObj\.x\s*\+\s*8\s*,\s*saveObj\.y\s*\+\s*8\s*,\s*12\s*,\s*12\s*,\s*0xffff66\s*\)/);
```
