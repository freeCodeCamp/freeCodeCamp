...
# --description--
Add an overlap callback between the player and the slime. On contact, respect the i-frame window — if the player took damage in the last 800 ms, ignore the overlap. Otherwise decrement HP, set the next i-frame deadline, emit the new HP to the UI, and shake the camera.
...
```js
assert.match(code, /this\.physics\.add\.overlap\(\s*this\.player\s*,\s*this\.slime\s*,/);
```

The overlap callback should bail when `this.time.now < this.iframesUntil` or the slime is inactive.

```js
assert.match(code, /this\.time\.now\s*<\s*this\.iframesUntil\s*\|\|\s*!\s*this\.slime\.active/);
```

On a successful hit, you should set `this.iframesUntil = this.time.now + 800`.

```js
assert.match(code, /this\.iframesUntil\s*=\s*this\.time\.now\s*\+\s*800/);
```

You should decrement HP using `Math.max(0, this.hp - 1)` and emit `"ui:hp"`.

```js
assert.match(code, /this\.hp\s*=\s*Math\.max\(\s*0\s*,\s*this\.hp\s*-\s*1\s*\)[\s\S]*this\.events\.emit\(\s*[\"']ui:hp[\"']/);
```

You should shake the camera with `cameras.main.shake(150, 0.005)`.

```js
assert.match(code, /this\.cameras\.main\.shake\(\s*150\s*,\s*0\.005\s*\)/);
```
