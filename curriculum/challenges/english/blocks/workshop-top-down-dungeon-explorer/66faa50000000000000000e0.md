...
# --description--
The chest is a sprite from the tileset (frame `27` is the closed chest, frame `28` is open). Initialize the score, read the chest object, and render it.

After the locked-door block in `MainScene.create`:
...
```js
assert.match(code, /const\s+chestObj\s*=\s*map\.findObject\(\s*["\']objects["\']\s*,\s*\(\s*\w+\s*\)\s*=>\s*\w+\.type\s*===\s*["\']chest["\']\s*\)/);
```

You should render the chest as a sprite using frame `27` of the `"tiles"` image.

```js
assert.match(code, /this\.chest\s*=\s*this\.add\s*\.sprite\(\s*chestObj\.x\s*\+\s*8\s*,\s*chestObj\.y\s*\+\s*8\s*,\s*["\']tiles["\']\s*,\s*27\s*\)/);
```

You should track `this.chestOpened = false`.

```js
assert.match(code, /this\.chestOpened\s*=\s*false/);
```
