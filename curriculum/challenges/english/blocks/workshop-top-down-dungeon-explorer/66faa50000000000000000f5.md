...
# --description--
Add a small "Gold" counter in the top-right corner that updates whenever MainScene emits `"ui:score"`.

After the room-label block in `UIScene.create`:
...
```js
assert.match(code, /this\.scoreText\s*=\s*this\.add\s*\.text\(\s*252\s*,\s*50\s*,/);
```

You should listen for `"ui:score"` and prefix the text with `"Gold: "`.

```js
assert.match(code, /main\.events\.on\(\s*["\']ui:score["\']\s*,\s*\(\s*s\s*\)\s*=>\s*this\.scoreText\.setText\(\s*["\']Gold:\s*["\']\s*\+\s*s\s*\)/);
```
