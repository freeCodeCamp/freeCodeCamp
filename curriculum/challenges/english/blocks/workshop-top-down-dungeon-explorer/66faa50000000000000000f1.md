...
# --description--
Set up room-tracking state. The dungeon has four rooms (NW, NE, SW, SE). Track which room the player is in, which rooms they've visited, and a cooldown for room transitions.
...
```js
assert.match(code, /this\.currentRoom\s*=\s*["\']NW["\']/);
```

You should initialize `this.visitedRooms` as a `Set(["NW"])`.

```js
assert.match(code, /this\.visitedRooms\s*=\s*new\s+Set\(\s*\[\s*["\']NW["\']\s*\]\s*\)/);
```

You should initialize `this.transitionLockedUntil = 0`.

```js
assert.match(code, /this\.transitionLockedUntil\s*=\s*0/);
```
