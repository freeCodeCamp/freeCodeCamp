...
# --description--
Before the slime can hurt the player, give the player some HP and a place to track invincibility frames.

After the slime patrol init:
...
```js
assert.match(code, /this\.hp\s*=\s*3[\s\S]*this\.maxHp\s*=\s*3[\s\S]*this\.iframesUntil\s*=\s*0/);
```
