...
# --description--
On the UI side, render a black overlay and a yellow "Victory!" title when MainScene emits `"ui:victory"`.

After the HP block in `UIScene.create`:
...
```js
assert.match(code, /main\.events\.on\(\s*["\']ui:victory["\']/);
```

You should add a full-screen 70%-opacity black rectangle.

```js
assert.match(code, /this\.add\s*\.rectangle\(\s*160\s*,\s*120\s*,\s*320\s*,\s*240\s*,\s*0x000000\s*,\s*0\.7\s*\)/);
```

You should add the text "Victory!" centred at `(160, 120)` in 32px yellow.

```js
assert.match(code, /this\.add\s*\.text\(\s*160\s*,\s*120\s*,\s*["\']Victory!["\']/);
```
