...
# --description--
Throughout the dungeon you'll mark progress in a `flags` Set — `talked-to-npc`, `door-unlocked`, `victory`, etc. Initialize the Set now so every later step can `add()` to it without checking for existence.
...
```js
assert.match(code, /this\.flags\s*=\s*new\s+Set\(\s*\)/);
```
