...
# --description--
When `UIScene` shuts down, unsubscribe from MainScene events. Without this, repeated scene restarts pile up listeners and double-fire dialogue/HP/notice handlers.

At the very end of `UIScene.create` (after the score-text listener):
...
```js
assert.match(code, /this\.events\.on\(\s*["\']shutdown["\']/);
```

You should call `main.events.off` for each of the six event names.

```js
assert.match(code, /main\.events\.off\(\s*["\']ui:dialogue["\']\s*\)[\s\S]*main\.events\.off\(\s*["\']ui:notice["\']\s*\)[\s\S]*main\.events\.off\(\s*["\']ui:hp["\']\s*\)[\s\S]*main\.events\.off\(\s*["\']ui:victory["\']\s*\)[\s\S]*main\.events\.off\(\s*["\']ui:room["\']\s*\)[\s\S]*main\.events\.off\(\s*["\']ui:score["\']\s*\)/);
```
