...
# --description--
Ask the minimap to skip rendering specific entities — the NPC, slime, key, chest, save point, goal, and swing rectangle — so the minimap shows the floor and walls cleanly.
...
```js
assert.match(code, /this\.minimap\.ignore\(\s*\[\s*this\.npc\s*,\s*this\.slime\s*,\s*this\.chest\s*,\s*this\.keyItem\s*,\s*this\.savePoint\s*,\s*this\.goal\s*,\s*this\.swing\s*\]\s*\)/);
```
