...
# --description--
The map's slime object carries a `polyline` array of `{x, y}` waypoints (relative to the object's origin). Convert them to world-space coordinates, set the initial target to the next waypoint, and give the slime two hit points.

After the slime block:
...
```js
assert.match(code, /this\.slimePoly\s*=\s*slimeObj\.polyline\.map\(\s*\(\s*\w+\s*\)\s*=>\s*\(?\s*\{\s*x:\s*slimeObj\.x\s*\+\s*\w+\.x\s*,\s*y:\s*slimeObj\.y\s*\+\s*\w+\.y\s*\}\s*\)?\s*\)/);
```

You should set `this.slimeTarget = 1` (next waypoint) and `this.slimeHp = 2`.

```js
assert.match(code, /this\.slimeTarget\s*=\s*1[\s\S]*this\.slimeHp\s*=\s*2/);
```
