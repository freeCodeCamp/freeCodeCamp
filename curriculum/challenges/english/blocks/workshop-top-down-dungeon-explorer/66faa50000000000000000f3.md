...
# --description--
Each frame, classify the player's room. If it changed and we're past the transition cooldown, fade the camera, mark the new room visited, and emit `"ui:room"` so the UI can announce it.

In `MainScene.update`, after the attack block:
...
```js
assert.match(code, /this\.detectRoom\(\s*this\.player\.x\s*,\s*this\.player\.y\s*\)/);
```

You should fire only when room changed and `time > this.transitionLockedUntil`.

```js
assert.match(code, /room\s*!==\s*this\.currentRoom\s*&&\s*time\s*>\s*this\.transitionLockedUntil/);
```

You should set the cooldown to `time + 600`.

```js
assert.match(code, /this\.transitionLockedUntil\s*=\s*time\s*\+\s*600/);
```

You should fade out for 150 ms then fade back in on `camerafadeoutcomplete`.

```js
assert.match(code, /this\.cameras\.main\.fadeOut\(\s*150\s*,\s*0\s*,\s*0\s*,\s*0\s*\)[\s\S]*this\.cameras\.main\.once\(\s*["\']camerafadeoutcomplete["\']\s*,[\s\S]*this\.cameras\.main\.fadeIn\(\s*150\s*\)/);
```

You should add the new room to `this.visitedRooms` and emit `"ui:room"`.

```js
assert.match(code, /this\.visitedRooms\.add\(\s*room\s*\)[\s\S]*this\.events\.emit\(\s*["\']ui:room["\']\s*,\s*room\s*\)/);
```
