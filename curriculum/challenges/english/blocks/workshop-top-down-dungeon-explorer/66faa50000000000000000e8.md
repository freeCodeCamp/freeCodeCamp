...
# --description--
Each frame, point the slime toward the current waypoint. When it's within 4 pixels, advance to the next waypoint (wrapping around the polyline). Otherwise drive its velocity along the unit vector at speed 30.
...
```js
assert.match(code, /this\.slime\.active\s*&&\s*this\.slimeHp\s*>\s*0/);
```

You should compute `dx`, `dy`, and `d = Math.hypot(dx, dy)`.

```js
assert.match(code, /const\s+dx\s*=\s*target\.x\s*-\s*this\.slime\.x[\s\S]*const\s+dy\s*=\s*target\.y\s*-\s*this\.slime\.y[\s\S]*const\s+d\s*=\s*Math\.hypot\(\s*dx\s*,\s*dy\s*\)/);
```

When within 4 pixels, you should advance `this.slimeTarget` by `(idx + 1) % length`.

```js
assert.match(code, /this\.slimeTarget\s*=\s*\(\s*this\.slimeTarget\s*\+\s*1\s*\)\s*%\s*this\.slimePoly\.length/);
```

Otherwise you should drive the slime's body velocity along `(dx/d, dy/d)` scaled by speed `30`.

```js
assert.match(code, /this\.slime\.body\.setVelocity\(\s*\(\s*dx\s*\/\s*d\s*\)\s*\*\s*sp\s*,\s*\(\s*dy\s*\/\s*d\s*\)\s*\*\s*sp\s*\)/);
```
