...
An element refers to both the opening and closing (if required) tags and everything in between. An element includes all three of these parts:

`<element attribute="value">`  
`    inner text`  
`</element>`

After the `card-container` element which contains the two `card` class elements:

1. Add a new `p` element with this text:

    `Review your selections and continue to checkout.`

2. Below the `p` element, create a `div` element with the `class` attribute set to `btn-container`.

   This container will group your navigation button elements.
