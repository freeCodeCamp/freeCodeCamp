...
The `rawData` array contains shipment strings separated by the `|` character. Each string follows the format `sku|name|qty|expires|zone`, where the `zone` value is optional. For example:
