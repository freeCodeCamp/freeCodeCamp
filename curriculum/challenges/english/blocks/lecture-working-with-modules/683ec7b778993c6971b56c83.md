...
A module groups related code, such as functions, classes, and constants, so you can reuse it in other Python files.

To use a module in your script, you use an import statement. Import statements are generally written at the top of the file. Start with the import keyword followed by the module's name:
...
It prevents other modules from being imported in the same script.

### --feedback--

Think about what happens when an imported name matches a name already used in your script.

---

It can overwrite names already defined in the importing script.

---

It requires the module's name before every imported function call.
...
It makes imported functions available only inside the current function.
