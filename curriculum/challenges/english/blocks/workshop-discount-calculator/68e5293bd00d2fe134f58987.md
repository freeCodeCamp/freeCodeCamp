...
Give it an `__init__` method that takes `self` and `amount` as parameters. Give `amount` parameter a type hint of `float` and a return type hint of `None` for the method. Inside the `__init__` method, store `amount` as an instance attribute.
