...
To complete the workshop, add the final print statement to display the best price with 2 decimal places using an f-string and this format: `Best price for [product.name] for [user_tier] user: $[best_price.xx]` where `product_name`, `user_tier`, and `best_price` should be replaced with the actual variable values.
