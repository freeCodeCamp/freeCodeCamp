...
Give it an `__init__` method that takes `self` and `percent` as parameters. Give a type hint of `float` to `percent` and set the return type of the method to `None`. Inside the `__init__` method, store `percent` as an instance attribute.
