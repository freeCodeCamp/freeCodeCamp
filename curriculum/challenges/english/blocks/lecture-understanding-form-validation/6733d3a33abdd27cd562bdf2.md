...
Every event that triggers in the DOM has some sort of default behavior. The `click` event on a checkbox toggles the state of that checkbox, by default. Pressing the <kbd>Spacebar</kbd> on a focused button activates the button. The `preventDefault()` method on these event objects stops that behavior from happening.
...
`e.key` gives you the value of the key pressed, such as `a` for the <kbd>A</kbd> key or `Enter` for the <kbd>Enter</kbd> key.
