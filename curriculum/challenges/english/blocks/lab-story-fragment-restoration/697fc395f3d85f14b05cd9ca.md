...
You should have a `compactFragments` function.

```js
assert.isFunction(compactFragments);
```

`compactFragments([{ id: 1, text: "Hello" }, undefined])` should log a message that starts with `[COMPACTED]`.
...
`compactFragments([{ id: 1, text: "Hello" },,])` should log a message that starts with `[COMPACTED]`.
...
`dedupeFragments([{ id: 1, text: "a" }, { id: 1, text: "dup" }, { id: 2, text: "b" }, { id: 2, text: "dup" }, { id: 3, text: "c" }])` should log a `[DEDUPED]` message for each duplicated `id`.
...
`dedupeFragments([{ id: 1, text: "a" }, { id: 2, text: "b" }, { id: 2, text: "dup" }])` should log a `[DEDUPED]` message when the last `id` is duplicated.
...
`fillMissingFragments([{ id: 1, text: "a" }, { id: 3, text: "c" }])` should log a message that starts with `[FILLED]`.
...
`assembleStory([{ id: 1, text: "Hello" }, { id: 2, text: "World" }])` should return a string containing each fragment's text in order, separated by newlines.
