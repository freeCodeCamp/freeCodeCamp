...
When the `compactFragments` function is called with the array `[{ id: 1, text: "Hello" }, undefined]`, it should log a console messages that starts with `[COMPACTED]`.

```js
const shuffledFragments__withUndefined = [{ id: 1, text: "Hello" }, undefined];
const spy = __helpers.spyOn(console, 'log');
compactFragments(shuffledFragments__withUndefined);
const consoleLogCalls = structuredClone(spy.calls);
assert.isTrue(spy.calls.some(call => call[0].match(/^\[COMPACTED\]/)));
spy.restore();
console.log(consoleLogCalls);
```
