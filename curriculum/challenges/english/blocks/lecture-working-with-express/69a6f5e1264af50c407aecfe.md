...
The `(req, res) => { ... }` part is a callback function when a request matches this route. The `req` object contains information about the incoming request, and the `res` object is used to send a response back to the client.
