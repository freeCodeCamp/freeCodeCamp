...
```js
assert.match(code, /<html\b[^>]*\blang\s*=\s*('|")en\1[^>]*>/i);
```

Your `html` element should have a closing tag. Closing tags have a `/` just after the `<` character.
...
```js
assert.match(code, /<html\b[^>]*>[\s\S]*<\s*head\b[^>]*>/i);
```
...
```js
assert.match(code, /<\s*head\b[^>]*>[\s\S]*<\s*title\b[^>]*>[\s\S]*<\/\s*head\s*>/i);
```
...
```js
assert.exists(document.head?.querySelector('meta[charset]'));
```
...
assert.strictEqual(meta?.getAttribute('charset')?.trim().toLowerCase(), 'utf-8');
```

You should have a `body` element within your `html` element.

```js
assert.match(code, /<\s*head\b[^>]*>[\s\S]*<\/\s*head\s*>[\s\S]*<\s*body\b[^>]*>/i);
```

Your `body` element should have a closing tag. Closing tags have a `/` just after the `<` character.

```js
assert.match(code, /<\/\s*body\s*>/i);
```

You should have only one `h1` element with the name of your recipe.
