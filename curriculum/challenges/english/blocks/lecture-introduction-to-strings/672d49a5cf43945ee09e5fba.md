...
Here is an example of trying to change the first character of a string before assigning a new string to the `developer` variable:
...
Assigning `"M"` to `developer[0]` does not change the string, so the first `console.log()` still prints `"Jessica"`. To change the value, a new string must be assigned to the `developer` variable. The second `console.log()` then prints `"Quincy"`.
