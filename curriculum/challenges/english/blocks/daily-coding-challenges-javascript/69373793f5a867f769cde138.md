...
- `"X wins"` if both players have three in a row, since player X moves first.
...
`ticTacToe([["X", "O", "X"], ["X", "O", "X"], ["O", "O", "X"]])` should return `"X wins"`.

```js
assert.equal(ticTacToe([["X", "O", "X"], ["X", "O", "X"], ["O", "O", "X"]]), "X wins");
```
