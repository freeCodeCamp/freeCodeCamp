...
```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(\s*(function\s*\(\s*acc\s*,\s*el\s*\)|\(\s*acc\s*,\s*el\s*\)\s*=>)/);
```
...
assert.match(
  ...
  /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(\s*(?:function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*return\s+acc\s*\+\s*el\s*;?\s*\}|\(\s*acc\s*,\s*el\s*\)\s*=>\s*(?:\{\s*return\s+acc\s*\+\s*el\s*;?\s*\}|acc\s*\+\s*el))/
);
```
