...
```js
assert.match(getVariance.toString(), /squaredDifferences\s*=\s*differences\.map\(\s*(?:function\s*\(\s*el\s*\)|(?:\(\s*el\s*\)|el)\s*=>)/);
```
...
```js
assert.match(getVariance.toString(), /squaredDifferences\s*=\s*differences\.map\(\s*(?:function\s*\(\s*el\s*\)\s*\{\s*return\s+|(?:\(\s*el\s*\)|el)\s*=>\s*)(?:Math\.pow\(\s*el\s*,\s*2\s*\)|el\s*\*\*\s*2)/);
```
