...
```js
assert.match(getVariance.toString(), /differences\s*=\s*array\.map\(\s*(?:function\s*\(\s*el\s*\)|(?:\(\s*el\s*\)|el)\s*=>)/);
```
...
```js
assert.match(getVariance.toString(), /differences\s*=\s*array\.map\(\s*(?:function\s*\(\s*el\s*\)\s*\{\s*return\s+|(?:\(\s*el\s*\)|el)\s*=>\s*)el\s*-\s*mean/);
```
