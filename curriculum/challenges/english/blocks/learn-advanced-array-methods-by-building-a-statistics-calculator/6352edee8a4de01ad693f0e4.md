...
```js
assert.match(getMode.toString(), /mode\s*=\s*Object\.keys\(\s*counts\s*\)\.filter\(\s*(?:function\s*\(\s*el\s*\)|(?:\(\s*el\s*\)|el)\s*=>)/);
```

Your `.filter()` method should return whether the value of `counts[el]` is equal to `counts[highest]`.

```js
assert.match(getMode.toString(), /mode\s*=\s*Object\.keys\(\s*counts\s*\)\.filter\([\s\S]*counts\s*\[\s*el\s*\]\s*===\s*counts\s*\[\s*highest\s*\]/);
```
