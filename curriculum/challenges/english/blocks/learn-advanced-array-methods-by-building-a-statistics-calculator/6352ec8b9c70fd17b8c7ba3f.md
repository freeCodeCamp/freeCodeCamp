...
```js
assert.match(getMode.toString(), /highest\s*=\s*Object\.keys\(\s*counts\s*\)\.sort\(\s*(?:function\s*\(\s*a\s*,\s*b\s*\)|\(\s*a\s*,\s*b\s*\)\s*=>)/)
```
...
```js
assert.match(getMode.toString(), /highest\s*=\s*Object\.keys\(\s*counts\s*\)\.sort\(\s*(?:function\s*\(\s*a\s*,\s*b\s*\)\s*\{\s*return\s+|\(\s*a\s*,\s*b\s*\)\s*=>\s*)counts\s*\[\s*b\s*\]\s*-\s*counts\s*\[\s*a\s*\]/)
```
