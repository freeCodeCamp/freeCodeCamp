...
```js
globalThis.PET_NAME = 'Fitzgerald';
globalThis.clock = __FakeTimers.install();
...
//  otherwise, null.
globalThis.deepMatchText = (elem, regex)  => {
  let nodes = [elem];
...
globalThis.findFirstStatFor = (targetStat) => {
  const statRegEx = {
    'hunger': /.*hunger.*/i,
    'energy': /.*energy.*/i,
  ...
  for (const statEl of getStatEls()) {
    const statTextEl = deepMatchText(statEl, statRegEx[targetStat.toLowerCase()]);
    if (statTextEl) return statEl;
  }
}

globalThis.getStatEls = () => {
  return document.querySelectorAll('.stat');
}

// Real time wait for fn to evaluate to true
globalThis.waitFor = async (fn, { timeout = 100, interval = 50} = {}) => {
...
globalThis.testEatButton = () => {
  const hungerStatEl = findFirstStatFor('hunger');
  ...
  const energyStatEl = findFirstStatFor('energy');
...
globalThis.testPlayButton = () => {
  const energyStatEl = findFirstStatFor('energy');
  const energyStatValueEl = energyStatEl.querySelector('.stat-value');
  const originalEnergyValue = parseFloat(energyStatValueEl.textContent);

  const happinessStatEl = findFirstStatFor('happiness');
...
```js
delete globalThis.PET_NAME;
globalThis.clock.uninstall();
delete globalThis.clock;

delete globalThis.deepMatchText;
delete globalThis.findFirstStatFor;
delete globalThis.getStatEls;
delete globalThis.waitFor;
delete globalThis.submitForm;
delete globalThis.testEatButton;
delete globalThis.testPlayButton;
delete globalThis.testSleepButton;
```
...
```js
const petNameEl = document.querySelector('.pet-name');
assert.strictEqual(petNameEl.textContent, PET_NAME);
```
...
let matches = 0;
for (const statEl of statEls) {
  if (deepMatchText(statEl, /.*hunger.*/i)) matches++;
}
...
let matches = 0;
for (const statEl of statEls) {
  if (deepMatchText(statEl, /.*energy.*/i)) matches++;
}
assert.equal(matches, 1);
```
...
let matches = 0;
for (const statEl of statEls) {
  if (deepMatchText(statEl, /.*happiness.*/i)) matches++;
}
assert.equal(matches, 1);
```
...
for (let i = 0; i < 100; i++) {
  await clock.tickAsync(10 * 1_1000); 
}
