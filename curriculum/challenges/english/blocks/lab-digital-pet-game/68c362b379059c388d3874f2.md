...
- 1. The pet should have a number of images or emoji tied to each `Mood`.
...
//  children, until the regex is matches or the tree is
...
const eatButton = document.querySelector('button#sleep-action');
eatButton.click();
clock.tick(50);
...
for (let i = 0; i < 100; i++) {
  clock.tick(10 * 1_1000); 
}

const hungerValue = parseFloat(hungerStatValueEl.textContent);
console.log(`Hunger Value: ${hungerStatValueEl.textContent}`);
