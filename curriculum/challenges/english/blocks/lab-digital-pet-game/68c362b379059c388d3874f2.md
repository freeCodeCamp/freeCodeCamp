...
- 1. This view will disappear on submission.

1. The second view:

- 1. This view will appear upon submission of the first view.
- 1. Should be contained within a `<main>` block.
- 1. Should contain three `<button>` elements.
- 1. Should track three properties of the pet. These are: `['Hunger', 'Happiness', 'Energy']`.
- 1. The three `<button>` elements should pertain to the following three actions. `[EAT, PLAY, SLEEP]`.
...
- 1. There should be three visual components that display the value of the three pet properties to the user in some form.
- 1. Each `Action` available to the pet should be encompassed by an enumerator.
- 1. The pet should have the following moods, encompassed by an enumerator: `['Happy', 'Excited', 'Content', 'Sad', 'Tired', 'Sick', 'Hungry']`.
...
- 1. The pet should have a number of images or emoji tied to each `Mood`.
  ...
  - These emoji or images should be tied to the current `Mood`, with a `Record`.


# --hints--

This element should exist. Maybe fail.

```js
const gameContainer = document.querySelector(".base-container.game-container");
assert.exists(gameContainer);
```
