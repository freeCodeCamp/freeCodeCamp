...
You can also pass a variable as the `count` value in the `repeat()` method.
...
console.log(word.repeat(times));  // "Hi Hi Hi "
```

:::

In this case, the `repeat()` method uses the value stored in the `times` variable to determine how many times the string should be repeated. This makes your code more flexible and reusable, especially when the number of repetitions can change.
