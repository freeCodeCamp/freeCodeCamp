...
let times = 4;
let symbol = "*";
let repeatedSymbols = symbol.repeat(times);
console.log(repeatedSymbols); // "****"
```

:::

In this example, the `times` variable stores the number of repetitions. This can be useful when the number of repetitions depends on user input or other dynamic values in your program.
