...
- Subtraction is only used when you would otherwise have to write a symbol more than three times in a row.
- `M` is the only symbol allowed appear more than three times in a row.
...
`to_roman(5678)` should return `"MMMMMDCLXXVIII"`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(to_roman(5678), "MMMMMDCLXXVIII")`)
}})
```
