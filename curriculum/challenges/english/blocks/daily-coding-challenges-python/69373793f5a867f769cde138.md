...
`tic_tac_toe([["X", "O", "X"], ["X", "O", "O"], ["O", "O", "X"]])` should return `"O wins"`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(tic_tac_toe([["X", "O", "X"], ["X", "O", "O"], ["O", "O", "X"]]), "O wins")`)
}})
```
...
`tic_tac_toe([["X", "X", "O"], ["X", "O", "X"], ["O", "X", "O"]])` should return `"O wins"`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(tic_tac_toe([["X", "X", "O"], ["X", "O", "X"], ["O", "X", "O"]]), "O wins")`)
}})
```
