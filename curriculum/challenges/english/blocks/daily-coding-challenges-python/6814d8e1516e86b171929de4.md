...
`is_balanced("😀ab")` should return `True`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertIs(is_balanced("😀ab"), True)`)
}})
```

`is_balanced("a")` should return `True`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertIs(is_balanced("a"), True)`)
}})
```

`is_balanced("E")` should return `True`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertIs(is_balanced("E"), True)`)
}})
```
...
def is_balanced(s):
    ...
    first_half = s[:half]
    second_half = s[half:] if n % 2 == 0 else s[half + 1:]
