...
`get_landing_stance("Goofy", 180)` should return `"Regular"`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_landing_stance("Goofy", 180), "Regular")`)
}})
```

`get_landing_stance("Goofy", 360)` should return `"Goofy"`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_landing_stance("Goofy", 360), "Goofy")`)
}})
```
