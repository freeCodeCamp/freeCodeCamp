...
`analyze_ideas([["Dark mode", 1, 3, 8], ["Real-time collaboration feature", 3, 8, 20], ["Add tooltip", 1, 2, 4]])` should return `["Add tooltip", "Dark mode", "Real-time collaboration feature"]`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(analyze_ideas([["Dark mode", 1, 3, 8], ["Real-time collaboration feature", 3, 8, 20], ["Add tooltip", 1, 2, 4]]), ["Add tooltip", "Dark mode", "Real-time collaboration feature"])`)
}})
```
...
def analyze_ideas(ideas):
    def expected(idea):
        ...
        return ((optimistic + 4 * realistic + pessimistic) / 6) * (len(name) / 20)
