...
1. You should declare an empty array named `inventory` to store product objects.
Each product object must always have:
● a key `name` (value is always stored in lowercase)
● a key `quantity` (value is an integer).
Whenever you add, find, or remove a product by name, always first convert the input name to lowercase using .toLowerCase(), to ensure comparisons are case-insensitive.
1. You should create a function named `findProductIndex` that takes the product name as its argument and returns the index of the corresponding product object inside the `inventory` array. All operations (`findProductIndex`, `addProduct`, `removeProduct`) must always use the lowercase version of product names for searching and matching. Convert all input names to lowercase before any comparison or action. If the product is not found, the function should return `-1`.
1. You should create a function named `addProduct` that takes a product object as its argument.
1. If the product is already present in the inventory, the `addProduct` function should add the new `quantity` to the existing `quantity` for that product (do not replace the quantity). Then, log this message to the console:
...
// inventory is now: [{ name: 'flour', quantity: 10 }]
`addProduct({ name: 'Flour', quantity: 5 });`
// inventory is now: [{ name: 'flour', quantity: 15 }]
// logs: "flour quantity updated"
`addProduct({ name: 'Salt', quantity: 2 });`
// inventory is now: [{ name: 'flour', quantity: 15 }, { name: 'salt', quantity: 2 }]
// logs: "salt added to inventory"
`removeProduct('FLOUR', 5);`
// inventory is now: [{ name: 'flour', quantity: 10 }, { name: 'salt', quantity: 2 }]
// logs: "Remaining flour pieces: 10"
`removeProduct('flour', 10);`
// inventory is now: [{ name: 'salt', quantity: 2 }]
// logs: "Remaining flour pieces: 0" (then 'flour' is removed completely)
`removeProduct('Sugar', 1);`
// logs: "sugar not found"
`removeProduct('SALT', 10);`
// logs: "Not enough salt available, remaining pieces: 2"
