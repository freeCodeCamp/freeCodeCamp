...
You should loop through each gate in the `gates` array after logging to the console, and call `handleGateAtTick()` with `gates`, `gate`, `tickIndex`, and `throughputSummary`.
...
You should increment `tickIndex` by 1 after your loop through the `gates` array.
