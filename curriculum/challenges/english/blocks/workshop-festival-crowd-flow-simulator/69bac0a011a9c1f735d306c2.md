...
Your `handleGateAtTick` function should call the `processGateFlow()` function.
...
const { result } = handleGateAtTick?.variables ?? {};
assert.isTrue(result?.value.matches("processGateFlow(gate, tickIndex)"));
```
