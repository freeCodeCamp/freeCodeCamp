...
Your `handleGateAtTick` function should first log the message `"Overflow of " + result.overflow + " attendees. Rerouting..."` when overflow exists (`result.overflow > 0`).
...
try {
  ...
  const overflowCall = spy.calls.find((call) =>
    String(call[0]).includes("Overflow of")
  );
  assert.exists(
    overflowCall,
  ...
  assert.deepEqual(
    ...
    "The West gate at tick 0 has an overflow of 2, so the message should be 'Overflow of 2 attendees. Rerouting...'"
  );
} finally {
  spy.restore();
}
...
Your `handleGateAtTick` function should reroute the overflow by calling `rerouteOverflow()` with `gates`, `gate`, `tickIndex`, and `result.overflow` when overflow exists (`result.overflow > 0`).
