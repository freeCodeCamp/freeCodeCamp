...
Finally, run your simulations for both datasets to see the festival flow in action!

- Call `simulateFestival()` first with `(morningGates, "Morning")`.
- Then call `simulateFestival()` with `(nightGates, "Night")`.
