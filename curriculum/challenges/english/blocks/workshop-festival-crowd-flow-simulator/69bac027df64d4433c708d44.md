...
const { nextGateIndex } = rerouteOverflow?.variables ?? {};
const validExpressions = [
  "(currentIndex + 1) % gates.length",
  "(1 + currentIndex) % gates.length"
];
assert.isTrue(
  validExpressions.some((expression) => nextGateIndex?.value.matches(expression))
);
```
