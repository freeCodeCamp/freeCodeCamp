...
Your `initializeThroughput` function should return `{}` when called with `[]`, without mutating the source input.
...
Your `initializeThroughput` function should return `{ North: 0 }` when called with `[{ id: "North", capacity: 5, queue: [1, 2, 3] }]`, without mutating the source input.
...
Your `initializeThroughput` function should return `{ North: 0, East: 0, South: 0 }` when called with `[{ id: "North", capacity: 5, queue: [] }, { id: "East", capacity: 3, queue: [] }, { id: "South", capacity: 4, queue: [] }]`, without mutating the source input.
...
Your `initializeThroughput` function should return `summary` after the loop, as the final line in the function.
