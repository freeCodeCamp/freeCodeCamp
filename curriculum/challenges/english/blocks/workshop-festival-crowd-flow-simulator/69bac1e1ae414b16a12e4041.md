...
Finally, you can run your simulations. Begin with the morning shift. Call `simulateFestival()` with `morningGates` and `"Morning"`.

# --hints--

You should call `simulateFestival()` with `morningGates` and `"Morning"`. Then, call `simulateFestival()` with `nightGates` and `"Night"`.
...
assert(
  endsCorrectly,
  "At the end of your script, you must call `simulateFestival()` with `morningGates` and Morning."
);
```
