...
Now that the `name` setter handles validation, update `__init__` to call it instead of assigning directly to `self._name`. Change `self._name = name` to `self.name = name`.

Also, since `name` validation is now handled by the setter, adjust the `if` condition to only check `level`, and update the `TypeError` message accordingly:
