...
```js
const expected = document.querySelectorAll('span[class~="two"] > span:nth-child(-n+2)');
const rules = [];
...
const validRule = rules.find(r => {
  if (!r.selectorText || !r.selectorText.includes('[class~="two"]')) return false;
  const elements = document.querySelectorAll(r.selectorText);
  if (elements.length !== expected.length) return false;
  return [...elements].every(el => [...expected].includes(el));
});

assert.exists(validRule);
```
...
assert.include(validRule?.style?.backgroundImage, 'linear-gradient(');
```
