...
Create a new selector for `input[type="checkbox"]`. Add the `:checked` pseudo-class followed by the `::after` pseudo-element.

* The `:checked` pseudo-class lets you target the checkbox when it is checked.
* The `::after` pseudo-element lets you insert styling or content (such as a checkmark) inside the checkbox when that state is active.
