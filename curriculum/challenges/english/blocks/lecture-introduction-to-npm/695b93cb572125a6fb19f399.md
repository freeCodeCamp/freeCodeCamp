...
The `license` field is used to specify the legal terms under which other people can use, modify, and distribute the code in your package. There are many different licenses like `MIT`, `ISC`, or `GPL`, which all have different terms and permissions. The default is `ICS`, the Internet Systems Consortium License, which is a very permissive open source license.
...
It's usually best to use `npm init` without the `-y` or `--yes` flag so you can go through each field and customize your project's information. If you don't andswer a question, npm simply uses a default value. For example, if you leave the license question empty, npm will set it to `ISC` by default.

Here's and example of what you'll see when running `npm init`:
