...
- **Synthetic Event System**: This is React's way of handling events. It serves as a wrapper around native events like the `click`, `keydown`, and `submit` events. Event handlers in React use camelCase naming convention. (Ex. `onClick`, `onSubmit`, etc)
