...
```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".lightbox")?.width, ["100%","100vw"]);
assert.equal(new __helpers.CSSHelp(document).getStyle(".lightbox")?.height, ["100%","100vh"]);
```
