...
Now put `normalizeDirectory` to use. Call it with `rawData` and store the result in a variable 
called `normalizedData`.

Log `normalizedData` to the console, then check it against the original rawData:

Pick one department — does it appear in departmentsById with the correct id and name, 
and does its programIds list the right program IDs?
Pick one program — does it appear in programsById with the correct 
departmentId pointing back to its department?
Pick one instructor — does it appear in instructorsById with the correct programId and departmentId, 
and if it has an email, does that email show up in instructorsByEmail pointing to the right 
instructor id?
