...
assert.match(
  __helpers.removeJSComments(code),
  /(?:const|let)\s+departments\s*=\s*Object\s*\.\s*keys\s*\(\s*normalizedData\.departmentsById\s*\)\s*\.\s*length/
);
```
...
assert.match(
  __helpers.removeJSComments(code),
  /(?:const|let)\s+programs\s*=\s*Object\s*\.\s*keys\s*\(\s*normalizedData\.programsById\s*\)\s*\.\s*length/
);
```
...
assert.match(
  __helpers.removeJSComments(code),
  /(?:const|let)\s+instructors\s*=\s*Object\s*\.\s*keys\s*\(\s*normalizedData\.instructorsById\s*\)\s*\.\s*length/
);
```
