...
const fn = explorer.functions.listRoomsByBuilding;

const fnSource = fn.toString();

assert.match(
  __helpers.removeJSComments(fnSource),
  /const\s+program\s*=/
);
```
...
assert.match(
  ...
  /const\s+program\s*=\s*programValues\s*\[\s*i\s*\]/
);
```
...
assert.match(
  __helpers.removeJSComments(code),
  /const\s+department\s*=\s*normalizedData\s*\.\s*departmentsById\s*\[\s*program\.departmentId\s*\]/
);
```
