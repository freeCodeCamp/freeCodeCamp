...
Inside the loop, declare a variable called `program` set to `programValues[i]`, and another called `department` set to `normalizedData.departmentsById[program.departmentId]`. This gives you access to the department's code for comparison.

# --hints--

You should declare a variable called `program`.

```js
assert.match(code, /const\s+program\s*=/);
```

`program` should be set to `programValues[i]`.

```js
assert.match(code, /const\s+program\s*=\s*programValues\s*\[\s*i\s*\]/);
```

You should declare a variable called `department`.

```js
assert.match(code, /const\s+department\s*=/);
```

`department` should be set to `normalizedData.departmentsById[program.departmentId]`.

```js
assert.match(code, /const\s+department\s*=\s*normalizedData\s*\.\s*departmentsById\s*\[\s*program\.departmentId\s*\]/);
```
