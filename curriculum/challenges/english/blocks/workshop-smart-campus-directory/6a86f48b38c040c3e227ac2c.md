...
To find the number of instructors are missing office hours, you will iterate through the `normalizedData.instructorsById` 
and count where `officeHours == null`.

Declare a variable called `instructorValues` set to `Object.values(normalizedData.instructorsById)`.

Then declare a variable called `missingOfficeHours` set to 0 — you'll use this to keep a running count.
