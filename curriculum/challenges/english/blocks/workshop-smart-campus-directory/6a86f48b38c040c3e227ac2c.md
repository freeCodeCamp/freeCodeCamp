...
To find the number of instructors who are missing office hours, you will iterate through the values in `normalizedData.instructorsById` and count those for which `officeHours == null`.

Declare a variable called `instructorValues` set to `Object.values(normalizedData.instructorsById)`.

Then declare a variable called `missingOfficeHours` and initialize it to `0`. You'll use it to keep a running count.

# --hints--

You should declare a variable called `instructorValues`.

```js
assert.match(code, /const\s+instructorValues\s*=/);
```

`instructorValues` should be set to `Object.values(normalizedData.instructorsById)`.

```js
assert.match(code, /const\s+instructorValues\s*=\s*Object\s*\.\s*values\s*\(\s*normalizedData\.instructorsById\s*\)/);
```

You should declare a variable called `missingOfficeHours`.

```js
assert.match(code, /let\s+missingOfficeHours\s*=/);
```

`missingOfficeHours` should be set to `0`.

```js
assert.match(code, /let\s+missingOfficeHours\s*=\s*0/);
```
