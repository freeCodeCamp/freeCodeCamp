...
assert.match(
  __helpers.removeJSComments(code),
  /const\s+instructorValues\s*=\s*Object\s*\.\s*values\s*\(\s*normalizedData\.instructorsById\s*\)/
);
```

You should use `let` to declare a variable called `missingOfficeHours`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /let\s+missingOfficeHours\s*=/
);
```
...
assert.match(
  __helpers.removeJSComments(code),
  /let\s+missingOfficeHours\s*=\s*0/
);
```
