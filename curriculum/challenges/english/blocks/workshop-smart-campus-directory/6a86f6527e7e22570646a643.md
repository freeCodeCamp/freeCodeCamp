...
Your `for` loop should run while `i` is less than `instructorValues.length`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*instructorValues\.length\s*;/
);
```
...
assert.match(
  __helpers.removeJSComments(code),
  /for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*instructorValues\.length\s*;\s*(?:i\+\+|\+\+i)\s*\)/
);
```
