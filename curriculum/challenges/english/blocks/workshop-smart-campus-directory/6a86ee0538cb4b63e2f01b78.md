...
Test your function with two calls to `listRoomsByBuilding`: one using argument `"CSE"` and `normalizedData`, 
then another using argument `"BBA"` and `normalizedData`.  

Log both results to confirm the function returns the correct sorted list of rooms for each building.

# --hints--

You should call `listRoomsByBuilding` with `"CSE"` and `normalizedData` as arguments.

```js
assert.match(code, /listRoomsByBuilding\s*\(\s*["']CSE["']\s*,\s*normalizedData\s*\)/);
```

You should call `console.log()` with the result of that call as the argument.

```js
assert.match(code, /console\s*\.\s*log\s*\(\s*listRoomsByBuilding\s*\(\s*["']CSE["']\s*,\s*normalizedData\s*\)\s*\)/);
```

You should call `listRoomsByBuilding` with `"BBA"` and `normalizedData` as arguments.

```js
assert.match(code, /listRoomsByBuilding\s*\(\s*["']BBA["']\s*,\s*normalizedData\s*\)/);
```

You should call `console.log()` with the result of that call as the argument.

```js
assert.match(code, /console\s*\.\s*log\s*\(\s*listRoomsByBuilding\s*\(\s*["']BBA["']\s*,\s*normalizedData\s*\)\s*\)/);
```
