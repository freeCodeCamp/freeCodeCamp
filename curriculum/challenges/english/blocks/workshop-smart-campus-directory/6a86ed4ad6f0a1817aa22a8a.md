...
After the loop finishes, call the `sort()` method on `rooms` and return the sorted array.
Rooms are stored as strings, and the default `sort()` method sorts alphabetically, which works correctly here.

# --hints--

You should call `.sort()` on `rooms`.

```js
assert.match(code, /rooms\s*\.\s*sort\s*\(\s*\)/);
```

You should use a `return` statement.

```js
assert.match(code, /return\s+rooms/);
```
