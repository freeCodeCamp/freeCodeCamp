...
Call `renderDirectorySummary` with `normalizedData` as its argument, and place that call inside `console.log()` to log the returned summary.

You've now built a complete normalization pipeline: turning deeply nested campus data into fast, 
ID-based lookup tables, and using them to implement `getInstructorByEmail`, `listRoomsByBuilding`, and `renderDirectorySummary`.

This pattern of flattening nested data into lookup tables keyed by `ID` is used constantly in real-world applications, 
from state management libraries to database query results.

And with that, you've completed the workshop!
...
You should call `renderDirectorySummary` with `normalizedData` as its argument.

```js
assert.match(code, /renderDirectorySummary\s*\(\s*normalizedData\s*\)/);
```

This `renderDirectorySummary` call should be inside the `console.log()`.
