...
Here, you should build one more function that creates a summary report of the whole directory. 

Start by deleting the logs for the `listRoomsByBuilding` function calls.

Then, declare a function called `renderDirectorySummary` that takes one parameter named `normalizedData`.
Leave the body empty for now.

# --hints--

You should remove the previous `console.log()` call for `listRoomsByBuilding("CSE", normalizedData)`.

```js
assert.notMatch(
  __helpers.removeJSComments(code),
  /console\s*\.\s*log\s*\(\s*listRoomsByBuilding\s*\(\s*["']CSE["']\s*,\s*normalizedData\s*\)\s*\)/
);
```

You should remove the previous `console.log()` call for `listRoomsByBuilding("BBA", normalizedData)`.

```js
assert.notMatch(
  __helpers.removeJSComments(code),
  /console\s*\.\s*log\s*\(\s*listRoomsByBuilding\s*\(\s*["']BBA["']\s*,\s*normalizedData\s*\)\s*\)/
);
```

You should declare a function named `renderDirectorySummary`.

```js
assert.isFunction(renderDirectorySummary);
```

`renderDirectorySummary` function should take one parameter.

```js
assert.strictEqual(renderDirectorySummary.length, 1);
```
...
console.log( listRoomsByBuilding("CSE", normalizedData));
console.log( listRoomsByBuilding("BBA", normalizedData));
