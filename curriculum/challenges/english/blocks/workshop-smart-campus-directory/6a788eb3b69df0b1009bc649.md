...
You should have a `if` clause that returns the string `"Instructor not found"` when `instructorId` is `Falsy`.

```js
assert.strictEqual(getInstructorByEmail("unknown@edu.com", normalizedData), "Instructor not found");
```
