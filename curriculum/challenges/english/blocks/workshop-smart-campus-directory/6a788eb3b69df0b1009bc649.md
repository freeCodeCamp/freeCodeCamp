...
Before adding the guard clause, delete `console.log(instructorId)` from the previous step.

Add an `if (!instructorId)` check that returns the string `Instructor not found`.

# --hints--

You should delete `console.log(instructorId);`.

```js
assert.notMatch(
  __helpers.removeJSComments(code),
  /console\s*\.\s*log\s*\(\s*instructorId\s*\)/
);
```
...
assert.match(
  __helpers.removeJSComments(code),
  /if\s*\(\s*!\s*instructorId\s*\)\s*{\s*return\s*["']Instructor not found["']/
);
```
