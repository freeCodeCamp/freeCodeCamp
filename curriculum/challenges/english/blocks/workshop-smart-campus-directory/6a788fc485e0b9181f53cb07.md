...
You should declare a variable called `instructor`.

```js
assert.match(code, /const\s+instructor\s*=/);
```

`instructor` should be set to `normalizedData.instructorsById[instructorId]`.

```js
assert.match(code, /const\s+instructor\s*=\s*normalizedData\s*\.\s*instructorsById\s*\[\s*instructorId\s*\]/);
```
