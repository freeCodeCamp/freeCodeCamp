...
Now, to find the rooms that belong to the requested building, write a `for` loop over `programValues`, 
from `i = 0` up to `programValues.length`.
...
assert.match(
  ...
  /for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*programValues\.length\s*;/
);
```
