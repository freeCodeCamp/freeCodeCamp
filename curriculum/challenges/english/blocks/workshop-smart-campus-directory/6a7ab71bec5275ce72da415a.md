...
Before declaring the variable, delete `console.log(instructor)` from the previous step.
...
assert.match(
  __helpers.removeJSComments(code),
  /const\s+department\s*=/
);
```

`department` should be set to `normalizedData.departmentsById[instructor.departmentId]`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /const\s+department\s*=\s*normalizedData\s*\.\s*departmentsById\s*\[\s*instructor\.departmentId\s*\]/
);
```

You should call `console.log()` with `department` as the argument.
