...
```js
assert.match(code, /const\s+department\s*=\s*normalizedData\s*\.\s*departmentsById\s*\[\s*instructor\.departmentId\s*\]/);
```

You should call `console.log()` with `department` as the argument.

```js
assert.match(code, /console\s*\.\s*log\s*\(\s*department\s*\)/);
```
