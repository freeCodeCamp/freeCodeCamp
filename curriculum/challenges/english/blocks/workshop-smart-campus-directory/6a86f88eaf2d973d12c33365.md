...
```js
assert.match(code, /let\s+programsWithoutRooms\s*=\s*0\s*;\s*for\s*\(\s*let\s+i\s*=\s*0\s*;/);
```

Your `for` loop should run while `i` is less than `programValues.length`.

```js
assert.match(code, /let\s+programsWithoutRooms\s*=\s*0\s*;\s*for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*programValues\.length\s*;/);
```
