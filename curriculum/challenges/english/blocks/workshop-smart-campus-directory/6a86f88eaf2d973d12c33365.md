...
Write a `for` loop over `programValues`, from `i = 0` up to `programValues.length`. 
Leave the loop body empty for now.

# --hints--

You should use a `for` loop after declaring `programsWithoutRooms`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /let\s+programsWithoutRooms\s*=\s*0\s*;\s*for\s*\(\s*let\s+i\s*=\s*0\s*;/
);
```

Your `for` loop should run while `i` is less than `programValues.length`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /let\s+programsWithoutRooms\s*=\s*0\s*;\s*for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*programValues\.length\s*;/
);
```

Your `for` loop should increment `i` using `i++` or `++i`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /let\s+programsWithoutRooms\s*=\s*0\s*;\s*for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*programValues\.length\s*;\s*(?:i\+\+|\+\+i)\s*\)/
);
```
