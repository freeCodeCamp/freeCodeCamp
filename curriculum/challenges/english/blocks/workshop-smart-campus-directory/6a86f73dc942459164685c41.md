...
Then declare a variable called `programsWithoutRooms` and initialize it to `0`. You'll use it to keep a running count.
...
const fn = explorer.functions.renderDirectorySummary;
...
You should use `let` to declare a variable called `programsWithoutRooms`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /let\s+programsWithoutRooms\s*=/
);
```
...
assert.match(
  __helpers.removeJSComments(code),
  /let\s+programsWithoutRooms\s*=\s*0/
);
```
