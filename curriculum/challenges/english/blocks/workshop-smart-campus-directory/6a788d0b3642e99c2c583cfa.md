...
You should declare a variable called `instructorId`.

```js
assert.match(code, /const\s+instructorId\s*=/);
```

`instructorId` should be set to `normalizedData.instructorsByEmail[email]`.

```js
assert.match(code, /const\s+instructorId\s*=\s*normalizedData\s*\.\s*instructorsByEmail\s*\[\s*email\s*\]/);
```

You should call `console.log()` with `instructorId` as the argument.

```js
assert.match(code, /console\s*\.\s*log\s*\(\s*instructorId\s*\)/);
```
