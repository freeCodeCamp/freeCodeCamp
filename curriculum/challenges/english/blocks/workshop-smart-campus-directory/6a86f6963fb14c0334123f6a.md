...
Inside the loop, check if `instructorValues[i].officeHours == null`. If true, increment `missingOfficeHours` by 1.
...
Your `if` statement should check `instructorValues[i].officeHours == null`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /if\s*\(\s*instructorValues\s*\[\s*i\s*\]\s*\.\s*officeHours\s*==\s*null\s*\)/
);
```

Inside the `if` block, you should increment `missingOfficeHours`.

```js
assert.match(
  __helpers.removeJSComments(code),
  /if\s*\(\s*instructorValues\s*\[\s*i\s*\]\s*\.\s*officeHours\s*==\s*null\s*\)\s*{\s*missingOfficeHours\s*(\+\+|\+=\s*1)/
);
```
