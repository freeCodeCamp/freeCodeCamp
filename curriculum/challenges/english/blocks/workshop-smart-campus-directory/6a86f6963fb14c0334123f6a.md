...
```js
assert.match(code, /if\s*\(\s*instructorValues\s*\[\s*i\s*\]\s*\.\s*officeHours\s*==\s*null\s*\)/);
```

Inside the `if` block, you should increment `missingOfficeHours`.

```js
assert.match(code, /if\s*\(\s*instructorValues\s*\[\s*i\s*\]\s*\.\s*officeHours\s*==\s*null\s*\)\s*{\s*missingOfficeHours\s*(\+\+|\+=\s*1)/);
```
