...
Declare a variable called `rooms` and initialize it to an empty array.

# --hints--

You should declare a variable called `rooms`.

```js
assert.match(code, /const\s+rooms\s*=/);
```

`rooms` should be set to an empty array.

```js
assert.match(code, /const\s+rooms\s*=\s*\[\s*\]/);
```
