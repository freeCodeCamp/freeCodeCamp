...
Congratulations on successfully completing the Python Functions and Scope workshop!
...
({
  test: () => assert(
    runPython(`_Node(_code).find_calls("check_kitchen_stock")[1].is_equivalent("check_kitchen_stock()")`)
  )
})
```
