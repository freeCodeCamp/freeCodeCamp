...
In CSS, when using the `@property` rule with a custom property like `--padding`, what happens if a user tries to assign an invalid value (for example, a color instead of a length)?
...
The browser falls back to the property's initial value (e.g., 10px), preserving type safety
