...
4. You should apply a non-white background color to the `body` element.
...
```
Your `body` element should have a non-white background color.
...
assert.notOneOf(bodyBg, whiteColors);
```
...
assert.oneOf(backgroundColor, whiteColors);
```
