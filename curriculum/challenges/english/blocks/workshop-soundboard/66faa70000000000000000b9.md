...
1. **`prefers-reduced-motion`**: some users set this OS-level preference not just to suppress visual motion but also to soften unexpected ambient audio. Default the music to `0` volume when the media query matches.

2. **Cleanup**: when the scene shuts down, stop every active sound and kill any in-flight tweens.
...
Toggle the OS preference and reload the preview - the music starts silent, but the slider still works as an override.

# --hints--
Read `prefersReduced` from `window.matchMedia("(prefers-reduced-motion: reduce)").matches`.

```js
assert.match(code, /window\.matchMedia\(\s*["\']\(prefers-reduced-motion: reduce\)["\']\s*\)\.matches/);
```

Guard with `typeof window !== "undefined"` and `typeof window.matchMedia === "function"`.

```js
assert.match(code, /typeof\s+window\s*!==\s*["\']undefined["\']\s*&&\s*typeof\s+window\.matchMedia\s*===\s*["\']function["\']/);
```

When `prefersReduced`, call `this.music.setVolume(0)`.

```js
assert.match(code, /if\s*\(\s*prefersReduced\s*\)\s*\{\s*this\.music\.setVolume\(\s*0\s*\)/);
```

Hook the `"shutdown"` event and call `this.sound.stopAll()` + `this.tweens.killAll()`.

```js
assert.match(code, /this\.events\.on\(\s*["\']shutdown["\']\s*,\s*\(\s*\)\s*=>\s*\{\s*this\.sound\.stopAll\(\s*\)[\s\S]*this\.tweens\.killAll\(\s*\)/);
```
