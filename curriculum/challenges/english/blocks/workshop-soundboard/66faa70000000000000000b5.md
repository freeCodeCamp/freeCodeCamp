...
# --description--
Add a stereo pan slider that fires the `sfx-ping` SFX with `setPan` set from the slider value. Pan is `-1..+1` (full left to full right), so we map the slider's `0..1` to that range.

First define `this.playPing` so we can also reuse it in step 16. Then add the pan label and slider:
...
Click far-left on the slider - the ping plays only in your left ear. Far-right, only on the right.

# --hints--
Initialise `this.pingPan = 0`.

```js
assert.match(code, /this\.pingPan\s*=\s*0/);
```

Define `this.playPing` to add a `"sfx-ping"` instance, call `setPan(this.pingPan)`, play it, and destroy it on `"complete"`.

```js
assert.match(code, /this\.playPing\s*=\s*\(\s*\)\s*=>\s*\{[\s\S]*this\.sound\.add\(\s*[\"']sfx-ping[\"'][\s\S]*ping\.setPan\(\s*this\.pingPan\s*\)[\s\S]*ping\.once\(\s*[\"']complete[\"']\s*,\s*\(\s*\)\s*=>\s*ping\.destroy/);
```

Add a `"Pan"` slider at `(80, 182, 300, 16, 0.5, ...)` whose handler maps `(v - 0.5) * 2` to `this.pingPan` and calls `this.playPing()`.

```js
assert.match(code, /this\.makeSlider\(\s*80\s*,\s*182\s*,\s*300\s*,\s*16\s*,\s*0\.5[\s\S]*this\.pingPan\s*=\s*\(\s*v\s*-\s*0\.5\s*\)\s*\*\s*2[\s\S]*this\.playPing\(\s*\)/);
```
