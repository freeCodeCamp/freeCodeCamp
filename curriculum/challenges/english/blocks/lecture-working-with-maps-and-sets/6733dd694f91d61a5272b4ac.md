...
The `set()`, `get()`, `has()`, and `delete()` methods are all available for use with a `WeakMap` as well. For example, here's how you can assign items to the `WeakMap` with the `set()` method:
