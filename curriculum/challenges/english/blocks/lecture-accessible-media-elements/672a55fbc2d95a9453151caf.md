...
Because browser, operating system, and assistive technology support is inconsistent, `accesskey` should be used with caution. Shortcuts can conflict with existing keyboard commands, may not map well across international keyboards, and can be hard for users to discover unless you clearly document them.

For most general-purpose websites and web applications, `accesskey` is often discouraged. If you do use it, provide clear instructions and test with keyboard and assistive technology users.

If you want to explore this topic further, review the [MDN accessibility concerns for `accesskey`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/accesskey#accessibility_concerns) and [WebAIM guidance on keyboard shortcuts and `accesskey`](https://webaim.org/techniques/keyboard/accesskey).
