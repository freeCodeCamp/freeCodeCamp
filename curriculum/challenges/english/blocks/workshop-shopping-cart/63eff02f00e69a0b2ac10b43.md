...
assert.exists(addItemMethod?.variables?.currentProductCount);
```
...
const cartClass = explorer.classes.ShoppingCart;
const addItemMethod = cartClass?.methods?.addItem;
const currentProductCountVariable = addItemMethod?.variables?.currentProductCount;
assert.isTrue(currentProductCountVariable?.toString().startsWith('const'));
```
