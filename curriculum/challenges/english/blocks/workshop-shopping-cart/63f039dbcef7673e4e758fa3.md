...
const clearCartMethod = cartClass?.methods.clearCart;
const isCartClearedVariable = clearCartMethod?.variables.isCartCleared;
assert.isTrue(isCartClearedVariable?.toString().startsWith('const'));
```
