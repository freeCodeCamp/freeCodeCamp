...
const calculateTotalMethod = cartClass?.methods.calculateTotal;
const subTotalVariable = calculateTotalMethod?.variables.subTotal;
assert.isTrue(subTotalVariable?.toString().startsWith('const'));
```
