...
const { cartBtn } = explorer.variables;
assert.isTrue(cartBtn?.toString().startsWith('const'));
```
...
```js
const explorer = await __helpers.Explorer(code);
const { clearCartBtn } = explorer.variables;
assert.isTrue(clearCartBtn?.toString().startsWith('const'));
```
