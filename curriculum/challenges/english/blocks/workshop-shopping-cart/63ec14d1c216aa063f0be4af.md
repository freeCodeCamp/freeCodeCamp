...
const { cartContainer } = explorer.variables;
assert.isTrue(cartContainer?.toString().startsWith('const'));
```
...
```js
const explorer = await __helpers.Explorer(code);
const { productsContainer } = explorer.variables;
assert.isTrue(productsContainer?.toString().startsWith('const'));
```
...
```js
const explorer = await __helpers.Explorer(code);
const { dessertCards } = explorer.variables;
assert.isTrue(dessertCards?.toString().startsWith('const'));
```
