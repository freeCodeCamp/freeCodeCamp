...
We'll explore four common approaches: the `Object.hasOwn()` method, the `hasOwnProperty()` method, the `in` operator, and checking against `undefined`.

Let's start with `Object.hasOwn()`. This is a static method that is the recommended modern way to check if an object has a specific property as its own property. It returns a boolean value. Here's an example:
...
In this example, we have an object called `person` with two properties: `name` and `age`. To check if `name` is a property in the `person` object, we use `Object.hasOwn()`. Since `name` is a property, it will return `true`. But when we check if `job` is a property, it will return `false` because it does not exist in the object.

`Object.hasOwn()` is recommended over `hasOwnProperty()` because it works for objects created with `Object.create(null)` (which don't have a prototype) and with objects that have overridden the inherited `hasOwnProperty()` method.

Another approach is the `hasOwnProperty()` method. Like `Object.hasOwn()`, this method returns a boolean indicating whether the object has the specified property as its own property. Here's an example:
...
In this example, `hasOwnProperty()` works the same way as `Object.hasOwn()`. However, since it is a method on the object's prototype, it can be unreliable in certain edge cases, which is why `Object.hasOwn()` is preferred.
