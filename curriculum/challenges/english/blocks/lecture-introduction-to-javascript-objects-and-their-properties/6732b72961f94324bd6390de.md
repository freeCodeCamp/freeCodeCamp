...
You can also use `Object.hasOwn()` inside a loop to check multiple required fields at once. This is a very common real-world pattern for validating data:
...
const requiredFields = ["name", "grade", "age", "email"];
...
In this example, we loop over a list of required fields and use `Object.hasOwn()` to check each one. This is a practical way to validate that an object has all the data you need before processing it.
