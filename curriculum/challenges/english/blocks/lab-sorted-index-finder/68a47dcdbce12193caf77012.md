...
sortSpy.restore();
assert.isAtLeast(sortSpy.calls.length, 1);
```
...
```js
const findIndexSpy = __helpers.spyOn(Array.prototype, 'findIndex');
getIndexToIns([10, 20, 30, 40, 50], 35);
findIndexSpy.restore();
assert.isAtLeast(findIndexSpy.calls.length, 1);
```
