...
const complaintTextAreaContainer = document.getElementById("complaint-description-container");
const solutions = document.querySelectorAll('input[name="solutions"]');
const solutionDescription = document.getElementById("solution-description");
const solutionTextAreaContainer = document.getElementById("solution-description-container");
const submitBtn = document.getElementById("submit-btn");
const messageBox = document.getElementById("message-box");
const clearBtn = document.getElementById("clear-btn");

const hideTextareas = () => {
    complaintTextAreaContainer.style.display = "none";
    solutionTextAreaContainer.style.display = "none";
}
...
const clearForm = () => {
    Object.values(formFields).forEach(field => {
        ...
        field.style.borderColor = "rgb(118, 118, 118)";
    });
    hideTextareas();
...
const validateForm = () => {
    ...
    if (complaintsArr[3]) {
        complaintTextAreaContainer.style.display = "block";
        const complaintsDescriptionVal = complaintDescription.value.trim();
        validationObject["complaint-description"] = complaintsDescriptionVal.length >= 20;
    } else {
        complaintTextAreaContainer.style.display = "none";
    }
    if (solutionsArr[2]) {
        solutionTextAreaContainer.style.display = "block";
        const solutionDescriptionVal = solutionDescription.value.trim();
        validationObject["solution-description"] = solutionDescriptionVal.length >= 20;
    } else {
        solutionTextAreaContainer.style.display = "none";
    }
...
const setBorderColor = (fieldKey, isValid) => {
...
Object.keys(formFields).forEach(key => {
    ...
    field.addEventListener(eventType, (e) => {
        ...
        let targetKey = key;
        if (e.target.name === 'complaint' || e.target.name === 'solutions') {
            targetKey = e.target.name + 's-group';
        }
        
        if (validationObject.hasOwnProperty(targetKey)) {
            setBorderColor(targetKey, validationObject[targetKey]);
        }
    });
});
...
form.addEventListener("submit", (e) => {
    ...
    if (!isValid(validationObject)) {
        ...
        Object.keys(validationObject).forEach(key => {
            setBorderColor(key, validationObject[key]);
        });
    } else {
        messageBox.innerText = "";
