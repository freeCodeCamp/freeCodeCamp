...
You should call `updateTaskContainer()` inside an `if` statement that checks `taskData.length`.

```js
assert.match(code, /if\s*\(\s*taskData\.length\s*\)\s*\{\s*updateTaskContainer\(\s*\)/)
```
