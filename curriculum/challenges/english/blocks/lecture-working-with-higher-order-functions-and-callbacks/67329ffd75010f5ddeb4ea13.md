...
In JavaScript, a callback function is a function that is passed as an argument to another function, so that it can be called during the outer function's execution to complete some kind of task.
...
To execute code during the execution of another operation.
