...
In JavaScript, a callback function is a function that is passed as an argument to another function, so that it can be called any time during outer function's execution to complete some kind of task. It' a way of handing over one function to another so it can run your code when it needs to.
...
To execute code during execution of another operation .
