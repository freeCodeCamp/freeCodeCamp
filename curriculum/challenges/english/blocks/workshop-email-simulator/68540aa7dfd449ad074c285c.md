...
- Have Ramy read the first email (using index 0).
- Have Ramy delete the first email (index 0).
...
You should call `ramy.read_email(0)` to read the first email.

```js
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().has_stmt("ramy.read_email(0)")`))
    }
})

```

You should call `ramy.delete_email(0)` to delete the first email.

```js
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().has_stmt("ramy.delete_email(0)")`))
    }
})
...
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().is_ordered("ramy.delete_email(0)", "ramy.check_inbox()")`))
    }
})
