...
Call `buildSummary(101)` inside `console.log()` to log the returned summary.

Congratulations! You have successfully completed the Artifact Provenance Auditor workshop.

# --hints--

You should call `buildSummary` with `101` and log the value it returns.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*buildSummary\s*\(\s*101\s*\)\s*\)|(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*buildSummary\s*\(\s*101\s*\)[\s\S]*console\.log\s*\(\s*\1\s*\)/);
```
