...
Create a function named `buildSummary` with a parameter `id`. Look up the artifact by `id` with `collection.find()`. If no artifact is found, return the string `"Artifact not found"`.
