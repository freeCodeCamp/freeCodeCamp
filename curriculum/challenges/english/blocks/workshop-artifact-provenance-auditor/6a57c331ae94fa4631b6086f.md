...
```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*collection\s*\[\s*101\s*\]\s*\.\s*curator\s*\.\s*name\s*\)[\s\S]*console\.log\s*\(\s*collection\s*\[\s*101\s*\]\s*\.\s*curator\s*\.\s*name\s*\)/);
```
