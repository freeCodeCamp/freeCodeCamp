...
```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*collection\s*\[\s*102\s*\]\s*\.\s*onDisplay\s*\)[\s\S]*toggleDisplayStatus\s*\(\s*102\s*\)[\s\S]*console\.log\s*\(\s*collection\s*\[\s*102\s*\]\s*\.\s*onDisplay\s*\)/);
```
