...
Declare a `currentLocation` variable and assign it `artifact.locations[artifact.locations.length - 1]`. This expression uses the array's `length` property to access its last element.
