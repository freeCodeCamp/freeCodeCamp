...
Create a function named `updateCurator` with parameters `id` and `name`. Look up the artifact by `id` with `collection.find()`. If the artifact exists, set the `name` of its `curator` object to the given `name`.
...
assert.strictEqual(collection[1].curator.name, "Test Curator");
```
