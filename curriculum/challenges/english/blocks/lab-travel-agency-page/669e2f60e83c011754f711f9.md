...
```js
assert(
  Array.from(document.querySelectorAll('head meta[charset]')).some(
    (meta) => meta.getAttribute('charset')?.toLowerCase() === 'utf-8'
  )
);
```

Your `head` element should have a closing tag.

```js
assert.match(code, /<\/head\s*>/i);
```

Your `title` element should have a closing tag.
...
assert.match(
  code,
  /<\s*head\s*>(?:(?!<\/head\s*>)[\s\S])*<\s*title\s*>/i
);
```
...
Your `body` element should have a closing tag.
