...
A word character is defined as any letter, from `a` to `z`, or from 'A' to 'Z', or a number from `0` to `9`, or the underscore (`_`) character.
...
The lesson defines what constitutes a "word character" in regular expressions.
...
Think about how the lesson describes creating ranges within square brackets.
