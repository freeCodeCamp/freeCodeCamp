...
This creates the same type of regular expression object, but it accepts the pattern as a string. The `RegExp` constructor is useful when the pattern comes from a variable or user input instead of being known ahead of time.

Because the pattern is just a string, it could just as easily come from a form input, a checkbox, or any other value only known while the program is running. That's something a regex literal can't do, since its pattern is fixed the moment the code is written.
...
Why would you use `new RegExp(pattern)` instead of a regex literal like `/pattern/`?
...
The constructor lets the pattern be a value determined at runtime, such as from a variable or user input.

---

Regex literals cannot be created any other way.

### --feedback--

The lesson shows two ways to create a regular expression: a literal and the `RegExp` constructor.

---

The constructor is required any time you use a regular expression.

### --feedback--

The lesson's earlier examples show a regex literal working fine on its own, without the constructor.
