...
You can also create a regular expression with the `RegExp` constructor:

```js
const regex = new RegExp("freeCodeCamp");
```

This creates the same type of regular expression object, but it accepts the pattern and flags as strings. The `RegExp` constructor is useful when the pattern or flags come from variables or user input instead of being known ahead of time.

The `RegExp` constructor also accepts a second argument for flags, passed as a string:

```js
const regex = new RegExp("freeCodeCamp", "gi");
```

This is equivalent to writing the literal `/freeCodeCamp/gi`. The real advantage of the constructor is that both the pattern and the flags can be variables, rather than fixed text known ahead of time:
...
Because `pattern` and `flags` are just strings, they could just as easily come from a form input, a checkbox, or any other value only known while the program is running. That's something a regex literal can't do, since its pattern and flags are fixed the moment the code is written.
...
Why would you use `new RegExp(pattern, flags)` instead of a regex literal like `/pattern/flags`?

## --answers--

They produce different types of objects.

### --feedback--

The lesson states the constructor creates the same type of regular expression object as a literal.

---

The constructor lets the pattern and flags be values determined at runtime, such as from variables or user input.

---

Regex literals cannot use flags at all.

### --feedback--

The lesson's earlier examples show flags used directly on a literal, such as `/freeCodeCamp/gi`.

---

The constructor is required any time you use the `g` flag.

### --feedback--

The `g` flag works the same whether you use a literal or the constructor.
