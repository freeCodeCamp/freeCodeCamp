...
1. You should give the `#legend-gradient` element a linear gradient that transitions between all the colors from `--color0` to `--color5`. Each color value should have two color stops (expressed as percentages) to make the transition between colors a hard line. You can use either shorthand syntax (e.g., `var(--color0) 0% 16%`) or expanded syntax (e.g., `var(--color0) 0%, var(--color0) 16%`).
...
You should use two color-stops (expressed in percentage) to make the transition from one color to the following color a hard line for your `#legend-gradient`. You can use either shorthand (e.g., `var(--color0) 0% 16%`) or expanded syntax (e.g., `var(--color0) 0%, var(--color0) 16%`). Remember to use your `--color#` variables.
...
if (shorthandMatches && shorthandMatches.length === EXPECTED_SHORTHAND_STOPS) {
  assert.lengthOf(shorthandMatches, EXPECTED_SHORTHAND_STOPS, 'gradient should have 6 color stops in shorthand format');
} else if (expandedMatches && expandedMatches.length === EXPECTED_EXPANDED_STOPS) {
  assert.lengthOf(expandedMatches, EXPECTED_EXPANDED_STOPS, 'gradient should have 12 color stops in expanded format');
}
```
