...
const colorMatches = [...gradientStr.matchAll(colorPattern)];
const colorNumbers = colorMatches.map(match => parseInt(match[1]));

let expectedColor = 0;
for (const colorNum of colorNumbers) {
  if (colorNum === expectedColor + 1) {
    expectedColor++;
  } else if (colorNum !== expectedColor) {
    assert.fail(`Colors should appear in sequential order (0-5). Expected --color${expectedColor} or --color${expectedColor + 1}, but found --color${colorNum}`);
  }
}

assert.equal(expectedColor, COLOR_COUNT - 1, `gradient should use all colors up to --color${COLOR_COUNT - 1}`);
```
