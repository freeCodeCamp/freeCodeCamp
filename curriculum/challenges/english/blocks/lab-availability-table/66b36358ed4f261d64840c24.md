...
1. Your table should have a `caption` element that describes the table's purpose.
1. Your column header cells (days of the week) should have `scope="col"` attributes.
1. Your row header cells (time values) should have `scope="rowgroup"` attributes.
1. All data cells should have `aria-label` attributes that describe the number of available people.
...
```js
assert.isNotNull(document.querySelector('table caption'));
```

Your `caption` element should not be empty.

```js
assert.isNotEmpty(document.querySelector('table caption')?.textContent.trim());
```

Your column header cells should have `scope="col"` attributes.

```js
const colHeaders = document.querySelectorAll('tr:first-child th:not(.time)');
assert.isNotEmpty(colHeaders);
colHeaders.forEach(header => {
    assert.strictEqual(header.getAttribute('scope'), 'col');
});
```

Your row header cells with the `time` class should have `scope="rowgroup"` attributes.

```js
const rowHeaders = document.querySelectorAll('th.time');
assert.isNotEmpty(rowHeaders);
rowHeaders.forEach(header => {
    assert.strictEqual(header.getAttribute('scope'), 'rowgroup');
});
```

All your data cells should have `aria-label` attributes.

```js
const dataCells = document.querySelectorAll('td.available-0, td.available-1, td.available-2, td.available-3, td.available-4, td.available-5');
assert.isNotEmpty(dataCells);
dataCells.forEach(cell => {
    assert.isNotEmpty(cell.getAttribute('aria-label'));
});
```
...
<body>
    <main>
        <table>
            <caption>Weekly availability schedule showing the number of available people for each time slot and day</caption>
            <tbody>
                <tr class="sharp">
                ...
                <tr class="half">
                    <td class="available-0" aria-label="0 people available"></td>
                    <td class="available-1" aria-label="1 person available"></td>
                    <td class="available-2" aria-label="2 people available"></td>
                    <td class="available-2" aria-label="2 people available"></td>
                    <td class="available-5" aria-label="5 people available"></td>
                </tr>
                <tr class="sharp">
                    <th rowspan="2" class="time" scope="rowgroup">10 AM</th>
                    <td class="available-3" aria-label="3 people available"></td>
                    <td class="available-4" aria-label="4 people available"></td>
                    <td class="available-4" aria-label="4 people available"></td>
                    <td class="available-5" aria-label="5 people available"></td>
                    <td class="available-5" aria-label="5 people available"></td>
                </tr>
