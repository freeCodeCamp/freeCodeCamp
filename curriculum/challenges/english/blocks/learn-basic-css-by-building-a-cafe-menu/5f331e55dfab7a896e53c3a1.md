...
```js
assert.exists(document.querySelector('meta'));
```
...
```js
assert.exists(document.querySelector('meta[charset]'));
```

Your `charset` attribute should have a value of `utf-8`.

```js
assert.equal(document.querySelector('meta')?.getAttribute('charset')?.toLowerCase(), 'utf-8');
```
