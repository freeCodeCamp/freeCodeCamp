...
2. You should use generics and a non-null assertion operator to capture the `cardButtonsContainer`.  
3. You should declare a class called `FlashCardController` using the `class` keyword. 
...
You should use a generic when using `document.querySelector` to capture `cardsButtonContainer`
...
You should use the `non-null` assertion operator while capturing `cardsButtonContainer`
...
`FlashCardController` was declared using the `class` keyword.

```js
assert.match(code, /class\s+FlashCardController/);
```
