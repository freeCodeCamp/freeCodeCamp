...
4) When the `#flashcard` element is clicked, the card should have the `flipped` class.  
5) You should have an element with an id of `delete-btn`. 
6) When the `#delete-btn` element is clicked, it should remove the current flash card information. 
7) You should create an entry form with an id of `entry-form` to be able to add more flashcard elements to a collection.
8) You should create call an `InvalidUserInputError` when either the question text or question answer is empty in the entry form.
