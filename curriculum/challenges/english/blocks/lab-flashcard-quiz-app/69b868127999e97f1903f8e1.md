...
4) You should have a collection of `FlashCard` elements called `cards`. 
...
8) You should create an entry form with an id of `entry-form` to be able to add more flashcards to the `cards` collection on submit.
9) The two elements inside of the form should have an id of `front-text` and `back-text` respectively. 
10) You should create call an `InvalidUserInputError` when either the question text or question answer is empty in the entry form.
...
```js
assert.match(code, /interface FlashCard\s*{/);
```

There should be a property called `questionText` on the `FlashCard` interface.

```js
const patternOne = /interface FlashCard\s+{\s*questionText/;
const patternTwo =
  /interface FlashCard\s+{\s*questionAnswer\s*:\s*string;?\s*questionText/;
const combinedRegex = __helpers.permutateRegex([patternOne, patternTwo], {
  elementsSeparator: 's*|'
});
assert.match(code, combinedRegex);
```

There should be a property called `questionAnswer` on the `FlashCard` interface.

```js
const patternOne = /interface FlashCard\s+{\s*questionAnswer/;
const patternTwo =
  /interface FlashCard\s+{\s*questionText\s*:\s*string;?\s*questionAnswer/;
const combinedRegex = __helpers.permutateRegex([patternOne, patternTwo], {
  elementsSeparator: 's*|'
});
assert.match(code, combinedRegex);
...
You should have an collection of `FlashCard` elements called `cards`. 
...
An `InvalidUserInputError` should be thrown when either the question text or question anser is empty in the entry form.

```js
const entryForm = document.querySelector("#entry-form"); 
const frontText = document.querySelector("#front-text");
const backText = document.querySelector("#back-text");
assert.throws(entryForm.submit, InvalidUserInputError); 
```
...
When the `delete-btn` is pressed, a flashcard element should be removed from the `cards` collection.

```js
const cardsLength = cards.length; 
cards.push({questionText:"First question", questionAnswer: "First answer"});
cards.push({questionText:"Second question", questionAnswer: "Second answer"});
const deleteBtn = document.querySelector("#delete-btn");
deleteBtn.click();
assert.strictEqual(cards.length, cardsLength - 1);
```


When the `delete-btn` is pressed, the previous flashcard data should be displayed. 
...
frontText.value = "First question";
backText.value = "First answer";
entryForm.submit();

frontText.value = "Second question";
backText.value = "Second answer";
entryForm.submit();

deleteBtn.click();
assert.strictEqual(flashCard.innerText, "First questionFirst Answer");
```
