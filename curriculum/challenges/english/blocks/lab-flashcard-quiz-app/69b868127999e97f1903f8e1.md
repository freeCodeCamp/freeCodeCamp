...
11) When a new flashcard is added, it should be displayed as the current card.

# --hints--

When a new flashcard is added using the `#entry-form`, it should immediately be displayed on the `#flashcard`.
...
frontText.value = "New Card Front";
backText.value = "New Card Back";
submitBtn.click();

assert.include(flashCard.textContent, "New Card Front");
```
