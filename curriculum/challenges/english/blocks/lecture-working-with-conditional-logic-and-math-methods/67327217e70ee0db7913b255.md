...
## Rounding Numbers with `Math.ceil()`, `Math.floor()`, and `Math.round()`
...
## Calculating Roots with `Math.sqrt()` and `Math.cbrt()`
...
The first log statement, will log `9` because the square root of `81` is `9`, while the second log statement will log `3` because the cube root of `27` is `3`.
...
## Calculating Exponents with `Math.pow()`

The last method we will look at will be the `Math.pow()` method:
