...
the accessibility tree is a structure used by assistive technologies, such as screen readers, to interpret and interact with the content on a webpage.
...
In the above example, smooth scrolling is enabled if the user has not set a preference for reduced motion  on their device.
...
Using placeholder text is not great for accessibility. Too often, users confuse the placeholder text with an actual input value,  and they think there is already a value in the input.
