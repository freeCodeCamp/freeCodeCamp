...
Create a function named `renderEntry` that takes an `entry` parameter. Inside, declare variables `title`, `author`, `year`, and `location`, each reading from the corresponding `entry` property with an `|| "Unknown"` fallback. For example: `const title = entry.title || "Unknown";`

Return a multi-line template literal that formats the entry as a card. Use `"─".repeat(50)` for top and bottom borders, with labeled lines:
...
Below the function, log the result of `renderEntry(catalog[0])`.

# --hints--

You should have a function named `renderEntry`.

```js

```

You should declare `title`, `author`, `year`, and `location` with `|| "Unknown"` fallbacks.

```js

```

`renderEntry` should return a formatted template literal string with borders and labels.

```js

```

You should log the result of `renderEntry(catalog[0])`.
