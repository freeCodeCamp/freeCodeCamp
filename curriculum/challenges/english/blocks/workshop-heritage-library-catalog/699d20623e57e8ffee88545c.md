...
const { groupByDecade } = explorer.allFunctions;
assert.exists(groupByDecade);
assert.equal(groupByDecade.parameters[0]?.toString(), "catalog");
```

You should declare a `const` variable named `grouped` set to an empty object.

```js
const explorer = await __helpers.Explorer(code);
const { groupByDecade } = explorer.allFunctions;
assert.exists(groupByDecade);
const { grouped } = groupByDecade.variables;
assert.isTrue(grouped?.matches('const grouped = {}'));
```
