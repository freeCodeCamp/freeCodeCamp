...
```js
assert.isFunction(groupByDecade);
```

`groupByDecade` should have a `catalog` parameter.

```js
assert.match(groupByDecade.toString(), /catalog/);
```
...
```js
assert.match(groupByDecade.toString(), /for\s*\([^)]*catalog\.length[^)]*\)/);
```
