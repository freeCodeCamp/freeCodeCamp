...
const { cardResult } = explorer.variables;
assert.exists(cardResult);
assert.isTrue(cardResult.value.matches('parseCard(rawCatalogCards[2])'));
```
...
```js
assert.strictEqual(getLogs()[0], cardResult);
```
