...
The variable `i` starts at `0`, the loop continues while `i < array.length`, and `i++` increments `i` by one after each iteration. You will learn more about loops in a later module.
