...
```js
assert.match(groupByDecade.toString(), /(?:!\s*grouped\s*\[\s*"Unknown"\s*\]|!Object\.hasOwn\s*\(\s*grouped\s*,\s*"Unknown"\s*\)|!grouped\.hasOwnProperty\s*\(\s*"Unknown"\s*\)|grouped\s*\[\s*"Unknown"\s*\]\s*===?\s*undefined)/);
