...
Declare four constants: `const title = trimmedParts[0]`, `const author = trimmedParts[1]`, `const year = trimmedParts[2]`, and `const location = trimmedParts[3]`. Then update the `return` statement to return an object with those four properties.
