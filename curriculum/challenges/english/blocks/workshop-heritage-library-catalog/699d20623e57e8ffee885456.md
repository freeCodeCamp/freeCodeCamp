...
Update the existing `return` statement so that your function returns an object with four properties: `title`, `author`, `year`, and `location`, each set to the corresponding value from `trimmedParts`.
