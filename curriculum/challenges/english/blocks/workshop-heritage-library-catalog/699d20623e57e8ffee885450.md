...
`parseCard` will take a raw catalog string and return a structured object with four properties: `title`, `author`, `year`, and `location`. For now, create the function with a parameter named `rawString`.
...
```js
assert.match(parseCard.toString(), /\brawString\b/);
```
