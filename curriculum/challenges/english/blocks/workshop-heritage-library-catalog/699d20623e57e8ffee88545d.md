...
Create a function named `validateEntry` that takes an `entry` parameter. Declare `let isValid = true`.

The `in` operator checks whether a property name exists in an object. For example, `"title" in entry` returns `true` if `entry` has a `title` property.

For each of the four fields (`title`, `author`, `year`, `location`), add a check: if the field is missing from the object, or is falsy, or equals `"Unknown"`, log a warning with `console.warn` and set `isValid` to `false`.
...
For the `author`, `year`, and `location` checks, include the entry's title in the warning message using a template literal, like: `` `Warning: Missing or unknown author for "${entry.title}"` ``.

After all four checks, return `isValid`.

# --hints--

You should have a function named `validateEntry`.

```js

```

You should declare `isValid` as `true`.

```js

```

You should use the `in` operator for each field check.

```js

```

You should call `console.warn` for each missing or unknown field.

```js

```

`validateEntry` should return `isValid`.
