...
```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+decade\s*=/);
```

You should declare a variable named `decadeKey` using a template literal.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+decadeKey\s*=/);
```

You should initialize `grouped[decadeKey]` as `[]` if it doesn't exist.

```js
assert.match(groupByDecade.toString(), /grouped\s*\[\s*decadeKey\s*\]/);
```

You should push `book` into `grouped[decadeKey]`.

```js
assert.match(groupByDecade.toString(), /\.push\s*\(\s*book\s*\)/);
```
