...
Return a multi-line template literal that formats the entry as a library card. Use `"-".repeat(50)` for the top and bottom border, and include labeled lines for `Title`, `Author`, `Year`, and `Location`. For example, a labeled line for the title would look like `Title: ${title}`.
...
The returned string should include a top and bottom border using `"-".repeat(50)`.

```js
assert.isAbove(renderEntry(catalog[0]).split("-".repeat(50)).length, 2);
```
