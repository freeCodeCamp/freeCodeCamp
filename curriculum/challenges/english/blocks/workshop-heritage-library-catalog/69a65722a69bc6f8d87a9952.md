...
Create a function named `exportToJSON` that takes `catalog` as a parameter. Return `JSON.stringify(catalog, null, 2)`. The first argument is the data to convert. The second argument (`null`) means no custom filtering. The third (`2`) adds two-space indentation, making the output readable.
