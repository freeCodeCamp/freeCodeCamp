...
const { parseCard } = explorer.functions;
assert.exists(parseCard);
const { trimmedParts } = parseCard.variables;
assert.exists(trimmedParts);
const trimmedPartsValue = eval(trimmedParts.value.toString());
assert.isArray(trimmedPartsValue);
assert.isEmpty(trimmedPartsValue);
```
