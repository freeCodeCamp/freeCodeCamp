...
Inside `parseCard`, declare an empty array named `trimmedParts`. Use a `for` loop to iterate over `parts`. On each iteration, push `parts[i].trim()` into `trimmedParts`. Then return `trimmedParts`.
