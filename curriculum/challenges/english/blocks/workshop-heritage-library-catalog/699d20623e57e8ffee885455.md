...
After splitting, each part still has extra whitespace. For example, splitting `"The Shining | King, Stephen"` produces `["The Shining ", " King, Stephen"]`. Notice the spaces around the values. To clean every element, you need to process each one individually. This is a perfect use case for a `for` loop.

A `for` loop repeats a block of code once for each element in an array. The syntax is:
...
```js
assert.match(parseCard.toString(), /for\s*\([^)]*parts\.length[^)]*\)/);
```
