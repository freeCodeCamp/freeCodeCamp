...
Then, below the function, call `groupByDecade(catalog)` and assign the result to a variable named `byDecade`. Log `byDecade` to inspect the grouped result — you should see an object with keys like `"1950s"`, `"1960s"`, `"1970s"`, and so on, each pointing to an array of books from that decade.
