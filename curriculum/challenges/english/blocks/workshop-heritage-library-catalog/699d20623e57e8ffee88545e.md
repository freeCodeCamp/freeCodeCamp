...
Below the function, call `groupByDecade(catalog)`, assign the result to a variable named `byDecade`, and log it. You should see an object with keys like `"1950s"`, `"1960s"`, `"1970s"`, and so on, each pointing to an array of books from that decade.
