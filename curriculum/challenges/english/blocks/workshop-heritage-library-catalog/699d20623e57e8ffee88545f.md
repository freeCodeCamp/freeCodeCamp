...
const { renderEntry } = explorer.allFunctions;
assert.exists(renderEntry);
```
...
```js
const explorer = await __helpers.Explorer(code);
const { parameters } = explorer.allFunctions.renderEntry;
assert.equal(parameters[0].toString(), 'entry');
```
...
```js
assert.strictEqual(getLogs()[0], renderEntry(catalog[0]));
```
