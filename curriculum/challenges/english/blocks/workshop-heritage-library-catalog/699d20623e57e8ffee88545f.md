...
Create a function named `exportToJSON` that takes `catalog` as a parameter. Return `JSON.stringify(catalog, null, 2)`.

`JSON.stringify` converts a JavaScript value to a JSON string. The second argument (`null`) means no replacer function, and the third argument (`2`) adds 2-space indentation for readability.

Below the function, log the result of `exportToJSON(catalog.slice(0, 2))` to preview the JSON output for the first two entries.

# --hints--

You should have a function named `exportToJSON`.

```js

```

`exportToJSON` should return `JSON.stringify(catalog, null, 2)`.

```js

```

You should log the result of calling `exportToJSON` with a slice of the catalog.
