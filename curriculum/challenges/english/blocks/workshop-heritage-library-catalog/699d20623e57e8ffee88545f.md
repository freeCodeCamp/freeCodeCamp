...
To make catalog entries easier to scan, format each one as a readable library card.

Create a function named `renderEntry` that takes `entry` as a parameter.

Declare a fallback constant for each field. For example, `const title = entry.title || "Unknown"`. Apply the same pattern for `author`, `year`, and `location`.

Return a multi-line template literal that formats the entry as a library card. Use `"-".repeat(50)` for the top and bottom border, and include labeled lines for `Title`, `Author`, `Year`, and `Location`.

Below the function, log `renderEntry(catalog[0])`.
