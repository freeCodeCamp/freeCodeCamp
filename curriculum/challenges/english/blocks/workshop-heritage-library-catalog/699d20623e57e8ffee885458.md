...
Call `findByAuthor(catalog, "king")` and assign the result to a variable named `kingBooks`. Log `kingBooks.length` to see how many books were found.

Then use a `for` loop to iterate over `kingBooks` and log each book's title and year in this format: `` `  - ${kingBooks[i].title} (${kingBooks[i].year})` ``.

# --hints--

You should declare `kingBooks` assigned to the result of `findByAuthor(catalog, "king")`.

```js

```

You should log `kingBooks.length`.
