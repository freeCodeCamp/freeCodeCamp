...
The year field arrives as a string like `"1978"`, but you will need it as a number in a later step when you calculate which decade a book belongs to.

`parseInt(year)` converts a string to an integer. Its optional second argument sets the numeric base; since base 10 is the default, you can omit it here.

The year field can also be blank, so you need a fallback. The ternary operator has the form `condition ? valueIfTrue : valueIfFalse`; it evaluates the condition and returns one of two values depending on the result. Applied here: if `year` is truthy (non-empty), convert it to a number; otherwise, use `"Unknown"`.
...
`parseCard` is now complete. The logged output should show `year` as a number.
