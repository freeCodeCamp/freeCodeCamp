...
Inside the loop, check whether the current entry's author (converted to lowercase) includes `searchTerm`. The `.includes()` method returns `true` if a string contains another string as a substring. If the condition is true, push the entry into `results`. `return results;` is already in place after the loop.
