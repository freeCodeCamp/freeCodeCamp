...
const { exportToCSV } = explorer.allFunctions;
assert.exists(exportToCSV);
const { parameters } = exportToCSV;
assert.equal(parameters[0]?.toString(), 'catalog');
```

You should declare a `const` variable named `header` with value `"Title,Author,Year,Location"`.

```js
const explorer = await __helpers.Explorer(code);
const { exportToCSV } = explorer.allFunctions;
assert.exists(exportToCSV);
const { header } = exportToCSV.variables;
assert.isTrue(header?.matches('const header = "Title,Author,Year,Location"'));
```

You should declare a `const` variable named `rows` set to an empty array.

```js
const explorer = await __helpers.Explorer(code);
const { exportToCSV } = explorer.allFunctions;
assert.exists(exportToCSV);
const { rows } = exportToCSV.variables;
assert.isTrue(rows?.matches(`const rows = ${rows?.value.toString()}`))
const rowsValue = eval(rows?.value.toString());
assert.isArray(rowsValue);
assert.isEmpty(rowsValue);
```
