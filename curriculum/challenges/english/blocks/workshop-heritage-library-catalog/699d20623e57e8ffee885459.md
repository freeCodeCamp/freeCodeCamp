...
`parseCard` handles one card at a time. Now you need to process all 19. This is one of the most common `for` loop patterns: applying a transformation to every item in a collection.

Create a function named `parseCatalog` that takes `rawCards` as a parameter. Inside, declare an empty array. Use a `for` loop to iterate over `rawCards`, calling `parseCard` on each entry and pushing the result into your array. Return the array when the loop is done.

Below the function, call `parseCatalog(rawCatalogCards)`, assign the result to a variable named `catalog`, and log `catalog.length`. You should see `19`.

# --hints--

`parseCatalog` should return an array.
