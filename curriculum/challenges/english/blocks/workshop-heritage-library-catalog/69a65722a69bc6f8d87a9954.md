...
```js
assert.strictEqual(getLogs()[2], catalog.length);
```

You should log `Object.keys(byDecade).length` to the console.

```js
assert.strictEqual(getLogs()[3], Object.keys(byDecade).length);
```
...
```js
assert.strictEqual(getLogs()[4], oldestYear);
```

You should log `newestYear` to the console.

```js
assert.strictEqual(getLogs()[5], newestYear);
```
