...
The catalog system is complete. Let's finish with a summary that uses one last `for` loop, this time for aggregating values across a collection.

Log `catalog.length` and `Object.keys(byDecade).length`. `Object.keys()` returns an array of an object's property names, so this tells you how many decade groups exist.

Declare `let oldestYear = Infinity` and `let newestYear = 0`. Starting with `Infinity` ensures any real year will be smaller; starting with `0` ensures any real year will be larger. Use a `for` loop to iterate over `catalog`. For each entry where `catalog[i].year` is not `"Unknown"`, update `oldestYear` if the current year is smaller and `newestYear` if the current year is larger. Log both values.

Finally, log `"Heritage Library Catalog complete!"`.
...
You should log `"Heritage Library Catalog complete!"`.
