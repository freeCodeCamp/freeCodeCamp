...
Use the `in` operator to check whether a property exists (for example, `"title" in entry`).

Add a validation check for `title` only: if the `title` property does not exist, or its value is falsy, or it equals `"Unknown"`, set `isValid = false`.
