...
With the catalog parsed and displayed, it's worth checking entries for data quality before trusting them. `validateEntry` will inspect a parsed entry and return `true` if all four fields are present and meaningful, `false` otherwise.

The `in` operator checks whether a property name exists on an object: `"title" in entry` returns `true` if `entry` has a `title` property, regardless of its value. This matters for validation because it catches the case where a field was never provided at all, not just the case where it's empty.

Create a function named `validateEntry` that takes `entry` as a parameter. Declare `let isValid = true`. Add a check for the `title` field: if the property does not exist on the object, or its value is falsy, or it equals `"Unknown"`, set `isValid = false`. Return `isValid` at the end.
