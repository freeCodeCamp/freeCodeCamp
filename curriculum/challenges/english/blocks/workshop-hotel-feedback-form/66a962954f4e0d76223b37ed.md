...
Below that `input`, add a `label` element with the text `Location` and the `for` attribute set to `"location"`.
...
Below that `input`, add a `label` element with the text `Reputation` and the `for` attribute set to `"reputation"`.
...
Your fourth checkbox `input` should have an `id` attribute set to `"reputation"`.
