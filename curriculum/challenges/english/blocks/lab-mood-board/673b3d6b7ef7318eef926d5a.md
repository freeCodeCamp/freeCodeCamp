...
Your `MoodBoard` component should render at least three `MoodBoardItem` components within the .mood-board element, each of which 

```js
{
  text: "You should render a div with a class of mood-board.",
  test: () => {
    const moodBoard =
      document.getElementsByClassName("mood-board")[0];
