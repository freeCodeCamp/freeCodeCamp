...
Since `-1` is not a valid value for an array's `length` (the length must be a non-negative integer), assigning it will result in a `RangeError`.
...
Which error occurs when you assign an invalid value (for example `-1`) to an array's `length`?
