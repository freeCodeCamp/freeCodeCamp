...
A descendant combinator is used to target elements matched by the second selector if they are nested anywhere within an ancestor element that matches the first selector. An ancestor can be a parent, grandparent, or any ancestor further up the document tree.
