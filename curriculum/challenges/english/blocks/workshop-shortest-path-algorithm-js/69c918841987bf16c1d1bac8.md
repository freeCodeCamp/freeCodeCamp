...
Inside the neighbor `for` loop, add an `if` statement that checks if `distance` is not strictly equals to `INF` **and** if `nodeNo` is not visited.

Inside the  block of the `if` statement, declare a variable `newDistance` assigned to `distances[current] + distance`. This is the total cost of reaching the neighbor through the current node.
