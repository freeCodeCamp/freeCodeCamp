...
Inside the `for...of` loop, add an `if` statement that checks if `nodeNo` is strictly equals to `startNode` **or** `distances[nodeNo]` is strictly equals to `INF`. If either is true, use `continue` to skip to the next iteration.
