...
Create an `if` statement to check if `age` is greater than `17`. Inside the `if` block, print `User is eligible to book a ticket`. This will print the message only when the user's age is greater than `17`.
