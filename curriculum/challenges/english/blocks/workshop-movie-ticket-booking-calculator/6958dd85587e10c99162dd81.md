...
The if statement condition should be `age > 18`.

```js 
({ test: () => (runPython(`
_cond = _Node(_code).find_ifs()[0].find_conditions()[0] 
assert _cond.is_equivalent("age > 18") or _cond.is_equivalent("18 < age")
`)) })
```

You should have a `print()` function inside your `if` statement. Remember to indent the body of the `if` statement.
