...
Then, create an `if` statement to check if `is_weekend` is `True`. Inside the body of `if` statement, update the `extra_charges` value to `2` and print `Extra charges will be applied` in the terminal.
...
The if statement condition should be `is_weekend`.
...
You should assign the integer value `2` to `extra_charges` variable inside your `if` statement.
...
You should print `Extra charges will be applied` inside your `if` statement. Remember to surround the message with single or double quotes.
