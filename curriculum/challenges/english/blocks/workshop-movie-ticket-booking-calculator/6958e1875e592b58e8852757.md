...
Then, create an `if` statement to check if `is_weekend` is truthy. Inside the body of the `if` statement, update the `extra_charges` value to `2` and print `Extra charges will be applied` in the terminal.
...
You should assign the integer value `2` to the `extra_charges` variable inside your new `if` statement.
