...
Add an `elif` statement to your `if...else` statement and check if `seat_type` is equal to `Gold`. Inside the body of `elif` statement, update the value of `service_charges` to `3`.

Below the `if...elif..else` statement, use the `print()` function to display a message that shows `Service charges:` followed by the value of `service_charges`. Then, check the output in the terminal.
