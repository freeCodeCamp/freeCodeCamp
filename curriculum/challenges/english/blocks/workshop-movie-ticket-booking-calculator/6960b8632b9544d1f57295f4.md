...
Users who are at least `18` years old but younger than `21` can book tickets, but only when the `show_time` is not `Evening`.
...
Your last `if` statement condition should be `age >= 21 or age >= 18 and show_time != 'Evening'`.
