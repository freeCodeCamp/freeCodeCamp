...
Create an `if` statement to check if `is_member` is truthy. Inside the body of `if` statement, update the `discount` value to `3` and print `User qualify for membership discount` to the terminal.
...
You should print `User qualify for membership discount` inside your new `if` statement. Remember to surround the message with single or double quotes.
