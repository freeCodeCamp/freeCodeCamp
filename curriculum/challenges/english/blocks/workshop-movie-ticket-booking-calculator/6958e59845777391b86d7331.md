...
Then, create an `if` statement to check if `seat_type` is equal to `Premium`. Inside the body of `if` statement, update the `service_charges` value to `5`.

Add an `else` clause to your `if` statement and update the `service_charges` value to `1` inside the `else` body.
...
```js
({ test: () => (runPython(`
_cond = _Node(_code).find_ifs()[4].find_ifs()[0].find_conditions()[1].is_empty() 
`)) })
```
