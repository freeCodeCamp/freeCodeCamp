...
Now, add another condition to your existing `if` statement using the `or` operator to check if `is_member` is `True`. Use the parentheses `()` to group the `show_time != Evening` and `is_member` condition together as shown in the above example.
...
You should use parentheses `()` to group the conditions correctly.
