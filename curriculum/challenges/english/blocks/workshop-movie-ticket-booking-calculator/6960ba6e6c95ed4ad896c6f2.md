...
There is one more exception to the booking rules. Users between `18` but younger than `21` can book evening shows if they are members.

When multiple logical operators are used in an `if` statement, conditions joined with `and` have a higher precedence than conditions joined with `or`. Parentheses `()` are used in Python to group conditions and control the order in which they are evaluated.
