...
You should print `Final price of ticket:` followed by the value of `final_price` below your `final_price` variable.

```js
({ test: () => (runPython(`
print_calls = ["print('Final price of ticket:', final_price)", "print(f'Final price of ticket: {final_price}')", "print('Final price of ticket: ' + str(final_price))", "print('Final price of ticket:', str(final_price))"]
_body = _Node(_code).find_ifs()[4].find_bodies()[0]
assert any(_body.has_call(pc) for pc in print_calls)
`)) })
```
