...
2. You should implement the function `validateManifest(manifest)`, which accepts a manifest object and:
    ...
    - If any of the input manifest's fields are invalid or missing, returns an object describing those fields - specifically, `missingField: "Missing"` and `invalidField: "Invalid"`. Below is an example return where the input object is missing the `destination` field and has an invalid `weight` field:
...
const regex = __helpers.functionRegex('normalizeUnits', ['manifest']);
assert.match(__helpers.removeJSComments(normalizeUnits.toString()), regex);
```
...
const regex = __helpers.functionRegex('validateManifest', ['manifest']);
assert.match(__helpers.removeJSComments(validateManifest.toString()), regex);
```
...
const regex = __helpers.functionRegex('processManifest', ['manifest']);
assert.match(__helpers.removeJSComments(processManifest.toString()), regex);
```

Your function `processManifest()` should, if the input manifest object is valid, log a success message with the object's `containerId`, and then log the object's `weight` in kilograms.
...
Your function `processManifest()` should, if the input manifest object is not valid, should log an error message with the object's `containerId`, and then log the object returned by calling `validateManifest()` on the input object.
