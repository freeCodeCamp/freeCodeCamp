...
1. You should implement a function named `normalizeUnits` with a `manifest` parameter.
    - If the weight of the manifest object is expressed in pounds, the function should convert the weight to kilograms using the approximate conversion 1 lb = 0.45 kg.
    - If the weight is already expressed in kilograms, the weight should remain unchanged.
    - The function must not mutate the original manifest object and must always return a new object containing the normalized weight in kilograms.
...
Calling `validateManifest()` with `{ containerId: -42, destination: 405, weight: -84, unit: "pounds", hazmat: "no" }` should return the new object `{ containerId: "Invalid", destination: "Invalid", weight: "Invalid", unit: "Invalid", hazmat: "Invalid"}` without mutating the source input.
