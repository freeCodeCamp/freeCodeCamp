...
2. You should implement the function `validateManifest(manifest)`, which accepts a manifest object, validates it by checking its fields (except `containerId`) for missing or invalid values, then returns an object based on the manifest's validity.
    - A manifest's fields are invalid if the following conditions are met: 
        ...
        - `weight` is not a positive number or is a `NaN` value
...
Calling `normalizeUnits()` with input `{ containerId: 68, destination: "Salinas", weight: 101, unit: "lb", hazmat: true }` should return the new object `{ containerId: 68, destination: "Salinas", weight: 45.45, unit: "kg", hazmat: true }`.
...
Calling `validateManifest()` with input `{ containerId: 84, destination: "Santa Cruz", weight: 304, unit: "kg", hazmat: false }` should return the new object `{}`.
...
Calling `validateManifest()` on `{ containerId: 42, destination: 405, weight: -84, unit: "pounds", hazmat: "no" }` should return the new object `{ destination: "Invalid", weight: "Invalid", unit: "Invalid", hazmat: "Invalid"}`.
...
const copy = validateManifest(testObj);
assert.isObject(copy);
assert.deepEqual(copy, expected, "validateManifest() did not return the expected object for invalid fields");
assert.notStrictEqual(copy, testObj, "validateManifest() should return a new object, not mutate the original");
...
Calling `validateManifest()` on `{ containerId: 23 }` should return the new object `{ destination: "Missing", weight: "Missing", unit: "Missing", hazmat: "Missing"}`.
...
const copy = validateManifest(testObj);
assert.isObject(copy);
assert.deepEqual(copy, expected, "validateManifest() did not return the expected object for missing fields");
assert.notStrictEqual(copy, testObj, "validateManifest() should return a new object, not mutate the original");
...
Calling `validateManifest()` on `{ containerId: 26, destination: "Wattsonville" , weight: NaN }` should return the new object `{ weight: "Invalid", unit: "Missing", hazmat: "Missing"}`.
...
const copy = validateManifest(testObj);
assert.isObject(copy);
assert.deepEqual(copy, expected, "validateManifest() did not return the expected object for NaN weight and missing fields");
assert.notStrictEqual(copy, testObj, "validateManifest() should return a new object, not mutate the original");
...
Calling `validateManifest()` on `{ containerId: 62, destination: "" }` should return the new object `{ destination: "Invalid", weight: "Missing", unit: "Missing", hazmat: "Missing"}`.
...
Calling `validateManifest()` on `{ containerId: 63, destination: "  " }` should return the new object `{ destination: "Invalid", weight: "Missing", unit: "Missing", hazmat: "Missing"}`.
...
Calling `processManifest()` on `{ containerId: 88, destination: "Soledad" , weight: NaN }` should first log `Validation error: 88` and then log the object `{ weight: "Invalid", unit: "Missing", hazmat: "Missing" }`.
...
Calling `processManifest()` on `{ containerId: 55, destination: "Carmel" , weight: 400, unit: "lb", hazmat: false }` should first log `"Validation success: 55"` and then log `"Total weight: 180 kg"`.
