...
`El departamento de aprendizaje y desarrollo capacita al equipo` means "the Learning and Development department trains the team".

Both `apoyar` ("to support") and `capacitar` ("to train") are third-person singular verbs that end with `-ar`.

They follow the same pattern as `desarrolla`, `verifica`, and `crea` from earlier tasks.
