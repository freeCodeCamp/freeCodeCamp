...
In this lab you will test a specific property of each object in an array to see if it has always a truthy value or not.
...
If you were asked to test th `name` property, in the objects of this array the property `name` has the values of `"Quincy"`, `"Naomi"`, and `"Camperbot"`, so it is always truthy.
