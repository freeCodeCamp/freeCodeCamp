...
An algorithm that visits a two nodes in one direction before moving another direction in the graph.
...
An algorithm that visits a four nodes at a time before moving to the next level in the graph.
...
A tree must have loops or cycles.
...
What is the average and worst case time complexities for inserting and extracting the minimum or maximum value from a heap?
