...
* `npm run <script_name>`: used to run a custom npm script.
    
* `npm start`: used to start the main application or development server.
    
* `npm run lint`: used to run a code style checker to check if your code meets code style standards and best practices.
