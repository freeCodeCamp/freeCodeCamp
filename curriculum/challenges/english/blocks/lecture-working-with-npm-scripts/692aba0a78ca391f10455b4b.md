...
CommonJS is the original and default way of working with packages in Node.js. It loads modules synchronously, which means that it has to wait for a module to be fully loaded before it start loading the next one or continues running the code.

It's still widely used, but it's gradually being replaced by the newer ES syntax.

CommonJS uses the `require()` function to import a module, taking its relative path as argument.
...
Let's say that we try to import the `math.js` module into another module. We call the `require()` function, and pass the relative path as argument. Then, we assign the object returned by `require()` to the `math` variable.
...
Does the code waits for a module to fully load before resuming its execution?
