...
An MP3 file has the MIME type `audio/mpeg`, which is the standard and widely supported MIME type.You may sometimes see `audio/mp3` used, but it is non-standard and not officially recommended. An MP4, however, can have the MIME type `audio/mp4` OR `video/mp4`, depending on whether it's a video file or audio-only. This distinction tells the browser how to handle the file.
