...
({
    ...
    _Node(_code).find_variable('running_total').is_equivalent('running_total += tip')
    `))
})
```
...
({
    test: () => assert(runPython(`
    _Node(_code).get_variable('running_total') == 248.5375
    `))
})
```
...
({
    test: () => assert(runPython(`
    _Node(_code).has_call("print('Total with tip: ', running_total)")`))
})
```
