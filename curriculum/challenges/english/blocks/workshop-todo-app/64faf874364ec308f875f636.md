...
After calling `deleteTask()`, the task should be removed from `taskData`.
...
const deleteBtn = tasksContainer.querySelector("button[onclick*=\"deleteTask\"]");
deleteTask(deleteBtn);
assert.equal(taskData.length, 0);
taskData.length = 0; tasksContainer.innerHTML = ""; currentTask = {};
```

After calling `deleteTask()`, the task element should be removed from `tasksContainer`.
...
const deleteBtn = tasksContainer.querySelector("button[onclick*=\"deleteTask\"]");
deleteTask(deleteBtn);
assert.equal(tasksContainer.querySelectorAll(".task").length, 0);
taskData.length = 0; tasksContainer.innerHTML = ""; currentTask = {};
```
