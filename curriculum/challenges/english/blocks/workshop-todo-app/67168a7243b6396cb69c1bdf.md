...
const { addOrUpdateTask } = explorer.allFunctions;
assert.exists(addOrUpdateTask);
const { taskObj } = addOrUpdateTask.value.variables;
assert.exists(taskObj);
const id = taskObj.value.objectProps.id;
assert.exists(id);
assert.isTrue(id.value.matches('`${removeSpecialChars(titleInput.value).toLowerCase().split(" ").join("-")}-${Date.now()}`'));
```
