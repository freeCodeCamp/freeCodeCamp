...
```js
assert.match(editTask.toString(), /taskData\.findIndex\(/)
```

Your `findIndex` callback should compare `item.id` with the id of `buttonEl`'s parent element.

```js
assert.match(editTask.toString(), /item\.id\s*===\s*buttonEl\.parentElement\.id|buttonEl\.parentElement\.id\s*===\s*item\.id/)
```
