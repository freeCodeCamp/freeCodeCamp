...
You should declare a `const` variable named `dataArrIndex` using `taskData.findIndex()`.

```js
assert.match(code, /const\s+dataArrIndex\s*=\s*taskData\.findIndex\(/)
```

Your `findIndex` callback should check if `item.id` equals `currentTask.id`.

```js
assert.match(code, /item\.id\s*(===|==)\s*currentTask\.id|currentTask\.id\s*(===|==)\s*item\.id/)
```
