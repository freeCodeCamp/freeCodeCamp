...
const { getTaskArrObj } = explorer.variables;
assert.exists(getTaskArrObj);
assert.isTrue(getTaskArrObj.toString().startsWith('const'));
assert.isTrue(getTaskArrObj.value.matches('JSON.parse(localStorage.getItem("data"))'));
```
