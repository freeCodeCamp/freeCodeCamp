...
Calling `updateTaskContainer()` multiple times should not duplicate tasks in `tasksContainer`.

```js
tasksContainer.innerHTML = ""; taskData.length = 0;
taskData.push({ id: "test-1", title: "Test Task", date: "2024-01-01", description: "A description" });
updateTaskContainer();
updateTaskContainer();
assert.equal(tasksContainer.querySelectorAll(".task").length, 1);
