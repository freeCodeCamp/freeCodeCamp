...
- **`span` element for accessibility**: A `span` element is a generic inline container for content. It's commonly used to style a piece of text differently from the surrounding text, or to wrap text and give it an `id` that other elements (such as ARIA attributes) can reference.
