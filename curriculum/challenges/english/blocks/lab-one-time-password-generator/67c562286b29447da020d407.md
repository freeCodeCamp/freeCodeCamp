...
Your `h2` element with the `id` `otp-display` should display a new OTP when the button is clicked.
...
Your `p` element with the `id` `otp-timer` should update its message every second to show the remaining time.
