...
3. Your `div` with the class `container` should include the following elements:
    ...
    - An `h1` element with an `id` of `otp-title` and text `"OTP Generator"`.
    - An `h2` element with an `id` of `otp-display` that either displays the message `"Click 'Generate OTP' to get a code"` or shows the generated OTP if one is available.
    - A `p` element with an `id` of `otp-timer` and an `aria-live` attribute set to a valid value that:
        - Starts off empty.
...
Your `.container` should contain an `h1` element with an `id` of `otp-title` and the text `"OTP Generator"`.
...
Your `.container` should contain an `h2` element with an `id` of `otp-display` that displays the OTP when generated. 
...
Your `.container` element should contain a `p` element with an `id` of `otp-timer`.
...
Your `.container` should contain a `button` element with an `id` of `generate-otp-button` and text `"Generate OTP"`. 
...
Your `h2` element with an `id` of `otp-display` should display a new OTP when the button is clicked.
...
Your `p` element with an `id` of `otp-timer` should show a 5-second countdown when the button is clicked.
...
Your `p` element with an `id` of `otp-timer` should update its message every second to show the remaining time.
