...
Give your custom radio buttons a size and shape.

Still targeting `.contact-method-radio-btn`, set `width` to `20px`, `height` to `20px`, `border-radius` to `50%` (to make them circular), and `border` to `2px solid gray`.
...
```js
const style = new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn');
assert.strictEqual(style?.border, '2px solid gray');
```
