...
Next, let's style the main container.

Select the element with the class `container`. Set its `background-color` to `#ffffff1a`. (The hex code `#ffffff1a` represents white with a low opacity. A `hex code` is a six-digit combination of numbers and letters used in HTML and CSS to represent colors. It starts with a `#` followed by three pairs:
