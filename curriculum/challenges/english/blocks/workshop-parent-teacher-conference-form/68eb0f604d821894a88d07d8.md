...
To make the container stand out, add a `box-shadow`. Set it to `0 5px 15px black`. (The `box-shadow` property adds a shadow effect around an element. The values control the horizontal offset, vertical offset, blur radius, and color respectively.)
...
Set the `box-shadow` property to `0 5px 15px black`.
