...
`Array.from()` creates an array with defined slots initialized to `undefined`, not a sparse array with empty slots.
...
Using `fill()` creates an array with defined slots containing `undefined`, not a sparse array with empty slots.
...
A regular array with values is not a sparse array. Sparse arrays have empty slots.
