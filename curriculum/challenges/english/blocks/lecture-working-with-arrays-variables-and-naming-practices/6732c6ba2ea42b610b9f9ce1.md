...
In this case even though we only have two defined elements, `1` and `4`, the length is `4` because the highest index (`3`) plus `1` gives us a length of `4`. The empty slots in the middle do not contain `undefined`; they are truly empty.
...
In this example, we create a new array using `Array(5)`. This creates a sparse array with a length of `5` where all the slots are empty (not containing `undefined`, but truly empty). Note: different environments may display empty slots differently, but they all represent the same sparse structure. 

Another way to create an empty array of fixed length is to use the `Array.from()` method with a length argument. Unlike `new Array(n)`, this method creates an array of the specified length with all elements **defined** and initialized to `undefined`: 
...
Notice the difference: this array has five defined slots (each containing `undefined`), whereas `new Array(5)` creates a sparse array with five empty slots. This distinction matters when iterating or using array methods.
...
The `splice()` method modifies the array in place, removing elements and adjusting length accordingly.
...
The `splice()` method removes the 3 elements at index 2 (i.e., elements at indices 2, 3, and 4), leaving 2 elements in the array.
...
The `length` property returns a number, not `undefined`.
