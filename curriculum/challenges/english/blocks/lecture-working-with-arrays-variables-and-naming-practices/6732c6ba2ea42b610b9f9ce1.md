...
`Array.from()` creates defined slots initialized to `undefined`, unlike `new Array(4)` which creates empty slots.
...
`Array.from()` returns an array, not a number. To get the length, you would need to access `arr.length`.
