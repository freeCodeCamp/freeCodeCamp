...
`randomBytes()` takes in a `size` and generates cryptographically secured tokens. That makes it a good for generating UUIDs (universally unique IDs). In addition, it's a good replacement for `Math.random()`, which is not secure for tokens and keys.
...
The `randomInt()` method takes in a `min` and `max` values and generate a secure random integer between them. It is useful for OTPs and random selection.
