...
The flexibility the `fs` module offers is that it allows you to use file system operations both synchronously and asynchronously. Many file system operations have both asynchronous and synchronous forms:
...
fs.openAsBlob() // Asynchronously opens a file as a Blob

fs.opendir() // Opens directory
fs.opendirSync() // Synchronous directory open
```

For file system operations that support these forms, you can choose between callbacks, promises, and synchronous methods depending on your needs:
...
Which of the following is **NOT** a valid way to write data to a file using the Node `fs` module?
## --answers--
