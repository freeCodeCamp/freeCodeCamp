...
Asynchronous methods avoid this by performing file system operations off the main event loop thread via libuv. You can use callbacks or promise-based APIs (`fs/promises`) with `async/await` syntax for better readability and maintainability. While promises provide cleaner async flow, callback APIs may still be preferable when minimizing memory allocation and overhead is critical.

In this lesson, we will use the promise-based `fs/promises` module.

The `writeFile()` method lets you write data to a file. If the file does not exist, it creates it at the specified path (which resolves relative to the current working directory when a filename without a path is provided).
...
The `appendFile()` method lets you add content to a file. By default, if the file does not exist, it creates it at the specified path before writing:
...
/*
File content: <Buffer 23 23 20 4e 6f 64 65 20 60 66 73 60 20 4d 6f 64 75 6c 65 3a 20 54 68 65 20 43 6f 6d 70 6c 65 74 65 20 47 75 69 64 65 0a 49 6e 20 74 68 69 73 20 61 72 ... 72 more bytes>
*/
```
