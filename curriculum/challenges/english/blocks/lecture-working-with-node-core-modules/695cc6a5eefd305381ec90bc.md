...
The `writeFile()` method is the method that lets you write to an existing file. If the file doesn't exist, it creates it in the current directory, then writes the specified content to it.
...
Which method in the Node `fs` module lets you write to an existing file, and if the file doesn’t exist, creates it in the current directory with the specified content?
