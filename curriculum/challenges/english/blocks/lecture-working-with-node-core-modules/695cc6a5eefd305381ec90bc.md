...
The flexibility the `fs` module offers is that it allows you to use its methods both synchronously and asynchronously. Many methods in the `fs` module are available in both asynchronous and synchronous forms:
...
It doesn't end there. Depending on your needs, you can use the `fs` module in three ways:
...
Which of the following is **NOT** one of the three ways you can use the Node `fs` module?
