...
In CSS, the `hsl()` function is used to define colors using the HSL color model. Here is the basic syntax in both the legacy and modern formats. Though both are supported, this lesson will use the modern format after presenting the two methods:
...
Just like the RGB model has an `rgba()` function to include transparency, the HSL model has an `hsla()` function. The fourth parameter in this function represents the alpha value, which controls the opacity of the color. Here is the basic syntax in both the legacy and modern formats:
