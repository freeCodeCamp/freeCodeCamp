...
Print both `is_student` and `type(is_student)` on the same line using a comma `,` as shown in the example. Check the output in the terminal, which shows the value of `is_student` and its type, `bool` (boolean).
