...
The malicious site cannot read your cookie, it simply piggybacks on the browser automatic cookie attachment.
...
- **CSRF Tokens:** The server generates a random, unique token per session and attaches it to each form. The client must include it in the request headers or form data. Because a malicious site cannot access the frontend to copy the token, forged requests fail validation.
