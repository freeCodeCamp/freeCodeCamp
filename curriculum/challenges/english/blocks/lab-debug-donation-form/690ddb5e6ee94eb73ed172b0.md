...
4. Use the `for` attribute on each `<label>` element and the corresponding `id` attribute on each `<input>` element to explicitly associate them together:
...
Your `Email Address:` input should be of type `email`.
