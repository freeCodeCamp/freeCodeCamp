...
You should not hardcode a random number for the `randomNum` variable. You should initialize `randomNum` to the result of the `Math.random()` method.

```js
const explorer = await __helpers.Explorer(code);
const randomNum = explorer.variables.randomNum;
assert.notInclude(randomNum?.value?.toString(), "0.");

// check for whole random numbers just in case a camper does this
assert.notMatch(randomNum?.value?.toString(), /^\d+$/);
```

You should log the `randomNum` variable to the console.

```js
assert.equal(getLogs()[2], randomNum);;
```
