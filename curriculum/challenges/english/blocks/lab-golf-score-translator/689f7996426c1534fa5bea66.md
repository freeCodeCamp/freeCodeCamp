...
`golfScore(5, 3)` should return the string `Eagle`

```js
assert.strictEqual(golfScore(5,3), 'Eagle');
```
...
`golfScore(3, 6)` should return the string `Go Home!`

```js
assert.strictEqual(golfScore(3,6), 'Go Home!');
```
...
`golfScore(5, 8)` should return the string `Go Home!`

```js
assert.strictEqual(golfScore(5,8), 'Go Home!');
```
