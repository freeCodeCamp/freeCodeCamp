...
For example, `&=` is the bitwise `AND` augmented assignment operator. It converts both values to binary and builds a new number. A bit becomes `1` only when both inputs have `1` in that position.
...
In the example above, the result is `8`, since the only bit `10` and `12` share in their binary forms is the leading 1. This produces `1000`, which is the binary form of 8.
