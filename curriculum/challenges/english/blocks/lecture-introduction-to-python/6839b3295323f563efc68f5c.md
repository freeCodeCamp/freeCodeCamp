...
These work by treating numbers as a string of bits, rather than a single value.

Basically, the numbers they're working with get converted into binary, that is, a series of 0s and 1s computers use to represent data, and then, they're operated on.

As an example, `&=`, the bitwise `and` augmented assignment turns both of its inputs into binary, and after that, builds a new number. Each bit of the output is 1 if the corresponding bit of x and of y is 1, otherwise it's 0.

```python
    x = 10 # 1010 in binary
    x &= 12 # 12 in binary is 1100

    print(x) # 8
```

In the above example, the result is 8, since the only bit 10 and 12 share in their binary forms is their leading 1, which makes the output be `1000`, the binary form of 8.
