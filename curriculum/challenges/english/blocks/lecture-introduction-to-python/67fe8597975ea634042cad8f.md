...
In the example above, the variable `name` was assigned the value `’John Doe’`. This type of data is called a string. Strings are sequences of characters used to represent text. You can create a string by enclosing text in single or double quotes, for example `'Hello'` or `"Hello"`. In future lessons, you’ll learn more about working with strings in Python.
