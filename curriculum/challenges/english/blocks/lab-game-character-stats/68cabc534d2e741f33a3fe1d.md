...
You should set `self._mana` to `0` if `value` is less than `0`, `50` if `value` is greater than `50`, and to `value` otherwise in the `mana` setter method.

```js
({ 
    test: () => runPython(`
    _cond = _Node(_code).find_class("GameCharacter").find_functions("mana")[1].find_ifs()[0]
    assert _cond.is_equivalent("if value < 0:\\n self._mana = 0\\nelif value > 50:\\n self._mana = 50\\nelse:\\n self._mana = value")
`)})
```
...
You should use f-string to print a message that shows `name` leveled up to `level` in the `level_up` method.
...
({ 
  test: () => runPython(`
    node = _Node(_code).find_class("GameCharacter").find_function("level_up")
    assert (
        node.has_call("print(f'{self._name} leveled up to {self._level}')") or
        node.has_call('print(f"{self._name} leveled up to {self._level}")')
    )
`)
})
```
...
You should use f-string to print `name`, `level`, `health`, and `mana` in the `__str__` method, each on a new line.
