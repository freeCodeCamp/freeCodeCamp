...
2. When instantiated, a new `GameCharacter` object should take a `name` parameter and initialize the following attributes:
...
4. For the `health` property:
     ...
     - Define a setter that takes a `value` parameter, prevents health from being set below `0`, and caps the health at `100`.
5. For the `mana` property:
     - Define a getter that returns the current mana.
     - Define a setter that takes a `value` parameter, prevents mana from being set below `0`, and caps the mana at `50`.
...
You should set `self._health` to `0` if `value` is less than `0`.
...
You should set `self._health` to `100` if `value` is greater than `100`.
...
You should set `self._health` to `value` if `value` is between `0` and `100`.
...
The `mana` setter method should have two parameters, `self` and `value`.

```js
({ 
    test: () => runPython(`assert _Node(_code).find_class("GameCharacter").find_functions("mana")[1].has_args("self, value")`) 
})
```

You should set `self._mana` to `0` if `value` is less than `0`.
...
You should set `self._mana` to `50` if `value` is greater than `50`.
...
You should set `self._mana` to `value` if `value` is between `0` and `50`.
...
You should use an f-string to print `f"{self._name} leveled up to {self._level}"`.

```js
({
  test: () => runPython(`
    func = _Node(_code).find_class("GameCharacter").find_function("level_up")
    assert (
      func.has_call('print(f"{self._name} leveled up to {self._level}")') or
      func.has_call("print(f'{self._name} leveled up to {self._level}')")
    )
  `)
})
```
...
You should return the character's stats with an f-string: `f"Name: {self.__name}\nLevel: {self.__level}\nHealth: {self.__health}\nMana: {self.__mana}"`.

```js
({ 
  test: () => runPython(`
    node = _Node(_code).find_class("GameCharacter").find_function("__str__")
    assert (
        node.has_return("f'Name: {self._name}\\\\nLevel: {self._level}\\\\nHealth: {self._health}\\\\nMana: {self._mana}'") or
        node.has_return('f"Name: {self._name}\\\\nLevel: {self._level}\\\\nHealth: {self._health}\\\\nMana: {self._mana}"')
    )
  `)
})
