...
Inside the `try` block, call the `fetch` function with the `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` URL, written as a template literal, and store the result in a variable named `response`. Since you're inside an `async` function, you should use `await` before `fetch`.
