...
When the user types into the `input` field, a list of fruits matching their query should appear.

To handle this, create a state variable named `results` with an initial value of an empty array (`[]`). Also define its setter function, `setResults`, using the `useState` Hook.
