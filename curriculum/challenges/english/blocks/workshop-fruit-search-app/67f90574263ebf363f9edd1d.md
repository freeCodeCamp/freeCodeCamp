...
To start, after the `if` statement, call `setTimeout` with an empty arrow function and a delay value of `700` as arguments, and store it in a variable named `timeoutId`. This allows you to cancel the timeout later when the effect cleans up.
