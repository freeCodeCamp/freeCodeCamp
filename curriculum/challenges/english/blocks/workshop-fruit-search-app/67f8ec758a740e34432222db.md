...
Add an `onChange` attribute to the `input` element to capture what the user types. Set it to an arrow function that takes `e` as an argument. Inside the function, update the `query` state by passing `e.target.value`, which holds the current value of the input, to the setter function. Here's an example:
...
const explorer = await __helpers.Explorer(`const handler = ${reactProps.onChange}`);
const { handler } = explorer.allFunctions;
assert.exists(handler);
assert.lengthOf(handler.parameters, 1);
assert.equal(handler.parameters[0].toString(), "e");
```
