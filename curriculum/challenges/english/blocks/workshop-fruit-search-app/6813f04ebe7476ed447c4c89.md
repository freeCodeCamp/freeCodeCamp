...
Next, call `.json()` on the `response` variable and store the result in a variable named `data`. Since you're still inside an `async` function, you should use `await` before `response.json()`.
