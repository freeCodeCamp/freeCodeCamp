...
Pass the `handleSubmit` function to the `onSubmit` attribute of the `form` element so the form submission can be handled correctly.
...
const reactProps = form[reactKey];
assert.exists(reactProps);

assert.isFunction(reactProps.onSubmit);
assert.equal(reactProps.onSubmit.name, "handleSubmit");
```
