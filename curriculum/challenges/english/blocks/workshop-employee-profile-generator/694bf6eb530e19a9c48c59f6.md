...
```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(year_code)") or _Node(_code).has_call("print(year_code, initials)")`)) })
```

You should print `initials` to the terminal.

```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(initials)") or _Node(_code).has_call("print(year_code, initials)")`)) })
```
