...
Create a variable `year_code` and assign it the slice of `employee_code` from index `4` up to (but not including) `8`. This will extract `2026`.

Then create a variable `initials` and assign it the slice of `employee_code` from index `9` up to (but not including) `11`. This will extract `JD`.
