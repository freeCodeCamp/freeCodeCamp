...
```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(first_name)") or _Node(_code).has_call("print(first_name, last_name)")`)) })
```

You should print your `last_name` variable to the terminal.

```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(last_name)") or _Node(_code).has_call("print(first_name, last_name)")`)) })
```
