...
# --description--
Add a player damage state so getting touched by an enemy triggers the full damage-juice stack: red flash, red tint, shake, alpha blink for invulnerability frames.
...
`flash(duration, r, g, b)` overlays a colored rectangle. The alpha tween with `repeat: 3, yoyo: true` runs the blink 7 times (initial + 3 yoyo pairs). Step 29 wires the game-over scene when `playerHp <= 0`.

# --hints--
Initialise `this.playerHp = 3` and `this.invulnerable = false`.

```js
assert.match(code, /this\.playerHp\s*=\s*3[\s\S]*this\.invulnerable\s*=\s*false/);
```

Register `this.physics.add.overlap(this.player, this.enemies, ...)`.

```js
assert.match(code, /this\.physics\.add\.overlap\(\s*this\.player\s*,\s*this\.enemies/);
```

Guard with `if (this.invulnerable || !this.player.active) return;`.

```js
assert.match(code, /if\s*\(\s*this\.invulnerable\s*\|\|\s*!this\.player\.active\s*\)\s*return/);
```
...
```js
assert.match(code, /this\.player\.setTintFill\(\s*0xff0000\s*\)[\s\S]*delayedCall\(\s*200[\s\S]*clearTint/);
```

Decrement `this.playerHp -= 1` and clear `invulnerable` after 800ms.

```js
assert.match(code, /this\.playerHp\s*-=\s*1[\s\S]*delayedCall\(\s*800[\s\S]*this\.invulnerable\s*=\s*false/);
```
