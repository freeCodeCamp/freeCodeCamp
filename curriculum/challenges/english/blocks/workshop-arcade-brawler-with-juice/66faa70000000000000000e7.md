...
# --description--
Emitters, graphics, sounds, and tweens leak when a scene restarts unless we tear them down. Hook the SHUTDOWN event in `/* create-cleanup */`.
...
`SHUTDOWN` fires when `scene.stop` runs. Without this, clicking PLAY AGAIN from the Game Over scene would leave the old particle emitter dangling - drawing forever to a destroyed scene.

# --hints--
Listen for `Phaser.Scenes.Events.SHUTDOWN`.

```js
assert.match(code, /this\.events\.on\(\s*Phaser\.Scenes\.Events\.SHUTDOWN/);
```

Destroy `this.fx` if it exists.

```js
assert.match(code, /if\s*\(\s*this\.fx\s*\)\s*this\.fx\.destroy\(\s*\)/);
```

Destroy `this.swingTrail` and `this.sword`.

```js
assert.match(code, /if\s*\(\s*this\.swingTrail\s*\)\s*this\.swingTrail\.destroy[\s\S]*if\s*\(\s*this\.sword\s*\)\s*this\.sword\.destroy/);
```

Call `this.tweens.killAll()` and `this.sound.stopAll()`.

```js
assert.match(code, /this\.tweens\.killAll\(\s*\)[\s\S]*this\.sound\.stopAll\(\s*\)/);
```
