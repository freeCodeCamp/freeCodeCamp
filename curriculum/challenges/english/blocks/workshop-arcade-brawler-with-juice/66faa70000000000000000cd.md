...
# --description--
Layer 6 (final): particle burst. The emitter we built in step 7 is sitting idle - call `explode(n, x, y)` to fire `n` particles from `(x, y)` in a one-shot burst.
...
Click an enemy now and you'll see all six juice layers stack on the same frame: SFX + shake + freeze + flash + squash + sparks. This is the recipe.

# --hints--
Call `this.fx.explode(20, enemy.x, enemy.y)`.

```js
assert.match(code, /this\.fx\.explode\(\s*20\s*,\s*enemy\.x\s*,\s*enemy\.y\s*\)/);
```
