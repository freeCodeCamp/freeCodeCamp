...
# --description--
Layer a Graphics-drawn arc behind the sword swing - the standard fighting-game speed line.

Insert this block right BEFORE the existing sword tween (after the lazy-create of `this.sword`):
...
`Graphics.arc` takes radians, not degrees - hence the conversion. We `clear()` before each draw so the previous swing doesn't accumulate.

# --hints--
Lazy-create `this.swingTrail = this.add.graphics()`.

```js
assert.match(code, /this\.swingTrail\s*=\s*this\.add\.graphics\(\s*\)/);
```

Use `lineStyle(4, 0xffeeaa, 0.8)` and `arc(player.x, player.y, 36, ...)` with `Phaser.Math.DegToRad` for the angles.

```js
assert.match(code, /lineStyle\(\s*4\s*,\s*0xffeeaa\s*,\s*0\.8\s*\)[\s\S]*Phaser\.Math\.DegToRad\(\s*baseAngle\s*-\s*60\s*\)[\s\S]*Phaser\.Math\.DegToRad\(\s*baseAngle\s*\+\s*60\s*\)/);
```

Tween the trail's `alpha` to 0 over 200ms.

```js
assert.match(code, /this\.tweens\.add\(\s*\{[\s\S]*targets:\s*this\.swingTrail[\s\S]*alpha:\s*0[\s\S]*duration:\s*200/);
```

Call `this.swingTrail.clear()` before each draw so previous swings don't pile up.

```js
assert.match(code, /this\.swingTrail\.clear\(\s*\)/);
```
