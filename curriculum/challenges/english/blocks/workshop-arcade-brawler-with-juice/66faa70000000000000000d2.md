...
# --description--
Spawn enemies on a 2-second timer. Drop the initial wave to 3 (the timer will refill the room) and add a looping `time.addEvent`.
...
Because `this.spawnEnemy` is an arrow function, its `this` is already bound to the scene - we don't need `callbackScope`.

# --hints--
Drop the initial spawn count to 3.

```js
assert.match(code, /for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*3\s*;\s*i\s*\+=\s*1\s*\)\s*this\.spawnEnemy\(\s*\)/);
```

Save the timer to `this.spawnTimer` so step 19 can ramp it.

```js
assert.match(code, /this\.spawnTimer\s*=\s*this\.time\.addEvent/);
```

Set `delay: 2000`, `callback: this.spawnEnemy`, `loop: true`.

```js
assert.match(code, /delay:\s*2000[\s\S]*callback:\s*this\.spawnEnemy[\s\S]*loop:\s*true/);
```
