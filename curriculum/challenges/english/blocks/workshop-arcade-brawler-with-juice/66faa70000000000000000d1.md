...
# --description--
Render the score on a separate UIScene so HUD draw calls don't compete with the main scene's camera shake.

Add a `UIScene` class between `MainScene` and `const config`, and update the scene list to include it. Phaser only auto-starts the first scene in the array, so we set `active: true` in the constructor:
...
`setScrollFactor(0)` keeps the text pinned to the camera even during shake. The `??` fallback covers the one frame where MainScene exists but `score` hasn't been initialised yet.

# --hints--
Define `class UIScene extends Phaser.Scene`.

```js
assert.match(code, /class\s+UIScene\s+extends\s+Phaser\.Scene/);
```

In the constructor pass `{ key: "UIScene", active: true }` to `super` so it auto-starts.

```js
assert.match(code, /super\(\s*\{\s*key:\s*["\']UIScene["\']\s*,\s*active:\s*true\s*\}\s*\)/);
```

Create `this.scoreText` with `setScrollFactor(0)` so the HUD survives camera shake.

```js
assert.match(code, /this\.scoreText\s*=[\s\S]*setScrollFactor\(\s*0\s*\)/);
```

In `update`, read `this.scene.get("MainScene")` and write `this.scoreText.setText(`SCORE ${main.score ?? 0}`)`.

```js
assert.match(code, /this\.scene\.get\(\s*["\']MainScene["\']\s*\)[\s\S]*this\.scoreText\.setText\(\s*`SCORE\s*\$\{main\.score\s*\?\?\s*0\}`\s*\)/);
```

Add `UIScene` to the config's `scene` array.

```js
assert.match(code, /scene:\s*\[\s*MainScene\s*,\s*UIScene\s*\]/);
```
