...
Add `this.score += 5;` to the kill branch and `this.score += 1;` to the non-fatal branch:
...
```js
assert.match(code, /if\s*\(\s*enemy\.hp\s*<=\s*0\s*\)\s*\{[\s\S]*this\.score\s*\+=\s*5/);
```

Increment `this.score += 1` for non-fatal hits.

```js
assert.match(code, /\}\s*this\.score\s*\+=\s*1/);
```
