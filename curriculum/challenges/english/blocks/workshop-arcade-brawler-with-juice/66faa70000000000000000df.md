...
# --description--
When the boss dies (`enemy.hp <= 0 && enemy.isBoss`), play a long shake plus a chain of 8 staggered particle bursts. Insert this branch FIRST inside the `if (enemy.hp <= 0)` block:
...
Cinematics like this are the perfect place for restraint: a long 800ms shake feels meaningful only because regular hits have 60-120ms shakes. If every hit shook for a second the player would tune it out by hit 5.

# --hints--
Detect boss kill with `if (enemy.isBoss)` inside the `enemy.hp <= 0` branch.

```js
assert.match(code, /if\s*\(\s*enemy\.hp\s*<=\s*0\s*\)\s*\{[\s\S]*if\s*\(\s*enemy\.isBoss\s*\)/);
```

Award `+100` score and `cameras.main.shake(800, 0.02)`.

```js
assert.match(code, /this\.score\s*\+=\s*100[\s\S]*shake\(\s*800\s*,\s*0\.02\s*\)/);
```

Stagger 8 bursts with `this.time.delayedCall(i * 100, ...)` and `Phaser.Math.Between(-50, 50)` jitter.

```js
assert.match(code, /for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*8[\s\S]*delayedCall\(\s*i\s*\*\s*100[\s\S]*Phaser\.Math\.Between\(\s*-50\s*,\s*50\s*\)/);
```

Each burst is 50 particles and plays `sfx-explosion`.

```js
assert.match(code, /this\.fx\.explode\(\s*50[\s\S]*sfx-explosion/);
```
