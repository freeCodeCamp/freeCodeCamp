...
# --description--
Reduce `enemy.hp` by 1 each hit; on zero, run a heavier juice stack and destroy. The kill branch returns early so the recoil/tint/squash tweens never run on a destroyed enemy.
...
Each enemy spawns with `hp = 2`, so first hit triggers the full non-lethal stack and second hit triggers the destroy stack.

# --hints--
Decrement `enemy.hp -= 1` immediately after the active check.

```js
assert.match(code, /if\s*\(\s*!enemy\.active\s*\)\s*return\s*;[\s\S]*enemy\.hp\s*-=\s*1/);
```

When `enemy.hp <= 0`, play `sfx-explosion`, shake bigger (`120, 0.008`), explode `40` particles, and `enemy.destroy()`.

```js
assert.match(code, /if\s*\(\s*enemy\.hp\s*<=\s*0\s*\)\s*\{[\s\S]*sfx-explosion[\s\S]*shake\(\s*120\s*,\s*0\.008\s*\)[\s\S]*this\.fx\.explode\(\s*40[\s\S]*enemy\.destroy\(\s*\)/);
```

Return early after destroy so the rest of the juice doesn't run on a dead target.

```js
assert.match(code, /enemy\.destroy\(\s*\)\s*;\s*return\s*;/);
```
