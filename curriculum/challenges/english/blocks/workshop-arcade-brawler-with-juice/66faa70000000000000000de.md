...
# --description--
Add a boss enemy with `hp = 20` that spawns 20 seconds in. The existing `onHit` already handles HP - the boss soaks 20 hits before exploding.
...
We attach `boss.isBoss = true` so step 31 can branch the kill cinematic. The boss is in the same `this.enemies` group, so the existing hitbox overlap automatically targets it.

# --hints--
Define `this.spawnBoss = () => { ... }` as a closure on the scene.

```js
assert.match(code, /this\.spawnBoss\s*=\s*\(\s*\)\s*=>\s*\{/);
```

Spawn the boss as an `80×80` `0x880000` rectangle at `(400, 80)` with `setStrokeStyle(3, 0xffeeaa)`.

```js
assert.match(code, /\.rectangle\(\s*400\s*,\s*80\s*,\s*80\s*,\s*80\s*,\s*0x880000\s*\)[\s\S]*setStrokeStyle\(\s*3\s*,\s*0xffeeaa\s*\)/);
```

Set `boss.hp = 20` and `boss.isBoss = true`.

```js
assert.match(code, /boss\.hp\s*=\s*20[\s\S]*boss\.isBoss\s*=\s*true/);
```

Schedule the boss with `this.time.delayedCall(20000, () => this.spawnBoss())`.

```js
assert.match(code, /this\.time\.delayedCall\(\s*20000\s*,\s*\(\s*\)\s*=>\s*this\.spawnBoss\(\s*\)\s*\)/);
```
