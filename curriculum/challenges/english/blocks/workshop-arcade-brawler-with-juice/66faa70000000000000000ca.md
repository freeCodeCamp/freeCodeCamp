...
# --description--
Layer 3: tint flash. Insert a white flash that restores after 80ms (we use `setFillStyle` because our enemies are `Rectangle` objects - on `Sprite` you'd use `setTint` / `clearTint`).
...
The `if (enemy.active)` guard inside the delayedCall avoids calling `setFillStyle` on a destroyed enemy - destroy can happen before the 80ms timer fires (preview of step 15).

# --hints--
Set `enemy.setFillStyle(0xffffff)` immediately on hit.

```js
assert.match(code, /enemy\.setFillStyle\(\s*0xffffff\s*\)/);
```

Restore `enemy.setFillStyle(0xff5252)` after `80ms` with a `time.delayedCall`.

```js
assert.match(code, /this\.time\.delayedCall\(\s*80[\s\S]*enemy\.setFillStyle\(\s*0xff5252\s*\)/);
```

Guard the restore with `if (enemy.active)` so it tolerates a destroyed enemy.

```js
assert.match(code, /if\s*\(\s*enemy\.active\s*\)\s*enemy\.setFillStyle\(\s*0xff5252\s*\)/);
```
