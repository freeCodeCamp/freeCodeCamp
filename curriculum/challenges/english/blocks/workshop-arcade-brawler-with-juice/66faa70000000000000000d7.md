...
# --description--
Show the combo in the HUD with a growing scale + warming tint. Replace the body of `UIScene` with:
...
Step 38 will clamp the scale and tint so combo 50 doesn't take over the screen.

# --hints--
Add `this.comboText` at `(8, 36)` styled `#ffeeaa`.

```js
assert.match(code, /this\.comboText\s*=[\s\S]*\.text\(\s*8\s*,\s*36[\s\S]*ffeeaa/);
```

Read `main.combo ?? 0` and only render when `combo > 1`.

```js
assert.match(code, /const\s+combo\s*=\s*main\.combo\s*\?\?\s*0[\s\S]*if\s*\(\s*combo\s*>\s*1\s*\)/);
```

Scale the combo text with `setScale(1 + combo * 0.05)`.

```js
assert.match(code, /this\.comboText\.setScale\(\s*1\s*\+\s*combo\s*\*\s*0\.05\s*\)/);
```

Tint via `Phaser.Display.Color.GetColor(255, Math.max(80, 255 - combo * 12), 100)`.

```js
assert.match(code, /Phaser\.Display\.Color\.GetColor\(\s*255\s*,\s*Math\.max\(\s*80\s*,\s*255\s*-\s*combo\s*\*\s*12\s*\)\s*,\s*100\s*\)/);
```
