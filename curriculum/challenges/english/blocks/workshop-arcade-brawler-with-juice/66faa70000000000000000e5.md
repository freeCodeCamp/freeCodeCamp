...
# --description--
By default, enemies in the same group don't collide. Add a self-collider so they bounce off each other instead of stacking on the same pixel.

After the boss spawn block (`/* create-world */`), add:

```js
this.physics.add.collider(this.enemies, this.enemies);
```

Phaser's collider applies symmetric momentum exchange. Without this, every enemy pursues the player and they pile up - making the hitbox overlap fire 5+ times per swing.

# --hints--
Call `this.physics.add.collider(this.enemies, this.enemies)`.

```js
assert.match(code, /this\.physics\.add\.collider\(\s*this\.enemies\s*,\s*this\.enemies\s*\)/);
```
