...
# --description--
Add on-screen ATK/HEAVY buttons for touch users. They live on the UIScene so they're pinned to the camera and survive shake.

Add a method `attachTouchControls()` to `UIScene`, call it from `create`, and the touch buttons forward to MainScene's `attack`:
...
We could conditionally show the buttons on touch devices only, but always-visible is simpler and lets desktop users click them.

# --hints--
Define `attachTouchControls()` on `UIScene` that fetches `this.scene.get("MainScene")`.

```js
assert.match(code, /attachTouchControls\s*\(\s*\)\s*\{[\s\S]*this\.scene\.get\(\s*["\']MainScene["\']\s*\)/);
```

Render an ATK rectangle at `(720, 540, 120, 80, 0x6c8aff, 0.6)`.

```js
assert.match(code, /\.rectangle\(\s*720\s*,\s*540\s*,\s*120\s*,\s*80\s*,\s*0x6c8aff\s*,\s*0\.6\s*\)/);
```

Render a HEAVY rectangle at `(580, 540, 100, 80, 0xff5252, 0.6)`.

```js
assert.match(code, /\.rectangle\(\s*580\s*,\s*540\s*,\s*100\s*,\s*80\s*,\s*0xff5252\s*,\s*0\.6\s*\)/);
```

Wire ATK to `main.attack()` and HEAVY to `main.attack(true)` via `pointerdown`.

```js
assert.match(code, /atk\.on\(\s*["\']pointerdown["\']\s*,\s*\(\s*\)\s*=>\s*main\.attack\s*&&\s*main\.attack\(\s*\)\s*\)[\s\S]*heavy\.on\(\s*["\']pointerdown["\']\s*,\s*\(\s*\)\s*=>\s*main\.attack\s*&&\s*main\.attack\(\s*true\s*\)\s*\)/);
```

Call `this.attachTouchControls()` at the end of UIScene's `create()`.

```js
assert.match(code, /this\.attachTouchControls\(\s*\)/);
```
