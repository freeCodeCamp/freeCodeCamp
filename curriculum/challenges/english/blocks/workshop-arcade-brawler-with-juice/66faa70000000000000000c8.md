...
# --description--
Layer 1 of the six-stack juice recipe: an audio cue. Replace the `this.onHit` closure body with:
...
Click an enemy. Hear the punch. Audio is the cheapest, fastest win in the juice stack - it lands before the eye finishes parsing the visual change.

# --hints--
Keep the early return on inactive enemies.

```js
assert.match(code, /if\s*\(\s*!enemy\.active\s*\)\s*return\s*;/);
```

Play `"sfx-hit"` with `{ volume: 0.5 }`.

```js
assert.match(code, /this\.sound\.play\(\s*["\']sfx-hit["\']\s*,\s*\{\s*volume:\s*0\.5\s*\}\s*\)/);
```
