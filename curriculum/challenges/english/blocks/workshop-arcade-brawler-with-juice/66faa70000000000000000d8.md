...
# --description--
Add a heavy attack on the `K` key that signals stronger juice downstream. We change the closure signature to `(heavy = false)`, stash the flag on `this.heavyMode`, and beef up the visuals (thicker arc, larger sword, longer windup).

Replace the entire `this.attack` closure AND its listener block with:
...
`heavy` toggles every juice ingredient: thicker trail (8 vs 4), wider arc radius (48 vs 36), 1.5× sword scale, slower swing (220 vs 150ms), and a heavier audio cue. Step 25 reads `this.heavyMode` to scale the shake.

# --hints--
Change the signature to `this.attack = (heavy = false) => { ... }`.

```js
assert.match(code, /this\.attack\s*=\s*\(\s*heavy\s*=\s*false\s*\)\s*=>/);
```

Track `this.heavyMode = heavy;` so `onHit` can read it.

```js
assert.match(code, /this\.heavyMode\s*=\s*heavy\s*;/);
```

Reset `this.heavyMode = false` inside the cooldown `delayedCall`.

```js
assert.match(code, /this\.heavyMode\s*=\s*false/);
```

Listen for `keydown-K` and call `this.attack(true)`.

```js
assert.match(code, /this\.input\.keyboard\.on\(\s*["\']keydown-K["\']\s*,\s*\(\s*\)\s*=>\s*this\.attack\(\s*true\s*\)\s*\)/);
```

Vary the SFX between `"sfx-jump"` (heavy) and `"sfx-ui-click"` (light).

```js
assert.match(code, /heavy\s*\?\s*["\']sfx-jump["\']\s*:\s*["\']sfx-ui-click["\']/);
```
