...
# --description--
Track a combo counter that bumps on every hit and resets after 2 seconds without one.

Insert RIGHT AFTER the active check in `onHit`:
...
Each hit re-arms the reset timer; if the player keeps hitting, the timer never fires. Step 23 wires the combo into the HUD.

# --hints--
Increment `this.combo += 1` on every hit.

```js
assert.match(code, /this\.combo\s*\+=\s*1/);
```

Cancel any existing `this._comboReset.remove()` before scheduling a new one.

```js
assert.match(code, /if\s*\(\s*this\._comboReset\s*\)\s*this\._comboReset\.remove\(\s*\)/);
```

Reset combo to 0 with `this.time.delayedCall(2000, () => { this.combo = 0; })`.

```js
assert.match(code, /this\._comboReset\s*=\s*this\.time\.delayedCall\(\s*2000[\s\S]*this\.combo\s*=\s*0/);
```
