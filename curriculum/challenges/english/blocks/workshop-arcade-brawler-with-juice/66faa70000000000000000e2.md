...
# --description--
Add a mute toggle to the HUD plus the `M` key as a shortcut. Also surface the `BEST` score in the corner.
...
`this.sound` is the global `BaseSoundManager`, so muting from UIScene affects MainScene too.

# --hints--
Read mute state via `this.sound.mute` and toggle on click.

```js
assert.match(code, /this\.sound\.mute\s*=\s*!this\.sound\.mute/);
```

Display BEST score with `this.bestText` pinned to the right via `setOrigin(1, 0)`.

```js
assert.match(code, /this\.bestText\s*=[\s\S]*setOrigin\(\s*1\s*,\s*0\s*\)/);
```

Wire `this.muteText.on("pointerup", toggleMute)` and `this.input.keyboard.on("keydown-M", toggleMute)`.

```js
assert.match(code, /this\.muteText\.on\(\s*["\']pointerup["\']\s*,\s*toggleMute\s*\)[\s\S]*this\.input\.keyboard\.on\(\s*["\']keydown-M["\']\s*,\s*toggleMute\s*\)/);
```

Update the BEST text every frame from `main.bestScore ?? 0`.

```js
assert.match(code, /this\.bestText\.setText\(\s*`BEST\s*\$\{main\.bestScore\s*\?\?\s*0\}`\s*\)/);
```
