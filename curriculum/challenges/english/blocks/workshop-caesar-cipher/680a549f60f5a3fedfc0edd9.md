...
Modify the `shifted_alphabet` variable and use the slicing syntax to extract the missing first portion of `alphabet` and concatenate  it to `alphabet[shift:]`. 
...
You should assign the concatenation of `alphabet[shift:]` and the missing first portion of `alphabet` to the `shifted_alphabet` 
variable on the same line it was declared. Use `shift` to specify where to stop the slicing.
