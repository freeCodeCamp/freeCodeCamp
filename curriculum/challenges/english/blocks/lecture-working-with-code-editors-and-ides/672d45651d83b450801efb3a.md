...
Once the command line is open, navigate to the home directory. If you are using Command Prompt on Windows, type `cd %USERPROFILE%` and hit `enter` on your keyboard. If you are using PowerShell or macOS Terminal, type `cd ~` and hit `enter`. Then type in the `mkdir my-project` command and hit `enter`. `mkdir`, or "make directory" is the command used to create a new directory. You should then open your new directory with VS Code.
