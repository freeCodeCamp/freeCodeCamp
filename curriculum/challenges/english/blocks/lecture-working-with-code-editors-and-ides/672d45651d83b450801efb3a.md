...
Once the command line is open, navigate to your home directory. In Command Prompt, type `cd %USERPROFILE%`. In PowerShell or the macOS Terminal, type `cd ~`. Then type in the `mkdir my-project` command and hit `Enter`. `mkdir`, or "make directory", is the command used to create a new directory. You should then open your new directory with VS Code. 
