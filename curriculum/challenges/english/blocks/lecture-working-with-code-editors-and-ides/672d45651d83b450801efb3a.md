...
On macOS, the `code` command isn't available in the Terminal until you enable it. Open VS Code, press <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>P</kbd> to open the Command Palette, then search for and run `Shell Command: Install 'code' command in PATH`. After that, the `code` command will work from the Terminal.
...
## Running Your Code with Live Server
