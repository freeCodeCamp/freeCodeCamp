...
Use `Ctrl + Shift + F` (Windows/Linus), or `Cmd + Shift + F` (macOS), to search the text contents of all the files in your workspace. Then `Ctrl + Shift + H` (Windows/Linus), or `Cmd + Shift + H` (macOS), if you want to run a find-and-replace.

If you need to remove a line, `Ctrl + Shift + K` (Windows/Linus), or `Cmd + Shift + K` (macOS), will delete it.

Need some extra room for all your code? `Ctrl + B` (Windows/Linus), or `Cmd + B` (macOS), will hide the sidebar, which has the file list and extensions menu.

Or maybe you just can't see your code? `Ctrl + plus` (Windows/Linus), or `Cmd + plus` (macOS), will increase the scaling of the editor, and `Ctrl + minus` (Windows/Linus), or `Cmd + minus` (macOS), will decrease it.
...
Finally, if you forget any of these shortcuts, you always have `Ctrl + Shift + P` (Windows/Linus), or `Cmd + Shift + P` (macOS), which opens the command palette for you to select whatever you may need.
