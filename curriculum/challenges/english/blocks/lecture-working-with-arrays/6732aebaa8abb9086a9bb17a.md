...
In the previous lesson, you were introduced to working with arrays and accessing different elements in arrays. Here is a reminder of how to access the second element in array:
...
In this example, we replaced `"banana"` with `"blueberry"` at index `1`. This method allows you to change any element in the array, as long as you know its index. You can also add new elements to an array by assigning a value to an index that does not yet exist:
