...
```js
assert.equal(document.querySelector('.business-card a.portfolio-link')?.textContent, 'Portfolio');
```

Your portfolio `a` element should have a valid `href`.

```js
assert.isAbove(document.querySelector('.business-card a.portfolio-link').href.length, 0);
```

After the `a` element containing your portfolio, there should be another `hr` element as a divider.

```js
assert.exists(document.querySelector('.business-card a.portfolio-link + hr'));
```
