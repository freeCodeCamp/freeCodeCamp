...
```js
assert.isNotEmpty(document.querySelectorAll('.business-card hr + p')[0].textContent.trim());
```
...
```js
assert.equal(document.querySelector('a.portfolio-link').textContent.trim(), 'Portfolio');
```
