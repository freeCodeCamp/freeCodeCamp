...
Within your `.image-wrapper` element, give your third `img` element a `src` of `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`, an `alt` of `four people collaborating on code`, a `loading` attribute of `lazy`, a `class` set to `image-3`, a `width` attribute set to `600`, and a `height` attribute set to `400`.
...
Your third `img` element should have an `alt` set to `four people collaborating on code`.

```js
assert.equal(
  document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('alt'),
  'four people collaborating on code'
);
```
