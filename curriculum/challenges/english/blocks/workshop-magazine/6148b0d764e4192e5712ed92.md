...
Set the `display` property of your `.hero` selector to `grid`.

# --hints--

Your `.hero` selector should have a `display` property set to `grid`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero')?.display, 'grid');
```
