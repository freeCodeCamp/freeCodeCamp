...
Within your `body`, create a `main` element. Inside the `main` element, create an `article` element to contain the magazine article. Then, inside the `article`, create a `section` with a `class` set to `heading`.
...
```js
assert.equal(document.querySelector('article')?.parentElement?.localName, 'main');
```

Your `section` element should be within your `article` element.

```js
assert.equal(document.querySelector('section')?.parentElement?.localName, 'article');
```
