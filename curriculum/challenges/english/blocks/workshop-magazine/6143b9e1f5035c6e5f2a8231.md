...
Within your `ul` element, create six `li` elements. Add an `h3` element with a `class` set to `list-subtitle` and a `p` element to each of your `li` elements.

Then give the `h3` and `p` elements the following text content, in order, with each `h3` using what's on the left side of the colon, and each `p` using what's on the right:
...
Each of your new `li` elements should have an `h3` and `p` element.

```js
const lis = document.querySelectorAll('.lists li');
assert.isNotEmpty(lis);
lis.forEach(li => {
  assert.equal(li?.children?.[0]?.localName, 'h3');
  assert.equal(li?.children?.[1]?.localName, 'p');
});
```

Your first `h3` should have the text `V1 - 2014`.

```js
assert.equal(document.querySelectorAll('.lists li h3')?.[0]?.innerText, 'V1 - 2014');
```
...
Your second `h3` should have the text `V2 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h3')?.[1]?.innerText, 'V2 - 2015');
```
...
Your third `h3` should have the text `V3 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h3')?.[2]?.innerText, 'V3 - 2015');
```
...
Your fourth `h3` should have the text `V4 - 2016`.

```js
assert.equal(document.querySelectorAll('.lists li h3')?.[3]?.innerText, 'V4 - 2016');
```
...
Your fifth `h3` should have the text `V5 - 2017`.

```js
assert.equal(document.querySelectorAll('.lists li h3')?.[4]?.innerText, 'V5 - 2017');
```
...
Your sixth `h3` should have the text `V6 - 2018`.

```js
assert.equal(document.querySelectorAll('.lists li h3')?.[5]?.innerText, 'V6 - 2018');
```
...
Your six `h3` elements should each have the class `list-subtitle`.

```js
const h3s = document.querySelectorAll('.lists li h3');
assert.isNotEmpty(h3s);
h3s.forEach(h3 => {
  assert.isTrue(h3?.classList?.contains('list-subtitle'));
});
```
