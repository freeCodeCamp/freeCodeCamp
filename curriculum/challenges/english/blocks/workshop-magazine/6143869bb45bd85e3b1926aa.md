...
Within your `article` element, create a `header` element with the `class` set to `hero`.
...
Your `header` element should be within your `article` element.

```js
assert.equal(document.querySelector('header')?.parentElement?.localName, 'article');
```
