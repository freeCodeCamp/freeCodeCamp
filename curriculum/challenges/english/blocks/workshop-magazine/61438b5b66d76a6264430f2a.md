...
Because these icons are purely decorative — the link text is already provided by the `aria-label` on the parent `a` — each `i` element should have `aria-hidden` set to `true`. This prevents screen readers from announcing the icon, avoiding duplicate or confusing output.
