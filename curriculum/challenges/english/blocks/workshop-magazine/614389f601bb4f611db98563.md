---
id: 614389f601bb4f611db98563
title: Step 8
challengeType: 0
dashedName: step-8
---

# --description--

Below your `.publish-date` element, create a new `nav` element with the class `social-icons` and an `aria-label` attribute set to `Social media links`.

The `nav` element is a semantic landmark used to mark a section of navigation links. Adding an `aria-label` distinguishes it from other navigation regions on the page, so screen reader users know exactly what this group of links is for.

Add five `a` elements within that new `nav`, and give them the following `href` attributes.

- The first `a` element should have an `href` set to `https://www.facebook.com/freecodecamp`.
- The second `a` element should have an `href` set to `https://twitter.com/freecodecamp`.
- The third `a` element should have an `href` set to `https://instagram.com/freecodecamp`.
- The fourth `a` element should have an `href` set to `https://www.linkedin.com/school/free-code-camp`.
- The fifth `a` element should have an `href` set to `https://www.youtube.com/freecodecamp`.

# --hints--

You should create a `nav` element.

```js
assert.exists(document.querySelector('nav'));
```

Your `nav` element should come after your `.publish-date` element.

```js
assert.equal(document.querySelector('.publish-date')?.nextElementSibling?.localName, 'nav');
```

Your `nav` element should have the class `social-icons`.

```js
assert.isTrue(document.querySelector('.publish-date')?.nextElementSibling?.classList?.contains('social-icons'));
```

Your `nav` element should be within your `header` element.

```js
assert.equal(document.querySelector('nav.social-icons')?.parentElement?.localName, 'header');
```

Your `nav` element should have an `aria-label` attribute set to `Social media links`.

```js
assert.equal(document.querySelector('nav.social-icons')?.getAttribute('aria-label'), 'Social media links');
```

Your `.social-icons` element should have five `a` elements.

```js
assert.lengthOf(document.querySelector('.social-icons')?.querySelectorAll('a'), 5);
```

Your first `a` element should have an `href` set to `https://www.facebook.com/freecodecamp`.

```js
assert.include(document.querySelector('.social-icons')?.querySelectorAll('a')?.[0]?.getAttribute('href'), 'https://www.facebook.com/freecodecamp');
```

Your second `a` element should have an `href` set to `https://twitter.com/freecodecamp`.

```js
assert.include(document.querySelector('.social-icons')?.querySelectorAll('a')?.[1]?.getAttribute('href'), 'https://twitter.com/freecodecamp');
```

Your third `a` element should have an `href` set to `https://instagram.com/freecodecamp`.

```js
assert.include(document.querySelector('.social-icons')?.querySelectorAll('a')?.[2]?.getAttribute('href'), 'https://instagram.com/freecodecamp');
```

Your fourth `a` element should have an `href` set to `https://www.linkedin.com/school/free-code-camp`.

```js
assert.include(document.querySelector('.social-icons')?.querySelectorAll('a')?.[3]?.getAttribute('href'), 'https://www.linkedin.com/school/free-code-camp');
```

Your fifth `a` element should have an `href` set to `https://www.youtube.com/freecodecamp`.

```js
assert.include(document.querySelector('.social-icons')?.querySelectorAll('a')?.[4]?.getAttribute('href'), 'https://www.youtube.com/freecodecamp');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Magazine</title>
    <link
      href="https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap"
      rel="stylesheet"
    />
    <link
      rel="stylesheet"
      href="https://use.fontawesome.com/releases/v5.8.2/css/all.css"
    />
    <link rel="stylesheet" href="styles.css" />
  </head>
  <body>
    <main>
      <article>
        <section class="heading">
        <header class="hero">
          <img
            src="https://cdn.freecodecamp.org/platform/universal/fcc_meta_1920X1080-indigo.png"
            alt="freecodecamp logo"
            loading="lazy"
            class="hero-img"
            width="400"
          />
          <hgroup class="hero-title">
            <h1>OUR NEW CURRICULUM</h1>
            <p class="hero-subtitle">
            Our efforts to restructure our curriculum with a more project-based
            focus
            </p>
          </hgroup>
          <address class="author">
          <p class="author-name">
            By
            <a href="https://freecodecamp.org" target="_blank" rel="author noreferrer"
              >freeCodeCamp</a
            >
          </p>
          </address>
          <p class="publish-date"><time datetime="2019-03-07">March 7, 2019</time></p>
--fcc-editable-region--
        
--fcc-editable-region--
        </header>
        </section>
      </article>
    </main>
  </body>
</html>
```

```css

```
