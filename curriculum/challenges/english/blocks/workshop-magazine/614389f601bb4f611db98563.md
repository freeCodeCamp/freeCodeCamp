...
Below your `.author` element, create a new `nav` element with the class `social-icons` and an `aria-label` attribute set to `Social media links`.
...
Your `nav` element should come after your `.author` element.

```js
assert.equal(document.querySelector('.author')?.nextElementSibling?.localName, 'nav');
```
