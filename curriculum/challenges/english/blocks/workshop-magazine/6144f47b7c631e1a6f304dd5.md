...
Create a `.hero` rule and set the `grid-column` property to `2 / 3`. This will tell the `.hero` element to start at grid line 2 and end at grid line 3.
...
Your `.hero` selector should have a `grid-column` property set to `2 / 3`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero')?.gridColumn, '2 / 3');
```
