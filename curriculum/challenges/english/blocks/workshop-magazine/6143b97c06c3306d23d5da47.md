...
Within your `article` element, create an `h2` element with the `class` set to `list-title` and the text of `A Brief History`. Below that, create a `p` element with the text `Of the Curriculum`. Then create a `ul` element with the class `lists`.

# --hints--

You should create an `h2` element within your `article` element.

```js
assert.exists(document.querySelector('article h2'));
```
...
```js
assert.equal(document.querySelector('article h2')?.className, 'list-title');
```

Your new `h2` element should have the text of `A Brief History`.

```js
assert.equal(document.querySelector('article h2')?.innerText, 'A Brief History');
```
