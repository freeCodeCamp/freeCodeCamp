...
The `article` element is a grid container too. Create a `main > article` selector. Give it a `display` property set to `grid`, a `grid-column` property set to `1 / -1`, a `grid-template-columns` property set to `inherit`, and a `row-gap` property set to `inherit`. This keeps the article as a semantic container while allowing its child sections to use the same grid tracks and spacing as `main`.
...
You should have a `main > article` selector.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main > article'));
```

Your `main > article` selector should have a `display` property set to `grid`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('main > article')?.display, 'grid');
```

Your `main > article` selector should span all of the `main` element's grid columns.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('main > article')?.gridColumn, '1 / -1');
```

Your `main > article` selector should inherit the `main` element's grid columns.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('main > article')?.gridTemplateColumns, 'inherit');
```

Your `main > article` selector should inherit the `main` element's row gap.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('main > article')?.rowGap, 'inherit');
```
    ...
    <main>
      ...
      <section class="text text-with-images">
        <section class="brief-history">
          <h2 class="list-title">A Brief History</h2>
          <p>Of the Curriculum</p>
          <ul class="lists">
            <li>
              <h3 class="list-subtitle">V1 - 2014</h3>
              <p>
            ...
            <li>
              <h3 class="list-subtitle">V2 - 2015</h3>
              <p>We added interactive algorithm challenges.</p>
            </li>
            <li>
              <h3 class="list-subtitle">V3 - 2015</h3>
              <p>
            ...
            <li>
              <h3 class="list-subtitle">V4 - 2016</h3>
              <p>
            ...
            <li>
              <h3 class="list-subtitle">V5 - 2017</h3>
              <p>We added the back-end and data visualization challenges.</p>
            </li>
            <li>
              <h3 class="list-subtitle">V6 - 2018</h3>
              <p>
        ...
        <aside class="image-wrapper">
          <img
            ...
            alt="Random Quote Machine project screenshot"
            loading="lazy"
            class="image-1"
            width="600"
            height="400"
          ...
          <img
            ...
            alt="four people collaborating on code"
            loading="lazy"
            class="image-3"
            width="600"
            height="400"
