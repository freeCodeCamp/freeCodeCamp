...
Create a `.hero-img` selector and give it a `grid-column` property set to `1 / -1`. This will tell the image to span the full width of the grid.

# --hints--

You should have a `.hero-img` selector.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-img'));
```

Your `.hero-img` selector should have a `grid-column` property set to `1 / -1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-img')?.gridColumn, '1 / -1');
```
