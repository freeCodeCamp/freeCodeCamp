...
Give your `.hero` selector a `row-gap` property set to `1.5rem`.

# --hints--

Your `.hero` selector should have a `row-gap` property set to `1.5rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero')?.rowGap, '1.5rem');
```
