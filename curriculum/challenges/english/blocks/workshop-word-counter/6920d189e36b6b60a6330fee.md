...
assert.exists(func, 'printCharacters function should exist');

const normalizedFunctionCode = __helpers.removeWhiteSpace(__helpers.removeJSComments(func.toString()));
const loopMatch = normalizedFunctionCode.match(/for\(constcharofstr\)\{(\S*?)}/);
assert.exists(loopMatch, 'printCharacters should contain a for...of loop using const char of str');

assert.match(loopMatch[1], /console\.log\(char\)/);
```
