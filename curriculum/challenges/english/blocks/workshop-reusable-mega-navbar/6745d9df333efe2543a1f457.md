...
```js
assert.exists(document.querySelector('li:nth-of-type(3) ul'));
```

Your `ul` element should have a `className` of `sub-menu`.

```js
assert.equal(document.querySelector('li:nth-of-type(3) ul')?.getAttribute('class'), 'sub-menu');
```

Your `ul` element should have an `aria-label` of `Apps`.

```js
assert.equal(document.querySelector('li:nth-of-type(3) ul')?.getAttribute('aria-label'), 'Apps');
```
