...
```js
assert.exists(document.querySelectorAll('.navbar > ul > li')[2]?.querySelector('ul'));
```

Your `ul` element should have a `className` of `sub-menu`.

```js
assert.equal(document.querySelectorAll('.navbar > ul > li')[2]?.querySelector('ul')?.getAttribute('class'), 'sub-menu');
```

Your `ul` element should have an `aria-label` of `Apps`.

```js
assert.equal(document.querySelectorAll('.navbar > ul > li')[2]?.querySelector('ul')?.getAttribute('aria-label'), 'Apps');
```
