...
```js
assert.exists(document.querySelector('li:nth-of-type(1) a'));
```

Your `a` element should have an `href` attribute of `#`.

```js
assert.equal(document.querySelector('li:nth-of-type(1) a')?.getAttribute('href'), '#');
```

Your `a` element should have the text `Dashboard`.

```js
assert.equal(document.querySelector('li:nth-of-type(1) a')?.textContent, 'Dashboard');
```
