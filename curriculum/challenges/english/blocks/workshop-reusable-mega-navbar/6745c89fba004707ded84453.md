...
```js
assert.exists(document.querySelectorAll('ul > li')[0]?.querySelector('a'));
```

Your `a` element should have an `href` attribute of `#`.

```js
assert.equal(document.querySelectorAll('ul > li')[0]?.querySelector('a')?.getAttribute('href'), '#');
```

Your `a` element should have the text `Dashboard`.

```js
assert.equal(document.querySelectorAll('ul > li')[0]?.querySelector('a')?.textContent, 'Dashboard');
```
