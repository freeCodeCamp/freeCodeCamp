...
```js
assert.exists(document.querySelector('li:nth-of-type(2) a'));
```

Your `a` element should have an `href` attribute of `#`.

```js
assert.equal(document.querySelector('li:nth-of-type(2) a')?.getAttribute('href'), '#');
```

Your `a` element should have the text `Widgets`.

```js
assert.equal(document.querySelector('li:nth-of-type(2) a')?.textContent, 'Widgets');
```
