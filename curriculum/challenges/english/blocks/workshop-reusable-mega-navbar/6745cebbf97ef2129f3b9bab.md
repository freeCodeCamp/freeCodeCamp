...
```js
assert.exists(document.querySelectorAll('ul > li')[1]?.querySelector('a'));
```

Your `a` element should have an `href` attribute of `#`.

```js
assert.equal(document.querySelectorAll('ul > li')[1]?.querySelector('a')?.getAttribute('href'), '#');
```

Your `a` element should have the text `Widgets`.

```js
assert.equal(document.querySelectorAll('ul > li')[1]?.querySelector('a')?.textContent, 'Widgets');
```
