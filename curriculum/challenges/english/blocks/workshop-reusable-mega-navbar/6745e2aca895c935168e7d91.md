...
To finish things up, set the text of the first submenu `a` element to `Calendar`, the second to `Chat`, and the third to `Email`.

With that, your `Navbar` component is complete!

# --hints--

You should set the text of the first submenu `a` element to `Calendar`.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

You should set the text of the second submenu `a` element to `Chat`.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

You should set the text of the third submenu `a` element to `Email`.
