...
This lets you extract values that are inside another object.
...
// Extracting the nested "flour" value
...
```js
// The pattern on the left mirrors the object’s shape.
// Here is the same destructuring with comments:

const {
  ingredients: { // look inside the "ingredients" property
    flour        // then extract the "flour" value into a variable named flour
  }
} = recipe;

// This “mirroring” simply means writing your destructuring pattern in the same nested structure as the object you are taking values from.
```
