...
await __helpers.retryingTest(
  () => logSpy.calls.some((call) => call[0] === "Author Data Array:"),
  'console.log was not called with the text "Author Data Array:"'
);
```
...
```js
const loggedCall = logSpy.calls.find((call) => call[0] === "Author Data Array:");
assert.isAtLeast(loggedCall?.length, 2);
```
...
```js
const loggedCall = logSpy.calls.find((call) => call[0] === "Author Data Array:");
assert.deepEqual(loggedCall?.[1], testData);
```
