...
At this point, the behavior of the app should not have changed. The code still fetches a file from the CDN and then processes the retrieved data into `JSON`. Once that's done, it saves that data into the `authorDataArr` variable and then calls `displayAuthors`. 
...
const { res } = initialFetch?.variables ?? {};
const url =
  'https://cdn.freecodecamp.org/curriculum/news-author-page/authors.json';
const validExpressions = ["'", '"', '`'].map(
  (quote) => `await fetch(${quote}${url}${quote})`
);
assert.isTrue(res?.toString().startsWith('const'));
assert.isTrue(
  validExpressions.some((expression) => res?.value?.matches(expression))
);
```
