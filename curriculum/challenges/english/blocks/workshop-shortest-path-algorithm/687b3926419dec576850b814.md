...
({
  test: () => {
    assert(runPython(`_Node(_code).find_function("shortest_path").has_stmt("if new_distance < distances[node_no]:")
    `))
  }
})
```
...
({
  test: () => {
    assert(runPython(`_Node(_code).find_function("shortest_path").has_stmt("distances[node_no] = new_distance")
    `))
  }
})
```
