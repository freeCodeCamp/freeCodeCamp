...
```md
\n[starting node]-[node number] length: [distance]\nPath: [path]
```
...
func_str = str(func)
"print(" in func_str and "start_node" in func_str and "node_no" in func_str and "distances[node_no]" in func_str and "path" in func_str
    `))
  }
})
```
