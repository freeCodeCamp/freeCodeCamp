...
({
  test: () => {
    assert(runPython(`"targets" in _code`))
  }
})
```
...
({
  test: () => {
    assert(runPython(`"[target_node]" in _code`))
  }
})
```
...
({
  test: () => {
    assert(runPython(`"targets = " in _code and " if " in _code and " else " in _code`))
  }
})
```
...
({
  test: () => {
    assert(runPython(`"target_node is not None" in _code`))
  }
})
```
...
({
  test: () => {
    assert(runPython(`"range(n)" in _code`))
  }
})
```
