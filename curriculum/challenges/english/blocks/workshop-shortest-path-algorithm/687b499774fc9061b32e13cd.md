...
({
  test: () => {
  assert(runPython(`"return" in _code`))
 }
})
```

You should return `distances`.

```js
({
  test: () => {
  assert(runPython(`"return distances" in _code`))
 }
})
```

You should return `paths`.

```js
({
  test: () => {
  assert(runPython(`"return distances, paths" in _code`))
 }
})
```
