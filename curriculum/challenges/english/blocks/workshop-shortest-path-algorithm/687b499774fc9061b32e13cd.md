...
({
  test: () => {
    assert(runPython(`_Node(_code).find_function("shortest_path").find_return()`))
  }
})
```
...
({
  test: () => {
    assert(runPython(`_Node(_code).find_function("shortest_path").find_return().is_equivalent("return distances, paths")`))
  }
})
````
