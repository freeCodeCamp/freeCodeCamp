...
It's important to note that async scripts are executed as soon as they're downloaded, which means they might not run in the order we desire. This is where the `defer` attribute comes to the rescue. Let's look at what the `defer` attribute looks like:
```js
