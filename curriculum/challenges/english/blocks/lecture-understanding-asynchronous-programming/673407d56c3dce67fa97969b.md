...
:::interactive_editor
```js
// This example demonstrates how async/await waits for Promises.
// delayedGreeting returns a Promise that resolves after 1 second.
...
// Using async/await - the function pauses at 'await' until the Promise resolves
async function greetWithAwait(name) {
  const message = await delayedGreeting(name);
  console.log('With await:', message);
}

// Not using await - returns immediately with a pending Promise
function greetWithoutAwait(name) {
  const promise = delayedGreeting(name);
  console.log('Without await (immediate):', promise);
  promise.then((message) => console.log('Without await (later):', message));
}

// Run both to compare
greetWithAwait('Ada');
greetWithoutAwait('Ada');
```
:::

One of the biggest advantages of `async`/`await` is error handling. With promises, we often had to use the `.catch()` method to handle errors. With `async`/`await`, we can use `try`/`catch` blocks. Here's an example:
