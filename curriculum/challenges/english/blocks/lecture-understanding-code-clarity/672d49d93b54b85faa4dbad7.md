...
However, ASI does not simply insert a semicolon after every line break. In some cases, an inserted semicolon can cause unexpected behavior. For example:
...
In this example, ASI inserts a semicolon after `return`, so the function returns `undefined` instead of `42`. Using semicolons explicitly can make statement boundaries clearer and help prevent subtle errors related to ASI.
