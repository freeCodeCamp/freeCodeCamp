...
In this code, the semicolons make the end of each statement explicit. JavaScript also has Automatic Semicolon Insertion (ASI), which can insert semicolons automatically in certain situations. For example, the following code is valid without explicit semicolons:
...
However, ASI does not simply insert a semicolon after every line break. In some cases, omitting a semicolon can cause unexpected behavior. For example:
...
In this example, ASI does not insert a semicolon after the `const` declaration before the function call. The code is therefore interpreted differently than if the two statements were separated explicitly. Adding a semicolon makes the statement boundary clear:
...
Using semicolons explicitly can make statement boundaries clearer and help prevent subtle errors related to ASI.
