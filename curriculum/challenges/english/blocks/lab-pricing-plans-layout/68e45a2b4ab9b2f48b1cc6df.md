...
assert.exists(h1);
assert.equal(h1.textContent, 'Pricing Plans', 'The h1 has the text Pricing Plans');
```

You should have three `div` elements with the class `pricing-card`

```js
const div = document.querySelectorAll('div.pricing-card');
assert.exists(div);
...
```js
const div = document.querySelector('div.pricing-card.basic-plan');
assert.exists(div);
```
...
```js
const div = document.querySelector('div.pricing-card.premium-plan');
assert.exists(div);
```
...
```js
const div = document.querySelector('div.pricing-card.pro-plan');
assert.exists(div);
```
