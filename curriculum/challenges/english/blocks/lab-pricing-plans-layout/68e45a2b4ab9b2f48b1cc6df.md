...
2. Your page should have a `div` element with the class `pricing-container`.
    - The `div` element should have a `display` property with the value of `flex`.
    - The `div` element should have a `flex-wrap` property with the value of `wrap`.
...
One of your `div` elements should have the class `pricing-card` and the class `basic-plan`.

```js
assert.exists(document.querySelector('div.pricing-card.basic-plan'));
```

One of your `div` elements should have the class `pricing-card` and the class `premium-plan`.

```js
assert.exists(document.querySelector('div.pricing-card.premium-plan'));
```

One of your `div` elements should have the class `pricing-card` and the class `pro-plan`.
