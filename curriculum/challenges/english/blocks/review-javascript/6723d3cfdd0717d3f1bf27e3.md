...
- The **`slice()` Method**:  This method returns a new value containing a portion of the original value, determined by the specified start and end indices. It does not modify the original value. The start index is included, while the end index is excluded. For strings, it returns a new string containing the selected characters. For arrays, it returns a shallow copy of the selected elements, meaning object references are copied rather than duplicated.
...
- **`m` Flag**: Anchors look for the beginning and end of the entire string. But you can make a regex handle multiple lines with the `m` flag, or the multi-line modifier.flag.
