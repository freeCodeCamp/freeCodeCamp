...
- **The `RegExp` Constructor**: Regular expressions can also be created using the `RegExp` constructor instead of a literal. This is useful when the pattern or flags aren't known ahead of time—for example, when they come from a variable or user input.
