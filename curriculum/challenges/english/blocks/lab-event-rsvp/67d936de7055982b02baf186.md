...
After submission, changing an input value should not update the confirmation message.

```js
  try {
    const nameInp = document.querySelector('input[type="text"][required]');
    assert.exists(nameInp);
    const setter = Object.getOwnPropertyDescriptor(
      window.HTMLInputElement.prototype,
      'value'
    ).set;
    await React.act(async () => {
      await adjustAllRequired();
      setter.call(nameInp, 'John Doe');
      nameInp.dispatchEvent(new Event('input', { bubbles: true }));

      submitForm();
    });

    const messageBeforeChange = getInnerTextExcept('input,script');
    assert.include(messageBeforeChange, 'John Doe');

    await React.act(async () => {
      setter.call(nameInp, 'Jane Doe');
      nameInp.dispatchEvent(new Event('input', { bubbles: true }));
    });

    const messageAfterChange = getInnerTextExcept('input,script');
    assert.include(messageAfterChange, 'John Doe');
    assert.notInclude(messageAfterChange, 'Jane Doe');
  } catch (e) {
    console.log(e);
    throw e;
  }
