...
Then, log the `lastCharacter` variable to the console.
...
You should log the `lastCharacter` variable to the `console`.

```js
assert.match(code, /console\.log\(\s*lastCharacter\s*\)\s*;?/);
```
