...
- **`Object.hasOwn()` Method**: This is the modern, recommended way to check if an object has a property as its own (not inherited). It works correctly even for `null`-prototype objects and for objects that have overridden the inherited `hasOwnProperty()` method.
