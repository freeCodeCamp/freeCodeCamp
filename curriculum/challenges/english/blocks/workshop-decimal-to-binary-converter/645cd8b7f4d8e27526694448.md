...
const { countdown } = explorer.allFunctions;
...
const explorer = await __helpers.Explorer(code);
const { countdown } = explorer.allFunctions;
assert.equal(countdown?.parameters.length, 1);
assert.equal(countdown?.parameters[0].toString(), "number");
```
...
const explorer = await __helpers.Explorer(code);
const { countdown } = explorer.allFunctions;
assert.isTrue(validExpressions.some(expr => countdown?.matches(expr)));
```
