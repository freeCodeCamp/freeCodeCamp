...
```js
const { variables } = await __helpers.Explorer(code);
const { numberInput } = variables;
const { value } = numberInput || {};
assert.exists(numberInput);
assert.isTrue(value?.matches('document.getElementById("number-input")'));
```
...
const { numberInput } = variables;
assert.exists(numberInput);
assert.isTrue(numberInput?.matches('const numberInput = document.getElementById("number-input")') || numberInput?.matches("const numberInput = document.getElementById('number-input')"));
```
...
```js
const { variables } = await __helpers.Explorer(code);
const { convertBtn } = variables;
const { value } = convertBtn || {};
assert.exists(convertBtn);
assert.isTrue(value?.matches('document.getElementById("convert-btn")'));
```
...
const { convertBtn } = variables;
assert.exists(convertBtn);
assert.isTrue(convertBtn?.matches('const convertBtn = document.getElementById("convert-btn")') || convertBtn?.matches("const convertBtn = document.getElementById('convert-btn')"));
```
...
assert.isTrue(value?.matches('document.getElementById("result")'));
```
...
const { result } = variables;
assert.exists(result);
assert.isTrue(result?.matches('const result = document.getElementById("result")') || result?.matches("const result = document.getElementById('result')"));
```
