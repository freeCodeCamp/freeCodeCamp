...
assert.exists(explorer.allFunctions.decimalToBinary?.variables?.binary);
```
...
```js
const explorer = await __helpers.Explorer(code);
const binaryVal = explorer.allFunctions.decimalToBinary?.variables?.binary?.value;
assert.isTrue(binaryVal?.matches("''") || binaryVal?.matches('""') || binaryVal?.matches('``'));
```
