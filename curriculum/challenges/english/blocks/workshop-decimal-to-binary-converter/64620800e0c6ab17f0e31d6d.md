...
assert.exists(explorer.allFunctions.showAnimation);
```
...
const { showAnimation } = allFunctions;
assert.exists(showAnimation);
assert.isTrue(showAnimation?.matches('const showAnimation = () => {}') || showAnimation?.matches('function showAnimation() {}'));
```
