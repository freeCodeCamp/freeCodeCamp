...
assert.exists(explorer.allFunctions.a);
```
...
const { a } = allFunctions;
const hasDoubleQuotes = a?.hasReturn('"freeCodeCamp " + b()');
const hasSingleQuotes = a?.hasReturn("'freeCodeCamp ' + b()");
