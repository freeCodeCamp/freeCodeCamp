...
You should have an `if` statement in your `__str__` method.

```js
({
  test: () => runPython(`assert _Node(_code).find_class("MediaCatalogue").find_function("__str__").find_ifs()[0]`)
})
```

Your `if` statement should add the string `=== MOVIES ===` (ending with a newline character) to the current value of `result` when the `movies` list is not empty.
