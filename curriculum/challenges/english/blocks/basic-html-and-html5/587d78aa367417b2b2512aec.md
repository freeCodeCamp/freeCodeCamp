...
You can add another level of organization in your HTML document within the `html` tags by using the `head` and `body` elements. Any markup with information about your page goes into the `head` tag. Then any markup with the content of the page (what is displayed to the user) goes into the `body` tag.

Metadata elements, such as `link`, `meta`, `title`, and `style`, usually go inside the `head` element.
