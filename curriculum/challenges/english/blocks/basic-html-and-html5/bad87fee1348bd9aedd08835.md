...
```js
assert.equal(document.querySelector('label[for="indoor"]')?.textContent?.trim(), 'Indoor');
```

You should not change the inner text of the `Loving` label.

```js
assert.equal(document.querySelector('label[for="loving"]')?.textContent?.trim(), 'Loving');
```
