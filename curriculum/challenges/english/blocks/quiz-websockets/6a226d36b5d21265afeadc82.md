...
Every WebSocket connections start with FTP, then upgrade to WebSocket.
...
What makes WebSockets different from HTTP in terms of connection behavior?

#### --distractors--

WebSockets use a new HTTP connection for every message exchanged.

---

WebSockets only allow the server to send data, the client can only receive.

---

WebSockets close the connection after each message, just like HTTP.

#### --answer--

WebSockets create a permanent, bidirectional connection that stays open until either side sends a closing signal.
