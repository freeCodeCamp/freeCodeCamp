...
12. When a user clicks the `#theme-switcher-button` and selects a theme different to the current theme, the background and font colors will match the selection on the `body` element.
...
When a user clicks the `#theme-switcher-button` and selects a theme the background and font colors will match the selection.
...
const firstBg = getComputedStyle(document.body).getPropertyValue("background-color");
const firstColor = getComputedStyle(document.body).getPropertyValue("color");

themeDropdownBtn.dispatchEvent(new Event("click"));
listItems[1]?.dispatchEvent(new Event("click", { bubbles: true }));
const secondBg = getComputedStyle(document.body).getPropertyValue("background-color");
const secondColor = getComputedStyle(document.body).getPropertyValue("color");

assert.isTrue(firstBg !== secondBg || firstColor !== secondColor);
```
