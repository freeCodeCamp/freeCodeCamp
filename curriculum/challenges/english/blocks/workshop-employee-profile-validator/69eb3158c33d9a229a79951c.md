...
Create two variables named `dot_count` and `dash_count`. `dot_count` should count how many `'.'` appear in `clean_email`, and `dash_count` should count how many `'-'` appear in `employee_code`.
