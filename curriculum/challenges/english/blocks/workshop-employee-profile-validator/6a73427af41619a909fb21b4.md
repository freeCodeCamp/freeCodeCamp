...
Now print `dot_count` and `dash_count` using f-strings that concatenate `'Dots in email: ` and `Dashes in code: ` with their values, respectively.

With that, your employee profile validator workshop is complete!

# --hints--

You should print `dot_count` by concatenating `'Dots in email: '` with the `dot_count` variable using an f-string.

```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(f'Dots in email: {dot_count}')")`)) })
```

You should print `dash_count` by concatenating `'Dashes in code: '` with the `dash_count` variable using an f-string.
