...
Now print `dot_count` and `dash_count` using f-strings with the prefixes `'Dots in email: '` and `'Dashes in code: '`.

With that, your employee profile validator workshop is complete!

# --hints--

You should print `dot_count` with the prefix `'Dots in email: '` using an f-string.

```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(f'Dots in email: {dot_count}')")`)) })
```

You should print `dash_count` with the prefix `'Dashes in code: '` using an f-string.
