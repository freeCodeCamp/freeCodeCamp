...
# --description--
The simple yoyo scale doesn't sell creation drama. Use `tweens.chain` for a 3-stage pulse: pop big, dip below baseline, settle with a `Back.easeOut` overshoot. The dip+rebound reads as 'gathering energy then releasing'.

Replace both special-creation tweens with `tweens.chain`. The run-spawned stripe / color-bomb gets:
...
And the wrapped intersection gets a slightly bigger pop (`scale: 1.7`, longer first stage) because T/L matches are rarer.

Three stages chained on one target = exactly what `tweens.chain` was designed for. The dip-and-rebound shape is what gives the gem its 'just born' feeling.

# --hints--
Use `this.tweens.chain({ targets: gem, tweens: [...] })`.

```js
assert.match(code, /this\.tweens\.chain\(\s*\{\s*targets:\s*gem\s*,\s*tweens:/);
```

Stage 1: `scale: 1.5, duration: 120, ease: "Cubic.easeOut"`.

```js
assert.match(code, /scale:\s*1\.5\s*,\s*duration:\s*120\s*,\s*ease:\s*[\"\']Cubic\.easeOut/);
```

Stage 2 dip: `scale: 0.85, duration: 80, ease: "Cubic.easeIn"`.

```js
assert.match(code, /scale:\s*0\.85\s*,\s*duration:\s*80\s*,\s*ease:\s*[\"\']Cubic\.easeIn/);
```

Stage 3 settle: `scale: 1, duration: 140, ease: "Back.easeOut"`.

```js
assert.match(code, /scale:\s*1\s*,\s*duration:\s*140\s*,\s*ease:\s*[\"\']Back\.easeOut/);
```

For wrapped (intersection), bump stage 1 to `scale: 1.7`.

```js
assert.match(code, /scale:\s*1\.7\s*,\s*duration:\s*140/);
```
