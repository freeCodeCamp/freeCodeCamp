...
# --description--
A wrapped gem clears every gem in a 3x3 area centered on itself. Add the `wrapped` branch to `activateSpecial`:
...
The bounds check `r < 0 || r >= this.rows` keeps the 3x3 from wrapping past the grid edges, so a wrapped on row 0 only clears 6 gems instead of 9.

# --hints--
Add an `else if (special === "wrapped")` branch.

```js
assert.match(code, /else\s+if\s*\(\s*special\s*===\s*["\']wrapped["\']\s*\)/);
```

Iterate `dr` and `dc` from -1 to 1.

```js
assert.match(code, /for\s*\(\s*let\s+dr\s*=\s*-1\s*;\s*dr\s*<=\s*1\s*;\s*dr\+\+\s*\)[\s\S]*for\s*\(\s*let\s+dc\s*=\s*-1\s*;\s*dc\s*<=\s*1/);
```

Bail with bounds checks `r < 0 || r >= this.rows`.

```js
assert.match(code, /if\s*\(\s*r\s*<\s*0\s*\|\|\s*r\s*>=\s*this\.rows\s*\)\s*continue/);
```
