...
# --description--
Add a transient bloom to every newly-created special gem. Tween the bloom's `strength` from peak to `0` over 700-800ms, then `postFX.remove` to dispose of it (otherwise it costs every frame for the rest of the gem's life).

Both creation paths get a bloom. The 4-/5-run special gets a white bloom; the wrapped intersection gets a magenta bloom for differentiation:
...
Tweening the controller's `strength` is exactly the per-frame FX manipulation pattern the lecture warned about - we pay the GPU cost for the duration of the bloom, then dispose.

# --hints--
Add a bloom via `gem.postFX.addBloom(...)`.

```js
assert.match(code, /gem\.postFX\.addBloom\(/);
```

Tween `bloom.strength` from peak to `0`.

```js
assert.match(code, /strength:\s*\{\s*from:\s*\d+(?:\.\d+)?\s*,\s*to:\s*0\s*\}/);
```

Remove the bloom on tween complete with `gem.postFX.remove(bloom)`.

```js
assert.match(code, /gem\.postFX\.remove\(\s*bloom\s*\)/);
```
