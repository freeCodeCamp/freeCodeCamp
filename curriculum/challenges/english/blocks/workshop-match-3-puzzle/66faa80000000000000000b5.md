...
# --description--
When a swap kicks off a chain of matches, each cascade level is one combo step. Track the combo counter and emit a `comboUpdate` event - UIScene will display it next step.
...
Note `this.combo = depth + 1`. Cascade depth `0` is the user-initiated swap (combo = 1). Depth `1` is the first cascade fall (combo = 2), and so on. The combo counter resets implicitly on the next user swap because the new cascade starts at depth `0` again.

# --hints--
Lazy-init combo with `this.combo ??= 0;` at the top of cascade.

```js
assert.match(code, /this\.combo\s*\?\?=\s*0/);
```

Set `this.combo = depth + 1` at every cascade level.

```js
assert.match(code, /this\.combo\s*=\s*depth\s*\+\s*1/);
```

Emit `"comboUpdate"` with the new combo value.

```js
assert.match(code, /this\.events\.emit\(\s*["\']comboUpdate["\']\s*,\s*this\.combo\s*\)/);
```
