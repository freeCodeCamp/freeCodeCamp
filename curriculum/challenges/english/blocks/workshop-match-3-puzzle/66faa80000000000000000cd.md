...
# --description--
Audio cues turn a silent grid into a satisfying juicy puzzle. Load 4 SFX in `preload`, then call `this.sound.play(...)` at the four key moments: swap initiated, match cleared, special created, and game lost.
...
The wrapped-special SFX uses `rate: 0.9` (lower pitch) and the stripe/color-bomb uses `rate: 1.2` (higher pitch), so the ear can distinguish them. Volumes stay below 0.5 so SFX never drown out one another.

# --hints--
Load `sfxClick`, `sfxMatch`, `sfxSpecial`, `sfxFail` in `preload`.

```js
assert.match(code, /this\.load\.audio\(\s*["\']sfxClick["\']/);
```

Play `"sfxClick"` at the start of `swap`.

```js
assert.match(code, /this\.sound\.play\(\s*["\']sfxClick["\']\s*,/);
```

Play `"sfxMatch"` after the score increment in `cascade`.

```js
assert.match(code, /this\.sound\.play\(\s*["\']sfxMatch["\']\s*,/);
```

Play `"sfxSpecial"` before each bloom in `cascade`.

```js
assert.match(code, /this\.sound\.play\(\s*["\']sfxSpecial["\']\s*,/);
```

Play `"sfxFail"` before each `levelFailed` emit in `checkLevelEnd`.

```js
assert.match(code, /this\.sound\.play\(\s*["\']sfxFail["\']\s*,/);
```
