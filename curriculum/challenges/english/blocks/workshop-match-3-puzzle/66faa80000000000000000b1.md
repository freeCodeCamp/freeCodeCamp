...
# --description--
Empty cells above each `writeRow` need new gems. Spawn them off the top of the playfield (negative `gemY`) and tween them down with a slight stagger so they don't all arrive on the same frame.

Inside the cascade column loop, after `writeRow--; }`, add the spawn loop:
...
And bump the lock-release timer to 420ms (longest possible drop). The `setInteractive()` call is critical - new gems spawn without input bindings unless you opt them in.

# --hints--
Loop `r` from `writeRow` down to `0` to fill empty cells.

```js
assert.match(code, /for\s*\(\s*let\s+r\s*=\s*writeRow\s*;\s*r\s*>=\s*0\s*;\s*r--\s*\)/);
```

Spawn the new gem above the playfield at `this.gemY(-1 - (writeRow - r))`.

```js
assert.match(code, /this\.gemY\(\s*-1\s*-\s*\(\s*writeRow\s*-\s*r\s*\)\s*\)/);
```

Call `.setInteractive()` on each new sprite.

```js
assert.match(code, /\.setInteractive\(\s*\)\s*;\s*this\.gems\[r\]\[c\]\s*=\s*sprite/);
```

Tween the new sprite `y` to `this.gemY(r)` with stagger `280 + (writeRow - r) * 40`.

```js
assert.match(code, /duration:\s*280\s*\+\s*\(\s*writeRow\s*-\s*r\s*\)\s*\*\s*40/);
```
