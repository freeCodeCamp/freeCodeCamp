...
# --description--
Idle players need a nudge. Add a `findHint()` helper that tries every adjacent swap, asks `findMatches()` if it would score, and returns the first pair that does. Then schedule a 5-second `time.delayedCall` after each cascade settles - if the player still hasn't moved, pulse the hinted pair.
...
Also call `this.hintTimer?.remove()` in `swap` so a player action immediately cancels the pending pulse.

# --hints--
Add a `this.findHint = () => { ... }` helper that simulates each adjacent swap.

```js
assert.match(code, /this\.findHint\s*=\s*\(\s*\)\s*=>/);
```

Inside, swap, test `this.findMatches().cells.length > 0`, then swap back.

```js
assert.match(code, /this\.findMatches\(\)\.cells\.length\s*>\s*0/);
```

Schedule the hint with `this.time.delayedCall(5000, ...)` after the settle.

```js
assert.match(code, /this\.hintTimer\s*=\s*this\.time\.delayedCall\(\s*5000/);
```

Pulse the pair using a yoyo tween with `repeat: 2`.

```js
assert.match(code, /repeat:\s*2/);
```

Cancel the hint on player action: `this.hintTimer?.remove()` at the top of swap.

```js
assert.match(code, /this\.hintTimer\s*\?\.\s*remove\(\)/);
```
