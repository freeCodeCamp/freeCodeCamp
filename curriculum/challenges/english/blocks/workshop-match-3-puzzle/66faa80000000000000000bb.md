...
# --description--
When a match forms a T or L shape - a horizontal run AND a vertical run that intersect at one cell, sharing the same type - the intersection cell becomes a WRAPPED gem. Wrapped gems clear a 3x3 area on activation (step 22).
...
The cross product of `hRuns x vRuns` is small - at most ~3x3 = 9 pairs. We bail early on type mismatch and on cells already spared by the 4-run / 5-run logic (so a 5-run doesn't get demoted from color-bomb to wrapped).

# --hints--
Filter horizontal runs with `matches.runs.filter((r) => r.orient === "h")`.

```js
assert.match(code, /hRuns\s*=\s*matches\.runs\.filter\(\s*\(\s*r\s*\)\s*=>\s*r\.orient\s*===\s*["\']h["\']\s*\)/);
```

Cross-iterate hRuns x vRuns.

```js
assert.match(code, /hRuns\.forEach\([\s\S]*vRuns\.forEach/);
```

Check `inH && inV` for the actual intersection cell.

```js
assert.match(code, /const\s+inH\s*=\s*vc\s*>=\s*h\.col\s*&&\s*vc\s*<\s*h\.col\s*\+\s*h\.len/);
```

Tag the intersection gem with `special: "wrapped"`.

```js
assert.match(code, /special:\s*["\']wrapped["\']/);
```
