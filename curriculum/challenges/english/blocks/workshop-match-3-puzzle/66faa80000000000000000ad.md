...
# --description--
After a swap settles you need to know if it triggered a match. Add `this.findMatches` - a sweep that returns every grid cell that's part of a 3+-in-a-row run. Then update `swap`'s `onComplete` to call it.
...
We use a sentinel run-type of `-2` past the array end so the final group flushes correctly. Both branches release `this.busy` for now - we'll wire revert (no match) and cascade (match) in the next two steps.

# --hints--
Define `this.findMatches = () => { ... };`.

```js
assert.match(code, /this\.findMatches\s*=\s*\(\s*\)\s*=>/);
```

Use a `Set` keyed by `${r},${c}` strings to dedupe horizontal/vertical hits.

```js
assert.match(code, /const\s+matched\s*=\s*new\s+Set\(\s*\)/);
```

Loop columns inside the row sweep and detect runs of length >= 3.

```js
assert.match(code, /if\s*\(\s*c\s*-\s*runStart\s*>=\s*3\s*&&\s*runType\s*>=\s*0\s*\)/);
```

Return an array of `{ row, col }` objects.

```js
assert.match(code, /return\s*\[\s*\.\.\.matched\s*\]\s*\.map\(/);
```

Have `swap`'s `onComplete` call `this.findMatches()`.

```js
assert.match(code, /onComplete:\s*\(\s*\)\s*=>\s*\{\s*const\s+matches\s*=\s*this\.findMatches\(\s*\)/);
```
