...
# --description--
Players need a number to climb. Each matched gem is worth `10` points. Use the lazy `??=` init pattern at the top of cascade so we don't need a separate state initializer.
...
Two effects: `this.score ??= 0;` initializes once on the first cascade (without needing a `/* create-state */` block), and `this.score += matches.length * 10;` runs *every* cascade level - so a 5-cascade combo from one swap collects scoring 5x. The combo counter (step 15) will scale this further.
...
```js
assert.match(code, /this\.score\s*\+=\s*matches\.length\s*\*\s*10/);
```
