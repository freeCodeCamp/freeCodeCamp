...
# --description--
Add a `PauseScene` triggered by the `P` key. The pause menu stops MainScene's update + tween processing, then offers Resume or Back-to-Title.
...
Define `PauseScene` after `GameOverScene`. Its `create` stacks a dim overlay, two buttons, and a P-key listener that mirrors the resume action so the player can toggle pause with the same key:
...
Add `PauseScene` to the config's `scene` array. Order doesn't matter for non-boot scenes, but keep it last for readability. `scene.pause()` halts MainScene's update loop AND its tween manager - the grid freezes mid-animation, exactly what we want for a clean pause.

# --hints--
Listen for `"keydown-P"` in MainScene to launch `PauseScene`.

```js
assert.match(code, /this\.input\.keyboard\s*\?\.\s*on\(\s*["\']keydown-P["\']/);
```

Call `this.scene.pause()` then `this.scene.launch("PauseScene")`.

```js
assert.match(code, /this\.scene\.pause\(\)[\s\S]*this\.scene\.launch\(\s*["\']PauseScene["\']/);
```

Define `class PauseScene extends Phaser.Scene` with `super("PauseScene")`.

```js
assert.match(code, /class\s+PauseScene\s+extends\s+Phaser\.Scene[\s\S]*super\(\s*["\']PauseScene["\']\s*\)/);
```

Resume button calls `this.scene.resume("MainScene")` after stopping itself.

```js
assert.match(code, /this\.scene\.resume\(\s*["\']MainScene["\']\s*\)/);
```

Add `PauseScene` to the config's `scene` array.

```js
assert.match(code, /scene:\s*\[\s*TitleScene\s*,\s*MainScene\s*,\s*UIScene\s*,\s*GameOverScene\s*,\s*PauseScene\s*\]/);
```
