...
# --description--
Until now, completing all 3 classic levels emits `gameWon` but nothing listens. Add a UIScene handler that launches GameOverScene with `win: true` so the player gets a celebration screen, not a frozen board.
...
GameOverScene already shows 'Level cleared!' or 'Run finished!' based on `payload.mode` and `payload.win`, so no further GameOverScene changes are needed for this celebration flow.

# --hints--
Listen for `"gameWon"`.

```js
assert.match(code, /main\.events\.on\(\s*["\']gameWon["\']/);
```

Build the payload with `win: true` and forward `score`, `best`.

```js
assert.match(code, /win:\s*true/);
```

Stop MainScene, then launch GameOverScene after 200ms via `this.time.delayedCall`.

```js
assert.match(code, /this\.time\.delayedCall\(\s*200\s*,/);
```
