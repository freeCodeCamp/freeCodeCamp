...
# --description--
A puzzle without a goal is just a sandbox. Define three levels, each with a `targetColor`, `target` count, and move budget. Track how many target-color gems have been matched.

Add the level state at the top of cascade, after the combo lines:
...
Three levels of escalating difficulty: 25 reds in 20 moves, 30 blues in 22 moves, 35 greens in 24 moves. The `matches.cells.filter` counts how many cleared gems matched the target color this cascade level - special-gem activations naturally count too because they expand `allCells` (which the filter doesn't see; we'll fix that in step 26).

# --hints--
Define `this.LEVELS ??= [...]` with three level configs.

```js
assert.match(code, /this\.LEVELS\s*\?\?=\s*\[/);
```

Lazy-init `this.level ??= 0`, `this.movesLeft ??= ...`, etc.

```js
assert.match(code, /this\.movesLeft\s*\?\?=\s*this\.LEVELS\[this\.level\]\.moves/);
```

Filter `matches.cells` by `c.type === this.targetColor` and accumulate.

```js
assert.match(code, /matches\.cells\.filter\(\s*\(\s*c\s*\)\s*=>\s*c\.type\s*===\s*this\.targetColor/);
```

Emit `"targetUpdate"` with `{ matched, target }`.

```js
assert.match(code, /this\.events\.emit\(\s*["\']targetUpdate["\']\s*,\s*\{\s*matched:\s*this\.matchedTarget/);
```
