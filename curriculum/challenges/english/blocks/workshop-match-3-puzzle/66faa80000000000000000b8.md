...
# --description--
A stripe gem is *fun* because activating it wipes a whole row/column of gems. When cascade processes the matches, any matched cell that's already tagged `special: "stripe-h"` (or `stripe-v`) should expand the destruction set to include every gem in that gem's row (or column).

Insert a new method `this.activateSpecial(gem, allCells)` *before* `this.findMatches`:
...
And replace `cascade`'s body so that it builds an `allCells` map, runs `activateSpecial` on any pre-existing specials in the match list, scores the *expanded* set, and skips creating a new stripe at a cell that's already a special (avoids double-counting).

The recursive `activateSpecial` call inside `targets.forEach` handles chain reactions: a stripe activated stripe-of-different orientation triggers both rows and columns clearing in a cascade. (We'll get richer chain reactions in steps 20 and 22.)

# --hints--
Define `this.activateSpecial = (gem, allCells) => { ... };`.

```js
assert.match(code, /this\.activateSpecial\s*=\s*\(\s*gem\s*,\s*allCells\s*\)\s*=>/);
```

Read the special tag from `gem.getData("special")`.

```js
assert.match(code, /const\s+special\s*=\s*gem\.getData\(\s*["\']special["\']\s*\)/);
```

Clear `gem.setData({ ..., special: null })` so it doesn't activate twice.

```js
assert.match(code, /special:\s*null/);
```

For "stripe-h", iterate `c < this.cols` and push every cell in row `sr`.

```js
assert.match(code, /if\s*\(\s*special\s*===\s*["\']stripe-h["\']\s*\)/);
```

For "stripe-v", iterate `r < this.rows` and push every cell in column `sc`.

```js
assert.match(code, /if\s*\(\s*special\s*===\s*["\']stripe-v["\']\s*\)/);
```

Recurse `this.activateSpecial(chainGem, allCells)` for chain reactions.

```js
assert.match(code, /this\.activateSpecial\(\s*chainGem\s*,\s*allCells\s*\)/);
```

In cascade, build `const allCells = new Map(matches.cells.map(c => [..., c]));`.

```js
assert.match(code, /new\s+Map\(\s*matches\.cells\.map\(/);
```
