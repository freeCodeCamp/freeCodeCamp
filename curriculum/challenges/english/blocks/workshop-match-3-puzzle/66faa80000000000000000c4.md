...
# --description--
A subtle vignette frames the playfield and pulls the eye toward the center where the action is. Apply it to the main camera once during create.
...
`(0.5, 0.5)` centers the falloff. `radius: 0.7` controls how wide the bright zone is, `strength: 0.42` how dark the corners get. Don't push strength much above `0.5` - the bottom-row gems start getting lost in shadow.

# --hints--
Call `this.cameras.main.postFX.addVignette(0.5, 0.5, 0.7, 0.42)`.

```js
assert.match(code, /this\.cameras\.main\.postFX\.addVignette\(\s*0\.5\s*,\s*0\.5\s*,\s*0\.7\s*,\s*0\.42\s*\)/);
```
