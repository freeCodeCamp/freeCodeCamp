...
# --description--
When a level ends (win or lose), check the player's score against the stored best for that level. Persist the higher one in `localStorage` under `match3-best-<level>`.

Insert at the top of `checkLevelEnd`, just after the early return:
...
Use `?.` chaining on `window.localStorage` so the code doesn't throw on iframes that block storage. The Game Over scene reads `main.bestScore` for the 'Best' line.

# --hints--
Read the stored best with `localStorage?.getItem("match3-best-" + this.level)`.

```js
assert.match(code, /window\.localStorage\s*\?\.\s*getItem\(\s*bestKey\s*\)/);
```

Compare with `Math.max(stored, this.score)`.

```js
assert.match(code, /Math\.max\(\s*stored\s*,\s*this\.score\s*\)/);
```

Persist with `localStorage?.setItem(bestKey, String(this.bestScore))`.

```js
assert.match(code, /window\.localStorage\s*\?\.\s*setItem\(\s*bestKey\s*,\s*String\(\s*this\.bestScore\s*\)/);
```
