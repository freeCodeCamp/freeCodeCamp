...
# --description--
Add a combo flash above the moves counter. UIScene already receives `comboUpdate` from the cascade - tween a 'Combo xN!' label upward and fade it out only when the combo is at least 2 (filtering out the trivial first-cascade combo of 1).
...
The `from`/`to` form on `alpha` and `y` resets to the start value on every fire, so each cascade stage gets a fresh flash. Combo `< 2` short-circuits the trivial single-cascade case.

# --hints--
Add a `this.comboText = this.add.text(400, 56, ...)` field.

```js
assert.match(code, /this\.comboText\s*=\s*this\.add/);
```

Listen for `"comboUpdate"`.

```js
assert.match(code, /main\.events\.on\(\s*["\']comboUpdate["\']/);
```

Skip the flash with `if (combo < 2) return;`.

```js
assert.match(code, /if\s*\(\s*combo\s*<\s*2\s*\)\s*return\s*;/);
```

Tween `alpha: { from: 1, to: 0 }` and `y: { from: 56, to: 30 }`.

```js
assert.match(code, /y:\s*\{\s*from:\s*56\s*,\s*to:\s*30\s*\}/);
```
