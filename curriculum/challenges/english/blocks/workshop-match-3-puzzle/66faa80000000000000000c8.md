...
# --description--
First-time players need to know they can swap gems. Show a dismissable tutorial overlay only on the first launch, gated by a `localStorage` flag.

Add a `/* create-tutorial */` block right before `/* create-cleanup */`. The overlay is a stack of `Rectangle` + `Text` game objects on `setDepth(100+)` to render above the grid. A pulsing 'Tap to start' CTA cues interaction. Dismiss via pointerdown on the dim layer or any keypress, and persist the seen-flag so it never shows again on this device:
...
All overlay objects live on `setDepth(100..102)`, layered above gems (default depth 0).

# --hints--
Gate the overlay on `window.localStorage?.getItem("match3-tutorial-seen")`.

```js
assert.match(code, /window\.localStorage\s*\?\.\s*getItem\(\s*["\']match3-tutorial-seen["\']\s*\)/);
```

Use `this.add.rectangle(...)` for the dim background and card.

```js
assert.match(code, /this\.add\s*\n?\s*\.rectangle\(/);
```

Layer the overlay with `setDepth(100)` and above.

```js
assert.match(code, /setDepth\(\s*10[0-2]\s*\)/);
```

Pulse the CTA with a yoyo/repeat tween.

```js
assert.match(code, /yoyo:\s*true\s*,\s*\n\s*repeat:\s*-1/);
```

Persist with `window.localStorage?.setItem("match3-tutorial-seen", "1")`.

```js
assert.match(code, /window\.localStorage\s*\?\.\s*setItem\(\s*["\']match3-tutorial-seen["\']\s*,\s*["\']1["\']\s*\)/);
```
