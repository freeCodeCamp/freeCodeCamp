...
# --description--
Activating a special is the most dramatic moment in a match-3. Make it feel that way: drop an expanding ring at the special's position, color-coded by special type, and tween the radius from 14 to 80 while alpha fades to 0.
...
Tweening `radius` directly works because Phaser's `Arc` (`this.add.circle`) exposes it as a property. The fill is alpha 0 (transparent) and the stroke is the color, giving us a clean ring rather than a filled circle. `setDepth(50)` keeps the ring above gems but below UI overlays.

# --hints--
Pick `flashColor` per special type with a nested ternary.

```js
assert.match(code, /flashColor\s*=\s*\n?\s*special\s*===\s*[\"']color-bomb[\"']/);
```

Build the ring with `this.add.circle(gem.x, gem.y, 14, flashColor, 0).setStrokeStyle(...)`.

```js
assert.match(code, /this\.add\s*\.circle\(\s*gem\.x\s*,\s*gem\.y\s*,\s*14\s*,\s*flashColor/);
```

Tween `radius: 80` with `alpha: { from: 1, to: 0 }`.

```js
assert.match(code, /radius:\s*80/);
```

Destroy the ring on `onComplete`.

```js
assert.match(code, /onComplete:\s*\(\s*\)\s*=>\s*ring\.destroy\(\s*\)/);
```
