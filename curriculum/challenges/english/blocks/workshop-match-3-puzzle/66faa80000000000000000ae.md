...
# --description--
When a swap creates no matches, the rules say it's illegal - tween the gems back to their original positions. Replace the no-match branch:
...
Why mirror the swap (un-swap state then re-tween)? Because the match check happened *after* the visual swap completed - the sprites are already in the swapped positions. We have to flip everything back.

# --hints--
Restore `this.gemTypes[a.row][a.col] = tA` (and the other three slots).

```js
assert.match(code, /this\.gemTypes\[a\.row\]\[a\.col\]\s*=\s*tA[\s\S]*this\.gemTypes\[b\.row\]\[b\.col\]\s*=\s*tB/);
```

Re-`setData` the sprite refs back to the original `(row, col, type)`.

```js
assert.match(code, /sA\.setData\(\s*\{\s*row:\s*a\.row\s*,\s*col:\s*a\.col\s*,\s*type:\s*tA/);
```

Tween `sA` back to `this.gemX(a.col), this.gemY(a.row)`.

```js
assert.match(code, /targets:\s*sA[\s\S]*x:\s*this\.gemX\(a\.col\)[\s\S]*y:\s*this\.gemY\(a\.row\)/);
```
