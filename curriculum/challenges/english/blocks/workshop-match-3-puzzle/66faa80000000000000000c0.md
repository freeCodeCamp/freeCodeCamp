...
# --description--
End conditions: the player WINS the level when `matchedTarget >= targetCount`, and LOSES when `movesLeft <= 0` without hitting target. Both check at the end of the cascade chain.

Insert `this.checkLevelEnd` *after* `this.activateSpecial` in `/* create-methods */`:
...
Right now both branches just emit events. UIScene won't react to them yet - we'll wire win/lose UI next steps.

# --hints--
Define `this.checkLevelEnd = () => { ... };`.

```js
assert.match(code, /this\.checkLevelEnd\s*=\s*\(\s*\)\s*=>/);
```

Emit `"levelComplete"` when target met.

```js
assert.match(code, /this\.events\.emit\(\s*["\']levelComplete["\']/);
```

Emit `"levelFailed"` when out of moves.

```js
assert.match(code, /this\.events\.emit\(\s*["\']levelFailed["\']/);
```

Call `this.checkLevelEnd()` after cascade releases busy.

```js
assert.match(code, /this\.busy\s*=\s*false\s*;\s*this\.checkLevelEnd\(\s*\)/);
```
