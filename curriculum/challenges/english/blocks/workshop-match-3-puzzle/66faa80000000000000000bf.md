...
# --description--
The HUD needs to show how many moves remain and progress toward the objective. Two changes:

1. **Decrement `this.movesLeft`** per matched user swap, in `swap`'s match branch. Emit `movesUpdate`.
1. **Add `movesText` and `targetText`** to UIScene plus the two new event listeners.

Replace UIScene with the v25 version (3 text elements, 3 listeners) and update swap's match branch to:
...
We bail with `typeof this.movesLeft === "number"` because the first user swap fires *before* cascade (which initializes `movesLeft`) - the very first move is free. The HUD will show `Moves: --` until then.

# --hints--
Add `this.movesText` to UIScene.create.

```js
assert.match(code, /this\.movesText\s*=\s*this\.add/);
```

Add `this.targetText` aligned to the right edge.

```js
assert.match(code, /setOrigin\(\s*1\s*,\s*0\s*\)/);
```

Listen for `"movesUpdate"` and update `movesText`.

```js
assert.match(code, /main\.events\.on\(\s*["\']movesUpdate["\']\s*,\s*\(\s*moves\s*\)\s*=>/);
```

Listen for `"targetUpdate"` with destructuring `{ matched, target }`.

```js
assert.match(code, /main\.events\.on\(\s*["\']targetUpdate["\']\s*,\s*\(\s*\{\s*matched\s*,\s*target\s*\}\s*\)/);
```

In `swap`'s match branch, decrement `this.movesLeft`.

```js
assert.match(code, /this\.movesLeft--/);
```
