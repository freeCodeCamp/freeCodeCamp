...
Notice the gray color along the edges of the line. Those edges are known as <dfn>borders</dfn>. Each side of an element can have a different color or they can all be the same.

Make all the edges of the `hr` element the same color as its background using the `border-color` property.
