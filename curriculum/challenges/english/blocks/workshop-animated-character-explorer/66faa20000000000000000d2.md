...
Inside `collectCoin`, animate the coin away. Call `this.tweens.add({ targets: coin, scale: 0, duration: 200, onComplete: () => coin.destroy() })`. The tween shrinks the coin to nothing over 200 ms, then destroys the sprite so the physics group stops tracking it.
