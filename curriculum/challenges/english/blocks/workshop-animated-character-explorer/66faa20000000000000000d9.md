...
One final touch — and a full playthrough. At the very top of `update`, add `if (this.cleared) return;` so once the level is cleared, no further player movement, slime tracking, or animation logic runs. The celebration text stays put.

Now play the game: walk the hero with the arrow keys, collect all 5 coins, watch the `Level Cleared!` overlay appear. Bump the slime to see the camera flash red.
