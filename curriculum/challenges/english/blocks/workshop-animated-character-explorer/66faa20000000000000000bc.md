...
The hero needs input. Phaser's keyboard manager exposes a helper that returns objects for the four arrow keys plus space and shift, all bundled under one variable.

Below `setScale(2)`, assign `this.input.keyboard.createCursorKeys()` to `this.cursors`.

# --hints--

You should assign `this.input.keyboard.createCursorKeys()` to `this.cursors`.
