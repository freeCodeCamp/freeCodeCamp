...
Add slime danger. Below the `scoreText` line, register a second overlap between the player and the slime that calls `this.hitSlime`: `this.physics.add.overlap(this.player, this.slime, this.hitSlime, null, this)`. You'll define the handler in the next step.
