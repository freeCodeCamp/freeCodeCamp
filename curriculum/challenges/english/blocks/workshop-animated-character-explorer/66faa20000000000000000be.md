...
Define the down-facing walk cycle. Frames `0..3` of the hero sheet are the four-frame down-walk loop.

Below the `idle` animation, register a `walk-down` animation: `key: "walk-down"`, frames from `start: 0, end: 3` of the hero, `frameRate: 8`, `repeat: -1`.
