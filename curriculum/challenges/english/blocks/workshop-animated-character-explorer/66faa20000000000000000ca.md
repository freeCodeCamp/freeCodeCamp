...
Inside the `forEach` loop, scale each coin to twice its size and start it spinning. After `const coin = this.coins.create(...)`, call `coin.setScale(2)` and `coin.anims.play("coin-spin")`.
