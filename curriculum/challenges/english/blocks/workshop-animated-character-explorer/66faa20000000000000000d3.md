...
Add a HUD that tracks coins collected. Below the overlap line, set `this.score = 0`. Then create a `Phaser.GameObjects.Text` at `(10, 10)` with the message `"Coins: 0/5"` and assign it to `this.scoreText`. Use `{ fontSize: "16px", fill: "#ffffff" }` for the style.
