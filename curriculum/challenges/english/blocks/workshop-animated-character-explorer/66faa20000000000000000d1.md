...
Define the handler. After the closing brace of `update()` (still inside `MainScene`), add an empty `collectCoin(player, coin)` method. You'll fill its body in the next step.
