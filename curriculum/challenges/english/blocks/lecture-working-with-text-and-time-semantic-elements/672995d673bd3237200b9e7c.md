...
An abbreviation is a shortened form of a word or phrase. For example, 'Dr.' is a common abbreviation for the word 'doctor'; note that some abbreviations may or may not include a period depending on style conventions.

There are two common forms of abbreviations: acronyms and initialisms.

An acronym is a word formed from the initial letters of a phrase, pronounced as a single word, where each letter represents the first letter of a word in the phrase.
...
An initialism is also formed from the first letters of a phrase, but it is pronounced by spelling out each letter individually rather than as a word.
...
The `title` attribute is optional, but if you decide to include it, it must be a human readable description of the abbreviation, acronym, or initialism. Screen readers can also read the title attribute, helping visually impaired users understand the abbreviation.
...
Usually, the style of the abbreviation element will change when you add this attribute. The specific style will depend on the browser. Some browsers may display a dotted underline by default, while others may use different styling; small caps are uncommon in modern browsers. When the user hovers over the abbreviation, the full form is displayed as a tooltip.
