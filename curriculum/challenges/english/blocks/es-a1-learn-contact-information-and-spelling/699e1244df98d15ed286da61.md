...
`Otro` (Masculine) and `otra` (Feminine) are used to talk about an additional or different thing. For examples:

- `Quiero otro café.` – I want another coffee. (`Café` is masculine that's why `otro` is used in here.)

- `Necesito otra copia.` – I need another copy. (`Copia` is feminine that why `otra` is used in here.)
