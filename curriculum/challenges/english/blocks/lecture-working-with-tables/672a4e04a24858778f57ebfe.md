...
So as you can see, each table row is represented by a `tr` element that contains `th` elements with headers, `td` elements with data, or both.
...
Nowadays, developers use CSS flexbox and grid to lay out their designs. freeCodeCamp will cover these tools in depth later.
