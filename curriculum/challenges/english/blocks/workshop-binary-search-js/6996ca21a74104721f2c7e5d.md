...
// "while (ANYTHING) { ANYTHING; if (ANYTHING) { ANYTHING } break; ANYTHING }"
// "while (ANYTHING) { ANYTHING; if (ANYTHING) { ANYTHING } break ANYTHING }"
assert.match(
  codeWithoutComments,
  /while\s*\([^)]*\)\s*\{[\s\S]*if\s*\([^)]*\)\s*\{[\s\S]*?\}\s*break\s*;?[\s\S]*\}/
);
```
...
const codeWithoutComments = __helpers.removeJSComments(binarySearch?.toString());
// Match the following variations:
// "while (ANYTHING) { ANYTHING } return [];"
// "while (ANYTHING) { ANYTHING } return([]);"
// "while (ANYTHING) { ANYTHING } return[];"
// "while (ANYTHING) { ANYTHING } return [   ];"
assert.match(
  codeWithoutComments,
  /while\s*\([^)]*\)\s*\{[\s\S]*\}\s*return\s*(?:\[\s*\]|\(\s*\[\s*\]\s*\))\s*;?/
);
```
