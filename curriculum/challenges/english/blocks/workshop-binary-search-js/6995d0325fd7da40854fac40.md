...
```js
const binarySearchStr = binarySearch.toString();
assert.match(binarySearchStr, /function\s+binarySearch\s*\(\s*searchList\s*,\s*value\s*\)|const\s+binarySearch\s*=\s*\(\s*searchList\s*,\s*value\s*\)\s*=>|let\s+binarySearch\s*=\s*\(\s*searchList\s*,\s*value\s*\)\s*=>|var\s+binarySearch\s*=\s*\(\s*searchList\s*,\s*value\s*\)\s*=>/);
```
