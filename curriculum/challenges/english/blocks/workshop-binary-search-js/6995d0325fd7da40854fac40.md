...
Your `binarySearch` function should have a `searchList` and `value` parameters in order.

```js
const binSearchParams = __helpers.getFunctionParams(binarySearch.toString());
assert.lengthOf(binSearchParams, 2);
assert.deepEqual(binSearchParams[0].name, "searchList");
assert.deepEqual(binSearchParams[1].name, "value");
```
