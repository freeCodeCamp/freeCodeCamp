...
You should update the `return` statement in the `if` so the function returns an array with `pathToTarget` as the first element and ``` `Value found at index ${mid}` ``` as the second element. Be sure to use backticks around your template literal.
