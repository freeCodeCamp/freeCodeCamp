...
You can express this with a `while` loop that will continue as long as your `low` pointer is less than or equal to `high` pointer.
...
assert.match(codeWithoutComments, /while\s*\(\s*low\s*<=\s*high\s*\)/);
```
