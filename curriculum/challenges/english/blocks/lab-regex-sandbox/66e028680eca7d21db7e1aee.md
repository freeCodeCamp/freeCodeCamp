...
1. When you click the `testButton` button, if the regex pattern matches the test string, you should update `stringToTest`'s `innerHTML` so that each matched portion is wrapped in a `span` element with the class of `highlight`, while all non-matching parts of the string remain unchanged as plain text. Note that the CSS for the `highlight` class has already been added for you.
