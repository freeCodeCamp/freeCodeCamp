...
Next, create a type `Position` with the value `typeof POSITIONS[number]`. This unpacks the `POSITIONS` array into a union of its individual string literals, effectively making it look like  `"GK" | "CB" | "LB" | ...` and so on. 

Finally, update the position field in your PlayerData interface from string to Position, so TypeScript can enforce valid values at compile time anywhere in your app that touches that field.
