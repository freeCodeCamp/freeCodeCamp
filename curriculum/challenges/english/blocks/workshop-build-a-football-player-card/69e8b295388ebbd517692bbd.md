...
Each `stat-row` should contain two `span` elements with the class name `stat-value` and `stat-label` respectively.
...
`DRI` represents dribbling, `DEF` for defending, and `PHY` for physique.
