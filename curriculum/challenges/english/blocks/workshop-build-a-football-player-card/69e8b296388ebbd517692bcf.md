...
Proceed by adding a `div` of class name `form-row`. Inside it, add two `div` elements with the class names `form-group`.

The first `div` element should have a `label` of class name `label`,  an `htmlFor` attribute of `position` a text `Position`, and an `input` with the `id` of `position`, a class name `input` and a `type` of `text`.

The second `div` element should have a `label` of class name `label`, an `htmlFor` of `overallRating` a text of `Overall`, and an `input` with an `id` set to `overallRating`, a class name  of `input` and a type of `number`.

# --hints--

Your Player Info section should contain a `div` with the class `form-row`.
...
Your `div.form-row` should contain two `div` elements each with the class `form-group`.
...
Your first `form-group` in the row should have a `label` with the class name `label` and an `htmlFor` of `position`.
...
Your first `form-group` in the row should have an `input` with an `id` of `position` and `type` of `text`.
...
Your second `form-group` in the row should have a `label` with the class name `label` and an `htmlFor` of `overallRating`.
...
Your `overallRating` `label` should have the text `Overall`.
