...
Inside the `card` div, add a `div` with the class name `card-header`. This section will hold the player rating, position, tier badge, and club name.

# --hints--

You should add a `div` with the class name `card-header` inside the `div` with class name `.card`.
