...
Now you will start building the form. Inside `div` with the class `form-panel`, add a `div` to wrap the "Player Info" section. Inside that new `div`, add `p` wth the `className` `form-section-title` and the text `Player Info`.
