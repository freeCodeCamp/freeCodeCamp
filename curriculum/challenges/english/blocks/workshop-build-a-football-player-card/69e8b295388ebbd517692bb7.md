...
After the left `div`, add the right side of the header. Add a `div` with class name `card-header-right`. Inside it, add a `div` with class name `card-tier-badge` and the text `GOLD`.
