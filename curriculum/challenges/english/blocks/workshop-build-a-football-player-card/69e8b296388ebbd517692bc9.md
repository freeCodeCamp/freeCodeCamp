...
Above `FootballPlayerCard`, create an object named `defaultPlayer` of type `PlayerData` with the following values:
...
Then update `FootballPlayerCard` to pass `player={defaultPlayer}` to `<PlayerCard />` instead of the inline object.
