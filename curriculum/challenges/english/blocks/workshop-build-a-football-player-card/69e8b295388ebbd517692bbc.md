...
Now add a `div` with the class name `card-stats`. Inside it, add a `div` with the class name `stat-col`. Inside that, add three `div` elements with each with the class name `stat-row"`. Each `stat-row` should two `span` elements with the class name `stat-value` and `stat-label` respectively.

The `span` elements in the three rows should have these values:

- `97` and `PAC`
- `98` and `SHO`
- `83` and `PAS`

`PAC` represents pace, `SHO` represents shot, and `PAS`represents passing.
