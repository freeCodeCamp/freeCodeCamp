...
In the `PlayerCard` being referenced inside `FootballPlayerCard`, accept the `player` prop and pass in an object with the following properties:
...
You should pass the correct Pele player data as the `player` prop.
...
assert.equal(player?.imageUrl, 'https://i.ibb.co/996kyTM0/Pele.jpg');
