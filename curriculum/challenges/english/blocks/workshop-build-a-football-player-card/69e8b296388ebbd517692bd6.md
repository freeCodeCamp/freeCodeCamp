...
Starting with the Name `input`, add a `value` attribute of `player.name` to it. Then add an `onChange` attribute set to an arrow function that takes `e` as its parameter and calls `setPlayer` with a spread of `player` and `name` set to `e.target.value`.
