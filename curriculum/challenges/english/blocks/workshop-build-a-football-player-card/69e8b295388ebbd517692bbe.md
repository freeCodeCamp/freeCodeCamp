...
Before the `PlayerCard` function, create an interface called `PlayerData` with these four properties: `name` of type `string`, `overallRating` of type `number`, `position` of type `string`, and `club` of type `string`.
