...
Add another `div` with the className `form-group`. Inside it, add a `label` element with the class name `label`, an `htmlFor` of `imageUrl`, and the text `Image URL`. Then add an `input` element with an `id` of `imageUrl`, a class name of `input`, and a `type` of `text`.
