...
Inside the `FootballPlayerCard` component, create a state variable called `player` with a setter `setPlayer` that will hold the current player data. Initialize this state with `defaultPlayer`, and specify that the state uses the `PlayerData` type so TypeScript knows what shape the player object should have.

After that, update the return to pass the `player` state variable as the prop to `PlayerCard`.

# --hints--

You should use `useState<PlayerData>(defaultPlayer)` to initialize the player state.

```js
assert.match(__helpers.removeJSComments(code), /useState\s*<\s*PlayerData\s*>\s*\(\s*defaultPlayer\s*\)/);
```

You should destructure the state into a `player` variable and a `setPlayer` setter.

```js
assert.match(__helpers.removeJSComments(code), /\[\s*player\s*,\s*setPlayer\s*\]/);
```
