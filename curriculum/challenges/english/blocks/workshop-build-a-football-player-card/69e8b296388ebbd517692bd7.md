...
You will now begin the process to change the position input to a select.

Change the input to a `select` element with the id `position`, a class name `input`, and a `value` of `player.position`.

# --hints--

You should change the position `input` to a `select` element with an `id` of `position` and class name `input`.
