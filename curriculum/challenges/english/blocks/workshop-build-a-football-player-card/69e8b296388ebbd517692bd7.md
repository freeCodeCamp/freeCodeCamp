...
Add a `value` attribute of `player.position` to the Position `input`. Then add an `onChange` attribute set to an arrow function that takes `e` as its parameter and calls `setPlayer` with a spread of `player` and `position` set to `e.target.value`.
...
export const FootballPlayerCard = () => {
  ...
  >
    {["GK", "CB", "LB", "RB", "CDM", "CM", "CAM", "LW", "RW", "ST", "CF"].map(
      (pos) => (
