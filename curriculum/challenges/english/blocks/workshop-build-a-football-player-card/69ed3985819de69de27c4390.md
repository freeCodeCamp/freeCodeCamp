...
Now update `FootballPlayerCard` to pass `defaultPlayer` to `PlayerCard` instead of the inline object.
...
```js
assert.strictEqual(window.index.FootballPlayerCard()?.props?.player, window.index.defaultPlayer);
```
