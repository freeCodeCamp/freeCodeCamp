...
The card header is complete. Now you need to add the player photo section.

After the closing `div` of `card-header` and still inside the `div` with class name `card`, add a `div` with the class name `card-image-wrap`. Leave it empty for now. You will add the image in the next step.

# --hints--

You should add a `div` with the class name `card-image-wrap`.
