...
Inside `main`, add `div` of class name `layout`. Inside `layout`, add two children:

- a `div` of class name `form-panel` you should leave empty for now
- a `div` of class name `preview-panel`. Inside this, add `p` element of  class name `preview-label` and text `Live Preview`, another `p` element of  class name `preview-hint` and the text `Updates as you type`, and a `div` with the class name set to the template literal `preview-box tier-${getPlayerTier(player.overallRating)`. Inside this last `div`, move in the `PlayerCard` component and its prop.
