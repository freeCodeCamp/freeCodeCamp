...
- It has **four parameters**: `err`, `req`, `res`, and `next`. The `err` parameter is what tells Express this is an error handler.
- Invoked when an error is passed to `next(err)`.
- Must be placed **at the end** of the middleware stack, after all other middleware and routes.
