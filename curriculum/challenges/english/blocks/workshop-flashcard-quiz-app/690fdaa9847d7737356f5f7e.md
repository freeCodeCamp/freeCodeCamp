...
assert.strictEqual(controller.elements.flipButton,actualFlipButton); 
assert.strictEqual(controller.elements.entryForm,actualEntryForm); 
assert.strictEqual(controller.elements.deleteButton,actualDeleteBtn); 
```
...
assert.match(code, /deleteBtn\s*:\s*document\.querySelector<HTMLButtonElement>/);
```
