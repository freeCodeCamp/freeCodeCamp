...
Using the same types as the last step, assign the values of the `elements` object to the correct HTML element. Be sure
to explicitly set the type of each element by using generics.

The queries to use are:

1. flashcard - `.flashcard`
2. flipBtn - `#flip-btn`
3. entryForm - `.entry-form`
4. deleteBtn - `#delete-btn`

Do not forget to use the `null-assertion` operator.

# --hints--

All four of the elements are correctly assigned.

```js
const controller = new FlashCardController();
const actualFlashCard = document.querySelector('.flashcard');
const actualFlipButton = document.querySelector('#flip-btn');
const actualEntryForm = document.querySelector('.entry-form');
const actualDeleteBtn = document.querySelector('#delete-btn');

assert.strictEqual(controller.elements.flashcard, actualFlashCard);
assert.strictEqual(controller.elements.flipBtn, actualFlipButton);
assert.strictEqual(controller.elements.entryForm, actualEntryForm);
assert.strictEqual(controller.elements.deleteBtn, actualDeleteBtn);
```

All four selectors use generics.

```js
assert.match(code, /flashcard\s*:\s*document\.querySelector<HTMLElement>/);
assert.match(code, /flipBtn\s*:\s*document\.querySelector<HTMLButtonElement>/);
assert.match(code, /entryForm\s*:\s*document\.querySelector<HTMLFormElement>/);
assert.match(
  code,
  /deleteBtn\s*:\s*document\.querySelector<HTMLButtonElement>/
);
```

All four selectors use the `null-assertion` operator.

```js
assert.match(code, /flashcard\s*:\s*document\.querySelector<HTMLElement>.+!/);
assert.match(
  code,
  /flipBtn\s*:\s*document\.querySelector<HTMLButtonElement>.+!/
);
assert.match(
  code,
  /entryForm\s*:\s*document\.querySelector<HTMLFormElement>.+!/
);
assert.match(
  code,
  /deleteBtn\s*:\s*document\.querySelector<HTMLButtonElement>.+!/
