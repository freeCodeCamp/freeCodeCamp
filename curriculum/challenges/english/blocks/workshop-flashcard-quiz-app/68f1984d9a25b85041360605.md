...
```js
assert.match(code,/(var|let|const) newCard/); 
```

The variable `newCard`'s type should be explicitly set to `FlashCard`. 

```js
assert.match(code,/(var|let|const) newCard\s*:\s*FlashCard/); 
```

The variable `newCard` should be equal to an object containing both `questionText` and `questionAnswer`. 

```js
assert.match(code,/(var|let|const)\s+newCard\s*:\s*FlashCard\s*=\s*{\s*questionText\s*,\s*questionAnswer\s*}/); 
```
