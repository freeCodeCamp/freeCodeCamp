...
The new card data should be wrapped into a new variable called `newCard`. Since the data matches the `FlashCard` interface, the type of `newCard` should be explicitly set to `FlashCard`. 

# --hints--

You should have a variable named `newCard`. 

```js
assert.match(uploadNewCard.toString(),/(var|let|const) newCard/); 
```

The variable `newCard`'s type should be explicitly set to `FlashCard`. 

```js
assert.match(uploadNewCard.toString(),/(var|let|const) newCard\s*:\s*FlashCard/); 
```

The variable `newCard` should be equal to an object containing both `questionText` and `answerText`. 

```js
assert.match(uploadNewCard.toString(),/(var|let|const) newCard\s*:\s*FlashCard\s*{\s*questionText\s*,\s*answerText\s*}/); 
``
