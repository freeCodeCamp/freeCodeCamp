...
Inside of the `elements` object, create the following elements and explicitly set their type.

1. flashcard - HTMLElement
2. flipBtn - HTMLButtonElement
3. entryForm - HTMLFormElement
4. deleteBtn - HTMLButtonElement

Don't forget to cast the `elements` to have the same shape as the data it contains using the `as` keyword.

# --hints--

`flashcard` should be defined in the `elements` object.

```js
const flashcardRegex = /private\s+elements:\s*{\s[\s\S]+flashcard/;
assert.match(code, flashcardRegex);
```

`flashcard` should be explicitly typed as an `HTMLElement`.

```js
assert.match(code, /flashcard\s*:\s*HTMLElement/);
```

`flipBtn` should be defined in the `elements` object.

```js
const flipRegex = /private\s+elements:\s*{\s[\s\S]+flipBtn/;
assert.match(code, flipRegex);
```

`flipBtn` should be explicitly typed as an `HTMLButtonElement`.

```js
assert.match(code, /flipBtn\s*:\s*HTMLButtonElement/);
```

`entryForm` should be defined in the `elements` object.

```js
const entryRegex = /private\s+elements:\s*{\s[\s\S]+entryForm/;
assert.match(code, entryRegex);
```

`entryForm` should be explicitly typed as an `HTMLFormElement`.

```js
assert.match(code, /entryForm\s*:\s*HTMLFormElement/);
```

`deleteBtn` should be defined in the `elements` object.

```js
const deleteRegex = /private\s+elements:\s*{\s[\s\S]+deleteBtn/;
assert.match(code, deleteRegex);
```

`deleteBtn` should be explicitly typed as an `HTMLButtonElement`.

```js
assert.match(code, /deleteBtn\s*:\s*HTMLButtonElement/);
```
