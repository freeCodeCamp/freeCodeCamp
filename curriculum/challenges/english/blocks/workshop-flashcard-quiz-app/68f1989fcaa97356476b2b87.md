...
assert.property(controller.elements, 'flashcard');
```
...
```js
let controller = new FlashCardController(); 
assert.property(controller.elements, 'flipBtn');
```
...
```js
let controller = new FlashCardController(); 
assert.property(controller.elements, 'entryForm');
```

`entryForm` should be explicitly typed as an `HTMLFormElement`. 

```js
assert.match(code,/entryForm\s*:\*HTMLFormElement/)
```
...
```js
let controller = new FlashCardController(); 
assert.property(controller.elements, 'deleteBtn');
```
