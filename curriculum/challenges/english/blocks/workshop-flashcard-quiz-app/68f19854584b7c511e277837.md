...
After pushing the new card data to the array, you need to set the index of the new card. 

Create a variable called `newIndex` and set it to `currentCards.length - 1`. 

# --hints--

You should have a variable called `newIndex`. 

```js
assert.match(uploadNewCard.toString(), /(let|const) newIndex/)
```

`newIndex` should be set to `currentCards.length - 1`. 

```js
assert.match(uploadNewCard.toString(), /(let|const) newIndex\s*=\s*currentCards\.length\s*\-\s*1/)
```
