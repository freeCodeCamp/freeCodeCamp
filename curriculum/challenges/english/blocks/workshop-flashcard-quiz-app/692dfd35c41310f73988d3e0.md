...
Update the `createCardButton` function from the last step so that it explicitly returns an `HTMLButtonElement` and
inside of the function body return a newly created `HTMLButtonElement`.

# --hints--

The `createCardButton` function should explicitly return an `HTMLButtonElement`.  

```js
assert.match(code, /function\s+createCardButton\(questionText\s*:\s*string\s*,\s*index\s*:\s*number\s*\)\s*:\s*HTMLButtonElement/)
```

You should return a newly created `HTMLButtonElement`. 

```js
assert.isTrue(createCardButton("This is a test", 1) instanceof HTMLButtonElement);
```
