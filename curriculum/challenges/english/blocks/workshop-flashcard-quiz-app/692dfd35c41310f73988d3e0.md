...
assert.match(
  code,
  /function\s+createCardButton\(questionText\s*:\s*string\s*,\s*index\s*:\s*number\s*\)\s*:\s*HTMLButtonElement/
);
```
...
```js
assert.isTrue(
  createCardButton('This is a test', 1) instanceof HTMLButtonElement
);
```
