...
In this workshop, you'll learn how to work with TypeScript and HTML by building a flashcard quiz app.

All the basic HTML and CSS you'll need has been provided for you.

Start by defining the interface for a `FlashCard` with the keyword `interface`.

# --hints--

You should have an interface named `FlashCard`.

```js
assert.match(code, /interface FlashCard\s*{\s*}/);
```
