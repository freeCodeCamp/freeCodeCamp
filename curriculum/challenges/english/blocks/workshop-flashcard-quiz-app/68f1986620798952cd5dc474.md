...
Inside of the `refresh` method we need to set the currently displayed card text. Start by grabbing a reference
to the currently active card then set the `textContent` of the `.card-front` and `.card-back` elements. When 
using `document.querySelector` always use the `non-null` assertion operator. 

Note: The current card is inside of `currentCards` at `currentCardIndex`. 

# --hints--

The `.card-front` element text content is set to the current card's question text.

```js
frontInput.value = "What is the capital of France?";
backInput.value = "Paris";
uploadNewCard();
let currentCard = currentCards[currentCardIndex]; 
refresh(); 
assert.strictEqual(cardDisplay.querySelector(".card-front").textContent, currentCard.questionText); 
```

The `.card-back` element text content is set to the current card's answer text.
...
let currentCard = currentCards[currentCardIndex]; 
refresh(); 
assert.strictEqual(cardDisplay.querySelector(".card-back").textContent, currentCard.questionAnswer); 
```
