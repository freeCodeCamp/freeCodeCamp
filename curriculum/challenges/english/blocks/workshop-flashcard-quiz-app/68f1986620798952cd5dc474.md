...
to the currently active card then set the `textContent` of the `.card-front` and `.card-back` elements. 
...
let currentCard = currentCards[currentCardIndex]; 
refresh(); 
assert.strictEqual(cardDisplay.querySelector(".card-back").textContent, currentCard.answerText); 
```
