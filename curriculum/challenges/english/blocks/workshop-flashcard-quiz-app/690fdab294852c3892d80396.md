...
After adding an event listener for the `flipBtn`, add an event listener for the `entryForm`. It should be listening
for a `submit` event and the listener take a `SubmitEvent` as a parameter.  Additionally the parameter should be named
`ev`. 

Inside of the event listener, call `preventDefault` on the event parameter since we don't want the typical behavior 
of an `submit` event to happen. Instead the `uploadNewCard` method should be called. 

# --hints--

The `entryForm` should have an `submit` event listener. 

```js
const submitRegex = /this\.elements\.entryForm\.addEventListener\(("|')submit("|')/;
assert.match(code, submitRegex); 
```

The `entryForm` should be using an `SubmitEvent` parameter.

```js
const submitRegex = /this\.elements\.entryForm\.addEventListener\(("|')submit("|')\s*,\s*\(ev\s*:\s*SubmitEvent\)\s*=>\s*{/;
assert.match(code, submitRegex); 
```

The default behavior of the submit event should be prevented.

```js
const preventRegex = /this\.elements\.entryForm\.addEventListener\(("|')submit("|')\s*,\s*\(ev\s*:\s*SubmitEvent\)\s*=>\s*{[.\s]+ev\.preventDefault/;
assert.match(code, preventRegex); 
```

When the `entryForm` has a `submit` event ran, it should upload a new card.

```js
const flashcardController = new FlashCardController();
flashcardController.initializeEventListeners(); 
frontInput.value = "What is the capital of France?";
backInput.value = "Paris";
const myForm = document.querySelector(".entry-form"); 
myForm.requestSubmit();
assert.lengthOf(currentCards,1); 
```
