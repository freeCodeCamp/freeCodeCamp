...
```js
const submitRegex =
  /this\.elements\.entryForm\.addEventListener\(("|')submit("|')/;
...
```js
const submitRegex =
  /this\.elements\.entryForm\.addEventListener\(("|')submit("|')\s*,\s*\(ev\s*:\s*SubmitEvent\)\s*=>\s*{/;
assert.match(code, submitRegex);
```
...
```js
const preventRegex =
  /this\.elements\.entryForm\.addEventListener\(("|')submit("|')\s*,\s*\(ev\s*:\s*SubmitEvent\)\s*=>\s*{[.\s]+ev\.preventDefault/;
