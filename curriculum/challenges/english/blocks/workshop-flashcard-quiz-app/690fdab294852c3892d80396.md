...
```js
const submitRegex = /this\.elements\.entryForm\.addEventHandler\(("|')submit("|')/;
...
```js
const submitRegex = /this\.elements\.entryForm\.addEventHandler\(("|')submit("|')\s*,\s*\(ev\s*:\s*SubmitEvent\)\s*=>\s*{/;
assert.match(code, submitRegex); 
```
