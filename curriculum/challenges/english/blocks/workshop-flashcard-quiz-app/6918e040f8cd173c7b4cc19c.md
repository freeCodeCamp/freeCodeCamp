...
Then assign the `onclick` method of the `btn` variable to a function that sets `currentCardIndex` to the `index` variable. 

# --hints--

The `currentCardIndex` needs to be set to `index`. 

```js
assert.match(code, /\(btn as HTMLButtonElement\)\.onclick\s*=\s*\(\)\s*=>\s*\{\s*currentCardIndex\s*=\s*index;?/)
```
