...
The `document` object has an `DOMContentLoaded` event listener. 

```js
const spy = __helpers.spyOn(document, 'addEventListener'); 
assert.lengthOf(spy.calls, 1);
assert.strictEqual(spy.calls[0][0],"DOMContentLoaded")
spy.restore();
```
...
```js
assert.match(code, /document\.addEventListener\(("|')DOMContentLoaded("|')\s*,\s*\(event\s*/);
```

The `event` parameter was explicitly made to be of type `Event`.

```js
assert.match(code, /document\.addEventListener\(("|')DOMContentLoaded("|')\s*,\s*\(event\s*:\s*Event\)\s*=>\s*{/);
```
