...
Luckily there is a method on `classList` that performs just that. Call `toggle` on `classList` passing in `"flipped"` as a parameter. 
...
The `toggle` method on `classList` is receives `"flipped"` as a parameter.
