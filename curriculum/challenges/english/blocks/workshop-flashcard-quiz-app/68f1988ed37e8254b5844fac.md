...
Much like earlier with `createCardButton`, this function will loop through `cardButtonsContainer.children`. However this time,
the current button should set `currentCardIndex` to the index of the child element. Additionally clicking the child element
should call the `refresh` method.

Additionally in the loop, cast the child as an `HTMLElement` using the `as` keyword.

# --hints--

Every child element in `cardButtonsContainer` should have an `onclick` listener.

```js
frontInput.value = 'What is the capital of France?';
backInput.value = 'Paris';
uploadNewCard();
for (let i = 0; i < cardButtonsContainer.children.length; i++) {
  assert.exists(cardButtonsContainer.children[i].onclick);
}
```

Clicking a child element should change `currentCardIndex`.

```js
frontInput.value = 'What is the capital of France?';
backInput.value = 'Paris';
uploadNewCard();
let oldValue = currentCardIndex;
cardButtonsContainer.children[0].click();
assert.notEqual(oldValue, currentCardIndex);
```

Clicking a child element should also call `refresh`.
