...
Inside of the `refresh` function, the currently selected element from the `cardButtonsContainer` children needs to have the `selected` 
class added to it's `classList` and removed from the other children. 



# --hints--

When the `refresh` method is called, only one child should have the `selected` class. 
...
uploadNewCard();
refresh(); 
const selectedButtons = [...cardButtonsContainer.children].filter(x => x.classList.contains("selected")); 
assert.lengthOf(selectedButtons,1); 
```

The index of the `selected` element matches the `currentCardIndex`. 

```js
const selectedButton = [...cardButtonsContainer.children][currentCardIndex]
assert.isTrue(selectedButton.classList.contains("selected")); 
```
