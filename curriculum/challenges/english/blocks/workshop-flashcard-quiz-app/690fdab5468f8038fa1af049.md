...
The last element event listener to be added should be a `click` event listener for the `deleteBtn`. When the 
`click` event is called it should be calling the `deleteCard` function. 

# --hints--

The `deleteBtn` should have a `click` event listener. 

```js
const controller = new FlashCardController();
const spy = __helpers.spyOn(controller.elements.deleteBtn, 'addEventListener'); 
assert.lengthOf(spy.calls, 1);
assert.strictEqual(spy.calls[0][0],"click")
spy.restore();
```

When the `click` event is called on the `deleteBtn`, a card should be deleted.

```js
let oldLength = currentCards.length; 
let controller = new FlashCardController();
controller.elements.deleteBtn.click(); 
assert.isBelow(currentCards.length, oldLength); 
```
...
```js
// Manual hack to define the method 
FlashCardController.prototype.flipCard = function()
{
