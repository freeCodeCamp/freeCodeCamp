...
While the new data exists in the browser, the currently displayed card isn't getting updated.

Create a function called `refresh` that returns `void`.

# --hints--

A function called `refresh` should exist.

```js
assert.isFunction(refresh);
```

The `refresh` function should explicitly return `void`.

```js
assert.match(code, /function refresh\(\):\s+void/);
```
