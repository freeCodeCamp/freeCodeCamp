...
Create two variables called `questionText` and `questionAnswer`. Assign `questionText` to the value of `frontInput` and assign
`questionAnswer` to the value of `backInput`. 
...
```js
assert.match(code,/(var|let|const)\s+questionText/);
```

You should create a variable called `questionAnswer`. 

```js
assert.match(code,/(var|let|const)\s+questionAnswer/);
```

You should assign `questionText` to the value of `frontInput`. 

```js
assert.match(code,/(var|let|const)\s+questionText\s+=\s+frontInput\.value/);
```

You should trim the value of `frontInput` before assigning it to `questionText`. 

```js
assert.match(code,/(var|let|const)\s+questionText\s+=\s+frontInput\.value\.trim\(\)/);
```

You should assign `questionAnswer` to the value of `backInput`. 

```js
assert.match(code,/(var|let|const)\s+questionAnswer\s+=\s+backInput\.value/);
```


You should trim the value of `backInput` before assigning it to `questionAnswer`. 

```js
assert.match(code,/(var|let|const)\s+questionAnswer\s+=\s+backInput\.value\.trim\(\)/);
```
