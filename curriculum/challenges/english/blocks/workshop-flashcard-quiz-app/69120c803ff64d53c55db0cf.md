...
Before minor issues in the app get corrected, automatically upload the first card by filling out the `frontInput` and `backInput`. Then
calling `uploadNewCard`.

The value `frontInput` should receive is `"What is the capital of France?"`

The value `backInput` should receive is `"Paris"`. 

# --hints--

The newly added question should be `"What is the capital of France?"`
...
assert.strictEqual(currentCards[0].questionText,"What is the capital of France?");
```

The newly added answer should be `"Paris"`
...
assert.strictEqual(currentCards[0].questionAnswer,"Paris");
```
