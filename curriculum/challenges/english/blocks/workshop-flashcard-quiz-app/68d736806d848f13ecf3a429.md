...
Following up on creating a collection of cards, it's time to create a function to add additional 
card buttons. Create an empty function called `createCardButton` that takes a `string` parameter called `questionText` 
and a `number` parameter called `index`. The `createCardButton` function should return an `HTMLButtonElement`. 

# --hints--

You should have defined a `createCardButton` function. 

```js
assert.exists(createCardButton); 
assert.isFunction(createCardButton);
```

The `createCardButton` function should have a first parameter named `questionText`. 

```js
const params = __helpers.getFunctionParams(createCardButton.toString()); 
assert.strictEqual(params[0].name, "questionText")
```

The `createCardButton` function should have a second parameter named `index`. 

```js
const params = __helpers.getFunctionParams(createCardButton.toString()); 
assert.strictEqual(params[1].name, "index")
```
