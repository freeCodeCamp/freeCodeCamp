...
The application looks great and works pretty but it needs some touching up. Start by creating a
new class `InvalidUserInputError` that extends the `Error` class. Inside it, create a `constructor` function that
takes a `string` called `message`.

Also inside of the constructor call the `super` method while passing an empty string. We will update it in the next chapter.

# --hints--

You should have a new class `InvalidUserInputError`.

```js
assert.isFunction(InvalidUserInputError);
```

`InvalidUserInputError` extends the `Error` class.

```js
assert.match(code, /class\s+InvalidUserInputError\s+extends\s+Error\s+{/);
```

You should have a `constructor` function inside of `InvalidUserInputError`.

```js
assert.exists(InvalidUserInputError.prototype.constructor);
```

The `constructor` function should take a parameter called `message`.

```js
assert.match(
  code,
  /class\s+InvalidUserInputError\s+extends\s+Error\s+{\n\s+constructor\(message/
);
```

The `constructor` function should take a parameter of type `string`.

```js
assert.match(
  code,
  /class\s+InvalidUserInputError\s+extends\s+Error\s+{\n\s+constructor\(message\s*:\s*string\)/
);
```

You should call the `super` function in the constructor.

```js
assert.match(
  code,
  /class\s+InvalidUserInputError\s+extends\s+Error\s+{\n\s+constructor\(message\s*:\s*string\)\s*{\n\s*super\s*\(\s*("|')\s*("|')/
);
```
