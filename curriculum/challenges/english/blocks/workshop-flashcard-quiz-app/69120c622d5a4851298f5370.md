...
Next create a `private` method called `flipCard` that returns `void`. This function will flip the `card` displayed
on screen and show the hidden side.

# --hints--

There is a function called `flipCard` defined on `FlashCardController`.

```js
assert.isFunction(FlashCardController.prototype.flipCard);
```

The `flipCard` function is `private`.

```js
assert.match(code, /private\s*flipCard\(\)/);
```

The `flipCard` function explicitly returns `void`.

```js
assert.match(code, /private\s*flipCard\(\)\s*:\s*void/);
```
