...
Inside of the `FlashCardController`'s constructor, call it's `initializeEventListeners` method. 

# --hints--

The `initializeEventListeners` method inside of the `constructor` method. 

```js
const oldRegex = /deleteBtn\s*:\s*document\.querySelector<HTMLButtonElement>\("#delete-btn"\)!,\s+};\s+/;
const newRegex = /this\.initializeEventListeners\(\);?/
const regex = __helpers.concatRegex(oldRegex,newRegex); 
