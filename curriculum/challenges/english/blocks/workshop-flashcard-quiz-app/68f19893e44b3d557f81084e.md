...
Inside of the `FlashCardController` class, you should declare a `private` object called `elements`. The `private` keyword
declares that the given variable is only meant to be accessible from that particular object.

Additionally initialize it to an empty object.

# --hints--

A variable called `elements` should be declared inside of `FlashCardController`.

```js
let controller = new FlashCardController();
assert.property(controller, 'elements');
```

The `elements` variable should be an object.

```js
let controller = new FlashCardController();
assert.isObject(controller.elements);
```

The `elements` variable was declared using the `private` keyword.

```js
assert.match(code, /private\s+elements\s*:/);
```
