...
Revisiting the `uploadNewCard`, if the `frontInput` or `backInput` elements are empty, they are allowed to push their 
values into the `currentCards` array. In order to combat this, an error will have to be thrown. Surround the function with a `try/catch`
block and inside the `catch` block store the error as `ex`. 

# --hints--

There should be a `try/catch` block inside of `uploadNewCard` 

```js
assert.match(uploadNewCard.toString(),/try\s+{.+catch\(/);
```

The catch value captures the error as `ex` .

```js
assert.match(uploadNewCard.toString(),/catch\(ex\)\s+{/);
```
