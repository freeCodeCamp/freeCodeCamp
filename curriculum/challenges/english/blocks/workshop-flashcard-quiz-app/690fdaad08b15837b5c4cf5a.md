...
Inside of the `FlashCardController` class, define a `initializeEventListeners` method that is `private` and
returns `void`.

# --hints--

A `initializeEventListeners` method is defined on the `FlashCardController` class.

```js
assert.isFunction(FlashCardController.prototype.initializeEventListeners);
```

The `initializeEventListeners` method is `private`.

```js
assert.match(code, /private\s+initializeEventListeners/);
```

The `initializeEventListeners` method returns `void`.

```js
assert.match(code, /private\s+initializeEventListeners\(\)\s*:\s*void/);
```
