...
Another function called `isButtonElement` needs to be created. This function will determine if a passed
value is an `HTMLButtonElement`. 

For this function, it's preferable to declare this function similar to how variables are declared using the 
`const` keyword. 

This function should take `element` as an argument and have `element` be explicitly typed as `unknown`. Variables typed
as `unknown` can only have their types determined during runtime and should be avoided whenever possible.

One additional thing, this function should explicitly marked to return `element is HTMLButtonElement`. The `is` keyword is used as 
a type guard for the function and will tell the transpiler inside of any code block `element` is used, narrow the type to
`HTMLButtonElement`. 

For now, the function should return `true` in the function body. In the next step it will be updated. 

# --hints--

You should declare an `isButtonElement` function. 

```js
assert.isFunction(isButtonElement);
```

Your `isButtonElement` function should take `element` as a parameter.

```js
assert.match(code,/const\s+isButtonElement\s*=\s*\(element/)
```

Your `element` variable should explicitly be typed as `unknown`. 

```js
assert.match(code,/const\s+isButtonElement\s*=\s*\(\s*element\s*:\s*unknown\)\s*:/)
```

`isButtonElement` should explicitly marked to return `element is HTMLButtonElement`

```js
assert.match(code,/const\s+isButtonElement\s*=\s*\(\s*element\s*:\s*unknown\)\s*:\s*element\s+is\s+HTMLButtonElement/); 
```

`isButtonElement` should return `true` in the function body.

```js
assert.isTrue(isButtonElement());
```
