...
One additional thing, this function should explicitly return `element is HTMLButtonElement`. The `is` keyword is used as 
...
```js
assert.match(isButtonElement.toString(),/function\s*isButtonElement\(element\)/)
```
...
`isButtonElement` should return `element is HTMLButtonElement`
