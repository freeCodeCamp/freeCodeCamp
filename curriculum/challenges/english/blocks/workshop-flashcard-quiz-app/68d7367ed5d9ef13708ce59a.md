...
Now that the data for a flash card can be stored, we should be able to keep track of how many flash cards there are.
Create a new array of type `FlashCard` called `currentCards` and assign it to an empty array. 
Explicitly set the type of `currentCards` to `FlashCard`. 

Note: Arrays can be explicitly mapped to a type much like other variables. 

# --hints--

There should be an variable called `currentCards`. 

```js
assert.exists(currentCards); 
```

The `currentCards` variable should be set to an empty array.

```js
assert.deepEqual(currentCards,[]); 
```

The `currentCards` variable should be explicitly mapped to a type. 

```js
const flashCardRegex = /(let|const) currentCards\s*:\s*FlashCard\[\]\s*=\s*=\s*\[\]/;
