...
Inside of the `InvalidUserInputError`, call the `super` method passing in the `message` parameter. This will call the 
constructor on the `Error` prototype. 
...
```js
assert.match(InvalidUserInputError.prototype.constructor.toString(),/super\(.+\);/)
```

You should pass `message` as a parameter inside the `super` call.

```js
assert.match(InvalidUserInputError.prototype.constructor.toString(),/super\(message);/)
```

You should set the `name` of `InvalidUserInputError` to `"InvalidUserInputError"`.

```js
const fakeError = new InvalidUserInputError(); 
assert.strictEqual(fakeError.name, "InvalidUserInputError");
```
