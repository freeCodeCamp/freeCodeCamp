...
Inside of the `initializeEventListeners` function on `FlashCardController` , three elements need to be assigned event listeners:
`entryForm`, `flipBtn`, and `deleteBtn`.

The first event listener to be added is the `flipBtn`. It needs a `click` event listener, and should call `this.flipCard` when
when the `click` event is called.

Note: In a future step the `flipCard` method will be created.

# --hints--

The `flipBtn` should have an `click` event listener.

```js
const controller = new FlashCardController();
const spy = __helpers.spyOn(controller.elements.flipBtn, 'addEventListener');
controller.initializeEventListeners();
assert.lengthOf(spy.calls, 1);
assert.strictEqual(spy.calls[0][0], 'click');
spy.restore();
```

The `flipBtn` element should call `this.flipCard` when the `click` event is ran.

```js
const spy = __helpers.spyOn(FlashCardController.prototype, 'flipCard');
const controller = new FlashCardController();
controller.elements.flipBtn.click();
assert.lengthOf(spy.calls, 1);
spy.restore();
```
