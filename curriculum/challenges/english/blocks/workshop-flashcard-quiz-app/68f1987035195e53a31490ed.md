...
Now that you can add cards, you have to be able to remove cards. Create a function called `deleteCard` that explicitly
returns void.

# --hints--

There should be a function called `deleteCard`

```js
assert.isFunction(deleteCard);
```

The `deleteCard` function should explicitly return `void`.

```js
assert.match(code, /function\s+deleteCard\(\)\s*:\s*void/);
```
