...
In global scope, create a variable called `currentCardIndex` and set it to `-1`. Then assign the `onclick` method of the `btn` variable to a function that sets `currentCardIndex` to the `index` variable. 

# --hints--

You should have a variable called `currentCardIndex` inside of global scope. 

```js
assert.isNumber(currentCardIndex); 
```

You should assign the value of `currentCardIndex` to `-1`. 

```js
assert.strictEqual(currentCardIndex, -1); 
```
