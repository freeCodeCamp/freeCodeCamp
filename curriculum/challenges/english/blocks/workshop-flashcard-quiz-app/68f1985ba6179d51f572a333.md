...
Append the new card button inside of `cardButtonsContainer` with the `appendChild` method so that the 
button shows up on the web page. 

Then set `currentCardIndex` to `newIndex`. This is preparation so that when a user creates a new card, the website
automatically switches to a new card.
...
The `frontInput` and `backInput` should not have any text content. 
