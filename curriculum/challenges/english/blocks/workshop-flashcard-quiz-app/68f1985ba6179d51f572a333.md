...
Then set `currentCardIndex` to `newIndex`. Afterwards set the `frontInput` and `backInput` values to empty strings.
...
```js
if (typeof cardButtonsContainer !== 'undefined') {
  ...
  uploadNewCard();
  assert.equal(cardButtonsContainer.children.length, cardButtonsLength + 1);
} else {
  assert.fail();
}
```
...
if (typeof cardButtonsContainer !== 'undefined') {
  ...
  backInput.value = 'Pass';
  uploadNewCard();
  assert.equal(currentCardIndex, cardButtonsLength);
} else {
  assert.fail();
}
```
...
```js
if (typeof frontInput !== 'undefined' && typeof backInput !== 'undefined') {
  frontInput.value = 'What should this test do?';
  backInput.value = 'Pass';
  uploadNewCard();
