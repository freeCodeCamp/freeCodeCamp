...
Inside the `uploadNewCard` function , append the new card button inside of `cardButtonsContainer` with the `appendChild` method.
That will add the new button to the web page. 

Then set `currentCardIndex` to `newIndex`. Immediately after that, call the `refresh` function. This is preparation 
so that when a user creates a new card, the website automatically switches to a new card.  

Afterwards set the `frontInput` and `backInput` values to empty strings. 

# --hints--

The new card button should have been added to `cardButtonsContainer`. 

```js
const cardButtonsLength = cardButtonsContainer.children.length; 
frontInput.value = "What should this test do?";
backInput.value = "Pass";
uploadNewCard();
assert.equal(cardButtonsContainer.children.length, cardsButtonLength + 1)
```

The `currentCardIndex` should be assigned to `newIndex`. 
...
backInput.value = "Pass";
uploadNewCard();
assert.equal(currentCardIndex, cardsButtonLength - 1)
```

The `frontInput` and `backInput` should not have any text content after running `uploadNewCard`. 

```js
frontInput.value = "What should this test do?";
backInput.value = "Pass";
uploadNewCard();
assert.isEmpty(frontInput.value.trim());
assert.isEmpty(backInput.value.trim());
```
