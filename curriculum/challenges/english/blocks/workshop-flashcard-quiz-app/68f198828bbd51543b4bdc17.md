...
Inside of the `deleteCard` function, the current card should be removed from `currentCards` array and the associated
button should be removed from `currentButtonsContainer`. In addition, the `currentCardIndex` should
be decreased by one to reflect the change. Afterwards call the `refresh` method.

# --hints--

`currentCards` should no longer contain the current card after calling `deleteCard`.

```js
frontInput.value = 'What is the capital of France?';
backInput.value = 'Paris';
uploadNewCard();
// Adding two cards because one card and deleting would break the app
frontInput.value = 'What is the capital of France?';
backInput.value = 'Paris';
uploadNewCard();
const currentCard = currentCards[1];
deleteCard();
assert.notInclude(currentCards, currentCard);
currentCards = [];
cardButtonsContainer.innerHTML = '';
currentCardIndex--;
```

The length of `currentCards` should be reduced by one after calling `deleteCard`.
...
The `cardButtonsContainer` should no longer have the button for the current card.
...
backInput.value = 'Paris';
uploadNewCard();
const currentCardButton = cardButtonsContainer.children[1];
deleteCard();
assert.notInclude(cardButtonsContainer.children, currentCardButton);
currentCards = [];
cardButtonsContainer.innerHTML = '';
currentCardIndex--;
```

The length of `cardButtonsContainer.children` should be reduced by one.
...
const currentCardButton = cardButtonsContainer.children[1];
deleteCard();
assert.lengthOf(cardButtonsContainer.children, 1);
currentCards = [];
cardButtonsContainer.innerHTML = '';
currentCardIndex--;
```

The `refresh` method should be called from inside the `deleteCard` method.
...
backInput.value = 'Paris';
uploadNewCard();
const spy = __helpers.spyOn(window, 'refresh');
deleteCard();
