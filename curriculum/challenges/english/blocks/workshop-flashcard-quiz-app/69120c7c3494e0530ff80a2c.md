...
Inside of the `DOMContentLoaded` event listener, instantiate a new instance of the `FlashCardController`. 

# --hints--

A new instance of `FlashCardController` has been created when the page loaded.

```js
const regex = /document\.addEventListener\(("|')DOMContentLoaded("|'),\s*\(event\s*:\s*Event\)\s*=>\s*{\s*new\s*FlashCardController\(\)/; 
