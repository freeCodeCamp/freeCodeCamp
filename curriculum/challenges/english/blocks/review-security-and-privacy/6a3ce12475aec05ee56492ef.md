...
- **Port number** (e.g., `443` — HTTPS default; `80` — HTTP default)
...
- **Organizational mergers:** Connecting two different corporate networks quickly creates blind spots — lost visibility over old servers, misconfigured firewalls, and excessive access permissions.
...
| **DPDP** | India | Digital Personal Data Protection Act — covers personal data of Indian residents. |
...
A **`.env` file** is a plain text file placed at the root of a project that stores sensitive configuration data as key-value pairs — API keys, database passwords, and port numbers — keeping them out of source code and away from public repositories.
