...
Now you can call `sortByPriorityDescending` on the local `eligible` array inside the `getEVAReadyCrew()` function so that it always returns the list by priority descending.

# --hints--

You should call `sortByPriorityDescending` with `eligible` inside `getEVAReadyCrew`.

```js
const cleaned = __helpers.removeJSComments(getEVAReadyCrew.toString()).replace(/\s+/g, "");
assert.match(cleaned, /sortByPriorityDescending\(eligible\)/);
```
