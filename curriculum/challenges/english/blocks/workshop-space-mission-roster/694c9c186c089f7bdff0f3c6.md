...
Generally, it is good practice to validate your input(s) inside a function. You should first loop through the input `crew` array to check for a duplicate `id` (an id equal to the input astronaut's). If you find one, call `console.log("Duplicate ID: " + astronaut.id)` to indicate the error, then call `return` to exit the function early. 
...
When given a valid input, your `addCrewMember` function should not invoke a `console.log()` error message or an early `return` exit.
