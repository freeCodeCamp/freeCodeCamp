...
First, create a copy of the input `crew` array using `slice()` and name the copy array `sorted`.

# --hints--

Use `slice()` to copy the input `crew` array into a new array named `sorted`.

```js
const cleaned = __helpers.removeJSComments(code).replace(/\s+/g, "");
assert.match(cleaned, /(let|const)sorted=crew\.slice\(\)/, "Use slice() to copy crew into sorted");
```
