...
Create a new helper function named `sortByPriorityDescending()` that accepts one `crew` parameter. This function should directly sort the input `crew` by priority descending, so it will not return anything. 
