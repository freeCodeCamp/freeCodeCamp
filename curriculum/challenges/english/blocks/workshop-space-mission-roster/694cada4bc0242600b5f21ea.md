...
Loop through the `remainingCrew` array and add each member astronaut to `squad` by using your `addCrewMember` function.

# --hints--

You should use a `for` loop to call your `addCrewMember` function with each astronaut in `remainingCrew`.
...
After the loop, your `squad` array should contain all ten astronauts.
