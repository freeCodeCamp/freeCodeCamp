...
Your function should log each astronaut's name in `updatedCrew`.

```js
const logs = [];
const originalLog = console.log;
console.log = (...args) => logs.push(args.join(" "));
const crewSample = [{ name: "A" }, { name: "B" }, { name: "C" }];
swapCrewMembers(crewSample, 0, 1);
console.log = originalLog;
assert.deepEqual(logs, ["B", "A", "C"]);
```
