...
const loopAndLogRegex = /(?:for\([^)]*updatedCrew[^)]*\)\s*\{[\s\S]*?console\.log\s*\([^)]*\.name[^)]*\)|_createForOfIteratorHelper\(updatedCrew\)[\s\S]*?console\.log\s*\([^)]*\.name[^)]*\))/;
assert.match(
  cleaned,
  loopAndLogRegex,
  'A for(...) loop over updatedCrew is required and console.log() must be inside the loop'
);
```
