...
Loop through the `updatedCrew` array and log each member astronaut's `name`. After the loop, return the `updatedCrew` array to complete your `swapCrewMembers` function.

# --hints--

You should use a `for` loop and `console.log()` to log the `name` of each astronaut inside `updatedCrew`. 
...
You should log only the `name` of each astronaut inside the `updatedCrew` array one time and in order.
