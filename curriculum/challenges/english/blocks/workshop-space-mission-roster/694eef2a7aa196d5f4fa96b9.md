...
Invoke your `getEVAReadyCrew` function with your `updatedSquad` roster and store the result in a new array named `EVAReadySquad`. Then, loop through the `EVAReadySquad` array and log the `name` of every member.

# --hints--

You should call your `getEVAReadyCrew` function with `updatedSquad` and store the result in a new array named `EVAReadySquad`.
