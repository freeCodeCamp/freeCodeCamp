...
Now, call `getEVAReadyCrew` on your `updatedSquad` roster and store the result in a new array named `EVAReadySquad`. Then, loop through `EVAReadySquad` and log the name of every member.

# --hints--

You should store the result of `getEVAReadyCrew(updatedSquad)` in `EVAReadySquad` using `let` or `const`.
...
`EVAReadySquad` should contain the EVA-ready crew in priority order after calling `getEVAReadyCrew`.
...
You should log each member of `EVAReadySquad` in order.

```js
const logs = [];
const originalLog = console.log;
console.log = (...args) => logs.push(args.join(" "));
const localEVA = getEVAReadyCrew(updatedSquad);
for (let i = 0; i < localEVA.length; i++) {
  console.log(localEVA[i].name);
}
console.log = originalLog;
assert.deepEqual(logs.slice(0, localEVA.length), localEVA.map(a => a.name));
```
