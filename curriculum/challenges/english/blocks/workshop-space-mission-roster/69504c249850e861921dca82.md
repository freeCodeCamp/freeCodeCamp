...
Console output should log the chunks and astronaut names.

```js
const cleaned = __helpers.removeJSComments(code).replace(/\s+/g, "");
assert.match(cleaned, /for\([^)]*EVAChunks[^)]*\)\{[^}]*console\.log[^;]*Chunk/, 'First for loop should iterate EVAChunks and log Chunk:');
assert.match(
  cleaned,
  /for\([^)]*EVAChunks[^)]*\)\{[\s\S]*?for\([^)]*EVAChunks[^)]*\)[\s\S]*?console\.log[^;]*\.name/,
  'Nested for loop should iterate inner EVAChunks elements and log .name'
);
```
