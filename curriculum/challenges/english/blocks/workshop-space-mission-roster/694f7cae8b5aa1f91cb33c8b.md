...
Your `chunkCrew` function should fail validation when `size` is less than 1. The message logged to the console should be `"Chunk size must be >= 1"`, followed by a `return` call to exit the function.
