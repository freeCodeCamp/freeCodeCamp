...
Mission Control needs one more function for logging a summary of your crew. Create a new, empty function named `printCrewSummary()` that accepts one `crew` parameter. Note: for this step, the function should be empty. 

# --hints--

You should create an `printCrewSummary` function.

```js
assert.isFunction(printCrewSummary);
```

Your function should have one parameter, `crew`.
