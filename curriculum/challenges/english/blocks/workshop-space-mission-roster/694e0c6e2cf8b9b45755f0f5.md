...
First, create an empty array named `eligible`. To populate this array, loop through every astronaut inside the input `crew` array and, if they are EVA-eligible (meaning their `isEVAEligible` property is `true`), push them into the `eligible` array. After the loop, return the `eligible` array.
...
Your loop should iterate through the entire `crew` array (including first and last elements).
