...
The `splice()` method is a powerful way to modify arrays, allowing you to add or remove elements from any position in an array, including the middle. Since `splice()` returns an array containing the elements removed, you can use it to swap two elements in an array with the following technique:
...
Call the `splice()` method with the `updatedCrew` array to swap the positions of its member astronauts at indices `fromIndex` and `toIndex`.
...
You should call `splice` with the `updatedCrew` array to perform the swap. You should use the one-line technique shown in the examples.
...
Your `swapCrewMembers` function should not mutate the original `crew` array (you should only modify the local `updatedCrew` copy array).
