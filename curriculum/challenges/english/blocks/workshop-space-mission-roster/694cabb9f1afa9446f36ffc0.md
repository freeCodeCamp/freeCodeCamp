...
Now you can add the first astronaut to the roster. Call `addCrewMember` with `squad` and `firstAstronaut`.
...
After the call, `squad` should contain one astronaut.

```js
assert.strictEqual(squad.length, 1);
```

The first astronaut in `squad` should be `firstAstronaut`.
