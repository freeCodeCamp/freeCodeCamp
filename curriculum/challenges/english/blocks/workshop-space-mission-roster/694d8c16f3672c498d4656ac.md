...
Your `swapCrewMembers()` function should validate indices inside an `if` statement.
...
Your `swapCrewMembers()` function should first validate indices and, if invalid, log the message `"Invalid crew indices"` before exiting the function using `return`.
