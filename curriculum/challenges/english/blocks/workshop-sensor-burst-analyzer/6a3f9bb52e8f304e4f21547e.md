...
Finally, you will create a function to combine cleaned data from multiple sensors into one flat array. Declare a function called `mergeSensorData` that takes `dataset1` and `dataset2` as parameters. Leave the body empty for now.

# --hints--

You should have a `mergeSensorData` function.

```js

```

`mergeSensorData` should accept two parameters.
