...
In prior lessons, you learned about the `concat()` method. This method creates a new array by merging two or more arrays. Here's an example:
...
"Use the `concat` method on `array1` to merge it with `array2` and return the result. You can also log `mergeSensorData([65, 75], [75, 53])` to see the two small arrays joined into one flat array.

# --hints--

`mergeSensorData` should use the `concat` method.

```js

```

`mergeSensorData` should return the combined array.

```js

```

`mergeSensorData` should not mutate either input array.
