...
Write a `for` loop that iterates over `cleanedBursts`. On each iteration, reassign `finalDataset` to the result of calling `mergeSensorData(finalDataset, cleanedBursts[i])`. This threads the merge across all cleaned bursts one at a time.

# --hints--

`finalDataset` should be a flat array with no nested arrays.

```js

```

`finalDataset` should have 4 elements.
