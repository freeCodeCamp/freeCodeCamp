...
Inside `mergeSensorData`, create copies of both parameters using the spread operator. Store them as `const array1` and `const array2`.

# --hints--

You should create `const array1 = [...dataset1]` inside `mergeSensorData`.

```js

```

You should create `const array2 = [...dataset2]` inside `mergeSensorData`.
