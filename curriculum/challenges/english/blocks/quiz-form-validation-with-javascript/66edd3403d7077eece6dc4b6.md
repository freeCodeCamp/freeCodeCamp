...
It returns `false` without displaying any error message to the user.
...
What does it mean when `patternMismatch` is `true`?
...
The value does not match the `pattern` attribute.
