...
The invalid input is automatically corrected.

---

An alert box is shown immediately.
...
What does the `reportValidity()` method do when called on a form element?
...
It silently returns `true` without showing any messages.
...
It checks the element’s validity and displays validation messages if the element is invalid.

### --question--

#### --text--

What happens if `reportValidity()` is called, but a field fails validation?
