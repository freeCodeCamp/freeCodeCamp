...
assert.exists(explorer.allFunctions.addOption);
assert.isFunction(addOption);
const parameters = explorer.allFunctions.addOption.parameters;
assert.lengthOf(parameters, 1);
assert.equal(parameters[0].toString(), 'option');
```
...
```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.allFunctions.vote);
assert.isFunction(vote);
const parameters = explorer.allFunctions.vote.parameters;
assert.lengthOf(parameters, 2);
assert.equal(parameters[0].toString(), 'option');
assert.equal(parameters[1].toString(), 'voterId');
```
