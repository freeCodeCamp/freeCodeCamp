...
The API endpoint returns JSON in the following shape (simplified for clarity):
...
1. Each image element should have a source derived from the `avatar_template` URL of the matched user (after replacing `{size}` with `30`). Because `avatar_template` is a relative path that already starts with `/`, prepend `avatarUrl` directly to it — do not add an extra `/` between them.
