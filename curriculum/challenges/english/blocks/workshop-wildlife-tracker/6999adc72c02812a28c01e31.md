...
```js
assert.exists(updateAge);
assert.isFunction(updateAge);
```

The `updateAge` function should have two parameters.

```js
assert.lengthOf(updateAge, 2);
```

You should call the `updateAge(elephant, 12)` function inside of the `console.log`.

```js
assert.match(
  code,
  /console\.log\(\s*updateAge\(\s*elephant\s*,\s*12\s*\)\s*\)/
);
```
