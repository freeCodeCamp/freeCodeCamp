...
You should call the `hasHabitat(tiger)` function inside of the `console.log`.

```js
assert.match(code, /console\.log\(\s*hasHabitat\(\s*tiger\s*\)\s*\)/);
```

You should call the `hasHabitat(elephant)` function inside of the `console.log`.

```js
assert.match(code, /console\.log\(\s*hasHabitat\(\s*elephant\s*\)\s*\)/);
```
