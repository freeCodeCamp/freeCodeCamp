...
const { renderMotorcycleCard } = explorer.getFunctions(true);
assert.isTrue(renderMotorcycleCard.hasReturnAnnotation("string"));
```
...
```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.getClasses().MotorcycleGalleryApp);
```
