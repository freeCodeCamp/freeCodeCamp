...
4. You should load `Motorcycle` data from the `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json` endpoint. 
   1. The `created_at` field in the raw JSON response is a `string`. You should convert it to a `Date` object for each motorcycle before returning the data.
   2. You should sort the returned motorcycles by `created_at` in descending order (most recent first).
5. You should create a function named `renderMotorcycleCard` that takes a single `Motorcycle` object as a parameter and returns a formatted HTML string for the motorcycle card. 
   ...
   3. Each motorcycle card component should have a wrapper `div` with a class of `motorcycle-card-image-container` containing an `img` element with a valid image and a class of `motorcycle-card-image`.
   4. Each motorcycle card component should have an element with a class of `motorcycle-card-year-badge` that lists the year in plain text.
...
7. The number of motorcycle cards that are displayed should be listed inside of an element with an id of `results-number`.
8. The displayed motorcycles should be filtered by name whenever the user types into the element with id of `name-filter-input`, updating on the `input` event.
...
async function fetchMotorcycles(): Promise<Motorcycle[]> {
    ...
    const rawMotorcycles: (Omit<Motorcycle, 'created_at'> & { created_at: string })[] = await result.json();
    const motorcycles: Motorcycle[] = rawMotorcycles.map((motorcycle) => ({
      ...motorcycle,
      created_at: new Date(motorcycle.created_at)
    }));
    return motorcycles.sort((a, b) => b.created_at.getTime() - a.created_at.getTime());
}
