...
4. You should load `Motorcycle` data from the `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycle-shop/motorcycles.json` endpoint.
5. You should create a function named `renderMotorcycleCard` that takes a single `Motorcycle` object as a parameter and returns the motorcycle card as a formatted HTML string.
   ...
   5. Each motorcycle-card component should have an element with a class of `motorcycle-card-year-badge` that lists the year in plain text.
...
6. You  should have a class called `MotorcycleGalleryApp`. 
   1. The class should have a `private` member called `allMotorcycles`, it should be statically typed to a type of an array of `Motorcycle` objects and set to empty array.
   ...
   3. The class should have a public function called `renderMotorcycles` that renders the equivalent motorcycle card components of the `allMotorcycles` property inside the element with id of `motorcycle-grid`.
   4. The class should have a `private` property `filterTerm` and should be statically typed as a string.
7. The number of motorcycle cards displayed should be listed in an element with the id `results-number`.
8. The `filterTerm` property should be updated on an `input` event.
9. You should create an instance of `MotorcycleGalleryApp`, await its `loadData` method, and then display the loaded motorcycle cards and update the count in `results-number`.
...
The `renderMotorcycleCard` function should return an HTML string whose outer element has a class of `motorcycle-card`.
...
const html = renderMotorcycleCard(sampleMotorcycle);
assert.isString(html);
const template = document.createElement("template");
template.innerHTML = html;
assert.lengthOf(template.content.children, 1);
assert.isTrue(template.content.firstElementChild.classList.contains("motorcycle-card"));
```
...
const explorer = await __helpers.Explorer(code);
const prop =
  explorer.classes.MotorcycleGalleryApp?.classProps.allMotorcycles;
assert.exists(prop);
```
...
The `filterTerm` property should be `private` and should be statically typed as a string.

```js
const explorer = await __helpers.Explorer(code);
const prop = explorer.classes.MotorcycleGalleryApp.classProps.filterTerm;
assert.isFalse(prop.isEmpty());
assert.isTrue(prop.isPrivate());
assert.isTrue(prop.hasAnnotation('string'));
```
...
```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.MotorcycleGalleryApp?.methods.loadData;
assert.exists(method);
assert.isFalse(method.isPrivate());
assert.isFalse(method.isProtected());
```
...
```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.MotorcycleGalleryApp?.methods.renderMotorcycles;
assert.exists(method);
assert.isFalse(method.isPrivate());
assert.isFalse(method.isProtected());
```

The `renderMotorcycles` function should render 25 motorcycle cards inside `#motorcycle-grid`, each containing an `img` element with a class of `motorcycle-card-image`.

```js
const gallery = new MotorcycleGalleryApp();
await gallery.loadData();
gallery.renderMotorcycles(); 
const cards = document.querySelectorAll("#motorcycle-grid .motorcycle-card");
assert.lengthOf(cards, 25);
for (const card of cards) {
  assert.exists(card.querySelector("img.motorcycle-card-image"));
}
```
...
Entering part of a motorcycle name in `#name-filter-input` should display only matching motorcycles and update `#results-number`.
...
try {
  for (const [term, expectedNames] of [
    ['Ninja', ['Ninja H2', 'Ninja 400']],
    ['Street', ['Street Triple RS', 'Street Glide']]
  ]) {
    input.value = term;
    input.dispatchEvent(new Event('input', { bubbles: true }));
    const cards = document.querySelectorAll('#motorcycle-grid .motorcycle-card');
    assert.lengthOf(cards, expectedNames.length);
    assert.sameMembers(
      Array.from(cards, card => card.querySelector('.motorcycle-card-title').textContent.trim()),
      expectedNames
    );
    assert.strictEqual(document.getElementById('results-number').textContent.trim(), String(expectedNames.length));
  }
} finally {
  input.value = '';
  input.dispatchEvent(new Event('input', { bubbles: true }));
...
async function fetchMotorcycles():  Promise<Motorcycle[]> {
    const res = await fetch(`https://cdn.freecodecamp.org/curriculum/labs/data/motorcycle-shop/motorcycles.json`);
    const motorcycles = await res.json();
    return [...motorcycles].map((m: Motorcycle) => ({ ...m, created_at: new Date(m.created_at) })).sort((a, b) => 
...
class MotorcycleGalleryApp {
  ...
  private renderResultsCount(): void {
    ...
    if (resultsNumber) {
      resultsNumber.textContent = this.allMotorcycles.filter((m: Motorcycle) => m.name.toLowerCase().includes(this.filterTerm.toLowerCase())).length.toString();
    }
  }
  ...
  renderMotorcycles(): void {
    ...
    const filteredResults = this.allMotorcycles.filter((m: Motorcycle) => m.name.toLowerCase().includes(this.filterTerm.toLowerCase()));

    if (filteredResults.length === 0 && noResults) {
