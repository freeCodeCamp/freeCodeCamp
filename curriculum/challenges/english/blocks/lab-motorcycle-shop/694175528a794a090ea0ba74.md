...
1. You should create a function named `fetchMotorcycles` that returns a `Promise` resolving to an array of `Motorcycle` objects.
2. You should load `Motorcycle` data from the `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json` endpoint. 
3. You should create a function named `renderMotorcycleCard` that takes a single `Motorcycle` object as a parameter and returns a string containing the HTML that contains a formatted HTML string for the motorcycle card.
...
4. You  should have a class called `MotorcycleGalleryApp`. 
...
5. The `Motorcycle` elements inside of the grid can be filtered by name using an element with an id of `name-filter-input`. 
6. The number of `Motorcycle` elements that are displayed should be listed inside of an element with an id of `results-number`.
7. The filter should be updated on an `input` event. 
