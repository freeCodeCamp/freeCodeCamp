...
To better visualize zero-indexing, you can think of `ourArray` mapping out like this:

- Index `0` holds `"a"` (1st position)
- Index `1` holds `"b"` (2nd position)
- Index `2` holds `"c"` (3rd position)
...
```
**Note:** If you try to access an index that is larger than the array itself (for example, `ourArray[5]`), JavaScript will not crash or throw an error. Instead, it will simply return the value `undefined`.
