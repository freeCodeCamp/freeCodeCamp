...
Below your third `source` element, add a fourth `source` element. Your fourth `source` element should have a `src` attribute with the value `https://cdn.freecodecamp.org/curriculum/labs/mapmethod.mov` and `type` attribute with the value `video/quicktime `.
...
assert.strictEqual(source?.getAttribute('src'), 'hhttps://cdn.freecodecamp.org/curriculum/labs/mapmethod.mov');
```
