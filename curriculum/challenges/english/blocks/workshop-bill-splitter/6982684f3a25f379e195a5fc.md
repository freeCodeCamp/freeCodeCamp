...
Use the `+=` operator to add `appetizers`, `main_courses`, `desserts`, and `drinks` to `running_total` in a single statement.

Finally, use `print()` to display `Total bill so far:` and the value of `running_total`. You can do this with `print('Total bill so far:', running_total)` or `print(f'Total bill so far: {running_total}')`.
...
You should update `running_total` using the `+=` operator to add all four course costs in a single statement.
