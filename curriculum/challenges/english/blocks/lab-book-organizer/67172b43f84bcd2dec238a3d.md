...
```js
const sortSpy = __helpers.spyOn(Array.prototype, 'sort');
let rerunResult;
let sortCalls;
let sortReturns;

try {
  rerunResult = new Function(`${code}\nreturn { filteredBooks, sortByYear };`)();
  sortCalls = sortSpy.calls;
  sortReturns = sortSpy.returns;
} finally {
  sortSpy.restore();
}
...
assert.isTrue(
  sortCalls.some(
    ([callback], index) =>
      callback === rerunResult.sortByYear &&
      sortReturns[index] === rerunResult.filteredBooks
  )
);
```
