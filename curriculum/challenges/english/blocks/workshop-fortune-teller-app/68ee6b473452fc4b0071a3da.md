...
Now do the same for `multipleCardsBtn` and `newReadingBtn` by attaching a click event listener to multipleCardsBtn that calls `multipleCardSelected`, and another to newReadingBtn that calls `newReading`.

# --hints--

You should have attached `click` event to the `multipleCardsBtn` element.

```js
assert.match(code, /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Inside the multipleCardsBtn callback event you should have called `multipleCardSelected` method.

```js
assert.match(code, /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

You should have attached `click` event to the `newReadingBtn` element.

```js
assert.match(code, /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Inside the newReadingBtn callback event you should have called `newReading` method.

```js
assert.match(code, /newReadingBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?newReading\s*\(/);
```
