...
Now, do the same for `multipleCardsBtn` and `newReadingBtn`.
Attach an event listener to `multipleCardsBtn` that calls `multipleCardSelected`.
and for newReadingBtn Attach an event listener to `newReadingBtn` that calls `newReading`.
