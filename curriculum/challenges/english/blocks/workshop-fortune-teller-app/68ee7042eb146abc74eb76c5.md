...
Again you need to make the card random and and reversed decalre variable named `isReversed` and `card` and for the first one call the `Math.random` methond and compare it with < 0.5 for the second one call `getRandomItem` pass `this.cards`.
