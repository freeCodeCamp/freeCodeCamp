...
Again you need to make the card random and reversed declare variable named `isReversed` and `card` and for the first one call the `Math.random` method and compare it with < 0.5 for the second one call `getRandomItem` pass `this.cards`.


# --hints--

You should have declared variable named `isReversed`.

```js
assert.match(code, /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)\s*=>\s*\{[\s\S]*?\bconst\s+isReversed\b/);
```

Your isReversed variable should use `Math.random()` to randomly decide if the card is reversed.

```js
assert.match(code, /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)\s*=>\s*\{[\s\S]*?const\s+isReversed\s*=\s*Math\.random\s*\(\s*\)\s*<\s*0\.5/);
```

You should have declared variable named `card`.

```js
assert.match(code, /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)\s*=>\s*\{[\s\S]*?\bconst\s+card\b/);
```

Your card variable should be assigned by calling `getRandomItem` method.

```js
assert.match(code, /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)\s*=>\s*\{[\s\S]*?const\s+card\s*=\s*getRandomItem\s*\(\s*this\.cards\s*\)/);
```

You should have passed `cards` as argument to getRandomItem.
