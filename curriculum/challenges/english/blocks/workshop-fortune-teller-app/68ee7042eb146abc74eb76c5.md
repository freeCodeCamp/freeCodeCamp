...
Again, you need to make the card random and reversed. Declare a variable named `isReversed` and `card`. For the first one, call the `Math.random` method and compare it with `< 0.5`. For the second one, call `getRandomItem` and pass `this.cards`.

# --hints--

You should declare a variable named `cardId`.
...
Your `card` variable should be assigned by calling the `getRandomItem` method.
...
You should pass `cards` as an argument to `getRandomItem`.
