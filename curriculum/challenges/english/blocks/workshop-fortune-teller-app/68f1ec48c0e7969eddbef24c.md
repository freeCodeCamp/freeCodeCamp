...
# --description--
Now inside the `hideElements`, pass these as arguments: `singleCard`, `multipleCard`, `fortuneContainer`, and `fortuneDescription`. Remember to get them from the elements.

# --hints--
You should pass `singleCard` as the first argument to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(\s*this\.elements\.singleCard/);
```

You should pass `multipleCard` as the second argument to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(\s*this\.elements\.singleCard\s*,\s*this\.elements\.multipleCard/);
```

You should pass `fortuneContainer` as an argument to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.fortuneContainer/);
```

You should pass `fortuneDescription` as an argument to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.fortuneDescription/);
```
