...
```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(\s*this\.elements\.singleCard/);
```

You should have passed `multipleCard` as second argument to `hideElements` function.

```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(\s*this\.elements\.singleCard\s*,\s*this\.elements\.multipleCard/);
```

You should have passed `fortuneContainer` as argument to `hideElements` function.

```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.fortuneContainer/);
```

You should have passed `fortuneDescription` as argument to `hideElements` function.

```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.fortuneDescription/);
```
