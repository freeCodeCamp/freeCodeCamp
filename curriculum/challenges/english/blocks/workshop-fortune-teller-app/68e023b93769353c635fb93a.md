...
Inside the `renderCard` function, create a template literal that returns a `div`. Inside the div, add an `h2` element that outputs `drawingType` and a figure element with the class `card_container ${isReversed ? "reversed-card" : ""}` and the attribute `data-id="${shortName}"`.

# --hints--

Inside the `renderCard` function you should have a `div` element returned using a template literal.

```js
const divTemplateRegex = /`\s*<\s*div\s*>[\s\S]*<\/\s*div\s*>\s*`/;
assert.match(code, divTemplateRegex);
```

Inside the `div` element you should have `h2` element with `drawingType` inside it.

```js
const headingWithRegex = /[\s\S]*<\s*h2\s*>\s*\$\{\s*drawingType\s*\}\s*<\/\s*h2\s*>[\s\S]/;
assert.match(code, headingWithRegex);
```

Inside the `div` element you should have `figure` element.

```js
const figureWithRegex = /<\s*figure\b[^>]*>[\s\S]*?<\/\s*figure\s*>/;
assert.match(code, figureWithRegex);
```

You should have class of `card_container ${isReversed ? "reversed-card" : ""}` for the `figure` element.

```js
const classRegex = /<\s*figure\s+class\s*=\s*"card_container\s*\$\{\s*isReversed\s*\?\s*"reversed-card"\s*:\s*""\s*\}"/;
assert.match(code, classRegex);
```

Your figure element should have attribute of `shortName`.

```js
const dataIdRegex = /<\s*figure[^>]*\sdata-id\s*=\s*"\$\{\s*shortName\s*\}"[^>]*>/;
assert.match(code, dataIdRegex);
```
