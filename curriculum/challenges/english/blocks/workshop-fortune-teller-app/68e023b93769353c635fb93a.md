...
Inside the template literal of the `renderCard` function, create a `div` element. Inside the `div`, add an `h2` element that outputs `drawingType`, and a `figure` element with the class `card_container ${isReversed ? "reversed-card" : ""}` and the attribute `data-id="${shortName}"`.
