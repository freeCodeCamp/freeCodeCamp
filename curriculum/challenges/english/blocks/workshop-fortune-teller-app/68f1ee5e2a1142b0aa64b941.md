...
Finally, create an instance of the `Game` class by declaring a variable named `game` and initializing it using the new keyword.

# --hints--


You should have  declared variable named `game`.

```js
assert.match( code, /\b(const|let|var)\s+game\b/);
```

Your game variable should be assigned by calling the `game` instance.

```js
assert.match( code, /\bgame\s*=\s*new\s+Game\s*\(/);
```
