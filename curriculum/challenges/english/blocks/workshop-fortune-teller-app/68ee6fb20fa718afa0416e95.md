...
As done for the previous multipleCard, get the `multipleCard` element again and set its innerHTML by iterating through `DrawingType`. Use `Object.values()` and call `Map` method on it, passing `type` as an argument.


# --hints--

You should have assigned multipleCard.innerHTML by calling Object.values.

```js
assert.match(code, /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

You should have passed `DrawingType` to Object.values method.

```js
assert.match(code, /Object\.values\s*\(\s*DrawingType\s*\)/);
```

You should have call `Map` method on Object.values.

```js
assert.match(code, /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Your Map should have parameter named `type`.

```js
assert.match(code, /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)/);
```
