...
```js
assert.match(__helpers.removeJSComments(code), /getRandomItem\s*=\s*<T,>\s*\(\s*items\s*:\s*T\[\]\s*\)/)
```

Your getRandomItem function should have return type of `T`.

```js
assert.match(__helpers.removeJSComments(code), /getRandomItem\s*=\s*<T,>\s*\([^)]*\)\s*:\s*T\s*=>/)
```

Inside your getRandomItem function you should have variable declared `index`.

```js
assert.match(__helpers.removeJSComments(code), /const\s+index\s*=/)
```

The `index` variable should use `Math.floor` and `Math.random` with `items.length` to produce a random index.

```js
assert.match(__helpers.removeJSComments(code), /Math\.floor\s*\(\s*Math\.random\s*\(\)\s*\*\s*items\.length\s*\)/)
```

getRandomItem function should return random item using the index random number form `items[index]`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+items\s*\[\s*index\s*\]/)
```
