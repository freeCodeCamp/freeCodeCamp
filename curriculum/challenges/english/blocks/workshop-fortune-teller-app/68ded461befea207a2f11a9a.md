...
Well, so far you have three functions: getElement, hideElements, and showElements.
Now, you need a function that can help you get a random fortune.

Declare a function named `getRandomItem` that takes an array of `items` as an argument. Use generics for this function.

Inside the function, use `Math.random()` and `Math.floor()` to select a random fortune from the items array, and don’t forget to return that random fortune.

# --hints--

You should declare arrow function named `getRandomItem`.

```js
assert.match(code, /getRandomItem/)
```

Your getRandomItem function should have parameter of `items` with the type of `T[]`.

```js
assert.match(code, /getRandomItem\s*=\s*<T>\s*\(\s*items\s*:\s*T\[\]\s*\)/)
```

Your getRandomItem function should have return type of `T`.

```js
assert.match(code, /getRandomItem\s*=\s*<T>\s*\([^)]*\)\s*:\s*T\s*=>/)
```

Inside your getRandomItem function you should have variable declared `index`.

```js
assert.match(code, /const\s+index\s*=/)
```

The index variable should be assign to the calling `math.random` and should round it with `math.floor`.

```js
assert.match(code, /Math\.floor\s*\(\s*Math\.random\s*\(\)\s*\*\s*items\.length\s*\)/)
```

The index variable assigned with math.random and math.floor should use `items.length` to make sure it's within the bound.

```js
assert.match(code, /Math\.floor\s*\(\s*Math\.random\s*\(\)\s*\*\s*items\.length\s*\)/)
```

getRandomItem function should return random item using the index random number form `items[index]`.

```js
assert.match(code, /return\s+items\s*\[\s*index\s*\]/)
```
