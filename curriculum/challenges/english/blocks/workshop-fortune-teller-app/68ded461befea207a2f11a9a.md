...
well  so far there only three function that can help us to `GetElement`, `hideElements` and `showElements`  so now we need function taht can help also to get random frotune  

declare function called `getRandomItem`, with array of `items` argument and you should use generics for this function too, inside the function use the `Math.floor` and `Math.random` methond to help you get random fortune
remember you should return the random fortune from the items argument 
