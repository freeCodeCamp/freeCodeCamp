...
Now, do the same for the rest of the properties listed below. use the `getElement` function to select each one.

`multipleCardsBtn → "#btn-multiple-cards",`, `multipleCard → ".multiple_card"`, `title → ".title"`,
`newReadingBtn → ".btn_reveal"`, `fortuneContainer → ".fortune_container"`, `fortuneDescription → ".fortune_description"`,
`headerTitle → ".header_title"`, `subTitle → ".sub_title"`, `cardTitle → ".desc_title"`, `description → ".description"`, `text → ".text"`,


# --hints--

You should have accessed the `multipleCardBtn` by using the `getElement` method and passing `#btn-multiple-cards`.

```js
assert.match(code, /multipleCardsBtn\s*:\s*getElement\s*\(\s*['"]#btn-multiple-cards['"]\s*\)/);
```

You should have accessed the `multipleCard` by using the `getElement` method and passing `multiple_card`.

```js
assert.match(code, /multipleCard\s*:\s*getElement\s*\(\s*['"]\.multiple_card['"]\s*\)/);
```

You should have accessed the `title` by using the `getElement` method and passing `title`.

```js
assert.match(code, /title\s*:\s*getElement\s*\(\s*['"]\.title['"]\s*\)/);
```

You should have accessed the `newReadingBtn` by using the `getElement` method and passing `btn_reveal`.

```js
assert.match(code, /newReadingBtn\s*:\s*getElement\s*\(\s*['"]\.btn_reveal['"]\s*\)/);
```

You should have accessed the `fortuneContainer` by using the `getElement` method and passing `fortune_container`.

```js
assert.match(code, /fortuneContainer\s*:\s*getElement\s*\(\s*['"]\.fortune_container['"]\s*\)/);
```

You should have accessed the `fortuneDescription` by using the `getElement` method and passing `fortune_description`.

```js
assert.match(code, /fortuneDescription\s*:\s*getElement\s*\(\s*['"]\.fortune_description['"]\s*\)/);
```

You should have accessed the `headerTitle` by using the `getElement` method and passing `header_title`.

```js
assert.match(code, /headerTitle\s*:\s*getElement\s*\(\s*['"]\.header_title['"]\s*\)/);
```

You should have accessed the `subTitle` by using the `getElement` method and passing `sub_title`.

```js
assert.match(code, /subTitle\s*:\s*getElement\s*\(\s*['"]\.sub_title['"]\s*\)/);
```

You should have accessed the `cardTitle` by using the `getElement` method and passing `desc_title`.

```js
assert.match(code, /cardTitle\s*:\s*getElement\s*\(\s*['"]\.desc_title['"]\s*\)/);
```

You should have accessed the `description` by using the `getElement` method and passing `description`.

```js
assert.match(code, /description\s*:\s*getElement\s*\(\s*['"]\.description['"]\s*\)/);
```

You should have accessed the `text` by using the `getElement` method and passing `text`.

```js
assert.match(code, /text\s*:\s*getElement\s*\(\s*['"]\.text['"]\s*\)/);
```
