...
Now, do the same for the rest of the properties listed below. Use the `getElement` function to select each one: `multipleCardsBtn → "#btn-multiple-cards"`, `multipleCard → ".multiple_card"`, `title → ".title"`, `newReadingBtn → ".btn_reveal"`, `fortuneContainer → ".fortune_container"`, `fortuneDescription → ".fortune_description"`, `headerTitle → ".header_title"`, `subTitle → ".sub_title"`, `cardTitle → ".desc_title"`, `description → ".description"`, `text → ".text"`.
...
```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*:\s*getElement\s*\(\s*['"]#btn-multiple-cards['"]\s*\)/);
```

You should access the `multipleCard` by using the `getElement` method and passing `.multiple_card`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCard\s*:\s*getElement\s*\(\s*['"]\.multiple_card['"]\s*\)/);
```

You should access the `title` by using the `getElement` method and passing `.title`.

```js
assert.match(__helpers.removeJSComments(code), /title\s*:\s*getElement\s*\(\s*['"]\.title['"]\s*\)/);
```

You should access the `newReadingBtn` by using the `getElement` method and passing `.btn_reveal`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*:\s*getElement\s*\(\s*['"]\.btn_reveal['"]\s*\)/);
```

You should access the `fortuneContainer` by using the `getElement` method and passing `.fortune_container`.

```js
assert.match(__helpers.removeJSComments(code), /fortuneContainer\s*:\s*getElement\s*\(\s*['"]\.fortune_container['"]\s*\)/);
```

You should access the `fortuneDescription` by using the `getElement` method and passing `.fortune_description`.

```js
assert.match(__helpers.removeJSComments(code), /fortuneDescription\s*:\s*getElement\s*\(\s*['"]\.fortune_description['"]\s*\)/);
```

You should access the `headerTitle` by using the `getElement` method and passing `.header_title`.

```js
assert.match(__helpers.removeJSComments(code), /headerTitle\s*:\s*getElement\s*\(\s*['"]\.header_title['"]\s*\)/);
```

You should access the `subTitle` by using the `getElement` method and passing `.sub_title`.

```js
assert.match(__helpers.removeJSComments(code), /subTitle\s*:\s*getElement\s*\(\s*['"]\.sub_title['"]\s*\)/);
```

You should access the `cardTitle` by using the `getElement` method and passing `.desc_title`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\s*:\s*getElement\s*\(\s*['"]\.desc_title['"]\s*\)/);
```

You should access the `description` by using the `getElement` method and passing `.description`.

```js
assert.match(__helpers.removeJSComments(code), /description\s*:\s*getElement\s*\(\s*['"]\.description['"]\s*\)/);
```

You should access the `text` by using the `getElement` method and passing `.text`.

```js
assert.match(__helpers.removeJSComments(code), /text\s*:\s*getElement\s*\(\s*['"]\.text['"]\s*\)/);
```
