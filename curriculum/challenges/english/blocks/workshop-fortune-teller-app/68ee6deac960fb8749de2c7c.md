...
Inside the singleCardSelected method, include the remaining arguments `chosenCard.name_short` and `chosenCard.img`.
and Outside the singleCardSelected method, reset the `multipleCard` element’s `innerHTML` to an empty string `""`.
