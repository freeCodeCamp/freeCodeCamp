...
Include the remaining arguments `chosenCard.name_short` and `chosenCard.img`. and Outside the `renderCard` function, reset the `multipleCard` element’s `innerHTML` to an empty string `""`.

# --hints--

You should have passed `chosenCard.name_short` as argument.

```js
assert.match(code, /renderCard\s*\([\s\S]*?chosenCard\.name_short/);
```

You should have passed `chosenCard.img` as last argument.

```js
assert.match(code, /renderCard\s*\([\s\S]*?chosenCard\.img/);
```

You should have reset `multipleCard` element innerHTML to empty `string`.

```js
assert.match(code, /this\s*\.\s*elements\s*\.\s*multipleCard\s*\.innerHTML\s*=\s*["']{2}/);
```
