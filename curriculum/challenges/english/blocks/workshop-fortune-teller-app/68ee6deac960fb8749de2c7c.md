...
Include the remaining arguments `chosenCard.name_short` and `chosenCard.img`. Outside the `renderCard` function, reset the `multipleCard` element's `innerHTML` to an empty string `""`.
