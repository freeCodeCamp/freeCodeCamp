...
Include the remaining arguments `chosenCard.name_short` and `chosenCard.img`. Then outside the `renderCard` function, reset the `multipleCard` element's `innerHTML` to an empty string.

# --hints--

You should pass `chosenCard.name_short` as an argument.

```js
assert.match(__helpers.removeJSComments(code), /renderCard\s*\([\s\S]*?chosenCard\.name_short/);
```

You should pass `chosenCard.img` as the last argument.

```js
assert.match(__helpers.removeJSComments(code), /renderCard\s*\([\s\S]*?chosenCard\.img/);
```

You should reset the `multipleCard` element's `innerHTML` to an empty string.

```js
assert.match(__helpers.removeJSComments(code), /this\s*\.\s*elements\s*\.\s*multipleCard\s*\.innerHTML\s*=\s*["']{2}/);
```
