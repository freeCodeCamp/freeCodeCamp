...
You should declare a variable named `target`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\([^)]*\)\s*\{[\s\S]*?\bconst\s+target\s*=/);
```

Your target variable should be assigned to `e.target`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\([^)]*\)\s*\{\s*[\s\S]*?\bconst\s+target\s*=\s*e\.\s*target/);
```
