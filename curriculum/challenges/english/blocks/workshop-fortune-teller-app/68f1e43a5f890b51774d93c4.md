...
const { target } = showFortune?.variables ?? {};
...
const { showFortune } = explorer.classes.Game?.methods ?? {};
const { target } = showFortune?.variables ?? {};
assert.isTrue(target?.value.matches("e.target"));
```
...
const explorer = await __helpers.Explorer(code);
const { showFortune } = explorer.classes.Game?.methods ?? {};
assert.exists(showFortune);
const method = __helpers.removeJSComments(showFortune.toString());
assert.match(method, /target\s*=\s*e\.target[\s\S]*?if\s*\(\s*\)\s*\{\s*\}/);
```
