...
Inside showFortune method declare variable called `target` and assign it to `e.target` and create another empty if statement.

# --hints--


You should have declared variable named `target`.

```js
assert.match(code, /showFortune\s*\([^)]*\)\s*\{[\s\S]*?\bconst\s+target\s*=/);
```

Your target variable should be assigned to `e.target`.

```js
assert.match(code, /showFortune\s*\([^)]*\)\s*\{\s*[\s\S]*?\bconst\s+target\s*=\s*e\.\s*target/);
```

Right after your target variable you should have empty `if` statement.

```js
assert.match(code, /showFortune\s*\([^)]*\)\s*\{[\s\S]*?if\s*\(\s*\)\s*\{\s*\}/);
```
