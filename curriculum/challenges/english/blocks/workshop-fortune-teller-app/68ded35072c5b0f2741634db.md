...
you probely guessed it that we need to have another function that can help to also to show Element in the DOM so declare function similar to the previouse one but this time with  you should use `classList.remove("hidden")` and the functin name should be `showElements`
