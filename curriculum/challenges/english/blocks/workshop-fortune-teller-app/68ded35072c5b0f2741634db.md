...
You probably guessed it  you need another function that shows elements in the DOM. Create a function similar to the previous one, but this time use `classList.remove("hidden")`. Name this function `showElements`.

# --hints--

You should have declared a function named `showElements`.

```js
assert.match(code , /const\s+showElements\s*=\s*/)
```
...
```js
const FunctionUsedRegex = /const\s+showElements\s*=\s*\(\s*\.\.\.elements\s*:\s*HTMLElement\[\]\s*\)\s*=>\s*{[\s\S]*}/;
assert.match(code, FunctionUsedRegex);
```

In your `showElements` function you should have `forEach` method attached to the `elements` parameter.

```js
const funcMatch = code.match(/const\s+showElements\s*=\s*\([\s\S]*?\)\s*=>\s*{([\s\S]*?)}/);
const body = funcMatch[1];
assert.match(body, /elements\s*\.\s*forEach\s*\(/);
```
...
const funcMatch = code.match(/const\s+showElements\s*=\s*\([\s\S]*?\)\s*=>\s*{([\s\S]*?)}/);
const body = funcMatch[1];
assert.match(body, /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Inside the `forEach` method, you should remove the hidden class using the `classList.remove` method.

```js
const funcMatch = code.match(/const\s+showElements\s*=\s*\([\s\S]*?\)\s*=>\s*{([\s\S]*?)}/);
const body = funcMatch[1];
assert.match(body, /el\s*\.\s*classList\s*\.\s*remove\s*\(\s*['"]hidden['"]\s*\)/)
```
