...
Inside the `map`, return the `renderCard` function by passing `type`, `isReversed`, `name_short`, and `img` as arguments, and add the `join` method on the `map` with an empty string.
