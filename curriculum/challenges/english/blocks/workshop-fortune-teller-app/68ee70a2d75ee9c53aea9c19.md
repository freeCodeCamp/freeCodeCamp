...
Inside the map return renderCard function by passing `type`, `isReversed`, `name_short` and `img` as argument and add join method on the map with empty string.

# --hints--

You should have returned `renderCard` function.

```js
assert.match(code, /return\s+renderCard\s*\(/);
```

You should have passed `type` as first argument in renderCard function.

```js
assert.match(code, /return\s+renderCard\s*\(\s*type\s*,/);
```

You should have passed `isReversed` as second argument in renderCard function.

```js
assert.match(code, /return\s+renderCard\s*\([\s\S]*?isReversed\s*,/);
```

You should have passed `card.name_short` as argument in renderCard function.

```js
assert.match(code, /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*,/);
```

You should have passed `card.img` as argument in renderCard function.

```js
assert.match(code, /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

You should have attached `join("")` on Map method.

```js
assert.match(code, /\.join\s*\(\s*["']{2}\s*\)/);
```
