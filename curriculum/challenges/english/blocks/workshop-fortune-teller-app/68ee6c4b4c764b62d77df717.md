...
# --description--
In some cases, the cards need to be reversed, since they are not always in the correct position.

Declare a variable called `isReversed`. inside the `singleCardSelected` method
Use the `Math.random()` method to generate a random boolean value.
To ensure the value stays within bounds, compare it with 0.5.
