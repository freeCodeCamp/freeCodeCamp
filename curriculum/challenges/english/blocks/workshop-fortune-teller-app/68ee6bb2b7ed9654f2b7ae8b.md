...
Add another event listener to the global `document` object that calls the `showFortune` function. Inside the callback, include a parameter `e` of type `Event`, and pass `e` as an argument to the `showFortune` function.
