...
Inside the `fetchCardsData` method, declare a variable named `response` and assign it by calling the `fetch` function with the following URL: `${CDN_URL}/card_data.json`.

Add the `await` keyword before the `fetch` call, and wrap the entire logic inside a `try...catch` block.
Make sure to include `error` as a parameter in the `catch` block so you can log or handle it properly.
...
```js
assert.match(__helpers.removeJSComments(code), /fetchCardsData\s*\([^)]*\)\s*{[\s\S]*?\bresponse\b/);
```

Your `response` variable should be assigned to the `fetch` function with the `${CDN_URL}/card_data.json` URL.

```js
assert.match(__helpers.removeJSComments(code), /const\s+response\s*=\s*await\s+fetch\s*\(\s*`\$\{CDN_URL\}\/card_data\.json`\s*\)/
);
```

Your `fetch` function should have the `await` keyword.

```js
assert.match(__helpers.removeJSComments(code), /response\s*=\s*await\s+fetch\s*\(/);
```

You should wrap your `fetch` function with a `try-catch` block.

```js
assert.match(__helpers.removeJSComments(code), /try\s*{[\s\S]*?await\s+fetch[\s\S]*?}\s*catch\s*\(/);
```

Your `catch` block should have an `error` parameter.

```js
assert.match(__helpers.removeJSComments(code), /catch\s*\(\s*error\s*\)/);
```
