...
Inside the fetchCardsData method, declare variable named response, assign to by calling the fetch function with the following URL:`${CDN_URL}/card_data.json`.
...
You should have declared `response` variable inside the fetchCardsData method.

```js
assert.match(code, /fetchCardsData\s*\([^)]*\)\s*{[\s\S]*?\bresponse\b/);
```

Your response variable should have assigned to `fetch` function with `${CDN_URL}/card_data.json` url.


```js
assert.match(code, /const\s+response\s*=\s*await\s+fetch\s*\(\s*`\$\{CDN_URL\}\/card_data\.json`\s*\)/
);
```

Your fetch function should have `await` keyword.

```js
assert.match(code, /response\s*=\s*await\s+fetch\s*\(/);
```

You should have wrapped your fetch function with `try-catch` block.

```js
assert.match(code, /try\s*{[\s\S]*?await\s+fetch[\s\S]*?}\s*catch\s*\(/);
```

Your catch block should have error parameter.

```js
assert.match(code, /catch\s*\(\s*error\s*\)/);
```
