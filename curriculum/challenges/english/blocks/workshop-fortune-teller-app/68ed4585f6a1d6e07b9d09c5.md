...
const { fetchCardsData } = explorer.classes.Game.methods;
assert.exists(fetchCardsData.variables.response);
```

Your `response` variable should be assigned by calling `await fetch` with the `${CDN_URL}/card_data.json` URL.

```js
assert.match(__helpers.removeJSComments(code), /const\s+response\s*=\s*await\s+fetch\s*\(\s*`\$\{CDN_URL\}\/card_data\.json`\s*\)/);
```

You should wrap your `fetch` call with a `try-catch` block.
