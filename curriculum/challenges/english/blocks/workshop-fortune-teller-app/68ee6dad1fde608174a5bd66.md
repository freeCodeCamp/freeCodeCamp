...
Within the singleCardSelected method, retrieve the `singleCard` element and update its `innerHTML` by calling `renderCard` function, Pass "Click the card and reveal the fortune" as the first argument and isReversed as the second argument.



# --hints--

You should have update singleCard innerHTML by calling `renderCard` function.

```js
assert.match(code, /singleCard\s*\.innerHTML\s*=\s*renderCard\s*\(/s);
```

renderCard function should have first argument of `"Click the card and reveal the fortune"`.

```js
assert.match(code, /renderCard\s*\(\s*["']Click the card and reveal the fortune["']/);
```

You should have passed `isReversed` as second argument.
