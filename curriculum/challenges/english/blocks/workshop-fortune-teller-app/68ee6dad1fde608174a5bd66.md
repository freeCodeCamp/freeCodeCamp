...
```js
assert.match(code, /singleCard\s*\.innerHTML\s*=\s*renderCard\s*\(/s);
```

Your `renderCard` function should have `"Click the card and reveal the fortune"` as the first argument.

```js
assert.match(code, /renderCard\s*\(\s*["']Click the card and reveal the fortune["']/);
```

You should pass `isReversed` as the second argument.

```js
assert.match(code, /renderCard\s*\([\s\S]*?["']Click the card and reveal the fortune["']\s*,\s*isReversed/);
```
