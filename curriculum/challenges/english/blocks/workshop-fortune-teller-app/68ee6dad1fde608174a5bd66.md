...
Within the singleCardSelected method, retrieve the `singleCard` element and update its `innerHTML` by calling `renderCard` Pass `"Click the card and reveal the fortune"` as the first argument and `isReversed` as the second argument.
