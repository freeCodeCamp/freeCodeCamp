...
Within the `singleCardSelected` method, retrieve the `singleCard` element and update its `innerHTML` by calling the `renderCard` function. Pass `"Click the card and reveal the fortune"` as the first argument, and `isReversed` as the second argument.

# --hints--

You should update the `singleCard` `innerHTML` by calling the `renderCard` function.

```js
assert.match(__helpers.removeJSComments(code), /singleCard\s*\.innerHTML\s*=\s*renderCard\s*\(/s);
```

Your `renderCard` function should have `"Click the card and reveal the fortune"` as the first argument.

```js
assert.match(__helpers.removeJSComments(code), /renderCard\s*\(\s*["']Click the card and reveal the fortune["']/);
```

You should pass `isReversed` as the second argument.

```js
assert.match(__helpers.removeJSComments(code), /renderCard\s*\([\s\S]*?["']Click the card and reveal the fortune["']\s*,\s*isReversed/);
```
