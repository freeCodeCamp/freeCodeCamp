...
The `img` element must have both `card-img` and `hidden` classes attached.
...
The `onload` method must be attached to the `img` element.
...
Within the onload handler, use `this.classList.remove("hidden")` to remove the hidden class.
