...
Add the `card-img` and `hidden` classes to the image element. Then use the `onload` event to remove the `hidden` class when the image loads and hide the loading placeholder using its `previousElementSibling`.

# --hints--

Your `img` element must have both `card-img` and `hidden` classes attached.

```js
const imgClassRegex = /<img\b[^>]*class\s*=\s*["'][^"']*\bcard-img\b[^"']*\bhidden\b[^"']*["'][^>]*>/;
assert.match(__helpers.removeJSComments(code), imgClassRegex)
```

Your `onload` method must be attached to the `img` element.

```js
const imgOnloadRegex = /<img\b[^>]*onload\s*=\s*["'][^"']*["'][^>]*>/;
assert.match(__helpers.removeJSComments(code), imgOnloadRegex)
```

Your onload handler, should use `this.classList.remove("hidden")` to remove the hidden class.

```js
const onloadRemoveHiddenRegex = /onload\s*=\s*["'][^"']*this\.classList\.remove\(["']hidden["']\)[^"']*["']/;
assert.match(__helpers.removeJSComments(code), onloadRemoveHiddenRegex)
```

The `previousElementSibling` of the `img` element must have its display style set to `"none"`.

```js
const previousSiblingDisplayRegex =  /onload\s*=\s*"[^"]*this\.previousElementSibling\.style\.display\s*=\s*'none';[^"]*"/;
assert.match(__helpers.removeJSComments(code), previousSiblingDisplayRegex)
```
