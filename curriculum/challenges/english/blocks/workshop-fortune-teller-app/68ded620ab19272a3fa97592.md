...
The parameters `drawingType`, `shortName`, and `img` must be of type `string`, while `isReversed` must be of type `boolean`. The function should have a return type of `string`, and for now should return an empty template literal — you will fill it in over the next few steps.
