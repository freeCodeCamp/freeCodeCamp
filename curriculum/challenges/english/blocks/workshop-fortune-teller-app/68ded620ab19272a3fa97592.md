...
Since the project now works with HTML DOM elements, you need a way to render the fortune to the DOM.
Declare a function named `renderCard` that takes four arguments: `drawingType`, `isReversed`, `shortName`, and `img`.
The parameters `drawingType`, `shortName`, and `img` must be of type `string`, while `isReversed` must be of type `boolean`.
The function’s return type should be `string`.

# --hints--

You should have function named `renderCard`.
...
```js
assert.match(__helpers.removeJSComments(code),/\(\s*[\s\S]*drawingType\s*:\s*string[\s\S]*isReversed\s*:\s*boolean[\s\S]*shortName\s*:\s*string[\s\S]*img\s*:\s*string[\s\S]*\)/)
```

Your `drawingType` parameter should have its type set to  `string`.

```js
assert.match(__helpers.removeJSComments(code), /drawingType\s*:\s*string/)
```

Your `shortName` parameter should have its type set to  `string`.

```js
assert.match(__helpers.removeJSComments(code), /shortName\s*:\s*string/)
```

Your `img` parameter should have its type set to  `string`.

```js
assert.match(__helpers.removeJSComments(code), /img\s*:\s*string/)
```

Your `isReversed` parameter should have its type set to `boolean`.

```js
assert.match(__helpers.removeJSComments(code), /isReversed\s*:\s*boolean/)
```

Your `renderCards` function should have return type of `string`.

```js
assert.match(__helpers.removeJSComments(code), /\)\s*:\s*string\s*=>/)
```
