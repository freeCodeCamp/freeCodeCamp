...
Declare a function named `renderCard` that takes four arguments: `drawingType`, `isReversed`, `shortName`, and `img`, and returns a template literal.

The parameters `drawingType`, `shortName`, and `img` must be of type `string`, while `isReversed` must be of type `boolean`. The function should have a return type of `string`.
...
const explorer = await __helpers.Explorer(code);
const { renderCard } = explorer.allFunctions;
assert.match(__helpers.removeJSComments(renderCard.toString()), /(?:return|=>)\s*`/);
```
