...
since we are working with HTML DOM element now we need way to render those furtune to the DOM so now declare `renderCard` function and you should have four argument with 
`drawingType, isReversed, shortName, img`, and for drawingType, shortName and img should have type of string and isReversed one with boolean type also the function return type should be string 
