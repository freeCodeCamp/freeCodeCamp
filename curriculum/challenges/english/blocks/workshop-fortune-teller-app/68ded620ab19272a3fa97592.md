...
Since the project is now working with HTML DOM elements, a way to render the fortune to the DOM is required.
...
You should have function named `renderCards`.

```js
assert.match(code, /renderCard\s*=\s*\(/)
```

Your renderCards function should have `four` parameter.

```js
assert.match(code,/\(\s*[\s\S]*drawingType\s*:\s*string[\s\S]*isReversed\s*:\s*boolean[\s\S]*shortName\s*:\s*string[\s\S]*img\s*:\s*string[\s\S]*\)/)
```

`drawingType` parameter should have type of `string`.

```js
assert.match(code, /drawingType\s*:\s*string/)
```

`shortName` parameter should have type of `string`.

```js
assert.match(code, /isReversed\s*:\s*boolean/)
```

`img` parameter should have type of `string`.

```js
assert.match(code, /shortName\s*:\s*string/)
```

`isReversed` parameter should have type of `boolean`.

```js
assert.match(code, /img\s*:\s*string/)
```

Your `renderCards` function should have return type of `string`.

```js
assert.match(code, /\)\s*:\s*string\s*=>/)
```
