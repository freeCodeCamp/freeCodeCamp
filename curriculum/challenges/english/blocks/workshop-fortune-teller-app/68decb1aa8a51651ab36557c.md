...
Now let's use the generics function, Inside the function add `querySelectore` that uses the generics and add if statement inside the function for chcking if the element does not exist  throw error with the message of `Element not found: `${error message here}`,  if the element exits return the element, Don't forget to declare variable for that. 
