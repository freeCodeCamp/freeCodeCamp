...
Now should use the generics function, Inside the function add `querySelector` that uses the generics and add if statement inside the function for checking if the element does not exist  throw error with the message of `Element not found: `${error message here}`,  if the element exits return the element, Don't forget to declare variable for that.
...
```js
assert.match(code, /const\s*\el\s*=\s*/)
```
...
assert.match(code, ifStatementRegex);
```
...
assert.match(code, ifInsideArrowRegex);
```
