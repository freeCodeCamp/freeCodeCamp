...
Now use the generics function. Inside the function, use `querySelector` with the generics and store the result in a variable. Add an if statement to check if the element does not exist, and if so, throw an error with the message `Element not found: ${error message here}}`. If the element exists, return it.

# --hints--

You should have declared a variable named `el`.

```js
assert.match(__helpers.removeJSComments(code), /const\s*\el\s*=\s*/)
```

Your selector should use the generic type `T` with the `selector` parameter passed to it.

```js
const ifStatementRegex = /if\s*\(\s*!\s*el\s*\)\s*(\{)?/;
assert.match(__helpers.removeJSComments(code), ifStatementRegex);
```

You should have if statement that check if `el` is not found.

```js
const ifInsideArrowRegex = /if\s*\(\s*!\s*el\s*\)\s*\{\s*[\s\S]*\}/;
assert.match(__helpers.removeJSComments(code), ifInsideArrowRegex);
```

Inside your if statement, you should throw an error with the message:
`Element not found: ${selector}`.

```js
const throwRegex = /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/;
assert.match(__helpers.removeJSComments(code), throwRegex);
```

Inside the getElement function, you should return the `el` variable.

```js
 assert.match(__helpers.removeJSComments(code), /return\s*el/)
```
