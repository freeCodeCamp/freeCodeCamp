...
Begin by declaring an interface named `Card`. Inside the `Card` interface, add the following properties: `name`, `name_short` and `value`. The `name`, and `name_short` properties should have the types set to `string` and the `value` should have a union type set to `string | number`.

# --hints--

You should have declared an interface named `Card`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*?}/)
```

Your `Card` interface should include a property `name` of type `string`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*name\s*:\s*string[\s\S]*}/);
```

Your `Card` interface should include a property `name_short` of type `string`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*name_short\s*:\s*string[\s\S]*}/);
```

Your `Card` interface should include a property `value` with a union type of `string` | `number`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*value\s*:\s*string\s*\|\s*number[\s\S]*}/);
```
