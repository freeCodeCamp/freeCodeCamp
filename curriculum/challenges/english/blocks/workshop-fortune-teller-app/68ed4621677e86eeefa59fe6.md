...
Since you're calling `initializeEventListeners()` inside the constructor, you need to define that method as well. Create the `initializeEventListeners` method inside the class and make it `private`. Give the return type `void`.

# --hints--

You should define a method named `initializeEventListeners`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.classes.Game.methods.initializeEventListeners);
```

Your `initializeEventListeners` method should be `private`.

```js
assert.match(__helpers.removeJSComments(code), /private\s+initializeEventListeners\s*\(/);
```

Your `initializeEventListeners` method should have a return type of `void`.

```js
assert.match(__helpers.removeJSComments(code), /initializeEventListeners\s*\([^)]*\)\s*:\s*void/);
```
