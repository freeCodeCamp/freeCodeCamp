...
Since you’re calling `initializeEventListeners()` inside the constructor, you need to define that method as well, Create the `initializeEventListeners` method inside the class and make it private. give the return type to void.


# --hints--

You need to define method named `initializeEventListeners`.

```js
assert.match(code, /(?<!this\.)initializeEventListeners\s*\(\s*\)\s*:\s*void/);
```

Your initializeEventListeners method should be `private`.

```js
assert.match(code, /private\s+initializeEventListeners\s*\(/);
```

Your initializeEventListeners method should have return type of `void`.

```js
assert.match(code, /initializeEventListeners\s*\([^)]*\)\s*:\s*void/);
```
