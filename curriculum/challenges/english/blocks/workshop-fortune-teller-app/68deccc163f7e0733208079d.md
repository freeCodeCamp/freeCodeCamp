...
now we have function that can help us to selecet element from the DOM, so for now your are going to create function that help to hide element from the DOM, 
declare `hideElements` function and pass `...elements as argument and give it type of `HTMLElement[]`. 
