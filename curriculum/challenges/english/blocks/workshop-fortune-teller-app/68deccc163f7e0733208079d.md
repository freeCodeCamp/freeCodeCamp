...
Now you have function that can help you to select element from the DOM, so for now your are going to create function that help to hide element from the DOM, declare `hideElements` function and pass `...elements` as argument and give it type of `HTMLElement[]`.
...
```js
assert.match(code , /const\s+hideElements\s*=\s*/)
```

Your hideElements function should have a parameter named `...elements` with the type `HTMLElement[]`.

```js
const FunctionUsedRegex = /const\s+hideElements\s*=\s*\(\.\.\.elements\s*:\s*HTMLElement\[\]\)\s*=>\s*{\s*}/;
