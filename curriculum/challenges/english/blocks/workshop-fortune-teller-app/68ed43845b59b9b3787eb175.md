...
Inside the class you should have `constructor`.

```js
assert.match(code, /class\s+\w+\s*{[^}]*constructor\s*\(/);
```

Inside your constructor should have access `elements` method.

```js
assert.match(code, /constructor\s*\([^)]*\)\s*{[^}]*this\.elements\s*\(/);
```

You should have access the `singleCardBtn` by using `getElement` method by passing `#btn-single-card id`.

```js
assert.match(code, /singleCardBtn\s*=\s*this\.getElement\(['"]#btn-single-card['"]\)/);
```

You should have access the `singleCard` by using `getElement` method by passing `.single_card` class.

```js
assert.match(code, /singleCard\s*=\s*this\.getElement\(['"]\.single_card['"]\)/);
```
