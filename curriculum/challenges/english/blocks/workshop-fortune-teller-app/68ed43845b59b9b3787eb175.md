...
```js
assert.match(code, /class\s+\w+\s*{[\s\S]*?constructor\s*\(/);
```

Your constructor should access the `elements` property.

```js
assert.match(code, /constructor\s*\([^)]*\)\s*{[\s\S]*?this\.elements\s*=/);
```

You should access the `singleCardBtn` by using the `getElement` method and passing the `#btn-single-card` id.

```js
assert.match(code, /this\.elements\s*=\s*{[\s\S]*?singleCardBtn\s*:\s*getElement\s*\(\s*['"]#btn-single-card['"]\s*\)/
);
```
...
```js
assert.match(code, /this\.elements\s*=\s*{[\s\S]*?singleCard\s*:\s*getElement\s*\(\s*['"]\.single_card['"]\s*\)/
);
```
