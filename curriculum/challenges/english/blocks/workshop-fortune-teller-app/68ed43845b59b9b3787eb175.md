...
```js
assert.match(__helpers.removeJSComments(code), /singleCardBtn\s*:\s*getElement\s*\(\s*['"]#btn-single-card['"]\s*\)/);
```

`singleCard` should be set using `getElement` with `'.single_card'`.

```js
assert.match(__helpers.removeJSComments(code), /singleCard\s*:\s*getElement\s*\(\s*['"]\.single_card['"]\s*\)/);
```
