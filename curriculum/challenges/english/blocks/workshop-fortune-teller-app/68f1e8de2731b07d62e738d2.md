...
Declare another variable named `foundCard`, and assign it to the result of calling the `find()` method on `cards`, passing `card` as the callback parameter, and compare `card.name_short` with `cardId` to find the matching card.

# --hints--

You should declare a variable named `foundCard`.

```js
assert.match(__helpers.removeJSComments(code), /\b(?:const|let|var)\s+foundCard\b/);
```

You should attach the `find` method on `cards`.

```js
assert.match(__helpers.removeJSComments(code), /\bcards\.find\s*\(/);
```

You should pass a parameter named `card` inside the `find` method callback function.

```js
assert.match(__helpers.removeJSComments(code), /\bcards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

You should have `card.name_short` comparing it with `cardId` inside the callback function.
