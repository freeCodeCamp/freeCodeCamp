...
Your `foundCard` should be assigned by calling `find` on `this.cards`, passing a callback that takes a `card` parameter and implicitly returns the result of comparing `card.name_short` with `cardId` using strict equality.
