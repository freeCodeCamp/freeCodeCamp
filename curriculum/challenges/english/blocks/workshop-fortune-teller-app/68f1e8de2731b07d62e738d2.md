...
Declare another variable named `foundCard`, assign it to the result of calling the `find()` method on `cards`, passing `card` as the callback parameter, and compare `card.name_short` with `cardId` to find the matching card.

# --hints--

You should have declared variable named `foundCard`.

```js
assert.match(code, /\b(?:const|let|var)\s+foundCard\b/);
```

You should have attached `find` method on cards.

```js
assert.match(code, /\bcards\.find\s*\(/);
```

You should have passed parameter name `card` inside `find` method callback function.

```js
assert.match(code, /\bcards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

You should have `card.name_short` comparing it with cardId inside the callback function.

```js
assert.match(code, /card\.name_short\s*===?\s*cardId/);
```
