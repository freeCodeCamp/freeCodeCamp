...
Declare a method named `newReading`, and inside it, call `showElements()` and `hideElements()`.

# --hints--


You should method named `newReading`.

```js
assert.match( code, /\bnewReading\s*\(\s*\)\s*\{/);
```

Inside newReading method you should have called `showElements` function.

```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

Inside newReading method you should have called `hideElements` function.

```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(/);
```
