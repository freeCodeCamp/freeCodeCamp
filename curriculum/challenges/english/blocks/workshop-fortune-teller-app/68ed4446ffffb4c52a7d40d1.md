...
```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\([^)]*\)\s*{[\s\S]*?fetchCardsData\s*\(\s*\)/);
```

You should call `initializeEventListeners()` method inside the constructor.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\([^)]*\)\s*{[\s\S]*?initializeEventListeners\s*\(\s*\)/);
```
