...
Inside the constructor, add calls to `fetchCardsData()` and `initializeEventListeners()` so they run when the class is initialized.



# --hints--

You should have called `fetchCardsData()` method inside the `constructor`.

```js
assert.match(code, /constructor\s*\([^)]*\)\s*{[\s\S]*?fetchCardsData\s*\(\s*\)/);
```

You should have called `initializeEventListeners()` method inside the constructor.

```js
assert.match(code, /constructor\s*\([^)]*\)\s*{[\s\S]*?initializeEventListeners\s*\(\s*\)/);
```
