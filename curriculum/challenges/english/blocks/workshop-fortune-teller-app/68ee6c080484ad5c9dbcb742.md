...
Now add another method called `singleCardSelected` inside the method call `hideElements` function and pass the following property. Remember you must access all of them using `this.elements`.
`singleCardBtn`, `multipleCardsBtn`, `multipleCard`, `text`, `headerTitle`

# --hints--

You should have defined method named `singleCardSelected`.

```js
assert.match(code, /\bsingleCardSelected\s*\([^)]*\)\s*\{/);
```

Inside singleCardSelected you need to call `hideElements` function.

```js
assert.match(code, /singleCardSelected[\s\S]*?\{[\s\S]*?hideElements\s*\(/,);
```

You should have passed `singleCardBtn` to the hideElements function.

```js
assert.match( code, /hideElements\s*\([\s\S]*?this\.elements\.singleCardBtn/);
```

You should have passed `multipleCardsBtn` to the hideElements function.

```js
assert.match(code, /hideElements\s*\([\s\S]*?this\.elements\.multipleCardsBtn/);
```

You should have passed `multipleCard` to the hideElements function.

```js
assert.match(code, /singleCardSelected\s*\([^)]*\)\s*\{[\s\S]*?hideElements\s*\([^)]*this\.elements\.multipleCard[^)]*\)/);
```

You should have passed `text` to the hideElements function.

```js
assert.match(code, /hideElements\s*\([\s\S]*?this\.elements\.text/);
```

You should have passed `headerTitle` to the hideElements function.

```js
assert.match(code, /hideElements\s*\([\s\S]*?this\.elements\.headerTitle/);
```
