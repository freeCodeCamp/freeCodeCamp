...
Now add another method called `singleCardSelected`. Inside the method, call the `hideElements` function and pass the following properties. Remember you must access all of them using `this.elements`: `singleCardBtn`, `multipleCardsBtn`, `multipleCard`, `text`, and `headerTitle`.
...
assert.exists(explorer.classes.Game.methods.singleCardSelected);
```

Inside `singleCardSelected`, you need to call the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /singleCardSelected[\s\S]*?\{[\s\S]*?hideElements\s*\(/,);
```

You should pass `singleCardBtn` to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /hideElements\s*\([\s\S]*?this\.elements\.singleCardBtn/);
```

You should pass `multipleCardsBtn` to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /hideElements\s*\([\s\S]*?this\.elements\.multipleCardsBtn/);
```

You should pass `multipleCard` to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /singleCardSelected\s*\([^)]*\)\s*\{[\s\S]*?hideElements\s*\([^)]*this\.elements\.multipleCard[^)]*\)/);
```

You should pass `text` to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /hideElements\s*\([\s\S]*?this\.elements\.text/);
```

You should pass `headerTitle` to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /hideElements\s*\([\s\S]*?this\.elements\.headerTitle/);
```
