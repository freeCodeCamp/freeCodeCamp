...
Now add another method called `showFortune` as parameter pass `e` and give it type of `Event`.


# --hints--

You should have declared method named `showFortune`.

```js
assert.match(code, /\bshowFortune\s*\(\s*e\s*:\s*Event\s*\)\s*\{/);
```

You should have passed `e` as argument.

```js
assert.match(code, /\bshowFortune\s*\(\s*e\s*:\s*Event\s*\)\s*\{/);
```

Your `e` argument should have type of `Event`.

```js
assert.match(code, /\bshowFortune\s*\(\s*e\s*:\s*Event/);
```
