...
Now add another method called `showFortune`. As a parameter, pass `e` and give it the type `Event`.
...
You should pass `e` as an argument.

```js
assert.match(code, /\bshowFortune\s*\(\s*e\s*:\s*Event\s*\)\s*\{/);
```

Your `e` argument should have the type `Event`.
