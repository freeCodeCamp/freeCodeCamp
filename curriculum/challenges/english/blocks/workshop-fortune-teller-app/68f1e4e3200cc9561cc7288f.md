...
```js
assert.match(code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?if\s*\(\s*!\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

You should throw an error inside the `if` statement.

```js
assert.match(code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?if\s*\([\s\S]*?\)\s*\{[\s\S]*?throw\s+new\s+Error/);
```

Your error message should be `"Target element is not correct"`.

```js
assert.match(code, /throw\s+new\s+Error\s*\(\s*["']target element is not correct["']\s*\)/i);
```
