...
Inside the `if` statement, check if the `target` is not an instance of `HTMLElement`, and throw an error with the message `"Target element is not correct"`.

# --hints--

You should check in your `if` statement if the `target` is NOT an instance of `HTMLElement`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?if\s*\(\s*!\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

You should throw an error inside the `if` statement.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?if\s*\([\s\S]*?\)\s*\{[\s\S]*?throw\s+new\s+Error/);
```

Your error message should be `"Target element is not correct"`.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\s*\(\s*["']target element is not correct["']\s*\)/i);
```
