...
# --description--
Since the fortune app only has three moments Past, Present, and Future, declare an `enum` named `DrawingType` with three properties: `Past`, `Present`, and `Future`. Each property should have a string value corresponding to its name.
...
```js
assert.match(code, /enum\s+DrawingType\s*{/)
```

In your enum you have to have `Past` property with `string` type of `past` value.

```js
assert.match(code, /Past\s*=\s*['"]past['"]/);
```

In your enum you have to have `Present` property with `string` type of `present` value.

```js
assert.match(code, /Present\s*=\s*['"]present['"]/);
```

In your enum you have to have `Future` with `string` type of future value.

```js
assert.match(code, /Future\s*=\s*['"]future['"]/)
```
