...
You should have set `cardTitle` textContent to `foundCard.name`.

```js
assert.match(code, /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

You should have set `description` textContent to `foundCard.desc`.

```js
assert.match(code, /description\.textContent\s*=\s*foundCard\.desc/);
```
