...
Create a `CDN_URL` variable and set it to `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. Also create a `defaultImg` variable with the value `new URL("default.svg", import.meta.url)`, and a `LOCAL_DEFAULT_IMG` variable with the value `defaultImg`.
