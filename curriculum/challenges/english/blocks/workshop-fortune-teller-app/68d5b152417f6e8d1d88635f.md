...
Next, extend the `Card` interface by adding the remaining properties.
Set `value_int` to type `number`.

All other properties should be of type `string`:

`suit`, `type`, `img`, `meaning_up`, `meaning_rev`, and `desc`.
# --hints--
