...
There are more properties to add to the `Card` interface. Start by setting `value_int` to type `number`. All other properties should be of type `string`: `suit`, `type`, `img`, `meaning_up`, `meaning_rev`, and `desc`.
