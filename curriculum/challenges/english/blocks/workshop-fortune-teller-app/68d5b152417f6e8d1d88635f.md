...
The next step is to add the remaining properties to the `Card` interface.
`value_int` should be of type number. All the remaining properties should be of type string.

`value_int`, `suit`, `type`, `img`, `meaning_up`, `meaning_rev`, `desc`

# --hints--


You should have a property called `value_int` with a type of `number`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*value_int\s*:\s*number[\s\S]*}/);
```

You should have a property called `suit` with a type of `string`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*suit\s*:\s*string[\s\S]*}/);
```

You should have a property called `type` with a type of `string`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*type\s*:\s*string[\s\S]*}/);
```

You should have a property called `img` with a type of `string`.


```js
assert.match(code, /interface\s+Card\s*{[\s\S]*img\s*:\s*string[\s\S]*}/);
```

You should have a property called `meaning_up` with a type of `string`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*meaning_up\s*:\s*string[\s\S]*}/);
```

You should have a property called `meaning_rev` with a type of `string`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*meaning_rev\s*:\s*string[\s\S]*}/);
```

You should have a property called `desc` with a type of `string`.

```js
assert.match(code, /interface\s+Card\s*{[\s\S]*desc\s*:\s*string[\s\S]*}/);
```
