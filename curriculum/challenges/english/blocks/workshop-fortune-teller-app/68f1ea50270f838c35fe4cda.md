...
# --description--
now do the same  `subTitle` by assigning to `meaning_up` and for the title with `foundCard.name` and call the `showElements` method and pass `fortuneDescription` from the element
