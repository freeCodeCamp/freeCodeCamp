...
# --description--
Now do the same  `subTitle` by assigning to `meaning_up` and for the title with `foundCard.name` and call the `showElements` method and pass `fortuneDescription` from the elements.

# --hints--


You should have set `subTitle` textContent to `foundCard.meaning_up`.

```js
assert.match( code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?subTitle\.textContent\s*=\s*foundCard\.meaning_up/);
```

You should have set `title` textContent to `foundCard.name`.

```js
assert.match( code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?title\.textContent\s*=\s*foundCard\.name/);
```

You should have called `showElements` function.

```js
assert.match( code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

You should have passed `fortuneDescription` as argument to showElements function.

```js
assert.match( code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?showElements\s*\(\s*this\.elements\.fortuneDescription/);
```
