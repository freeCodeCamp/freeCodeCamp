...
Now do the same for `subTitle` by assigning it to `meaning_up`, and for `title` assign it to `foundCard.name`. Call the `showElements` method and pass `fortuneDescription` from the elements.

# --hints--
You should set the `subTitle` `textContent` to `foundCard.meaning_up`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?subTitle\.textContent\s*=\s*foundCard\.meaning_up/);
```

You should set the `title` `textContent` to `foundCard.name`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?title\.textContent\s*=\s*foundCard\.name/);
```

You should call the `showElements` function.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

You should pass `fortuneDescription` as an argument to the `showElements` function.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?showElements\s*\(\s*this\.elements\.fortuneDescription/);
```
