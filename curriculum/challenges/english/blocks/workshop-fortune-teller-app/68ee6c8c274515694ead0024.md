...
The fortune must be random, so a helper function is required to select a random fortune. Declare a variable named `chosenCard` inside the `singleCardSelected` method. Use the `getRandomItem` function to obtain a random card and pass `this.cards` as an argument.
...
const { chosenCard } = singleCardSelected?.variables ?? {};
assert.exists(chosenCard);
```
...
const { singleCardSelected } = explorer.classes.Game?.methods ?? {};
const { chosenCard } = singleCardSelected?.variables ?? {};
assert.isTrue(chosenCard?.value.matches("getRandomItem(this.cards)"));
```
