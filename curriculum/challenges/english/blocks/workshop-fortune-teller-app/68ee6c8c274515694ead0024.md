...
The fortune must be random, so a helper function is required to select a random fortune. Declare a variable named `chosenCard` inside the singleCardSelected method. Use the `getRandomItem` method to obtain a random card, pass `cards` as argument.

# --hints--

You should have declared variable named `chosenCard`.

```js
assert.match(code, /\bconst\s+chosenCard\b|\blet\s+chosenCard\b|\bvar\s+chosenCard\b/);
```

Your chosenCard variable should be assigned by calling `getRandomItem` method.

```js
assert.match(code, /chosenCard\s*=\s*getRandomItem\s*\(/);
```

You should have passed `cards` as argument.
