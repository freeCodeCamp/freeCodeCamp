...
const { singleCardSelected } = explorer.classes.Game.methods;
assert.exists(singleCardSelected.variables.chosenCard);
```
...
const explorer = await __helpers.Explorer(code);
const { singleCardSelected } = explorer.classes.Game.methods;
assert.isTrue(singleCardSelected.variables.chosenCard.value.matches("getRandomItem(this.cards)"));
```
