...
The fortune must be random, so a helper function is required to select a random fortune. Declare a variable named `chosenCard` inside the `singleCardSelected` method. Use the `getRandomItem` method to obtain a random card and pass `this.cards` as an argument.
