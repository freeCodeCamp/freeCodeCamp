...
The fortune must be random, so a helper function is required to select a random fortune. Declare a variable named `chosenCard` inside the `singleCardSelected` method. Use the `getRandomItem` method to obtain a random card and pass `cards` as an argument.

# --hints--

You should declare a variable named `chosenCard`.

```js
assert.match(__helpers.removeJSComments(code), /\bconst\s+chosenCard\b|\blet\s+chosenCard\b|\bvar\s+chosenCard\b/);
```

Your `chosenCard` variable should be assigned by calling the `getRandomItem` method.

```js
assert.match(__helpers.removeJSComments(code), /chosenCard\s*=\s*getRandomItem\s*\(/);
```

You should pass `cards` as an argument.

```js
assert.match(__helpers.removeJSComments(code), /getRandomItem\s*\(\s*this\s*.\s*cards\s*\)/);
```
