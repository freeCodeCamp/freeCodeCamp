...
```js
assert.match(code, /\bconst\s+chosenCard\b|\blet\s+chosenCard\b|\bvar\s+chosenCard\b/);
```

Your `chosenCard` variable should be assigned by calling the `getRandomItem` method.

```js
assert.match(code, /chosenCard\s*=\s*getRandomItem\s*\(/);
```

You should pass `cards` as an argument.

```js
assert.match(code, /getRandomItem\s*\(\s*this\s*.\s*cards\s*\)/);
```
