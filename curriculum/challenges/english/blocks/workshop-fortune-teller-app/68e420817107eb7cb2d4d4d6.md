...
Add an `onerror` handler to the image element to handle loading failures. On the first failure, mark it using dataset.failed and replace the source with a local default image. If the image fails again, remove the hidden class and hide the loading placeholder using `previousElementSibling`.

# --hints--


The onerror event must be attached to the `img` element.
...
An `if` statement must be placed inside the `onerror` handler.
...
The `if` condition must check whether `dataset.failed` is `false`.
...
Inside the if block, `dataset.failed` must be updated to `1`.
...
The image `URL` must then be updated to `LOCAL_DEFAULT_IMG`.
...
The else block must remove the `hidden` class.
