...
```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\([^)]*\)\s*\{[\s\S]*?\b(?:const|let|var)\s+cardId\b/);
```

Your `cardId` variable should be assigned to `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\([^)]*\)\s*\{[\s\S]*?\bcardId\s*=\s*cardElement/);
```

You should attach the `getAttribute` method to `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\([^)]*\)\s*\{[\s\S]*?cardElement\.getAttribute\s*\(/);
```

You should pass `data-id` to the `getAttribute` method.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\([^)]*\)\s*\{[\s\S]*?cardElement\.getAttribute\s*\(\s*['"]data-id['"]\s*\)/);
```
