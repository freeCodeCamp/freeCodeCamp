...
You should have declared variable named `cardId`.

```js
assert.match(code, /showFortune\s*\([^)]*\)\s*\{[\s\S]*?\b(?:const|let|var)\s+cardId\b/);
```

Your cardId variable should be assigned to `cardElement`.

```js
assert.match(code, /showFortune\s*\([^)]*\)\s*\{[\s\S]*?\bcardId\s*=\s*cardElement/);
```

You should have attached the `getAttribute` to cardElement.

```js
assert.match(code, /showFortune\s*\([^)]*\)\s*\{[\s\S]*?cardElement\.getAttribute\s*\(/);
```

You should have passed `data-id` to getAttribute method.

```js
assert.match(code, /showFortune\s*\([^)]*\)\s*\{[\s\S]*?cardElement\.getAttribute\s*\(\s*['"]data-id['"]\s*\)/);
```
