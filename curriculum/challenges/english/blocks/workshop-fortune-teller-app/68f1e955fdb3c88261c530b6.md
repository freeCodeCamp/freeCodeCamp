...
You should declare an `if` statement.

```js
assert.match(__helpers.removeJSComments(code), /\bif\s*\(\s*foundCard\s*\)/);
```

You should have an `if` statement that checks if `foundCard` exists.

```js
assert.match(__helpers.removeJSComments(code), /\bif\s*\(\s*foundCard\s*\)\s*\{/);
```
