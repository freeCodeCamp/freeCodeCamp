...
The fortune-teller app supports rendering multiple cards, so you need to have a method that can help render multiple cards. Declare a method called `multipleCardSelected`. Inside it, call the `hideElements` function and pass the following properties:
...
assert.exists(explorer.classes.Game.methods.multipleCardSelected);
```

You should call the `hideElements` function inside the `multipleCardSelected` method.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(/);
```

You should pass `singleCard` as the first argument to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(\s*this\.elements\.singleCard/);
```

You should pass `singleCardBtn` as the second argument to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.singleCardBtn/);
```

You should pass `multipleCardsBtn` as an argument to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.multipleCardsBtn/);
```

You should pass `headerTitle` as an argument to the `hideElements` function.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.headerTitle/);
```
