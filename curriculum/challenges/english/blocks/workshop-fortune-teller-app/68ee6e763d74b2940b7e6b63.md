...
The fortune-teller-app support rendering multiple card so you have to have method that can help to render multiple card so declare method called `multipleCardSelected`, inside it call the `hideElements` and pass the following property.
`singleCard`, `singleCardBtn`, `multipleCardsBtn`, `headerTitle`

# --hints--

You should have declared `multipleCardSelected` method.

```js
assert.match(code, /\bmultipleCardSelected\s*\(\s*\)\s*\{/);
```

You should have called `hideElements` function, inside multipleCardSelected method.

```js
assert.match( code, /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(/);
```

You should have passed `singleCard` as first argument to hideElements function.

```js
assert.match(code, /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\(\s*this\.elements\.singleCard/);
```

You should have passed `singleCardBtn` as second argument to hideElements function.

```js
assert.match(code, /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.singleCardBtn/);
```

You should have passed `multipleCardsBtn` as argument to hideElements function.

```js
assert.match(code, /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.multipleCardsBtn/);
```

You should have passed `headerTitle` as argument to hideElements function.

```js
assert.match(code, /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?hideElements\s*\([\s\S]*?this\.elements\.headerTitle/);
```
