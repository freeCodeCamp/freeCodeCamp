...
Now it is time to create another interface called `Deck`.
This interface should have a `cards` property, which is an array of the previous interface.

# --hints--

You should have declared an interface named `Deck`.

```js
assert.match(code, /interface\s+Deck\s*{[\s\S]*?}/)
```

You should have a property called `cards` inside `Deck` interface.

```js
assert.match(code, /interface\s+Deck\s*{[\s\S]*cards\s*:\s*Card\s*\[\]\s*[\s\S]*}/);
```

Your `cards` property should have type of `Card`.
