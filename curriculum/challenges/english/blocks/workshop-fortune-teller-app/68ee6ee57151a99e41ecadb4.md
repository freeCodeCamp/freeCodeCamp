...
Now call the `showElements` function and pass the following property.
`multipleCard`, `fortuneContainer`, `text`

# --hints--


You should have called `showElements` function, inside multipleCardSelected method.

```js
assert.match(code, /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/
);
```

You should have passed `multipleCard` as first argument to showElements function.

```js
assert.match(code, /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

You should have passed `fortuneContainer` as argument to showElements function.

```js
assert.match(code, /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\([^)]*this\.elements\.fortuneContainer/);
```

You should have passed `text` as argument to showElements function.

```js
assert.match(code, /multipleCardSelected\s*\([\s\S]*?showElements\s*\([^)]*this\.elements\.text/);
```
