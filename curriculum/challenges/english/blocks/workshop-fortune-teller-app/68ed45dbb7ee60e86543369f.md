...
After fetching the data, store the response in a variable and parse it to JSON.
Give the parsed data the type Deck, and then assign it to the cards property.
Inside the catch block, make sure to log the error with this message `"Error fetching cards:", error`.


# --hints--

You should declared variable named `data`.

```js
assert.match(code, /data\s*:\s*Deck\s*=\s*await\s*response\.json\s*\(\s*\)/);
```

Your data variable should have type of `Deck`.

```js
assert.match(code, /this\.cards\s*=\s*data\.cards/);
```
...
You should store the `data.cards` to `cards`.

```js
assert.match(code, /cards\s*=\s*data\.cards/);
```

Your `catch` block should `log` the error with the message of `"Error fetching cards:"`.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*{\s*console\.error\s*\(\s*"Error fetching cards:",\s*error\s*\)/);
```
