...
After fetching the data, store the response in a variable and parse it to JSON. Give the parsed data the type `Deck`, and then assign it to the `cards` property. Inside the `catch` block, make sure to log the error with this message: `"Error fetching cards:", error`.

# --hints--

You should declare a variable named `data`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=\s*await\s*response\.json\s*\(\s*\)/);
```
...
You should parse the data to JSON.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=\s*await\s*response\.json\s*\(\s*\)/);
```

You should store `data.cards` in the `cards` property.

```js
assert.match(__helpers.removeJSComments(code), /cards\s*=\s*data\.cards/);
```

Your `catch` block should log the error with the message `"Error fetching cards:"`.

```js
assert.match(__helpers.removeJSComments(code), /catch\s*\(\s*error\s*\)\s*{\s*console\.error\s*\(\s*"Error fetching cards:",\s*error\s*\)/);
```
