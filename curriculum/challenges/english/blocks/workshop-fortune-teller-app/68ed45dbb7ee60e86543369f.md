...
assert.exists(fetchCardsData.variables.data);
```
...
const explorer = await __helpers.Explorer(code);
const { fetchCardsData } = explorer.classes.Game.methods;
assert.isTrue(fetchCardsData.variables.data.value.matches("await response.json()"));
```
...
```js
assert.match(__helpers.removeJSComments(code), /catch\s*\(\s*error\s*\)\s*\{[\s\S]*?console\.error\s*\(\s*["']Error fetching cards:['"]/);
```
