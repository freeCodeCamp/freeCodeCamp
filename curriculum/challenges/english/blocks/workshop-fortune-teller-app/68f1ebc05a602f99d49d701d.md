...
Now inside the `showElements` pass these as arguments `singleCardBtn`, `multipleCardsBtn` and `headerTitle` remember you get them from the elements.

# --hints--


You should have passed `singleCardBtn` as first argument to `showElements` function.

```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(\s*this\.elements\.singleCardBtn/);
```

You should have passed `multipleCardsBtn` as second argument to `showElements` function.

```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(\s*this\.elements\.singleCardBtn\s*,\s*this\.elements\.multipleCardsBtn/);
```

You should have passed `headerTitle` as argument to `showElements` function.

```js
assert.match( code, /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\([\s\S]*?this\.elements\.headerTitle/);
```
