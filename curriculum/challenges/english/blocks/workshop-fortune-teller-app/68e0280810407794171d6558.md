...
Now, to make the app more user-friendly, add a div with the class `img-loader`. Next, create an `img` element and give it a conditional URL for the image using `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}`.

# --hints--

Inside your `figure` element you should have a `div` element.

```js
const divInsideFigureRegex = /<figure\b[^>]*>[\s\S]*?<div\b[^>]*>[\s\S]*?<\/div>[\s\S]*?<\/figure>/;
assert.match(code, divInsideFigureRegex)
```

Your div should have a class of `img-loader`.

```js
const divClassRegex = /<figure\b[^>]*>[\s\S]*?<div\b[^>]*class\s*=\s*["']img-loader["'][^>]*>[\s\S]*?<\/div>[\s\S]*?<\/figure>/;
assert.match(code, divClassRegex)
```

Inside your `figure` element you should have a `img` element.

```js
const imgInsideFigureRegex = /<figure\b[^>]*>[\s\S]*?<img\b[^>]*>[\s\S]*?<\/figure>/;
assert.match(code, imgInsideFigureRegex)
```

The `img` element should have its `src ` attribute set to `img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG`.

```js
const imgSrcOnlyRegex = /src\s*=\s*"\$\{\s*img\s*\?\s*`\$\{CDN_URL\}\/\$\{img\}`\s*:\s*LOCAL_DEFAULT_IMG\s*\}"/;
assert.match(code, imgSrcOnlyRegex)
```
