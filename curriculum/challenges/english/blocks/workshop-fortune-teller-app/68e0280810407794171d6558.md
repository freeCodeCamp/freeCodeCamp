...
Now, to make the app more user-friendly, add a div with the class `img-loader`. and after that, create an `img` element and give it a conditional URL for the image using `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}`.

# --hints--


Inside the `figure` element you should have `div` element.
...
Your div should have the class of `img-loader`.
...
Inside the figure you should have `img` element.
...
The `img` element should have it's src attribute set to `img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG`.
