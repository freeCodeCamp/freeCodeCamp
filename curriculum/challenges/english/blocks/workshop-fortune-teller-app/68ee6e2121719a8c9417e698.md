...
Right after you clear `multipleCard`, inside the singleCardSelected call the `showElements` and call `singleCard` as first argument and `fortuneContainer`.
...
```js
assert.match(code, /showElements\s*\(/);
```

You should have passed `elements.singleCard` as the first argument.

```js
assert.match(code, /showElements\s*\(\s*this\.elements\.singleCard/);
```

You should have passed `elements.fortuneContainer` as the second argument.

```js
assert.match(code, /showElements\s*\([\s\S]*?this\.elements\.fortuneContainer/);
```
