...
```js
assert.match(code, /class\s+Game\s*{/);
```

Inside the class you should have `cards` property.

```js
assert.match(code, /class\s+Game\s*{[^}]*cards\s*:/);
```

Your cards property should have type of `Card[]`.

```js
assert.match(code, /class\s+Game\s*{[^}]*cards\s*:\s*Card\[\]/);
```

Your cards property should be assign to empty `array`.

```js
assert.match(code, /class\s+Game\s*{[^}]*cards\s*:\s*Card\[\]\s*=\s*\[\]/);
```
