...
You should have an `if` statement declared.

```js
assert.match(code, /if\s*\(\s*!cardElement\s*\)/);
```

Your `if` statement should check if `cardElement` is not found by using `!` to negate it.

```js
assert.match( code, /if\s*\(\s*!cardElement\s*\)\s*(?:return\s*;|\{\s*return\s*;\s*\})/);
```
