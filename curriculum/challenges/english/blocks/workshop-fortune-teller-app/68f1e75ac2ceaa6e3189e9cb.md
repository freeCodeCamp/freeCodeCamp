...
You should add another `if` statement that checks if `cardElement` is NOT an instance of `HTMLElement`.
...
```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*cardElement\s+instanceof\s+HTMLElement\s*\)[\s\S]*?throw\s+new\s+Error/);
```
