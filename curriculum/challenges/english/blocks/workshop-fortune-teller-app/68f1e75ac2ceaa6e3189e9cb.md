...
You should check in your `if` statement if `cardElement` is NOT an instance of `HTMLElement`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?if\s*\(\s*!\s*\(\s*cardElement\s+instanceof\s+HTMLElement\s*\)\s*\)\s*\{/);
```

You should throw an error inside the `if` statement.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?if\s*\(\s*!\s*\(\s*cardElement\s+instanceof\s+HTMLElement\s*\)\s*\)\s*\{\s*throw\s+new\s+Error/);
```

Your error message should be `"Card element is not correct"`.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\s*\(\s*["']Card element is not correct["']\s*\)\s*;?/);
```
