...
Your if statement should check if the `cardElement` is `NOT` an instance of `HTMLElement`.

```js
assert.match( code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?if\s*\(\s*!\s*\(\s*cardElement\s+instanceof\s+HTMLElement\s*\)\s*\)\s*\{/);
```

Inside the if statement you should throw error.

```js
assert.match( code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?if\s*\(\s*!\s*\(\s*cardElement\s+instanceof\s+HTMLElement\s*\)\s*\)\s*\{\s*throw\s+new\s+Error/);
```

error message should be `Target element is not correct`.

```js
assert.match(code, /throw\s+new\s+Error\s*\(\s*["']Card element is not correct["']\s*\)\s*;?/);
```
