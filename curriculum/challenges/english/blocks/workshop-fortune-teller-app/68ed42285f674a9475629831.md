...
Inside elements method `singleCardBtn` property should be assign to `HTMLElement`.
...
Inside elements method `multipleCardsBtn` property should be assign to `HTMLElement`.

```js
assert.match(code, /class\s+\w+\s*{[\s\S]*?private\s+elements\s*:\s*{[\s\S]*?multipleCardsBtn\s*:\s*HTMLElement\s*;[\s\S]*?}/);
```

Inside elements method `singleCard` property assign should be to `HTMLElement`.
