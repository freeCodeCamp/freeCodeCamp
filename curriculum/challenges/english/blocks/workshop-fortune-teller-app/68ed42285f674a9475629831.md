...
Inside the class add another private `elements` as object and inside it add `singleCardBtn`  `singleCard` `multipleCardsBtn` each should have `HTMLElement`.



# --hints--

You should have declared method named `elements` with `private` method, inside the class.

```js
assert.match(code, /class\s+\w+\s*{[\s\S]*?private\s+elements\s*:\s*{[\s\S]*?}/);
```

Inside elements method you should have `singleCardBtn` property should be assigned to `HTMLElement`.
...
Inside elements method you should have `multipleCardsBtn` property should be assigned to `HTMLElement`.

```js
assert.match(code, /class\s+\w+\s*{[\s\S]*?private\s+elements\s*:\s*{[\s\S]*?multipleCardsBtn\s*:\s*HTMLElement\s*;[\s\S]*?}/);
```

Inside elements method you should have `singleCard` property assigned should be to `HTMLElement`.
