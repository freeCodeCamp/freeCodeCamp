...
Inside the class, add a private `elements` property as an object. Inside it, add `singleCardBtn`, `singleCard`, and `multipleCardsBtn`, each typed as `HTMLElement`.

# --hints--

You should declare a property named `elements` with the `private` modifier inside the class.

```js
assert.match(__helpers.removeJSComments(code), /class\s+\w+\s*{[\s\S]*?private\s+elements\s*:\s*{[\s\S]*?}/);
```

Inside the `elements` property, the `singleCardBtn` property should be assigned the type `HTMLElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+\w+\s*{[\s\S]*?private\s+elements\s*:\s*{[\s\S]*?singleCardBtn\s*:\s*HTMLElement\s*;[\s\S]*?}/
);
```

Inside the `elements` property, the `multipleCardsBtn` property should be assigned the type `HTMLElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+\w+\s*{[\s\S]*?private\s+elements\s*:\s*{[\s\S]*?multipleCardsBtn\s*:\s*HTMLElement\s*;[\s\S]*?}/);
```

Inside the `elements` property, the `singleCard` property should be assigned the type `HTMLElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+\w+\s*{[\s\S]*?private\s+elements\s*:\s*{[\s\S]*?singleCard\s*:\s*HTMLElement\s*;[\s\S]*?}/);
```
