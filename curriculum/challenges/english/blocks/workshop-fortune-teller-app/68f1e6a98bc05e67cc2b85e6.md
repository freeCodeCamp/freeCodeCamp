...
Declare variable named `cardElement` and by using `card_container` class get the element use the `closest` selector to get it from the target.

# --hints--

You should declare a variable named `cardElement`.

```js
assert.match(code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?\bconst\s+cardElement\b/);
```

You should use the `closest` method on the `target` element.

```js
assert.match(code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?target\s*\?\.\s*closest\s*\(/);
```

You should pass `.card_container` as the argument to the `closest` method.

```js
assert.match(code, /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?target\s*\?\.\s*closest\s*\(\s*['"]\.card_container['"]\s*\)/);
```
