...
```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?\bconst\s+cardElement\b/);
```

You should use the `closest` method on the `target` element.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?target\s*\?\.\s*closest\s*\(/);
```

You should pass `.card_container` as the argument to the `closest` method.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*(?::\s*Event)?\s*\)\s*\{[\s\S]*?target\s*\?\.\s*closest\s*\(\s*['"]\.card_container['"]\s*\)/);
```
