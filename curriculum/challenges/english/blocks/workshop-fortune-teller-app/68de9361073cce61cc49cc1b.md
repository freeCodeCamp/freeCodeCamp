...
Well now in a previous lecture, you learned how that an interface is TypeScript's ability to automatically determine and assign types to variables, function returns, and expressions based on their usage and context.


In TypeScript, generics make functions, classes, and types more flexible.
Generics allow the creation of type variables, eliminating the need to explicitly define types each time.
This enables the same function to work safely with different types.

 here a simple example of generics.
...
For this lesson, declare a function called `getElement` that takes `selector` as a parameter to select elements from the DOM. Use Generics so it can work with any element type.
# --hints--

You should have declared an arrow function named `getElement`.

```js
assert.match(code, /const\s*getElement\s*=/)
```

The function should take a parameter named `selector` of type `string`.

```js
const ParameterRegex = /const\s*getElement\s*=[^{]*\(\s*selector\s*:\s*string\s*\)/;
assert.match(code, ParameterRegex);
```

The function should have a return type of `T`.

```js
const ReturnTypeRegex = /\)\s*:\s*T\s*=>/;
assert.match(code, ReturnTypeRegex);
```

The type parameter `T` should extend the `HTMLElement` type.

```js
const GenericRegex = /<\s*T\s+extends\s+HTMLElement\s*>/;
assert.match(code, GenericRegex);
```
