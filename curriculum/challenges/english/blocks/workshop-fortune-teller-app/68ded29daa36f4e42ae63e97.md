...
Wrap up the function by using the `forEach` array method to iterate over the elements argument. Inside the callback function, add the hidden class to each element by using the `.add()` method of `classList`.
