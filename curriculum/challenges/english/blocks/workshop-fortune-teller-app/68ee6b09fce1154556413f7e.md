...
Inside the initializeEventListeners method, attach an `click` event listener to `singleCardBtn`. Within the event handler, call the singleCardSelected method. remember to get the singleCardBtn from `elements`.

# --hints--

You should have attached `click` event to the `singleCardBtn` element.

```js
assert.match(code, /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Inside event callback you should have called `singleCardSelected` method.

```js
assert.match(code, /singleCardSelected\s*\(/);
```
