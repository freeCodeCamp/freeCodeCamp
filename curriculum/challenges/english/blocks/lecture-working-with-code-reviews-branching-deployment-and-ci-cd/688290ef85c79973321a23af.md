...
In the previous lecture, you learned how to make a pull request to your own repository. You also learned how to merge that pull request. But you could only do that because the repository belongs to you - you have maintainer access to your own repositories, which means you have permission to merge pull requests.
...
The first is to confirm that the CI, or continuous integrations, passes. You'll learn more about these in a future lecture, so for now you can think of them as checks to validate that the code meets style guidelines and passes a test suite.
...
If everything looks good, a maintainer will approve your pull request. An approval looks like so:

Sometimes the review may also include a comment like this:

LGTM is a common acronym you'll see in code review. It stands for "Looks good to me".

After your pull request has been approved, it should be merged whenever the maintainers are ready to pull the changes in. Some projects may require more than one approval - freeCodeCamp, for example, requires two.
...
Sometimes, a maintainer may just leave a comment with their request:

Comments like this indicate something you should change or update, but with more freedom of how you implement it. In this case, the review was asking for a test to confirm the fix proposed in the PR.

Maintainers may also leave an official "Request changes" comment:

Typically, a comment like this blocks the pull request from being merged. You'll need to make the desired changes, and then the same maintainer who requested the changes will need to re-review and request more changes or approve your work.

When maintainers have specific changes in mind, they can actually propose changes through GitHub's UI:

When they use a direct suggestion like this, GitHub will give you a button to commit the suggestion to your branch directly. However, if there are multiple suggestions like this, you'll want to switch to the files view.
...
What if you need to work on a large request, like the review we saw earlier that asked for a test to be added? You can make your changes locally, commit them to the same branch that you used for the PR, and push them up. GitHub will automatically update your pull request to reflect the new commits.
