...
Now clean up your code by removing all the individual button variables and event listeners from the previous approach. Keep only the `btns` variable with `querySelectorAll`, the `updateCount` function, and the `forEach` loop.

Congratulations! You've successfully built an Emoji Reactor app!

# --hints--

You should not have individual button variables for happy, confused, sad, or loving buttons.
...
All buttons should still work correctly.
