...
Right after the function declaration, create an `if` statement with this condition: `array.length <= 1`. Within the body of the `if` statement add a `return` statement to stop the execution of the function.

# --hints--

You should create an `if` statement that checks `array.length <= 1`.
