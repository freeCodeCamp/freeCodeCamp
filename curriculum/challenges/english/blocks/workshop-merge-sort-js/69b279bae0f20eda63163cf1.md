...
const { leftArrayIndex } = explorer.functions.mergeSort.variables;
assert.exists(
  leftArrayIndex,
...
```js
const explorer = await __helpers.Explorer(code);
const { rightArrayIndex } = explorer.functions.mergeSort.variables;
assert.exists(
  rightArrayIndex,
...
```js
const explorer = await __helpers.Explorer(code);
const { sortedIndex } = explorer.functions.mergeSort.variables;
assert.exists(
  sortedIndex,
