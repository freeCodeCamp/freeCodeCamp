...
To do this, add a `while` loop that runs while as long as `rightArrayIndex` is less than `rightPart.length`, assigns `rightPart[rightArrayIndex]` into `array[sortedIndex]`, and increments both `rightArrayIndex` and `sortedIndex`.
