...
aria-describedby correctly references the id of an element (password-help) that provides additional descriptive information for the input field.
...
aria-describedby points to the input’s own id (password) instead of the element that contains helper text. This does not provide any extra description to assistive technologies.
...
The paragraph with descriptive text has no id="password-help", so the aria-describedby reference doesn’t link to any valid element.
...
aria-label provides an accessible name, not a description. It replaces the visible label rather than adding supplementary information — incorrect for this case.
