...
The terminal is the area where you can see text output from a program. Python includes `print()` as a built-in function, which means you can use it without defining it yourself. The `print()` function sends text and other values to the terminal. Let's take a closer look at how it works.

A common first program displays `Hello world!` in the terminal. You can write this program in Python with the `print` function.
