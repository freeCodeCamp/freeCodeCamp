...
fix/react-introduction-lecture
In modern React, developers use functional components, and every example in this lesson is a functional component.

The two types of components in React are functional and class-based components. In modern React, developers use functional components, and every example in this lesson is a functional component.

At a high level, you can think of components like JavaScript functions that return elements describing the UI.
main
...
fix/react-introduction-lecture
We are also giving the `h1` element a class named `title` using the `className` attribute.
But why are we using `className` instead of `class` like with regular HTML elements?

Well, this is because in JavaScript, class is a reserved keyword. So, we need to use className instead.

We are also applying a class called `title` to the `h1` element using the `className` attribute.
...
We are also using a comment inside the JSX. A comment inside JSX has to be wrapped in curly braces. Outside JSX, in the body of the component, you write a normal JavaScript comment:

```jsx
{/* Block Comments */}
```

Another thing you might have noticed is that we are using a capital letter at the beginning of the component name. But why can't we use all lowercase letters?
...
When working with JSX, all tags and uses of components need to be explicitly closed. So if the component or tag does not have any children, then you need to explicitly close it as shown here:
...
The reason why you are getting that error message is because multiple sibling elements need to be wrapped in a parent element. While you could wrap the `h1` and `p` elements in a simple `div`, there is another way to satisfy that requirement.
