...
assert.include(strippedCode, 'previousButton.addEventListener(');
```
...
const withoutComments = __helpers.removeJSComments(code);
const strippedCode = __helpers.removeWhiteSpace(withoutComments);
assert.match(strippedCode, /previousButton\.addEventListener\((['"`])click\1/);
```
...
const withoutComments = __helpers.removeJSComments(code);
const strippedCode = __helpers.removeWhiteSpace(withoutComments);
assert.match(strippedCode, /previousButton\.addEventListener\((['"`])click\1,playPreviousSong\)/);
```
