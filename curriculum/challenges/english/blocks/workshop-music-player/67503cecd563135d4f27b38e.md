...
assert.include(strippedCode, 'nextButton.addEventListener(');
```
...
const withoutComments = __helpers.removeJSComments(code);
const strippedCode = __helpers.removeWhiteSpace(withoutComments);
assert.match(strippedCode, /nextButton\.addEventListener\((['"`])click\1/);
```
...
const withoutComments = __helpers.removeJSComments(code);
const strippedCode = __helpers.removeWhiteSpace(withoutComments);
assert.match(strippedCode, /nextButton\.addEventListener\((['"`])click\1,playNextSong\)/);
```
