...
assert.include(strippedCode, 'pauseButton.addEventListener(');
```
...
const withoutComments = __helpers.removeJSComments(code);
const strippedCode = __helpers.removeWhiteSpace(withoutComments);
assert.match(strippedCode, /pauseButton\.addEventListener\((['"`])click\1/);
```
...
const withoutComments = __helpers.removeJSComments(code);
const strippedCode = __helpers.removeWhiteSpace(withoutComments);
assert.match(strippedCode, /pauseButton\.addEventListener\((['"`])click\1,pauseSong\)/);
```
