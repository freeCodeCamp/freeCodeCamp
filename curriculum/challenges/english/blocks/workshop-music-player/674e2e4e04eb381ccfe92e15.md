...
assert.match(withoutComments, /pauseButton\s*\.\s*addEventListener\s*\(/);
```
...
```js
const withoutComments = __helpers.removeJSComments(code);
assert.match(withoutComments, /pauseButton\s*\.\s*addEventListener\s*\(\s*(['"`])click\1/);
```
...
```js
const withoutComments = __helpers.removeJSComments(code);
assert.match(withoutComments, /pauseButton\s*\.\s*addEventListener\s*\(\s*(['"`])click\1\s*,\s*pauseSong\s*\)/);
```
