...
const { getNextSong } = explorer.allFunctions;
const getNextSongSource = getNextSong?.toString() ?? '';
const withoutComments = __helpers.removeJSComments(getNextSongSource);
const strippedGetNextSong = __helpers.removeWhiteSpace(withoutComments);
assert.include(strippedGetNextSong, 'getCurrentSongIndex()');
```
