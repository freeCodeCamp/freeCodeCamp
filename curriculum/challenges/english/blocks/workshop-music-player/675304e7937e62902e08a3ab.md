...
const { playSong } = explorer.allFunctions;
const playSongSource = playSong?.toString() ?? '';
const withoutComments = __helpers.removeJSComments(playSongSource);
const strippedPlaySong = __helpers.removeWhiteSpace(withoutComments);
assert.include(strippedPlaySong, 'setPlayButtonAccessibleText()');
```
