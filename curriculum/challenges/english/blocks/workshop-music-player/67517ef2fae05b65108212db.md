...
const { highlightCurrentSong } = explorer.allFunctions;
const highlightCurrentSongSource = highlightCurrentSong?.toString() ?? '';
const withoutComments = __helpers.removeJSComments(highlightCurrentSongSource);
const strippedHighlightCurrentSong = __helpers.removeWhiteSpace(withoutComments);
assert.include(strippedHighlightCurrentSong, 'document.getElementById(`song-${userData.currentSong?.id}`)');
```
...
const explorer = await __helpers.Explorer(code);
const { highlightCurrentSong } = explorer.allFunctions;
const { songToHighlight } = highlightCurrentSong?.variables ?? {};
assert.exists(songToHighlight);
assert.isTrue(songToHighlight.value.matches('document.getElementById(`song-${userData.currentSong?.id}`)'));
```
