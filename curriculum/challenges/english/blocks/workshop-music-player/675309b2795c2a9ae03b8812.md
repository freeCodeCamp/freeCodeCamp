...
assert.include(strippedCode, 'audio.addEventListener(');
```
...
const withoutComments = __helpers.removeJSComments(code);
const strippedCode = __helpers.removeWhiteSpace(withoutComments);
assert.match(strippedCode, /audio\.addEventListener\((['"])ended\1/);
```
...
const withoutComments = __helpers.removeJSComments(code);
const strippedCode = __helpers.removeWhiteSpace(withoutComments);
assert.match(strippedCode, /audio\.addEventListener\((['"])ended\1,playNextSong\)/);
```
