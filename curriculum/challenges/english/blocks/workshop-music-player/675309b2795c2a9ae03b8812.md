...
assert.match(withoutComments, /audio\s*\.\s*addEventListener\s*\(/);
```
...
```js
const withoutComments = __helpers.removeJSComments(code);
assert.match(withoutComments, /audio\s*\.\s*addEventListener\s*\(\s*(['"])ended\1/);
```
...
```js
const withoutComments = __helpers.removeJSComments(code);
assert.match(withoutComments, /audio\s*\.\s*addEventListener\s*\(\s*(['"])ended\1\s*,\s*playNextSong\s*\)/);
```
