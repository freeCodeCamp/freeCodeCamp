...
const { getPreviousSong } = explorer.allFunctions;
const getPreviousSongSource = getPreviousSong?.toString() ?? '';
const withoutComments = __helpers.removeJSComments(getPreviousSongSource);
const strippedGetPreviousSong = __helpers.removeWhiteSpace(withoutComments);
assert.include(strippedGetPreviousSong, 'getCurrentSongIndex()');
```
