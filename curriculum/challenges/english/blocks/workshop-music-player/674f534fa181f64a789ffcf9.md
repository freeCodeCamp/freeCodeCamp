...
const { playNextSong } = explorer.allFunctions;
const playNextSongSource = playNextSong?.toString() ?? '';
const withoutComments = __helpers.removeJSComments(playNextSongSource);
const strippedPlayNextSong = __helpers.removeWhiteSpace(withoutComments);
assert.include(strippedPlayNextSong, 'pauseSong()');
```
