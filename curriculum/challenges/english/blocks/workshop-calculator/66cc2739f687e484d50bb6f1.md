...
const { calculateSquareRoot } = explorer.allFunctions;
assert.lengthOf(calculateSquareRoot?.parameters, 1);
assert.equal(calculateSquareRoot?.parameters[0].toString(), "num");
```
