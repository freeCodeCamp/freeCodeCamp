...
```js
assert.isAtLeast(document.querySelectorAll('label')[0]?.textContent.trim().length, 1);
```

You should give the second `label` element an appropriate text content.

```js
assert.isAtLeast(document.querySelectorAll('label')[1]?.textContent.trim().length, 1);
```

You should give the third `label` element an appropriate text content.

```js
assert.isAtLeast(document.querySelectorAll('label')[2]?.textContent.trim().length, 1);
```
