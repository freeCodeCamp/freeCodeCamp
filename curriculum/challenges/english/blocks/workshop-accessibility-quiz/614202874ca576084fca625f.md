...
```js
assert.isAtLeast(document.querySelectorAll('h2')?.[0]?.textContent?.trim()?.length, 1);
```

You should give the second `h2` element suitable text content. _Hint: I would have chosen `HTML`_

```js
assert.isAtLeast(document.querySelectorAll('h2')?.[1]?.textContent?.trim()?.length, 1);
```

You should give the third `h2` element suitable text content. _Hint: I would have chosen `CSS`_

```js
assert.isAtLeast(document.querySelectorAll('h2')?.[2]?.textContent?.trim()?.length, 1);
```
