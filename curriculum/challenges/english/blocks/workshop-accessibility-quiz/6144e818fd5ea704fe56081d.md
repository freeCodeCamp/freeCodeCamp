...
```js
assert.notEmpty(document.querySelectorAll('legend')?.[0]?.textContent.trim());
```

You should give the second `legend` element text content.

```js
assert.notEmpty(document.querySelectorAll('legend')?.[1]?.textContent.trim());
```

You should not use the same text content for both `legend` elements.

```js
assert.notEqual(document.querySelectorAll('legend')?.[0]?.textContent.trim(), document.querySelectorAll('legend')?.[1]?.textContent.trim());
```
