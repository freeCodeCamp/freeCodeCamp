...
Following the same pattern, query for your `number` inputs in the `#entertainment` element. Assign it to a variable following the naming scheme of the previous two.
...
```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

You should assign the result of your `document.querySelectorAll()` call to `entertainmentInputs`.

```js
assert.match(calculateBudget.toString(), /entertainmentInputs\s*=\s*document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```
