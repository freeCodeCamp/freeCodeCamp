...
Your next step is to write a function that will get the total amounts from the user's entries.

Declare a `getTotalFromInputs` function, and give it a parameter called `list`.

# --hints--

You should declare a `getTotalFromInputs` variable.

```js
assert.isDefined(getTotalFromInputs);
```

Your `getTotalFromInputs` variable should be a function.

```js
assert.isFunction(getTotalFromInputs);
```
