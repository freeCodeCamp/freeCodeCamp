...
Your `output.innerHTML` string will need a `span` element. Create that, and give it a class attribute set to the `statusClass` variable.

Inside your `span` element, add the `statusText` variable as the text content.
...
Your `span` should have the `class` attribute set to `${statusClass}`.

```js
const htmlString = code.split(/output\s*\.\s*innerHTML\s*=\s*/)[1].split(/`/)[1];
assert.match(htmlString, /<span\s+class\s*=\s*"\$\{statusClass\}"\s*>/);
```

Your `span` should have `${statusText}` as the text content.

```js
const htmlString = code.split(/output\s*\.\s*innerHTML\s*=\s*/)[1].split(/`/)[1];
assert.match(htmlString, /<span\s+class\s*=\s*"\$\{statusClass\}"\s*>\$\{statusText\}<\/span>/);
```
