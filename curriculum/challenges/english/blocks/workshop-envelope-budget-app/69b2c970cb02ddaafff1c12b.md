...
Now declare a `netRemaining` variable, and give it the value of subtracting `expenses` from `income`.
...
```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```
