...
Following this same pattern, declare `utilities` and `entertainment` variables for the `number` inputs in the `#utilities` and `#entertainment` elements. Assign them the appropriate `getTotalFromInputs` calls.
...
```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```
...
```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```
