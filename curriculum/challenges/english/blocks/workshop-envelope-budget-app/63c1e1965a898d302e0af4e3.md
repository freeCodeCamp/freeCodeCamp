...
Replace your concatenated string in the `querySelector` with a template literal – be sure to keep `#` before your `category` variable and the space between `${category}` and `.input-container`.
...
```js
assert.match(addEntry.toString(), /targetInputContainer\s*=\s*document\.querySelector\(\s*`/);
```

Your template literal should start with `#${category}`.

```js
assert.match(addEntry.toString(), /`#\$\{category\}/);
```

Your template literal should include  `.input-container` after `#${category}`. Remember to include the space before `.input-container`.

```js
assert.match(addEntry.toString(), /`#\$\{category\}\s+\.input-container`/);
```

Your `targetInputContainer` should be assigned `document.querySelector()` with the template literal.

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=\s*document\.querySelector\(\s*`#\$\{category\}\s+\.input-container`\s*\)/);
```
