...
Declare a `foodInputs` variable, and give it the value of calling `document.querySelectorAll()` with the selector `#food input[type='number']`. This will return any `number` inputs that are in the `#food` element.
...
```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

You should assign the result of your `document.querySelectorAll()` call to `foodInputs`.

```js
assert.match(calculateBudget.toString(), /foodInputs\s*=\s*document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```
