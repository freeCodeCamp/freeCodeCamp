...
```js
assert.match(calculateBudget.toString(), /statusText\s*=\s*`/);
```

Your template literal should contain the text "Over Budget by $".

```js
assert.match(calculateBudget.toString(), /statusText\s*=\s*`Over Budget by \$\$\{/);
```

You should use `Math.abs(netRemaining)` to get the absolute value.

```js
assert.match(calculateBudget.toString(), /statusText\s*=\s*`Over Budget by \$\$\{Math\.abs\(netRemaining\)\}`/);
```
