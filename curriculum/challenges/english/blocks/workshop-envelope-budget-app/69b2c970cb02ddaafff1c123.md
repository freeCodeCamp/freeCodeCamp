...
Using that same syntax, query your `number` inputs in the `#utilities` element and assign them to `utilitiesInputs`.
...
```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

You should assign the result of your `document.querySelectorAll()` call to `utilitiesInputs`.

```js
assert.match(calculateBudget.toString(), /utilitiesInputs\s*=\s*document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```
