...
Use the addition assignment operator to add `currVal` to your `total`. You'll need to use the `Number` constructor to convert `currVal` to a number.
...
After your `if` statement, you should use the addition assignment operator on `total`.
...
You should add the value of `Number(currVal)` to `total`.
