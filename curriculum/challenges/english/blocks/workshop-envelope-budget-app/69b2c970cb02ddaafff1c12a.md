...
It is time to start preparing your calculations. Start by declaring an `expenses` variable, and assign it the sum of `rent`, `food`, `utilities`, and `entertainment` (note that order matters for the tests). Be sure to do this after your if statement.
...
```js
assert.isAbove(calculateBudget.toString().indexOf('expenses'), calculateBudget.toString().indexOf('if'));
```

You should calculate the sum of `rent`, `food`, `utilities`, and `entertainment`, in order.  

```js
assert.match(calculateBudget.toString(), /rent\s*\+\s*food\s*\+\s*utilities\s*\+\s*entertainment\s*;?\s*$/m);
```
