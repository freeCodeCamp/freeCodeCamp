...
Continuing the pattern, create a `fieldset` for `utilities` with the same nested elements.
...
Your third `fieldset` element should have an `id` set to `utilities`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.id, "utilities");
```
...
Your new `legend` element should have the text `Utilities`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.querySelector('legend')?.innerText, "Utilities");
```
