...
Now declare a `food` variable, and give it the value of calling `getTotalFromInputs` with your `foodInputs`.
...
```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```
