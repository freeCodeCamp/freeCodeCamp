...
Still inside your `if` statement, set `statusClass` to the string `"deficit"`.

# --hints--

You should assign `"deficit"` to `statusClass`

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)\s*\{[\s\S]*statusClass\s*=\s*('|")deficit\1/);
```
