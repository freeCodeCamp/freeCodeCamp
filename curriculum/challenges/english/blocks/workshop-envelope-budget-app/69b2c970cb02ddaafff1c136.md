...
```js
assert.match(calculateBudget.toString(), /output\.classList/);
```

You should call the `.remove()` method of the `classList` property.

```js
assert.match(calculateBudget.toString(), /output\.classList\.remove\(/);
```

You should pass `hide` as the argument to the `.remove()` method.

```js
assert.match(calculateBudget.toString(), /output\.classList\.remove\(\s*('|")hide\1\s*\)/);
```
