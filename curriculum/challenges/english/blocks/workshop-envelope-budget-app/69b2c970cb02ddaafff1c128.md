...
You also need to get the value of your `#income` input. You already queried this at the top of your code, and set it to the `incomeInput` variable. However, you used `getElementById`, which returns an `Element`, not a `NodeList`.

A `NodeList` is an array-like object, which means you can iterate through it and it shares some common methods with an array. For your `getTotalFromInputs` function, an array will work for the argument just as well as a `NodeList` does.

Declare an `income` variable and set it to the result of calling `getTotalFromInputs` – pass an array containing your `incomeInput` as the argument.

# --hints--

Your `calculateBudget` function should have a `income` variable.
...
```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```
