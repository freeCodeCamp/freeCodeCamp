...
```js
assert.match(getTotalFromInputs.toString(), /invalidInputMatch\s*=/);
```

You should assign the result of calling `isInvalidInput` to your `invalidInputMatch` variable.

```js
assert.match(getTotalFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(/);
```

You should pass `currVal` as the argument to `isInvalidInput`.

```js
assert.match(getTotalFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(\s*currVal\s*\)/);
```
