...
```js
assert.match(calculateBudget.toString(), /statusText\s*=/g);
```

Your `calculateBudget` function should have a `statusClass` variable.

```js
assert.match(calculateBudget.toString(), /statusClass\s*=/g);
```

You should initialize `statusText` and `statusClass` as empty strings.

```js
assert.match(calculateBudget.toString(), /statusText\s*=\s*""/g);
assert.match(calculateBudget.toString(), /statusClass\s*=\s*""/g);
```

Both variables should be declared using `let`.

```js
assert.match(calculateBudget.toString(), /let\s+statusText\s*=\s*('|")\1/g);
assert.match(calculateBudget.toString(), /let\s+statusClass\s*=\s*('|")\1/g);
```
