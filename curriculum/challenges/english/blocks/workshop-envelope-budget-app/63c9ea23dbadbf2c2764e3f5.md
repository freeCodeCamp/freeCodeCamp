...
Using the same interpolation syntax, add a second `p` element with the text `$${expenses} Total Expenses` below the existing `p` element.

# --hints--

You should add a second `p` element below the existing one.

```js
const htmlString = code.split(/output\.innerHTML\s*=\s*/)[1];
assert.match(htmlString, /<p>\$\$\{expenses\}\sTotal\sExpenses<\/p>/);
```

Your second `p` element should have the text `$${expenses} Total Expenses`.

```js
incomeInput.value = 3000;
rentInput.value = 1200;
const fakeEvent = { preventDefault: () => {} };
calculateBudget(fakeEvent);
assert.strictEqual(output.children[4].innerText, "$1200 Total Expenses");
```
