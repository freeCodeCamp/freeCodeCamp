...
You should pass `'#'` concatenated with `category` to your `querySelector()` method.

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=\s*document\.querySelector\(\s*('|")#\1\s*\+\s*category/);
```

You should concatenate `' .input-container'` after `category`. Remember to include the space at the beginning of `.input-container`.

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=\s*document\.querySelector\(\s*('|")#\1\s*\+\s*category\s*\+\s*('|")\s\.input-container\3\s*\)/);
```
