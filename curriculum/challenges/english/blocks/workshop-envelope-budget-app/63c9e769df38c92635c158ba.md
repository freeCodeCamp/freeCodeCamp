...
Now you need to handle the case when the user is not over budget (when `netRemaining` is greater than or equal to `0`).

Add an `else` block to your `if` statement that sets `statusText` to `"$X Remaining"`, where `X` is the value of `netRemaining`. Also, set `statusClass` to `"surplus"`.
...
```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)\s*\{[\s\S]*\}\s*else\s*\{/);
```

You should use a template literal for `statusText`.

```js
assert.match(calculateBudget.toString(), /else\s*\{[\s\S]*statusText\s*=\s*`/);
```

Your `statusText` should be assigned `$${netRemaining} Remaining`.

```js
assert.match(calculateBudget.toString(), /statusText\s*=\s*`\s*\$\s*\{\s*netRemaining\s*\}\s*Remaining\s*`/);
```

You should assign `"surplus"` to `statusClass`.

```js
assert.match(calculateBudget.toString(), /else\s*\{[\s\S]*statusClass\s*=\s*('|")surplus\1/);
```
