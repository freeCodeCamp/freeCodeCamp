...
Your income `input` element needs some additional attributes to ensure users enter valid values.

Update the `#income` element you created with the following attributes:

- `type` set to `number` (to allow only numeric inputs)
- `min` set to `0` (to allow only positive numbers)
- `placeholder` set to `e.g. 2000`
...
Your `input` element should have a `placeholder` attribute set to `e.g. 2000`.
