...
Now it's time to start putting it all together. Declare an empty `calculateBudget` function, which takes a parameter named `e`. This function will be another event listener, so the first argument passed will be the browser event – `e` is a common name for this parameter.

# --hints--

You should declare a `calculateBudget` variable.

```js
assert.isDefined(calculateBudget);
```

Your `calculateBudget` variable should be a function.

```js
assert.isFunction(calculateBudget);
```
