...
Add a line to your `calculateBudget` function that calls the `preventDefault()` method on the `e` parameter. Then, reset your global error flag `isError` to `false`.

# --hints--

Your `calculateBudget` function should call `e.preventDefault()`.

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

Your `calculateBudget` function should reset the global error flag `isError` to `false`.

```js
assert.match(calculateBudget.toString(), /isError\s*=\s*false/);
```
