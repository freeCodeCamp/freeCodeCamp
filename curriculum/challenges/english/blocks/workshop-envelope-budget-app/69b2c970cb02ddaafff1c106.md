...
```js
const explorer = await __helpers.Explorer(code);
const { addEntry } = explorer.functions;
assert.exists(addEntry);
const { targetInputContainer } = addEntry.variables;
assert.exists(targetInputContainer);
assert.isTrue(
  targetInputContainer.value.matches(
