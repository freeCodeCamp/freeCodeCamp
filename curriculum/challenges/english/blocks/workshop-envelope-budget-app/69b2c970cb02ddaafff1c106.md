...
Use concatenation to combine `"#"`, the `category` variable, and `" .input-container"`, and pass that string to `querySelector()`.
...
```js
assert.match(addEntry.toString(), /document\.querySelector\(\s*("|')#\1\s*\+\s*category/);
```

You should concatenate `"#"`, the `category` variable, and `" .input-container"`. Remember to include the `#` before `category` and the space before `.input-container`.

```js
assert.match(addEntry.toString(), /document\.querySelector\(\s*("|')#\1\s*\+\s*category\s*\+\s*("|')\s\.input-container\2/);
```
