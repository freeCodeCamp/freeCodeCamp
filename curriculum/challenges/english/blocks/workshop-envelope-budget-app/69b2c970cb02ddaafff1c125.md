...
Now that you have your lists of elements, you can pass them to your `getTotalFromInputs` function to extract the total.

Declare a `rent` variable, and assign it the result of calling `getTotalFromInputs` with `[rentInput]` as the argument.
...
```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```
