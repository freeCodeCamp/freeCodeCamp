...
Start by assigning an empty template literal to the `innerHTML` property of the `output` element on a new line at the end of the `if` statement.
...
```js
assert.match(calculateBudget.toString(), /output\s*\.\s*innerHTML/);
```
