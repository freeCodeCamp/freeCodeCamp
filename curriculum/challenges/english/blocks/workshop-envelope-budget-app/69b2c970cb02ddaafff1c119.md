...
You should call the `cleanInputString` function inside the `getTotalFromInputs` function.

```js
assert.match(getTotalFromInputs.toString(), /cleanInputString\(/);
```

You should pass `item.value` as the argument for `cleanInputString` function.

```js
assert.match(getTotalFromInputs.toString(), /cleanInputString\(\s*item\.value\s*\)/);
```
