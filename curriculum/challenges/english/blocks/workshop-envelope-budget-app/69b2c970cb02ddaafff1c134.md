...
Now create a `p` element with the text `$${income} Total Income`, using interpolation to replace income with the appropriate variable.
...
Your `p` element should have the text `$${income} Total Income`.

```js
const htmlString = code.split(/output\s*\.\s*innerHTML\s*=\s*/)[1].split(/`/)[1];
assert.match(htmlString, /<p>\$\$\{income\}\s*Total\s*Income<\/p>/);
```
