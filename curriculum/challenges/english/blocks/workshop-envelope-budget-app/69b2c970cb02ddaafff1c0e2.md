...
Next, create a second `fieldset` element with the `id` set to `food`.

Within that element, create a `legend` element with the text `Food`, and an empty `div` with the `class` set to `input-container`.
...
Your second `fieldset` element should have an `id` set to `food`.
...
Your new `legend` element should have the text `Food`.
