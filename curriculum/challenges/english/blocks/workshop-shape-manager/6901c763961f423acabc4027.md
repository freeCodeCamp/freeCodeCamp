...
You need to access the select element from the event by casting it to the correct type and validating it exists. 

To do that, create a `Val` variable set to `e.currentTarget as HTMLSelectElement`. After that, use an `if` statement to check if `Val` is falsy. If it is, return `target value not found`.
