...
Now that you've made everything resusable, you don't need the earlier references and logic for calculating the area of a circle, so remove them all.

With that, the shape manager workshop is complete!

# --hints--

You should remove the earlier logic calculating the area of a circle. 

```js
assert.notMatch(
  __helpers.removeJSComments(code),
  /radiusInput\.addEventListener|Enter a radius to see the area|Math\.PI\s*\*\s*radius/
)
```
