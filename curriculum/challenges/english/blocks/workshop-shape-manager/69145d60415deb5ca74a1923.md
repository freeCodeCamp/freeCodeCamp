...
Add a `default` case to handle unexpected shape types. Assign `shape` to a variable `_nonExistant` typed as `never` and return the variable. This ensures all possible types are covered.
