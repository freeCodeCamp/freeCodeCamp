...
Create a `clearInput` function. Inside it, use `querySelectorAll` to get all the input fields and loop through them using `forEach()` with an `input` parameter right away. Inside the `forEach()`, set the value of each `input` to an empty string.

# --hints--

You should create a `clearInput` function.

```js
assert.isFunction(clearInput)
```

Your `clearInput` function should set each input field to an empty string.
