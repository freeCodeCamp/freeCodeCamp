...
Now, call the `toggleResultCard` function with `hasSelection`, the `chooseShape` function with `Val.value`, and the `updateResult` function.

After doing that, you should be able to see the input fields of each shape and type in them.
