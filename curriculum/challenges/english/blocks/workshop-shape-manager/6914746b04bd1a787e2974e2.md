...
Add an `else if` block that checks if `shape` is strictly equal to `"rectangle"`. If it is, set the `result` variable to the calling of the `CalculateArea` function.

To properly calculate the area of the rectangle, pass in a `type` of `"rectangle"`, a `width` set to `Number(propertyInputs.width.value)`, and a `height` set to `Number(propertyInputs.height.value)`.
