...
The rest of the project will focus on calculating the areas of the selected shape immediately the inputs are filled out.

Use arrow syntax to create a `handleInput` function with an `e` parameter of type `Event`. Inside it, create a `value` variable set to the value of the current target. Make sure it is converted to a number and cast to the `HTMLInputElement` type using the `as` keyword.
