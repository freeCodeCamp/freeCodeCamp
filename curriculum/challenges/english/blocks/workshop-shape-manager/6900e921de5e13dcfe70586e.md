...
Now, you will start connecting the functions you've created to user interactions by using the `oninput` event handler.

Add an `oninput` event to the `shapeTypeSelect` element using an arrow function syntax. The function should take an `e` parameter typed as `Event`. Inside the function, call `e.preventDefault()` for now to stop the browser default behavior.
