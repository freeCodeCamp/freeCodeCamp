...
```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\s*\(\s*["']result-text["']\s*\);?/)
```
...
const { initializeApp } = explorer.allFunctions;
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\s*\(\s*["']result-text["']\s*\)\s+as\s+HTMLElement;?/)
```
...
```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\s*\(\s*["']result-card["']\s*\);?/)
```
...
const explorer = await __helpers.Explorer(code);
const { initializeApp } = explorer.allFunctions;
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\s*\(\s*["']result-card["']\s*\)\s+as\s+HTMLElement;?/)
```
