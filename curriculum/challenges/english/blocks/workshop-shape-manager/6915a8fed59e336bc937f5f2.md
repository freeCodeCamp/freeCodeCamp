...
The last set of elements to select are the result elements. Use your `getElements` function to get the element with the ids `result-text` and `result-card`. Both should use `HTMLElement` as the type.

Remember you also created inline types `resultText` and `resultCard` for them, so you only need to reassign them.

# --hints--

You should reassign the `resultText` variable with appropriate value.
...
```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElements\(["']result-text["']\);?/)
```

You should use `HTMLElement` as the generic type of the `getElements` you're using to select the `result-text` element.

```js
assert.match(code, /resultText\s*=\s*getElements<HTMLElement>\(["']result-text["']\);?/)
```

You should reassign the `resultCard` variable with appropriate value.
...
```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElements\(["']result-card["']\);?/)
```

You should use `HTMLElement` as the generic type of the `getElements` you're using to select the `result-card` element.

```js
assert.match(code, /resultCard\s*=\s*getElements<HTMLElement>\(["']result-card["']\);?/)
```
