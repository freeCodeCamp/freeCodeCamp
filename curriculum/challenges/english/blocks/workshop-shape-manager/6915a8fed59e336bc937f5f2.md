...
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```
...
const explorer = await __helpers.Explorer(code);
const { initializeApp } = explorer.allFunctions;
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\)\s+as\s+HTMLElement;?/)
```
