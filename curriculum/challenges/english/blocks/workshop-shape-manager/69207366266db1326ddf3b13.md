...
```js
const domListeners = [];
const { addEventListener: originalAddEventListener } = document;
document.addEventListener = function (...args) {
  domListeners.push(args);
...
```js
assert.isTrue(domListeners.some(([type]) => type === 'DOMContentLoaded'));
```

You should reference `initializeApp` as the function to run in your event listener.

```js
assert.isTrue(domListeners.some(([type, listener]) => type === 'DOMContentLoaded' && listener === initializeApp));
```
