...
Back to the `initializeApp` function, this is where you will put the inline types and `GetElements()` function you created earlier into use by getting the DOM elements needed for the app.

Inside the `initializeApp` function, use the `GetElements()` function to select the `shape-type` select element and pass in `HTMLSelectElement` as the generic type parameter. Assign it to the variable `shapeTypeSelect`.

# --hints--

You should reassign `shapeTypeSelect`.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

You should call the `GetElements` function with `shape-type` and assign it to `shapeTypeSelect`.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*GetElements\(["']shape-type["']\);?/)
```

You should call `GetElements` with `HTMLSelectElement` as its generic type.

```js
assert.match(code, /shapeTypeSelect\s*=\s*GetElements<HTMLSelectElement>\s*\(["']shape-type["']\);?/)
```
