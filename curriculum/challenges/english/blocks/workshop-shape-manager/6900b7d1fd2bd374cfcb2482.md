...
Now, create an `updateResult` function. Inside it, create a `shape` variable set to `shapeTypeSelect.value`, then use `let` to create a `result` variable of type `string` set to an empty string.

This function will read the selected shape type from the dropdown, gather the corresponding input values, create a `shape` object, pass it to the `CalculateArea()` function to compute the area, and display the calculated result in the `resultText` element.
