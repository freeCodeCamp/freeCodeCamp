...
const spy = __helpers.spyOn(event, 'preventDefault');
try {
...
inputs.forEach(input => { input.value = '5'; });
handleShapeSelect({ preventDefault() {} });
assert.isNotEmpty(inputs);
assert.isTrue(inputs.every(input => input.value === ''));
```
