...
Inside the `handleShapeSelect` function, call the `.preventDefault()` method on `e`, and call the `clearInput()` function to clear the input fields.
...
You should call the `clearInput` function.

```js
console.log(handleShapeSelect.toString())
assert.match(handleShapeSelect.toString(), /clearInput\(\);?/)
```
