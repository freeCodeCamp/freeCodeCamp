...
Inside the `updateResult` function, add an `if` statement that checks if `shape` is strictly equal to `"circle"`. If it is, update the `result` variable by calling the `CalculateArea` function and passing an object with `type: "circle"` and `radius: Number(propertyInputs.radius.value)`. This ensures the value from the radius input is converted to a number.
