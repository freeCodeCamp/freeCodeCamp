...
Now, create an `if` statement that checks if `value` is less than `0`. IF that is true, it means the value entered is negative. So, using an alert, show the message `Negative values are not allowed.` to the user, and clear the input fields by calling the `clearInput` function.
