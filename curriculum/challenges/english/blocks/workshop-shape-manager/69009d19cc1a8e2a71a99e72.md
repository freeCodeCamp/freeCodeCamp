...
Inside the `forEach()` loop, check if `name` is equal to `shapeType`. If it is, use `classList` to remove the `hidden` class from `group`. Otherwise, add the `hidden` class to the `group`.
