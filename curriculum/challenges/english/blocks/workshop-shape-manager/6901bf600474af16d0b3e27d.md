...
YOu should also work on a function that will clear the inputs fields.

Create a `clearInput` function using arrow syntax. Inside it, use `querySelectorAll` to get all the input fields and loop through them with `forEach()` with an `input` parameter right away. Inside the `forEach()`, set the value of each `input` to an empty string.
