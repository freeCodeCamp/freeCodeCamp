...
Add a `"circle"` case that returns a template literal string showing the area. Use the formula `Math.PI * shape.radius ** 2`, and format the result to two decimal places with the `toFixed()` method. 

The output should look like: `Area of Circle: ${area}`
