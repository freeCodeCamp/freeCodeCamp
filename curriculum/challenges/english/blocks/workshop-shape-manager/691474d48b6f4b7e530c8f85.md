...
Add another `else if` block that checks if shape is strictly equal to `"triangle"`. If it is, set the `result` variable to the calling of the `CalculateArea` function.

To properly calculate the area of the triangle, pass in a `type` of `"triangle"`, a `base` set to `Number(propertyInputs.base.value)`, and a `height` set to `Number(propertyInputs.triangleHeight.value)`. 
