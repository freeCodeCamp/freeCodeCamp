...
Now, you should work on the function that calculates the areas of the shapes.

Create a `CalculateArea` function that takes `shape` of type string as parameter and returns a string. Inside the function, add an empty `switch` statement that takes in `shape.type`.

You will work on the cases in the coming steps.
...
```js
Create a function called CalculateArea that takes a parameter shape of type Shapes and returns a string. Add an empty switch statement that switches on shape.type.
```
