    ...
    setTimeout(
      () =>
        resolve({
          ...
          text: () => Promise.resolve(JSON.stringify(cityMap[city])),
        }),
      0
    )
  );
...
Neither the `getWeather` function nor the `showWeather` function should be declared using `const`.
