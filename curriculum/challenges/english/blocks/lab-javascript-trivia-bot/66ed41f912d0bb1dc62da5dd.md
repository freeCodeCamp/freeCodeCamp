...
const explorer = await __helpers.Explorer(code);
const { codingFact } = explorer.variables;
assert.include(codingFact?.value.toString(), "favoriteLanguage");
```
...
assert.include(getLogs(), codingFact);
...
assert.notEqual(codingFactLogs[codingFactLogs.length - 1], codingFactLogs[codingFactLogs.length - 2]);
```
