...
```js
assert.ok(getLogs().some(log => log === "Hello! I'm your coding fun fact guide!"))
```
...
```js
assert.ok(getLogs().some(log => log === `My name is ${botName} and I live on ${botLocation}.`))
```

You should log to the console `"My favorite programming language is (favoriteLanguage)."` and add the variable to the string.

```js
assert.ok(getLogs().some(log => log === `My favorite programming language is ${favoriteLanguage}.`))
```
...
assert.ok(getLogs().some(log => log && String(log).includes(favoriteLanguage)));
...
const codingFactLogs = getLogs().filter(log => log && String(log).includes(favoriteLanguage));
assert.isAtLeast(codingFactLogs.length, 2);
...
```js
assert.ok(getLogs().some(log => log === `It was fun sharing these facts with you. Goodbye! - ${botName} from ${botLocation}.`))
```
