---
id: 68705554ed9eb4337be6bb4d
title: Task 32
challengeType: 22
dashedName: task-32
lang: zh-CN
---

<!-- (Audio) speaker: shuǐ -->

# --description--

`uei` is often written as `ui` in syllables. As `i` and `u` appear together in `ui`, the tone mark goes on the second vowel, which is `i`. For example, `chuī`.

# --instructions--

Listen to the audio and complete the syllable with the correct tone mark.

# --fillInTheBlank--

## --sentence--

`BLANK`

## --blanks--

`shuǐ`

### --feedback--

This syllable combines the initial `sh` with the final `uei` (written as `ui`), and uses the third tone, which starts in the middle, falls to a low point and then rises again..
