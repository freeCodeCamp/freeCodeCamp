...
```js
assert.match(code, /vigenere\s*\(\s*encryption\s*,\s*custom_key\s*,\s*-\s*1\s*\)/)
```

You should declare a `decryption` variable.

```js
assert(__userGlobals.has("decryption"))
```

Your `decryption` variable should have the value of `vigenere(encryption, custom_key, -1)`.

```js
assert.match(code, /^decryption\s*=\s*vigenere\s*\(\s*encryption\s*,\s*custom_key\s*,\s*-\s*1\s*\)/m)
```
