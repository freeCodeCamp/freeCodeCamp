...
Start by finding the position of the first letter in the string. One way is to call `.find()` string method on the string you want to parse:
...
```js
({test: () => assert.match(code, /^alphabet\.find\s*\(\s*text\[\s*\s*0\s*\]\s*\)/m) })
```
