...
Start by finding the position of the first letter in the string. One way is to use the `.find()` string method:
...
```js
assert.match(code, /alphabet\.find\s*\(\s*text\[\s*\s*0\s*\]\s*\)/)
```
