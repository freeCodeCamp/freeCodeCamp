...
For that purpose, you need to use a `return` statement. You need to write `return` followed by the value that the function should return. Once the `return` statement is found, that value is returned and the execution of the function stops, proceeding to the next line of code after the function call.
...
({ test: () => {
    ...
    assert(function_body.match(regex)) && assert.notMatch(block_body, regex);
  }
})
```
