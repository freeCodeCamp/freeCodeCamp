---
id: 65520e6f2b9678799977f24d
title: Step 17
challengeType: 20
dashedName: step-17
---

# --description--

Remove the last `print()` call. Then, instead of `text[0]`, pass `text[0].lower()` to `.find()` and see the output.

# --hints--

You should not have `print(text.lower())` in your code.

```js
assert.isFalse(/^print\s*\(\s*text\.lower\s*\(\s*\)\s*\)/m.test(code))
```

You should still print your `index` variable.

```js
assert.match(code, /^print\s*\(\s*index\s*\)/m)
```

You should pass `text[0].lower()` to the `.find()` method. Pay attention to place the method call at the beginning of the line.

```js
({test: () => assert.match(code, /^index\s*=\s*alphabet\.find\s*\(\s*text\[\s*\s*0\s*\]\.lower\s*\(\s*\)\s*\)\s*(#.*)?$/m) })
```

# --seed--

## --seed-contents--

```py
--fcc-editable-region--
text = 'Hello World'
shift = 3
alphabet = 'abcdefghijklmnopqrstuvwxyz'
index = alphabet.find(text[0])
print(index)
print(text.lower())
--fcc-editable-region--
```
