...
```js
assert.match(code, /vigenere\s*\(\s*text\s*,\s*custom_key\s*\)/)
```

You should have an `encryption` variable.

```js
assert(__userGlobals.has("encryption"))
```

You should assign your function call to the `encryption` variable.

```js
assert.match(code, /encryption\s*=\s*vigenere\s*\(\s*text\s*,\s*custom_key\s*\)/)
```
