...
The `__mul__` special method can be implemented to specify what should happen when the current instance is multiplied by another object.

Create an empty `__mul__` method within the `R2Vector` class and give it two parameters: `self`, and `other`.

# --hints--

You should define a `__mul__` method within the `Vector` class.
