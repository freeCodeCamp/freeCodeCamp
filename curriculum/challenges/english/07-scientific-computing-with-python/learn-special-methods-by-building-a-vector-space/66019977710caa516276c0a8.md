...
```js
assert(runPython(`_Node(_code).find_class("R3Vector").has_function("cross")`))
```

Your `cross` method should have two parameters, `self` and `other`.

```js
assert(runPython(`_Node(_code).find_class("R3Vector").find_function("cross").has_args("self, other")`))
```
