...
```js
assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__eq__")`))
```

Your `__eq__` method should take two parameters: `self` and `other`.

```js
assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__eq__").has_args("self, other")`))
```
