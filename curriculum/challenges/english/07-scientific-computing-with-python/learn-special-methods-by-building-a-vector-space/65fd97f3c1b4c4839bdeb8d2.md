...
After the `if` statement you created in the previous step, return a call to the `all()` function. Pass it a generator expression that iterates over `__dict__` and compares each attribute of the current instance to the same attribute of `other`.
