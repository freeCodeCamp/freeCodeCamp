...
```js
assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__ge__")`))
```

Your `__ge__` method should have two parameters, `self` and `other`.

```js
assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__ge__").has_args("self, other")`))
```

Your `__ge__` method should return the opposite of `self < other`.

```js
assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__ge__").has_return("not self < other")`))
```
