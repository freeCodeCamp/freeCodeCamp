...
Add a new key `'z'` to `kwargs` and assign it the difference between the `x` attribute of the current instance times the `y` attribute of `other` and the `y` attribute of the current instance and the `x` attribute of `other`.
