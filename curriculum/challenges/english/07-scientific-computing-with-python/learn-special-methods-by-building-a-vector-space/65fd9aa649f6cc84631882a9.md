...
```js
assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__ne__")`))
```

Your `__ne__` method should have two parameters, `self` and `other`.

```js
assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__ne__").has_args("self, other")`))
```
