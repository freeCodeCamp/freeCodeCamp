...
Vectors can be multiplied by a *scalar*, i.e. a number that multiplies each single components. The result of a scalar multiplication is a vector with the same orientation but different magnitude.

Create an `if` statement to check if `other` is not an `int` or a `float`. Use the `isinstance` function and pass it the tuple `(int, float)` as the second argument.
