...
```js
assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__lt__")`))
```

Your `__lt__` method should have two parameters, `self` and `other`.

```js
assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__lt__").has_args("self, other")`))
```
