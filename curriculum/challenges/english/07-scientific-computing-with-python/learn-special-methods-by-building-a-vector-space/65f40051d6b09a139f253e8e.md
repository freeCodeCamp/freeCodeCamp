...
Inheritance enables you to use define a class from an existing one. The new class, called child, inherits all the methods and properties of the existent class, called parent.
...
({
    test: () => assert(runPython(`
      _Node(_code).find_class("R3Vector")
    `))
})
```
