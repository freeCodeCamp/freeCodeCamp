...
```js
assert(runPython(`_Node(_code).find_class("R3Vector").find_function("__init__").find_body().is_equivalent("super().__init__(x=x, y=y)\\nself.z = z")`))
```

You should modify the assignment of `v1` to be `R2Vector(x=2, y=3)`.

```js
assert(runPython(`_Node(_code).find_variable("v1").is_equivalent("v1 = R2Vector(x=2, y=3)")`))
```

You should modify the assignment of `v2` to be `R3Vector(x=2, y=2, z=3)`.

```js
assert(runPython(`_Node(_code).find_variable("v2").is_equivalent("v2 = R3Vector(x=2, y=2, z=3)")`))
```
