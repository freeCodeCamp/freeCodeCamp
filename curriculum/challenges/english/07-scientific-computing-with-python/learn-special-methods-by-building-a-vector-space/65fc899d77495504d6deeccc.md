...
Declare a variable `args`. Assign it a list comprehension that iterates over `__dict__` and for each attribute (`i`) multiplies the `i` attribute of the current instance and the `i` attribute of `other`.
