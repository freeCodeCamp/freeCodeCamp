...
```js
assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__add__")`))
```

Your `__add__` method should have two parameters, `self` and `other`.

```js
assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__add__").has_args("self, other")`))
```
