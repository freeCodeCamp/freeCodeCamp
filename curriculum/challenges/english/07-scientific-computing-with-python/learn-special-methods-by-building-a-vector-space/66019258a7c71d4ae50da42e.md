...
```js
assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__le__")`))
```

Your `__le__` method should have two parameters, `self` and `other`.

```js
assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__le__").has_args("self, other")`))
```

Your `__le__` method return the opposite of `self > other`.

```js
assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__le__").has_return("not self > other")`))
```
