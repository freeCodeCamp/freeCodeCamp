...
There are still two possible comparisons to implement. Define a `__le__` method with `self` and `other` as the parameters and make it return the opposite of `self > other`.
