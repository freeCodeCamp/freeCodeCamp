...
Add a new key `'y'` to `kwargs` and assign it the difference between the `z` attribute of the current instance times the `x` attribute of `other` and the `x` attribute of the current instance and the `z` attribute of `other`.
