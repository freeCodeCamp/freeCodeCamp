...
Create an `if` statement that checks if `other` is not an instance of the current class and raise a `TypeError` using the following f-string as a custom message: `f"Argument must be a '{self.__class__.__name__}' instance"`.
