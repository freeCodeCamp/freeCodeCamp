...
```js
assert(runPython(`_Node(_code).find_class("Board").has_function("find_empty_cell")`))
```

Your `find_empty_cell` method should have a parameter `self`.

```js
assert(runPython(`_Node(_code).find_class("Board").find_function("find_empty_cell").has_args("self")`))
```
