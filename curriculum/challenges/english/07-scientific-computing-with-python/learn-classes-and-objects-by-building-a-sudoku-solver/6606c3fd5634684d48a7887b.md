...
Pass this generator expression to the `all()` function to check if all the elements in the column are different from `num` and return the result.

# --hints--

You should pass the generator expression as the argument to an `all()` call an return the result.
