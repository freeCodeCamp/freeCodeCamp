...
Delete `pass` and create an `if` statement that ckecks if the value returned by `find_empty_cell` is `None`. In that case, the puzzle is solved. Therefore, return `True` from the `if` body.

# --hints--

You should delete `pass` and create an `if` statement that ckecks if the value returned by `find_empty_cell` is `None`.
