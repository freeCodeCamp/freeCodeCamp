...
```js
assert(runPython(`_Node(_code).find_class("Board").has_function("is_valid")`))
```

Your `is_valid` method should have three parameters: `self`, `empty`, and `num`, in this order.

```js
assert(runPython(`_Node(_code).find_class("Board").find_function("is_valid").has_args("self, empty, num")`))
```
