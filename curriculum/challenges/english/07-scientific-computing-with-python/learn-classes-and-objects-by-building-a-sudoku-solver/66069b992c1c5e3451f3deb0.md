...
```js
assert.isFalse(runPython(`_Node(_code).find_class("Board").has_function("spam")`))
```

You should define an `__init__` method in your `Board` class.

```js
assert(runPython(`_Node(_code).find_class("Board").has_function("__init__")`))
```

Your `__init__` method should have a `self` parameter.

```js
assert(runPython(`_Node(_code).find_class("Board").find_function("__init__").has_args("self")`))
```
