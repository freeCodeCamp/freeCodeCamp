...
Now, you need to iterate only over the rows inside the 3x3 square. Create a `for` loop for that. Use the `range()` function to generate the sequence of rows starting at `row_start`. Also, use `row_no` as the loop variable.
