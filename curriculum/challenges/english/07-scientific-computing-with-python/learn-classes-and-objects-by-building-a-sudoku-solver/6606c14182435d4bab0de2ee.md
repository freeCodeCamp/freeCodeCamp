...
```js
assert(runPython(`_Node(_code).find_class("Board").has_function("valid_in_col")`))
```

Your `valid_in_col` method should have three parameters: `self`, `col`, and `num`, in this order.

```js
assert(runPython(`_Node(_code).find_class("Board").find_function("valid_in_col").has_args("self, col, num")`))
```
