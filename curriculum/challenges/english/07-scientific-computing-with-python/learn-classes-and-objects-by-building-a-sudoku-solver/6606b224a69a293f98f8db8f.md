...
The `.index()` method raises a `ValueError` exception when the value is not found. To avoid stopping the program execution, you will nest this line of code inside a `try` block. The `try` statement is used to wrap some code that might raise an exception. The `except` clause instead, provides alternative code to run in case the code inside the `try` block has raised an exception:
