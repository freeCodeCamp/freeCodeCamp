...
```js
({ test: () => assert(runPython(`_Node(_code).find_function("total_expenses").find_calls("map").find_call_args()[0].is_equivalent("lambda expense: expense['amount']")`)) })
```

You should pass the `expenses` list as the second argument to the `map()` call.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("total_expenses").find_calls("map").find_call_args()[1].is_equivalent("expenses")`)) })
```
