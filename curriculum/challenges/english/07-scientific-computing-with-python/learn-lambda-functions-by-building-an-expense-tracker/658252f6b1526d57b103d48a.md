...
You should have `amount = input('Enter amount: ')` in your `if` statement.
...
({ test: () =>
  {
    ...
    assert(function_body.match(/^\s+if\s+choice\s*==\s*("|')1\1\s*:\s*amount\s*=\s*input\s*\(\s*("|')Enter\samount:\s\1\s*\)/m));
  }
})
```
