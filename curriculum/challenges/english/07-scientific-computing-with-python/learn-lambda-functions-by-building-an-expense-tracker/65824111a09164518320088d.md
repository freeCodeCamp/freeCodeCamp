...
```js
assert(runPython(`len(_Node(_code).find_function("filter_expenses_by_category").find_calls("filter")) == 1`))
```

You should pass `lambda expense: expense['category'] == category` as the first argument to the `filter()` call.

```js
assert(runPython(`_Node(_code).find_function("filter_expenses_by_category").find_calls("filter")[0].find_call_args()[0].is_equivalent("lambda expense: expense['category'] == category")`))
```

You should pass `expenses` as the second argument to the `filter()` call.

```js
assert(runPython(`_Node(_code).find_function("filter_expenses_by_category").find_calls("filter")[0].find_call_args()[1].is_equivalent("expenses")`))
```
