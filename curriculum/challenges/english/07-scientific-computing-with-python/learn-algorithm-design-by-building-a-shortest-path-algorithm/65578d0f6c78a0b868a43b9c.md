...
```js
assert(runPython(`_Node(_code).find_function("shortest_path").find_whiles()[0].has_variable("current")`))
```

You should assign `min(unvisited)` to your `current` variable. Remember to delete `pass`.

```js
assert(runPython(`_Node(_code).find_function("shortest_path").find_whiles()[0].find_bodies()[0].is_equivalent("current = min(unvisited)")`))
```
