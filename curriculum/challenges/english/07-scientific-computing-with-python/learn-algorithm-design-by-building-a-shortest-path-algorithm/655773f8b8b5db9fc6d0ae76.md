...
```js
assert(runPython(`_Node(_code).find_function("shortest_path").has_variable("unvisited")`))
```

You should assign an empty list to your `unvisited` variable. Remember to delete `pass`.

```js
assert(runPython(`_Node(_code).find_function("shortest_path").find_body().is_equivalent("unvisited = []")`))
```
