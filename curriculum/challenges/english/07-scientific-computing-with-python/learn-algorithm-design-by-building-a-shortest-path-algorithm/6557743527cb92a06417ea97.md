...
When iterating in dictionaries using a for each element will be the key of the dictionary. If you want to iterate over the values you can use `for value in my_dict.values()` or to iterate over the keys and values at the same time you can use `for key, value in my_dict.items()`.
