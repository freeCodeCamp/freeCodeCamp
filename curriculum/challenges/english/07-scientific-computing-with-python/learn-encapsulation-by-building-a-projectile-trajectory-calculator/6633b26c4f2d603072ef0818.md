...
It's good practice to give a representation to the class by using the `__repr__` special method. While `__str__` method returns readable string representation, that's intended to be user friendly, `__repr__` is intended for programmer. Often `__repr__` provides string that can be used to recreate the object.

Write `__repr__` method, which will return the string needed to instantiate the object.
