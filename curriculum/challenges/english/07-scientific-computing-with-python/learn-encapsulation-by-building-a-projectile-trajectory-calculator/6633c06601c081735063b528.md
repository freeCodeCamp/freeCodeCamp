...
Now to conclude, create a little utility function called`projectile_helper` outside the classes that takes in the desired values for speed, height and angle and print to the terminal in sequence, the details of the projectile, the table of coordinates and the graph of the trajectory.
...
class Graph:
    ...
    def create_trajectory(self):
        ...
        matrix = ["".join(list) for list in matrix_list]

        matrix_axes = [y_axis_tick + row for row in matrix]
        matrix_axes.append(" " + x_axis_tick * (len(matrix[0]) - 1))
