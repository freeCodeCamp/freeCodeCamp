...
You have a list of lists of strings. Join the inner lists in `matrix` to have a list of strings.
...
The method `create_trajectory` should return `matrix`.

```js
({
    test: () => runPython(`
    assert _Node(_code).find_class('Graph').find_function('create_trajectory').has_return('matrix'), "return matrix missing"
    `)
})
```

`matrix` should have the correct value. 
