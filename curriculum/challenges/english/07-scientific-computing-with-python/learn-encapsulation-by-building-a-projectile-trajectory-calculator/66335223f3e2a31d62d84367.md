...
The new class `Graph` has a private property `__coordinates` where the list of coordinates is stored. Create the class, its `__slots__` special method and its `__init__` special method, and its `__repr__` special method.
...
The `__slots__` variable should have value of `('__coordinates')`.

```js
({
    test: () => assert(runPython(
        `_Node(_code).find_class('Graph').find_variable('__slots__').is_equivalent('__slots__ = ("__coordinates")')`
    ))
})
```
