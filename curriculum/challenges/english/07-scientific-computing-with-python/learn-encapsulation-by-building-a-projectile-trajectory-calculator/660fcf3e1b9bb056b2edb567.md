...
```js
assert(runPython(`_Node(_code).has_variable('ball')`))
```

The `ball` variable should have a value of `Projectile(10, 3, 45)`.

```js
assert(runPython(`_Node(_code).find_variable('ball').is_equivalent('ball = Projectile(10, 3, 45)')`))
```
...
```js
assert(runPython(`p = Projectile(10, 10, 10)
str(
    p
) == """
Projectile details:
