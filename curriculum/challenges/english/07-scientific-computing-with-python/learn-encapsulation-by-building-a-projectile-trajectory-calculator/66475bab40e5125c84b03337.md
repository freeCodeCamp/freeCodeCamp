...
Now that you have the coordinates like that, find the max value between all the x and the max value between all the y, these max values will be the number of rows and columns in the graph. Save the first in a new variable named `x_max` and the second in a variable named `y_max`.
