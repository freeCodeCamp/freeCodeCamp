...
For the value of the angle, the argument should be a degrees value, but it should be saved internally in radians, so you need to use `math.radians()` to convert it.
...
A new `Projectile` class instance should have the values of the three attributes properly initialised.

```js
({test: () => assert(runPython(`
p = Projectile(20, 21, 22)
p._Projectile__height == 20 and p._Projectile__speed == 21 and p._Projectile__angle == math.radians(22)
`))})
```
