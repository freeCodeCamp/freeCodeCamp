...
For that, inside the `else` block, create a `for` loop that runs up to `max_iterations` times. To iterate you can use range function, it returns a sequence of numbers starting from 0 and stops before a specified number.

You should use `_` as a loop variable. The `_` acts as a placeholder and is useful when you need to use a variable but don't actually need its value.
