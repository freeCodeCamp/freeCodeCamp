...
For that, inside the `else` block, create a `for` loop that runs up to `max_iterations` times. To iterate you can use range function, it returns a sequence of numbers, starting from 0 by default, and increments by 1 (by default), and stops before a specified number.
