...
Then, return a dictionary with the keys `'slope'` and `'y-intercept'` and the values of the slope and the y-intercept, respectively.
...
The `analyze` method should return a dictionary with the keys `'slope'` and `'y-intercept'` and the values of the slope and the y-intercept, respectively.
...
assert a['y-intercept'] == 1.5, "Expected different intercept"
`) })
```
