...
```js
assert(runPython(`_Node(_code).find_class("LinearEquation").find_function("analyze").has_stmt("slope, intercept = self.coefficients.values()")`))
```
...
```js
assert(runPython(`_Node(_code).has_call("print(lin_eq.analyze())")`))
```
