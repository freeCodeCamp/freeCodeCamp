...
You should modify the pattern passed as the first argument to the `re.sub()` call using a negative lookbehind to match the character `1` only if it is not preceded by a digit. Use the syntax `(?<!...)` where `...` represents the pattern for the preceding condition.
