...
After the `if` statement you wrote in the previous step, create other two `if` statements. Check if the `results` and `details` attributes of `equation` contain elements. If not, call the `solve` method, and the `analyze` method, respectively. 

# --hints--

You should create an `if` statement to check if `equation.results` does not contain elements and call `equation.solve()` within its body.

```js
({ test: () => runPython(`
ifs = _Node(_code).find_function("solver").find_ifs()

assert False, 'missing test'
`) })
```

You should create an `if` statement to check if `equation.details` does not contain elements and call `equation.analyze()` within its body.

```js
({ test: () => runPython(`assert False, 'missing test'`) })
```
