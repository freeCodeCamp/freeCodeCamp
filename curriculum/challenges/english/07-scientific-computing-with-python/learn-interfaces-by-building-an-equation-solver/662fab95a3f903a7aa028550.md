...
In the example above, the `swim` property of `trout` is accessed through `trout.`swim` instead of `trout.swim()`, as it would be for a regular method.

Decorators can be combined (by specifying them in separate lines before the method to decorate) to obtain enhanced functionalities.
