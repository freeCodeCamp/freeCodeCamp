...
```js
assert(runPython(`_Node(_code).find_function("solver").has_stmt("output_string = f'\\\\n{equation.type:^24}'")`))
```

Your `solver` function should return `output_string`.

```js
assert(runPython(`_Node(_code).find_function("solver").has_return("output_string")`))
```

You should print `solver(lin_eq)`.

```js
assert(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`))
```
