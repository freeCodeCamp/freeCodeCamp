...
For example, a `LinearEquation` object instantiated with `2` and `4` should have the folowing `coefficients` attribute: `{0: 4, 1: 2}`, because `4` corresponds to zero-th degree of `x` and `2` corresponds to the first degree of `x`.
