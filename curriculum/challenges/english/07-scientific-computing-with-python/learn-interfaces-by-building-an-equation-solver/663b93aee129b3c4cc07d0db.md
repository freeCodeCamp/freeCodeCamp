...
Inside the loop, create an `if` statement that checks if the coefficient at the current iteration is not `0`.
...
You should create an `if` statement to check if `coefficient` has non-zero value inside your `for` loop.

```js
({ test: () => assert(runPython(`
if_cond = _Node(_code).find_class("Equation").find_function("__str__").find_for_loops()[0].find_ifs()[0].find_conditions()[0]
conditions = ["coefficient", "coefficient != 0", "0 != coefficient"]
