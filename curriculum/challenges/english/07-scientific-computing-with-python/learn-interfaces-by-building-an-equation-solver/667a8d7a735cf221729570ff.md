...
```js
assert(runPython(`len(_Node(_code).find_function("solver").find_matches()[1].find_match_cases()) == 2`))
```

You should create a `for` loop to iterate over `details_list`.

```js
assert(runPython(`_Node(_code).find_function("solver").find_for_loops()[1].find_for_iter().is_equivalent("details_list")`))
```
