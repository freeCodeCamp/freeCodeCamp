...
It would be better if the `solve` method could return the same data type for each class. The `solve` method from the `QuadraticEquation` class returns a list, because quadratic equations can have two roots.
