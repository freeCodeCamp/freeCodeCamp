...
Within the `QuadraticEquation` class, define an `__init__` method. Use a `super()` call to inherit the `__init__` method defined inside the interface and define a new atttribute named `delta`, which stores the value of the discriminant of the equation.
