...
```js
assert(runPython(`_Node(_code).find_class("QuadraticEquation").has_function("__init__")`))
```

Your `__init__` method should take two parameters, `self` and `*args`.

```js
assert(runPython(`_Node(_code).find_class("QuadraticEquation").find_function("__init__").has_args("self, *args")`))
```

You should call `super().__init__(*args)` within your `__init__` method.

```js
assert(runPython(`_Node(_code).find_class("QuadraticEquation").find_function("__init__").has_call("super().__init__(*args)")`))
```
