...
```js
assert(runPython(`_Node(_code).find_class("Equation").has_stmt("type: str")`))
```
...
```js
assert(runPython(`_Node(_code).find_class("LinearEquation").has_stmt("type = 'Linear Equation'")`))
```

The `type` attribute of the `QuadraticEquation` class should have the value `'Quadratic Equation'`.

```js
assert(runPython(`_Node(_code).find_class("QuadraticEquation").has_stmt("type = 'Quadratic Equation'")`))
```
