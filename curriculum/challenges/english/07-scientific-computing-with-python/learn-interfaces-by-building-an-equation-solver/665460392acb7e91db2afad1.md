...
TODO: rewrite it to enable free implementation

Create an `if`/`else` statement to check the concavity of the parabola. In each clause, declare a `concavity` variable and assign it either the string `'upwards'` or `'downwards'`. Also, assign either the string `'min'` or `'max'` to the `'m'` key of the `vertex` dictionary.
...
assert eq.analyze() == {'vertex': {'x': -0.0625, 'y': 0.9375, 'm': 'min'}, 'concavity': 'upwards'}
`) })
```
