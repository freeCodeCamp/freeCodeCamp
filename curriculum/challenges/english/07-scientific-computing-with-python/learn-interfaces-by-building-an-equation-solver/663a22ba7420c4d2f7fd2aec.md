...
```js
assert(runPython(`_Node(_code).find_class("LinearEquation").find_function("solve").has_stmt("a, b = self.coefficients.values()")`))
```

You should declare a variable named `x` and assign it the solution of the linear equation.

```js
assert(runPython(`_Node(_code).find_class("LinearEquation").find_function("solve").has_stmt("x = -b/a")`))
```

You should return `x` from your `solve` method.

```js
assert(runPython(`_Node(_code).find_class("LinearEquation").find_function("solve").has_return("x")`))
```
