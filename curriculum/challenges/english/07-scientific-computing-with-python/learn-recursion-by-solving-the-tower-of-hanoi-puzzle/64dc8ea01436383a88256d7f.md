...
Instead of adding the items manually to the first list, generate a sequence of numbers counting down from `3` to `1` by using the `range()` function and assign it to `rods['A']`. Here, 3 represents the smallest disk at the top of the pile and 1 represents the largest disk at the bottom of the pile.
