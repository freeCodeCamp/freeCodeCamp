...
```js
assert(__userGlobals.has('NUMBER_OF_DISKS'))
```

The `NUMBER_OF_DISKS` variable should have the value `3`.

```js
assert.equal(__userGlobals.get('NUMBER_OF_DISKS'), 3)
```

You should change the value of the `range()` function's first argument into the `NUMBER_OF_DISKS` variable.

```js
assert.match(code, /('|")A\1\s*:\s*list\s*\(\s*range\s*\(\s*NUMBER_OF_DISKS\s*,\s*0\s*,\s*-\s*1\s*\)\s*\)/)
```
