...
Now you need to check if the `node` parameter is `None`(translates to `if node is None`). If it is, this means that the method has reached a leaf node or an empty spot in the tree where the new node should be inserted.
