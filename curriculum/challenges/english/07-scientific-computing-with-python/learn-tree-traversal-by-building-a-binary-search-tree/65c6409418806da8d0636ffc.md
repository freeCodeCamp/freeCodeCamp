...
The `insert` and `search` functionality is complete, it's time to test it.
Note that, at this point, the nodes are not sorted and just inserted. We will work on sorting using inorder traversal later on.
