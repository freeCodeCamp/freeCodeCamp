...
If the `square_target` is less than `0`, raise a `ValueError` with the message `'The square root of a negative number is not defined in real numbers.'` Don't forget to remove the `pass` keyword.
...
You should raise the `ValueError` with the message `'The square root of a negative number is not defined in real numbers.'` inside `if`.
