...
Remove the pass statement and create an `if` condition to check if `square_target` is less than `0`.
...
You should create the if condition `if square_target < 0:`.

```js
({
    test: () => {
       assert(runPython(`_Node(_code).find_function("square_root_bisection").find_if("square_target < 0")`))
