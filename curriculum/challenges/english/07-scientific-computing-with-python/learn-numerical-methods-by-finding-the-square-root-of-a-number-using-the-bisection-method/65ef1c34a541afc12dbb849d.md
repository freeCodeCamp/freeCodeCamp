...
The `abs()` function returns the positive value of a number. The returned value, regardless of the sign, is enough to assess if the estimated square root is close enough to the actual value, within a defined tolerance.
...
You should have the condition `if abs(square_mid - square_target) < tolerance:` inside the body of the `for` loop.
