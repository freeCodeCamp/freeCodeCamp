...
Create an `else` condition to handle the case where the value of `root` is not `None` and found as a result of convergance. If it is not `None`, print the message `'The approximate square root of {square_target} is {root}.'`. Remember to format using an f-string.

# --hints--

There should be an `else` condition
