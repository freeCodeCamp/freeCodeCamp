...
Now you would repeatedly narrow down the interval by finding the midpoint of the current interval and comparing the square of the midpoint with the target value.

For that, inside the else block, create a `for` loop that runs up to `max_iterations` times.

# --hints--

Create a for loop with _ as the iteration variable and range(max_iterations) as the range.
