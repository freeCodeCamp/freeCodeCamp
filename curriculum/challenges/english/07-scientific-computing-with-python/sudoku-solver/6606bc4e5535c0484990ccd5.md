...
Create a method named `valid_in_row` and give it three parameters: `self`, `row`, and `num`. Where `self` represents the instance of the class, and `row` and `num` are the row index and the number to be checked, respectively.
...
Your `valid_in_row` method should have three parameters: `self`, `row`, and `num`.
