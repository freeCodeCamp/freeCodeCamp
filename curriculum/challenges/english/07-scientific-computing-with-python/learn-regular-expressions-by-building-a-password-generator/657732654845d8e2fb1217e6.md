...
Right now, `all()` is taking an empty list. Populate that empty list using the comprehension syntax so that the list  stores the results of evaluating the expression `constraint <= len(re.findall(pattern, password))` for each `constraint`-`pattern` tuple in the `constraints` list.

In this way, you'll break out of the `while` loop only once all the requirements are fulfilled.
