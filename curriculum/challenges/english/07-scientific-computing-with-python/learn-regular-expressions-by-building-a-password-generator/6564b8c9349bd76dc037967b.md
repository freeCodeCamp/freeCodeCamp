...
```js
assert(__userGlobals.has("quote"))
```

You should assign the provided string to your new `quote` variable.

```js
assert.equal(__userGlobals.get("quote"), "Not all those who wander are lost.")
```

You should print `pattern.search(quote)`.

```js
assert.match(code, /^print\s*\(\s*pattern\.search\s*\(\s*quote\s*\)\s*\)/m)
```
