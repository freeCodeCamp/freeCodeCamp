...
Turn the expression inside your `for` loop into an `if` statement. Use `constraint <= len(re.findall(pattern, password))` as the condition in your `if` statement.

Inside this new `if` statement, increment the `count` variable by `1`.

Remember, `count` should be initialized to `0` before the `for` loop.
