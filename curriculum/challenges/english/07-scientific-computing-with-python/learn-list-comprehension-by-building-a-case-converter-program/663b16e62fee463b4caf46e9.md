...
In the example above, the `join` method is called on the list `words_list`, and the result is used as the object for the `upper` method call.

Modify the return statement to return the joined `snake_cased_char_list` with the `strip` method applied to remove any leading or trailing underscores.
