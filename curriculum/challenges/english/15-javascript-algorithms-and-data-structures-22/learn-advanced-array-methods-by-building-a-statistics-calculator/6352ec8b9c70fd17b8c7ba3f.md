...
```js
assert.match(getMode.toString(), /highest\s*=\s*Object\.keys\(\s*counts\s*\)\.sort\(\s*\(\s*a\s*,\s*b\s*\)/)
```
...
```js
assert.match(getMode.toString(), /highest\s*=\s*Object\.keys\(\s*counts\s*\)\.sort\(\s*\(\s*a\s*,\s*b\s*\)\s*=>\s*\{?\s*(return)?\s*counts\[\s*b\s*\]\s*-\s*counts\s*\[\s*a\s*\]\s*;?\s*\}?\s*\)/)
```
