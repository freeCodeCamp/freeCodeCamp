...
To get the median for an odd number of elements in an array, you will need to find the middle number.

Here is the expression you would use:

```js
arr[Math.floor(arr.length / 2)];
```

Here is how to apply that expression to an array with an odd number of elements: 
