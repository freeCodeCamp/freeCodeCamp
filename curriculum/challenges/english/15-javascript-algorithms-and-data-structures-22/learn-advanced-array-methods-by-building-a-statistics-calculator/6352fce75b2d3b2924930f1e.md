...
assert.match(getVariance.toString(), __helpers.concatRegex(/variance\s*=\s*array\.reduce\(\s*/, functionRe));
```
...
const match = getVariance.toString().match(functionRe);
assert.match(__helpers.removeWhiteSpace(match[1]), /\(acc,el\)=>{}|function\(acc,el\){}/);
```

Your `reduce` callback should have an initial value of `0`.

```js
// The callback has been checked, so we only need to check the initial value.
