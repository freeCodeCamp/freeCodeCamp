...
```js
assert.match(getVariance.toString(), /squared\s*=\s*Math\.pow\(\s*difference\s*,\s*2\s*\)|difference\s*\*\*\s*2\s*;?/);
```

Your `reduce` callback should return the value of `acc` plus `squared`.

```js
assert.match(getVariance.toString(), /return\s+acc\s*\+\s*squared\s*;?/);
```

You should not remove the initial value of `0` from your `.reduce()` method.

```js
assert.match(__helpers.removeWhiteSpace(getVariance.toString()), /,0\);?}$/);
```
