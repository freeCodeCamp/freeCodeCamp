...
```js
assert.match(calculate.toString(), /array\.map\(\s*(\(\s*el\s*\)|el)\s*=>|array\.map\(\s*function\s*\(\s*el\s*\)\s*\{|array\.map\(\s*Number\s*\)/)
```

Your callback function should use the `Number` constructor to convert `el` to a number.

```js
assert.match(calculate.toString(), /Number\(\s*el\s*\)|Number/);
```
...
```js
assert.match(calculate.toString(), /(array\.map\(\s*(\(\s*el\s*\)|el)\s*=>|array\.map\(\s*function\s*\(\s*el\s*\)\s*\{)\s*(return\s+)?Number\(\s*el\s*\)|array\.map\(\s*Number\s*\)/);
```
