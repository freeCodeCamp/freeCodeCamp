...
To calculate a root exponent, such as $\sqrt[n]${x}$, you can use an inverted exponent $x^{1/n}$. Declare a `standardDeviation` variable, and use the `Math.pow()` function to assign it the value of $variance^{1/2}$.
...
--fcc-editable-region--
const getStandardDeviation = (array) => {
  const variance = getStandardDeviation(array);
}
--fcc-editable-region--
