...
```js
const functionRe = __helpers.functionRegex(null, ["acc", "el"], { includeBody: false });
assert.match(getVariance.toString(), __helpers.concatRegex(/sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(\s*/, functionRe));
```

Your `reduce` callback should return the sum of `acc` and `el`.

```js
const functionRe = __helpers.functionRegex(null, ["acc", "el"], { capture: true });
const match = getVariance.toString().match(__helpers.concatRegex(/sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(\s*/,functionRe));
assert.match(__helpers.removeWhiteSpace(match[1]), /\(acc,el\)=>(acc\+el;?|{returnacc\+el;?})|function\(acc,el\){return+acc\+el;?}/);
```
