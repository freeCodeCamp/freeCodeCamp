...
Start by creating an `if` statement. In the condition, create a new `Set()` and pass the `Object.values()` of your `counts` object. If the `size` property of this new `Set` is equal to `1`, that tells you every value appears the same number of times. In this case, return `null` from your function.
