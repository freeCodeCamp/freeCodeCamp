...
If the `array.length` is even, pass an array with the two middle numbers from the sorted array to the `getMean` function. This first item in the array argument should be the value at index `array.length / 2 - 1` from `sorted`, and the second should use the value at index `array.length / 2` from `sorted`.

```js
assert.match(getMedian.toString(), /median\s*=\s*array\.length\s*%\s*2\s*===\s*0\s*\?\s*getMean\(\s*\[sorted\[array\.length\s*\/\s*2\s*-\s*1\]\s*,\s*sorted\[array\.length\s*\/\s*2\]\]\)\s*\:/);
```
