...
Declare a `median` variable. Using the ternary operator, check if the length of `array` is even. You can do this by using the <dfn>modulus operator</dfn>, which is represented by the `%` symbol. This operator returns the remainder of the division of two numbers. If the remainder is `0`, the number is even. If the remainder is `1`, the number is odd.

```js
array.length % 2 === 0;
```

If the array is even, find the two middle numbers and calculate the mean of those numbers. If the array is odd, find the middle number and assign it to the `median` variable.
