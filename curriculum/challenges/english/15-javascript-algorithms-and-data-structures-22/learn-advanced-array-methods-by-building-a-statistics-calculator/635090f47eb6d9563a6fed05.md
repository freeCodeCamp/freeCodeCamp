...
Inside your `getMedian` function, check if the length of `array` is even. If the length of `array` is even, find the middle two numbers, calculate the mean of those numbers and return the result. Otherwise, if the length of `array` is odd, find the middle number and return it.
...
Your `getMedian` function should return the result of the mean of the middle two numbers if the array length is even.
