...
If `array.length` is even, pass an array with its two middle numbers to the `getMean` function.The first item in the array argument should use the value at index `array.length / 2` of `sorted`, and the second should use the value at index `array.length / 2 -1]` of `sorted`.
