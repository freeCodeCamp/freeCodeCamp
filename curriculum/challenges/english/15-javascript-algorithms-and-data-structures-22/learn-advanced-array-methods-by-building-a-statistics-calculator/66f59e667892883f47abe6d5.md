...
Without a code block inside the callback, we have to convert the statements we had into an `arrow function expression`. This is called refactoring. 

In order to start this refactor, we should start by taking over `if/else` statement and using a 
`ternary` operator. 

The `ternary` operator is used like the following. 
