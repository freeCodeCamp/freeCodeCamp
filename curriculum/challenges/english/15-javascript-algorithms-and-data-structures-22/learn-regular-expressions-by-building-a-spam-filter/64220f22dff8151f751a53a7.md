...
A <dfn>capture group</dfn> allows you to define a part of your expression that should be captured and saved for later reference. Capture groups are defined by wrapping a part of your expression in parentheses. For example, `/h(i|ey) camper/` would match either `hi camper` or `hey camper`, and would capture `i` or `ey` in a group.
