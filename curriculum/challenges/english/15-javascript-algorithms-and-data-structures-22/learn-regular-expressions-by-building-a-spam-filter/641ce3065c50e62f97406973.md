...
Regular expressions take <dfn>flags</dfn>, which alter the behavior of the expression. For example, the `i` flag tells the expression to ignore case, so that the expression `/hello/` would match `hello`, `HELLO`, and `Hello`.
