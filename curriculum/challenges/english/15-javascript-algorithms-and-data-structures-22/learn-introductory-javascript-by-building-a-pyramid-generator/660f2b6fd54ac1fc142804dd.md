...
The `function` keyword tells JavaScript that the `name` variable is going to be a function. `parameter` is a variable that represents a value that is passed into the function when it is used. A function may have as many, or as few, <dfn>parameters</dfn> as you'd like. Multiple parameters are separated by commas, like with elements within an `array`. Like a `for` loop, the space between the curly braces is the <dfn>function body</dfn>.
