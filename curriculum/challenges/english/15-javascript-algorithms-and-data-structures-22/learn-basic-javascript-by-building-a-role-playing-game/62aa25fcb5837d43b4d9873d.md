...
Your eighth `locations` value should have a `button text` property and an array value with the strings `2`, `8`, and `Go to town square?`.

```js
assert.deepEqual(locations[7]["button text"], ["2", "8", "Go to town square?"]);
```

Your eighth `locations` value should have a `button functions` property and an array value with the variables `pickTwo`, `pickEight`, and `goTown`.
