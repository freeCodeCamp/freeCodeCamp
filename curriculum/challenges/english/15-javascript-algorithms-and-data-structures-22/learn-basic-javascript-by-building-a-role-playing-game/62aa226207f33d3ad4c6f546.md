...
```js
assert.match(attack.toString(), /text\.innerText\s*\+=\s*"Your\s"/);
```
...
assert.match(attack.toString(), /text\.innerText\s*\+=\s*('|")Your\s\1\s*\+\s*inventory\.pop\(\)/);
```

You should add ` breaks` to the `"Your " + inventory.pop()` string.

```js
assert.match(attack.toString(), /text\.innerText\s*\+=\s*('|")Your\s\1\s*\+\s*inventory\.pop\(\)\s*\+\s*('|")\sbreaks\2/);
```
