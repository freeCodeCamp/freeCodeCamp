...
Use the `+=` operator to add ` Your [weapon] breaks.`, with a single space character in front of `Your`, to the end of `text.innerText`. Replace `[weapon]` with the last item in the `inventory` array using `inventory.pop()`, which will remove the last item in the array AND return it so it appears in your string..
...
You should add `Your` (remember to add two space characters, one before and after it), to `text.innerText`. Remember that spacing matters.
...
You should add `breaks.`(remember to add a single space character in front of it), to the end of your string.
