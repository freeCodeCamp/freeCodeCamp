...
```js
assert.match(defeatMonster.toString(), /Math\.floor\(\s*monsters\s*\[\s*fighting\s*\]\s*\.level\s*\*\s*6\.7\s*\)/);
```

You should add the result of `Math.floor(monsters[fighting].level * 6.7)` to `gold`.

```js
assert.match(defeatMonster.toString(), /gold\s*\+=\s*Math\.floor\(\s*monsters\s*\[\s*fighting\s*\]\s*\.level\s*\*\s*6\.7\s*\)/);
```
