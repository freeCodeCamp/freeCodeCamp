...
Next, set `health` to equal `health` minus the monster's level. Remember you can get this from the `health` property.
...
```js
assert.match(attack.toString(), /health\s*-=\s*monsters\[fighting\]\.health/);
```
