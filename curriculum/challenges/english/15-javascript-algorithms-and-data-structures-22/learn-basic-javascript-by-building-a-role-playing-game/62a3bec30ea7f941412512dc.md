...
You should not redefine `xp`. You need to define `xpText`. 

```js
assert.notMatch(code, /const\s+xp\s+=\s+document/);
```

You should not redefine `health`. You need to define `healthText`. 

```js
assert.notMatch(code, /const\s+health\s+=\s+document/);
```

You should not redefine `gold`. You need to define `goldText`. 

```js
assert.notMatch(code, /const\s+gold\s+=\s+document/);
```
