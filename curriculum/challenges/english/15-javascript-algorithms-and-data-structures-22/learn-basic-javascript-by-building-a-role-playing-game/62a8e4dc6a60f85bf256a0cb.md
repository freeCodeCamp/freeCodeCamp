...
```js
assert.match(attack.toString(), /Math\.floor\(Math\.random\(\) \* xp(?: \+ 1)?\)(?: \+ 1)?/);
```

You should add the result of `Math.floor(Math.random() * xp) + 1` to the result of `weapons[currentWeapon].power`.

```js
assert.match(attack.toString(), /monsterHealth\s*-=\s*weapons\[currentWeapon\]\.power\s*\+\s*Math\.floor\(Math\.random\(\) \* xp(?: \+ 1)?\)(?: \+ 1)?/);
```
