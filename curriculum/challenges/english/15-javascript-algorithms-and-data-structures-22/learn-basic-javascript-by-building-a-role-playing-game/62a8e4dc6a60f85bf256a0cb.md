...
```js
assert.match(attack.toString(), /Math\.floor\(\s*Math\.random\(\s*\)\s*\*\s*xp(?:(?:\s*\+\s*1){1}\s*\)|\s*\)(?:\s*\+\s*1){1})/);
```

You should add the result of `Math.floor(Math.random() * xp) + 1` to the result of `weapons[currentWeapon].power`.

```js
assert.match(attack.toString(), /monsterHealth\s*-=\s*weapons\s*\[\s*currentWeaponIndex\s*\]\s*\.power\s*\+\s*Math\.floor\(\s*Math\.random\(\s*\)\s*\*\s*xp(?:(?:\s*\+\s*1){1}\s*\)|\s*\)(?:\s*\+\s*1){1})/);
```
