...
Your first `span` element should have the text `Monster Name: `.

```js
const spans = document.querySelectorAll(`#monsterStats > span`);
assert.equal(spans[0].innerText.trim(), 'Monster Name:');
```

Your second `span` element should have the text `Health: `.

```js
const spans = document.querySelectorAll(`#monsterStats > span`);
assert.equal(spans[1].innerText.trim(), 'Health:');
```
