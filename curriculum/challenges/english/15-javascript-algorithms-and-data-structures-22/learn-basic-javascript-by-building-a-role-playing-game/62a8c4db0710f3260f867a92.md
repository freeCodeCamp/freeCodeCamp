...
Your `buyHealth` function should have an `if` statement that check if `gold` is greater than or equal to `10`.

```js
assert.match(buyHealth.toString(), /if\s*\(gold\s*>=\s*10\)\s*\{\s*gold\s*-=\s*10;?\s*health\s*\+=\s*10;?\s*goldText\.innerText\s*=\s*gold;?\s*healthText\.innerText\s*=\s*health;?\s*\}/);
```
