...
You should assign correct function name `goCave` reference to `button2.onclick`.

```js
assert.match(code, /button2\.onclick\s*=\s*goCave;/);
```

You should not call the `goCave` function. Remove the parentheses.
...
You should assign correct function name `fightDragon` reference to `button3.onclick`.

```js
assert.match(code, /button3\.onclick\s*=\s*fightDragon;/);
```

You should not call the `fightDragon` function. Remove the parentheses.
