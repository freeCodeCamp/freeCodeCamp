...
Since you subtracted health from the player, you need to check `if` the player's `health` is less than `0`. If it is, call the `lose` function.
...
Your `if` statement should check if the player's `health` is less than `0`.
