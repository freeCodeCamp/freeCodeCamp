...
Your fifth `locations` value should have a `button text` property and an array value with three `Go to town square` strings.

```js
assert.deepEqual(locations[4]["button text"], ['Go to town square', 'Go to town square', 'Go to town square']);
```

Your fifth `locations` value should have a `button functions` property and an array value with three `goTown` variables.
