...
You should set the `innerText` property of `monsterHealthText` to be the `health` value of the current monster. Remember that you can get the current monster with `monsters[fighting]`.

```js
assert.match(goFight.toString(), /monsterHealthText\.innerText\s*=\s*monsters\[fighting\]\.health/);
```
