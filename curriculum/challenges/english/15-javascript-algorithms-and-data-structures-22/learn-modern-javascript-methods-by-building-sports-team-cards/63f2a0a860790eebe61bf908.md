...
Start by adding a case for `"nickname"` inside your `switch` statement. 


# --hints--

You should add a new case for ``"nickname"`` inside your `switch` statement.

```js

assert.match(code, /\s*case\s*('|")\s*nickname\s*('|")\s*:\s*/)
