...
assert.match(code, /\s*case\s*('|")\s*goalkeeper\s*('|")\s*:\s*/)

```

You should call the `setPlayerCards` function and pass in `players.filter()` that filters out players with a position of goalkeeper. 

```js

assert.match(code, /\s*setPlayerCards\s*\(\s*players.filter\s*\(\s*\(?\s*player\s*\)?\s*=>\s*player.position\s*===\s*('|")goalkeeper('|")\)\s*\)\s*/)
