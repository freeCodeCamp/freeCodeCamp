...
You should call the `setPlayerCards` function and pass in `players.filter()` that filters out players without a nickname. 

```js

assert.match(code, /\s*setPlayerCards\s*\(\s*players.filter\s*\(\s*\(?\s*player\s*\)?\s*=>\s*player.nickname\s*!==\s*null\)\s*\)\s*/)
