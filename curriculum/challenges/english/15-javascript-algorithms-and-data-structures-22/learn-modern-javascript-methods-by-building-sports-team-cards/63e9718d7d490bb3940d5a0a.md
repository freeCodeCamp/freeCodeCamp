...
You should use a ternary operator to check if the player has a `nickname`. 

```js

assert.match(code, /\s*<p\s*>\s*Nickname:\s*(\$\{nickname\s*!==\s*null\s*\?\s*nickname\s*:\s*"\N\/A\"})\s*<\/p>\s*/)
