...
assert.match(code, /\s*case\s*('|")\s*midfielder\s*('|")\s*:\s*/)

```

You should call the `setPlayerCards` function and pass in `players.filter()` that filters out players with a position of midfielder. 

```js

assert.match(code, /\s*setPlayerCards\s*\(\s*players.filter\s*\(\s*\(?\s*player\s*\)?\s*=>\s*player.position\s*===\s*('|")midfielder('|")\)\s*\)\s*/)
