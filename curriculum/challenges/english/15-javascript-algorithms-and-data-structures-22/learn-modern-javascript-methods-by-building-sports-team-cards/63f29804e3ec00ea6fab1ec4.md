...
Inside the callback function, add the `innerHTML` method to the `playerCards` element and assign it a value of an empty string. 
...
You should use the `innerHTML` method on the `playerCards` element and assign it an empty string.

```js

assert.match(code, /\s*playerCards.innerHTML\s*=\s*('|")\s*('|")\s*/)
