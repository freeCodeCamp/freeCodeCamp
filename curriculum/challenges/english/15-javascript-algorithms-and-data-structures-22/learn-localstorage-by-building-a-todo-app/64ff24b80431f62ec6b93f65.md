...
```js
assert.notMatch(code, /const\s+myTaskArr\s*=\s*\[\s*\{\s*task:\s"Walk\s*the\s*Dog",\s*date:\s*"22-04-2022"\s*\},\s*\{\s*task:\s"Read\s*some\s*books",\s*date:\s*"02-11-2023"\s*\},\s*\{\s*task:\s"Watch\s*football",\s*date:\s*"10-08-2021"\s*\},\s*\]\s*;?\s*localStorage\.setItem\("data", JSON\.stringify\(myTaskArr\)\);\s*localStorage\.clear\(\);?\s*const\s+getTaskArr\s*=\s*localStorage\.getItem\("data"\)\s*console\.log\(getTaskArr\)\s*const\s+getTaskArrObj\s*=\s*JSON\.parse\(localStorage\.getItem\("data"\)\);?\s*console\.log\(getTaskArrObj\);?/)
```
