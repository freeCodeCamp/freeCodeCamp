...
**N.B:** `showModal()` is a method associated with the HTML `dialog` element. It is used to display a modal dialog box on a web page.
...
Your event listener should use arrow syntax to set the `showModal()` method on `confirmCloseDialog`. Don't use curly braces.

```js
assert.match(code, /closeTaskFormBtn\.addEventListener\(('|"|`)click\1\,\s*\(\)\s*\s*=>\s*confirmCloseDialog\.showModal\(\)\s*\);?/)
```
