...
At top of the `addOrUpdateTask` function just update the `innerText` of the `addOrUpdateTaskBtn` to `Add Task`.

```js
assert.match(code, /addOrUpdateTaskBtn\.innerText\s*=\s*"Add Task"\s*/)
```
