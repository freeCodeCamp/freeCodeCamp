...
Using arrow syntax, use addition assignment to set the `innerHTML` of `tasksContainer` to empty backticks.
...
```js
assert.match(code, /taskData\.forEach\(\(\{.*\}\)\s*=>\s*(\s*\{\(\s*tasksContainer\.innerHTML\s*\)\s*\})?/)
```

You should use addition assignment to set the `innerHTML` of `tasksContainer` to an empty pair of backticks.

```js
assert.match(code, /taskData\.forEach\(\(\{.*\}\)\s*=>\s*\{\s*(\s*\(?tasksContainer\.innerHTML\s*\+=\s*`\s*`\)?\s*\})/)
```
