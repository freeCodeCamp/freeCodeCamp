...
```js
assert.match(code, /taskData\.forEach\(\s*\(\s*\{\s*.*\}\)\s*=>\s*\(?\s*tasksContainer\.innerHTML/)
```

You should use addition assignment to set the `innerHTML` of `tasksContainer` to an empty pair of backticks.

```js
assert.match(code, /taskData\.forEach\(\s*\(\s*\{\s*.*\}\)\s*=>\s*\(?\s*tasksContainer\.innerHTML\s*\+=\s*`\s*`\)?\s*\);?/)
```
