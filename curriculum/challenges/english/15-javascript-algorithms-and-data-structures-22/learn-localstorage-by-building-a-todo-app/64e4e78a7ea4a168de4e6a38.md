...
In earlier projects, you learned how to add and remove classes to an element with `el.classList.add()` and `el.classList.remove()`. Another method to use with the `classList` property is the toggle method.
...
```js
assert.match(code, /openTaskFormBtn\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\)\s*=>\s*(taskForm\s*\.classList\s*\.\s*toggle\s*\(\s*('|"|`)hidden\2\s*\)\s*;?)?\s*\);?/)
```
