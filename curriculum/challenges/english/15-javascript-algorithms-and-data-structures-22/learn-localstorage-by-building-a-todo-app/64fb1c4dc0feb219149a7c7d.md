...
```js
assert.match(code, /const\s+formInputValuesUpdated\s*=\s*(?:(?:titleInput\.value\s*(!==|!=)\s*currentTask\.title));?/)
```
...
```js
assert.match(code, /const\s+formInputValuesUpdated\s*=\s*(?:(?:titleInput\.value\s*(!==|!=)\s*currentTask\.title)|(?:dateInput\.value\s*(!==|!=)\s*currentTask\.date));?/
)
```
