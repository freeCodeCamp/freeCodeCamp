...
```js
const regex = /const\s+formInputValuesUpdated\s*=(?=.*titleInput\.value\s*(!==|!=)\s*currentTask\.title)(?=.*dateInput\.value\s*(?:!==|!=)\s*currentTask\.date)(?=.*descriptionInput\.value\s*(?:!==|!=)\s*currentTask\.description);?/
