...
```js
assert.match(code, /const\s+editTask\s+=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)\s*=>\s*(item\.id\s*===\s*buttonEl\.parentElement\.id|buttonEl\.partentElement\.id\s*===\s*item\.id)\s*\)\s*;?\s*\}\s*;?/)
```
