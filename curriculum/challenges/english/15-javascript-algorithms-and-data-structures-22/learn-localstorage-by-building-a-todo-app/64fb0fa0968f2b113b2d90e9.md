...
```js
assert.match(code, /const\s+editTask\s*=\s*\(buttonEl\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(/)
```

You should pass in `item` as the parameter of the `findIndex()` arrow function callback. Don't use curly braces.

```js
assert.match(code, /const\s+editTask\s+=\s*\(buttonEl\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)/)
```

Your arrow function callback should check if `item.id === buttonEl.parentElement.id`.

```js
assert.match(code, /const\s+editTask\s+=\s*\(buttonEl\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)\s*=>\s*(item\.id\s*===\s*buttonEl\.parentElement\.id|buttonEl\.partentElement\.id\s*===\s*item\.id\s*)\s*\);?\s*\};?/)
```
