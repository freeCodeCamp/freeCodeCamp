...
When the new user creates a task, it should be saved in an object. 
...
You should have a variable called `taskObj`.

```js
assert.isDefined(taskObj);
```

You should use `const` to create your `taskObj`.

```js
assert.match(code, /const\+taskObj\s*=/);
```

You should assign an empty object to your `taskObj` variable.

```js
assert.deepEqual(taskObj, {});
```
