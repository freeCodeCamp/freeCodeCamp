...
```js
assert.match(code, /const\s*deleteTask\s*=\s*\(buttonEl\)\s*=>\s*\{\s*const dataArrIndex\s*=\s*taskData\.findIndex\(\s*\(item\)\s*=>\s*item.id\s*===\s*buttonEl\.parentElement\.id\s*\);?\s*\};?/)
```
