...
```js
assert.match(code, /const\s+deleteTask\s*=\s*\(buttonEl\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*\(?item\)?/)
```

Your arrow function callback should check if `item.id === buttonEl.parentElement.id`. Don't use curly braces.

```js
assert.match(code, /const\s+deleteTask\s*=\s*\(buttonEl\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*\(?\s*item\s*\)?\s*=>\s*item\.id\s*===\s*buttonEl\.parentElement\.id\s*\);?\s*\};?/)
```
