...
```js
assert.match(code, /const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)\s*=>\s*((?:item\.id\s*===\s*currentTask\.id|currentTask\.id\s*===\s*item\.id)\s*\)|(\s*{\s*return\s*(?:item\.id\s*===\s*currentTask\.id|currentTask\.id\s*===\s*item\.id);?\s*}\)))\s*;?/)
```
