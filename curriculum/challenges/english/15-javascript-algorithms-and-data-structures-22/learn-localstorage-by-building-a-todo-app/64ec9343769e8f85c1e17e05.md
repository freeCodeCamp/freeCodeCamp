...
```js
const classFirst = `<div\s+class\s*=\s*('|")task\1`;
const idFirst = `<div\s+id\s*=\s*('|")task\2\s+class\s*=\s*('|")task\3`
assert.match(code, new Regex(`${classFirst}|${idFirst}`));
```

Your `div` element should have the `id` `${id}`.

```js
const classFirst = `<div\s+class\s*=\s*('|")task\1\s*id\s*=\s*('|")\$\{id\}\2\s*>\s*<\/div>`; 
const idFirst = `<div\s+id\=\s*('|")\$\{id\}\3\s+class\s*=\s*('|")task\4\s*>\s*<\/div>`;
assert.match(code, new Regex(`${classFirst}|${idFirst}`));
```
