...
```js
setPlayerCards(myFavoriteFootballTeam.players)
const cards = document.querySelectorAll('.player-card > h2');
const players = myFavoriteFootballTeam.players
for (const i in players) {
  const playerNameText = cards[i].innerHTML;
  const regex = players[i].isCaptain ? new RegExp("\\s*\\(Captain\\)\\s*" + players[i].name + "\\s*"):
              new RegExp("\\s*" + players[i].name + "\\s*");
  assert.match(playerNameText, regex)
}
```
