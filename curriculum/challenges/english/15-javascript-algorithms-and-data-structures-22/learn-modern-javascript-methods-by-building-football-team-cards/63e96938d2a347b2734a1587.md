...
```js
const player = { name: "Name", isCaptain: true, position: 'goalkeeper', number: 4 };

playerCards.innerHTML = "";
setPlayerCards([player]);

assert.ok(playerCards.innerHTML.includes(`<p>Number: ${player.number}</p>`));
```
...
assert.equal(playerCards.querySelector(".player-card p:nth-child(2)").innerText, `Number: ${player.number}`);
```
