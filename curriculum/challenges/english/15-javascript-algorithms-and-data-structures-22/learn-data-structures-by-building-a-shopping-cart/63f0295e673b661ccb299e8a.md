...
Add a `getCount` method to the `ShoppingCart` class. It should return the number of items in the `items` array.

# --hints--

Your `ShoppingCart` class should have a `getCount` method.
...
```js
const afterCounts = code.split('getCount')[1];
assert.match(afterCounts, /this\s*\.\s*items\s*\.\s*length/);
```

Your `getCount` method should return the number of items in the `items` array.
