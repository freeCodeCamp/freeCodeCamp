...
To start, you will need to get some of your elements from the DOM. Start by using `document.getElementById()` to get the`#cart-container`, `#products-container`, and `#dessert-card-container` elements. Store them in variables named `cartContainer`, `productsContainer`, and `dessertCardContainer`, respectively.
...
You should assign the `#dessert-card-container` element to a variable named `dessertCardContainer`. Remember to use `const` to declare the variable.

```js
assert.match(code, /const\s+dessertCardContainer\s*=\s*document\.getElementById\(\s*('|"|`)dessert-card-container\1\s*\)/);
```
