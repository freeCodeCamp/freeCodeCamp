...
After your `h2` element, create two `p` elements. Give the first a `class` of `dessert-price`, and the text of the `price` variable with a dollar sign in front of it. Give the second a `class` of `product-category`, and the text `Category: ` followed by the value of the `description` variable.
...
assert.equal(document.querySelector('.dessert-card p')?.length, 2)
```
