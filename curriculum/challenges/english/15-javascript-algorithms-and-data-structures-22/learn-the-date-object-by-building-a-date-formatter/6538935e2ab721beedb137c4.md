...
The `.join()` and `.split()` methods should use a `"-"` as a separator.  Make sure to use `.reverse()` at the end.

```js
assert(code.match(/\.join\(\s*('|")-\1\s*\)/g));
```

The `.split()` method should use a `"-"` as a separator.

```js
assert(code.match(/\.split\(\s*('|")-\1\s*\)/g));
```

Use the `.reverse()` method at the end.
