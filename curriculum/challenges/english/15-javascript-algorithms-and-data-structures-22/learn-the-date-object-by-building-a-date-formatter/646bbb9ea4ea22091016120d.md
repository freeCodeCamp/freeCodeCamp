...
Next, create a constant named `formattedDate` and assign it empty template literals.
...
```js
assert(code.match(/const\s+formattedDate\s+=\s+``/g));
```
