...
```js
assert.notMatch(code, /const\s+exampleSentence\s*=\s*['"]freeCodeCamp rules['"]\s*\.split\(['"]['"]\)\s*\.reverse\(\)\s*\.join\(['"]['"]\)/);
```
...
--fcc-editable-region--
const exampleSentence = "freeCodeCamp rules".split("").reverse().join("");
