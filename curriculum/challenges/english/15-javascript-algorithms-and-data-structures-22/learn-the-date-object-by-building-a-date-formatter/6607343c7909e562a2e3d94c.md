...
The `split()` method is used to take a string and return a new array of substrings. 
...
Create a new `const` variable called `exampleSentence` and assign it the result of `"freeCodeCamp rules".split()`. 
...
You should assign `"freeCodeCamp rules".split()` to your `exampleSentence` variable.

```js
assert.match(code, /const\s+exampleSentence\s*=\s*('|"|`)freeCodeCamp\s+rules\1\.split\(\)/);
```
