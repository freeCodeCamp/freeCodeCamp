...
Create a class named `support` and assign the `background-color` property to `var(--light-orange)`.

# --hints--

You should set the `background-color` to `var(--light-orange)` in the `support` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.support')?.getPropertyValue('background-color');
assert.equal(bgc, 'var(--light-orange)');
```
