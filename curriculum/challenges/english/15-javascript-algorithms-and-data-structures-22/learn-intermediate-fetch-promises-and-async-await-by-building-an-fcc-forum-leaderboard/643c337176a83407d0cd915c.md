...
```js
assert.match(code, /const\s*fetchData\s*=\s*async\(\)\s*=>\s*{\s*try{\s*const\s*res\s*=\s*await\s*fetch(.+);?\s*const\s*data\s*=\s*await\s*res\.json\(\);?\s*console\.log\(data\)\s*}\s*catch\(err\)\s*{\s*}\s*}/);
```
