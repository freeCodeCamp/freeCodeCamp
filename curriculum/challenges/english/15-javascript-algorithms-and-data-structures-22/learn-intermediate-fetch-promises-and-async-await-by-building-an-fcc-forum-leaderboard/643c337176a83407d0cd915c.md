...
To see the results, you will console log the `data` variable. The `data` variable is the JSON object that you converted from the response from the API.

# --hints--

You should console log the `data` variable after the `data` variable is assigned.

```js
assert.match(code, /async\s*function\s*fetchData\(\)\s*{\s*let\s*res\s*=\s*await\s*fetch\(.+\);\s*let\s*data\s*=\s*await\s*res\.json\(\);\s*console\.log\(data\)/);
```
