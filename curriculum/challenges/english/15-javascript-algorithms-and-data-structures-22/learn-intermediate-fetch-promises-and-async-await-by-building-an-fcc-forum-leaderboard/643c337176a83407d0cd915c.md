...
Add a `console.log` statement to check the value of `data`. 

# --hints--

You should use `console.log` to log the `data` variable after the `data` variable is assigned.

```js
assert.match(code, /const\s*fetchData\s*=\s*async\s*\(\)\s*=>\s*{\s*try\s*{\s*const\s*res\s*=\s*await\s*fetch(.+);?\s*const\s*data\s*=\s*await\s*res\.json\(\);?\s*console\.log\(data\)\s*}\s*catch\s*\(\s*err\s*\)\s*{\s*}\s*}/);
```
