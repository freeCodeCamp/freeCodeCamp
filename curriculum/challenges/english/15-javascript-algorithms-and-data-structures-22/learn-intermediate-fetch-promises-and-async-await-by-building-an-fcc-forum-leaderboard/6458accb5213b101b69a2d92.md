...
Create a class named `html-css` and assign the `background-color` property to `var(--light-blue)`.

# --hints--

You should set the `background-color` to `var(--light-blue)` in the `html-css` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.html-css')?.getPropertyValue('background-color');
