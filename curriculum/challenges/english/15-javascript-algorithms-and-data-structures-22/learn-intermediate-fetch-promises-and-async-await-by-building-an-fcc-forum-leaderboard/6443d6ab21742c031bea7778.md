...
You will write another `if` statement inside the `showTime` function. This time it will check whether the constant `hoursAgo` is less than 24. If it is, then it will `return` the template literal saying `${hoursAgo}h ago`. What it does is that it will return the number of hours ago the post was created.

# --hints--

You should create an `if` statement that checks whether the constant `hoursAgo` is less than 24.

```js
assert(code.match(/if\s*\(\s*hoursAgo\s*<\s*24\s*\)/));
```

You should use a `return` statement inside the if statement.

```js
assert(code.match(/if\s*\(\s*hoursAgo\s*<\s*24\s*\)\s*{\s*return\s*.*/));
```
...
Inside the template literal you should write `${hoursAgo}h ago`.

```js
assert(code.match(/return\s*`\s*\${hoursAgo}\s*h\s*ago\s*`\s*/));
```
