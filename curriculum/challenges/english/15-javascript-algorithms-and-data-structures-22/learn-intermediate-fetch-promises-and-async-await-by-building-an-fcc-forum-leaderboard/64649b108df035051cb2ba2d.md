...
On the HTML `a` tag, set the `target` attribute to `_blank` to open the link in a new tab.

# --hints--

You should set the `target` attribute to `_blank` on the `a` tag after the class.

```js
const regex = /<a\s+(?:class=['"]post-title['"]\s+target=['"]_blank['"]|target=['"]_blank['"]\s+class=['"]post-title['"])(?:.|\n)*?<\/a>/;
