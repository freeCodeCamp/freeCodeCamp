...
```js
assert.match(code, /(var|let|const)\s*data/);
```

You should assign `await res.json()` to the `data` variable.

```js
assert.match(code, /const\s*fetchData\s*=\s*async\(\)\s*=>\s*{\s*try{\s*const\s*res\s*=\s*await\s*fetch(.+);?\s*(var|let|const)\s*data\s*=\s*await\s*res\.json\(\);?\s*}\s*catch\(err\)\s*{\s*}\s*}/);
```
