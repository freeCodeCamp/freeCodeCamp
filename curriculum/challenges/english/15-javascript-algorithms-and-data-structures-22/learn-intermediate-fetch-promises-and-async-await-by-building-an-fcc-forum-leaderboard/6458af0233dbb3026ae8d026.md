...
Create a class named `javascript` and assign the `background-color` property to `var(--golden-yellow)`.

# --hints--

You should set the `background-color` to `var(--golden-yellow)` in the `javascript` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.javascript')?.getPropertyValue('background-color');
assert.equal(bgc, 'var(--golden-yellow)');
```
