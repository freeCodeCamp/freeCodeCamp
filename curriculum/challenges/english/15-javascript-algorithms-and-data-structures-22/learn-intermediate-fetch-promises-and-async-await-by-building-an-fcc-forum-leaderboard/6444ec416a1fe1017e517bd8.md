...
Create a constant named `thousands` and assign `Math.floor(views / 1000)` to it. This will give you the number of thousands in the `views` variable and round down to the nearest thousand.

# --hints--

You need to create a constant named `thousands`.

```js
assert.match(code, /\s*const\s*thousands\s*=/);
```

`thousands` should be a constant.

```js
assert.match(code, /\s*const\s*thousands\s*=/);
```

`thousands` should be assigned to `Math.floor(views / 1000)`.
