...
Finally on the `else` statement, using the dot notation, you will set the `id` property of the `selectedCategory` object to the number `1`.

# --hints--

You should use the dot notation to target the `id` property of the `selectedCategory` object.

```js
assert(code.match(/selectedCategory\.id/g));
```

You should assign the number `1` to the `id` property of the `selectedCategory` object.

```js
const regex = /selectedCategory\.id\s*=\s*1/g;
assert(code.match(regex));
```

Please don't use single or double quotes around the number `1`.
