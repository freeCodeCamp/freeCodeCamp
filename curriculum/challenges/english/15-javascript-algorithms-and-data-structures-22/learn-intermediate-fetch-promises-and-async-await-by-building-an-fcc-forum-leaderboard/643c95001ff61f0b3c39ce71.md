...
You should use a `const` variable to destructure the `id`, `title`, `views`, `posts_count`, `slug`, `posters`, `category_id`, and `bumped_at` properties of the `item` object. The properties can be in any order.
