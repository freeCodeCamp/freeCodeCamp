...
You need an image tag to use all the constants you created in the past couple of steps, but first use the `return` keyword to return an empty template literal.

# --hints--

Please don't use single or double quotes after the `return` keyword.

```js
const noQuotesAfterReturn = /return\s+['"]/g;
assert(!code.match(noQuotesAfterReturn));
```

You should return an empty template literal

```js
const emptyTemplateLiteral = /return\s+`(\s*)`/g;
