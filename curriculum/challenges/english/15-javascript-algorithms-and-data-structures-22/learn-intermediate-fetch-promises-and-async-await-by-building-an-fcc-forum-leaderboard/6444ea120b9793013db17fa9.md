...
In the function named `countingViews`, pass the argument named `views`, so that the function can use it.

# --hints--

You should pass the argument named `views` to the function named `countingViews`.

```js
assert.match(code, /\s*function\s*countingViews\s*\(\s*views\s*\)\s*{\s*}/);
```
