...
To get the current time, you can use the `Date` object. It returns the current date and time as a string. Here is an example:

```js
const now = new Date();
console.log(now); // Sat Apr 03 2021 15:24:51 GMT+0800 (Singapore Standard Time)
```

In this step, create a constant named `currentTime` inside the `showtime` function and assign `new Date()` to it.

# --hints--

Your code should create a constant named `currentTime` inside the `showTime` function and then assign `new Date()` to it.

```js
assert.match(code, /const\s+currentTime\s*=\s*new\s+Date\s*\(\s*\)\s*/);
```
