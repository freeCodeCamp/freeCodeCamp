...
While staying inside the function named `countingViews` you will create a if statement where it will check whether the constant `thousands` is more than or equal to 1000. A more than or equal to operator is represented by `>=`. If it is, then it will return the template literal saying `${thousands}k`. What it does is that it will return the number of thousands of views the post has. If it has 10000 views, it will return 10k.

# --hints--

You should create an `if` statement that checks whether the constant `thousands` is more than or equal to 1000.

```js
assert(code.match(/if\s*\(\s*thousands\s*>=\s*1000\s*\)/));
```

You should use a `return` statement inside the if statement.

```js
assert(code.match(/if\s*\(\s*thousands\s*>=\s*1000\s*\)\s*{\s*return\s*.*/));
```
