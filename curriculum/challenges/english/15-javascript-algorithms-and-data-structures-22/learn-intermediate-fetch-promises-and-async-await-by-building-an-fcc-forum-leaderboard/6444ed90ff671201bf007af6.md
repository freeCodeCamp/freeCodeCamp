...
While staying inside the function named `countingViews` you will create a if statement where it will check whether the argument `views` is more than or equal to 1000. A more than or equal to operator is represented by `>=`. If it is, then it will return the template literal saying `${thousands}k`. What it does is that it will return the number of thousands of views the post has. As an example, If it has 10000 views, it will return 10k.
