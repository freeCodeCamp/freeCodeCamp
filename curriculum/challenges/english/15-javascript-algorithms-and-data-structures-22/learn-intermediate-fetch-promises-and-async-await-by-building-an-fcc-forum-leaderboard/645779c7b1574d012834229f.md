...
As you can see that the color of the text has been changed but there is a underline, which is not required. You will set the property of `text-decoration` to `none` in the class named `category`, then set the property of the `display` to `block`
...
You should add the `display` of `block` to the `category` class name. Don't forget to add semicolon `;` at the end of the line.
