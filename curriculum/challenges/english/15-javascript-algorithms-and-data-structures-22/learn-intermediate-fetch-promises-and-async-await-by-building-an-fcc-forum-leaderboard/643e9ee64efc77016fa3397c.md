...
Add 5 table data `td` elements inside the `tr` element of the template literal.

# --hints--

Your code should contain 5 `td` elements inside the `tr` element of the template literal.

```js
assert(/return `\s*<tr>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```
