...
Now, you need to set the `href` attribute of the `a` tag to `${FORUM_TOPIC_URL}${slug}/${id}`. This will make the link point to the correct forum topic.

# --hints--

You should set the `href` attribute of the `a` tag to `${FORUM_TOPIC_URL}${slug}/${id}` after the target attribute.

```js
const regex = /<a\s+class=['"]post-title['"]\s+target=['"]_blank['"]\s+href=['"]\$\{FORUM_TOPIC_URL\}\$\{slug\}\/\$\{id\}['"]\s*>([\s\S]*?)<\/a>/;
...
function showLatestPosts(data){
  ...
  topics.forEach((item)=>{
    ...
    <tr>
      <td>
--fcc-editable-region--
        <a class="post-title" target="_blank">${title}</a>
--fcc-editable-region--
