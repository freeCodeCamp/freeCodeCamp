...
You will add the `return` keyword first, then apply the `map` method to the `posters` array. Then, add a callback function to the `map` method that takes a parameter named `poster`.

# --hints--

You should apply the `map` method to the `posters` array, then add a callback function that takes a parameter named `poster`.

```js
assert.match(code, /\s*return\s+posters\s*\.\s*map\s*\(\s*function\s*\(\s*poster\s*\)\s*{\s*}/);
```
