...
You will add the `return` keyword first, then apply the `map` method to the `posters` array. Then, add an ES6 arrow function callback to the `map` method that takes a parameter named `poster`.

# --hints--

You should apply the `map` method to the `posters` array, then add an ES6 arrow function callback that takes a parameter named `poster`.
