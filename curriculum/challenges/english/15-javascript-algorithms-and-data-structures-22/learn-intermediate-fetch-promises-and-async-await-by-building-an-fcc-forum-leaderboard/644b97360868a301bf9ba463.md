...
Inside the `if` statement you will destructure `className` and `category` from the `categorieList` object using the `id` as the key.

# --hints--

You should use a `const` keyword to destructure `className` and `category` from the `categorieList[id]` object using the `id` as the key. The properties can be in any order.

```js
assert(code.match(/const\s*\{\s*(className|category)\s*,\s*(className|category)\s*\}\s*=\s*categorieList\[id\]\s*/g));
```
