...
As you can see that the category names has been added under the post title, but its in the default color of blue due to the `a` tag. You will target the `category` class name in CSS, add `padding` property of `3px` and change the `color` property to `var(--black)`.
