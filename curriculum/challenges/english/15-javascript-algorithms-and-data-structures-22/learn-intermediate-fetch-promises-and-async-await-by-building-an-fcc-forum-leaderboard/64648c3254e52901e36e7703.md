...
You will trigger the function named `displayUsers` and pass 2 arguments `posters` and `users`

# --hints--

You should call the function named `displayUsers` wrapped in curly braces with a dollar sign in front of it

```js
const displayUsersTest = /\$\{[\s]*displayUsers[\s]*\([\s]*[^)]*[\s]*\)[\s]*\}/gm;
assert(code.match(displayUsersTest));
```

You should call 2 arguments named `posters` and `users` in the function named `displayUsers`

```js
const displayUsersTest = /\$\{[\s]*displayUsers[\s]*\([\s]*posters[\s]*,[\s]*users[\s]*\)[\s]*\}/gm;
assert(code.match(displayUsersTest));
```
