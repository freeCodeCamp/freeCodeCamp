...
You need a function to convert view counts to a more readable format. For example, if the view count is 1000, it should be converted to 1k. If the view count is 100,000 then it will be 100k. For that, create a function named `countingViews`

# --hints--

You need to create a es6 arrow function named `countingViews`

```js
assert.match(code, /\s*const\s*countingViews\s*=\s*\(\s*\)\s*=>\s*{\s*}/);
```
...
```js
assert.match(code, /\s*const\s*countingViews\s*=\s*\(\s*\)\s*=>\s*{\s*}\s*/);
```
