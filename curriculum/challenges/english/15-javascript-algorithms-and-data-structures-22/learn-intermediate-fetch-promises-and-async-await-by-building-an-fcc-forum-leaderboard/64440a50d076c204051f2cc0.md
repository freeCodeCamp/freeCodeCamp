...
You need a function that will be used to convert view counts to a more readable format. For example, if the view count is 1000, it should be converted to 1k. If the view count is 100,000 then it will be 100k. For that, create a function named `countingViews`

# --hints--

You need to create a function named `countingViews`

```js
assert.match(code, /\s*function\s*countingViews()/);
```

`countingViews` should be defined.

```js
assert.isDefined(countingViews);
```

`countingViews` should be a function.

```js
assert.isFunction(countingViews);
```

`countingViews` should be an empty function.

```js
assert.match(code, /\s*function\s*countingViews\s*\(\s*\)\s*{\s*}/);
```
