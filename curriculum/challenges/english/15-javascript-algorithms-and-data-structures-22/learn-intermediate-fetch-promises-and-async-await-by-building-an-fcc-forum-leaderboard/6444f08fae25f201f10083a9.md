...
Finally you will add the `return` statement of this function named `countingViews` which will help to show the amount of views less than 1000. First, write the `return` statement and then return the `views` variable.
...
You should not use single, double quotes or backticks after the `return` statement, you should return the `views` variable.

```js
assert(!code.match(/.*};\s*return\s*['"`]/));
```

You should return the `views` variable.

```js
assert(code.match(/return\s*views\s*/));
```
