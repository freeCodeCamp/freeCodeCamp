...
You will create a constant named `daysAgo` that will store the number of days that have passed since the last post. You can do this by dividing `hoursAgo` by `24`, and then rounding down to the nearest whole number. Do it like this :

```js
const daysAgo = Math.floor(hoursAgo / 24); // 1 day = 24 hours
```

# --hints--

You should declare a constant named `daysAgo` and assign it `Math.floor(hoursAgo / 24)`.

```js
assert.match(code, /const\s+daysAgo\s*=\s*Math\.floor\s*\(\s*hoursAgo\s*\/\s*24\s*\)\s*/);
```
