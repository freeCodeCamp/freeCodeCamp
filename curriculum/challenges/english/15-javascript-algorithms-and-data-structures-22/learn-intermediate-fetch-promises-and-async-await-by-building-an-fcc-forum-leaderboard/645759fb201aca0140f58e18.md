...
Lastly, in between the anchor tags, you need to use the constant `linkText` to display the text in the link.

# --hints--

You should use the constant `linkText` between the anchor tags to display the text in the link.

```js
const linkTextValue = /return\s+`<a[^>]*href\s*=\s*['"]\s*\${url}\s*['"][^>]*class\s*=\s*['"]\s*\${linkClass}\s*['"][^>]*target\s*=\s*['"]\s*\${linkTarget}\s*['"][^>]*>\s*\${linkText}\s*<\/a>`/g;
...
--fcc-editable-region--
return `<a href="${url}" class="${linkClass}" target="${linkTarget}"></a>`
--fcc-editable-region--
}
