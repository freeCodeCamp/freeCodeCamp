...
Pass the `id` argument to the `forumCategories` function. This argument will be used to get the category name and the corresponding class name from the object `categoryList`  

# --hints--

You should pass a argument named `id` in the function named `forumCategories`

```js
assert.match(code, /\s*const\s+forumCategories\s*=\s*\(\s*id\s*\)\s*=>\s*{\s*}/);
```
