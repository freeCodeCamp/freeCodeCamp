...
In the function named `displayUsers`, pass 2 arguments named `posters` and `users`, so that the function can use it.

# --hints--

You should pass the arguments named `posters` and `users` to the function named `displayUsers`.

```js
assert.match(code, /\s*function\s*displayUsers\s*\(\s*posters,\s*users\s*\)\s*{\s*}/);
```
