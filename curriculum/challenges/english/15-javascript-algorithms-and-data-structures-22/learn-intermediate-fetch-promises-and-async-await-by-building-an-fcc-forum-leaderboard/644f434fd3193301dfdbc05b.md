...
Users should be able to click on any category, and it will direct them to the category page on the freeCodeCamp forum. 

Start by creating a constant named `url` and assign it a template literal with an embedded expression of `${FORUM_CATEGORY_URL}`.
...
Your template literal should have an embedded expression of `${FORUM_CATEGORY_URL}`.
