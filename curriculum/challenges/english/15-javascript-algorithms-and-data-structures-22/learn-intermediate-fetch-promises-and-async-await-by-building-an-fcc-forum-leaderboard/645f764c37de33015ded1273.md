...
You can create a constant called `avatar` and set it to the `user` object's `avatar_template`. This `avatar_template` is a string that has a special placeholder `{size}` in it. 

To customize the avatar's size, you can replace this `{size}` with the value `30`. This way, you can control the size of the user's avatar in your application.

# --hints--

Replace the `{size}` placeholder, in the `avatar_template` property of `user` object, with `30`. Then assign it to the `avatar` constant.
