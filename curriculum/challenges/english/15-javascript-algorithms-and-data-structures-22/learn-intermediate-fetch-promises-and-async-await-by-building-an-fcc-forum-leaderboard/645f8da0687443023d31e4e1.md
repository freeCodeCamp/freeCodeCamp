...
You need to set the `src` attribute of the image tag to the constant named `avatarUrl`. This will make the image tag display the image from the specified URL Address.

# --hints--

You should write the `src` attribute inside the image tag

```js
const imgTagWithSrc = /return\s+`<img[^>]*src\s*=\s*['"][^'"]*['"][^>]*\/>`/g;
assert(code.match(imgTagWithSrc));
```

You should set the value for the `src` attribute to the constant named `avatarUrl`.

```js
const srcValue = /return\s+`<img[^>]*src\s*=\s*['"]\s*\${avatarUrl}\s*['"][^>]*\/>`/g;
