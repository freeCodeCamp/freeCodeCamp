...
Create a class named `feedback` and assign the `background-color` property to `var(--light-blue)`.

# --hints--

You should set the `background-color` to `var(--light-blue)` in the `feedback` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.feedback')?.getPropertyValue('background-color');
assert.equal(bgc, 'var(--light-blue)');
```
