...
`forEach` is an array method in JavaScript that allows you to iterate over each element of an array and perform some operation on each element. Here's an example:
...
In this step, you will use the `forEach` method on the `topics` array, Like this 
...
You should use the `forEach` method on the `topics` array.

```js
assert(code.match(/topic_list;\s*topics\.forEach\(/g));
```
