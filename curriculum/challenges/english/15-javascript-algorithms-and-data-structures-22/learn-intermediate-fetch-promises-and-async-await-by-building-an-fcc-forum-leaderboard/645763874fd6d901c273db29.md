...
Now, you will use the function named `forumCategories` with the argument called `category_id` inside the function named `showLatestPosts`, after the post title. This will help to show the category of the post.

# --hints--

You should call the function `forumCategories` with the argument `category_id` using a dollar sign and curly braces `${}`.

```js
const forumCategoriesValue = /\s*\${\s*forumCategories\(\s*category_id\s*\)\s*}/g;
assert(code.match(forumCategoriesValue));
```
