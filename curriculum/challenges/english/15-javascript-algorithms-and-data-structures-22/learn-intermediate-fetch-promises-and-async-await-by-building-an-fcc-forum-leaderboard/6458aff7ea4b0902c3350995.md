...
Create a class named `backend` and assign the `background-color` property to `var(--gold)`.

# --hints--

You should set the `background-color` to `var(--gold)` in the `backend` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.backend')?.getPropertyValue('background-color');
assert.equal(bgc, 'var(--gold)');
```
