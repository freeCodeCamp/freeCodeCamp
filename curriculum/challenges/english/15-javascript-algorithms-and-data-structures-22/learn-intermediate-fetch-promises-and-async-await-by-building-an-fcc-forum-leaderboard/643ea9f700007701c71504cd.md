...
Inside the `p` tag, add the `title` variable using curly braces with a dollar sign in front of it. This will display the title of the post

# --hints--

Your code should add the `title` variable inside the `p` tag using curly braces with a dollar sign in front of it

```js
assert(/postsContainer\.innerHTML \+= `\s*<tr>\s*<td>\s*<p>\s*\${title}\s*<\/p>\s*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```
