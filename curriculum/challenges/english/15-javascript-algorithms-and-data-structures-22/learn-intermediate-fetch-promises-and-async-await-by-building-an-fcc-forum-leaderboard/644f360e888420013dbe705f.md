...
Your code will run the `else` statement only when the `id` is not in the `categoryList` object. You need to use the dot notation to set the `className` property of the `selectedCategory` object to `General`.
...
You should assign the word `General` to the `className` property of the `selectedCategory` object.

```js
const regex = /selectedCategory\.className\s*=\s*['"]General['"]/g;
