...
Your code will run the `else` statement only when the `id` is not in the `allCategories` object. You need to use the dot notation to set the `className` property of the `selectedCategory` object to `General`.
