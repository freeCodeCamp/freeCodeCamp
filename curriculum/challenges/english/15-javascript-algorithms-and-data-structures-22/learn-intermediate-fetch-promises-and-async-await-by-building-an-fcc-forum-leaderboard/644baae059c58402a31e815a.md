...
Incase the `id` is not in the `categorieList` object, you need to handle this case first by creating an `else` statement.

# --hints--

You need to create an else statement

```js
assert(code.match(/else/));
```

You need to keep the else statement empty

```js
assert(code.match(/else\s*{\s*}/));
```
