...
In the function named `showTime`, pass the argument named `time`, so that the function can use it.

# --hints--

You should pass the argument named `time` to the function named `showTime`.

```js
assert.match(code, /\s*function\s*showTime\s*\(\s*time\s*\)\s*{\s*}/);
```
