...
Create a class named `General` and assign the `background-color` property to `var(--light-green)`.

# --hints--

You should set the `background-color` to `var(--light-green)` in the `General` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.General')?.getPropertyValue('background-color');
assert.equal(bgc, 'var(--light-green)');
```
