...
After the `class` attribute you should set the `target` attribute of the anchor tag to constant named `linkTarget`. This will open the link in a new tab.

# --hints--

You should write the `target` attribute inside the anchor tag after the `class` attribute

```js
const anchorTagWithHrefClassAndTarget = /return\s+`<a[^>]*href\s*=\s*['"][^'"]*['"][^>]*class\s*=\s*['"][^'"]*['"][^>]*target\s*=\s*['"][^'"]*['"][^>]*>.*<\/a>`/g;
assert(code.match(anchorTagWithHrefClassAndTarget));
```

You should set the value of the `target` attribute to the constant named `linkTarget`.

```js
const targetValue = /return\s+`<a[^>]*href\s*=\s*['"]\s*\${url}\s*['"][^>]*class\s*=\s*['"]\s*\${linkClass}\s*['"][^>]*target\s*=\s*['"]\s*\${linkTarget}\s*['"][^>]*>.*<\/a>`/g;
...
--fcc-editable-region--
  return `<a href="${url}" class="${linkClass}"></a>`
--fcc-editable-region--
}
