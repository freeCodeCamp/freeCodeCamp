...
In this step, you will create a constant named `timeDifference` that will store the difference between `currentTime` and `lastPost`. Do it like this :
...
You should declare a constant named `timeDifference` and assign it the difference between `currentTime` and `lastPost`.

```js
assert.match(code, /const\s+timeDifference\s*=\s*currentTime\s*-\s*lastPost\s*/);
```
