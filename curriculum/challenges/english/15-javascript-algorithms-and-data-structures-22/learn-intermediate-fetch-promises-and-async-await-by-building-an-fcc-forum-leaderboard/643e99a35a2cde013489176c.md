...
Next, remove everything inside the backticks of the `return` statement. Also, add a table row `tr` tag inside the backticks of the template literal.

# --hints--

Your code should contain opening and closing `tr` tags inside the backticks of the `return` statement.

```js
assert(/return `\s*<tr>.*<\/tr>\s*`/.test(code));
```
