...
You can see images of the users displaying on the result screen. Now, you need to target the class named `avatar-container` and apply these styles in the css file:
...
You should set the `display` property to `flex`

```js
const display = new __helpers.CSSHelp(document).getStyle('.avatar-container')?.getPropertyValue('display');
assert.equal(display, 'flex');
```

You should set the `flex-wrap` property to `wrap`

```js
const flexWrap = new __helpers.CSSHelp(document).getStyle('.avatar-container')?.getPropertyValue('flex-wrap');
