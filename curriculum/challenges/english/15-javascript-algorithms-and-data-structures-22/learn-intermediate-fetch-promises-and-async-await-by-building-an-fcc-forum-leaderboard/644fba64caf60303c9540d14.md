...
After the `href` attribute, you need to set the `class` attribute of the anchor tag to the constant named `linkClass`. This will set the styles according to the class names defined in the CSS file.

# --hints--

You should write the `class` attribute inside the anchor tag after the `href` attribute

```js
const anchorTagWithHrefAndClass = /return\s+`<a[^>]*href\s*=\s*['"][^'"]*['"][^>]*class\s*=\s*['"][^'"]*['"][^>]*>.*<\/a>`/g;
assert(code.match(anchorTagWithHrefAndClass));
```

You should set the value of the `class` attribute to the constant named `linkClass`.

```js
const classValue = /return\s+`<a[^>]*href\s*=\s*['"]\s*\${url}\s*['"][^>]*class\s*=\s*['"]\s*\${linkClass}\s*['"][^>]*>.*<\/a>`/g;
