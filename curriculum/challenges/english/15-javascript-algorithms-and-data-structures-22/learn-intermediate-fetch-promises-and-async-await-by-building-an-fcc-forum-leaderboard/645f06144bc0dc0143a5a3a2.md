...
Every item inside the `topics` array is an object which carries one property called `user_id`. You will use this id to display the user profile picture in our project. First, create a function named `avatars`

# --hints--

You need to create a function named `avatars`

```js
assert.match(code, /\s*function\s*avatars()/);
```

`avatars` should be defined.

```js
assert.isDefined(avatars);
```

`avatars` should be a function.

```js
assert.isFunction(avatars);
```

`avatars` should be an empty function.

```js
assert.match(code, /\s*function\s*avatars\s*\(\s*\)\s*{\s*}/);
```
