...
Create a constant named `hoursAgo` that will store the number of hours that have passed since the last post. You can do this by dividing `minutesAgo` by `60`, and then rounding down to the nearest whole number. Here's an example to do that:

```js
const hoursAgo = Math.floor(minutesAgo / 60); // 1 hour = 60 minutes
```

# --hints--

You should declare a constant named `hoursAgo` and assign it `Math.floor(minutesAgo / 60)`.

```js
assert.match(code, /const\s+hoursAgo\s*=\s*Math\.floor\s*\(\s*minutesAgo\s*\/\s*60\s*\)\s*/);
```
