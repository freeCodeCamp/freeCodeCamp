...
Using the dot notation, you will set the `category` property of the `selectedCategory` object to the word `General` as well.
...
You should assign the word `General` to the `category` property of the `selectedCategory` object.

```js
const regex = /selectedCategory\.category\s*=\s*['"]General['"]/g;
