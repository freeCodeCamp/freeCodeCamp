...
Create a class named `motivation` and assign the `background-color` property to `var(--peach)`.

# --hints--

You should set the `background-color` to `var(--peach)` in the `motivation` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.motivation')?.getPropertyValue('background-color');
assert.equal(bgc, 'var(--peach)');
```
