...
The `topic_list` object contains 4 more properties, but you only need the `topics` array. In this step, you will destructure the `topics` array from the `topic_list` object like this:
...
You should use a `const` variable to destructure the `topics` variable from the `topic_list` object.

```js
assert(code.match(/{\s+const\s+{topic_list, users}\s+=\s+data;\s+const\s+{\s*topics\s*}\s+=\s+topic_list/g));
```
