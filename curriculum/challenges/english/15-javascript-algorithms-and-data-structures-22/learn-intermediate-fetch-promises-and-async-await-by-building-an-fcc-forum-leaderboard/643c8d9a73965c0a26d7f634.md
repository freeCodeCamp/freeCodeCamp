...
The `topic_list` object contains 4 more properties, but we only need the `topics` array. In this step, you will destructure the `topics` array from the `topic_list` object like this 
...
```js
assert(code.match(/{\s+const\s+{topic_list, users}\s+=\s+data;\s+const\s+{topics}\s+=\s+topic_list/g));
```
