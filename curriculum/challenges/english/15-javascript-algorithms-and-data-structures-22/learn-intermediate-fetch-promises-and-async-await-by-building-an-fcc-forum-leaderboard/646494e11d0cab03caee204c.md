...
When anyone clicks on the post title, they should be able to read the entire post on a different page, for that, replace the `p` tag with an HTML anchor `a` tag

# --hints--

You should replace the `p` tag with an `a` tag, please set the class name to `post-title`

```js
const regex = /<a\s+class=['"]post-title['"]>(?:.|\n)*?<\/a>/;
