...
You will create a `if` statement to check if the argument named `id` is available to the `categorieList` object. To do this task, you will need to use the `hasOwnProperty()` method.

# --hints--

You should use the `hasOwnProperty()` method to check if the `id` argument is available to the `categorieList` object.

```js
assert(code.match(/if\s*\(\s*categorieList\s*\.hasOwnProperty\s*\(\s*id\s*\)\s*\)/));
```
