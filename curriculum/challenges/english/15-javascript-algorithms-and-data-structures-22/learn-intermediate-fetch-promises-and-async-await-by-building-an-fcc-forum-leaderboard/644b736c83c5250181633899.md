...
The `hasOwnProperty()` method is used to check if a specific property exists in an object. It returns `true` if the object has the property, otherwise `false`. For example, consider an object `person` with a property `name`:
...
You will create an `if` statement to check if the argument named `id` is available to the `categoryList` object. To do this task, you will need to use the `hasOwnProperty()` method.

# --hints--

You should use the `hasOwnProperty()` method to check if the `id` argument is available to the `categoryList` object.

```js
assert(code.match(/if\s*\(\s*categoryList\s*\.hasOwnProperty\s*\(\s*id\s*\)\s*\)/));
```
