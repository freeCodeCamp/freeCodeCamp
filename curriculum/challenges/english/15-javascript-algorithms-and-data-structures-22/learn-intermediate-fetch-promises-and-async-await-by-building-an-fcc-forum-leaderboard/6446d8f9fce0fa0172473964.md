...
Below every `title`, you need to display which `category` the topic belongs to. The `category_id` property of the `topic` object contains only a number. We provided an object named `allCategories` that contains some category names and class names. You can use the `category_id` to get the category name and the corresponding className that will be used to style & color the category. To do this task, you need to create a function named `forumCategory

# --hints--

You need to create a function named `forumCategory

```js
assert.match(code, /\s*const\s*forumCategory*=\s*\(\s*\)\s*=>\s*{\s*}/);
```

`forumCategoryshould be defined.

```js
assert.isDefined(forumCategory
```

`forumCategoryshould be a function.

```js
assert.isFunction(forumCategory
```

`forumCategoryshould be an empty function.
