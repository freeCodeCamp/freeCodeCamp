...
Now you need to create a function to display the data on your page.

Start by creating a function called `showLatestPosts`, which takes a single `data` parameter.

# --hints--

You should declare `showLatestPosts` with the `const` keyword.

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*\(\s*\w*\s*\)\s*=>\s*/);
```

Your `showLatestPosts` function should have a `data` parameter.

```js
assert.match(code, /showLatestPosts\s*=\s*\(\s*data\s*\)\s*=>/);
```
