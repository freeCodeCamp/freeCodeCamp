...
Create a constant named `linkClass`, and assign 2 things in a template literal, first the word `category`, followed by a space, and then the property `selectedCategory.className`. Both of them are class names that are used to apply styles to the anchor tag.

# --hints--

You should create a constant named `linkClass` and assign a template literal to it.

```js
assert(code.match(/const\s+linkClass\s*=\s*`/g));
```

You should assign the word `category` to the template literal.

```js
assert(code.match(/const\s+linkClass\s*=\s*`\s*category.*`/g));
```

You should assign property `selectedCategory.className` after the word `category` inside the template literal using a dollar sign and curly braces.
