...
Create a class named `python` and assign the `background-color` property to `var(--green)`.

# --hints--

You should set the `background-color` to `var(--green)` in the `python` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.python')?.getPropertyValue('background-color');
assert.equal(bgc, 'var(--green)');
```
