...
You have targeted and colored all the 9 categories, now you need to style the post title. But before that, you need to assign a class named `post-title` on the `p` tag

# --hints--

You should a class named `post-title` to the `p` tag.

```js
const className = /<p[^>]*class\s*=\s*['"]\s*post-title\s*['"][^>]*>.*<\/p>/g;
assert(code.match(className));
```
