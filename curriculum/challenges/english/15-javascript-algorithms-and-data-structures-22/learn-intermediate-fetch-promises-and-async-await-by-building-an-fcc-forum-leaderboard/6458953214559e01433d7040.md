...
It's time to color the categories so that they look nice. In total, we have 9 categories including the `General` category. So, we will have a couple of class names to color the categories. First, create a class named `career` and assign the `background-color` property to `var(--salmon-color)`.

# --hints--

You should set the `background-color` to `var(--salmon-color)` in the `career` class name. Don't forget to use the semicolon at the end of the code.

```js
const bgc = new __helpers.CSSHelp(document).getStyle('.career')?.getPropertyValue('background-color');
assert.equal(bgc, 'var(--salmon-color)');
```
