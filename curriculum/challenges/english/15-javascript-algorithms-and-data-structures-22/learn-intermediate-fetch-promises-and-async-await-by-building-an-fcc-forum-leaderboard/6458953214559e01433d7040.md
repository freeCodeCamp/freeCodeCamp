...
Its time to color the categories so that they look nice. In total we have 9 categories including the `General` category. So, we will have a couple of class names to color the categories. First, create a class named `career` and assign the `background-color` property to `var(--salmon-color)`.
