...
You will add a callback function to the `forEach` method. The callback function will take the `item` as a parameter. Like this: 
...
You should use the callback function in the `forEach` method. Don't forget to add `item` as a parameter.

```js
assert(code.match(/topics\.forEach\(\s*(\(\s*item\s*\)|item)\s*=>\s*{/g));
```
