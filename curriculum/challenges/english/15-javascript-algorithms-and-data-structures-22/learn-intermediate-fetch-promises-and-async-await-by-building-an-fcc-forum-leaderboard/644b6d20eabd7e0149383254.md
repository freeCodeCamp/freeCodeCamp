...
Create a empty object variable named `category` in the function named `forumCategories`, this variable will be used to store the category name and class name for each category.

# --hints--

You should use a `let` variable to store the empty object named `category` in the function named `forumCategories`

```js
assert.match(code, /\s*let\s*category\s*/);
```

Your object named `category` should be empty

```js
assert.match(code, /\s*let\s*category\s*=\s*{\s*}\s*/);
```
