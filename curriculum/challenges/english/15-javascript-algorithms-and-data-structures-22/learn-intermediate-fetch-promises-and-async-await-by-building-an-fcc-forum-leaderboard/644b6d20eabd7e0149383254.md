...
Create an empty object variable named `selectedCategory` in the function named `forumCategories`, this will be used to store the category name and class name for each category.

# --hints--

You should use a `let` variable to store the empty object named `selectedCategory` in the function named `forumCategories`

```js
assert.match(code, /\s*(let|const|var)\s*selectedCategory\s*/);
```

Your object named `selectedCategory` should be empty

```js
assert.match(code, /\s*(let|const|var)\s*selectedCategory\s*=\s*{\s*}\s*/);
```
