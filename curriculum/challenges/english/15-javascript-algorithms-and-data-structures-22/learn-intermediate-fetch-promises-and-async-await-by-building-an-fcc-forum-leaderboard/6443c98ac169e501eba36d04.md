...
Create a constant named `MS_PER_MINUTE` that will store the number of milliseconds in a minute. There are `1000 * 60` milliseconds in a minute. You can do the calculation like this :
...
Your code should declare a constant named `MS_PER_MINUTE` and assign it `1000 * 60`.

```js
assert.match(code, /const\s+MS_PER_MINUTE\s*=\s*1000\s*\*\s*60\s*/);
```
