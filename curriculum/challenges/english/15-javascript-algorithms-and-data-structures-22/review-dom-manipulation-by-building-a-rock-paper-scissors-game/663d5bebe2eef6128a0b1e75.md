...
An example function call has been provided for you so you can test your code.
...
Your `resetGame` function should set the `playerScoreSpanElement` to display the new player score.

```js
assert.equal(playerScoreSpanElement.innerText, "0");
```

Your `resetGame` function should set the `computerScoreSpanElement` to display the new computer score.

```js
assert.equal(computerScoreSpanElement.innerText, "0");
```

Your `resetGame` function should hide the `resetGameBtn` button.

```js
assert.equal(resetGameBtn.style.display, "none");
```

Your `resetGame` function should show the `optionsContainer`.

```js
assert.equal(optionsContainer.style.display, "block");
```
