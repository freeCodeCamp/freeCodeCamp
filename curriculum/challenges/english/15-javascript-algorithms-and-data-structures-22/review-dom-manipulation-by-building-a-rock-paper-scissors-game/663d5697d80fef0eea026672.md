...
- Remember that you learned how to work with the `el.style.display` property in previous projects. You can use this property to show the `resetGameBtn` button and hide the `optionsContainer`.

# --hints--

You should update the `winnerMsgElement` if the player has reached three points.
...
You should update the `winnerMsgElement` if the computer has reached three points.
...
computerScore = 3;
showResults("Rock");
assert.equal(resetGameBtn.style.display, "block");
```
