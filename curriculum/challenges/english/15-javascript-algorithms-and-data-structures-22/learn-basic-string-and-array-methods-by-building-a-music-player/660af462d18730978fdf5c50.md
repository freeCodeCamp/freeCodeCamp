...
```js
assert.strictEqual(printGreeting, undefined);
```

You should not have a `printMessage` function.

```js
assert.strictEqual(printMessage, undefined);
```

You should not have an `addTwoNumbers` function.

```js
assert.strictEqual(addTwoNumbers, undefined);
```

You should not have a `printGreeting();` function call.

```js
assert.strictEqual(console.log.calledWith('Hello there!'), false);
```

You should not have a `printMessage('freeCodeCamp');` function call.

```js
assert.strictEqual(console.log.calledWith('freeCodeCamp is awesome!'), false);
```

You should not have a `console.log(addTwoNumbers(3,4));` console statement.

```js
assert.strictEqual(console.log.calledWith(7), false);
```
