...
Use `classList` and `remove()` method to remove the `.playing` class since the song will be paused at this point. To finally pause the song, use the `pause()` method on the `audio` variable.
...
```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime;?\s*.*\s*\};?/)
```
