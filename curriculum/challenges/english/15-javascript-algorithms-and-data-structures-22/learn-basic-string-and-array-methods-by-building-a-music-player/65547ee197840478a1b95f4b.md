...
Add a `click` event listener to the `nextButton` element. Use arrow syntax to add a callback and call the `playNextSong` function inside it.
...
```js
assert.match(code, /nextButton\.addEventListener\(('|"|`)click\1,\s*\(\)\s*=>\s*\{\s*/)
```

You should call the `playNextSong` function inside your `nextButton` event listener.

```js
assert.match(code, /nextButton\.addEventListener\(('|"|`)click\1,\s*\(\)\s*=>\s*\{\s*playNextSong\(\);?\s*\}\);?/)
```
