...
Loop through the `playlistSongElements` with a `forEach` method and pass in the parameter `songEl`. After that, use arrow syntax to add in a callback, and use `removeAttribute()` method to remove the `"aria-current"` attribute on each of the songs.
