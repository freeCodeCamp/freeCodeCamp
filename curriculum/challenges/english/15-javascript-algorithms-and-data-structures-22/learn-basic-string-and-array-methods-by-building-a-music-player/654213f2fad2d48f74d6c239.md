...
Create another `button` element with the class `playlist-song-delete` and the `aria-label` attribute set to `Delete` interpollated with `song.title`. For the content of the delete icon, paste in the following SVG:
