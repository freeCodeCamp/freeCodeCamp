...
All the songs are now in the `allSongs` array. The next thing you will need to do is to initialize the <dfn>Web Audio API</dfn>.

The Audio API is a part of the Web Audio API specification, which is a high-level API for processing and synthesizing audio in web applications.
