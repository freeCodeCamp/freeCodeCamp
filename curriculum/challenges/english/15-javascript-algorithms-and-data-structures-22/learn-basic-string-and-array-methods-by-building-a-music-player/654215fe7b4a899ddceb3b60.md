...
Now you should introduce the `join()` method to combine the `songsHTML` markup into a single string.

Lastly use `innerHTML` to add `songsHTML` to the `playlistSongs`. This step updates the playlist in your HTML document.
