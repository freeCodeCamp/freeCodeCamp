...
We have identified a bug in the music player with handling an empty playlist. When all songs in the playlist are deleted, a "Reset Playlist" button correctly appears, allowing the user to restore the playlist. However, if the shuffle button is clicked while the playlist is empty, the "Reset Playlist" button disappears. This issue arises because the shuffle() function does not account for the edge case of an empty playlist. To fix this, update the shuffle() function to handle this scenario appropriately. Ensure that the "Reset Playlist" button remains visible and functional even when the playlist is empty and the shuffle button is clicked.

# --hints--

You should return from shuffle() when the playlist is empty.

```js
assert.match(code, /if\s*\(\s*(!?userData\.songs\.length\s*|\s*userData\.songs\.length\s*===\s*0\s*)\)\s*{\s*return;\s*}/);
```
