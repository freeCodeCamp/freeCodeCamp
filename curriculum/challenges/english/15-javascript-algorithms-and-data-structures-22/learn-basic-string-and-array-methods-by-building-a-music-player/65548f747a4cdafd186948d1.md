...
```js
assert.match(code, /document\.getElementById\(`song-\$\{userData\?\.currentSong\?\.id\}`\);?\s*\};?/)
```

You should assign your `getElementById()` to a `songToHighlight` constant.

```js
assert.match(code, /const\s*songToHighlight\s*=\s*document\.getElementById\(`song-\$\{userData\?\.currentSong\?\.id\}`\);?\s*\};?/)
```
