...
```js
assert.match(code, /if\s*\(.*a.title.*>.*b.title.*\)/);
``` 

Your `if` statement should return the number `1` if `a.title` is greater than `b.title`.

```js
assert.match(code, /if\s*\(.*a\.title.*>.*b\.title.*\)\s*{\s*return\s*1;?\s*}|if\s*\(.*a\.title.*>.*b\.title.*\)\s*return\s*1;?/);
```
...
const sortedSongList = userData?.songs.sort((a,b) => {
  if (a.title < b.title) {
    return -1;
  }
