...
`textContent` is one of the properties of `DOM`. It sets the text content of a node and allows you to set or retrieve the text content of an HTML element.
...
console.log(element.textContent); // Output: Modified text content
```

Use `ternary` to set the `textContent` of `playingSong` to the `currentTitle` variable and that of `songArtist` to the `currentArtist` variable if they exist. Otherwise, set them both to an empty string.
...
```js
assert.match(code, /playingSong.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*"";?/)
```
...
```js
assert.match(code, /songArtist.textContent\s*=\s*currentArtist\s*\?\s*currentArtist\s*:\s*"";?/)
```
