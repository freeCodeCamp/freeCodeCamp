...
Use `textContent` property to update the contents of the `playingSong` and `songArtist` elements to display the title and artist of the current song.

The `textContent` property is part of DOM and it sets the text content of a node and allows you to set or retrieve the text content of an HTML element.

Set the `textContent` of `playingSong` to the `currentTitle` variable if it exists, otherwise set it to an empty string.

Set the `textContent` of `songArtist` to the `currentArtist` variable if it exists, otherwise set it to an empty string.
