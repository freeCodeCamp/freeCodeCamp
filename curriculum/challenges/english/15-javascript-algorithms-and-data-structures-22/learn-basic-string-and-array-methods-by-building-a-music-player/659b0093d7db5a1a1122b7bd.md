...
```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\)\s*=>(\s*userData\?\.songs\.indexOf\()|(\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

You should pass in `userData?.currentSong` into the `indexOf()` method.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\)\s*=>(\s*userData\?\.songs\.indexOf\(userData\?\.currentSong\);?)|(\s*{\s*return\s+userData\?\.songs\.indexOf\(userData\?\.currentSong\);?)/)
```
