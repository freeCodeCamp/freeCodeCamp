...
Next, you will need to retrive the next song in the playlist. For that we will need to get the index of the current song and then add 1 to it. 

Create a constant variable named `nextSong` and assign it `userData?.songs[currentSongIndex + 1]`.

Lastly, utilize the `playSong` function to start the playback of the next song. Within the function pass in the `nextSong.id` as an argument.
