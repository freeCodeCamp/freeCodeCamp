...
If there is a match then set `userData.currentSong` to `null` and `userData.songCurrentTime` to `0`. Then, call the `pauseSong()` function to stop the playback and the `setPlayerDisplay()` function to update the player display.
