...
Use the `addEventListener()` method and pass in the string `click` for the first argument as the event type to listen for and an empty callback function with arrow syntax for the second argument. Ex.(`()=>{}`).
