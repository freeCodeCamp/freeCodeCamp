...
Using an `else` statement, denote the alternative condition when the `userData.currentSong` is not `null`.

Inside the `else` statement, call the `playSong()` function with the ID of the currently playing song as an argument. 
