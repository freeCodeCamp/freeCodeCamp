...
```js
assert.match(code, /if\s*\(userData\?\.currentSong\s*===\s*null\)\s*\{\s*playSong\(userData\?\.songs\s*\[\s*0\s*\]\s*\.id\);?\s*\}/)
```

You should add an `else` block to your `if` statement.

```js
assert.match(code, /if\s*\(userData\?\.currentSong\s*===\s*null\)\s*\{\s*playSong\(userData\?\.songs\s*\[\s*0\s*\]\s*\.id\);?\s*\}\s*else\s*\{/)
```
...
```js
assert.match(code, /if\s*\(userData\?\.currentSong\s*===\s*null\)\s*\{\s*playSong\(userData\?\.songs\s*\[\s*0\s*\]\s*\.id\);?\s*\}\s*else\s*\{\s*playSong\(userData\?\.currentSong\.id\);?\s*\}/)
```
