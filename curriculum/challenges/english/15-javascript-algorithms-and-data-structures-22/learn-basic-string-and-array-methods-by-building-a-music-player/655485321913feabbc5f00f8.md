...
Add a `click` event listener to the `previousButton` element, pass in a callback using an arrow syntax, then call the `playPreviousSong` function.
...
```js
assert.match(code, /previousButton\.addEventListener\(('|"|`)click\1,\s*\(\)\s*=>\s*\{\s*/)
```

You should call the `playPreviousSong` function inside your `previousButton` event listener.

```js
assert.match(code, /previousButton\.addEventListener\(('|"|`)click\1,\s*\(\)\s*=>\s*\{\s*playPreviousSong\(\);?\s*\}\);?/)
```
