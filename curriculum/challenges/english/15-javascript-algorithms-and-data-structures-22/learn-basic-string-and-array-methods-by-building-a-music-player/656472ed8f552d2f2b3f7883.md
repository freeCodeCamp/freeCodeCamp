...
```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\|\|\s*userData\?\.currentSong\.id\s*!==\s*song\.id\s*\)\s*\{\s*audio\.currentTime\s*=\s*0;?\s*\}\s*else\s*{/)
```

You should set `audio.currentTime` to `userData?.songCurrentTime` inside the `else` block of your `if` statement.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\|\|\s*userData\?\.currentSong\.id\s*!==\s*song\.id\s*\)\s*\{\s*audio\.currentTime\s*=\s*0;?\s*\}\s*else\s*{\s*audio\.currentTime\s*=\s*userData\?\.songCurrentTime;?\s*\}/)
```
