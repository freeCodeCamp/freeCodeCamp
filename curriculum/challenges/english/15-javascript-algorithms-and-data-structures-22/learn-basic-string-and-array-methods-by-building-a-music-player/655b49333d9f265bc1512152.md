...
Once the `ended` event has being triggered, you need to check if there is a next song to play.

Retrieve the current song index by calling the `getCurrentSongIndex()` function, and store it in a variable called `currentSongIndex`.

Check if the next song exists by verifying if the song at the index `currentSongIndex + 1` exists in the `userData?.songs` array. Store the result in a variable called `nextSongExists`.
