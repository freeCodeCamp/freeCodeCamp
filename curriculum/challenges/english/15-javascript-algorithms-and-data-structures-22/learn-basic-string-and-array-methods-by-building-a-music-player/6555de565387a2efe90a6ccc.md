...
Use the `setAttribute` method on the `playButton` element to set an attribute named `aria-label`. For the value, use a `ternary operator` to set `song?.title` to `Play ${song.title}` or `"Play"` if there's no `song.title` available.
