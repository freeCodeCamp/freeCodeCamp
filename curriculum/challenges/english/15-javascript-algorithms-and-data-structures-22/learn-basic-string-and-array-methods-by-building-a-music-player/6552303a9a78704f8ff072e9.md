...
Add a `click` event listener to the `pauseButton` element, use arrow syntax to open up a callback, then call the `pauseSong` function.
...
```js
assert.match(code, /pauseButton\.addEventListener\(('|"|`)click\1,\s*\(\)\s*=>\s*\{\s*/)
```

You should call the `pauseSong` function inside your `pauseButton` event listener.

```js
assert.match(code, /pauseButton\.addEventListener\(('|"|`)click\1,\s*\(\)\s*=>\s*\{\s*pauseSong\(\);?\s*\}\);?/)
```
