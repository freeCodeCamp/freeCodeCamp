...
Inside the `pauseSong` function, access the `userData` object to make updates to the `songCurrentTime` property.

Use the `audio.currentTime` property to assign the current time of the song to the `songCurrentTime` property. 
This property stores the current time of the song when paused.
