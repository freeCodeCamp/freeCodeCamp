...
You need re-rendered the current song, highlight it and set the play button's accessible text since the current song have changed. 

Call the `renderSongs` function and pass in the `userData?.songs` array as an argument, this displays the modified playlist.

Call the `highlightCurrentSong` function to highlight the current song if there is any.
