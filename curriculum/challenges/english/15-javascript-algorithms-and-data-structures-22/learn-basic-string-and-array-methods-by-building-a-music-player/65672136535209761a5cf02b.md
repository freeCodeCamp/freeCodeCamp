...
Make sure you to include the class `playlist-song-info` to the the `button` element.

```js
assert(document.querySelector(".playlist-song button").classList.contains("playlist-song-info"))
```
