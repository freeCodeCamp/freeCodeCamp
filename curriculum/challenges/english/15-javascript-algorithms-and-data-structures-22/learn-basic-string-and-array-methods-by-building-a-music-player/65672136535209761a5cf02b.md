...
You do not need to change the other parts of the code. All you need to do is add the `onclick` attribute to the button element.

```js
assert.match(code, /<button\s.*>\s*<span\s+class=('|")playlist-song-title\1>\$\{song\.title\}<\/span>\s*<span\s+class=\1playlist-song-artist\1>\$\{song\.artist\}<\/span>\s*<span\s+class=\1playlist-song-duration\1>\$\{song\.duration\}<\/span>\s*<\/button>/)
```

You do not need to change the `class` attribute. All you need to do is add the `onclick` attribute to the button element.

```js
assert.match(code, /\s*class=('|")playlist-song-info\1\s*/)
```

Make sure you call the `playSong` function with `song.id` inside the `onclick` attribute.

```js
assert.match(code, /\s*onclick=('|")playSong\(\$\{song\.id\}\)\1\s*/)
```
