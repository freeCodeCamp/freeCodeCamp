...
You should not modify the existing button and its content.

```js
assert.match(code, /<button\s+class=('|")playlist-song-info\1.*>\s*<span\s+class=\1playlist-song-title\1>\$\{song\.title\}<\/span>\s*<span\s+class=\1playlist-song-artist\1>\$\{song\.artist\}<\/span>\s*<span\s+class=\1playlist-song-duration\1>\$\{song\.duration\}<\/span>\s*<\/button>/)
```

YOu should add an `onclick` with the value `playSong(${song.id})` as the second attribute and value of the `button` element.

```js
assert.match(code, /<button\s+class=('|")playlist-song-info\1\s*onclick=\1playSong\(\$\{song\.id\}\)\1>\s*<span\s+class=\1playlist-song-title\1>\$\{song\.title\}<\/span>\s*<span\s+class=\1playlist-song-artist\1>\$\{song\.artist\}<\/span>\s*<span\s+class=\1playlist-song-duration\1>\$\{song\.duration\}<\/span>\s*<\/button>/)
```
