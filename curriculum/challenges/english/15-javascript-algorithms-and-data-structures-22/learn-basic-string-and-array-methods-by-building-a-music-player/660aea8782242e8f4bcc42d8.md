...
```js
assert.match(code,/let|const\s+printMessage\s*=\s*\(\s*org\s*\)\s*=>/);
```
...
```js
assert.match(printMessage.toString(),/console\.log\(\s*`\s*\$\{org\}\s*is\s*awesome\s*!\s*`\s*\)/);
```
...
```js
assert.isBelow(code.indexOf('printMessage("freeCodeCamp")'), code.indexOf('printMessage'));
```
