...
Finally, with the event listener attached to the `resetButton`, it's time to render the songs again, update the play button's accessible text, and remove the reset button from the playlist.

Call the `renderSongs()` function with the `userData?.songs` as an argument to render the songs again.
