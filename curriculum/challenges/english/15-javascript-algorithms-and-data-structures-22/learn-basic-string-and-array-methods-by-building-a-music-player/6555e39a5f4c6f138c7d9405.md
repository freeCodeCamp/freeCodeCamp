...
Use the `sort()` method on the `userData?.songs` array. For the callback, introduce `Math.random()`, and subtract `0.5`. Assign all of that to `userData.songs`.
...
You should assign the `sort()` method you used on `userData?.songs` to `userData.songs`.

```js
assert.match(code, /userData\.songs\s*=\s*userData\?\.songs\.sort\(\(\)\s*=>\s*Math\.random\(\)\s*-\s*0\.5\);?/)
```
