...
If the playlist is empty, create a `resetButton` and `resetText` elements. 

Declare a varible `resetButton` and assign it the text "button" using the `createElement()` method.

Declare a variable `resetText` and assign it the text "Reset Playlist" using the `createTextNode()` method.
