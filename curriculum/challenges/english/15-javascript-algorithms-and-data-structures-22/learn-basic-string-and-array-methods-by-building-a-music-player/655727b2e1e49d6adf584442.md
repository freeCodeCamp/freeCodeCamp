...
If the playlist is empty, you need to create a `resetButton` and `resetText` elements. 

`createElement()` is a Document Object Model (DOM) method you can use to dynamically create an element right within JavaScript. To use `createElement()`, you call it, and pass in the tag name as a string:
...
Declare a `resetButton` constant, then use `createElement()` to create a `button`.
