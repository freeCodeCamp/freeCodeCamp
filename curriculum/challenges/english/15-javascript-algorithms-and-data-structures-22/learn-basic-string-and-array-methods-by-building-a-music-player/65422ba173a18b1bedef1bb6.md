...
Within the arrow function of the event listener, add an `if` statement to check if there is a currently playing song.

The `if` statement should check if the `userData.currentSong` is `null`. If it is, then call the `playSong()` function with the `id` of the first song in the `userData.songs` array. 

If it's not `null`, then proceed to the `else` statement.
