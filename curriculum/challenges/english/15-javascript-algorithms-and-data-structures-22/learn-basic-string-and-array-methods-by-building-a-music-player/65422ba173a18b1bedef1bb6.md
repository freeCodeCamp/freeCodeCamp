...
```js
  assert.match(code, /if\s*\(userData\?\.currentSong\s*===\s*null\s*\)\s*\{/);
```

You should call the `playSong` function with `userData?.songs[0].id` inside your `if` block.

```js
assert.match(code, /if\s*\(userData\?\.currentSong\s*===\s*null\s*\)\s*\{\s*playSong\(\s*userData\?\.songs\s*\[\s*0\s*\]\s*\.id\s*\)\s*;?\s*\}/);
```
