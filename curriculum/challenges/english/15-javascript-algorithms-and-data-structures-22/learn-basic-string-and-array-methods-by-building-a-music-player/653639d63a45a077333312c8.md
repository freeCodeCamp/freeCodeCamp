...
```js
assert.match(code, /if\s*\(\s*(?:userData\?\.currentSong\s*===\s*(?:null|undefined)\s*\|\|\s*userData\?\.currentSong\.id\s*!==\s*song\.id|userData\?\.currentSong\.id\s*!==\s*song\.id\s*\|\|\s*userData\?\.currentSong\s*===\s*(?:null|undefined))\s*\)\s*\{\s*/);
```
