...
Add an `if` statement to check whether the `userData?.currentSong` is `null` or if the `id` of the current song `userData?.currentSong.id` is different from the `id` of the selected song `song.id`. If either of these conditions is true, set the `currentTime` property of the `audio` object to `0`.
