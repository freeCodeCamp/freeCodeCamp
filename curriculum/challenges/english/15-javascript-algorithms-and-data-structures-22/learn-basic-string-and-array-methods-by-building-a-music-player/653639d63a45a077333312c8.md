...
You should create an `if` statement that checks whether `userData.currentSong` is either falsy *OR* `userData.currentSong.id` is not equal to `song.id`. Don't forget to use optional chaining.

```js
assert.match(code, /if\s*\(\s*(?:(?:!userData\?\.currentSong|\s*userData\?\.currentSong\s*===\s*undefined)\s*\|\|\s*userData\?\.currentSong\.id\s*!==\s*song\.id\s*|\s*userData\?\.currentSong\.id\s*!==\s*song\.id\s*\|\|\s*(?:!userData\?\.currentSong|\s*userData\?\.currentSong\s*===\s*undefined))\s*\)\s*\{/);
```
...
assert.match(
  code,
   /if\s*\(\s*(?:(?:!userData\?\.currentSong|\s*userData\?\.currentSong\s*===\s*undefined)\s*\|\|\s*userData\?\.currentSong\.id\s*!==\s*song\.id\s*|\s*userData\?\.currentSong\.id\s*!==\s*song\.id\s*\|\|\s*(?:!userData\?\.currentSong|\s*userData\?\.currentSong\s*===\s*undefined))\s*\)\s*\{\s*audio\.currentTime\s*=\s*0\s*;?\s*\}/
);
```
