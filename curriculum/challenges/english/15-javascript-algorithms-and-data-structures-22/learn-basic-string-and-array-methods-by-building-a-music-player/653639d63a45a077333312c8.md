...
You need to construct an `if` statement that verifies whether `userData?.currentSong` is falsy (either null or undefined) and also confirms that `userData?.currentSong.id` doesn't match the `id` of the song.
