...
Add an `if` statement that will check whether the `userData.currentSong` is `null` or if the `id` of the current song `userData.currentSong.id` is different from the `id` of the selected song `song.id`. If either of these conditions is true, set the `currentTime` property of the `audio` object to `0`.

Inside the `else` block, set the `audio.currentTime` to `userData.songCurrentTime`.
