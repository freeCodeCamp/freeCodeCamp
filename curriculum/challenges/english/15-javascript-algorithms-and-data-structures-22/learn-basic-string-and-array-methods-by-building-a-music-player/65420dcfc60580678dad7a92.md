...
Inside the backticks, create an `li` tag with the class `playlist-song`. Also, add an `id` attribute with the value of `song-${song.id}` to the `li` tag.
...
Your `li` tag should have the `id` `song-${song.id}` as its first attribute and value.

```js
assert.match(code, /return\s*`\s*<li\s*id=('|")song-\$\{song\.id\}\1\s*/)
```

Your `li` tag should have the class `playlist-song` as its second attribute and value.

```js
assert.match(code, /return\s*`\s*<li\s*id=('|")song-\$\{song\.id\}\1\s*class=\1playlist-song\1>\s*<\/li>\s*`;?/)
```
