...
Inside the backticks, create an `li` tag with an id attribute `song-${song.id}` and a class attribute `playlist-song`.
...
Your opening `li` tag should have two attributes: an `id` attribute `song-${song.id}`, and a `class` attribute `playlist-song`.

```js
assert.match(code, /return\s*`\s*<li\s*(?:id\s*=\s*('|")song-\$\{song\.id\}\1\s*class\s*=\s*('|")playlist-song\2|class\s*=\s*('|")playlist-song\3\s*id\s*=\s*('|")song-\$\{song\.id\}\4)\s*>\s*<\/li>\s*`;?/)
```
