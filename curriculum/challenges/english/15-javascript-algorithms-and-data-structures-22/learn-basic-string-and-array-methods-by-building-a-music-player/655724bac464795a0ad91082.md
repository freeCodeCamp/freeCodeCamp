...
Next, we need to check if the playlist is empty. If it is, we need to reset the `userData` object to its original state.

Use an `if` statement to check if the `userData.songs` has a length of 0, meaning check if the playlist if empty.
