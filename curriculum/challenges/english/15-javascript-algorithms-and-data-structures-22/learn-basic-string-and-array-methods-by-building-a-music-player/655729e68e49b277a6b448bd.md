...
After initializing the `resetButton`, you need to assign its attributes. JavaScript provides the `id` and `ariaLabel` attributes you need to use for this.

For example, `element.id` would set an `id` attribute, and `element.ariaLabel` would set an `aria-label` attribute.

Set the `id` attribute to `reset` and the `aria-label` attribute to `Reset playlist`.
