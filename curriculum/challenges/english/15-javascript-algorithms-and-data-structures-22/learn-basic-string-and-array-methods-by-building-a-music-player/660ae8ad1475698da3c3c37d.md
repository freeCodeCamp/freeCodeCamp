...
Your `printGreeting` function call should output `Hello there!` to the console, but it logged `--fcc-actual--` instead

```js
const spy = __helpers.spyOn(console, "log");
try {
  printGreeting();
  assert.strictEqual(spy.calls[0][0], "Hello there!");
} finally {
  spy.restore()
}
```

Your `printGreeting` function should call console log once. 
