...
The `map()` method is used to iterate through an array and return a new array, it's helpul when you want to create a new array based on the values of an existing array. For example:
...
Your `map()` method should have a `songs` parameter in it's callback function.

```js
assert.match(code, /array.map\(\(songs\).*\)/)
```

The callback function of your `map()` method should use arrow function syntax.

```js
assert.match(code, /array.map\(\(songs\)\s*=>\s*\{\s*\}\)/)
```

You should assign your `array.map` to a `songsHTML` constant.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array.map\(\(songs\)\s*=>\s*\{\s*\}\)/)
```
