...
Create two constants: `playingSong` and `songArtist`. Use `getElementById()` to select the elements with the ids `player-song-title` and `player-song-artist` respectively. 
