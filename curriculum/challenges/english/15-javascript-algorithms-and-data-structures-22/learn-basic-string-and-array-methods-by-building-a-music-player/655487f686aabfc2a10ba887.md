...
Before you start working on playing the next song and the previous song, you need a function to get the index of each song in the `userData.songs`

Use an arrow syntax to create a `getCurrentSongIndex` function. Use the `indexOf` method on `userData?.songs`, and pass in `userData.currentSong` into the `indexOf()`.
