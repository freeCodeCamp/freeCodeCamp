...
Within the `playPreviousSong` function, check if there's no currently playing song with the condition `userData?.currentSong === null`. If there isn't any, exit the function using `return`.

Inside the `else` block, create a constant named `currentSongIndex` and assign it the calling of `getCurrentSongIndex`.
