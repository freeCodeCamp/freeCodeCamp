...
You need to add the `resetButton` to the `playlistSongs` element. Use the `appendChild()` method to add the `resetButton` and pass in `resetText`. This method allows you to add the `resetButton` to the end of the `playlistSongs` element.

Next, call the `appendChild()` method on the `playlistSongs` element and pass in `resetButton`. This replaces the `resetButton` as a child element of the `playlistSongs` element.
