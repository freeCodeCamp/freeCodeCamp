...
With all the functionality set in place, what happens next when a song finishes playing? Set up an event listener which will detect when the currently playing song ends.

Using an arrow function, add an event listener to the `audio` element which listens for the `ended` event. Leave the callback function empty for now.
