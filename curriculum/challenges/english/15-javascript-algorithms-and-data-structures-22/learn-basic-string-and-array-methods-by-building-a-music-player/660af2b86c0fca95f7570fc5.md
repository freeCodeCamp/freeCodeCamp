...
Refactor(update) your `addTwoNumbers` function to remove the `return` keyword and the curly braces `{}`. Your `addTwoNumbers` function should instead use an implicit return.
...
Your function call should still return the correct output of `7`.
