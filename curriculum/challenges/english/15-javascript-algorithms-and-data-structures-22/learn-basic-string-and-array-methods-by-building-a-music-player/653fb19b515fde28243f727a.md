...
Arrow function is a shorter way to write functions in JavaScript. It uses the variable keywords (`var`, `let`, and `const`) and fat arrow syntax (`=>`) to create functions in a more concise way:
...
If the function body consists of a single expression, you don't need the curly braces and the `return` keyword:
