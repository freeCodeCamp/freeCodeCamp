...
Arrow function is a shorter and more concise way to write functions in JavaScript. It uses the variable keywords (`var`, `let`, and `const`) and fat arrow syntax (`=>`):
...
If the function body consists of a single expression, you don't need the curly braces and the `return` keyword. This is called `implicit return`.
