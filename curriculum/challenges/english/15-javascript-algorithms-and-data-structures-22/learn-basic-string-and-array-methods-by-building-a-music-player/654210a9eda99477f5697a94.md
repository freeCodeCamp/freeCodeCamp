...
```js
assert.match(code, /<button\s*class=('|")playlist-song-info\1>\s*/)
```

You should create a `span` element inside your `button` element.

```js
assert.match(code, /<button\s*class=('|")playlist-song-info\1>\s*<span\s*/)
```

Your `span` element should have the class `playlist-song-title`.

```js
assert.match(code, /<button\s*class=('|")playlist-song-info\1>\s*<span\s*class=\1playlist-song-title\1>/)
```

You should interpolate `song.title` as the content of your `span` element.

```js
assert.match(code, /<button\s*class=('|")playlist-song-info\1>\s*<span\s*class=\1playlist-song-title\1>\$\{song\.title\}\s*<\/span>\s*<\/button>/)
```
