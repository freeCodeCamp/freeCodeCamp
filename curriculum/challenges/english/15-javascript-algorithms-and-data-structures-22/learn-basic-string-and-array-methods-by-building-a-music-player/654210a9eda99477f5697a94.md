...
Inside the `li` tag, add a `button` element with class `playlist-song-info`. Inside the `button`, add a `span` element with the class `playlist-song-title`, then interpollate `song.title` as the text.
