...
If there is no next song in the playlist, use the `else` block to reset the `userData` object by setting its `songs` property to `userData?.songs`, `currentSong` to `null`, and `songCurrentTime` to `0`.
...
```js
assert.match(code, /if\s*\(nextSongExists\)\s*\{\s*playNextSong\(\);?\s*.*\s*.*\s*.*\s*.*\s*.*\s*.*\s*\}/)
```

Your `if` statement should have an `else` block.

```js
assert.match(code, /if\s*\(nextSongExists\)\s*\{\s*playNextSong\(\);?\s*\}\s*else\s*\{\s*.*\s*.*\s*.*\s*.*\s*.*\s*\}/)
```

You should reintroduce the `userData` object in the `else` block of your `if` statement. You don't need `const` or `let` for this.

```js
assert.match(code, /if\s*\(nextSongExists\)\s*\{\s*playNextSong\(\);?\s*\}\s*else\s*\{\s*userData\s*=\s*\{\s*.*\s*.*\s*.*\s*.*\s*\}/)
```

You should have a `songs` property set to `userData?.songs` inside your `userData` object. Don't forget to add comma.

```js
assert.match(code, /if\s*\(nextSongExists\)\s*\{\s*playNextSong\(\);?\s*\}\s*else\s*\{\s*userData\s*=\s*\{\s*songs:\s*userData\?\.songs,\s*.*\s*.*\s*.*\s*\}/)
```

You should set the `currentSong` property to `null` inside your `userData` object. Don't forget to add comma.

```js
assert.match(code, /if\s*\(nextSongExists\)\s*\{\s*playNextSong\(\);?\s*\}\s*else\s*\{\s*userData\s*=\s*\{\s*songs:\s*userData\?\.songs,\s*currentSong:\s*null,\s*.*\s*.*\s*\}/)
```

You should set the `songCurrentTime` property to `0` inside your `userData` object.

```js
assert.match(code, /if\s*\(nextSongExists\)\s*\{\s*playNextSong\(\);?\s*\}\s*else\s*\{\s*userData\s*=\s*\{\s*songs:\s*userData\?\.songs,\s*currentSong:\s*null,\s*songCurrentTime:\s*0,?\s*.*\s*\}/)
```
