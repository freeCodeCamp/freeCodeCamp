...
```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\);?\s*.*\s*.*\s*.*\s*\};?/)
```

Your `if` statement should have an `else` block.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\);?\s*\}\s*else\s*\{\s*.*\s*.*\s*\};?/)
```

You should use dot notation to set the `currentSong` property of `userData` to `null` in the `else` block of your `if` statement.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\);?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null;?\s*.*\s*\};?/)
```

You should use dot notation to set the `songCurrentTime` property of `userData` to `0` in the `else` block of your `if`statement.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\);?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null;?\s*userData\.songCurrentTime\s*=\s*0;?\s*\};?/)
```
