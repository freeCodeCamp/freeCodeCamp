...
If there is no next song in the playlist, use the `else` block to reset the `userData` object by setting `songs` to `userData?.songs`, `currentSong` to `null`, and `songCurrentTime` to `0`.
