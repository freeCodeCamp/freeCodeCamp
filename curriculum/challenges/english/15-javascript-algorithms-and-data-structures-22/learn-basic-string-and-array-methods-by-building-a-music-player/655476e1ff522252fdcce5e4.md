...
Add an `else` block to get the position of the currently playing song. Make use of the `getCurrentSongIndex()` function to get the index of the currently playing song. 

Call the `getCurrentSongIndex()` function and store the returned value in a constant varibale named `currentSongIndex`.
