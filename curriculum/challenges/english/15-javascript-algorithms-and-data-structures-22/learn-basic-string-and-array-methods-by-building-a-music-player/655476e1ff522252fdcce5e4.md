...
assert.match(splitter[0], /if\s*\(userData\?\.currentSong\s*===\s*null\)\s*\{\s*playSong\(userData\?\.songs\s*\[\s*0\s*\]\s*\.id\);?\s*\}/)
```
...
assert.match(splitter[0], /if\s*\(userData\?\.currentSong\s*===\s*null\)\s*\{\s*playSong\(userData\?\.songs\s*\[\s*0\s*\]\s*\.id\);?\s*\}\s*else\s*\{\s*/)
```
