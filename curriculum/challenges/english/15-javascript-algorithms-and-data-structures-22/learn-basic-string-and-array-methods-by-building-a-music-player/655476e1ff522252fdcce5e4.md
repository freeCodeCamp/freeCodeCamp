...
Add an `else` block to to the if statetment. Inside the `else` block, call the `getCurrentSongIndex()` function and save the it in a constant named `currentSongIndex`.
