...
```js
assert.match(code, /const\s+sortedSongList\s*=\s*userData\?\.songs\.sort\(\s*\(\s*a\s*,\s*b\s*\)\s*=>\s*{\s*}\s*\);?/);
```
...
--fcc-editable-region--
const sortedSongList = userData?.songs.sort();
--fcc-editable-region--
