...
// Play NExt Song Changes
const playNextSong = () => {
  const currentSongIndex = getCurrentSongIndex();

  // If the current song is the last in the playlist, do nothing
  if (currentSongIndex + 1 >= userData?.songs.length) return;

  // Get the next song from the playlist
  const nextSong = userData?.songs[currentSongIndex + 1];

  // If a next song exists, play it
  if (nextSong) {
    playSong(nextSong.id);
  } else {
    // If no next song exists, loop back to the first song
    playSong(userData?.songs[0].id);
  }
};

// Play Previous Song Changes

const playPreviousSong = () => {
  const currentSongIndex = getCurrentSongIndex();

  // If the current song is the first song, we can either:
  // - Do nothing, or
  // - Loop to the last song (uncomment the next line for that behavior)
  if (currentSongIndex === 0) {
    playSong(userData?.songs[0].id);  // Or loop back to the last song
    // playSong(userData?.songs[userData.songs.length - 1].id); // Uncomment to loop
...
const deleteSong = (id) => {
  ...
  // Filter out the deleted song from the playlist
  ...
  // If the deleted song is currently playing, handle player state
  if (userData?.currentSong?.id === songToDelete?.id) {
    if (userData?.songs.length > 0) {
      // If there are more songs, play the next one
      playNextSong();
    } else {
      // No songs left, reset the player state
      userData.currentSong = null;
      userData.songCurrentTime = 0;
      pauseSong();
    }
