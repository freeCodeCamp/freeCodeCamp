...
You need to assign the `userData.songs` property to the initial `allSongs` array. This will reset the playlist to its original state. Make use of the spread operator to create a new array and assign it to `userData.songs`.

Inside the callback, use the spread operator to create a new array containing the elements from `allSongs` and assign this new array to `userData.songs`.
