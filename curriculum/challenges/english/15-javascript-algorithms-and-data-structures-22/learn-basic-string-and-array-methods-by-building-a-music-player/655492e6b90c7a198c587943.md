...
Create an if statement with the condition `songToHighlight`. For the statement, use `setAttribute` on `songToHiglight` to pass in `"aria-current"` and `"true"` as the first and second parameters.
