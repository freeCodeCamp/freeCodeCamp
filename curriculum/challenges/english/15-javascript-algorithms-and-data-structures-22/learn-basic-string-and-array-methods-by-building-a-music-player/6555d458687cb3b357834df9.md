...
You will need to obtain the current song's title and artist from the `userData` object and assign them to the `currentTitle` and `currentArtist` variables. 

Access the `userData?.currentSong?.title` and `userData?.currentSong?.artist` properties and assign them to the `currentTitle` and `currentArtist` variables respectively.
