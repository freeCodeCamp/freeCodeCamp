...
Inside the `playSong` function, add the <dfn>find()</dfn> method. The `find` method returns the first element of an array that satisfies the function passed to it. If no element can be found, then it will return `undefined`.

For example, below is an array named `numbers` that contains five numbers. The `find()` method is used to find the first number greater than 25.
...
Use `const` to create a variable named `song` and assign it to the `find()` method. This method scans the `userData?.songs` array and searches for a song that matches the `id` passed into the `playSong` function.
