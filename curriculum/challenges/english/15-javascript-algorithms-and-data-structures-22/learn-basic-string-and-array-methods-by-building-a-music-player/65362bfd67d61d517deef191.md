...
The <dfn>find()</dfn> method returns the first element of an array that satisfies the callback function passed to it. If no element is found, then it will return `undefined`.
...
Use `const` to create a variable named `song` and assign it to the `find()` method. Use pass in `song` as the parameter of the `find()`callback and check if `song.id` is strictly equal to `id`.

This wiil scan through the `userData?.songs` array and searches for a song that matches the `id` passed into the `playSong` function.
...
Your `find` method should have `song` as the parameter of its arrow function callbakc.
...
```js
assert.match(code, /userData\?\.songs\.find\(\(song\)\s=>\s*song.id\s===\s*id\);?/)
```

Your `find` method should be assigned to a `song` constant.

```js
assert.match(code, /const\s*song\s*=\s*userData\?\.songs\.find\(\(song\)\s=>\s*song.id\s===\s*id\);?/)
```
