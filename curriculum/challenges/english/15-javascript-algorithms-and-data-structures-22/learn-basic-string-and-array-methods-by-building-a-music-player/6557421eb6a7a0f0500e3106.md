...
Within the button element in the `renderSongs` function, add an `onclick` attribute as the last attribute. For the value, call the `deleteSong` function and interpolate `song.id`.
...
```js
assert.match(code, /<button\s+class=('|")playlist-song-delete\1\s*aria-label=\1Delete\s*\$\{song\.title\}\1\s*onclick=\1deleteSong\(\$\{song\.id\}\)\1>/)
```
