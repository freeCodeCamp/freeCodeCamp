...
You should add an `onclick` attribute as an attribute of the delete button and pass in `deleteSong(${song.id})`.

```js
assert.match(code, /<button(?=.*\bclass="playlist-song-delete")(?=.*\baria-label="Delete [^"]*")(?=.*\bonclick="deleteSong\([^)]*\)")\s+(?:class|aria-label|onclick)="[^"]*"\s+(?:class|aria-label|onclick)="[^"]*"\s+(?:class|aria-label|onclick)="[^"]*"\s*>/)
```
