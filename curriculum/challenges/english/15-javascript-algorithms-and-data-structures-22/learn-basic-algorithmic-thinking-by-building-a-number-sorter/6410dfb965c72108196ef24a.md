...
Because you need to compare elements, you'll need to use a nested `for` loop. This loop should iterate through every element in the array *except* the last one. Use `j` for your inner loop's iterator variable.
...
Your inner `for` loop should initialise `j` to `0`.
