...
```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.querySelectorAll\(\s*('|"|`)\.values-dropdown\1\s*\)\s*\]\s*\.map\(/);
```

Your `map` callback function should use arrow function syntax.

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.querySelectorAll\(\s*('|"|`)\.values-dropdown\1\s*\)\s*\]\s*\.map\s*\(\s*\(?(dropdown)?\)?\s*\)\s*=>/);
```
...
```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.querySelectorAll\(\s*('|"|`)\.values-dropdown\1\s*\)\s*\]\s*\.map\s*\(\s*\(?dropdown\s*\)?\s*=>\s*Number\s*\(\s*dropdown\.value\s*\)/);
```
