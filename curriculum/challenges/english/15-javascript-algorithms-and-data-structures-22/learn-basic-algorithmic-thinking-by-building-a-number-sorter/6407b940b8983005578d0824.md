...
You need to get the values from your `select` elements. These values will currenly be strings and you will convert them into numbers.

Use the `map` function to iterate over the array. Pass a callback function to `map` that takes a `dropdown` perimeter and returns `dropdown.value`.
