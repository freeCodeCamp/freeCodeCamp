...
Update the `inputValues` map method by using the `Number()` function on the `dropdown.value`.

Check the console to see the `inputValues` array like you did in the previous step. 

You have converted the previous string values into numbers.

# --hints--

Use the `Number()` function on the `dropdown.value`.
