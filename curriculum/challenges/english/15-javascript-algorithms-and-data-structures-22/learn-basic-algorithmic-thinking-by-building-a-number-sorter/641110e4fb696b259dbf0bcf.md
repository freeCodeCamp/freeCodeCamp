...
- Second, it should not run after it finds a value smaller than the current value.
...
Your `while` loop should have it's first condition that checks the value of `j` is greater than or equal to `0`.
