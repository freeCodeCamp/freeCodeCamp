...
Remember that the `querySelector` method uses CSS selectors to find elements in the DOM. A hash symbol (`#`) is used to select elements by their `id` attribute.

# --hints--

You should fix the `document.querySelector("bg-hex-code")` line so it selects the correct element by its `id`.
