...
Remember that `getRandomIndex` is a function. If you want to use a value returned by a function, you need to call the function.

# --hints--

You should call the function inside the `darkColorsArr[getRandomIndex]` line to get the random color from the `darkColorsArr` array.

```js
assert.match(changeBackgroundColor.toString(), /darkColorsArr\[getRandomIndex\(\)\]/);
```
