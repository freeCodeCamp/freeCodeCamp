...
If the user rolls three of one number, and two of another number, this is called a full house. You will need to declare a `detectFullHouse` function that accepts an array argument to check for this.
...
```js
assert.match(code, /const\s+detectFullHouse\s*=\s*(\(\s*\w+\s*\))\s*=>\s*/);
```
