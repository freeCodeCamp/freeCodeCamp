...
for (let attempt = 0; attempt < 10; attempt++) {
  // Reset rolls count for each attempt
  totalRolls = 0;
  
  // Simulate a dice roll by clicking the roll button
  rollDiceBtn.click();
  
  // Store the current score display states before checking for straights
  const previousScoreDisplays = [...scoreSpans].map(span => span.innerText);
  
  // Execute the function that checks for straights based on the rolled dice values
  checkForStraights(diceValuesArr);
  
  // Validate that the score displays remain unchanged after the check
  assert.deepEqual([...scoreSpans].map(span => span.innerText), previousScoreDisplays, "Score displays should not change unexpectedly.");
}
```
