...
```js
assert.notMatch(String(code), /(var|let|const)\s+countdown\s*=\s*\(\s*\)\s*=>\s*\{[\s\S]+\}/);
```
...
```js
assert.match(String(code), /else\s*\{\s*countDownAndUp\(\s*number\s*-\s*1\s*\)/);
```

You should call `countDownAndUp()` with an argument of `3`.

```js
assert.match(String(code), /countDownAndUp\(\s*3\s*\)/);
```
