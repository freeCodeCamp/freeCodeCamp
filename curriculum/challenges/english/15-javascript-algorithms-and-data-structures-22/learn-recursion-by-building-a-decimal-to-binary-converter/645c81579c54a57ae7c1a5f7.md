...
```js
assert.match(String(code), /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=/);
```

You should use `Math.floor()` to round the quotient down.

```js
assert.match(String(code), /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=\s*Math\.floor\(/);
```

You should set `input` equal to the quotient of `input` divided by `2`, rounded down, within your `while` loop.

```js
assert.match(String(code), /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=\s*Math\.floor\(\s*input\s*\/\s*2\s*\)\s*;?\s*\}/);
```
