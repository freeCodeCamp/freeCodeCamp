...
```js
assert.match(String(checkUserInput), /console\.log\(\s*numberInput/);
```
...
```js
assert(
  String(checkUserInput).match(/console\.log\(\s*numberInput\.value\s*\)/) ||
  String(checkUserInput).match(/console\.log\(\s*numberInput\s*\[\s*('|"|`)value\1\s*\]\s*\)/)
);
```
