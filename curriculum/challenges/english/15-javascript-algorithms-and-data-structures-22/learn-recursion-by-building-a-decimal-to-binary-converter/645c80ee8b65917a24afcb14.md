...
Create a `while` loop that runs as long as `input` is greater than `0`. Inside the loop, set `input` equal to `0` for now.
...
Inside the loop, you should set `input` equal to `0`.

```js
assert.match(String(decimalToBinary), /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=\s*0\s*;?\s*\}/);
```
