...
assert.match(
  code,
  /animationContainer\.innerHTML\s*\+=\s*`\s*<p\s+id\s*=\s*('|"|`)\s*\$\{\s*obj\.inputVal\s*\}\s*\1\s*>\s*<\s*\/\s*p\s*>\s*`|animationContainer\.innerHTML\s*\+=\s*`\s*<p\s+id\s*=\s*('|"|`)\s*\$\{\s*obj\[\s*('|"|`)\s*inputVal\s*\3\s*\]\s*\}\2\s*>\s*<\s*\/\s*p\s*>\s*`/
);
```
