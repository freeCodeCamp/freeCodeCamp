...
```js
assert.match(String(showAnimation), /setTimeout\(/);
```

You should pass a callback function as the first argument to the `setTimeout` function.

```js
assert.match(String(showAnimation), /setTimeout\(\s*\(\s*\)\s*=>\s*\{?|setTimeout\(\s*function\s*\(\s*\)\s*\{/);
```

Within the callback function, you should log the text `Code` to the console.

```js
assert.match(String(showAnimation), /setTimeout\([\s\S]+\{?\s*console\.log\(\s*('|"|`)\s*Code\s*\1\s*\)/i);
```

You should pass `1000` as the second argument to the `setTimeout` function.

```js
assert.match(String(showAnimation), /setTimeout\([\s\S]+\}?\s*,\s*1000\s*\)/);
```
