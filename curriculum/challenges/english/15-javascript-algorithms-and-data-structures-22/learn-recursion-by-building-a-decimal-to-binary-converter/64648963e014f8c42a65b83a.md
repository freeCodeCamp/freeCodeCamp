...
The object at the end of the `animationData` array should have an `inputVal` property set to the number `1`, a `marginTop` property set to the number `-200`, and an `addElDelay` property set to the number `2000`.

```js
assert.deepEqual(animationData[2], {
  inputVal: 1,
  marginTop: -200,
  addElDelay: 2000,
});
