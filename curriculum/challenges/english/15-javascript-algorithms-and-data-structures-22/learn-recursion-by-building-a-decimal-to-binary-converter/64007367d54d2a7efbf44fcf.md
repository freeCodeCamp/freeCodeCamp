...
Then, add another condition to check if the value returned by `parseInt()` is less then `0`.  This will make sure only positive numbers are accepted by the `checkUserInput` function.
...
```js
assert.match(String(checkUserInput), /if\s*\(\s*!\s*numberInput\s*\.\s*value\s*\|\|\s*isNaN\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*\)/);
```

You should wrap the value returned by the condition check `numberInput.value` is less than `0` using the `parseInt()` method.

```js
assert.match(String(checkUserInput), /if\s*\(\s*!\s*numberInput\s*\.\s*value\s*\|\|\s*isNaN\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*\)\s*\|\|\s*parseInt\(\s*numberInput\s*\.\s*value\s*\<\s*0\s*\)\s*\)\s*\{/);
```
