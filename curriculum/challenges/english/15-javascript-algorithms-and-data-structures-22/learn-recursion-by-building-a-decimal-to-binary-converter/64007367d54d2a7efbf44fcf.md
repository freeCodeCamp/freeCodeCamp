...
Then, add another condition to check if the value returned by `parseInt()` is less than `0`.  This will make sure only positive numbers are accepted by the `checkUserInput` function.
...
For the condition check `numberInput.value` is less than `0`, wrap `numberInput.value` using the `parseInt()` method.
