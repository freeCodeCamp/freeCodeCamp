...
Your object should have an `inputVal` property set to the number `5`, a `marginTop` property set to the number `300`, and an `addElDelay` property set to the number `1000`.

```js
assert.deepEqual(animationData[0], {
  inputVal: 5,
  marginTop: 300,
  addElDelay: 1000,
});
