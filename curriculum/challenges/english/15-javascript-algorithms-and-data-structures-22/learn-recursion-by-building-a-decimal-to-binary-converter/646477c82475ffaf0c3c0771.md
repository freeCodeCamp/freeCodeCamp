...
```js
assert.lengthOf(
  String(showAnimation).match(/setTimeout\(\s*\(\s*\)\s*=>\s*\{?|setTimeout\(\s*function\s*\(\s*\)\s*\{/g),
  3
);
```

Within the new `setTimeout()` functions callback, you should log the text `Camp` to the console.

```js
assert.match(String(showAnimation), /setTimeout\([\s\S]+\{?\s*console\.log\(\s*('|"|`)\s*Camp\s*\1\s*\)/i);
```

You should pass `1500` as the second argument to the new `setTimeout()` function.

```js
assert.match(String(showAnimation), /setTimeout\([\s\S]+\}?\s*,\s*1500\s*\)/);
```
