...
```js
assert.match(String(code), /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*quotients\.push\(/);
```

You should use the `.push()` method to append `quotient` to the `quotients` array.

```js
assert.match(String(decimalToBinary), /quotients\.push\(\s*quotient\s*\);?/);
```

You should use the `.push()` method on the `remainders` array within your `while` loop.

```js
assert.match(String(code), /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*remainders\.push\(/);
```

You should use the `.push()` method to append `remainder` to the `remainders` array.

```js
assert.match(String(decimalToBinary), /remainders\.push\(\s*remainder\s*\);?/);
```
