...
You should declare variables named `inputs`, `quotients`, and `remainders`, and assign an empty array to each of them.

```js
assert.match(String(decimalToBinary), /(var|let|const)\s+inputs\s*=\s*\[\s*\]\s*;?/);
assert.match(String(decimalToBinary), /(var|let|const)\s+quotients\s*=\s*\[\s*\]\s*;?/);
assert.match(String(decimalToBinary), /(var|let|const)\s+remainders\s*=\s*\[\s*\]\s*;?/);
```
