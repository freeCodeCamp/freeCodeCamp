...
```js
assert.match(String(code), /let\s+binary\s*/);
```

You should create a variable named `binary` inside your `decimalToBinary` function.

```js
assert.match(String(decimalToBinary), /function\s+decimalToBinary\s*\(\s*input\s*\)\s*\{[\s\S]*(var|let|const)\s+binary\s*=/);
```

You should assign `binary` an empty string.

```js
assert.match(String(decimalToBinary), /(var|let|const)\s+binary\s*=\s*('|"|`)\2\s*;?/);
```
...
assert.match(String(code), /const\s+decimalToBinary\s*=\s*\(\s*input\s*\)\s*=>\s*\{\s*(var|let|const)\s+binary\s*=\s*('|"|`)\2\s*;?\s*\}/);
```
