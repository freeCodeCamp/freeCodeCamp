...
```js
assert.notMatch(String(code), /(var|let|const)\s+callStack\s*=\s*\[\s*\]/);
```

You should remove the `a()`, `b()`, and `c()` functions from your code.

```js
assert.notMatch(String(code), /(var|let|const)\s+a\s*=\s*\(\s*\)\s*=>\s*\{[\s\S]+\}/);
assert.notMatch(String(code), /(var|let|const)\s+b\s*=\s*\(\s*\)\s*=>\s*\{[\s\S]+\}/);
assert.notMatch(String(code), /(var|let|const)\s+c\s*=\s*\(\s*\)\s*=>\s*\{[\s\S]+\}/);
```

Your code should not have a `console.log()` statement.

```js
assert.notMatch(String(code), /console\.log\([\s\S]+\)/);
```
