...
Now your `showAnimation()` function is set up. But if you look closely at your `checkUserInput()` function, you'll notice that it's not very DRY (DRY is a software development principle that stands for `Don't Repeat Yourself`) – you're calling `parseInt()` to convert `numberInput.value` into a number several times.
