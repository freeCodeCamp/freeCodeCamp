...
Now, use the `getElementById` method to select the element whose `id` matches the `obj.inputVal` property of the current object. Then, set the `textContent` property of the selected element to the `msg` property of the same object.

# --hints--

You should use the `.getElementById()` method to select the element whose `id` matches the `inputVal` property of the current object, and chain `.textContent` to set the selected element’s text to the `msg` property of the same object.
...
You should use the `.getElementById()` method to select the element whose `id` matches the `inputVal` property of the current object.

```js
assert.match(
  String(showAnimation),
  /setTimeout\s*\([\s\S]+document\.getElementById\s*\(\s*obj\.inputVal\s*\)/
);
```

You should chain `.textContent` to the `.getElementById()` method and set it to the `msg` property of the current object.
