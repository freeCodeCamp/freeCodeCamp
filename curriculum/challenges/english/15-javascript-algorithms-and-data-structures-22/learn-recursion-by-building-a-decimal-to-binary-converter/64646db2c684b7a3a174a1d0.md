...
Within the new `setTimeout()` functions callback, you should log the text `free` to the console.

```js
assert.match(String(showAnimation), /setTimeout\([\s\S]+\{?\s*console\.log\(\s*('|"|`)\s*free\s*\1\s*\)/i);
```

You should pass `500` as the second argument to the new `setTimeout()` function.

```js
assert.match(String(showAnimation), /setTimeout\([\s\S]+\}?\s*,\s*500\s*\)/);
```
