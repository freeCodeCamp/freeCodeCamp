...
After your `h2` element, create two `p` elements. Give the first a `class` of `dessert-price`, and the dollar sign `"$"` followed by the value of the `price` variable. Give the second a `class` of `product-category`, and the text `"Category: "` followed by the value of the `category` variable.
