...
assert.match(cart.addItem.toString(), __helpers.concatRegex(/products\.find\(\s*/, functionRe));
```
...
```js
const cart = new ShoppingCart();
const functionRe = __helpers.functionRegex(null, ["item"], { includeBody: false });
assert.match(cart.addItem.toString(), __helpers.concatRegex(/products\.find\(\s*/), functionRe);
```
...
const cart = new ShoppingCart();
const functionRe = __helpers.functionRegex(null, ["item"], { includeBody: false });
const idCompRe = /item\.id\s*===\s*id|id\s*===\s*item\.id/; 
assert.match(cart.addItem.toString(), __helpers.concatRegex(/products\.find\(\s*/,functionRe,/\s*(return\s)?\s*/,idCompRe,/\s*;?\s*\}?/));
```
