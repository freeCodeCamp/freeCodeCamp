...
Using `localStorage.clear()` won't just delete an item from local storage but remove all items.

Remove `localStorage.removeItem()` and use `localStorage.clear()` instead. You don't need to pass in anything. You should also see `null` in the console too.
