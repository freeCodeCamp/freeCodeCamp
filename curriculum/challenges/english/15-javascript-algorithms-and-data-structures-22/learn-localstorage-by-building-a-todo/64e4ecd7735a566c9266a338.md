...
To start, add a `submit` event listener to your `taskForm` element and pass in `e` as the parameter of your arrow function. Inside an explicit return, use `e.preventDefault()` to stop the browser from refreshing the page after submitting the form.
