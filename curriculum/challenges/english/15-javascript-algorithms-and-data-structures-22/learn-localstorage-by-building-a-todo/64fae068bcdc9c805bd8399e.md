...
Now you should work on the edit and delete buttons for each task. Start by adding an `onclick` attribute to both buttons. Let the value of the `onclick` attribute be `editTask(this)` for the `Edit` button and `deleteTask(this)` for the `Delete` button. `editTask(this)` would handle editing and `deleteTask(this)` would handle deleting.
