...
Now follow the same process from the previous step for the task `date` and `description` you destructured. Don't forget your `strong` would now have the texts `Date:` and `Description`.
