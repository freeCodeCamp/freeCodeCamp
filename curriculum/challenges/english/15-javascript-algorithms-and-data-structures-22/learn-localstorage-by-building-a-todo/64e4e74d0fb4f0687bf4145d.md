...
Create a `taskData` constant and set it to an empty array. This is where all the tasks and their data (title, due date, and description) will be saved so you can track them, display them on the page, and save them to `localStorage`.

Also, use `let` to create a `currentTask` variable and set it to an empty object. You will use it to track the state when editing and discarding tasks.
