...
`splice()` is an array method that alters arrays by removing, replacing, or adding elements at a specified index, returning removed elements in the process. It takes up to 3 parameters – the first is the compulsory index to start, the second is the number of items to remove and the third is an optional replacement. Here's an example:
...
`dataArrIndex` is the index to start and `1` is the number of item to remove.
