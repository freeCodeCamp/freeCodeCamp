...
You also want a deleted task to be deleted in local storage. For this, you don't need the `removeItem()` or `clear()` methods. Since you already use `splice()` to remove the deleted task from `taskData`, all you need now is to save `taskData` in local storage again.

Use `setItem()` to save the `taskData` array again. Pass in `data` as the key and make sure `taskData` is stringified.
