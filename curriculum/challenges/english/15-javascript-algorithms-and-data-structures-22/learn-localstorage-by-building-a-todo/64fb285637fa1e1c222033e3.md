...
Now add `formInputValuesUpdated` as the second compulsory condition of the `if` statement. This way, the `Cancel` and `Discard` buttons modal won't be shown to the user if they make no changes to the input fields while trying to edit a task.
