...
You need to figure out whether the task being added exists or not. If it doesn't exist, you will add it, and if it already exists, you will set it up for editing. You can use the `findIndex()` method to do this.
...
Use the `findIndex()` method on the `taskData` array, then set it to a `dataArrIndex` variable. Use arrow syntax to pass in a callback and use `item` as the parameter, then check if the `id` of `item` is equal to the `id` of `currentTask`.
