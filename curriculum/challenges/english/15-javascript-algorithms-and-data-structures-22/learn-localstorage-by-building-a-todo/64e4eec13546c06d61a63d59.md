...
Use `const` to declare a `dataArrIndex` variable and set it to the `findIndex()` of `taskData`. Use arrow syntax to pass in a callback and use `item` as the parameter, then check if the `id` of `item` is equal to the `id` of `currentTask`.
