...
Create an `if` statement to check if `formInputsContainValues` is true. If `formInputsContainValues` is true, it means there are changes, so use the `showModal()` method on `confirmCloseDialog`. Else, it means there are no changes, so call `reset()` to clear the input fields and hide the form modal.
