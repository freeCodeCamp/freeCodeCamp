...
Create a `dataArrIndex` variable. For the value, use `findIndex()` on `taskData`, pass in `item` as the parameter of its arrow callback, then check if the `id` of `item` is equal to the `id` of the `parentElement` of `buttonEl`.
...
```js
assert.match(code, /const\s*deleteTask\s*=\s*\(buttonEl\)\s*=>\s*\{\s*const dataArrIndex\s*=\s*taskData\.findIndex\(\(item\)/)
```

Your arrow function callback should check if `item.id === buttonEl.parentElement.id`. Don't use curly braces.

```js
assert.match(code, /const\s*deleteTask\s*=\s*\(buttonEl\)\s*=>\s*\{\s*const dataArrIndex\s*=\s*taskData\.findIndex\(\(item\)\s*=>\s*item.id\s*===\s*buttonEl\.parentElement\.id\s*\);?/)
```
