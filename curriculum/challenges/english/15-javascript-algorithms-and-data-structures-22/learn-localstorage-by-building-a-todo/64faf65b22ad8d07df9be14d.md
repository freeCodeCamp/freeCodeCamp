...
Create a `dataArrIndex` variable. For the value, use `findIndex()` on `taskData`, pass in `item` as the parameter of its callback function, then check if the `id` of `item` is equal to the `id` of the `parentElement` of `buttonEl`.
