...
Start by adding a `forEach` to the `checkpoints` array. For the callback function, use `checkpoint`, and `index` for the parameters.
...
Your callback function should have a `checkpoint` parameter and `index`.

```js
assert.match(code, /\s*checkpoints\s*\.\s*forEach\s*\(\s*\(checkpoint,\s*index\)\s*=>\s*{/s);
```
