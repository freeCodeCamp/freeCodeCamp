...
You now you need to create a `draw` method for the `CheckPoint` class. 
...
```js
assert.match(code, /\s*draw\s*\(\s*\)\s*{\s*(.*\S)?\s*}\s*;/s);
```

Your `draw` method should have a `fillStyle` property.

```js
assert.match(code, /\s*ctx\.fillStyle\s*=\s*['"]#f1be32['"]\s*;?/);
```

Your `draw` method should have a `fillRect` method.

```js
assert.match(code, /\s*ctx\.fillRect\s*\(\s*this\.position\.x\s*,\s*this\.position\.y\s*,\s*this\.width\s*,\s*this\.height\s*\)\s*;?/);
```
