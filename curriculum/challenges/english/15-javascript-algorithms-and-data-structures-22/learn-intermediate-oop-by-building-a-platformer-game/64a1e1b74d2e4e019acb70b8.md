...
assert.match(player.update.toString(), /if\s*\(\s*this\.position\.y\s*\+\s+this\.height\s*\+\s+this\.velocity\.y\s*<=\s*canvas\.height\s*\)\s*{\s*if\s*\(\s*this\.position\.y\s*<\s*0\s*\)\s*{\s*}/);
```
