...
You should add an `else if` statement that checks if the left key was pressed and the player's `x` position is greater than 100.

```js
assert.match(code, /else\s+if\s*\(\s*keys\.leftKey\.pressed\s*&&\s*player\.position\.x\s*>\s*100\s*\)\s*{\s*/);
```

You should assign the number -5 to the player's `x` velocity inside the `else if`.

```js
assert.match(code, /player\.velocity\.x\s*=\s*-5\s*;?/);
```
