...
```js
assert.match(code, /if\s*\(collisionDetectionRules\.every\(\(rule\)\s*=>\s*rule\)\)\s*\{\s*/);
```
...
```js
assert.match(code, /if\s*\(collisionDetectionRules\.every\(\(rule\)\s*=>\s*rule\)\)\s*\{\s*player\.velocity\.y\s*=\s*0;?\s*return;?\s*\};?/);
```
