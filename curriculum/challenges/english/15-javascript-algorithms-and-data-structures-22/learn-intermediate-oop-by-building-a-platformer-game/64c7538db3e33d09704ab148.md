...
Start by creating a new `const` variable called `platformPositions` and assigning it an empty array.

# --hints--

You should have a `const` variable called `platformPositions`.

```js
assert.match(code, /const\s+platformPositions\s*=\s*\[\s*\]\s*;?/);
```

Your `platformPositions` variable should be assigned an empty array.
