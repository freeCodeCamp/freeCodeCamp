...
```js
assert.match(checkpointDetectionRules[4], /player\.position\.x\s*-\s*player\.width\s*<=\s*checkpoint\.position\.x\s*-\s*checkpoint\.width\s*\+\s*player\.width\s*\*\s*0\.9/i);
```

You should have a checkpoint detection rule that checks for the following: `index === 0 || checkpoints[index - 1].claimed === true`.

```js
assert.match(checkpointDetectionRules[5], /index\s*===\s*0\s*\|\|\s*checkpoints\[index\s*-\s*1\]\.claimed\s*===\s*true/i);
```
