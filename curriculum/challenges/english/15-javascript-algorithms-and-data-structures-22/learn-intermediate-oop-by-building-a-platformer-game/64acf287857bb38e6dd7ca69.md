...
You should have an if statement that checks if the right key was pressed and the player's position is less than 400.

```js
assert.match(code, /if\s*\(\s*keys\.rightKey\.pressed\s*&&\s*player\.position\.x\s*<\s*400\s*\)\s*{\s*/);
```
