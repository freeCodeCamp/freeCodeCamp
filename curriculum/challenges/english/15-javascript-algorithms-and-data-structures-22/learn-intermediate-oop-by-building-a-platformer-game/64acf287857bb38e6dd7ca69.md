...
You should have an `if` statement that checks if the right key was pressed and the player's `x` position is less than `proportionalSize(400)`. The `this` keyword should not be used here because that is only for the `Player` class and not the use of the `player` object.

```js
assert.notMatch(
    animate.toString(), /keys\.rightKey\.(pressed|pressed\s*===\s*true)\s*&&\s*this\.position\.x\s*<\s*proportionalSize\(\s*400\s*\)/
  );
