...
```js
assert.match(code, /const\s+platformDetectionRules\s*=\s*\[\s*player\.position\.x\s*>=\s*platform\.position\.x\s*-\s*player\.width\s*\/\s*2\s*,\s*player\.position\.x\s*<=\s*platform\.position\.x\s*\+\s*platform\.width\s*-\s*(?:player\.width\s*\/\s*3|\(\s*player\.width\s*\/\s*3\s*\)|player\.width\s*\*\s*1\s*\/\s*3|\(\s*player\.width\s*\*\s*1\s*\/\s*3\s*\)|1\s*\/\s*3\s*\*\s*player\.width|\(\s*1\s*\/\s*3\s*\*\s*player\.width\s*\))\s*,?\s*\]\s*;?/
)
```
