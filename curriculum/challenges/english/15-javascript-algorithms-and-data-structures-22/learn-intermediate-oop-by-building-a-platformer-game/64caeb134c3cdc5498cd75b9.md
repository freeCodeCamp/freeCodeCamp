...
```js
const xPositionCheck = /player\.position\.x\s*>=\s*platform\.position\.x\s*-\s*(?:player\.width\s*\/\s*2|\(\s*player\.width\s*\/\s*2\s*\)|player\.width\s*\*\s*0\.5|\(\s*player\.width\s*\*\s*0\.5\s*\)|0\.5\s*\*\s*player\.width|\(\s*0\.5\s*\*\s*player\.width\s*\))/;

const regexPattern = new RegExp(
  `const\\s+platformDetectionRules\\s*=\\s*\\[\\s*${xPositionCheck.source}\\s*,?\\s*\\]\\s*;?`
);
