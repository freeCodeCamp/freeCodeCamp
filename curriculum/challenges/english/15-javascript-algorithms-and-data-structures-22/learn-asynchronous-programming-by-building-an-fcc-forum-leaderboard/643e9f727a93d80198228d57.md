...
Inside the `p` element, add an embedded expression that contains the  `title` variable.
...
```js
assert(/return\s*`\s*<tr>\s*<td>\s*\S*\s*<p>\${title}\s*<\/p>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```
