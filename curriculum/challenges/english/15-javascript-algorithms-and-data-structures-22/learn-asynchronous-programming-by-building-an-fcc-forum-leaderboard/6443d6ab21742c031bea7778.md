...
Write another `if` statement inside the `timeAgo` function to check whether the constant `hoursAgo` is less than 24. If it is, then it will `return` the template literal `${hoursAgo}h ago`, showing the number of hours ago the post was created.
