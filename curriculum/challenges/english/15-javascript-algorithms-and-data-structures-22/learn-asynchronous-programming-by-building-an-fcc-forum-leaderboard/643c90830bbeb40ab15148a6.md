...
Call the `.map()` method on your `topics` array. Do not pass it a callback function yet. Then assign it to `postsContainer.innerHTML`
...
```js
assert.match(code, /postsContainer\.innerHTML\s*=\s*topics\.map\(.*/);
```
