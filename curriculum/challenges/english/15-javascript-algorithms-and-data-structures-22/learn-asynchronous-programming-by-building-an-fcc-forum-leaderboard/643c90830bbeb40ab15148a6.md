...
Start by calling the `map()` method on your `topics` array. For the callback function, pass in `item` for the parameter name.
...
```js
assert.match(code, /postsContainer\.innerHTML\s*=\s*topics\.map\(\s*(\(\s*item\s*\)|item)\s*=>\s*{/g);
```
