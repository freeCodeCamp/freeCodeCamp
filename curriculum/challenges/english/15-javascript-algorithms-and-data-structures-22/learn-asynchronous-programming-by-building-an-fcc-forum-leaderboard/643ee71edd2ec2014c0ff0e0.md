...
```js
assert(/return\s*`\s*<tr>\s*<td>\s*<p[^>]*class\s*=\s*['"]\s*post-title\s*['"][^>]*>\s*\${title}\s*<\/p>\s*<\/td>\s*<td>\s*<\/td>\s*<td>\s*\${posts_count\s*-\s*1}\s*<\/td>\s*<td>\s*\${views}\s*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```
