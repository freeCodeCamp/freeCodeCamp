...
You should use `const` to destructure the `topic_list` and `users` variables from the `data` object.
  
```js
assert(code.match(/const\s*{\s*(topic_list\s*,\s*users|users\s*,\s*topic_list)\s*}\s*=\s*data/));
```
