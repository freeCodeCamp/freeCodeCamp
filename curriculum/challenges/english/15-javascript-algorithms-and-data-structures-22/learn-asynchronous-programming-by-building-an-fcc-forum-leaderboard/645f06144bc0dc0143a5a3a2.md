...
Each forum post will include a list of user avatar images which represents all of the users participating in the conversation for that topic. 
...
```js
assert.match(code, /\s*const\s*avatars\s*=\s*\(\s*(posters|users)\s*,\s*(posters|users)\s*\)\s*=>\s*{\s*}/);
```
