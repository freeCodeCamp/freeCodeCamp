...
Each object inside the `topics` array contains a property called `user_id`. This `id` is used to display the user's avatar. 

Create an arrow function called `avatars` and pass in two parameters called `posters` and `users`.
