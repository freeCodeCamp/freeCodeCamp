...
```js
const anchorTagWithHrefClassAndTarget = /return\s+`<a[^>]*href\s*=\s*['"][^'"]*['"][^>]*class\s*=\s*['"]\s*\${linkClass}\s*['"][^>]*target=['"]_blank['"][^>]*>/g;
...
```js
const targetValue = /return\s+`<a[^>]*target=['"]_blank['"][^>]*>/g;
...
```js
const linkTextValue = /return\s+`<a[^>]*href\s*=\s*['"]\s*\${url}\s*['"][^>]*class\s*=\s*['"]\s*\${linkClass}\s*['"][^>]*target=['"]_blank['"][^>]*>\s*\${linkText}\s*<\/a>`/g;
