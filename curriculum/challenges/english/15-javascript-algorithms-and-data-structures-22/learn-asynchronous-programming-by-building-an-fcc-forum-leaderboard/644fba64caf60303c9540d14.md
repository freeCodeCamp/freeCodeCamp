...
After the `class` attribute, set the `target` attribute to the constant named `linkTarget`. 
...
```js
const anchorTagWithClass = /<a[^>]*(\sclass\s*=\s*['"][^'"]*['"])[^>]*>/;
...
```js
const classValue = /<a[^>]*class\s*=\s*['"]\s*\${linkClass}\s*['"][^>]*>/g;
...
```js
const anchorTagWithHrefClassAndTarget = /return\s+`<a[^>]*href\s*=\s*['"][^'"]*['"][^>]*class\s*=\s*['"]\s*\${linkClass}\s*['"][^>]*target\s*=\s*['"]\s*\${linkTarget}\s*['"][^>]*>/g;
...
```js
const targetValue = /return\s+`<a[^>]*target\s*=\s*['"]\s*\${linkTarget}\s*['"][^>]*>/g;
