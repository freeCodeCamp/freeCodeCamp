...
```js
assert.match(code, /<a\s+(?:class\s*=\s*('|")post-title\1\s+target\s*=\s*('|")_blank\2|target\s*=\s*('|")_blank\3\s+class\s*=\s*('|")post-title\4)(?:.|\n)*?<\/a>/);
```

Your opening anchor tag should have a `href` attribute set to `${forumTopicUrl}${slug}/${id}` after the `target` attribute.

```js
assert.match(code, /<a\s+class\s*=\s*('|")post-title\1\s+target\s*=\s*('|")_blank\2\s+href\s*=\s*('|")\$\{forumTopicUrl\}\$\{slug\}\/\$\{id\}\3\s*>([\s\S]*?)<\/a>/);
```
