...
To show some data in the Activity column, you need to use the `bumped_at` property of each topic. This property is a timestamp which is in the ISO 8601 format with the timezone information. You need to process this data before displaying it on the project. 
...
```js
assert.match(code, /\s*const\s+timeAgo\s*=\s*\(\s*time\s*\)\s*=>\s*{\s*}/);
```
