...
You need to set the `src` attribute of the image tag to the constant named `userAvatarURL`. This will make the image tag display the image from the specified URL Address.
...
```js
const srcValue = /return\s+`<img[^>]*src\s*=\s*['"]\s*\${userAvatarURL}\s*['"][^>]*\/>`/g;
