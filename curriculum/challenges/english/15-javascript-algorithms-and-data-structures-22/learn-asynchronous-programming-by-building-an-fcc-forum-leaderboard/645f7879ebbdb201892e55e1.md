...
```js
const hasStartsWith = code.includes('startsWith');
const regex = hasStartsWith
   ? /const\s+userAvatarUrl\s*=\s*avatar\.startsWith\(\s*('|")\/user_avatar\/\1\s*\)\s*\?\s*avatarUrl\.concat\(\s*avatar\s*\)\s*:\s*avatar\s*/
   : /const\s+userAvatarUrl\s*=\s*avatar\.test\(\s*\/(?:\^)?user_avatar\/\s*\)\s*\?\s*avatarUrl\.concat\(\s*avatar\s*\)\s*:\s*avatar\s*/;
