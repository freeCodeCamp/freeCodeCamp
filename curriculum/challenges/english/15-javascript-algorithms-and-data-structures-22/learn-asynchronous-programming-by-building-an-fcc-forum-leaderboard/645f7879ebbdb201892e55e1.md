...
```js
assert(code.match(/const\s+userAvatarUrl\s*=\s*(\/\^user_avatar\/\.test\(avatar\)\s*\?\s*avatarUrl\.concat\(avatar\)\s*:\s*avatar;?|avatar\.startsWith\(\s*('|")\/user_avatar\/\2\s*\)\s*\?\s*avatarUrl\.concat\(\s*avatar\s*\)\s*:\s*avatar\s*)/));
```
