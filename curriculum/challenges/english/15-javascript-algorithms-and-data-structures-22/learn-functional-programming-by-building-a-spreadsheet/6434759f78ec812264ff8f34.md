...
In your callback, make two function calls. Call `createLabel()` and pass `number` as the argument. You should see some numbers appear in your spreadsheet.
...
Your callback function should be nested in `range(1, 99)`'s forEach callback

```js
assert.match(code, /letters\.forEach\(\s*\(?\s*letter\s*\)?\s*=>\s*\{\s*\}.*\s*\}\)/)
```
