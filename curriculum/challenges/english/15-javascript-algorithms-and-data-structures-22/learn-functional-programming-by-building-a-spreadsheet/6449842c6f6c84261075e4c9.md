...
Be sure to use an implicit return in both the `idToText` function and the callback function of `.find()`.
...
Your `idToText` function should return the result of calling the `.find()` method on your `cells` array.
