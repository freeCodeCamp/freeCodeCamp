...
Add a `random` property which takes the first numbers from an array and returns a random whole number that is between the smaller positive number (inclusive) in the array and exclusive of the sum of the first two numbers in the array. Use the `Math.floor()` and `Math.random()` functions to help.
