...
You should set an attribute using the syntax `el.attribute = value`.

```js
assert.match(code, /input\.aria[lL]abel\s*=/);

```

Property names should follow camel case when updating hyphenated HTML attribute values.
...
Attribute `aria-label` for the `input` element should be set to the same value as the `id` attribute.

```js
assert.match(code, /=\s*input\.aria[label]+\s*=\s*(input\.id|letter\s*\+\s*number)\s*;/i);
