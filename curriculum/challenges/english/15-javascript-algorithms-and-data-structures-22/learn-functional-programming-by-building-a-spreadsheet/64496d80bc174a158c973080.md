...
You should use return a value from the function. 

```js
assert.exists(median([2,4,6,8]));
```

If `length` is even, you should use the `sorted` array. 

```js
assert.strictEqual(median([4,8,6,2]),5); 
```
...
If `length` is odd, you should use the `sorted` array. 

```js
assert.strictEqual(median([10,8,2,4,6]),6); 
```
