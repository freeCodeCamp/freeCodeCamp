...
Declare a `sum` function that takes a `nums` argument, which will be an array of numbers. It should return the result of calling `reduce` on the array to sum all of the numbers.
...
```js
assert.match(code, /const\s*sum\s*=\(?\s*nums\s*\)?\s*=>/);
```
...
You should not pass anything to your `.reduce()` call.

```js
assert.match(code, /const\s*sum\s*=\(?\s*nums\s*\)?\s*=>\s*nums\.reduce\(\s*\)/);
```
