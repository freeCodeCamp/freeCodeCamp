...
Remove `console.log(res)` and return `res.json()` instead. Open your browser console to see what the `Response` object looks like.

# --hints--

You should remove the console log of the `res` parameter.

```js
assert.notMatch(code, /\.then\(\(?res\)?\s*=>\s*console\.log\(res\)\)/)
```

You should return `res.json()` in your `.then()`.
...
=======
  .then((res) => console.log(res))
>>>>>>> c9a273b68b (feat: wrote tests for the first twelve steps, also corrected step 11 test)
