...
```js
assert.match(code, /<p\s*class=("|')bio\1>\$\{\s*bio\.length\s*>\s*50\s*\?\s*bio\.slice\(\s*0\,\s*50\s*\)\s*\+\s*("|')\.\.\.\2\s*\:|<p\s*class="bio">\$\{bio\.length\s*>\s*50\s*\?\s*`\$\{bio\.slice\(0,\s*50\)\}\.\.\.`\s*:\s*bio\}<\/p>/);
```

If the `bio` text is less than 50 characters, use the `bio` text directly.

```js
assert.match(code, /<p\s*class=("|')bio\1>\$\{\s*bio\.length\s*>\s*50\s*\?\s*bio\.slice\(\s*0\,\s*50\s*\)\s*\+\s*("|')\.\.\.\2\s*\:\s*bio\s*\}<\/p>|<p\s*class=("|')bio\1>\$\{\s*bio\.length\s*>\s*50\s*\?\s*("|')\$\{bio\.slice\(\s*0\,\s*50\s*\)\}...("|')\2\s*\:\s*bio\s*\}<\/p>|/)
```
