...
Now you need to show information about the author. First, show the author's name.
...
assert.equal(document.querySelectorAll('h2')[0]?.className, 'author-name');
```

You should interpolate the `author` in your `h2` tag to show the authors' names

```js
assert.match(code, /\$\{author\}/)
```
