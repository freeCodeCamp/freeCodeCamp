...
<<<<<<< HEAD
Now, it's time to pass a callback to the `forEach` so you can start populating the author items you destructured. You will put all of the items inside the `authorContainer` `div` in the HTML by using the `innerHTML` method of `DOM`.

Add a callback with a fat arrow inside the `forEach`, then add in a bracket and attach the `innerHTML` method to the `authorContainer` right inside the bracket.
=======
...
>>>>>>> 5941e62114 (chore: cross-checked tests and fixed broken ones)
