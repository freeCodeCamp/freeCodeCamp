...
for (const array of exampleAuthors) {
  const authorNamesArr = Object.values(array)
  
  displayAuthors(array);
  
  const authorDivs = document.querySelectorAll("#author-container div");
  const authorDivTitles = document.querySelectorAll("#author-container div h2");

  authorNamesArr.forEach((authorObj, index) => {
    console.log(authorObj, index);
    console.log(authorDivs[index].className, authorDivs[index].id, authorDivTitles[index].className, authorDivTitles[index].innerHTML);

    assert.equal(authorDivs[index].className, "user-card");
    assert.equal(authorDivs[index].id, index);
    assert.equal(authorDivTitles[index].className, "author-name"); 
    assert.equal(authorDivTitles[index].innerHTML, authorObj["author"]);
  }); 
