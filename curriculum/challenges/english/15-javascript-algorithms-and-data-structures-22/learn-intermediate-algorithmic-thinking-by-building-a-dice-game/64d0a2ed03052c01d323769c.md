...
Now call the function named `getHighestDuplicates` with argument `arr` inside the function named `findRollResult` so that everytime the user rolls the dice, it will detect and show the highest duplicate value.

# --hints--

You should call the function named `getHighestDuplicates` with argument `arr`.

```js
assert.match(code, /getHighestDuplicates\s*\(\s*arr\s*\)/);
```
