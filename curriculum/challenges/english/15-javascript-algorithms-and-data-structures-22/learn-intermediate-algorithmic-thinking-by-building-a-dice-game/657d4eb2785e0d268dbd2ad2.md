...
Inside your `if` statement, reassign `radioButton.value` to `selectedValue` and reassign `radioButton.id` to `achieved`. 
...
You should reassign `radioButton.value` to `selectedValue`.

```js
assert.match(code, /selectedValue\s*=\s*radioButton\s*\.\s*value/);
```

You should reassign `radioButton.id` to `achieved`.
