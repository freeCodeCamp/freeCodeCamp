...
You should declare a variable named `stringifyArray`.

```js
assert.match(code, /(?:var|let|const)\s+stringifyArray\s*=?\s*;?/);
```

You should join all the elements in the `uniqueNumbers` array into a string.

```js
assert.match(code, /(?:var|let|const)\s+stringifyArray\s*=?\s*uniqueNumbers\.join\(\s*['"]['"]\s*\);?/);
```
