...
Create an `else` statement and display `Please select an option or roll the dice` using `window.alert()`.

# --hints--

You should create an `else` statement and display `Please select an option or roll the dice` using `window.alert()`.

```js
assert.match(code, /else\s*{\s*window\.alert\s*\(\s*['"`]Please\s*select\s*an\s*option\s*or\s*roll\s*the\s*dice['"`]\s*\)\s*;?\s*}/);
```
