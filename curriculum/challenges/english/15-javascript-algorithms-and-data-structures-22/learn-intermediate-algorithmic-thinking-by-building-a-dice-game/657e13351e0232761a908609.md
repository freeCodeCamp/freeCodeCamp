...
Inside your `for...of` loop, you will need to check if the current value `counts[num]` is truthy. If so, then increment the count by one, otherwise  initializes `counts[num]` with a value of 1. You can use a `ternary` operator for this.
