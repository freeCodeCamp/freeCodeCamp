...
Again, Use the `optionNode` argument to target the `scoreInputs` and set its `value` property to `score` argument.
...
You should set the `value` property of the `scoreInputs[optionNode]` to `score` argument.
