...
```js
assert.match(code, /totalScoreText\s*\.\s*textContent\s*=\s*totalScore\s*;?\s*\n*scoreHistory\s*\.\s*innerHTML\s*=\s*('|"|`)\s*\1\s*;?/);
```

You should assign `rolls` to `currentRoundRollsText.textContent`.

```js
assert.match(code, /totalScoreText\s*\.\s*textContent\s*=\s*totalScore\s*;?\s*\n*scoreHistory\s*\.\s*innerHTML\s*=\s*('|"|`)\s*\1\s*;?\s*\n*currentRoundRollsText\s*\.\s*textContent\s*=\s*rolls\s*;?/);
```

You should assign `round` to `currentRoundText.textContent`.

```js
assert.match(code, /totalScoreText\s*\.\s*textContent\s*=\s*totalScore\s*;?\s*\n*scoreHistory\s*\.\s*innerHTML\s*=\s*('|"|`)\s*\1\s*;?\s*\n*currentRoundRollsText\s*\.\s*textContent\s*=\s*rolls\s*;?\s*\n*currentRoundText\s*\.\s*textContent\s*=\s*round\s*;?/);
```
