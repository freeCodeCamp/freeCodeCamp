...
Start by creating an `if` statement to check if any element in `smallStraightsArr` is included in `uniqueNumbersStr`. If so, call the `updateRadioOption` function with arguments `3` and `30`.
...
```js
assert.match(code, /if\s*\(\s*smallStraightsArr\.some\(\s*straight\s*=>\s*uniqueNumbersStr\.includes\(\s*straight\s*\)\s*\)\s*\)\s*{?/);
```

You should call your `updateRadioOption` with `3` and `30` for the arguments. 

```js
assert.match(code, /if\s*\(\s*smallStraightsArr\.some\(\s*straight\s*=>\s*uniqueNumbersStr\.includes\(\s*straight\s*\)\s*\)\s*\)\s*{\s*updateRadioOption\s*\(\s*3\s*,\s*30\s*\)\s*;\s*}/);
