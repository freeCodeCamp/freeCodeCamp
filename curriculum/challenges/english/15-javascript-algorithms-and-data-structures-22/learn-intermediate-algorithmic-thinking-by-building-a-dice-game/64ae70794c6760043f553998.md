...
Add an event listener of `click` to the variable named `keepScoreBtn`. This will be used to select and save one option from the given list of options on the display. Also, pass a function as an argument after the `'click'`.

# --hints--

You should add an event listener of `click` to the `keepScoreBtn` variable.

```js
assert.match(code, /keepScoreBtn\.addEventListener\(\s*"click"/);
```

You should pass a `function` as an argument after the `'click'`.

```js
assert.match(code, /keepScoreBtn\.addEventListener\(\s*"click"\s*,\s*function/);
```
