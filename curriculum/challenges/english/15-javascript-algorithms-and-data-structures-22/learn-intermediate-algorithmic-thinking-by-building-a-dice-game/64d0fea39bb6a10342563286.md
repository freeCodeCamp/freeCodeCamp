...
You should create an `if` statement to check if `hasThreeOfAKind` and `hasPair` are true or not. If so, then call the function named `updateRadioOption` with arguments `2` and `25`
