...
Below your `if` statement, include an `else` clause that increments the count for the first occurrence of that number in the `counts` object.
...
You should increment the value of `counts[num]` by `1` inside your `if` statement.
