...
```js
assert.match(code, /const\s+hasThreeOfAKind\s*=\s*Object\.values\(\s*counts\s*\).*/);
```

You should chain `.includes(3)` to your `Object.values(counts)`.

```js
assert.match(code, /const\s+hasThreeOfAKind\s*=\s*Object\.values\(\s*counts\s*\)\.includes\(\s*3\s*\)\s*;?/);
```
