...
To detect a full house, you need to create a function named `detectFullHouse` with an argument of `arr`. Then, create a variable named `counts` and set it to an empty object.

# --hints--

You need to create a function named `detectFullHouse` with an argument of `arr`.

```js
assert.match(code, /((const|let|var)\s*detectFullHouse\s*=\s*\(?\s*arr\s*\)?\s*=>|function\s*detectFullHouse\s*\(\s*arr\s*\)\s*)\s*{/);
```

You need to create a variable named `counts` and set it to an empty object.

```js
assert.match(code, /((function\s+detectFullHouse\s*\(arr\)\s*{\s*(let|var|const)\s*counts\s*=\s*{\s*}\s*;?\s*})|((const|let|var)\s+detectFullHouse\s*=\s*\(\s*arr\s*\)\s*=>\s*{\s*(let|var|const)\s*counts\s*=\s*{\s*}\s*;?\s*}))/);
```
