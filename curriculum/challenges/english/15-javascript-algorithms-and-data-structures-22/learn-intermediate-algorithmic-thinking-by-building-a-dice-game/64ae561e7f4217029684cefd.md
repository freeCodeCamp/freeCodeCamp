...
You will call the function named `findRollResult` with argument of `displayArray` inside the event listener for the `rollDiceBtn` button. This will display the results of the roll on the screen every time the user clicks the roll dice button

# --hints--

You should call the function `findRollResult` with the argument of `displayArray`.

```js
assert.match(code, /findRollResult\s*\(\s*displayArray\s*\)/);
```
