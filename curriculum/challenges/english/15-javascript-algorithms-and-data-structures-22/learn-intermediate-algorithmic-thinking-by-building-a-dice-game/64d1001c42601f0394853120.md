...
The last step of this project is to call the function named `detectFullHouse` with argument of `arr` inside the function named `findRollResult`

# --hints--

You should call the function named `detectFullHouse` with argument of `arr` inside the function named `findRollResult`

```js
assert.match(code, /detectFullHouse\s*\(\s*arr\s*\)\s*;?/);
```
