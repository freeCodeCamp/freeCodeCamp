...
Using an `if` statement, you should check if the `num` key exists in the `counts` object. If it does, then increment the value of `counts[num]` by 1. If it doesn't, then set `counts[num]` to 1 using an `else` statement

# --hints--

You should create an `if` statement to check if the `num` key exists in the `counts` object.

```js
assert.match(code, /if\s*\(\s*counts\[num\]\s*\)/);
```

You should increment the value of `counts[num]` by 1 if the `num` key exists in the `counts` object.

```js
assert.match(code, /counts\[num\]\s*\+\s*=\s*1|counts\[num\]\+\+/);
```

You should set `counts[num]` to 1 if the `num` key doesn't exist in the `counts` object using an `else` statement.
