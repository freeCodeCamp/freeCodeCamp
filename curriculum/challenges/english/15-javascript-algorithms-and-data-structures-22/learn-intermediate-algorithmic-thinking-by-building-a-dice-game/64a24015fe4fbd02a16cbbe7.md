...
NEED HELP ! The test is not passing, any ideas? 
...
You should create a variable named `uniqueNumbers` and assign to it the `new Set()` constructor and pass to it the `sortNumbers` array

```js
assert.match(code, /(var|let|const)\s+uniqueNumbers\s*=\s*(?:new\s+Set\(\s*sortNumbers\s*\)|\[\s*(?:\.\.\.|new\s+)?Set\(\s*sortNumbers\s*\)\s*\]);?/);
```

You should spread the values in an array using the spread operator `...` 

```js
assert.match(code, /(var|let|const)\s+uniqueNumbers\s*=\s*\[\s*\.\.\.\s*new\s+Set\(\s*sortNumbers\s*\)\s*\];?/);
```
