...
Create an `if` statement that checks if `highestCount` is more than or equal to `4`. If so, it will call the function named `updateRadioOption` with arguments `1` and `sumOfAllDices`.

# --hints--

You should create an `if` statement that checks if `highestCount` is more than or equal to `4`.

```js
assert.match(code, /if\s*\(\s*highestCount\s*>=\s*4\s*\)\s*\{\s*[\s\S]*?\}/);
```

You should call the function named `updateRadioOption` with arguments `1` and `sumOfAllDices`.

```js
assert.match(code, /updateRadioOption\s*\(\s*1\s*,\s*sumOfAllDices\s*\)/);
```
