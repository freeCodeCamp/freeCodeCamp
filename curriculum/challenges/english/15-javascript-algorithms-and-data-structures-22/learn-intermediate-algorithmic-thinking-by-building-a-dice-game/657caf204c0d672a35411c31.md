...
Start by creating an arrow function called `updateRadioOption` that takes an index and a score value as arguments. It should set the `scoreInputs` at that index to be enabled, set the value of that input to the score, and display `score = ${score}` in the correct `scoreSpans` element.
...
assert.isTrue(scoreInputs[0].enabled);
```
