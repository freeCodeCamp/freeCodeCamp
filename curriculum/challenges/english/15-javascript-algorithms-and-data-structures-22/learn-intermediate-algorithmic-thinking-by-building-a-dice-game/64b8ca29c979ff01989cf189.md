...
You need to go back to the `resetGame` function and add a couple of more lines of code so that it works smoothly. First, using `innerHTML` property, set the `totalScoreText` to `totalScore`, and `scoreHistory` to an empty string. Lastly, call the `resetRadioOption` function.

# --hints--

You should use the `innerHTML` property to set the `totalScoreText` to `totalScore`.
...
You should call the `resetRadioOption` function inside the function named `resetGame`.
