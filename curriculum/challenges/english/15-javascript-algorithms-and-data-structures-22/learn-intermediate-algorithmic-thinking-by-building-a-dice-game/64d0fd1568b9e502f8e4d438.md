...
Like the last step, you should use the `Object.values()` to check if `counts` variable includes a value of `2` and then store the result in a variable called `hasPair`.

# --hints--

You should declare a variable named `hasPair` and assign it the value of `Object.values(counts).includes(2)`

```js
assert.match(code, /(var|let|const)\s+hasPair\s*=\s*Object\.values\(counts\)\.includes\(2\)\s*;?/);
```
