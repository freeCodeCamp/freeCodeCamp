...
Inside the `if` statement you will target the `display` property on `rulesContainer` and set it to `block`. Then, on the `else` statement, you will target the `display` property on `rulesContainer` and set it to `none`.
