...
The `arr` argument represents the results of the dice roll. You will use a `for` loop to iterate through each element. Within the loop, create a variable named `num` to represent the current element of the `arr` array. The main purpose is to count the occurrences of each number in the array so that we can detect a three of a kind or four of a kind.

# --hints--

You should Create a `for` loop to iterate through the `arr` array. Within the loop, create a variable named `num` to represent the current element of the `arr` array.
