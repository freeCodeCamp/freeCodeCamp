...
Again, create an `if` statement to check if `highestCount` is more than or equal to 3 this time. If so, then call the function named `updateRadioOption` with arguments of `0` and `sumOfAllDices`

# --hints--

You should create an `if` statement that checks if `highestCount` is more than or equal to `3`.

```js
assert.match(code, /if\s*\(\s*highestCount\s*>=\s*3\s*\)\s*\{\s*[\s\S]*?\}/);
```

You should call the function named `updateRadioOption` with arguments `0` and `sumOfAllDices`.

```js
assert.match(code, /updateRadioOption\s*\(\s*0\s*,\s*sumOfAllDices\s*\)/);
```
