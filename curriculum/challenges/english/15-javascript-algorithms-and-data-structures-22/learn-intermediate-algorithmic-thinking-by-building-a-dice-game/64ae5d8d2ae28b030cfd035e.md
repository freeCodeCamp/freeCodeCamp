...
You should apply the `forEach` method to the `scoreOptions` array with a callback function with an argument named `input`.
...
```js
assert.match(code, /scoreOptions\.forEach\(\s*(function\s*\(\s*input\s*\)|\(\s*input\s*\)\s*=>)\s*{/);
```
