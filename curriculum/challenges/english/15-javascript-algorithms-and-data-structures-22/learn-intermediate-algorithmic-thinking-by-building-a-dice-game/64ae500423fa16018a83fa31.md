...
For the final time, you should use the `optionNode` argument to target the `scoreInputs` and set its `textContent` property to `, score = ${score}`
...
You should set the `textContent` property of the `scoreInputs[optionNode]` to `, score = ${score}`, make sure to use template literals

```js
assert.match(code, /scoreInputs\[\s*optionNode\s*\]\.textContent\s*=\s*`, score = \${score}`/);
```
