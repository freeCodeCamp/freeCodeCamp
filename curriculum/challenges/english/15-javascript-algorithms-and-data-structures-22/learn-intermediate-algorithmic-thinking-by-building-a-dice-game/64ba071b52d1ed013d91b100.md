...
You need to create a function named `getHighestDuplicates` with an argument of `arr`, so that the game can detect if the player has a three of a kind or four of a kind. Then, create a variable named `counts` and set it to an empty object. Next, create a variable named `highestCount` and set its value to `0`

# --hints--

You need to create a function named `getHighestDuplicates` with an argument of `arr`.

```js
assert.match(code, /function\s*getHighestDuplicates\s*\(\s*arr\s*\)\s*{/);
```

You need to create a variable named `counts` and set it to an empty object.

```js
assert.match(code, /(let|var|const)\s*counts\s*=\s*{\s*};?/);
```

You need to create a variable named `highestCount` and set its value to `0`.

```js
assert.match(code, /(let|var|const)\s*highestCount\s*=\s*0\s*;?/);
```
