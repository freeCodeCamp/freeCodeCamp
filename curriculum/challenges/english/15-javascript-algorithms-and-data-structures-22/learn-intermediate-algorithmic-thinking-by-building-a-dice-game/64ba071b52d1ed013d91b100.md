...
Create a function named `getHighestDuplicates` with an argument of `arr`, to detect if the player has a three of a kind or four of a kind. Then, create a variable named `counts` and set it to an empty object. Next, create a variable named `highestCount` and set its value to `0`.
