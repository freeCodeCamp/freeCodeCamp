...
You need to run the newly created `straightDetector` function to see the results on the display. To do this, create a function named `findRollResult` and set the argument to `arr`. Then, call the function `straightDetector` with the argument of `arr`.

# --hints--

You should create a function named `findRollResult` with an argument of `arr`.
...
```js
assert.match(code, /(function\s*findRollResult\s*\(\s*arr\s*\)\s*{\s*straightDetector\s*\(\s*arr\s*\)\s*}|const\s*findRollResult\s*=\s*\(arr\)\s*=>\s*{\s*straightDetector\s*\(\s*arr\s*\)\s*})/);
```
