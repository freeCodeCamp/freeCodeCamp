...
Use the `reduce` method on the `diceValuesArr`. For the callback function, use `a` and `b` for the parameter names. For the initial value, use the number `0`. Then assign the entire result to a `const` variable called `sumOfAllDices`.

# --hints--

You should have a `const` variable named `sumOfAllDices`.

```js
assert.match(code, /const\s*sumOfAllDices/);
```
...
```js
assert.match(code, /.*diceValuesArr\s*\.\s*reduce\s*\(\s*\(\s*a\s*,\s*b\s*\)\s*=>\s*a\s*\+\s*b\s*,\s*0\s*\)\s*;?/);
```

You should assign the result of the `reduce` method to the `sumOfAllDices` variable.

```js
assert.match(code, /const\s*sumOfAllDices\s*=\s*diceValuesArr\s*\.\s*reduce\s*\(\s*\(\s*a\s*,\s*b\s*\)\s*=>\s*a\s*\+\s*b\s*,\s*0\s*\)\s*;?/);
```
