...
To count the occurences of each number in an array, use a `for` loop. Within the loop, create a variable named `num` to represent the current element of the `arr` array.

# --hints--

You should use a `for` loop to iterate through the `arr` array. 

```js
assert.match(code, /const\s+detectFullHouse\s*=\s*\(arr\s*\)\s*=>\s*{.*\s*for\s*\(\s*(const|let|var)\s+num\s+of\s+arr\s*\)\s*{\s*.*}/s);
```
