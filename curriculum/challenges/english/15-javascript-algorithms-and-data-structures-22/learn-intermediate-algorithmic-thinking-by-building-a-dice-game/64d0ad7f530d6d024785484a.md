...
To count the occurences of each number, use a `for` loop to iterate through every element of the array `arr`. Within the loop, create a variable named `num` to represent the current element of the `arr` array.
