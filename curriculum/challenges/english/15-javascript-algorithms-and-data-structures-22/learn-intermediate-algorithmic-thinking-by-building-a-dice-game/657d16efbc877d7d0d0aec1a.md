...
Add an `if` statement that checks if `highestCount` is greater than or equal to `3`. If so, call the `updateRadioOption` and pass in the number `0` and `sumOfAllDices` for the arguments. 
...
You should have the number `0` and `sumOfAllDices` variable as arguments to your `updateRadioOption` function.
