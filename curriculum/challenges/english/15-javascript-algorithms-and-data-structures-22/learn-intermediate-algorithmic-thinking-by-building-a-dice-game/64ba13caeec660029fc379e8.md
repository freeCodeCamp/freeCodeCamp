...
Using the `reduce` array method, add all the elements of the `displayArray` array and assign the value to the `sumOfAllDices` variable.
...
You should use the `reduce` array method on the `displayArray` array.

```js
assert.match(code, /displayArray\s*\.\s*reduce/);
```

You should make sure to use `a,b` as the parameters of the callback function of the `reduce` method to add all the elements of the `displayArray` array.

```js
assert.match(code, /displayArray\s*\.\s*reduce\s*\(\s*\(\s*a\s*,\s*b\s*\)\s*=>\s*a\s*\+\s*b\s*/);
```
