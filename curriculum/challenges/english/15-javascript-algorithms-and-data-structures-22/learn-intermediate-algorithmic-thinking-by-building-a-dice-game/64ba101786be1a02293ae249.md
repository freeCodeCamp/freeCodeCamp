...
You should create an `if` statement with the condition `count >= 3 && count > highestCount`. This condition checks if the value of count is greater than or equal to 3 and if it's greater than the current `highestCount`. Inside the `if` statement, set `highestCount` to the value of `count`. This will update `highestCount` if the current `count` is the highest one encountered so far that is also greater than or equal to 3. The main purpose of this is to detect a three of a kind.

# --hints--

You should create an `if` statement with the condition `count >= 3 && count > highestCount`. 
