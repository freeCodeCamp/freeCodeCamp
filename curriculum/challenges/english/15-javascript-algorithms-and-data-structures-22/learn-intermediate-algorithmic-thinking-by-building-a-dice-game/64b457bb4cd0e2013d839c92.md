...
Inside the loop, use a `if` statement to check if the current element `radioButton` is checked using `.checked` property. If so, then reassign the `selectedValue` to `radioButton.value`. Then, reassign the `achieved` variable to `radioButton.id`. Finally add a `break` statement to exit the loop.
