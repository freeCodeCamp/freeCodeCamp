...
To display the options to select score, you will create a function named `updateRadioOption` and pass 2 arguments named `optionNode` and `score`
...
```js
assert.match(code, /function\s+updateRadioOption\s*\([^)]*\)\s*{\s*}/);
```

You should pass 2 arguments named `optionNode` and `score` to the `updateRadioOption` function.

```js
assert.match(code, /function\s+updateRadioOption\s*\(\s*optionNode\s*,\s*score\s*\)\s*{\s*}/);
```
