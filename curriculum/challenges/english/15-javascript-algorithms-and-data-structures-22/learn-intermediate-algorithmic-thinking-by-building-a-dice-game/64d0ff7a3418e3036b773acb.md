...
Finally, call the function named `updateRadioOption` with arguments of `5` and `0`. This will run when the conditions of the `if` statement are met.
...
```js
assert.match(code, /updateRadioOption\s*\(\s*5\s*,\s*0\s*\)\s*;?/);
```
