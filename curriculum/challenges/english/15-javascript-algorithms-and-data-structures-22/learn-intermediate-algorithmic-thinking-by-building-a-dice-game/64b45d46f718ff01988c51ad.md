...
```js
assert.match(code, /setTimeout\s*\(\s*function\s*\(\s*\)\s*{\s*[^}]*\balert\s*\(\s*`Game\s*over!\s*Your\s*total\s*score\s*is\s*\${totalScore}`\s*\)[^}]*\s*}\s*,\s*500\s*\)/);
```

You should call the `resetGame` function after the alert message.

```js
assert.match(code, /setTimeout\s*\(\s*function\s*\(\s*\)\s*{\s*alert\s*\(\s*`Game\s*over!\s*Your\s*total\s*score\s*is\s*\${totalScore}`\s*\)\s*;?\s*resetGame\s*\(\s*\)\s*;?\s*},\s*500\s*\)/);
```
