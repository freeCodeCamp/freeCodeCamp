...
Create an `if` statement to check if `round` is more than `6` or not. Then, create a setTimeout function to show the alert message after 500 milliseconds. The alert message should be `Game over! Your total score is ${totalScore}`. After the alert message, call the `resetGame` function to reset the game.
...
```js
assert.match(code, /setTimeout\s*\(\s*function\s*\(\s*\)\s*{\s*[\s\S]*?\s*}\s*,\s*500\s*\)/);
```

You should use `window.alert` to show the alert message stating `Game over! Your total score is ${totalScore}` inside the setTimeout function.

```js
assert.match(code, /setTimeout\s*\(\s*function\s*\(\s*\)\s*{\s*[^}]*\bwindow\.alert\s*\(\s*`Game\s*over!\s*Your\s*total\s*score\s*is\s*\${totalScore}`\s*\)[^}]*\s*}\s*,\s*500\s*\)/);
```

You should call the `resetGame` function after the alert message.

```js
assert.match(code, /setTimeout\s*\(\s*function\s*\(\s*\)\s*{\s*window\.alert\s*\(\s*`Game\s*over!\s*Your\s*total\s*score\s*is\s*\${totalScore}`\s*\)\s*;?\s*resetGame\s*\(\s*\)\s*;?\s*},\s*500\s*\)/);
```
