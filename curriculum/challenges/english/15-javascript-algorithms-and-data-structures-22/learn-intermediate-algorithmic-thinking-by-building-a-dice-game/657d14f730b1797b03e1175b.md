...
Add an `if` statement that checks if `highestCount` is greater than or equal to `4`. If so, call the `updateRadioOption` and pass in the number `1` and `sumOfAllDices` for the arguments. 
...
You should have the number `1` and `sumOfAllDices` variable as arguments to your `updateRadioOption` function.
