...
Using `Object.values()`, check if `counts` variable includes the value of `3` and save the result in a variable named `hasThreeOfAKind`

# --hints--

You should declare a variable named `hasThreeOfAKind` and assign it the value of `Object.values(counts).includes(3)`

```js
assert.match(code, /(var|let|const)\s+hasThreeOfAKind\s*=\s*Object\.values\(counts\)\.includes\(3\)\s*;?/);
```
