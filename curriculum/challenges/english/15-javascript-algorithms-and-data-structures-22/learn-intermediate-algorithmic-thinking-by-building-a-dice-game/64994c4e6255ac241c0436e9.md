...
The first thing you will do is set the initial values for the game. First, declare variables named `rolls`, `score`, and `totalScore` and set all of their values to `0`. Then, declare a variable named `round` and set its value to `1`. These variables will change throughout the code, so use `let` when declaring all of them.
