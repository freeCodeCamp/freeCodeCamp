...
Pass the string `input[type="text"]` to the `querySelectorAll()` method. Remember that if you use single quotes for your string, you must use double quotes within, and if you use double quotes for your string, then you must use single quotes within.
...
```js
assert.match(addEntry.toString(), /\.querySelectorAll\(\s*(["'])\s*input\s*\[\s*type\s*=\s*(["'])text\2\s*\]\s*\1\s*\)\.length/)
```

Your `entryNumber` variable should be the `length` of the `querySelectorAll`.

```js
assert.match(addEntry.toString(), /entryNumber\s*=\s*targetInputContainer\.querySelectorAll\(\s*(["'])\s*input\s*\[\s*type\s*=\s*(["'])text\2\s*\]\s*\1\s*\)\.length/)
```
