...
You should have a ternary operator that checks if `remainingCalories` is less than `0`.

```js
assert.match(calculateCalories.toString(), /remainingCalories\s*<\s*0\s*\?/g);
```

Your ternary operator return the string `Surplus` if `remainingCalories` is less than  `0`.

```js
assert.match(calculateCalories.toString(), /remainingCalories\s*<\s*0\s*\?\s*('|")Surplus\1\s*:/g);
```

Your ternary operator return the string `Deficit` if `remainingCalories` is not less than `0`.
