...
```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(('|")#lunch input\[\s*type\s*=\s*number\s*\]\s*\1\s*\)/);
```

You should assign the result of your `document.querySelectorAll()` call to `lunchNumberInputs`.

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=\s*document\.querySelectorAll\(('|")#lunch input\[\s*type\s*=\s*number\s*\]\s*\1\s*\)/);
```
