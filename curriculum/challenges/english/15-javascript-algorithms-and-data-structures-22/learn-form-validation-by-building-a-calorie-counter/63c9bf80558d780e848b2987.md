...
```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(('|")#breakfast input\[\s*type=number\s*\]\s*\1\s*\)/);
```

You should assign the result of your `odocument.querySelectorAll()` call to `breakfastNumberInputs`.

```js
assert.match(calculateCalories.toString(), /breakfastNumberInputs\s*=\s*document\.querySelectorAll\(('|")#breakfast input\[\s*type=number\s*\]\s*\1\s*\)/);
```
