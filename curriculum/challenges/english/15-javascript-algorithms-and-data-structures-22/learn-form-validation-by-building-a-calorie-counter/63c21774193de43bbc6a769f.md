...
```js
assert.match(code, /HTMLString\s*=\s*`\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>/)
```

Your `label` element should have the same text as before.

```js
assert.match(code, /HTMLString\s*=\s*`\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>Entry\s\$\{entryNumber\}\sName<\/label>/);
```
