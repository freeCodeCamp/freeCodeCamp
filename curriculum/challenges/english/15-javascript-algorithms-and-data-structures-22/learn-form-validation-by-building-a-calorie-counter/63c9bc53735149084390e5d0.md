...
You should display the first element in the `invalidInputMatch` array after `Invalid Input:`  text using template literal.

```js
assert.match(code.split(/function\s+getCaloriesFromInputs/)[1], /alert\(`Invalid Input: \${invalidInputMatch\s*\[\s*0\s*\]\s*}`\s*\)/);
```
