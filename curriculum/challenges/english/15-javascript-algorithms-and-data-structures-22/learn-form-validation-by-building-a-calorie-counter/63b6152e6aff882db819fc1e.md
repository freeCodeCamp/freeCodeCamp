...
When the user provides a daily calorie budget, the input will only accept numbers. However, if the user provides a number with a `+` or `-` sign, then you will need to filter those characters out.

Start by declaring a `cleanInputString` function that takes a `str` parameter.

*NOTE*: Values from an HTML `input` field will be received as strings in JavaScript. You will need to convert the string to a number before performing any calculations. You will learn how to convert string values into a number in a future step. 
