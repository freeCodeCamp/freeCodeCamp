...
```js
assert.match(getCaloriesFromInputs.toString(), /const\s+currVal\s*=/);
```

You should assign `item.value` to `currVal`.

```js
assert.match(getCaloriesFromInputs.toString(), /const\s+currVal\s*=\s*item\.value/);
```
...
assert.match(getCaloriesString, /for\s*\(\s*const\s+item\s+of\s+list\s*\)\s*\{const\s+currVal\s*=\s*item\.value/);
```
