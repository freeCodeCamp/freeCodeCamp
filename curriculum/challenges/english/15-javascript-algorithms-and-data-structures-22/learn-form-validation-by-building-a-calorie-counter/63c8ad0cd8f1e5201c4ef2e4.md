...
The `list` parameter is going to be the result of a query selector, which will return a `NodeList`. A `NodeList` is a list of elements similar to an array where, it contains the elements that match the query selector. You will need to loop through these elements in the list. 

In previous steps, you learned how to loop through an array using a `for` loop. However, you can also loop through a `NodeList` using a <dfn>for...of</dfn> loop.
