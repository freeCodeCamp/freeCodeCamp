...
```js
assert.match(getCaloriesFromInputs.toString(), /cleanInputString\(\s*list\s*\[\s*i\s*\]\s*\.value\s*\)/);
```

You should assign the result of your `cleanInputString` call to your `currVal` variable.

```js
assert.match(getCaloriesFromInputs.toString(), /currVal\s*=\s*cleanInputString\(\s*list\s*\[\s*i\s*\]\s*\.value\s*\)/);
```
