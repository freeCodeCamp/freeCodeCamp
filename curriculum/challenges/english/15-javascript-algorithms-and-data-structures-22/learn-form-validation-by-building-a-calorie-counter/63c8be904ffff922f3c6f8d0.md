...
You should pass `item.value` as the argument for `cleanInputString`.
...
```js
assert.match(getCaloriesFromInputs.toString(), /const\s+currVal\s*=\s*cleanInputString\(\s*item\.value\s*\)/);
```
