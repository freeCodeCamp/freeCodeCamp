...
When you click on the `Go to store` button, the text for the buttons should show options for buying health and weapons. The `#text` element should also change to show that the player is in the store. To accomplish this, you can use the <dfn>innerText</dfn> property.

The `innerText` property controls the text that appears in an HTML element. The following example would change the text of the `p` element from `Demo content` to `Hello World`.
...
Test out your changes by clicking on the `Go to store` button and checking that the text of the first button has changed.
