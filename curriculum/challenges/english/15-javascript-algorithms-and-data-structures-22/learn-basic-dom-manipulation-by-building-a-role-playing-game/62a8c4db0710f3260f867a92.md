...
What if the player doesn't have enough `gold` to buy `health`? To check if the player has enough `gold`, you can use an `if` statement.

In the previous project, you learned about `if` statements. Here is the basic syntax:
...
To test out your changes, click on the `"Go to store"` button and then click on the `"Buy 10 health"` button a few times. 

You will notice that you will only be able to increase your `health` to a max of `150`. This is because once `gold` drops below `10`, the `health` and `gold` values will no longer update.
