...
When you click on the `"Go to store"` button, the button text correctly updates to show that you can buy health, buy a weapon, or go back to the town square. 

However, if you try to click on the second button now labeled, `"Buy weapon (30 gold)"`, you will notice that the message of `Going to cave.` is logged to the console. This is because the `goCave` function is still being called when the second button is clicked. A similar issue occurs when you click on the third button labeled `"Go to town square"` where the message `Fighting dragon.` is logged to the console.

To fix this, you need to update the `onclick` property for each button to run the correct function when clicked.

In your `goStore()` function, update the `onclick` property for each button to run `buyHealth`, `buyWeapon`, and `goTown`, respectively.

*NOTE*: You will create the `buyHealth`, `buyWeapon`, and `goTown` functions in the next few steps.
