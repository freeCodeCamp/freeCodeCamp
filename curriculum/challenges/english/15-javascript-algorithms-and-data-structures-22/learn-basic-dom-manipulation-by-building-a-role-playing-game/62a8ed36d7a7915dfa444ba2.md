...
The next step is to show the updates to the player's health and the monster's health on the screen.

Update `healthText.innerText` and `monsterHealthText.innerText` to equal `health` and `monsterHealth`.

To see your changes, click on the `"Fight dragon"` button, followed by the `"Attack"` button. Now you should see the player's health and the monster's health update.
