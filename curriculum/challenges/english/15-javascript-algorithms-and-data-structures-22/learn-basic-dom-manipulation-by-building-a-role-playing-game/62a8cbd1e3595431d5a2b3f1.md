...
It would nice if the player could see the new weapon that they just bought. 

In between the two lines you just wrote, use `let` to initialize a new variable called `newWeapon`. Assign it the value of `weapons`.
...
`newWeapon` should be initialized to have the value of `weapons`. 
