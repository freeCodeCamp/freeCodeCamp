...
When the player sells their weapon, the first weapon in the `inventory` array should be removed.

In the previous project, you learned how to work with the `shift` method which removes the first element from an array and returns it. 

```js
let names = ["Jess", "Nancy"];
