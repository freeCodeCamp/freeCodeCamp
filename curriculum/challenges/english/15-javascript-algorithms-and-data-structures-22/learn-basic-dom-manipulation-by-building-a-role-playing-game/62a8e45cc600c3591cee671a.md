...
When the monster hurts the player, the player's `health` should decrease. 

Set `health` to equal `health` minus the monster's level. Remember you can get this from the `monsters[fighting].level` property.

# --hints--

You should use the `-=` operator with `health`.
