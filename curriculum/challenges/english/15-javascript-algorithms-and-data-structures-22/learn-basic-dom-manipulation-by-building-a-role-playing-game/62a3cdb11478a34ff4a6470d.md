...
Here is an example where the `buttonEl` variable represents the button element, and `myFunction` is a reference to a function. When the button is clicked, `myFunction` will be called.

```js
buttonEl.onclick = myFunction;
```

Set `button1.onclick` to the function reference of `goStore`. 

Open up the console, click the `"Go to store"` button, and you should see the message `"Going to store."` printed to the console.
