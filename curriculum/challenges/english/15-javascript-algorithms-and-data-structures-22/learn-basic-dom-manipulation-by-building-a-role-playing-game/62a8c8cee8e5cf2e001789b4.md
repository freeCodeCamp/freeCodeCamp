...
In the `buyWeapon` function, use the addition assignment operator to add `1` to `currentWeapon`. The number `1` represents the index of the next weapon in the `weapons` array.

# --hints--

You should use the addition assignment operator(`+=`) to add one to `currentWeapon`.
