...
If the player defeats the dragon, then they will have won the game.

Inside the `else if` block for your `attack` function, create another `if` and `else` statement. If the player is fighting the dragon (`fighting` would be `2`), call the `winGame` function. Move the `defeatMonster()` call to the `else` block.
