...
Right now you are setting the button functions to the values of the `goStore`, `goCave`, and `fightDragon` functions. But this is not practical because the location will change throughout the game and the functions should change accordingly.
...
You should access the first element of the `location["button functions"]` array.

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

You should set the `button1.onclick` property to `location["button functions"][0]`.

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

You should access the second element of the `location["button functions"]` array.

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

You should set the `button2.onclick` property to `location["button functions"][1]`.

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

You should access the third element of the `location["button functions"]` array.

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*2\s*\]/);
```

You should set the `button3.onclick` property to `location["button functions"][2]`.
