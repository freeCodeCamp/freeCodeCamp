...
title: While for Simultaneous Actions
challengeType: 19
dashedName: while-for-simultaneous-actions
...
`While` is used in sentences to indicate that two actions or events are happening at the same time. 

It often sets the scene for one action that occurs during the time another action is taking place. For example:
