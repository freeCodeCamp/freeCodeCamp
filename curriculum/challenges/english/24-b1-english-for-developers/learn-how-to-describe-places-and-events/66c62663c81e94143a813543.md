...
`It's supposed to be. This is frustrating.Are these IDEs always BLANK BLANK?`
...
`Tricky` is used to describe something that is difficult, complicated, or challenging to deal with. When Tom asks if these IDEs are always `tricky,` he means that they can be hard to work with or understand. For example:
