...
`How about we BLANK to bi-weekly detailed updates BLANK there's a significant development?`
...
`To switch` means to change or change from one state, system, or method to another. In this context, Bob is suggesting changing the frequency of the updates. For example:
...
`I will attend the meeting unless I have a conflicting appointment.` – This means I will go to the meeting unless there's something else that prevents me, such as another appointment. You could say the same using `if` and a negative statement with `I will attend the meeting if I don't have a conflicting appointment.`
