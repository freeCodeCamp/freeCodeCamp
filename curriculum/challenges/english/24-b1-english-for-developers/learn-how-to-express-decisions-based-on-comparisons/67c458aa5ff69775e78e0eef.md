...
In comparative form, adjectives or adverbs with one syllable usually add `-er` (like `stronger` and `faster`) in the end. Longer adjectives or adverbs use `more` (like `more interesting` and `more efficiently`). Some words have irregular comparative form, like `good` changes to `better`. For example:
