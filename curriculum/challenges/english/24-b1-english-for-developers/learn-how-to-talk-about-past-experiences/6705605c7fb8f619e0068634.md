...
The word `issue` in this context means a problem or difficulty that needs to be solved. In the sentence, it refers to the database credentials being incorrect. Examples:  
...
The word `them` is a pronoun used to replace plural nouns already mentioned. In Lisa’s question, `them` refers to the `credentials`. For example:
...
Lisa’s question asks if James fixed the credentials, showing how `them` simplifies the sentence by replacing the noun.
