...
The negative form of the verb `to be` in the Simple Past tense is `wasn’t` (or `was not`) for singular subjects and `weren’t` (or `were not`) for plural subjects. It is used to describe something that did not happen or was not true. Examples:  
