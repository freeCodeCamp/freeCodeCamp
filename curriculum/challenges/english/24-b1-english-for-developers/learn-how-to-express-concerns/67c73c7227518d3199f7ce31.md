...
The **`Present Perfect Continuous`** tense (`have/has been` + verb in `-ing` form) is used to describe an action that started in the past and is still happening or has recent effects. When James says `We've already been putting in extra hours`, he means that the team started working extra hours some time ago and is still doing it now. 
Another example:
