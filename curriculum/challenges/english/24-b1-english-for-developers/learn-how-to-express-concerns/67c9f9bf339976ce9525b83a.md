...
The **first conditional** structure (`If + present simple`) is used here in combination with the verb `could`, which is used instead of `will` to indicate that the consequence is **possible but not certain**. For example:
