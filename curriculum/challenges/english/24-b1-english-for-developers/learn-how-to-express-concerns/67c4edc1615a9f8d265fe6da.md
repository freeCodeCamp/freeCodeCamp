...
`Maria: Understood. In that case, let's at BLANK prioritize the tasks.`  
...
This phrase is used to suggest a minimum effort or action when other options are not possible.  

# --explanation--

The phrase `at least` is used to indicate a minimum effort, action, or requirement when a better option is not available. For example:
