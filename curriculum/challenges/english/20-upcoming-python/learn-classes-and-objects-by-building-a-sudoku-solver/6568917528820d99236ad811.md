...
You should create a `for` loop to iterate over `enumerate(self.board)`. Remember to use two iterating variable separated by a comma.

```js
assert.match(
  e.code.original["main.py"],
  /^\s{8}for\s+(\w+)\s*,\s*(?!\1)(\w+)\s+in\s+enumerate\s*\(\s*self\.board\s*\)\s*:/m
);
```
