...
You should call the `find_empty_cell` function on `self`.

```js
assert.match(code, /self\.find_empty_cell\(\)/);
```

You should assign the result of `self.find_empty_cell()` to the variable `next_empty` using a walrus operator.
