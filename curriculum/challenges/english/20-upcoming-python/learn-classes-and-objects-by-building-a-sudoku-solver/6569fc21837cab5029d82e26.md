...
Your function should be named `solve_sudoku`.

```js
({ test: () => assert(e.code.original["main.py"], /^def\s+solve_sudoku/m) });
```

The method should take a single `board` parameter.

```js
({ test: () => assert.match(e.code.original["main.py"], /^def\s+solve_sudoku\s*\(\s*board\s*\)\s*:/m) })
```
