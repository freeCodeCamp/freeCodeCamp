...
HELP needed in modifying test(redundant test string)
...
```js
const functionNamedDelete = code.match(/def\s+_delete\s*\(\s*self\s*,\s*node\s*,\s*key\s*\)\s*:(.*?)(?=\n\w)/s)[1];

assert.include(functionNamedDelete, "return node");
```
