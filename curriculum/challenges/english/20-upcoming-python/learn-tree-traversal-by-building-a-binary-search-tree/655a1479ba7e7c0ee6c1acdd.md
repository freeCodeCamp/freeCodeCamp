...
```js
({ test: () => assert.match(e.code.original["main.py"], /^\s{4}def\s+_insert\s*\([^(]*\)\s*:/m) })
```

Your `_insert` method should take three parameters: `self`, `node` and `key`.

```js
({ test: () => assert.match(e.code.original["main.py"], /^\s{4}def\s+_insert\s*\(\s*self\s*,\s*node\s*,\s*key\s*\)\s*:/m) })
```
