...
You should have again `print('Search for 40:', bst.search(40))` at the bottom of your code.

```js
({ test: () => assert.equal(e.code.original["main.py"].match(/^print\s*\(\s*("|')Search for 40:\1\s*,\s*bst\.search\s*\(\s*40\s*\)\s*\)/gm).length, 2) })
```
