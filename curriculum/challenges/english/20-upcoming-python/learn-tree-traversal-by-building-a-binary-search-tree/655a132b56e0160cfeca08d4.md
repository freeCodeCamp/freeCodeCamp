...
Inside the `__init__` method, delete `pass` and initialize an instance attribute called `root` to `None`. 
...
```js
({ test: () => assert.match(e.code.original["main.py"], /^\s{8}self\.root\s*=\s*None/m) })
```
