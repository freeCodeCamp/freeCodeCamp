---
id: 657effaa2a5e0277d71f9cbe
title: Step 8
challengeType: 20
dashedName: step-8
---

# --description--

By the end of the loop, `snake_cased_char_list` should contain all the converted characters in correct order. Use the `join()` string method to convert the list of characters into a string.

```py
''.join(snake_cased_char_list)
```

This joins the characters from the list to the empty string you called the `join()` method on. Save the resultant inside a variable named `snake_cased_string` on the same level as the `snake_cased_char_list` variable.

# --hints--

Test 1

```js

```

# --seed--

## --seed-contents--

```py
def convert_to_snake_case(pascal_or_camel_cased_string):
    snake_cased_char_list = []
    for char in pascal_or_camel_cased_string:
        if char.isupper():
            snake_cased_char_list.append(f'_{char.lower()}')
        else:
            snake_cased_char_list.append(char)
--fcc-editable-region--


--fcc-editable-region--
```
