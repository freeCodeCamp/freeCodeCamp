...
You should join the characters in the list into a string using the `.join()` method and assign the string to a variable named `snake_cased_char_list` after your `for` loop.
...
({
    test: () => {
        ...
        assert.match(function_body, / +snake_cased_char_list\s*=\s*["']{2}\.join\s*\(\s*snake_cased_char_list\s*\)/);
    }
})
```
