...
```js
({ test: () => assert.match(e.code.original["main.py"], /^encryption\s*=\s*encrypt\s*\(\s*text\s*,\s*custom_key\s*\)/m) })
```

Your `decryption` variable should have the value of `decrypt(encryption, custom_key)`.

```js
({ test: () => assert.match(e.code.original["main.py"], /^decryption\s*=\s*decrypt\s*\(\s*encryption\s*,\s*custom_key\s*\)/m) })
```
