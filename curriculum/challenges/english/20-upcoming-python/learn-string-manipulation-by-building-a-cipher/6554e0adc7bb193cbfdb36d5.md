...
Let's check if the function can decrypt the string back to the plain text.

Create another variable called `decryption` and assign it `vigenere(encryption, custom_key, -1)`.
...
```js
({ test: () => assert.match(e.code.original["main.py"], /^decryption\s*=\s*vigenere\s*\(\s*encryption\s*,\s*custom_key\s*,\s*-\s*1\s*\)/m) })
```
