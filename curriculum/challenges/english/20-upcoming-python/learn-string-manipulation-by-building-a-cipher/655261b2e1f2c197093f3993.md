...
At the top of your for loop, replace `print(char == ' ')` with an `if` statement that triggers when `char` is an empty space. Inside the `if` body, print the string `space!`. Remember to indent this line.
