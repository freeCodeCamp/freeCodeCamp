...
In Python, there's a way to easily format strings. *f-strings* enables you to interpolate values in your strings. 

Interpolation means to put placeholders to replace with specified values. For example, you can achieve the same result of `'Encrypted text: ' + text` with `f'Encrypted text: {text}'`. You need to put an `f` before the quotes to create the f-string and put the variables or expressions you want to interpolate between curly braces.
