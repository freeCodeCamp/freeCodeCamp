...
As a final step, remove the `print` statement from the `verify_card_number` function, and change the `card_number` back to something valid.

# --hints--

You should not have a `print` statement within the `verify_card_number` function.
...
({
    test: () => {
        const spyCode = "a = ''\ndef printSpy(*x):\n    global a\n    a = str(x)\n" + e.code.original["main.py"].replaceAll("print(", "printSpy(") + "\na";
