...
Within the even digit `for` loop, replace the `print` statement with a variable named `number` and assign it the value of `digit` multiplied by `2`.

# --hints--

You should have `number = digit * 2` within the `for` loop.
...
({
  test: () => {
    ...
    assert.match(function_body, /number\s*=\s*digit\s*\*\s*2/);
  }
})
```
