...
Create a `while` loop with this condition: `right_array_index < len(right_part)`, and use the `pass` keyword in the body of the loop.
...
({
  test: () => {
    ...
    assert.match(function_body, /while\s+right_array_index\s*<\s*len\(right_part\):/);
  }
})
```
