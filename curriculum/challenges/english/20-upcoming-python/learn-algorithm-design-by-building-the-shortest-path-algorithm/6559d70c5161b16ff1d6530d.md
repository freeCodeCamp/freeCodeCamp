...
Delete your last `print` call and create a variable called `targets_to_print` after your `while` loop. Use the ternary syntax to assign it `[target]` when `target` is truthy, and `graph` otherwise.
...
You should create a variable called `targets_to_print`.

```js
({ test: () =>  {
    const shortest = __helpers.python.getDef(e.code.original["main.py"], "shortest_path");
    const {function_body} = shortest;    
    assert(function_body.match(/(?<=unvisited\.remove\(current\).*)^\s{4}targets_to_print\s*=/m));
  }
})
```
