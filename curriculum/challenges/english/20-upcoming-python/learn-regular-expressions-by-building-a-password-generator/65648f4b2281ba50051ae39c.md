...
({ test: () =>
  {
    ...
    assert(function_body.match(/for\s+\w\s+in\s+range\s*\(\s*length\s*\)\s*:/));
  }
})
```
...
({ test: () =>
  {
    ...
    const {function_body} = generate_pwd;    
    assert(function_body.match(/^(\s*)for\s+\w\s+in\s+range\s*\(\s*length\s*\)\s*:\s*^\1\1password\s*\+=\s*secrets\.choice\s*\(\s*all_characters\s*\)/m));
  }
})
```
