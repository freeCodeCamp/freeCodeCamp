...
A tuple is another built-in data structure in Python. Tuples are very much like lists, but they are identified by parentheses `()`, instead of square brackets. Also, tuples are immutable, unlike lists.

```py
my_tuple = ('larch', 1, True)
```

Your `constraints` list is going to store tuples in which the first item is a constraint parameter.
