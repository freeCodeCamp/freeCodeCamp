...
You should change each occurence of `target` inside `make_allowed_move` into `rod2`.

```js
({ test: () => {
  assert.match(code, /if\s+len\s*\(\s*rods\s*\[\s*rod2\s*\]\s*\)\s*==\s*0\s*:/);
  assert.match(code, /elif\s+len\s*\(\s*rods\s*\[\s*rod1\s*\]\s*\)\s*!=\s*0\s*and\s+len\s*\s*\(\s*rods\s*\[\s*rod2\s*\]\s*\)\s*!=\s*0\s*and\s+rods\s*\[\s*rod1\s*\]\[\s*-\s*1\s*\]\s*<\s*rods\s*\[\s*rod2\s*\]\[\s*-\s*1\s*\]\s*:/);
...
def make_allowed_move(rod1, rod2):    
    ...
    if len(rods[target]) == 0:
        forward = True
    elif len(rods[rod1]) != 0 and len(rods[target]) != 0 and rods[rod1][-1] < rods[target][-1]:
        forward = True              
    if forward:
