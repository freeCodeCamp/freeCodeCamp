...
({ test: () => {
  ...
  assert.match(code, /elif\s+rods\s*\[\s*rod1\s*\]\s+and\s+rods\s*\[\s*rod2\s*\]\s+and\s+rods\s*\[\s*rod1\s*\]\[\s*-\s*1\s*\]\s*<\s*rods\s*\[\s*rod2\s*\]\[\s*-\s*1\s*\]\s*:/);
...
def make_allowed_move(rod1, rod2):    
    ...
    if not rods[target]:
        forward = True
    elif rods[rod1] and rods[target] and rods[rod1][-1] < rods[target][-1]:
        forward = True              
    if forward:
