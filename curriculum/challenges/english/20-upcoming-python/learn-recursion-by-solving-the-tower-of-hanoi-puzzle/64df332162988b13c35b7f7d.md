...
```js
({ test: () => assert.match(code, /if\s+n\s*%\s*2\s*!=\s*0\s*:\s+print\(\s*f('|")Move\s\{\s*i\s*\+\s*1\s*\}\sallowed\sbetween\s{\s*source\s*\}\sand\s\{\s*auxiliary\s*\}\1\s*\)\s+make_allowed_move\(\s*source\s*,\s*auxiliary\s*\)/) })
