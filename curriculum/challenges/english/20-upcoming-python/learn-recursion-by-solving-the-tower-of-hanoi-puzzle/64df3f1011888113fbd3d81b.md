...
```js
({ test: () => assert.match(code, /def\s+move\(\s*n\s*,\s*source\s*,\s*target\s*,\s*auxiliary\s*\)\s*:\s+if\s+n\s*>\s*0\s*:/) })
```

You should call `move(n - 1, source, target, auxiliary)` inside the `if` statement.

```js
({ test: () => assert.match(code, /def\s+move\(\s*n\s*,\s*source\s*,\s*target\s*,\s*auxiliary\s*\)\s*:\s+if\s+n\s*>\s*0\s*:\s+move\(\s*n\s*-\s*1\s*,\s*source\s*,\s*target\s*,\s*auxiliary\s*\)/) })
```
