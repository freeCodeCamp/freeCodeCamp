...
```js
({ test: () => assert.match(e.code.original["main.py"], /^def\s+move\(\s*n\s*,\s*source\s*,\s*auxiliary\s*,\s*target\s*\)\s*:\s+^(\s{4})if\s+n\s*>\s*0\s*:\s+^\1\1move\(\s*n\s*-\s*1\s*,\s*source\s*,\s*auxiliary\s*,\s*target\s*\).+?^\1\1print\s*\(\s*rods\s*,\s*("|')\\n\2\s*\)/ms) })
```
