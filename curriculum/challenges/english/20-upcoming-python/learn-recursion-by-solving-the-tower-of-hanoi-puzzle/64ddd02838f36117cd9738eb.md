...
Outside the `else` statement, add a comment saying `display our progress` and print the content of the lists to check that everything is working.

# --hints--

You should use the provided text to add a comment outside the `else` statement.

```js
({ test: () => assert.match(code, /(?<=rods\[\s*source\s*\]\.append\s*\(\s*rods\[\s*target\s*\]\.pop\s*\(\s*\)\s*\)\s+)#\s*display\sour\sprogress/) })
```

You should print the `rods` object outside the `else` statement, after the comment.
