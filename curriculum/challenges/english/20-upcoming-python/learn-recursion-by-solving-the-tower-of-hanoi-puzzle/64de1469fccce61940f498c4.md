...
The `rods` object will represent the three rods with their disks. Give it the strings `'A'`, `'B'`, and `'C'` as keys and set each of them to an empty list.

# --hints--

Your `rods` object should have an `'A'` key.
...
Your `rods` object should have an `'B'` key.
...
Your `rods` object should have an `'C'` key.
