...
You should modify the order of the parameters in your `move(n - 1, source, target, auxiliary)` call.

```js
({ test:  () => assert.match(code, /move\(\s*n\s*-\s*1\s*,\s*source\s*,\s*auxiliary\s*,\s*target\s*\)/) })
```
