...
The steps of moving `n - 1` disks can be broken down furtherly until only a single disk is considered. This will be the first move occurring. And, after that, all the others from the previous recursive calls backwards. Keep in mind that in each recursive step the role played by the rods changes between source, target, and auxiliary.
