...
```js
({ 
  test: () => assert.match(code, /def\s+main\s*\(\)\s*:\s*[\s\S]*expenses\s*=\s*\[\]\s*[\s\S]*while\s+True\s*:\s*[\s\S]*print\(('|")\\nExpense Tracker\1[\s\S]*print\(('|")1. Add an expense\2[\s\S]*print\(('|")2. List all expenses\3[\s\S]*print\(('|")3. Show total expenses\4[\s\S]*print\(('|")4. Filter expenses by category\5[\s\S]*print\(('|")5. Exit\6\)/,) 
})
