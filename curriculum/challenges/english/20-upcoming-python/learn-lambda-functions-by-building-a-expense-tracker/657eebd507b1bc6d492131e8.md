...
```js
({
    test: () => assert.match(code, /elif\s+choice\s*==\s*('|")5\1\s*:\s*print\(\s*('|")Exiting the program.\2\s*\)/,)
})
...
```js
({
    test: () => assert.match(code, /elif\s+choice\s*==\s*('|")5\1\s*:\s*.*print\(\s*('|")Exiting the program.\2\s*\)\s*;?\s*break/,)
})
