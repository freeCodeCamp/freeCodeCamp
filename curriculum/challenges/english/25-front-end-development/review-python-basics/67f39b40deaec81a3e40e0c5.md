...
- **`input()` Function**: This is used to get input from a user.
...
If you have an empty function body, then you can use the `pass` keyword which represents a null operation.
...
- **Lambda Functions**: Is this still being taught in this module?
- **Decorators**: Is this also being taught in this module?
...
- **`if` Statements**: These are conditions used to determine if something is true or not. If the condition is true, then that block of code will run.
...
- **`elif` Statement**: These are conditions that come after an `if` statement. If the `elif` condition is true, then that block of code will run.
...
- **`else` Clause**: This will run if no other conditions are true.
...
- **Logical AND Operator**: This will return `True` if both operands are true:
...
- **Logical OR Operator**: This will return `True` if one of the operands is true:
...
- **Logical NOT Operator**: This will return `True` if the operand is false:

```py
is_raining = False

if not is_raining:
  print("It's not raining. You can go outside!") # It's not raining. You can go outside!
else:
  print("It's raining. Better to stay indoors.")
```

- **Short Circuiting**: This is when the second expression is not evaluated if the first one already determines the result.

```py
has_ingredients = False
has_time_to_cook = True

if has_ingredients and has_time_to_cook:
  print("Let's cook something!")
else:
  print('Better order takeout.') # Better order takeout.
```
