...
First remove the `displayStory("scary")` function call. After that, add a `click` event listener to the `scaryStoryBtn`, `funnyStoryBtn`, and `adventureStoryBtn` buttons. 
...
```js
assert.notMatch(code, /displayStory\(["']scary["']\)/);
```
...
// Normalize whitespace in the text comparison
...
// Convert and check border color
const rgbColor = storyContainer.style.borderColor;
const hexColor = rgbToHex(rgbColor);

// Define the expected story and normalize whitespace
