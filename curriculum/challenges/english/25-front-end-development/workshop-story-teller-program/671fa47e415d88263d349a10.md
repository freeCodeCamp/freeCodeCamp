...
Next, you will start working on the `JavaScript`. For that, begin by linking the `script.js` file to your html file.

# --hints--

You should element with the `src` attribute set to `./script.js`.

```js
// fix test
assert.equal(document.querySelector('script')?.getAttribute('src'), './script.js');
```
