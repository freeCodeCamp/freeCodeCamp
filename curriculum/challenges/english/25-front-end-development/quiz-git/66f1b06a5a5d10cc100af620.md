---
id: 66f1b06a5a5d10cc100af620
title: Git Quiz
challengeType: 8
dashedName: quiz-git
---

# --description--

Answer all of the questions below correctly to pass the quiz.

# --quizzes--

## --quiz--

### --question--

#### --text--

Git is open to....?

#### --distractors--

Developers

---

Administrators

---

Subscribers/Premium Users

#### --answer--

Everyone

### --question--

#### --text--

Why using version control software is a best practice for high performing software and DevOps teams, due to...?

#### --distractors--

A complete long-term change history of every file

---

Branching and merging

---

Traceability

#### --answer--

All of the options

### --question--

#### --text--

What is the maximum storage capacity available for public repo in GitHub and GitLab respectively?

#### --distractors--

Unlimited, 4GB

---

5GB, 10GB

---

5GB, Unlimited

#### --answer--

Unlimited, 5GB

### --question--

#### --text--

Which one is incorrect command to check the Git version?

#### --distractors--

git --version

---

git -v

---

git version

#### --answer--

git v

### --question--

#### --text--

Choose the correct option:
Statement 1 - GPG keys can be used for encrypting data and communication.
Statement 2 - SSH keys can be used for encrypting data and communication.

#### --distractors--

Statement 2

---

Neither statement 1 nor statement 2

---

Both statement 1 and statement 2

#### --answer--

Statement 1

### --question--

#### --text--

A repository is a place where you can store shared code with....?

#### --distractors--

Friends/classmates

---

Co-workers

---

Strangers

#### --answer--

All of the options

### --question--

#### --text--

What is the advantage of Repository?

#### --distractors--

When we work on an open-source repository, it becomes easy to detract from the project.

---

By using GitHub or any other repository management tool, it’s easy to develop robust ignorance of the project.

---

Markdown allows you to use a simple text editor to write unformatted text documents.

#### --answer--

The repositories that you create on GitHub or Bitbucket can also integrate with different CI/CD tools like Jenkins.	R

### --question--

#### --text--

Which push command is acceptable?

#### --distractors--

git push BRANCH-NAME

---

git push -r REMOTE-NAME BRANCH-NAME

---

git push REMOTE-NAME

#### --answer--

git push REMOTE-NAME BRANCH-NAME

### --question--

#### --text--

What is the correct way to contribute to other people's repositories?

#### --distractors--

git add -> git commit -> git clone -> git push

---

git clone -> git commit -> git add -> git push

---

None of the options

#### --answer--

git clone -> git add -> git commit -> git push

### --question--

#### --text--

Git checkout works hand-in-hand with?

#### --distractors--

git pull

---

git push

---

git commit

#### --answer--

git branch

### --question--

#### --text--

Adam created a branch named "alex" for his work; which command should he execute in the terminal?

#### --distractors--

git checkout

---

git checkout alex branch

---

git checkout -- alex

#### --answer--

git checkout alex

### --question--

#### --text--

Choose the correct option:
Statement 1 - 
Team members can collaborate without affecting one another's contributions.
Statement 2 - Developers can submit changes without waiting for the project maintainer to merge them.

#### --distractors--

Statement 1

---

Statement 2

---

Neither statement 1 nor statement 2

#### --answer--

Both statement 1 and statement 2

### --question--

#### --text--

User can create pull requests on ....?

#### --distractors--

GitHub.com

---

GitHub Codespaces

---

GitHub Mobile

#### --answer--

All of the options

### --question--

#### --text--

What are the reasons Git code reviews are important?

#### --distractors--

Promoting secrecy among teams

---

Keeping innovation private.

---

Decreasing effectiveness.

#### --answer--

Coaching new developers.

### --question--

#### --text--

What is the file structure required for a GitHub Pages site?

#### --distractors--

All files must be in a src folder.

---

Only CSS files are allowed in the root.

---

JavaScript files cannot be included.

#### --answer--

HTML files should be in the root directory.

### --question--

#### --text--

How do I troubleshoot a 404 error on my GitHub Pages site?

#### --distractors--

Clear your browser cache.

---

Check if the correct branch is published.

---

Ensure the file names match the links exactly.

#### --answer--

All of the options.

### --question--

#### --text--

What command do you use to fetch changes from the remote repository?

#### --distractors--

git pull

---

git sync

---

git update

#### --answer--

git fetch

### --question--

#### --text--

How do you ignore all log files in your repository?

#### --distractors--

log/*

---

logs/*.log

---

**/*.log

#### --answer--

*.log

### --question--

#### --text--

What is a common branching strategy for managing features in Git?

#### --distractors--

Monolithic branching

---

Single branching

---

Nested branching

#### --answer--

Feature branching

### --question--

#### --text--

What happens if you encounter a conflict during a rebase?

#### --distractors--

The rebase automatically completes.

---

The changes are lost.

---

The rebase fails and cannot be retried.

#### --answer--

You must resolve the conflict before continuing the rebase.
