...
You should add the `span` before the number 1 within the first `h3` element.

```js
assert.notEqual(document.querySelector('h3 > span')?.previousSibling.textContent, '1');
```

You should add the `span` before the number 2 within the second `h3` element.

```js
assert.notEqual(document.querySelectorAll('h3 > span')[1]?.previousSibling.textContent, '2');
```
