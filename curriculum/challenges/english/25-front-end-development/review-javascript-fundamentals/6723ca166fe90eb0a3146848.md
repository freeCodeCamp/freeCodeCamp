...
- **Definition**: `var` was the original way to declare variables before 2015. But there some issues that came with `var` in terms of scope, redeclaration and more. So that is why modern JavaScript programming using `let` and `const` instead.
