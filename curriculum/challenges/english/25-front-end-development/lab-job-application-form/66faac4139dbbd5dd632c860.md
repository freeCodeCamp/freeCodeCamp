...
1. Add a `:first-of-type` pseudo-class to the `input` elements to style the first input fields differently. (e.g., rounded corners).
...
Add a `:first-of-type` pseudo-class to the `input` elements to style the first input fields differently.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input:first-of-type'));
```
