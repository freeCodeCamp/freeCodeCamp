...
1. Inside `.radio-group` you should have a set of `input` elements with the type `radio` for selecting availability options (e.g., Full-Time, Part-Time). The group `name` sshould be `availability`.
...
Add a `:focus` pseudo-class to the `input` and `textarea` elements to change their border color when focused. Use a list selector in the given order.
...
The `button` element should have a `:disabled` pseudo-class that changes the background color when the form is incomplete or invalid.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('button:disabled').getPropertyValue('background-color'))
```
...
Add an `:nth-child` pseudo-class to the `textarea` element to style the fifth textarea field differently.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('textarea:nth-child(5)'));
```
