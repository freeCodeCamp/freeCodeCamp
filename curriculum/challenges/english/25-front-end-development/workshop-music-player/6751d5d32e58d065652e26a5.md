...
Loop through the `playlistSongElements` and use the `removeAttribute()` method to remove the `"aria-current"` attribute for each of the songs.

Then, create an `if` statement with the condition `songToHighlight` to add the attribute back to the currently playing song. Within the `if` statement use `setAttribute` on `songToHighlight` and pass in `"aria-current"` and `"true"` as the first and second arguments.

# --hints--

`highlightCurrentSong` should add the `aria-current` attribute to the currently playing song and remove the attribute from the other songs in the playlist.
