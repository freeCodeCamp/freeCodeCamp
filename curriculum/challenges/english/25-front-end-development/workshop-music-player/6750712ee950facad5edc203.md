...
  
  // Bug fix: Reset the playback time if the song is starting new
  if (start) {
    audio.currentTime = 0;
  } else {
...
  if (previousSong) {
    // Bug fix: Pass `true` to `start` to reset playback
    playSong(previousSong.id, true);
  } else {
    // Bug fix: Pass `true` to `start` to reset playback
    playSong(userData.songs[0].id, true);
  }
};
...
  } else {
    // No change here, we want to resume from where we left off
