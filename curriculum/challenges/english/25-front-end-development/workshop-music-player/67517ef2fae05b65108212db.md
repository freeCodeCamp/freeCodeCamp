...
Use `getElementById()` to get the `id` of the currently playing song, then use template literals to prefix it with `song-`. Assign it to a variable named `songToHighlight`.
...
```js
assert.match(__helpers.removeJSComments(code), /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*\s{\s*const\s+playlistSongElements\s*=\s*document\.querySelectorAll\(\s*('|")\.playlist-song\1\s*\)\s*;?\s*/)
```
