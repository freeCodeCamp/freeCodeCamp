...
You should still call the `playSong` function with the `id` of first song in your playlist when the `playButton` is clicked and `userData.currentSong` is `null`, 
