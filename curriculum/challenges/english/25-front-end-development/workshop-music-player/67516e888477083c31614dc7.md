...
Create a function named `highlightCurrentSong`. Inside the function, use `querySelectorAll` to get the `.playlist-song` elements and assign the collection to a `playlistSongElements` variable.
...
You should use the `querySelectorAll()` method to select the `.playlist-song` elements and assign the collection to the `playlistSongElements` variable.

```js
assert.match(highlightCurrentSong.toString(), /var\s+playlistSongElements\s*=\s*document.querySelectorAll\s*\(\s*('|"|`)\.playlist-song\1\s*\)\s*;/)
```
