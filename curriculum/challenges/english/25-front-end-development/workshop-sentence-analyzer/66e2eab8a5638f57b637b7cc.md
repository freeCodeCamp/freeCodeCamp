...
You should count the number of puctuations now.
...
Your punctuation count should be case-insensitive.

```js
assert.strictEqual(getPunctuationCount("Wow! freeCodeCamp, again? You're unstoppable!"), 5);
assert.strictEqual(getPunctuationCount("Hey! Isn't freeCodeCamp amazing, right?"), 4);
```
