...
Test you `getWordCount` by creating a `wordCount` variable set to the calling of the `getWordCount` function with the sentence `"I love freeCodeCamp"`.

After that, log the `wordCount` variable to the console with the text `"Word Count:"` and the interpolation of the `wordCount` variable. You can also use plus (`+`) concatenator if you want.
