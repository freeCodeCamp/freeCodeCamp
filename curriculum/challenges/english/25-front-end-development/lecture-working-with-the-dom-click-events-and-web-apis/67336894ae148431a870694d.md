...
Let's learn about the DOM and why it's so important for web development.
...
The HTML document is the root node in the DOM hierarchy.

It has one child node, the `html` element. This is the root element of the HTML document, since all other nodes descend from it.
...
This line of JavaScript code gets an element with the id `"container"` and assigns that object to a JavaScript constant. You must pass the id within quotation marks as an argument.

If you log this object to the console, you will see it in the output.
...
With `querySelector()`, you can get the first element in the HTML document that matches the CSS selector passed as argument.

In this example, you will get the first element with the class `"section"` and assign it to a variable. 
