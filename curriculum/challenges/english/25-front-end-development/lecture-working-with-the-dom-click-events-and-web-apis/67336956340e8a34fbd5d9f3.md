...
Creating smooth animations on a web page can be tricky, especially if you're not sure how to handle timing properly.

The good news is that the `requestAnimationFrame()` API makes it easier. 
...
But there's a lot more to do. Calling `requestAnimationFrame()` must first occur inside a function that handles the animation, such as `animate()`, along with a function to update the animation, traditionally called `update()`:
...
The HTML for this example is a `div` element with the text `"freeCodeCamp is Awesome"`:
...
This is the commented JavaScript that moves the rectangle `2px` to the right at every calling of `requestAnimationFrame(animate)`:
