...
8. Inside the `ul` element, you should have multiple `li` elements each containing an actor's name wrapped in a `strong` element and their corresponding character role (e.g., `James Holloway as Ethan Carter`).  
