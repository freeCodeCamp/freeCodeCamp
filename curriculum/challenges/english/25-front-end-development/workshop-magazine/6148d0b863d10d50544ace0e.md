...
```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

Your `.first-paragraph::first-letter` selector should have a `margin-right` property set to `1rem`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.marginRight, '1rem');
```
