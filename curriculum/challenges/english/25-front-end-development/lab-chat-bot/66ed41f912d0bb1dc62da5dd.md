...
You should create the three `const` variables `botName`, `botLocation`, `favoriteLanguage`.

```js
const codeWithoutComments = __helpers.removeJSComments(code);
assert.exists(botName);
assert.isString(botName);
assert.match(__helpers.removeJSComments(code), /const\s+botName/);
assert.exists(botLocation);
assert.isString(botLocation);
assert.match(codeWithoutComments, /const\s+botLocation/);
assert.exists(favoriteLanguage);
assert.isString(favoriteLanguage);
assert.match(codeWithoutComments, /const\s+favoriteLanguage/);
```
...
You should create a new variable `codingFact` with `let`.
...
You should give a value to `codingFact` that includes `favoriteLanguage` using concatenation.
