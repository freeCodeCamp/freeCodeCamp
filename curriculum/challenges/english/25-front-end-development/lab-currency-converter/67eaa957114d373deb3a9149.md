...
export function CurrencyConverter() {
  ...
  // Memoize the calculation. This only recalculates when amount or startCurrency change.
  const convertedAmounts = useMemo(() => {
    const converted = {};
    // Calculate the value of the input amount in the base currency (USD)
    const baseAmount = amount / exchangeRates[startCurrency];
    
    // Convert the base amount to all other currencies
    Object.keys(exchangeRates).forEach((curr) => {
      converted[curr] = (baseAmount * exchangeRates[curr]).toFixed(2);
    });
    return converted;
  ...
  return (
    <main>
      ...
      <div className="conversion-display">
        <p>{amount} {startCurrency} to {endCurrency} Conversion</p>
      </div>
      ...
      />
      <label htmlFor="start-currency">From:</label>
      <select id="start-currency" value={startCurrency} onChange={(e) => setStartCurrency(e.target.value)}>
      ...
      <label htmlFor="end-currency">To:</label>
      <select id="end-currency" value={endCurrency} onChange={(e) => setEndCurrency(e.target.value)}>
        {Object.keys(exchangeRates).map((curr) => (
          <option key={curr} value={curr}>
            {curr}
          </option>
      ...
      <p>Converted Amount: <span>{convertedAmounts[endCurrency]} {endCurrency}</span></p>
    </main>
  );
}
```
