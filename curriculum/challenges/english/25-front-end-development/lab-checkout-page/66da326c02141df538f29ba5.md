...
You should include a `span` element with the text `*` and `aria-hidden` set to `true` inside the `label` element for each required input.
...
You should have a `p` element with an `id` of `card-number-help` after the `#card-number` input.
...
Your `#card-number-help` element should not be empty.
