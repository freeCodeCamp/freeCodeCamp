...
There is a digit of the pin hidden in any line, so inside the function use `split` to divide the string in a list of lines (you can split on the newline character `\n` for that), and assign the resulting list to a variable called `lines`.

# --hints--

You should have a variable named `lines`.
