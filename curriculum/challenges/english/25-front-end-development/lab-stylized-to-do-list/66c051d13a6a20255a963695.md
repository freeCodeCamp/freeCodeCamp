...
```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.todo-list li a').getPropVal('text-decoration'), 'none' || new __helpers.CSSHelp(document).getStyle('a').getPropVal('text-decoration'), 'none');
```
