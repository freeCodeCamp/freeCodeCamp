...
`WeakSet` has the `add()`, `delete()`, and the `has()` methods you can use to work with it:
...
The contents appear empty because `WeakSet`s are not iterable and do not expose their contents directly.
...
Here are other noticeable differences between a `Set` and a `WeakSet`:
...
You can see the their differences in the types of values that they store, their referencing iteration, methods and properties and use cases. Please take a moment to read the content of this table.
