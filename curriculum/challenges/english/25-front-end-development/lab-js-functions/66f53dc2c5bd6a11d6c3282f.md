...
You should have a `generatePassword`arrow function with a `length` parameter.

```js
assert.isFunction(generatePassword, 'generatePassword is a function');
assert.isTrue(generatePassword.length === 1, 'generatePassword has one parameter');
```
