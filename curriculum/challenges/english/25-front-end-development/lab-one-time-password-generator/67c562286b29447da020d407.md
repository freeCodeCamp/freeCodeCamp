...
The `p` element should have an `aria-live` attribute with a valid value.

```js
const pElement = document.querySelector('.container p');
assert.exists(pElement?.ariaLive, "The `p` element should have the `aria-live` attribute");
assert.oneOf(pElement?.ariaLive, ["assertive", "polite"], "The `aria-live` attribute should be set to a valid value");
```
