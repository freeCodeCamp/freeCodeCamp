...
3. The `.container` `div` should include the following elements:
    ...
    - An `h2` element that either displays the message `"Click 'Generate   OTP' to get a code"` or shows the generated OTP if one is available.
    - A `p` element that:
        - Starts off empty.
    ...
    - A `button` element labeled `"Generate OTP"`. When clicked, it should generate a new OTP and start a 5-second countdown.
