...
Initially, the `.container` element should contain an empty `p` element with the ID `otp-timer` and the `aria-live` attribute set to a valid value.
...
assert.oneOf(pElements[0].ariaLive, ["assertive", "polite"], "The `aria-live` attribute should be set to a valid value.");
