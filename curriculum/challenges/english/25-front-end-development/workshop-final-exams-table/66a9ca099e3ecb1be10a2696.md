...
For the fourth student table row, use the following data for the table data elements:
...
```js
assert.isNotNull(document.querySelectorAll('tbody tr')[3]);
```
...
```js
assert.isNotNull(document.querySelectorAll('tbody tr')[4]);
```
