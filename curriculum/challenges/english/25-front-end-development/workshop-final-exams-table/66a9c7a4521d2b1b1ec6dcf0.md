...
For the second student table row, use the following data for the table data elements:
...
```js
assert.isNotNull(document.querySelectorAll('tbody tr')[1]);
```
...
```js
assert.isNotNull(document.querySelectorAll('tbody tr')[2]);
```
