...
You should have a `tfoot` element inside your `table` element.

```js
assert.isNotNull(document.querySelector('tfoot'));
```

Your `tfoot` element should have a `tr` element inside it.

```js
assert.isNotNull(document.querySelector('tfoot tr'));
```

Your first `td` element should contain the text `Average Grade`.

```js
assert.strictEqual(document.querySelector('tfoot tr td:first-child').textContent, 'Average Grade');
```

Your second `td` element should contain the text `78.8`.
