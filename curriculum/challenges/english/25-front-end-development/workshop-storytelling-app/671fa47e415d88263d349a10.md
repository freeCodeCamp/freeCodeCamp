...
The `script` element is used to add JavaScript to your webpage. To link to an external script file, you can use the `src` attribute. For example: `<script src="./app.js"></script>`.

Create a `script` element and set its `src` attribute to `script.js`.

Adding the `script` element just before the closing `</body>` tag allows the browser to load the HTML content before the script is loaded, which can improve the page's loading speed.
