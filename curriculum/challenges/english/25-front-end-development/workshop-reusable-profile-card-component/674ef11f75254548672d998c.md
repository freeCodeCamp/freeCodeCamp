...
Start by using the `export` keyword to create a `Card` functional component with `name`, `title`, and `bio` as the props. Don't forget to make sure they're all wrapped in a curly braces. That way, you destructure them instead of accessing them from `props`.
