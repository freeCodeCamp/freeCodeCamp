...
- Have Bob delete the first email (index 0).
...
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().has_stmt("bob.check_inbox()")`))
    }
})
```
...
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().has_stmt("bob.read_email(0)")`))
    }
})

```

You should call `bob.delete_email(0)` to delete the second email.

```js
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().has_stmt("bob.delete_email(0)")`))
    }
})
...
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().is_ordered("bob.delete_email(0)", "bob.check_inbox()")`))
    }
})
