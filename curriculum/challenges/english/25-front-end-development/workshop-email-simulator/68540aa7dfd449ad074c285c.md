...
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().has_stmt("bob.delete_email(1)")`))
    }
})
...
({
    test: () => 
    {
        assert(runPython(`_Node(_code).find_function("main").find_body().is_ordered("bob.delete_email(1)", "bob.check_inbox()")`))
    }
})
