...
({
    test: () => {
        assert.isFalse(runPython(`_Node(_code)..has_stmt("email_obj2 = Email(sender='mona@example.com',receiver='lisa@example.com',subject='Lunch?',body='Are you free for lunch tomorrow?')")`));
    }
})
```
