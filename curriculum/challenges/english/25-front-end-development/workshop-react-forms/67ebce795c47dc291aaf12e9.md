...
Next, create a `label` with the `className` of `section` and `column`, and a text `How did you get your power`

Inside the label create a `select` with options for the user to choose their power source. The select should have seven options listed below:
