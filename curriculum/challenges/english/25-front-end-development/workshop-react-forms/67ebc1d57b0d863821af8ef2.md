...
Now, you will notice you can't type in the `input`. That's because you have to account for `setHeroName` setter function too.

To do that, use the `onChange` event to update `heroName` with the `input` value by using using `setHeroName(e.target.value)`. This should be done in one line with an arrow syntax.
