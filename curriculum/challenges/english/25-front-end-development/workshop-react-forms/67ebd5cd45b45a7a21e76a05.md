...
Remove all the options apart from the first one. After that first `option`, map through the `powerSourceOptions` array with `source` as the parameter.

Then inside the map, create an `option` element with a `key` of `source` and a `value` of `source`. To account for the value of each `option` element, fill in `{source}`
