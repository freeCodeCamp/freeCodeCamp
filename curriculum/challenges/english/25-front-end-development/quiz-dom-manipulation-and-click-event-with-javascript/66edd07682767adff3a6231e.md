...
A JavaScript class we can instantiate to transform HTML into an mutable and serializable JSON object.
...
A web standard representing the elements of an HTML page as a tree graph and how we are supposed to interact with them using code no matter which programming language we use.
...
We can access an element siblings using the `siblings` property.
...
How can we use JavaScript to find all HTML anchors with missing hyperlinks ?
...
In JavaScript, how can we remove the last child node of a paragraph element with the "target" identifier and replace it with a "That's it." text node ?
...
We can use the user screen properties (like position and size) using the `Window` object.
...
We can access the clipboard using
```js
...
Which event is triggered when the HTML page and it's ressources are fully loaded ?
...
Which code is not a way to trigger a `validate` function when we click on a `button` element ?
...
Using the JavaScript Web animation API, how can we animate a "square" identified element to scale down to 0 in one second.
...
Which method do not exists on this `Animation` object instance ?
...
We have an HTML page with a 100x100 pixel canvas identified by the "art" id, and the following code :
```js
...
Let's say we have an HTML `dialog` element stored into a JavaScript variable `welcomeDialog`. It does not show on screen by default. How can we reveal it and then hide it after a 1 seconds delay ?
