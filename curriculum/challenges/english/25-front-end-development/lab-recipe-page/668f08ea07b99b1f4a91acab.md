...
```js
assert.strictEqual(document.querySelector('h1')?.nextElementSibling.tagName, 'P', 'The paragraph should be immediately below an H1');
assert.ok(document.querySelector('p')?.textContent.trim().length > 0, 'The paragraph should contain some text');
