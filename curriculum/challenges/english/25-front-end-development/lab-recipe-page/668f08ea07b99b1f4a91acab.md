...
assert(rawSrc?.trim().length > 0, "The 'src' attribute must be explicitly set.");
assert(resolvedSrc != window.location.href, "The 'src' should not be the same as the current page URL.");
...
img.onerror = (error) => {
  ...
  assert(false, "Your image's URL should ve valid."); // Make the test instafail
};
