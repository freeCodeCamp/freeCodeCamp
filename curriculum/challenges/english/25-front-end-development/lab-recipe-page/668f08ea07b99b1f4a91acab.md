...
```js
// 1. Check that a <p> tag is below the <h1> tag
const h1 = document.querySelector('h1');
const p = document.querySelector('p');
assert(h1 && p && h1.compareDocumentPosition(p) 
  && Node.DOCUMENT_POSITION_FOLLOWING
);

// 2. Check that the <p> tag has text content
assert(p && p.textContent.trim().length > 0,);
```
...
```js
assert(document.querySelectorAll('h2')[1].nextElementSibling.tagName === "OL");
```
