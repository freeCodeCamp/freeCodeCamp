...
3. In `id` the `#drum-machine` element you should have a `p` element with an `id` of `display`.
...
Some audio samples you can use for your drum machine can be found at `https://cdn.freecodecamp.org/curriculum/drum/fileName`, where `fileName` is as follow:
...
In `id` the `#drum-machine` element you should have another `div` with an `id` of `pad-bank`.
