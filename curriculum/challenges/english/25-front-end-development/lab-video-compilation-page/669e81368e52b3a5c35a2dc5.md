...
for (let e of $('section')) {
    assert.equal(e.children[0]?.tagName, 'H2');
    assert.isAbove(e.children[0]?.innerText.length, 0);
}
```
...
for (let e of $('section')) {
    ...
    assert.isAbove(e.children[1]?.innerText.length, 0);
}
```
...
```js
for (let e of $('section')) {
    assert.equal(e.children[2]?.tagName, 'IFRAME');
}
```
