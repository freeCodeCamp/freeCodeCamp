...
```js
assert.equal(document.querySelectorAll('p')[0]?.previousElementSibling.tagName, 'H1');
```
...
Each `section` element should contain an `h2` element as its first child.

```js
$('section').forEach(e => assert.equal(e.children[0].tagName, 'H2'));
```

Each `section` element should contain a `p` element as its second child.

```js
$('section').forEach(e => assert.equal(e.children[1].tagName, 'P'));
```

Each `section` element should contain an `iframe` element as its third child.

```js
$('section').forEach(e => assert.equal(e.children[2].tagName, 'IFRAME'));
```
