...
For this lab, you start with the css and html already written for you. You will need to add the functionality of being able to test a regular expression against a string.
 
**Objective:** Fulfill the user stories below and get all the tests to pass to complete the lab.

**User Stories:**

1. You should access the `#pattern`, `#test-string`, `#test-btn`, and `#result` elements and save in them in the variables `pattern`, `stringToTest`, `testButton`, and `result`, respectively.
...
You should access the `#pattern` element and save it in the `pattern` variable.

```js
assert.equal(pattern, document.querySelector('#pattern'))
```
...
You should access the `#result` element and save it in the `result` variable.

```js
assert.equal(result, document.querySelector('#result'))
```
...
The `getFlags` function should return `"ig"` or `"gi"` if both checkboxes are checked.
