...
For the `og:title` attribute, you will need to specify that it is `og:title`. The `content` attribute is where you will write the title you want displayed for social media sites.
...
Pay close attention to the value of the `og:title` attribute here because it heavily implies what that value does.
