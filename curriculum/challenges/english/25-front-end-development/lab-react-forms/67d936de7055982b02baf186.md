...
```js
const input = document.querySelectorAll('input[type="text"][required]');
...
async () => {
  ...
  // So, the test needs to manually check for the `.preventDefault` call.
   ...
   await React.act(async () => {
      ...
      f[Object.keys(f).find((k) => k.startsWith("__reactProps"))].onSubmit(mockEv);
      assert.equal(c, 1);
   });
}
```
...
async () => {
   ...
   const nonInputEls = document.querySelectorAll("body *:not(input,script)");

   const stateEl = [...nonInputEls].find((el) => el.textContent.includes("John Doe"));
   assert.exists(stateEl);
}
```
...
async () => {
   ...
   const stateEl = [...nonInputEls].find((el) => el.textContent.includes("fcc@freecodecamp.org"));
   assert.exists(stateEl);
}
```
...
async () => {
   ...
   const stateEl = [...nonInputEls].find((el) => el.textContent.includes("2"));
   assert.exists(stateEl);
...
async () => {
   ...
   const stateEl = [...nonInputEls].find((el) => el.textContent.includes("diet"));
   assert.exists(stateEl);
...
async () => {
   ...
   let nonInputEls = document.querySelectorAll("body *:not(input,script)");

   // Test submission without checked input, then test submission with checked input - compare for difference.
   const stateWithFalse = [...nonInputEls].map((el) => el.textContent).join("\n");
   ...
   nonInputEls = document.querySelectorAll("body *:not(input,script)");
   const stateWithTrue = [...nonInputEls].map((el) => el.textContent).join("\n");
