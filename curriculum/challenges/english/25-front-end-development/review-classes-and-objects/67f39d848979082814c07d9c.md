...
- **Property Decorator**: Converts a method into a property using `@property`. Allows access like an attribute but executes code behind the scenes.
...
- **Setter Method**: Use `@<property>.setter` to define how a property can be modified with validation.
