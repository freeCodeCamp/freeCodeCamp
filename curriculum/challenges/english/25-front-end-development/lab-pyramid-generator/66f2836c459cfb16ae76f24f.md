...
```js
const actual = pyramid("o", 4, false);
const expected = "\n   o\n  ooo\n ooooo\nooooooo\n";
const normalizeOutput = (str) => str.split('\n').map(line => line.trimEnd()).join('\n');
assert.equal(normalizeOutput(actual), normalizeOutput(expected));
```

`pyramid("p", 5, true)` should return `"\nppppppppp\n ppppppp\n  ppppp\n   ppp\n    p\n"`.

```js
const actual = pyramid("p", 5, true);
const expected = "\nppppppppp\n ppppppp\n  ppppp\n   ppp\n    p\n";
const normalizeOutput = (str) => str.split('\n').map(line => line.trimEnd()).join('\n');
assert.equal(normalizeOutput(actual), normalizeOutput(expected));
```
