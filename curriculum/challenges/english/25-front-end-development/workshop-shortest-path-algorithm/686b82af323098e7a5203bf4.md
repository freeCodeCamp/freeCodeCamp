...
You should have a list named `visited`.

```js
({
  test: () => {
    assert(runPython(`_Node(_code).has_variable("visited")`))
  }
})
```

You should initialize the `visited` list with `False` for every node.

```js
({
  test: () => {
    assert(runPython(`_Node(_code).find_variable("visited").is_assigned("[False] * n")`))
  }
})
```
