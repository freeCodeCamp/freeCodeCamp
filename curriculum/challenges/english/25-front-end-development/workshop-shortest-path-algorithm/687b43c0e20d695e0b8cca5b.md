---
id: 687b43c0e20d695e0b8cca5b
title: Step 26
challengeType: 20
dashedName: step-26
---

# --description--

At this point, you only want to display results for nodes that are not the start node and are reachable from it.

Remove the `pass` statement, and add a conditional that checks, if `node_no` is equal to `start_node` or if the distance to `node_no` is equal to infinity (`INF`). 

If either condition is true, use `continue` to skip to the next iteration of the loop.



# --hints--

You should remove the `pass` statement.

```js
({
  test: () => {
    assert(!runPython(`"pass" in _code`))
  }
})
```

You should add a conditional statement.

```js
({
  test: () => {
    assert(runPython(`_Node(_code).has_if()`))
  }
})
```

Your conditional should check if `node_no` is equal to `start_node`.

```js
({
  test: () => {
    assert(runPython(`"node_no == start_node" in _code`))
  }
})
```

You should also check if the distance to `node_no` is equal to infinity (`INF`).

```js
({
  test: () => {
    assert(runPython(`"distances[node_no] == INF" in _code`))
  }
})
```

You should use `continue` to skip to the next iteration of the loop.

```js
({
  test: () => {
    assert(runPython(`"continue" in _code`))
  }
})
```

# --seed--

## --seed-contents--

```py
INF = float("inf")
adj_matrix = [
     [0, 5, 3, INF, 11, INF],
     [5, 0, 1, INF, INF, 2],
     [3, 1, 0, 1, 5, INF],
     [INF, INF, 1, 0, 9, 3],
     [11, INF, 5, 9, 0, INF],
     [INF, 2, INF, 3, INF, 0],
 ]

def shortest_path(matrix, start_node, target_node=None):
  n = len(matrix)
  distances = [INF] * n
  distances[start_node] = 0
  paths = [[node_no] for node_no in range(n)]
  visited = [False] * n

  for _ in range(n):
    min_distance = INF
    current = -1
    for node_no in range(n):
      if not visited[node_no] and distances[node_no] < min_distance:
        min_distance = distances[node_no]
        current = node_no

    if current == -1:
      break
    visited[current] = True

    for node_no in range(n):
      distance = matrix[current][node_no]
      if distance != INF and not visited[node_no]:
        new_distance = distances[current] + distance
        if new_distance < distances[node_no]:
          distances[node_no] = new_distance
          paths[node_no] = paths[current] + [node_no]

--fcc-editable-region--
  targets = [target_node] if target_node is not None else range(n)
  for node_no in targets:
    
--fcc-editable-region--
```
