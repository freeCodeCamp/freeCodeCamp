...
Add a conditional that checks if `current` is still `-1` and `break` out of the loop if it is.

# --hints--

You should check if `current` is still `-1`.

```js

```

If it is, you should `break` out of the loop.
