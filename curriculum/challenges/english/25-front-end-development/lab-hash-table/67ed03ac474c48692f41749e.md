...
When the hash value is the same, the `remove` method should only remove that key and not the entire hash table for that hash value.
...
({
  test: () => runPython(`
    ...
    assert index not in ht.collection
  `)
})
```
...
When the `golf` key exists in the hash table, `HashTable.remove('golf')` should remove the `golf` key from the collection.
