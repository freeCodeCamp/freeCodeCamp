...
If multiple keys hash to the same index, the `remove` method should only delete the specific key-value pair and not the entire list of pairs at that index.
...
When a key exists in the hash table, `HashTable.remove()` should remove the key-value pair from the collection.
