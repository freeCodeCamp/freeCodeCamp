...
7. When you add `("rose", "flower")` to the hash table, the `collection` should look like this:
...
8. When you add a key that hashes to the same value as an existing key, like `"fcc"` and `"cfc"`, the `collection` should look like this:
