...
```js
const item = document.getElementsByClassName('mood-board-item')[0];
// TODO - check the background color
```
...
```js
const img = document.getElementsByClassName('mood-board-image')[0];
assert.exists(img);
assert.equal(img.tagName, 'IMG');

// TODO - check the src attribute
```
...
```js
const text = document.getElementsByClassName('mood-board-text')[0];
assert.exists(text);
assert.equal(text.tagName, 'H3');

// TODO - check the text content  
```
...
```js
const items = document.getElementsByClassName('mood-board-item');
assert.greaterThan(items.length, 2);
