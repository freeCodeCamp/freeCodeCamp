...
- **`try...except`** allows you to run a block of code and handle specific exceptions that might occur. The `try` block is the block of code where you anticipate an error might occur, while the `except` block takes a specified exception and runs if that specified error is raised. Here's an example:
...
- **`...else..finally`** is an extension of `try...except`. If no exception occurs, the `else` block runs. The `finally` block always runs no matter what happens.
