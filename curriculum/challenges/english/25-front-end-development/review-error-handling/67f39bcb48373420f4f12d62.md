...
- **`try...except`** lets you execute a block of code that might raise an exception. The `try` block is the block of code where you anticipate an error might occur, while the `except` block takes a specified exception and runs if that specified error is raised. Here's an example:
...
- **`else` and `finally`** blocks extend `try...except`. If no exception occurs, the `else` block runs; the `finally` block always runs regardless of errors.
