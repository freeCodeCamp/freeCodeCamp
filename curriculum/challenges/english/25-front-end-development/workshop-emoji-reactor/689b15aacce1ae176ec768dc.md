...
Parse the text content of the count element to extract the current number. Use `textContent`, `split("/")`, and the unary plus operator (+) to convert the string to a number.
...
document.querySelector("#count").textContent = testString;
document.querySelector(`#happy-btn`).click();

assert.sameDeepOrderedMembers(spy.calls, [["Current count:", 5]]);

const testString2 = "10/10";
document.querySelector("#count").textContent = testString2;
document.querySelector(`#happy-btn`).click();
