...
Now it's time to link the `script.js` file.

Add a `script` element right before the `</body>` closing tag with the right attribute to link the file.
...
```js
assert.match(document.querySelector("script").src, /^(\.\/)?script.js$/);
```
