...
Inside the `button` element, create a `span` element that contains `😊`, and has a `role` of `img` and an `aria-hidden` attribute of `true`.

Create a second `span` element with a `class` of `count` and a content of `0/10`.
...
The second `span` element should have a text content of `1/10`.

```js
assert.equal(document.querySelectorAll("button > span")[1]?.textContent.trim(), "1/10");
```
