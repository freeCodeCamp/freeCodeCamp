...
To remove an element from a map object, you can use the `delete` method, which takes a key as argument and removes the entry associated with that key. It returns a boolean indicating if the removal was successful.
...
Your `removePlant` function should return a boolean value.
