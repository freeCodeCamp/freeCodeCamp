...
Your `while` loop should have these conditions: `left_array_index < len(left_part) and right_array_index < len(right_part)`.

```js
({
    test: () => assert(runPython(`_Node(_code).find_function("merge_sort").find_whiles()[0].find_conditions()[0].is_equivalent("left_array_index < len(left_part) and right_array_index < len(right_part)")`)) 
