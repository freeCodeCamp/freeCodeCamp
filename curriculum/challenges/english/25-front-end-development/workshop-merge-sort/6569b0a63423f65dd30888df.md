...
```js
assert(runPython(`_Node(_code).find_function("merge_sort").find_whiles()[1].find_bodies()[0].find_body()[0].is_equivalent("array[sorted_index] = left_part[left_array_index]")`))
```

You should not have `pass` in the `while` loop.

```js
assert.isFalse(runPython(`_Node(_code).find_function("merge_sort").find_whiles()[1].find_bodies()[0].has_pass()`))
```
