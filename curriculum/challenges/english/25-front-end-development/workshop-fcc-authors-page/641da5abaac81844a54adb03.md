...
Start by creating a function called `displayAuthors` that takes an `authors` parameter. Your function should then loop through the `authors` array and add a card for each author to the `div` with the id of `authorContainer`. 
...
When the `displayAuthors` function is called with an author's array, it should add a card for each author to the `div` with an id of `authorContainer`.
