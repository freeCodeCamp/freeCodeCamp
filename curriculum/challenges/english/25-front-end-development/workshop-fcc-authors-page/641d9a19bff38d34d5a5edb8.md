...
All the HTML and CSS for this workshop has been provided for you. You can take a look at the two files to familiarize yourself with them.
...
```js
assert.isDefined(authorContainer, 'The variable authorContainer should be defined');
```

You should assign the `#author-container` element to the `authorContainer` variable.

```js
assert.strictEqual(authorContainer, document.querySelector('#author-container'), 'authorContainer should be assigned the #author-container element');
```

You should have a variable called `loadMoreBtn`.

```js
assert.isDefined(loadMoreBtn, 'The variable loadMoreBtn should be defined');
```
...
```js
assert.strictEqual(loadMoreBtn, document.querySelector('#load-more-btn'), 'loadMoreBtn should be assigned the #load-more-btn element');
```
