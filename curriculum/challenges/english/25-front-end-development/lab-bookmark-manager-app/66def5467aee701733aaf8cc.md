...
For this lab, and you will use JavaScript to complete the Bookmark Manager app to use the local storage to handle a collection of bookmarks. All the HTML and CSS has been provided to you.

**Objective:** Fulfill the user stories below and get all the tests to pass to complete the lab. 

**User Stories:**

1. You should have a `bookmarks` array containing bookmarks objects. Each bookmark object should have three keys: `name`, `category`, and `url`.
...
You should have a `bookmarks` array.
...
```js
const mainSection = document.querySelector("#main-section").classList;
const formSection = document.querySelector("#form-section").classList;
const getHidden = () => {
  if (mainSection.contains("hidden") && !formSection.contains("hidden")) {
    return "main"
  } else if (!mainSection.contains("hidden") && formSection.contains("hidden")) {
    return "form";
  }
}
const hidden = getHidden()
displayOrCloseForm();
const hiddenAfterCall = getHidden()
...
addBookMarkButton.dispatchEvent(new Event("click"));
const categoryName = document.querySelector(".category-name").innerText;
const categoryDropdown = document.getElementById("category-dropdown").value;
assert.strictEqual(categoryName.toLowerCase(), categoryDropdown.toLowerCase());
```
...
assert.strictEqual(getHidden(), "form");
addBookMarkButton.dispatchEvent(new Event("click"));
assert.strictEqual(getHidden(), "main");
```
...
assert.strictEqual(getHidden(), "main");
closeFormButton.dispatchEvent(new Event("click"));
assert.strictEqual(getHidden(), "form");
 ```
...
const temp1 = originalLocalStorage;
const temp2 = bookmarks;
try{
  const bookmarkName = document.getElementById("name");
  bookmarkName.value = "test name 1";
  const categoryDropdown = document.getElementById("category-dropdown");
  const bookmarkURL = document.getElementById("url");
  bookmarkURL.value = "test URL 1";
  ...
  bookmarks.length = 0;
  bookmarks.push({name: "a", category: "news", url: "example1.com"}, {name: "b", category: "entertainment", url: "example2.com"}, {name: "c", category: "work", url: "example3.com"})
  localStorage.setItem("bookmarks", JSON.stringify(bookmarks));
  const expected = [{name: "a", category: "news", url: "example1.com"}, {name: "b", category: "entertainment", url: "example2.com"}, {name: "c", category: "work", url: "example3.com"}, bookmarkObject];
  const addBookmarkButtonForm = document.getElementById("add-bookmark-button-form");
  addBookmarkButtonForm.dispatchEvent(new Event("click"));
  bookmarkName.value = "";
  bookmarkURL.value = "";
  const actual = JSON.parse(localStorage.getItem("bookmarks"));
  ...
  bookmarks = temp2;
  console.log("test 5", JSON.parse(localStorage.getItem("bookmarks")))
}
```
...
try {
  const bookmarkName = document.getElementById("name");
  bookmarkName.value = "test name 2";
  const categoryDropdown = document.getElementById("category-dropdown");
  const bookmarkURL = document.getElementById("url");
  bookmarkURL.value = "test URL 2";
  const addBookmarkButtonForm = document.getElementById("add-bookmark-button-form");
  addBookmarkButtonForm.dispatchEvent(new Event("click"));
  const nameVal = bookmarkName.value;
  const URLVal = bookmarkURL.value;
  assert.strictEqual(nameVal, "");
  assert.strictEqual(URLVal, "");
} finally {
  localStorage.setItem("bookmarks", JSON.stringify(temp));
  bookmarkName.value = "";
  bookmarkURL.value = "";
  console.log("test 6", JSON.parse(localStorage.getItem("bookmarks")))
}
```
...
try {
  const bookmarkName = document.getElementById("name");
  bookmarkName.value = "test name 3";
  const categoryDropdown = document.getElementById("category-dropdown");
  const bookmarkURL = document.getElementById("url");
  bookmarkURL.value = "test URL 3";
  const mainSection = document.querySelector("#main-section").classList;
  const formSection = document.querySelector("#form-section").classList;
  const getHidden = () => {
    if (mainSection.contains("hidden") && !formSection.contains("hidden")) {
  ...
  localStorage.setItem("bookmarks", JSON.stringify(temp));
  console.log("test 7", JSON.parse(localStorage.getItem("bookmarks")))
}
```
...
```js
const mainSection = document.querySelector("#main-section").classList;
const bookmarkListSection = document.querySelector("#bookmark-list-section").classList;
const getHidden = () => {
  if (mainSection.contains("hidden") && !bookmarkListSection.contains("hidden")) {
    return "main"
  } else if (!mainSection.contains("hidden") && bookmarkListSection.contains("hidden")) {
...
viewCategoryButton.dispatchEvent(new Event("click"));
const categoryName = document.querySelector(".category-name").innerText;
const categoryDropdown = document.getElementById("category-dropdown").value;
assert.strictEqual(categoryName.toLowerCase(), categoryDropdown.toLowerCase());
```
...
try {
  const categoryDropdown = document.getElementById("category-dropdown");
  const test = [{name: "a", category: "news", url: "example1.com"}, {name: "b", category: "entertainment", url: "example2.com"}, {name: "c", category: "work", url: "example3.com"}];
  localStorage.setItem("bookmarks", JSON.stringify(test));
  const viewCategoryButton = document.getElementById("view-category-button");
  categoryDropdown.value = "miscellaneous";
  viewCategoryButton.dispatchEvent(new Event("click"));
...
try {
  const categoryDropdown = document.getElementById("category-dropdown");
  const test = [{name: "a", category: "news", url: "example1.com"}, {name: "b", category: "entertainment", url: "example2.com"}, {name: "c", category: "work", url: "example3.com"}, {name: "d", category: "news", url: "example4.com"}];
  const viewCategoryButton = document.getElementById("view-category-button");
  categoryDropdown.value = "news";
  localStorage.setItem("bookmarks", JSON.stringify(test));
  console.log("test", localStorage.getItem("bookmarks"));
  viewCategoryButton.dispatchEvent(new Event("click"));
  console.log("test2", localStorage.getItem("bookmarks"));
  console.log("cat value", categoryDropdown.value)
  const categoryList = document.getElementById("category-list").innerHTML;
  const bookmarksDisplayed = document.querySelectorAll('#category-list input[type="radio"]');
  console.log(document.getElementById("category-list").innerHTML)
  console.log(bookmarksDisplayed);
  //assert.lengthOf(bookmarksDisplayed, 2);
  console.log("failed?")
  assert.strictEqual(bookmarksDisplayed[0].id, "a");
  assert.strictEqual(bookmarksDisplayed[0].value, "a");
  assert.strictEqual(bookmarksDisplayed[1].id, "d");
  assert.strictEqual(bookmarksDisplayed[1].value, "d");
} finally {
  localStorage.setItem("bookmarks", JSON.stringify(temp));
  console.log("finally", localStorage.getItem("bookmarks"))
}
```
...
console.log("before", getHidden())
assert.strictEqual(getHidden(), "main");
closeListButton.dispatchEvent(new Event("click"));
console.log("after", getHidden())
assert.strictEqual(getHidden(), "bookmark list");
```
