...
Arrow functions should not be used inside a `setState` function.
...
What happens when the returned variable of a `useRef` hook is updated?
...
How many times will this code print to the console in production?
...
Every time the `feedback` variable is updated.
...
Executing an apps most complex functionality.
...
`useMediaQuery`, `useFetch`, `useStateAction` and `useMemo`.

---

`useLocalStorage`, `useReducer`, `useDeferredValue` and `useSyncExternal`.

---

`useDebugValue`, `useRef`, `useState` and `useDebounce`.

#### --answer--

`useReducer`, `useContext`, `useTransition` and `useId`.
...
Controlled is where the state is managed in React, uncontrolled is managed in the DOM by the browser.
...
If I have a complex function inside a component, which hook can I use to memoize its result?
...
Allows you to define a callback function.

---

Is a custom hook not managed by React.

---

Checks an app's performance in development only.

#### --answer--

Ensures a function isn't re-created each time a component re-renders.
...
SPA's don't actually use a single page for routing.
