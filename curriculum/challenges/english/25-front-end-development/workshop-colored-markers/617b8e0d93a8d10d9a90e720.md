...
```js
assert.notEqual(new __helpers.CSSHelp(document).getStyle('.two')?.backgroundColor, 'green');
```

Your `.two` CSS rule should have a `background-color` property set to `rgb(0, 255, 0)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.two')?.backgroundColor, 'rgb(0, 255, 0)');
```

Your `.three` CSS rule should not use the `blue` color keyword to set the `background-color` property.

```js
assert.notEqual(new __helpers.CSSHelp(document).getStyle('.three')?.backgroundColor, 'blue');
```

Your `.three` CSS rule should have a `background-color` property set to `rgb(0, 0, 255)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.three')?.backgroundColor, 'rgb(0, 0, 255)');
```
