...
```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.two')?.backgroundColor, 'green');
```
...
```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.three')?.backgroundColor, 'blue');
```
