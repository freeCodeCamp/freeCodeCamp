...
```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.container')?.backgroundColor, 'rgb(0, 0, 0)');
```

Your `.container` CSS rule should have a `padding` property set to `10px 0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.container')?.padding, '10px 0px');
```
