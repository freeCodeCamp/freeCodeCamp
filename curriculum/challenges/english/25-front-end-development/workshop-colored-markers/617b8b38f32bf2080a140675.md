...
```js
assert.notEqual(new __helpers.CSSHelp(document).getStyle('.one')?.backgroundColor, 'red');
```

Your `.one` CSS rule should have a `background-color` property set to `rgb(255, 0, 0)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.one')?.backgroundColor, 'rgb(255, 0, 0)');
```
