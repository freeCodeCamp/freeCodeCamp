...
1. You should define a function named `update_setting` that takes the a dictionary of settings and a tuple containing a key-value pair as parameters.
...
1. You should define a function named `delete_setting` that takes the a dictionary of settings and a key as parameters.
...
`add_setting` should return the error message `Setting [key] already exists! Cannot add a new setting with this name.` when the key already exists.
...
`add_setting` should add a new key-value pair and return the success message `Setting [key] added with value [value] successfully!`.
...
`update_setting` should update an existing key and return the success message `Setting [key] updated to [value] successfully!`.
...
`update_setting` should return the error message `Setting [key] does not exist! Cannot update a non-existing setting.` when the key doesn't exist.
...
`delete_setting` should remove an existing key and return the success message `Setting [key] deleted successfully!`.
