...
To make the action to perform clear and have a cleaner code, declare two functions named `encrypt` and `decrypt`, both with `text` and `shift` parameters.

You'll use `encrypt` for the encryption process, and `decrypt` for the decryption.

Return a `caesar` call passing in `text` and `shift` from both of them, but make sure to pass in also `False` as the third argument for the `decrypt` function.
...
Your `decrypt` function return `caesar(text, shift, False)`.
