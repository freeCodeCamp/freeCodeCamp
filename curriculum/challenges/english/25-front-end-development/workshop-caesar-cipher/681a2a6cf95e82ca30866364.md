...
```js
assert(runPython(`_Node(_code).find_function("encrypt").has_args("text, shift")`))
```

Your `encrypt` function should return `caesar(text, shift)`.

```js
assert(runPython(`_Node(_code).find_function("encrypt").has_return("caesar(text, shift)")`))
```

You should have a function named `decrypt` with two parameters, `text` and `shift`.

```js
assert(runPython(`_Node(_code).find_function("decrypt").has_args("text, shift")`))
```
