...
The `not` operator is a unary logical operator that negates an expression:
...
Use the `not` operator to fix the condition of the `if` statement so that your function returns `Shift must be an integer value.` when `shift` is *not* an integer number.
