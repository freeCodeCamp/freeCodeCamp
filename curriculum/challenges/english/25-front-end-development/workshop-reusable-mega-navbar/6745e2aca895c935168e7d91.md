...
```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Calendar');
```

You should set a text of `Chat` for the second anchor element.

```js
assert.equal(document.querySelectorAll('li a')[4]?.textContent, 'Chat');
```

You should set a text of `Email` for the third anchor element.

```js
assert.equal(document.querySelectorAll('li a')[5]?.textContent, 'Email');
```
