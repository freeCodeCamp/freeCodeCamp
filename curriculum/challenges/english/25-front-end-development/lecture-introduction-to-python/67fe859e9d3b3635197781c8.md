...
Floor division divides two numbers and rounds down the result to the nearest whole number. This is done with double forward slashes (`//`):
...
Exponentiation raises a number to the power of another, and is done with double asterisks (`**`):
