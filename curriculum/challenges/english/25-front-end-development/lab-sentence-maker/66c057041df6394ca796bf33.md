...
```js
const condition1 = /console\.log\(\"|'First\s*story:\s+\"|'\s*\+s*firstStory\)/gm.test(code);
const condition2 = /console\.log\(`First\s*story:\s+\$\{firstStory\}`\)/gm.test(code);
...
```js
const condition1 = /console\.log\(\"|'Second\s*story:\s+\"|'\s*\+s*secondStory\)/gm.test(code);
const condition2 = /console\.log\(`Second\s*story:\s+\$\{secondStory\}`\)/gm.test(code);
assert.isTrue(condition1 || condition2);
```
