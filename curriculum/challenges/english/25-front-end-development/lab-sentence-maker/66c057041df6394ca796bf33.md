...
for (const name of ['adjective', 'noun', 'noun2', 'place', 'adjective2', 'verb']) {
    assert.match(__helpers.removeJSComments(code), new RegExp(`\\$\\{\\s*${name}\\s*\\}`), 
        `You must use the variable "${name}" in firstStory with template literals or concatenation.`);
}
...
```js
const expectedSecond = `Once upon a time, there was a(n) ${adjective} ${noun} who loved to eat ${noun2}. The ${noun} lived in a ${place} and had ${adjective2} nostrils that blew fire when it was ${verb}.`;


const secondStoryMatch = __helpers.removeJSComments(code).match(/const\s+secondStory\s*=\s*(`[^`]+`)/);
assert.isNotNull(secondStoryMatch, "You must declare secondStory using a template literal.");

for (const name of ['adjective', 'noun', 'noun2', 'place', 'adjective2', 'verb']) {
    assert.match(secondStoryMatch[1], new RegExp(`\\$\\{\\s*${name}\\s*\\}`),
        `You must use the variable "${name}" in secondStory with template literals or concatenation.`);
}


assert.strictEqual(secondStory, expectedSecond);
