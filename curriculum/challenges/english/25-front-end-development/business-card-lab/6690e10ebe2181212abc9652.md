...
```js
assert.match(document.querySelector('img')?.alt, /.+/);
```
...
```js
assert.match(document.querySelector('p.full-name')?.textContent, /.+/);
```
...
```js
const parentEl = document.querySelector('.business-card');
const children = parentEl.children;
assert.match(children[4].textContent, /.+/);
```
