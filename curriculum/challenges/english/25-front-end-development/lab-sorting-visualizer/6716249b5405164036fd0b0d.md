...
1. When you click `#generate-btn` you should fill `#starting-array` with five `span` elements, each with a random number as its text.
...
**Note:** the browser zoom level needs to be at 100% in order to pass the tests.
...
Your `highlightCurrentEls` function should set the border of the given element's descendants placed at the given index and at the next one to `2px dashed red`.
...
highlightCurrentEls(testDiv, 0);
console.log(getComputedStyle(children[0]).border)
...
generateArrayBtn.addEventListener("click", () => {
    ...
    fillArrContainer(generateArray(), document.getElementById("starting-array"));
    showSortBtn();
});
