...
Then, when you click on the `span` element you will see the text `"Click me~!"`.
...
This will give you an idea of how the event bubbles up:
...
And then click our `span` again. This time, we don't see our `p` listener trigger. The event never fires for the `p` element, because we told it to stop propagation while it was being processed for the child `span` element.
...
Now this works well for our one little `span` element. But what if you have twenty `span` elements? Or maybe you use JavaScript to create more `span` elements on the fly?
...
Notice how we no longer have any listener attached to the `span` element at all. We have properly delegated the event handling to the `p` element. But does it work?
