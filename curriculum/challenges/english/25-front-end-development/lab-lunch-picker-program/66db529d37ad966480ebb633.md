...
7. You should create a function `showLunchMenu` that:
   ...
   - If there are items in the `lunches` array, returns a string in the format:  
     - `"Menu items: [Lunch Item], [Lunch Item]...`
     - If the `lunches` array is empty, returns a string:  
       - `"The menu is empty."`
...
The function `addLunchToEnd` should take a string as an argument.

```js
assert.match(addLunchToEnd.toString(), /\s*addLunchToEnd\(\s*\w+\s*\)/);
```
...
The function `addLunchToStart` should take a string as an argument.

```js
assert.match(addLunchToStart.toString(), /\s*addLunchToStart\(\s*\w+\s*\)/);
```
...
When `lunches = ["Stew", "Soup", "Toast"]`, the function `removeLastLunch` should return the string `"Toast removed from the lunch menu (last)."`.

```js
lunches = ["Stew", "Soup", "Toast"];
assert.strictEqual(removeLastLunch(), "Toast removed from the lunch menu (last).");

// prevent hardcoding
assert.strictEqual(removeLastLunch(), "Soup removed from the lunch menu (last).");
assert.strictEqual(removeLastLunch(), "Stew removed from the lunch menu (last).");
```
...
When `lunches = ["Salad", "Eggs", "Cheese"]`, the function `removeFirstLunch` should return the string `"Salad removed from the lunch menu (first)."`.

```js
lunches = ["Salad", "Eggs", "Cheese"];
assert.strictEqual(removeFirstLunch(), "Salad removed from the lunch menu (first).");

// prevent hardcoding
assert.strictEqual(removeFirstLunch(), "Eggs removed from the lunch menu (first).");
assert.strictEqual(removeFirstLunch(), "Cheese removed from the lunch menu (first).");
```
...
lunches.forEach((lunch) => {
  const result = getRandomLunch();
  if(result === `Randomly selected lunch: ${lunch}`){
