...
When the input array is not empty, the function `getRandomLunch` should log a string in the format `"Randomly selected lunch: [Lunch Item]"` to the console.Additionally, the `getRandomLunch` function  must not mutate the input array.

```js
function checkIfMutate(input, fn) {
  const original = [...input];  
  fn(input);  // Call the function passed as `fn` that might mutate the array
  return !input.every((val, index) => val === original[index]);  
}
...
try {
  ...
  // First, check for mutation before any operations
  let mutated = checkIfMutate(testLunches, getRandomLunch);
  ...
  // Check for mutation after the first call
  mutated = checkIfMutate(testLunches, getRandomLunch);
  assert.strictEqual(mutated, false);  
  ...
  // Check for mutation after the second call
  mutated = checkIfMutate(testLunches, getRandomLunch);
  assert.strictEqual(mutated, false);  
  ...
  // Check for mutation after the third call
  mutated = checkIfMutate(testLunches, getRandomLunch);
  assert.strictEqual(mutated, false);  
