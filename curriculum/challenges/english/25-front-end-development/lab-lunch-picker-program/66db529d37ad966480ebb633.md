...
In this lab, you will create a program that will help you decide what to eat for lunch. You will have two arrays with different food items. You will use the `push`, `pop` and `shift` methods to modify the arrays. You will also use the `Math.random()` method to generate a random number that will be used to select a random meal from the arrays.

**Objective:** Fulfill the user stories below and get all the tests to pass to complete the lab. 

**User Stories:**

1. You should create a `kidsMeal` array and assign more than two string values to it. You can either use `push()` or assignment to add the items to the array.
2. You should create an `adultsMeal` array and assign more than two string values to it. You can either use `push()` or assignment to add the items to the array.
3. Once items have been added to the `kidsMeal`, you should remove the last item from it using `pop()`.
4. Once items have been added to the `adultsMeal`, you should remove the first item from it using `shift()`.
5.  Generate a random number between `0` to the length of the `kidsMeal` array using `Math.random()` and store it in a variable called `randomIndexKids`.
6. Generate a random number between `0` to the length of the `adultsMeal` array using `Math.random()` and store it in a variable called `randomIndexAdults`.
