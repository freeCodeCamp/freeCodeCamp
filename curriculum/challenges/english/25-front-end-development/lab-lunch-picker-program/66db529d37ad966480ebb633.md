...
const tempArr = [];
const temp = console.log;
// Prevent mutation by freezing
const frozenLunches = Object.freeze([...testLunches]); 
...
try {
  ...
  Math.random = () => 0;
  getRandomLunch(frozenLunches);
  assert.strictEqual(tempArr[0], `Randomly selected lunch: ${frozenLunches[0]}`);

  // second item
  Math.random = () => 0.4;
  getRandomLunch(frozenLunches);
  assert.strictEqual(tempArr[1], `Randomly selected lunch: ${frozenLunches[1]}`);

  // third item
  Math.random = () => 0.8;
  getRandomLunch(frozenLunches);
  assert.strictEqual(tempArr[2], `Randomly selected lunch: ${frozenLunches[2]}`);
