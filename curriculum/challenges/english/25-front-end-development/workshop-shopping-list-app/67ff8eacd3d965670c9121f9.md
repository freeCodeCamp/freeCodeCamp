...
Set `toggleItem` equal to `useCallback`, and pass it an arrow function that takes `item` as an argument. Move your existing `toggleItem` function logic inside the arrow function, where `setSelectedItems` is called. Finally, add `setSelectedItems` as a dependency in the dependencies array.
