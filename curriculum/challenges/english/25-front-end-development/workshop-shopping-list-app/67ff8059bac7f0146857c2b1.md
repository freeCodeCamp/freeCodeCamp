...
You should destructure `useCallback` from `React` at the top of the file.

```js
const destructuredObjContents = (code) => {
  const match = code.match(/const\s+{([^}]+)}\s*=\s*React/);
  return match ? match[1].split(",").map((s) => s.trim()) : [];
};

assert.isTrue(
  destructuredObjContents(code).includes("useState") &&
  destructuredObjContents(code).includes("useMemo") &&
  destructuredObjContents(code).includes("useCallback")
);
```
