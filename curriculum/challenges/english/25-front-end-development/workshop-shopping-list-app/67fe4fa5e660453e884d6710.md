...
assert.match(shoppingListString, /function toggleItem\s*\(\s*item\s*\)\s*{/);
```
...
assert.match(shoppingListString, /function toggleItem\s*\(\s*item\s*\)\s*{.*setSelectedItems\s*\(\s*\)/s);
```
