...
First, call the `setSelectedItems` function, and pass it a function as an argument. This function should take `prev` as an argument, which is the previous state of `selectedItems`.

Inside the function, use the `includes()` method to check if `prev` includes `item`.
...
export const ShoppingList = () => {
  ...
  --fcc-editable-region--
  const toggleItem = (item) => setSelectedItems();
  --fcc-editable-region--
