...
Next, you need to you need to create a state variable, a setter function, and set the initial state with the `useState` hook to store the user's input.
...
You should create a state variable named `query` and a setter function to update it named `setQuery`.

```js
assert.match(code, /(const|let)\s+\[\s*query\s*,\s*setQuery\s*\]\s*=\s*useState\(/);
```

The initial value of `query` should be an empty string.

```js
const queryValue = code.match(/(?:const|let)\s+\[\s*query\s*,\s*setQuery\s*\]\s*=\s*useState\(\s*(['"])(.*?)\1\s*\)/)?.[2] ?? null;
