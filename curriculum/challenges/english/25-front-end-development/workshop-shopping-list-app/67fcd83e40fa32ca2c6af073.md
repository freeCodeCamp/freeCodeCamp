...
const itemArrContentsString = code.match(/(?:const|let)\s+items\s*=\s*\[\s*([\s\S]*?)\s*\]/)?.[1] ?? null;
const itemArr = itemArrContentsString ? itemArrContentsString.split(",").map(item => item.replace(/['"]/g, "")) : [];
const normalizeArray = (arr) => arr.map((str) => str.toLowerCase().trim());

assert.isTrue(
  itemArr.length === expected.length &&
  normalizeArray(itemArr).every((item) => normalizeArray(expected).includes(item))
);
```
