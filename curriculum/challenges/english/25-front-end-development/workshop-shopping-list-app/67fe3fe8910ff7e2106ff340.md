...
First, add a `ul` element below the `p` element in the `form`. Inside the unordered list, use the `map` method to iterate over the `filteredItems` array, and pass an arrow function to it. The arrow function should take `item` as an argument, and return an `li` element for each item. Each `li` should display the `item` text.
...
Your `map` method should take a function that takes `item` as an argument.
...
Your `map` function should return an `li` element with the `item` text for each element in the `filteredItems` array.
