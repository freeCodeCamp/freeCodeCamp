...
Each of your `.left`, `.middle`, and `.right` elements should not be empty.

```js
const els = document.querySelectorAll('.right, .middle, .left');
assert.isAtLeast(els.length, 1);
els.forEach(el => assert.isAtLeast(el.innerText.trim().length, 1));
```
