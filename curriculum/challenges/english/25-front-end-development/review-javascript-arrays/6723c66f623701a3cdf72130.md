...
- **`slice()` Method**: This method returns a shallow copy of a portion of the array, starting from a specified index or the entire array. Since the slice() method creates a new array, it does not just copy the reference of the original array. However, if the array contains objects, the references to those objects are copied, not the objects themselves.
