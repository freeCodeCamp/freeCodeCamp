...
- **`slice()` Method**: This method returns a shallow copy of a portion of the array, starting from a specified index, or the entire array. A shallow copy means the new array contains the same elements: for primitive values the values are copied, but for objects the references are copied (so modifying an object element will affect that object in both arrays).
