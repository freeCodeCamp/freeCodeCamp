...
`const` can be updated but not re-declared. `let` can be re-declared, but not updated.

#### --answer--

`const` cannot be updated or re-declared. `let` can be updated, but not re-declared.
