...
**Question:** What is the PRIMARY disadvantage of using a single table to store both movie and actor data in this scenario?
...
You downloaded a compressed archive called "notes.tar.gz" from the internet. You need to unarchive this file in a directory called "study_materials" that you've already created in your home directory. Which command would you use?
...
You have a database table named `orders` with one column named `order_date`, showing the date each order was placed. Your task is to write a SQL query that retrieves all the orders taken in the year 2022. Of which one of the following statements is true?
