...
Create a `getDifficultyLevel` function that takes number indicating cooking time as a parameter.

If cooking time is less than or equal to `30`, the function should return `"easy"`. If it is less than or equal to `60`, the function should return `"medium"`. Otherwise, the function should return `"hard"`.
