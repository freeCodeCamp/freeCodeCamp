...
Create `getDifficultyLevel` function with `cookingTime` as the parameter. Inside the function. Create the conditional to help you assign an `easy`, `medium`, and `hard` difficulty levels based on the `cookingTime` of each recipe.
