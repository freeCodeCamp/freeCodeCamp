...
```js
assert.throws(() => shouldThrow(recipe1Name, "recipe1Name"), ReferenceError);
```
...
```js
assert.throws(() => shouldThrow(recipe2Name, "recipe2Name"), ReferenceError);
```
...
```js
assert.throws(() => shouldThrow(recipe1CookingTime, "recipe1CookingTime"), ReferenceError);
```
...
```js
assert.throws(() => shouldThrow(recipe2CookingTime, "recipe2CookingTime"), ReferenceError);
```
...
```js
const shouldThrow = (identifier, identifierName) => {
  try {
    identifier;
    assert.fail("Variable is defined/declared");
  } catch (e) {
    console.log(identifierName, "- should not be defined/declared");
  }
};
```
