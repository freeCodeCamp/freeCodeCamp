...
It is used to create space below each list item.
...
What is WCAG's recommended minimum contrast ratio for normal text? 
...
What is the role of the `linear-gradient` property?
...
It creates a background that radiates and transitions between multiple colors along a straight line.
...
Which of the following is the correct way to use the `background` shorthand property?
