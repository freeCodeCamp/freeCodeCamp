...
`pause()` automatically resets to beginning while `play()` starts from where the audio was paused
...
Check if audio is currently playing in the browser

---

Verify if a media file exists in the browser

---

Test if an audio element has controls enabled
