...
- **Structural hierarchy for heading elements**: `h1` to `h6`, from highest level to lowest.
...
- **Main**: Holds main web page content.
- **Navigation (`nav`)**: Contains navigation links.
- **Figure**:  Contains illustrations and diagrams.
- **Emphasis (`em`) element**: Marks text that has stress emphasis.
- **Idiomatic Text (`i`) element**: Highlights alternative voice or mood, idiomatic terms from another language, technical terms, and thoughts.
- **Strong Importance (`strong`) element**: Marks text that has strong importance.
- **Bring Attention To (`b`) element**: Brings attention to text that is not important for the meaning of the content.
- **Description List (`dl`) element**: Represents a list of term-description groupings.
- **Description Term (`dt`) element**: Represents the term being defined.
- **Description Details (`dd`) element**: Represents the description of the term.
- **Block Quotation (`blockquote`) element**: Represents a quoted section from another source.
- **Inline Quotation (`q`) element**: Represents a short inline quotation.
- **Abbreviation (`abbr`) element**: Represents an abbreviation or acronym.
- **Contact Address (`address`) element**: Represents the contact information. 
- **(Date) Time (`time`) element**: Represents a date and/or time.
- **Superscript (`sup`) element**: Represents superscript text.
- **Subscript (`sub`) element**: Reperesents subscript text.
- **Inline Code (`code`) element**: Represents a fragment of computer code.
- **Unarticulated Annotation (`u`) element**: Represents a span of inline text which should be rendered in a way that indicates that it has a non-textual annotation.
- **Ruby Annotation (`ruby`) element**: Represents the text of a ruby annotation.
- **Strikethrough (`s`) element**: Represents content that is no longer accurate or relevant.
