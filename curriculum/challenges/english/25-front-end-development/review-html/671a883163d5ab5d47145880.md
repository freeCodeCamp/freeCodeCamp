...
- **Web Content Accessibility Guidelines**: set of guidelines for making web content more accessible to people with disabilities. The four principles of WCAG are POUR which stands for Perceivable, Operable, Understandable, and Robust.
...
- **`aria-label` and `aria-labelledby` attributes**: used to provide a text alternative for an element. The `aria-labelledby` attribute can be used to associate an element with another element that serves as its label. This helps screen readers and other assistive technologies understand the purpose of the element.
- **`aria-hidden` attribute**: used to hide an element from screen readers. This can be useful for decorative elements that do not provide any meaningful content.
