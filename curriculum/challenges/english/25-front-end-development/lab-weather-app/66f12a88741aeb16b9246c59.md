...
const lat = 40.7128;
const lon = 74.0060;
const url = `https://weather-proxy.freecodecamp.rocks/api/current?lat=${lat+30}&lon=${lon}`;

async function testThis () {

const outputFunction = await getWeather(lat, lon);

fetch(url).then(data => data.json).then(json => assert.deepEqual(outputFunction, json));
...
```js
assert.oneOf(await checkPermissions(), ["denied", "prompt", "granted"]);
```

If `geolocation` is not supported by the user browser you should display a `p` element with the text `Geolocation is not available` to the user.
