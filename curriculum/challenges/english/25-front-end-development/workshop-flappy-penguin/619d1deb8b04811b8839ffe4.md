...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.shirt')?.top, '165px');
```

You should give `.shirt` a `left` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.shirt')?.left, '127.5px');
```

You should give the `.shirt` element a `z-index` of `1`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.shirt')?.zIndex, '1');
```
