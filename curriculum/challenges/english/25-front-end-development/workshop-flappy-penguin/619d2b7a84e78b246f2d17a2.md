...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.arm.left')?.top, '35%');
```

You should give `.arm.left` a `left` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.arm.left')?.left, '5%');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.arm.right')?.right, '-5%');
```
