...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.shirt div')?.top, '22.5px');
```

You should give `.shirt div` a `left` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.shirt div')?.left, '12px');
```
