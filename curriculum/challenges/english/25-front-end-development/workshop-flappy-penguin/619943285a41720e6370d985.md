...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top, '40%');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.left, '23.5%');
```
