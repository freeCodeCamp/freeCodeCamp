...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.eye')?.width, '15%');
```

You should give `.eye` a `height` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.eye')?.height, '17%');
```

You should give `.eye` a `background-color` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.eye')?.backgroundColor, 'black');
```
