...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak.top')?.width, '20%');
```

You should give `.beak.top` a `top` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak.top')?.top, '60%');
```

You should give `.beak.top` a `left` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak.top')?.left, '40%');
```
