...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

You should give `.back-mountain` a `height` of `300px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```
