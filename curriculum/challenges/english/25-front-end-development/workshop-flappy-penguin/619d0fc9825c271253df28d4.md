...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.blush')?.width, '15%');
```

You should give `.blush` a `height` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.blush')?.height, '10%');
```

You should give `.blush` a `background-color` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.blush')?.backgroundColor, 'pink');
```
