...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.sun')?.position, 'absolute');
```

You should give `.sun` a `border-radius` of `50%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.sun')?.borderRadius, '50%');
```
