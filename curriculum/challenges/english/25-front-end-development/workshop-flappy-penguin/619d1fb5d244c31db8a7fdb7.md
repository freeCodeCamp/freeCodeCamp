...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.foot')?.width, '15%');
```

You should give `.foot` a `height` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.foot')?.height, '30%');
```

You should give `.foot` a `background-color` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.foot')?.backgroundColor, 'orange');
```
