...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.width, '50%');
```

You should give `.penguin-body::before` a `height` of `45%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.height, '45%');
```

You should give `.penguin-body::before` a `background-color` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.backgroundColor, 'gray');
```
