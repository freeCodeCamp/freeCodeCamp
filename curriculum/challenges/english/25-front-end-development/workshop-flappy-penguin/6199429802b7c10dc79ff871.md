...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.top, '10%');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.left, '25%');
```
