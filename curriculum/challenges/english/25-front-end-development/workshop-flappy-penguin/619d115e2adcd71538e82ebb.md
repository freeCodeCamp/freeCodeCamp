...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak')?.height, '10%');
```

You should give `.beak` a `background-color` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak')?.backgroundColor, 'orange');
```

You should give `.beak` a `border-radius` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak')?.borderRadius, '50%');
```
