...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.width, '300px');
```

You should give `.left-mountain` a `height` of `300px`. Expected `--fcc-actual--` to be `--fcc-expected--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.height, '300px');
```
