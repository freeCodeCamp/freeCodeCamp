...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin')?.marginRight, 'auto');
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin')?.marginLeft, 'auto');
```

You should give `.penguin` a `margin-top` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin')?.marginTop, '75px');
```
