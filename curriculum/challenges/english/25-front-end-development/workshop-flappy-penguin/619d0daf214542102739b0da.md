...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

You should give `.eye-lid` a `height` of `--fcc-expected--`, found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.height, '100%');
```

You should give `.eye-lid` a `background-color` of `var(--penguin-face)`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.getPropVal('background-color', true), 'var(--penguin-face)');
```
