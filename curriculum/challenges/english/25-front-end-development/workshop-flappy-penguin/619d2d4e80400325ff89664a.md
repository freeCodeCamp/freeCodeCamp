...
if (leftArm?.rotate && leftArm?.transform) {
  assert.strictEqual(leftArm.getPropVal('rotate', true), '130deg');
  assert.strictEqual(leftArm.getPropVal('transform', true), 'scaleX(-1)');
} else {
  assert.strictEqual(leftArm.getPropVal('transform', true), 'rotate(130deg)scaleX(-1)');
}
```
