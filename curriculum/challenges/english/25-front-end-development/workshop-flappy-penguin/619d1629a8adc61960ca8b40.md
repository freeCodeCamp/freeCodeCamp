...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontSize, '25px');
```

You should give `.shirt` a `font-family` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontFamily, 'Helvetica, sans-serif');
```

You should give `.shirt` a `font-weight` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontWeight, 'bold');
```
