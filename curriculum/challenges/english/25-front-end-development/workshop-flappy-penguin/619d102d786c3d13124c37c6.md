...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.blush')?.top, '65%');
```

You should give `.blush` a `border-radius` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.blush')?.borderRadius, '50%');
```
