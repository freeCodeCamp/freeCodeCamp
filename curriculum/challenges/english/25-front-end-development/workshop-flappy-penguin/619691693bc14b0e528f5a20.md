...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.ground')?.width, '100vw');
```

You should give the `.ground` element a `height` of `400px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.ground')?.height, '400px');
```
