...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.chin')?.width, '90%');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.chin')?.height, '70%');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.chin')?.backgroundColor, 'white');
```
