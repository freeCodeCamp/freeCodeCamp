...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.width, '16%');
```

You should give `.beak.bottom` a `top` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.top, '65%');
```

You should give `.beak.bottom` a `left` of `--fcc-expected--`, but found `--fcc-actual--`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.left, '42%');
```
