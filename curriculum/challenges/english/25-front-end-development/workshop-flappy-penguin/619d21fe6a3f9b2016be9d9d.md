...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.foot.left')?.left, '25%');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.foot.right')?.right, '25%');
```
