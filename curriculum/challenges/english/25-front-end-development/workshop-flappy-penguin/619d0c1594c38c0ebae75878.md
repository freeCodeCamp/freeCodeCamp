...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.eye.left')?.left, '25%');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.eye.right')?.right, '25%');
```
