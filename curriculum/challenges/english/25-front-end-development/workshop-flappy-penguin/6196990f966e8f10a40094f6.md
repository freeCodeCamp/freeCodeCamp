...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin')?.width, '300px');
```

You should give `.penguin` a `height` of `300px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin')?.height, '300px');
```
