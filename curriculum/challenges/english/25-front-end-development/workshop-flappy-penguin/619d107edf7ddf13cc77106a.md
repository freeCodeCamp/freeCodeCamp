...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.blush.left')?.left, '15%');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.blush.right')?.right, '15%');
```
