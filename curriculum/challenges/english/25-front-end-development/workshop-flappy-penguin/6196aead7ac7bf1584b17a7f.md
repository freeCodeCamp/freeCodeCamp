...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.marginTop, '100px');
```

You should give `.ground` a `margin-top` of `-58px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.ground')?.marginTop, '-58px');
```
