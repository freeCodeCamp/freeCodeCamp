...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex, '4');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin')?.position, 'relative');
```
