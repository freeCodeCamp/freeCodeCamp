...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.sun')?.width, '200px');
```

You should give `.sun` a `height` of `200px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.sun')?.height, '200px');
```

You should give `.sun` a `background-color` of `yellow`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.sun')?.backgroundColor, 'yellow');
```
