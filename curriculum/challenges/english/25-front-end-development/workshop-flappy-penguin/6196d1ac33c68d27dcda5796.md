...
if (backMountain?.transform) {
  assert.strictEqual(backMountain?.transform, 'rotate(45deg)');
} else {
  assert.strictEqual(backMountain?.rotate, '45deg');
}
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.left, '110px');
```
...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.top, '225px');
```
