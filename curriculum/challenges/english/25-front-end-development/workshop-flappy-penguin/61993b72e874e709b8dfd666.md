...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

You should give `.penguin-head` a `height` of `45%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```
