...
```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

You should give `.chin` a `background-color` of `var(--penguin-face)`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.chin')?.getPropVal('background-color', true), 'var(--penguin-face)');
```
