...
Now call `.json()` on the response and store the result in a variable called `data`. Since you're still inside an `async` function, you should use `await` before `response.json()`.
