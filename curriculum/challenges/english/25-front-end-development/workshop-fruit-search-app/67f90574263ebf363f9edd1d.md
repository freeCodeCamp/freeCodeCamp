...
Inside the `useEffect` hook, after the `if` statement, you should call `setTimeout` and store the return value in a variable called `timeoutId`. This allows you to cancel the timeout later when the effect cleans up.
...
```js
assert.match(code, /const\s+timeoutId\s*=\s*setTimeout\s*\(/s);
```
