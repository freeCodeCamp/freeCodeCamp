...
You should update the `results` state by calling `setResults` and passing in an array that contains only the `name` property from each fruit in the `data` array. You can use the `map()` method for this.
