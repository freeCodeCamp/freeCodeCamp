...
Inside the `#results` element, use a ternary operator to check if the `results` array has items. If it does, display each fruit name in a `p` element with the class of `result-item`. If it's empty, render a `p` element with the message `No results found`.
