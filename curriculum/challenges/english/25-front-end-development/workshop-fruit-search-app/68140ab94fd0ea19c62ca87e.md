...
Below your `timeoutId` variable, return a cleanup function that clears the timeout using `clearTimeout(timeoutId)`. This prevents multiple delayed fetches from stacking up. Make sure your cleanup function is an arrow function.

Now try out your fruit search app by typing in the name of a fruit like `apple` or `pear` to see the results appear.

And with that your Fruit Search app is complete!
