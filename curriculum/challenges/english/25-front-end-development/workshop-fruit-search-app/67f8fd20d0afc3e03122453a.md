...
Use the `useEffect` hook to run side effects based on the `query` state. Make sure to pass an arrow function as the first argument to `useEffect`, and include `query` in the dependency array.
