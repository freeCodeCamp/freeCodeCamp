...
Inside the `form` element, add an `input` element with an `id` of `search-input` and `type` `text`.

# --hints--

You should have an `input` element inside the `form` element with an `id` of `search-input`.
...
The `input` element should have a `type` value of `text`
