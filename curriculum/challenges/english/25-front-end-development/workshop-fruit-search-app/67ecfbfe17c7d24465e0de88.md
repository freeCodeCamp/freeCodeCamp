...
Inside the `form` element, add a search input with an `id` of `search-input` and `type` `search`.

# --hints--

You should have a search input inside the `form` element with an `id` of `search-input`.
...
The search input should have a `type` value of `search`
