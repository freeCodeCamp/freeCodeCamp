...
In the previous lecture videos, you learned about handling side effects like data fetching by using the `useEffect` Hook. In this workshop, you'll build an application that fetches fruit data from an API based on user input and displays the results dynamically. 

First, return a `div` element with an `id` of `search-container`.
