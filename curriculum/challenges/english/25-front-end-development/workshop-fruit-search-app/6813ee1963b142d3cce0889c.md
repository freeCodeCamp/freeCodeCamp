...
Inside the `try` block, call the `fetch` function with the `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` URL, and store the result in a variable named `response`.
...
assert.match(
  code,
  /const\s+response\s*=\s*await\s+fetch\(\s*`https:\/\/fruit-search\.freecodecamp\.rocks\/api\/fruits\?q=\$\{query\}`\s*\)/s
);
```
