...
title: Build a Poker Hand Page
challengeType: 14
dashedName: build-a-poker-hand-page
demoType: onClick
---
...
You should have a `main` element with an `id` of `poker-hand`.

```js
assert.exists(document.querySelector('main#poker-hand'));
```

You should have at least three `div` elements with a class of `card` within your `#poker-hand` element

```js
assert.isAtLeast(document.querySelectorAll('#poker-hand > div.card')?.length, 3);
```
...
const lefts = document.querySelectorAll('.card > .left')?.length;
assert.equal(lefts, cards);
```
...
```js
const cards = document.querySelectorAll('.card')?.length;
const middles = document.querySelectorAll('.card > .middle')?.length;
assert.equal(middles, cards);
```
...
```js
const cards = document.querySelectorAll('.card')?.length;
const rights = document.querySelectorAll('.card > .right')?.length;
assert.equal(rights, cards);
```

You should have a `#poker-hand` selector that sets its elements' `display` to `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#poker-hand')?.display, 'flex');
```

You should have a `#poker-hand` selector that sets its elements' `justify-content` to `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#poker-hand')?.justifyContent, 'center');
```

You should have a `#poker-hand` selector that sets its elements' `flex-wrap` to `wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#poker-hand')?.flexWrap, 'wrap');
```

You should have a `#poker-hand` selector that sets its elements' `gap` to `20px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#poker-hand')?.gap, '20px');
```
