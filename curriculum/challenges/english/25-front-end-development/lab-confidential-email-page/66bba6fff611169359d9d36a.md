...
```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#email')?.padding, '50px');
```

You should have an `#email` selector that sets its elements' `margin-top` to `50px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#email')?.marginTop, '50px');
```

You should have an `#email` selector that sets its elements' `width` to `500px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#email')?.width, '500px');
```
