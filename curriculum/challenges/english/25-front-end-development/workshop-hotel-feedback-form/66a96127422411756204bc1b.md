...
Below the label element, add another checkbox input with the `id` and `name` attributes set to `"recommendation"`.
...
```js
assert(document.querySelector("fieldset:nth-of-type(3) input[type=checkbox]"));
```

Your checkbox should have an `id` set to `"recommendation"`.

```js
assert.strictEqual(document.querySelector("fieldset:nth-of-type(3) input[type=checkbox]").id, "recommendation");
```

Your checkbox should have a `name` attribute set to `"recommendation"`.

```js
assert.strictEqual(document.querySelector("fieldset:nth-of-type(3) input[type=checkbox]").name, "recommendation");
```
