...
Your `input` should have an `id` attribute set to `"no-option"`.

```js
assert.strictEqual(document.querySelector('fieldset:nth-of-type(2) input[type="radio"]').id, 'no-option');
```

Your `input` should have a `name` attribute set to `"hotel-stay"`.
