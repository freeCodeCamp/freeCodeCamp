...
Next, add another `label` element with the text of `Location` and the `for` attribute set to `"location"`. For the checkbox `input`, both the `id` and `name` attributes should be set to `"location"`.

Below that `label` element, add another `label` element with the text of `Reputation` and the for attribute set to `"reputation"`. For the checkbox `input`, both the `id` and `name` attributes should be set to `"reputation"`.
