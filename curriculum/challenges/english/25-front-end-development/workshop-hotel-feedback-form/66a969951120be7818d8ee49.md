...
```js
assert.lengthOf(document.querySelectorAll('fieldset').length, 4);
```

Your `fieldset` should have a `legend` element.

```js
assert.lengthOf(document.querySelectorAll('fieldset:nth-of-type(4) legend').length, 1);
```
...
```js
assert.lengthOf(document.querySelectorAll('fieldset:nth-of-type(4) legend + label').length, 1);
```
