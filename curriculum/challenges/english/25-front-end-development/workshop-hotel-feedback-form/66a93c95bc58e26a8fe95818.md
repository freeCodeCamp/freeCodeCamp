...
Next, add an `input` with the type of `"email"` to below your email `label`. This `input` should have an `id` attribute set to the value of `"email"`. Also, set the `name` attribute to the value of `"email"`. 
