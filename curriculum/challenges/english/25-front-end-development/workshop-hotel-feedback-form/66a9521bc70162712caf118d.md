...
```js
assert(document.querySelector('fieldset:nth-of-type(2) label').htmlFor === 'yes-option');
```

Your label should have the text `"Yes"`.

```js
assert.strictEqual(document.querySelector('fieldset:nth-of-type(2) label').textContent, 'Yes');
```
