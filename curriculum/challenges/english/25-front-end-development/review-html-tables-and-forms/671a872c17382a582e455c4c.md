...
- **`name` attribute**: defines the name for an input's submitted `value`. For **radio buttons**, a shared `name` groups them, allowing only one selection from that group.
