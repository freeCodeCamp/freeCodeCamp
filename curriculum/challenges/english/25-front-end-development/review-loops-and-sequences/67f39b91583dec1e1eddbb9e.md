...
- **Slicing Lists**: Slicing is the concept of accessing a portion of a list by using the slice operator `:`. 
...
- **index()**: Used to find the index where a particular item is present in the tuple. 
...
- **Modifying Sorting Behaviour**: You can customize the sorting behavior for an iterable using the optional `reverse` and `key` arguments. 
...
- **`for` loop**: Used to iterate over a sequence (like a list, tuple or string) and execute a block of code for each item in that sequence.
...
- **`while` loop**: Repeats a block of code until the condition is `False`. 
...
- **`enumerate()`**: used to iterate over a sequence and keep track of the index for each item in that sequence.
...
-  **`zip()`** : Used to iterate over multiple iterables in parallel.
...
- **`filter()`**: Used to filter elements from an iterable based on a condition. It returns an iterator that contains only the elements that satisfy the condition.
