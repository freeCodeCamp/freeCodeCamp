...
(combined with &&) The `moon` `div` should have a width and height of `30px`.
...
```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.earth')?.backgroundColor);
```

Your `.earth` element should have `border-radius` to make it look like a circle.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.earth')?.borderRadius);
```

Your `.moon` `div` element should have a background color.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.moon')?.backgroundColor);
```

Your `.moon` element should have `border-radius` to make it look like a circle.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.moon')?.borderRadius);
```
