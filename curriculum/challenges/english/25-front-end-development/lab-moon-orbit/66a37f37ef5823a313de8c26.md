...
dimensions of orbit, moon, earth
positioning
...
```js
const shortHand = new __helpers.CSSHelp(document).getStyle('.orbit');
assert(
    shortHand?.animationName === "orbit" &&
    shortHand?.animationDuration === "5s" &&
    shortHand?.animationTimingFunction === "linear" &&
    shortHand?.animationIterationCount === "infinite"
)
```
