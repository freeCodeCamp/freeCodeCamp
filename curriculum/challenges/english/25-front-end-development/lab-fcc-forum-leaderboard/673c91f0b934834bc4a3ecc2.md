...
1. Each `tr` element should have five `td` elements in it:
    ...
    - a `td` containing the images returned by the `avatars` function called with `posters` and `users` as arguments and nested within a `div` element with the class of `avatar-container`.
...
assert.equal(pContainer.querySelector("tr:first-child>td:nth-child(5)").innerText, "225d ago");

assert.equal(pContainer.querySelector("tr:last-child>td:nth-child(5)").innerText, "221d ago");
```
