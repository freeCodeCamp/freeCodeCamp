...
Attach an `addEventListener` to the `note` variable. The first argument should be the `"focus"` event and the second argument should be a callback function. Inside of that callback function, set the `status.textContent` to an empty string. This wil reset live region on focus.

And with that last change, your workshop is complete!

# --hints--

When the user focuses on the note, you should set the `status.textContent` to an empty string.

```js
note.dispatchEvent(new Event("blur"));

note.innerHTML = "New content";
assert.equal(note.innerHTML, "New content");
assert.equal(status.textContent, "Note saved successfully!");

note.dispatchEvent(new Event("focus"));
