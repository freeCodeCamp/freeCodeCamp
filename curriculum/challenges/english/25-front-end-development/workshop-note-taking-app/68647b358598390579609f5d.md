...
Attach an `addEventListener` to the `note` variable. The first argument should be the `"focus"` event and the second argument should be a callback function. Inside of that callback function, set the `statusEl.textContent` to an empty string. This wil reset live region on focus.
