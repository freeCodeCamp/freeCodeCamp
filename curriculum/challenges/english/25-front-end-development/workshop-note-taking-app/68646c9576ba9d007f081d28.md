...
When everything in the HTML document has been loaded and parsed, you will be able to access the `note` safely.

Start by attaching an `addEventListener` method to the `window` object. The first argument for the `addEventListener` method should be the `"DOMContentLoaded"` event. The second argument should be an arrow function. Inside the body of the arrow function, reassign `currentContent` to the value of `note.textContent`.
...
Inside the body of the arrow function, reassign `currentContent` to the value of `note.textContent`.

```js
const event = new Event("DOMContentLoaded");
window.dispatchEvent(event);

assert.equal(currentContent, note.textContent);
```
