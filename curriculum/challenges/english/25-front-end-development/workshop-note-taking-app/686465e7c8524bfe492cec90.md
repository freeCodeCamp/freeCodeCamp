...
Start by creating a variable called `note` and assigning it the result of querying the document for the element with the `id` of `note`.

Then create another variable called `status` and assign it the result of querying the document for the element with the `id` of `status`.

# --hints--

You should have a variable called `note`.

```js
assert.exists(note);
```

Your `note` variable should be the result of querying the document for the element with the `id` of `note`.

```js
assert.equal(note, document.getElementById("note"));
```

You should have a variable called `status`.

```js
assert.exists(status);
```   

Your `status` variable should be the result of querying the document for the element with the `id` of `status`.

```js
assert.equal(status, document.getElementById("status"));
```
