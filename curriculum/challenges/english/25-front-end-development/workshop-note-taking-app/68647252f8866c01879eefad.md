...
Attach an `addEventListener` method to the `note` variable. The first argument should be the `blur` event and the second argument should an arrow function. Inside of the body of that arrow function, use `const` to create a new variable called `newContent` and assign it the value of `note.innerHTML`. 

`innerHTML` is used here to help preserve spacing and formatting for the note. 

# --hints--

You should attach an `addEventListener` method to the `note` variable.

```js
assert.match(code, /note\.addEventListener/);
```

You should use the `blur` event as the first argument to the `addEventListener` method.

```js
assert.match(code, /note\.addEventListener\(['"]blur['"]\)/);
```

Inside of the body of the arrow function, use `const` to create a new variable called `newContent` and assign it the value of `note.innerHTML`.

```js
assert.match(code, /note\.addEventListener\(['"]blur['"],\s*\(\)\s*=>\s*{[^}]*const\s+newContent\s*=\s*note\.innerHTML[^}]*}\)/);
```
