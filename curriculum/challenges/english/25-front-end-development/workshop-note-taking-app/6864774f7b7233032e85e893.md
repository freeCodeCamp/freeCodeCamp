...
You should add a `console.log(currentContent);` inside of the event listener.

```js
assert.match(code, /note\.addEventListener\(['"]blur['"],\s*\(\)\s*=>\s*{[^}]*const\s+newContent\s*=\s*note\.innerHTML[^}]*currentContent\s*=\s*newContent[^}]*console\.log\(currentContent\)[^}]*}\)/);
```
