...
2. The `transactions` object defined inside the `BankAccount` constructor should be able to take two properties: `type` and `amount`. The `type` property should be either `deposit` or `withdraw`, and the `amount` property should be the amount deposited or withdrawn.
