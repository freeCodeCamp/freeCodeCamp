...
You already have access to the `units` array — a `forEach` loop is a convenient way to process each unit.

Check the current selected unit with `unitToConvert.value` and skip it with a simple `if` condition.

`processIngredient` will give you the converted quantity — you may want to format it with `.toFixed(2)` before displaying.

You can add each new result to `resultList` by updating its `innerHTML` with a string like:

`<li>Flour: 0.10 cup</li>`
