...
In this step, you will create an `updateResultsList` function and connect it to the form so it runs when the **Convert** button is clicked.

First, define the `updateResultsList` function so that it clears the `resultList` element. This will make sure old results are removed before new ones are added.

Then, add an event listener to `recipeForm` for the `"submit"` event. Inside the event listener, prevent the default form submission behavior with `event.preventDefault()`, and call `updateResultsList()`.

This will allow you to trigger the function through the UI by filling in the form and pressing **Convert**, even though the conversion logic will be added in a later step.

# --hints--

Your `updateResultsList` function should clear the `resultList` and be called when the form is submitted.
