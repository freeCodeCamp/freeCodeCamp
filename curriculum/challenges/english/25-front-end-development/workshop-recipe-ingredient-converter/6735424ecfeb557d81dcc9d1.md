...
In this step, you will connect `updateResultsList` function to the form so it runs when the **Convert** button is clicked.

Add an event listener to `recipeForm` for the `"submit"` event. Inside the event listener, prevent the default form submission behavior with `event.preventDefault()`, and call `updateResultsList()`.
