...
title: Build Football Team Cards
challengeType: 14 
...
In this lab, you will write the functionality of adding data to the page from a JavaScript object, and of filtering dinamically these datas.

**Objective:** Fulfill the user stories below and get all the tests to pass to complete the lab. 

**User Stories:**

1. You should create a `footballTeam` object, this object should have properties `team`, `year`, `headCoach`, `players`.
...
1. The `isCaptain` property should have value of a boolean, and it should be `true` only once between all the players.
1. You should display the values from the object on the page.
1. You should implent a filtering function: when the dropdown menu is used to select one of the positions, only players of that position should be shown.
...
assert.hasAllKeys(footballTeam, ['team', 'year', 'headCoach', 'players']);
// it could be containsAllKeys if we want to give freedom to have more than these keys
```
...
footballTeam.players.forEach(player => {
  ...
  assert.hasAllKeys(player, ['name', 'position', 'isCaptain']);
})
```
...
You should display the values from the `footballTeam` object on the page.
...
When the option defenders is selected, only defender players should be shown.
