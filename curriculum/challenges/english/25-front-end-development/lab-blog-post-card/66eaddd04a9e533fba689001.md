...
2. Within the `div.blog-post-card`, you should have an image with a valid `alt` text and the class of `post-img`. You can use `https://cdn.freecodecamp.org/curriculum/labs/cover-photo.jpg` for the `src` attribute of your image.
3. You should have a `div` with a class of `post-content` within your `div.blog-post-card`.  
   Within the `.post-content` element:

   - You should have an `h2` with a class of `post-title`. Assign some text to act as the title of your blog post.
   - You should have a `p` with a class of `post-excerpt`. Assign some text to summarize the blog post.
...
You should have a an `img` element with a `class` of `post-img`. Make sure your image has an `alt` text and `src` value.
...
You should have an `h2` with a `class` of `post-title`. Make sure it has some text content.
...
You should have a `p.post-excerpt`.
...
You should have an `a` with a `class` of `read-more`.
...
Your `a.read-more` should have a `Read More` text.
