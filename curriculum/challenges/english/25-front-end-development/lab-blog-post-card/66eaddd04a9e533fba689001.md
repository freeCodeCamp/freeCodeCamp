...
assert.isTrue(hasBoxShadow(card), 'The .blog-post-card should have a box-shadow property set.');
```
...
assert.exists(postImg, 'The element with class "post-img" exists');

const imgWidth = getComputedStyle(postImg).width;
const cardWidth = getComputedStyle(postImg.parentElement).width;
assert.strictEqual(imgWidth, cardWidth, 'The image fills the width of the card');

const imgBorderBottom = getComputedStyle(postImg).borderBottomWidth;
assert.notStrictEqual(imgBorderBottom, '0px', 'The image has a border at the bottom');
```
...
assert.isTrue(isColorApplied(title), 'The post-title should have a color set.');
assert.isTrue(isMarginApplied(title), 'The post-title should have a margin set.');

assert.isTrue(isColorApplied(excerpt), 'The post-excerpt should have a color set.');
assert.isTrue(isMarginApplied(excerpt), 'The post-excerpt should have a margin set.');
...
assert.isTrue(hasTitleColorRule, 'The .post-title should have a defined color in the stylesheet.');
assert.isTrue(hasExcerptColorRule, 'The .post-excerpt should have a defined color in the stylesheet.');
```
