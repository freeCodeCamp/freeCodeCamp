...
**NOTE**: The `push` method is a function that belongs to the `array` object. A function is a block of code that performs a specific task and arguments are the values that the function uses to perform the task.

You will learn more about functions in later sections.
