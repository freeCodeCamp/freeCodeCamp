...
The data in the form will be URL encoded as `name=value` pairs and appended to the `action` URL as query parameters. For example, by submitting the form with the value of `3342` for the `input` field from our previous example, the form would try to `GET` the URL `http://127.0.0.1:5500/data?number=3342`.
