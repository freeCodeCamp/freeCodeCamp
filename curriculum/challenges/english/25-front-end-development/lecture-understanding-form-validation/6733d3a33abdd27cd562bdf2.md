...
And if we look at the result, we can type in the input field as expected. But maybe we don’t want that. Maybe, instead, we’d like to show the character the user types in a separate element.

First, let’s define our element for that:
