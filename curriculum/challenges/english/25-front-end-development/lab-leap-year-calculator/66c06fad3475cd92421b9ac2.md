...
1. Define a function called `isLeapYear` that takes `year` as an argument.
1. Declare a variable `year` that stores the value of the year to check.
1. Inside the funtion, use an `if/ else` statement or a ternary operator to check if the year is a leap year. 
1. To check if the year is a leap year, fulfill the following conditions:
...
1. If the year is a leap year, return `[year] is a leap year.`. Otherwise, return `[year] is not a leap year.`. You will replace [year] with the year variable.
1. You should output the `result` variable to the console using `console.log()`.
1. Don't forget to call `isLeapYear` and pass the year as an argument.
...
The `isLeapYear` funtion should take `year` as an argument.

```js
const condition1 = /isLeapYear\s*=\s*\(year\)\s*=>\s*{/.test(code);
const condition2 = /isLeapYear\(\s*year\s*\)/.test(code);
...
You should store the result of the `isLeapYear` function in a variable named `result`.
