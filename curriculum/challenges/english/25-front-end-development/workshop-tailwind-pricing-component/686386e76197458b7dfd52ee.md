...
```js
const listItems = document.querySelectorAll("ul > li")
const firstLiTextAfterSpan = listItems[0].childNodes[1].textContent.trim();

assert.equal(firstLiTextAfterSpan, "Ad-supported streaming");
```
...
```js
const listItems = document.querySelectorAll("ul > li")
const secondLiTextAfterSpan = listItems[1].childNodes[1].textContent.trim();

assert.equal(secondLiTextAfterSpan, "Curated playlists");
```
