...
Still inside the `div`, create a `p` element with the text `Enjoy the full music experience with unlimited access and downloads.`, and a `ul` element. Leave the `ul` element empty for now.
...
You should create an empty `ul` element inside the `div` element.

```js
const targetDiv = document.querySelectorAll("div")[5]

assert.exists(targetDiv.querySelector("ul"))
```
