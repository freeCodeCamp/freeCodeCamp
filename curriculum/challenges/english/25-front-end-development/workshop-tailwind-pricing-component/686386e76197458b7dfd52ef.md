...
Now, outside the inner `div`, you need to create a CTA button. So, create an anchor element with an `href` of `#` and the text `Start listening`.

A Call to Action (CTA) is a button or link designed to prompt a user to take a specific action, in this case, to start listening.
