...
(Test it) The `alt` attribute of the image should be set to a meaningful description.
...
(Test it) After the first `hr` element, there should be a `p` element with an email address as your text.
...
(To test) Your portfolio `a` element should have a valid `href`.
...
(Works) The Twitter `href` should point to a valid link.
...
(Test it) The GitHub `href` should point to a valid link.
