...
Inside the `.social-media` element, each `a` element should contain the text `Twitter`, `LinkedIn`, and `GitHub`.

```js
const anchorsAway = document.querySelectorAll('.social-media a');
assert.isNotEmpty(anchorsAway);
for (let anchor of anchorsAway) {
  const twitter = anchor.textContent == 'Twitter';
  const linkedin = anchor.textContent == 'LinkedIn';
  const github = anchor.textContent == 'GitHub';
  assert.isTrue(twitter || linkedin || github);
}
```

Inside the `.social-media` element, each `a` elements `href` attribute should point to a valid link.

```js
const anchors = document.querySelectorAll('.social-media a');
assert.isNotEmpty(anchors);
for (let anchor of anchors) {
  assert.isAbove(anchor.getAttribute('href').length, 0);
}
```
