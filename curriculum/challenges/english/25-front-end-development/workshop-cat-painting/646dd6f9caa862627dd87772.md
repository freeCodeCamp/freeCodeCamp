...
```js
assert.isTrue(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.position === 'absolute');
```

Your `.cat-left-inner-ear` selector should have a `top` property set to `22px`.

```js
assert.lengthOf(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.top, '22px');
```

Your `.cat-left-inner-ear` selector should have a `left` property set to `-20px`.

```js
assert.lengthOf(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.left, '-20px');
```
