...
```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderBottomRightRadius).equals('40%')
```

Your `.cat-right-inner-ear` selector should have a `border-bottom-left-radius` property set to `40%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderBottomLeftRadius).equals('40%')
```

Your `.cat-right-inner-ear` selector should have a `border-top-left-radius` property set to `90px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderTopLeftRadius).equals('90px')
```

Your `.cat-right-inner-ear` selector should have a `border-top-right-radius` property set to `10px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderTopRightRadius).equals('10px')
```
