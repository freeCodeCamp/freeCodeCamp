...
```js
assert.equals(new __helpers.CSSHelp(document).getStyle('.cat-mouth div')?.width, '30px')
```

Your `.cat-mouth div` selector should have a `height` property set to `50px`.

```js
assert.equals(new __helpers.CSSHelp(document).getStyle('.cat-mouth div')?.height, '50px')
```

Your `.cat-mouth div` selector should have a `border` property set to `2px solid #000`.

```js
assert.equals(new __helpers.CSSHelp(document).getStyle('.cat-mouth div')?.border, '2px solid rgb(0, 0, 0)')
```
