...
```js
assert.lengthOf(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderTopLeftRadius, '90px');
```

Your `.cat-right-ear` selector should have a `border-top-right-radius` property set to `10px`. Don't forget to add a semicolon.

```js
assert.lengthOf(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderTopRightRadius, '10px');
```
