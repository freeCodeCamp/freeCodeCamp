...
```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.position === 'absolute');
```

Your `.cat-whisker-right-bottom` selector should have a `top` property set to `134px`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.top === '134px');
```

Your `.cat-whisker-right-bottom` selector should have a `left` property set to `109px`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.left === '109px');
```
