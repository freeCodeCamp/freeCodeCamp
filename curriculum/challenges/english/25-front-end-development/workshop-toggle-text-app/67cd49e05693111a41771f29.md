...
In the prior lecture videos, you learned how to create state variables by using the `useState` hook and array destructuring:

```js
const [user, setUser] = useState("Jessica");
```

The convention for naming state variables is to use the `[example, setExample]` format.
