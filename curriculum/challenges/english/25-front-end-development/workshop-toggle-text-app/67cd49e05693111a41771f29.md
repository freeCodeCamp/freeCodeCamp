...
Then assign the `useState` hook with an initial value of `true` to those variables. 

# --hints--

You should have a state variable called `isVisible`.

```js
assert.isDefined(isVisible);
```

You should have a set function called `setIsVisible`.

```js
assert.isDefined(setIsVisible);
```

You should use the array destructuring syntax for these state variables. Refer back to the example.

```js
assert.match(code, /const\s+[\s*isVisible\s*,\s*setIsVisible\s*]\s*/);
```
...
Your `useState` hook should have an initial value of `true`.
