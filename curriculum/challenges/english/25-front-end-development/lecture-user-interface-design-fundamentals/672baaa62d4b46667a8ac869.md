...
Let's consider a basic page layout:

The HTML for the page is semantically correct, but the styling applied does not create a strong visual hierarchy.

While things are separated by headings, the font size isn't distinct so there is no visible indication of the document flow.

If you apply a stronger styling, you can now see how each section is "nested" within the others, with the heading tiers having different font sizes.

But maybe you want to call out your work section specifically? You could use something like a "callout box" to highlight that information.
