...
- **String variables for output formatting**: Use text variables to store reusable strings for consistent output formatting. Helps maintain consistent messaging across script output, easier to modify output format by changing variable once, and combine with other variables for dynamic messages.
...
- **Conditional statements**: Use `if` statements to execute code based on conditions.
  - Syntax: `if [[ CONDITION ]] then STATEMENTS fi`
  - Must have spaces inside the brackets `[[ ... ]]` and around operators
  - `fi` (if backwards) marks the end of the conditional block
  - Common operators: `==` (equal), `!=` (not equal), `-eq` (numeric equal)
  ...
  if [[ $1 == arg1 ]]; then echo "true"; fi
  ```

- **If-else statements**: Add an `else` clause to handle when conditions are not met.
...
- **Elif statements**: Add multiple conditional branches using `elif` (else if).
  - Allows testing multiple conditions in sequence
  - Syntax: `if [[ CONDITION ]] then STATEMENTS elif [[ CONDITION ]] then STATEMENTS fi`
  - Can have multiple `elif` blocks before optional `else`
  - Can mix double parentheses `(( ... ))` and double brackets `[[ ... ]]` in same if statement
