...
It would be nice to horizontally center the `div` element. To apply `margin` to two sides at once, you can use the following patterns: `mx-number` or `my-number`. `mx` sets the margin for left and right while `my` sets the margin for the top and bottom.
