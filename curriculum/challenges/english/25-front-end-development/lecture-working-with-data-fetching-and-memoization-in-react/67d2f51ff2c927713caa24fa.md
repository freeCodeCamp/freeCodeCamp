...
In the last lecture, you learned about memoization and how the us`eMemo hook works.

With this lecture, you'll learn how the `useCallback` hook and `React.memo` work.
...
This counter also has a timer in state that updates every second. This makes the component re-render every time the `timer` changes, making the `handleClick` function get recreated on every render.
...
Without memoization, here's how the two components are doing:

That's not close to fine, because as the component re-renders due to the timer updating every second, the `CounterChild` component is also re-rendered.
