...
A) Absolute paths start from the root directory, while relative paths depend on the location of the current file.
B) Relative paths are shorter and more flexible for linking within a website, while absolute paths include the full URL.
C) Both absolute and relative paths can be used to link to files within the same website.
D) Relative paths cannot use .. to move up directories, but absolute paths can.
...
A) It adds built-in playback controls like play, pause, and volume adjustment.
B) It defines the audio file format for the browser to use.
C) It automatically starts the audio when the page loads.
D) It specifies the audio file’s source URL.

Answer: A

4. Which HTML meta tag helps improve SEO by providing a short description of the web page?
A) og:title
B) meta description
C) og:image
D) og:url
...
A) Sets the language of the webpage to English.
B) Defines the character encoding to ensure proper text display.
C) Links an external CSS stylesheet.
D) Specifies the title of the webpage.

Answer: B

6. What is the difference between <link> and <script> elements in HTML?
A) <link> is for JavaScript files, and <script> is for CSS files.
B) <link> is used to add styles from an external CSS file, while <script> is used to run JavaScript code.
C) Both <link> and <script> are used to style web pages.
D) <script> is for adding styles, and <link> is for running JavaScript.

Answer: B

7. Which of the following statements about HTML classes and IDs is incorrect?
A) A class name can be used multiple times throughout an HTML document.
B) ID names cannot contain underscores and must be unique.
C) The rel attribute defines the relationship between the linked resource and the HTML document.
D) The og:image property is used to specify the image displayed in social media posts.

Answer: B

8. Which of the following statements about HTML heading elements is true?
A) The text size increases from <h1> to <h6>.
B) The text size remains the same for all heading elements.
C) The text size decreases from <h1> to <h6>.
D) <h3> is larger than <h2>.

Answer: C

9. Which statement is incorrect about HTML elements?
A) HTML elements are the basic building blocks of a webpage.
B) Void elements contain content and only have a start tag, like <img> and <meta>.
C) Some codebases include a forward slash (/) inside void elements.
D) A boolean attribute is one that works simply by being present or absent in a tag.

Answer: B

10. Which of the following statements about void elements in HTML is true?
A) A void element must always include a forward slash / before closing.
B) A void element should never include a forward slash /.
C) Both <img> and <img/> are acceptable in modern HTML.
D) The forward slash / is mandatory for self-closing void elements in all versions of HTML.

Answer: C

11. What is the primary purpose of comments in programming?
A) To make the code run faster.
B) To leave notes for yourself and other developers in the code.
C) To change how the browser displays content.
D) To store temporary data within the code.
Answer: B

12. Which statement about HTML heading elements is correct?
A) <h6> is the most important heading element.
B) <h1> has the highest importance, and <h6> has the least importance.
C) All heading elements <h1> to <h6> have the same importance.
D) <h3> is more important than <h1>.
Answer: B

13. What is the purpose of the <em> element in HTML?
A) To make text bigger.
B) To highlight important text by making it italic.
C) To change the text color.
D) To underline words for emphasis.
Answer: B

14. What does the <strong> element do in HTML?
A) Makes text bold to show importance.
B) Increases the font size.
...
A) <figure> is used for captions, while <figcaption> is used for images.
B) <figure> groups related content like images or diagrams, while <figcaption> provides a caption for that content.
C) <figcaption> is used to display images, while <figure> provides a caption.
D) There is no difference; both are used for the same purpose.
Answer: B

16. Why are HTML entities used?
A) To add styling to text.
B) To represent reserved characters like < and & in HTML.
C) To create hyperlinks in a webpage.
D) To increase the font size of special characters.
Answer: B

17. Which of the following statements about Open Graph tags is correct?
A) Open Graph tags are used to style a webpage with CSS.
B) The og:title property sets the title that appears when content is shared on social media.
C) Open Graph tags are only used for SEO and have no effect on social media.
D) Open Graph tags must be placed inside the <body> section of HTML.
Answer: B

18. Which of the following statements about image formats is incorrect?
A) PNG and JPG are the best formats for optimized images today.
B) WEBP and AVIF are better for modern image optimization.
C) SVG images can scale without losing quality.
D) Compression helps reduce image file sizes.
Answer: A

19. What does the loop attribute do in the <audio> element?
A) Plays the audio once.
B) Stops the audio after one play.
C) Repeats the audio continuously.
D) Slows down the audio playback.
Answer: C

20. Which of the following best describes an HTML boilerplate?
A) A predefined template containing essential HTML structure for web development.
B) A special tool used by web developers to add watermarks to documents.
C) A browser plugin that speeds up webpage loading.
D) A coding framework exclusively for animations in HTML.
Answer: B
