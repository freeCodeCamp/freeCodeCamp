...
const singleStop = legendGradient.backgroundImage.match(/var\(\s*--color[0-5]\s*\)\s+\d+(\.\d+)?%/g) || [];
const doubleStop = legendGradient.backgroundImage.match(/var\(\s*--color[0-5]\s*\)\s+\d+%(\s+\d+%)?/g) || [];

if (singleStop.length === 12) {
  const colorList = singleStop.map(stop => stop.match(/--color[0-5]/)[0]);
  ...
  const percentages = singleStop.map(stop => parseFloat(stop.match(/\d+(\.\d+)?%/)[0]));

  for (let i = 1; i < percentages.length - 2; i += 2) {
    assert.strictEqual(percentages[i], percentages[i + 1]);
  }
} else if (doubleStop.length === 6) {
  doubleStop.forEach((arg, i) => {
    if (i !== 0 && i !== 5) {
      assert.lengthOf(arg.match(/%/g), 2);
    }
  });

  const percentagePairs = doubleStop.map(stop => {
