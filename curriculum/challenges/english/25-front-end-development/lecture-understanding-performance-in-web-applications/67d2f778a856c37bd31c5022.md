...
What Are Some Key Metrics for Measuring Performance? To measure and improve web performance, we use key performance metrics like First Contentful Paint or FCP, Speed Index, Total Blocking Time or TBT, Bounce Rate, and Unique Users. Let's look at all of these metrics individually.

First Contentful Paint or FCP measures how quickly the first piece of content—text or image—appears on the screen. A good FCP is regarded as a time below 1.8 seconds, and a poor FCP: Above 3 seconds. You can check your FCP using Chrome DevTools like this.

(Show a quick demo of measuring FCP in DevTools.)
...
- Click the Reload (⟳) button or press Ctrl + Shift + E (Cmd + Shift + E on Mac).

Step 4: Analyze the Results

- Once the recording finishes, scroll down to the "Timings" section.

- Look for the "First Contentful Paint" (FCP) marker.

- Hover over the marker to see the exact FCP time (in milliseconds or seconds).

First Contentful Paint or FCP and Interaction to Next Paint or INP that we looked at previously, are both performance metrics, but they measure different aspects of user experience. it's important not to get them mixed up.
...
Speed Index measures how quickly visible parts of the page are painted. Lower scores mean better performance! You can check your Speed Index in Lighthouse Reports.

(Show an example of a Speed Index score from a performance audit.)
