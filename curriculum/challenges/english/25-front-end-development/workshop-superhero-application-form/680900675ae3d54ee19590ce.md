...
Use an arrow function to map through the `powersOptions` array using parameter of `power`. Inside the map, create a `label` element with a `key` of `power`. Inside that `label`, create an `input` of type `checkbox` and a `value` of `power`.

Finally, display the values from the array next to each checkbox. Create a `span` element just before the closing `label` with a text of `{power}`.
