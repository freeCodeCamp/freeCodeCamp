...
// Button should be disabled intially;
...
powerCheckbox.dispatchEvent(new Event("change", { bubbles: true }));


// Use setTimeout to ensure the DOM has updated after all inputs are filled
setTimeout(() => {
  // Button should now be enabled because all fields are filled
  const updatedBtn = document.querySelector("button");
  assert.isFalse(updatedBtn.disabled);
}, 0);
```
