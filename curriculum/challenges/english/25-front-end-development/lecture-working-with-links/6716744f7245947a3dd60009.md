...
The third value is `_parent`, which opens the link in the parent of the current context. For example, if your website has an `iframe`, a `_parent` `target` in that `iframe` would open in your website's tab/window, not in the embedded frame.
