...
The following example will target all anchor elements where the the `href` attribute value starts with `"https://"`. To target elements where the attribute values ends with a specific value, you can use the `[attr$=value]` syntax. Here is an example:
...
The following example will target all anchor elements where the `href` attribute value ends with `.jpg`. 
