...
You should not modify the existing code in the `if` block.

```js
assert.match(code, /if\s*\(\s*annualIncome\s*>=\s*minIncomeForDuplex\s*&&\s*creditScore\s*>=\s*minCreditScoreForDuplex\s*\)\s*\{/)
```

You should change the `loanMessage` to `"- Duplex loan\n- Car loan\n- Condo loan\n"`

```js
assert.match(code, /("|'|`)-\s*Duplex\s*loan\\n-\s*Car\s*loan\\n-\s*Condo\s*loan\\n("|'|`);?/)
```
