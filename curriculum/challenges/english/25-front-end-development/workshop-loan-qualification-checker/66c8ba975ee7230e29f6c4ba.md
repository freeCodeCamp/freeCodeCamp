...
You should create an `if` statement with the condition `qualificationMessage === 'You qualify for the following loan(s):\n \n'`.

```js
assert.match(code, /if\s*\(qualificationMessage\s*===\s*("|'|`)You\s+qualify\s+for\s+the\s+following\s+loan\(s\):\\n\s*\\n("|'|`)\)\s*\{\s*/)
```

You should reassign the `qualificationMessage` variable to `"You do not qualify for any loans"` inside your `if` statement.

```js
assert.match(code, /if\s*\(qualificationMessage\s*===\s*("|'|`)You\s+qualify\s+for\s+the\s+following\s+loan\(s\):\\n\s*\\n("|'|`)\)\s*\{\s*qualificationMessage\s*=\s*("|'|`)You\s+do\s+not\s+qualify\s+for\s+any\s+loans("|'|`);?\s*\}/)
```
