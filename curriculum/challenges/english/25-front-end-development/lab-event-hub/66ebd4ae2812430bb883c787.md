...
```js
const pElement1 = document.querySelectorAll('#upcoming-events article:nth-of-type(1) p').length > 0;
const pElement2 = document.querySelectorAll('#upcoming-events article:nth-of-type(2) p').length > 0;
const pElements = pElement1 && pElement2;
assert.isTrue(pElements);
```
