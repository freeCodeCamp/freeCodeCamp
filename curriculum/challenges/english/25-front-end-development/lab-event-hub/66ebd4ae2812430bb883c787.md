...
1. Inside the `#upcoming-events` section, you should have:
   ...
   - Two `article` elements. Each article should represent an event, and it should have :
      ...
      - A `p` element for the event description.
      - A `p` element with the text starting from `"Date:"` specifying when the event will take place.
...
```js
const liElements = document.querySelectorAll('header>h1+nav>ul>li');

assert.isNotNull('header>h1+nav>ul');
...
```js
const firstLink = document.querySelectorAll('header>h1+nav>ul>li>a')[0];
...
```js
const anchorElement = document.querySelectorAll("header>h1+nav>ul>li>a")[0];
...
```js
const anchorElement = document.querySelectorAll("header>h1+nav>ul>li>a")[1];
const hrefAttribute = anchorElement.getAttribute("href");
assert.strictEqual(hrefAttribute, "#past-events");
```

You should have a `main` element after the `header` element closing.
...
```js
const pElements = document.querySelectorAll('#upcoming-events article p');
// atleast one p for each
assert.isAtLeast(pElements.length, 2);
```

Both of the `article` elements inside the `#upcoming-events` section should have a date after the event description.
 
```js
const dateElements = document.querySelectorAll('#upcoming-events article p')[1].innerHTML;
assert.match(dateElements, /^Date:/);
```
...
```js
const pElements = document.querySelectorAll('#past-events article p');
// atleast one p for each
assert.isAtLeast(pElements.length, 2);
```
