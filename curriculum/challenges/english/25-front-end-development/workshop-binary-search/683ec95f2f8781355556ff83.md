...
You can see that the first call returns `[3]`, which is the direct and sole middle value in the `search_list`. This is becuase the `search_list` passed has a total of five elements, an odd number in which the middle index is `2`, and the value at that index is `3`.
