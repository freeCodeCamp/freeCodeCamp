...
Now, create an `elif` block with a condition to check if `value` is greater than `value_at_middle`. If that is true, then update the value of the `low` variable to `mid + 1`.
