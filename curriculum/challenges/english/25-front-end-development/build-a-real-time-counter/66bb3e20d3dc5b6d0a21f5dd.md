...
```js
assert.match(code, /let\s+textInput\s*=\s*document\.getElementById\(\"textInput\"\);/);
```
...
```js
assert.match(code, /let\s+charCount\s*=\s*textInput\.value\.length;/);
```
...
```js
assert.match(code, /if\s*\(charCount\s*>\s*50\)/);
```
...
```js
assert.match(code, /charCountDisplay\.innerHTML\s*=\s*`Character Count:\s*\$\{charCount\}\/50`;/);
```

If the character count is equal to `50`, the text should be displayed in `red`.

```js
assert.match(code, /if\s*\(charCount\s*===\s*50\)\s*\{[\s\S]*charCountDisplay\.style\.color\s*=\s*\"red\";/);
```
