...
When `#order-no` contains an invalid email address `validateForm()["order-no"]` should be `false`.
...
When `#product-code` contains an invalid email address `validateForm()["product-code"]` should be `false`.
