...
2. The `permuteString` function should take three parameters- a string, a prefix value and an empty array for storing and returning the results.
...
```js
// help
assert.lengthOf(permuteString, 3);
```
...
When the prefix is `*`, `permuteString('hat', '*')` should return `[ '*hat', '*hta', '*aht', '*ath', '*tha', '*tah' ]`


```js
assert.sameMembers(permuteString('hat', '*'), [ '*hat', '*hta', '*aht', '*ath', '*tha', '*tah' ]);
```

`permuteString('101')` should return `[ '101', '110', '011', '110', '101' ]`.

```js
assert.sameMembers(permuteString('101'), [ '101', '110', '011', '110', '101' ]);
```
...
const permuteString = (str, prefix = '', result = []) => {
  ...
  // Sort the string to ensure we can skip duplicates
