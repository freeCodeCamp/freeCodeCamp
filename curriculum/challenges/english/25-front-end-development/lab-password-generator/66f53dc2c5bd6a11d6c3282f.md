...
4. You should have a `console.log` that logs a single string made by concatenating the message `"Generated password:"` and the `password` variable. This can be done using either string concatenation (`+`) or a template literal (`` `...${password}` ``).
...
You should log a single string to the console that combines the message "Generated password:" and the password variable. This can be done with a `+` operator or using a template literal.
