...
The `box-sizing` property can be set to either `content-box` or `border-box` to control how the width and height oof elements are calculated. Here you can see the box-sizing property and the two possible values. This property is usually set on the root element HTML to apply to all the elements within the document. Here you can see how `border-box` is applied to all the elements within the HTML element.
...
For example, here we have a CSS type selector for the div elements. In this case, if `content-box` is used the content area will have a width of 300 pixels and a height of 200 pixels. However, the total width, what you will see on the screen, will be the result of adding the width of the content area which is 300 pixels, plus the padding on both sides 40 pixels, and the borders on both sides 8 pixels. 
...
The padding and border are inside the box, so when you set the width and height properties of an element, you're really setting the width and height of the inner part of the box. Here's a div example with the same properties and values that we had before. 
...
Likewise, the value of the height property is the result of adding the height of the content area, the top and bottom padding, and the top and bottom border. Height equals the height of the content, plus top padding, plus bottom padding, plus top border, plus bottom border. The margin is not included in the width or hheight.
...
We assign them IDs to select them individually in CSS. These are the CSS rules for the red div and the blue div (showing the CSS rules for the red and blue divs). You can see that they both have the same width, height, border and margin. The only differences are the colors and the value of the box-sizing property. This small difference has a very important impact on the final dimensions. 
