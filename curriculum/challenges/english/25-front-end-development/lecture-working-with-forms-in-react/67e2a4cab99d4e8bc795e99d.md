...
To fix this issue, you need to wrap the action in `startTransition`, like this:
...
If you're wondering what `startTransition` is, it's a function that tells React that a state update is of low-priority and can be interrupted. This keeps the UI responsive while handling async updates like server actions.
