...
You can also choose to use destructuring like this:
...
console.log(developers.map({name} => name));
// ["Alice", "Bob"]
```

Inside the `getBookInformation` function, use the `map()` method on the `arr` parameter to return a new array of just book titles. Refer to the example if you need help.
