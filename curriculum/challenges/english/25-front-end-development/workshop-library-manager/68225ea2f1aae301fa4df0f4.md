...
Chain the `join()` method with a `\n` for the separator to the `arr.map()`. Now you should still a string in the console instead of the array of results. 
...
function getBookInformation(arr) {
  return arr.map(book => `${book.title} by ${book.author}`)
}
--fcc-editable-region--
