...
Start by displaying all the books. Create a `displayBooks` function with parameter accepting array with book objects. The function should return a string that contains the `title`, `author`, and `pages` of all the books in array passed to function.
