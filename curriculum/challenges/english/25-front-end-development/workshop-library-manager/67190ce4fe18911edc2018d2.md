...
Create four new variables: `bookSummaries`, `libraryBooks`, `booksByArvidKahl`, and `totalPagesOfBooksInLibrary`. Set them all to the calling of `displayBooks`, `getBookSummaries`, `getBooksByAuthor`, and `getTotalPages` repectively with the appropriate parameters.
