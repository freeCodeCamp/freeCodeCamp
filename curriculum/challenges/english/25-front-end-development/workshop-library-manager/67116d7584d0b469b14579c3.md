...
Create a `getBooksByAuthor` function with a `catalog` and `author` parameters. The function must return an array that contains the books by a particularauthor. The books inside the array should be objects too.
