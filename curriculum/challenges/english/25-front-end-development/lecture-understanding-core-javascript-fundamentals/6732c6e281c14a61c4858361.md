...
To understand closures lets start with an example:
...
Closures can also capture multiple varibles from their outer scope. For example:
...
let increament = createIncrementer();
increament(); // Output: 1
increament(); // Output: 2
```

Each time we call `increament` its working with the same `count` variable not a copy of it's initial value. Closures are a powerful tool in JavaScript. as you continue to work with JavaScript you'll find that understanding and using closures effectively can enhance your ability to write clean, efficient and powerful code.
