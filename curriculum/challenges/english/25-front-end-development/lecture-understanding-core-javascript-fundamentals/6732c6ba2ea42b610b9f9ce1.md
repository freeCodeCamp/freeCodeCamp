...
In this case even though we only have two defined elements, one and four, the length is four because the highest index three plus one gives us a length of four.
...
In this example, we create a new array using the array five. This creates a sparse array with a length of five where all the slots are empty. 
...
This creates an array of length three and fills all elements with the value zero.
