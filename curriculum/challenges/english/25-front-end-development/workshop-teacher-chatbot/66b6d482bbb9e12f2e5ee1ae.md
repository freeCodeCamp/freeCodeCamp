...
Replace `[subject]` with the value of the `subject` variable like you did earlier.
...
You should use template literal syntax to assign the string `Here is an example of using the length property on the word JavaScript.` to the `strLengthIntro` variable.
...
```js
assert.lengthOf(code.match(/console.log\(.*\)/), 4);
```
