...
The next part of this workshop is to learn how to access certain characters from a string.
...
```js
assert.lengthOf(code.match(/console.log\(.*\)/), 8);
```

Your `console` statement should output the message `Here is an example of accessing the first letter in the word [subject].`. Remember to replace `[subject]` with the `subject` variable and using `${}` syntax.
