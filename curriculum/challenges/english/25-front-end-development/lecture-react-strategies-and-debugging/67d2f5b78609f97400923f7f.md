...
`CounterProvider` uses the `useState` hook to initialize and manage the `count` state and its setter. Then both are then passed into child components through the `Provider`.
