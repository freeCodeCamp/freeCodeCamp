...
8. When the `#theme-dropdown` element is displayed, the `aria-expanded` attribute should be set to `"true"` for the `button` element.
...
assert.equal(themeDropdownEl?.getAttribute("aria-expanded", "false"));
themeDropdownBtn?.dispatchEvent(new MouseEvent("click"));
assert.equal(themeDropdownEl?.getAttribute("aria-expanded", "true"));
```
...
const themeDropdownBtn = document.getElementById("theme-switcher-button");
const listItems = document.querySelectorAll("ul li");
const originalBodyBgColor = getComputedStyle(document.body).getPropertyValue("background-color");

themeDropdownBtn?.dispatchEvent(new Event("click"));
listItems[1]?.dispatchEvent(new Event("click"));

const newBodyBgColor = getComputedStyle(document.body).getPropertyValue("background-color");

console.log("Original:", originalBodyBgColor);
console.log("New:", newBodyBgColor);

assert.notEqual(newBodyBgColor, originalBodyBgColor);
```
