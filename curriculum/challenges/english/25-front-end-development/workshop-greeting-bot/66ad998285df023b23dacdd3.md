...
```js
assert.match(__helpers.removeJSComments(code), /let\s*);
```

Use the `let` keyword to declare the `botLocation` variable.

```js
assert.match(__helpers.removeJSComments(code), /let\s*=\s*botLocation);
```
