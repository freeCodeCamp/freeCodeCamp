...
assert.equal(currentDate.toString(), expectedDate.toString());
```
...
The function`formatDateMMDDYYYY` should take a date object as a parameter.

```js
assert.match(formatDateMMDDYYYY.toString(), /\s*formatDateMMDDYYYY\(\s*\w+\s*\)/);
```
...
The function`formatDateLong` should take a date object as a parameter.

```js
assert.match(formatDateLong.toString(), /\s*formatDateLong\(\s*\w+\s*\)/);
```
...
let dateString = "Fri Sep 27 2024 16:21:18 GMT+0500 (Pakistan Standard Time)";
let dateObject = new Date(dateString);
assert.strictEqual(formatDateLong(dateObject).toString(), "Formatted Date (Month Day, Year): September 27, 2024");

// prevent hardcoding

dateString = "Fri Sep 28 2024 18:31:18 GMT+0500 (Pakistan Standard Time)";
dateObject = new Date(dateString);
assert.strictEqual(formatDateLong(dateObject).toString(), "Formatted Date (Month Day, Year): September 28, 2024");
```
...
function formatDateLong(date) {
  const longDateFormat =  date.toLocaleDateString(undefined, {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  });
