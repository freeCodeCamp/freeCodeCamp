...
The second common JavaScript error is a `ReferenceError`. There are several types of `ReferenceError`s, triggered in different ways. The first type of reference error would be not defined variables.
...
The following example will result in a `Cannot access 'b' before initialization` error. 
