...
Your `getAverage` function should take an array of test scores as a parameter and return the average score.

```js
assert.strictEqual(getAverage([2, 4, 5]), 4);
assert.strictEqual(getAverage([2, 3, 4, 5]), 3.75);
assert.strictEqual(getAverage([1, 2, 3, 4, 5]), 3.2);
```
