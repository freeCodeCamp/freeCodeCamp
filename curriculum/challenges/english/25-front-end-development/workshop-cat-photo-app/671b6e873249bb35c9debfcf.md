...
You should have the words `See more ` before the anchor element. Make sure to have a space after the word `more`. You should not include `p` tags in your answer.

```js
assert.match(code, /See more <a href="https:\/\/freecatphotoapp\.com">cat photos<\/a>/)
```

You should have the words ` in our gallery` after the anchor element. Make sure to have a space before the word `in`. You should not include `p` tags in your answer. 
