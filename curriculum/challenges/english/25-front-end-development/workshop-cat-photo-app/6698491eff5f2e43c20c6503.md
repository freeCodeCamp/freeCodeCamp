...
```js
assert.lengthOf(document.querySelectorAll('figure').length, 2);
```

Your `figure` element should have a closing tag. Closing tags have a `/` just after the `<` character.

```js
assert.lengthOf(code.match(/<\/figure\>/g).length, 2);
```

The `figcaption` element should be nested in the `figure` element.

```js
assert.lengthOf(document.querySelectorAll('figure > figcaption').length, 2);
```
