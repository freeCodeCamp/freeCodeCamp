...
```js
assert.equal($('section')[1].lastElementChild.nodeName, 'IMG');
```
...
```js
assert.exists($('section')[1].lastElementChild.hasAttribute('alt'));
```
...
```js
assert.exists($('section')[1].lastElementChild.hasAttribute('src'));
```
