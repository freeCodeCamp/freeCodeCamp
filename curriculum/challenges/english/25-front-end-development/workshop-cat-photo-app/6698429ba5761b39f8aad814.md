...
```js
assert.lengthOf(document.querySelectorAll('section').length, 2);
```
...
```js
assert.lengthOf(code.match(/<\/section>/g).length, 2);
```
...
assert.isEmpty(foundElems.length);
```
...
assert.lengthOf(foundElems.length, 2);
```
