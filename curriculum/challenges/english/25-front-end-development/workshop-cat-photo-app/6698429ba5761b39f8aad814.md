...
```js
assert.isTrue(document.querySelectorAll('section').length >= 2);
```
...
```js
assert.isTrue(code.match(/<\/section>/g).length >= 2);
```

You should only add one closing `section` tag. Please remove any extras.

```js
assert.equal(code.match(/<\/section>/g).length, 2);
```
...
assert.equal(foundElems.length, 0);
```
...
assert.equal(foundElems.length, 2);
```
