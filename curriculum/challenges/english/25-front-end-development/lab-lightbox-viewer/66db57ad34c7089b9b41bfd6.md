...
1. have a different background colour. Initially, its `display` property should be set to `none` to hide it.
...
You should have an element with the class of `lightbox` inside your `body` element.
...
When you click one of your `.gallery-item` elements, the `.lightbox` element’s `display` property should be set to `flex` to make it, and the two elements within it, visible.

```js
const lightbox = document.getElementById("lightbox");
// assert if lightbox is hidden
assert.strictEqual(lightbox.display, 'none');
lightbox.dispatchEvent(new Event('click'));
lightbox.dispatchEvent(new Event('change'));
assert.strictEqual(lightbox.display, 'flex');
```
...
// assert if the image source contains the word 'thumbnail'
galleryItems.forEach((item) => {
    assert.match(item.src, /-thumbnail/);
});

// fire click event on each gallery item to check if the src does not contain the word 'thumbnail'
galleryItems.forEach((item) => {
    item.dispatchEvent(new Event('click'));
    item.dispatchEvent(new Event('change'));
    assert.notMatch(item.src, /-thumbnail/);  
});
```
