...
Create a `Category` class to represent different budget categories like *food*, *clothing*, and *entertainment*. Each category should maintain a ledger of transactions and allow basic operations like deposits, withdrawals, and transfers.

When a `Category` object is instantiated, it should be initialized with a name and an empty `ledger` list.

Implement the following methods for the class:

- `deposit(amount, description='')`: Add an entry to the ledger with the specified amount and description (defaults to an empty string).
- `withdraw(amount, description='')`: Subtract the specified amount from the ledger if there are enough funds. The amount should be recorded as a negative number. Return `True` if successful, `False` otherwise.
- `get_balance()`: Return the current balance by summing all the amounts in the ledger.
- `transfer(amount, category)`: Withdraw the given amount from the current category and deposit it into another category. Use the descriptions `"Transfer to [Destination Category]"` and `"Transfer from [Source Category]"`. Return `True` if the transfer occurred, `False` if there were insufficient funds.
- `check_funds(amount)`: Return `True` if the amount is less than or equal to the current balance, else return `False`. This method should be used in `withdraw()` and `transfer()`.

When a category object is printed using `print(category)`, display the following:

- A title line of 30 characters with the category name centered in a line of asterisks (`*`).
- A list of transactions in the ledger, showing the first 23 characters of the description and the amount, right-aligned, showing two decimal places and a maximum of 7 characters.
- A final line displaying the total balance as `Total: x.xx`.
