...
In this lab, you will build a simple budget app by implementing a Python `Category` class and a `create_spend_chart` function to visualize spending across categories. You will produce exact string outputs to satisfy the tests.

**Objective:** Fulfill the user stories below and get all the tests to pass to complete the lab.

**User Stories:**
1. Implement a `Category` class that can instantiate objects for budget categories like *food*, *clothing*, and *entertainment*. When creating an object, pass in the name of the category. The class must have an instance variable called `ledger` that is a list.
2. Implement a `deposit(amount, description='')` method that accepts an amount and description (defaulting to an empty string) and appends an object to the `ledger` list in the form of `{'amount': amount, 'description': description}`.
3. Implement a `withdraw(amount, description='')` method that is similar to `deposit`, but stores the amount in the `ledger` as a negative number. If there are not enough funds, nothing should be added to the `ledger`. This method must return `True` if the withdrawal took place, and `False` otherwise.
4. Implement a `get_balance()` method that returns the current balance of the budget category based on the deposits and withdrawals that have occurred.
5. Implement a `transfer(amount, other_category)` method that accepts an amount and another budget category as arguments. It must add a withdrawal with the amount and the description `Transfer to [Destination Budget Category]`, then add a deposit to the other budget category with the amount and the description `Transfer from [Source Budget Category]`. If there are not enough funds, nothing should be added to either `ledger`. This method must return `True` if the transfer took place, and `False` otherwise.
6. Implement a `check_funds(amount)` method that accepts an amount as an argument and returns `False` if the amount is greater than the balance of the budget category and `True` otherwise. Use this method in both the `withdraw` and `transfer` methods.
7. When a budget object is printed, it should display:
   - A title line of 30 characters where the name of the category is centered in a line of `*` characters.
   - A list of the items in the `ledger`. Each line should show the description and amount. Display the first 23 characters of the description, then the amount. The amount should be right aligned, contain two decimal places, and display a maximum of 7 characters.
   - A line displaying the category total.
8. Create a function (outside of the class) called `create_spend_chart(categories)` that takes a list of categories as an argument and returns a string that is a bar chart showing the percentage spent in each category passed into the function.
   - Calculate the percentage spent only with withdrawals (not deposits), and as the percentage of the amount spent for each category to the total spent for all categories.
   - Down the left side of the chart, show labels 0 - 100.
   - Make the bars in the bar chart out of the `o` character.
   - Round the height of each bar down to the nearest 10.
   - Draw the horizontal line below the bars so that it goes two spaces past the final bar.
   - Write each category name vertically below its bar.
   - Add a title at the top that says `Percentage spent by category`.
   - This function will be tested with up to four categories.
   - Ensure the spacing of the output matches the example exactly.
...
- Example spend chart output (spacing must match exactly):
