...
In this lab you will create a budget tracking application that allows users to manage different budget categories, track deposits and withdrawals, transfer funds between categories, and visualize spending patterns through charts.

**Objective:** Fulfill the user stories below and get all the tests to pass to complete the lab.

**User Stories:**

1. You should have a `Category` class that can instantiate objects based on different budget categories like *food*, *clothing*, and *entertainment*.
2. The `Category` class should have an instance variable called `ledger` that is a list.
3. You should have a `deposit` method that accepts an amount and description, appending an object to the ledger list in the form of `{'amount': amount, 'description': description}`.
4. The `deposit` method should default to an empty string description if no description is given.
5. You should have a `withdraw` method that stores the amount as a negative number in the ledger and returns `True` if the withdrawal took place, `False` otherwise.
6. The `withdraw` method should not add anything to the ledger if there are not enough funds.
7. You should have a `get_balance` method that returns the current balance of the budget category.
8. You should have a `transfer` method that accepts an amount and another budget category, adding a withdrawal with description 'Transfer to [Destination Budget Category]' and a deposit with description 'Transfer from [Source Budget Category]'.
9. The `transfer` method should return `True` if the transfer took place, `False` otherwise.
10. The `transfer` method should not add anything to either ledger if there are not enough funds.
11. You should have a `check_funds` method that returns `False` if the amount is greater than the balance, `True` otherwise.
12. The `check_funds` method should be used by both the `withdraw` method and `transfer` method.
13. When the budget object is printed, it should display a title line of 30 characters with the category name centered in asterisks.
14. The printed output should show each ledger item with the first 23 characters of the description and the amount right-aligned with two decimal places.
15. The printed output should display the category total.
16. You should have a `create_spend_chart` function that takes a list of categories and returns a string representing a bar chart.
17. The chart should show percentage spent in each category calculated only with withdrawals, not deposits.
18. The chart should have labels 0-100 down the left side with bars made of 'o' characters.
19. The height of each bar should be rounded down to the nearest 10.
20. The horizontal line below the bars should go two spaces past the final bar.
21. Each category name should be written vertically below the bar.
22. The chart should have a title "Percentage spent by category".

**Hint:** Pay close attention to the exact spacing and formatting requirements for both the printed category output and the spend chart. The `check_funds` method is crucial for preventing invalid withdrawals and transfers.
