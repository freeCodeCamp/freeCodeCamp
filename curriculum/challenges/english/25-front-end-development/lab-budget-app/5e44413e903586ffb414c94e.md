...
1. Each instance of the class should have an instance variable called `ledger` that is a list.
1.The class should contain a method called `deposit` that accepts an amount and description. If no description is given, it should default to an empty string. The method should append an object to the ledger list in the form of `{'amount': amount, 'description': description}`.
1. The class should contain a method called `withdraw`that is similar to the `deposit` method, but the amount passed in should be stored in the ledger as a negative number. If there are not enough funds, nothing should be added to the ledger. This method should return `True` if the withdrawal took place, and `False` otherwise.
1. The class should contain a method called `get_balance` that returns the current balance of the budget category based on the deposits and withdrawals that have occurred.
1. The class should contain a method called `transfer` that accepts an amount and another budget category as arguments. The method should add a withdrawal with the amount and the description 'Transfer to [Destination Budget Category]'. The method should then add a deposit to the other budget category with the amount and the description 'Transfer from [Source Budget Category]'. If there are not enough funds, nothing should be added to either ledgers. This method should return `True` if the transfer took place, and `False` otherwise.
1. The class should contain a method called `check_funds` that accepts an amount as an argument. It returns `False` if the amount is greater than the balance of the budget category and returns `True` otherwise. This method should be used by both the `withdraw` method and `transfer` method.
1. When the budget object is printed, a title line of 30 characters where the name of the category is centered in a line of `*` characters should be displayed.
1. All of the items in the ledger should be displayed in a list when the budget object is printed. Each line should show the description and amount. The first 23 characters of the description should be displayed, then the amount. The amount should be right aligned, contain two decimal places, and display a maximum of 7 characters.
1. When the budget object is printed it should display a line displaying the category total.
1. You should have a function (outside of the class) called `create_spend_chart` that takes a list of categories as an argument. 
1. The `create_spend_chart` should return a string that is a bar chart.
1. The bar chart should show the percentage spent in each category passed in to the `create_spend_chart` function. The percentage spent should be calculated only with withdrawals and not with deposits, and it should be the percentage of the amount spent for each category to the total spent for all categories. 
1. Down the left side of the chart should be labels 0 - 100. 
1. The 'bars' in the bar chart should be made out of the 'o' character. The height of each bar should be rounded down to the nearest 10. 
1. The horizontal line below the bars should go two spaces past the final bar. 
1. Each category name should be written vertically below the bar. There should be a title at the top that says 'Percentage spent by category'.
