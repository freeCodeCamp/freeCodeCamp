...
2. On `Category`, You should have a `deposit` method that accepts an amount and an optional description (default `""`) and appends `{'amount': amount, 'description': description}` to `ledger`.  
3. You should have a `withdraw` method that accepts an amount and an optional description (default `""`), stores the amount as negative in `ledger`, and returns `True` if the withdrawal succeeded (otherwise `False` with no ledger entry).  
4. You should have a `get_balance` method that returns the current category balance based on `ledger`.  
5. You should have a `transfer` method that accepts an amount and another `Category` instance, withdraws the amount with description `Transfer to [Destination]`, deposits it into the other category with description `Transfer from [Source]`, and returns `True` if successful (otherwise `False`).  
6. You should have a `check_funds` method that accepts an amount and returns `False` if it exceeds the balance or `True` otherwise; this must be used by both `withdraw` and `transfer`.  
7. When a `Category` object is printed, it should:  
   - Display a title line of 30 characters with the category name centered in `*`.  
   ...
   - Show a final line `Total: <balance>`.
...
8. You should have a function **outside the `Category` class** named `create_spend_chart(categories)` that returns a bar-chart string titled `Percentage spent by category`. 
   - Calculate percentages from withdrawals only, and it should be the percentage of the amount spent for each category to the total spent for all categories (rounded to the nearest 10).
   - Label the y-axis from `100` down to `0` in steps of 10.
   - Use `o` for the bars, include a horizontal line two spaces past the last bar, and write category names vertically below.
