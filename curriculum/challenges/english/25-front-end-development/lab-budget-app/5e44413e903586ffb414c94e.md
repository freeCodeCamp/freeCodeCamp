...
2. `Category` should have an instance variable `ledger` that is a list, and contains the list of transactions.
3. On `Category`, You should have a `deposit` method that accepts an amount and an optional description (default `""`) and appends `{'amount': amount, 'description': description}` to `ledger`.  
4. You should have a `withdraw` method that accepts an amount and an optional description (default `""`), stores the amount as negative in `ledger`, and returns `True` if the withdrawal succeeded (otherwise `False` with no ledger entry).  
5. You should have a `get_balance` method that returns the current category balance based on `ledger`.  
6. You should have a `transfer` method that accepts an amount and another `Category` instance, withdraws the amount with description `Transfer to [Destination]`, deposits it into the other category with description `Transfer from [Source]`, and returns `True` if successful (otherwise `False`).  
7. You should have a `check_funds` method that accepts an amount and returns `False` if it exceeds the balance or `True` otherwise; this must be used by both `withdraw` and `transfer`.  
...
9. You should have a function **outside the `Category` class** named `create_spend_chart(categories)` that returns a bar-chart string titled `Percentage spent by category`. 
