...
By default, the button will not do anything when activated. However, you can add some JavaScript code to make the button interactive, such as showing an alert in this case. Another possible value for the `type` attribute is to use the `submit` value. Here is an example of using a `button` element with the `submit` type:
