...
The opposite is `justify-content: flex-end`. This will align `flex` items to the end of the cross axis, vertically or horizontally:
...
This also works with elements that are automatically sized, such as those without set `width` or `height`, or with only a minimum `width` or `height`. The `flex` items will stretch to fill the container in the direction of the cross axis.
