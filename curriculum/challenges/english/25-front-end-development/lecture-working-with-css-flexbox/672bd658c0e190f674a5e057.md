...
In the code below, we have three `div` elements. Let's focus on the `width`. The `main` container with the `border` that is `red` has a `width` of `200px`, while its three child `div` elements combined have a `width` of `240px` (`80px` each):
