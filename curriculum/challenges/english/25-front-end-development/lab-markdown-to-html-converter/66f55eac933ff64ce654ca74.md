...
When the value of `#markdown-input` is `# title`, `convertMarkdown()` should return `<h1>title 1</h1>`.
...
assert(testImg.src.endsWith("image-source"));
```
...
assert(testAnchor.href.endsWith("URL"));
