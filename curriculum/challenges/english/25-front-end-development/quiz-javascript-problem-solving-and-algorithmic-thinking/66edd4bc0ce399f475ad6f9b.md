...
Write a function that compresses a string by counting repeated characters. For example, the input "mississippi" would return "m1i1s2i1s2i1p2i1".
...
Use forEach() to count consecutive characters and append counts.
...
Iterate through the string, counting consecutive occurrences of each character while building answer.
...
How would you check if a given string is a palindrome (the same forwards and backwards) for example "madam"?
...
Use a while loop with modulo operators.
...
Use a for loop and check divisibility with if conditions or switch.

### --question--

#### --text--

The Caesar cipher is a simple encryption technique where each letter in a string is shifted by a fixed number of positions in the alphabet. How would you shift all letters in a string by 3 positions?
...
How would you check if two strings are anagrams of each other (i.e., contain the same characters in a different order)?
...
How would you write a function to check if a given number is prime?
...
How would you find the greatest common divisor (GCD) of two numbers using Euclid’s algorithm?
...
Which of the following approaches can be used to calculate the factorial of a given number n (n!)?
...
How would you efficiently search a given integer within a distinct array of sorted integers?
