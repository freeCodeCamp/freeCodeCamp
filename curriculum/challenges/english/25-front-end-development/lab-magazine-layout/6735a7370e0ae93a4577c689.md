...
5. You should set the display property of the `.magazine-cover` to `grid.`
...
assert.match(magazineCoverStyle.gridTemplateAreas, /^"title title title"/);
```
...
```js
const magazineCoverStyle = getComputedStyle(document.querySelector('.magazine-cover'));
assert.match(magazineCoverStyle.gridTemplateAreas, /."feature-article feature-article cover-image"./);
```
...
```js
const magazineCoverStyle = getComputedStyle(document.querySelector('.magazine-cover'));
assert.match(magazineCoverStyle.gridTemplateAreas, /"small-article1 small-article2 cover-image"$/);
```
