...
- **Definition**: A `switch` statement evaluates an expression and matches its value against a series of `case` clauses. When a match is found, the code block associated with that case is executed. The `break` statement terminates the current switch statement and transfers control to the statement following the terminated statement.The switch statement also includes an optional `default` statement,which is executed if the value of the expression does not match any of the cases.   
