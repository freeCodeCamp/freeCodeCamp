...
The <code>map</code> method iterates over each item in an array and returns a new array (without mutating the original) after applying a callback function to every element.
The callback function accepts three arguments (with names of your choice).  While iterating over the array, the arguments can be referenceed to change value of the  elements in the array returned.  The first argument is the current element being processed.  The second argument is the index of the current element being processed.  The third argument is the actual array upon which the map method was called.
