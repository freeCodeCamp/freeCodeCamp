...
```yml
tests:
  - text: <code>resolve</code> should be executed with string as message when the <code>if</code> condition is <code>true</code>.
    testString: assert(removeJSComments(code).match(/if\s*\(\s*responseFromServer\s*\)\s*{\s*resolve\s*\(\s*('|"|`)We got the data\1\s*\)(\s*|\s*;\s*)}/g));
  - text: <code>reject</code> should be executed with string as message when the <code>if</code> condition is <code>false</code>.
