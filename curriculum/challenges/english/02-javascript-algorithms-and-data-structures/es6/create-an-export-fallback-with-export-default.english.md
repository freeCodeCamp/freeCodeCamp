...
```yml
tests:
  - text: Proper use of <code>export</code> fallback.
    testString: getUserInput => assert(getUserInput('index').match(/export\s+default\s+function\s+subtract\(x,y\)\s+{return\s+x\s-\s+y;}/g), 'Proper use of <code>export</code> fallback.');
