...
Like the `rect` element, a `text` element needs to have `x` and `y` attributes, to place it on the SVG area. It also needs to access the data to display those values.
