...
With the scales set up, it's time to map the scatter plot again. The scales are like processing functions that turn the `x` and `y` raw data into values that fit and render correctly on the SVG area. They keep the data within the screen's plotting area.
...
Scales set shape coordinate attributes to place the data points onto the SVG area. You don't need to apply scales when you display the actual data value, for example, in the `text()` method for a tooltip or label.

# --instructions--

Use `xScale` and `yScale` to position both the `circle` and `text` shapes onto the SVG area. For the `circles`, apply the scales to set the `cx` and `cy` attributes. Give them a radius of `5` units, too.
