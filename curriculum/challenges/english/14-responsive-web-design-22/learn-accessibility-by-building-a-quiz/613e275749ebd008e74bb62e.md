...
Currently, the `img` is assuming its default size, which is too large. The `max` function will always return the largest of a set of comma-separated values, and thus it will always be at minimum 100px. And as the viewport grows, the image will grow accordingly to be 18 percent of the viewport width. 

Correctly, scale the image using it's `id` as a selector, and setting the `width` to `max(100px, 18vw)`. 
