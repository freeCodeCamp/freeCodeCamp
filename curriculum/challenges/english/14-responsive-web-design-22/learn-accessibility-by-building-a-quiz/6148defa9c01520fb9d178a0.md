...
While `ul`/`li` elements are great at providing bullets for list items, our radio buttons will instead be used to mark each list item. We
can control what the bullets look by using the `list-style` css property.  For example you can turn your bullets into the circles
with the following. 
