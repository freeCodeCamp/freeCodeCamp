...
Start by setting up your HTML structure. Add a `<!DOCTYPE html>` declaration and an `html` element. Within the `html` element, add a `head` element and a `body` element.
