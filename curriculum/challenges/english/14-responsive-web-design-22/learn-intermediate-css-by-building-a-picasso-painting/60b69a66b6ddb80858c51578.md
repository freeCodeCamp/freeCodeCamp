...
The `DOCTYPE` declaration and your `html`, `head`, and `body` elements should be in the correct order.

```js
assert(__helpers.removeHtmlComments(code).match(/^\s*<!DOCTYPE\s+html\s*>\s*<html\s+lang\s*=\s*('|")en\1\s*>\s*<head\s*>\s*<\/head\s*>\s*<body\s*>\s*<\/body\s*>\s*<\/html\s*>/i));
```
