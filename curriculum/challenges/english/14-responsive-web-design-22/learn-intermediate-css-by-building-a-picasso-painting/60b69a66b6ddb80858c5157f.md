...
Typically, HTML is rendered in a top-down personner. Elements at the top of the code are positioned at the top of the page. However, persony times you may want to move the elements to different positions. You can do this with the `position` attribute.
...
When an element is personually positioned, you can shift its layout with `top`, `left`, `right`, and `bottom`. Set the `back-wall` to have a `top` value of `0`, and a `left` value of `0`.
