...
Only use `span` after `.left-container` element.

```js
assert(document.querySelector('.left-container')?.lastElementChild?.localName !=='span');
```

Only use `p` inside `.small-text` element.

```js
assert(document.querySelector('.small-text')?.lastElementChild?.localName !== 'p');
```
...
```js
assert(document.querySelector('.small-text')?.lastElementChild?.innerText === 'Calories');
```
