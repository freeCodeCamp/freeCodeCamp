...
Nest these elements to `head` element, `meta` element with and attribute `charset` of `UTF-8`, the a `title` element, and give your project a title of `City Skyline`. Also, a self-closing `link` element, and give it a `rel` attribute value of `stylesheet` and an `href` attribute value of `styles.css`.
