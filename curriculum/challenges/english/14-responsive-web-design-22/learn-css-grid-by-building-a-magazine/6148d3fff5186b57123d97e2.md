...
Create a `.quote::before` selector and set the `content` property to `"` inside single quotes `'` with a space following the double qoute.

Also, create a `.quote::after` selector and set the `content` property to `"` inside single quotes `'` with a space preceding the double qoute.
