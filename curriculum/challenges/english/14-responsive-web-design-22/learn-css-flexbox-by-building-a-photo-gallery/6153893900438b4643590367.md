...
Remove the margin from your `body` element, set the `font-family` to `sans-serif`, and give it a background color of `#f8f8ff` as the value.
...
Your `body` selector should have a `background-color` property set to `#f8f8ff` as the value.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.backgroundColor === 'rgb(248, 248, 255)');
```
