...
Within the `body` element, create a `header` element. Give the `header` element a class with `header` as the value.

Inside the `header` element, create an `h1` element and give it `css flexbox photo gallery` as the text.
