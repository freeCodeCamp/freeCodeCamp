...
Your images are currently too large.

Create a `.gallery img` selector to target your images. Give it a `width` property set to `100%` and a `max-width` set to `350px` to let the images shrink down as needed but not grow larger than the `max-width` value.
...
Your `.gallery img` selector should have a `width` property set to `100%` and a `max-width` set to `350px` as the value.
