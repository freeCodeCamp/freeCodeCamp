...
Your images are too big. Create a `.gallery img` selector to target them. Give them all a `width` of `100%` and a `max-width` of `350px` so they shrink as needed but don't get too big. From there, change the `box-sizing` property from `content-box` to `border-box`. Unlike `content-box`, the `border-box` value will keep the width and height of the images  the same and changes their padding and borders. Also, set the `height` property to `300px` to keep your images a uniform size.
