...
Below your `header` element, create a new `div` element give it a `class` and set `gallery` as the value.
...
```js
assert(document.querySelectorAll('body > *')?.[1]?.classList?.contains('gallery'));
```
