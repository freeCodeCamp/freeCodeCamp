...
Change the `margin` property to include `1em` on the top and bottom. Be sure that the right and left margins are set to `auto`, then set the width as described above.
