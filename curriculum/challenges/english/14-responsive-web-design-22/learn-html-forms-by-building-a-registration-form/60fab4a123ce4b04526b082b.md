...
```js
assert.equal(document.querySelector('fieldset:nth-child(2) label:nth-child(3) input[type="radio"]')?.name, 'account-type');
```

You should not give the `checkbox` the `name` attribute.

```js
assert.isEmpty(document.querySelector('fieldset:nth-child(2) label:nth-child(4) input[type="checkbox"]')?.name);
```
