...
Don't forget to add an `id` attribute for accessibility.
...
```js
assert(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[1]?.matches('#terms-and-conditions'))
```

The `label` element should have a `for` attribute with a value of `terms-and-conditions`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(2) label')?.[0]?.matches('label[for="terms-and-conditions"]'))
```
