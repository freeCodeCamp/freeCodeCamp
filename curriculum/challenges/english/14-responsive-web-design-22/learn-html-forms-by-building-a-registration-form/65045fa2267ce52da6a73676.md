...
Currently when someone submit the form, they can submit it without checking the radio inputs. Although you had used `required` attribute to indicate the the input is required previously, this can't work in this case, because selecting one of the two inputs is required not both of them.
...
```js
console.log(document.querySelector('fieldset:nth-of-type(2) > legend')?.textContent);
