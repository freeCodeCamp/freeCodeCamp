...
Add a `title` and `meta` elements to the `head`. Give your project a `title` of `Registration Form`, and `charset` attribute with value of `UTF-8` to your `meta` element. 
