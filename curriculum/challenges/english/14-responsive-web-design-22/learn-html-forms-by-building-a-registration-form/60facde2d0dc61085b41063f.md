...
To allow users to register with a bio, at the end of the `fieldset` add a `label` with the text `Provide a bio:` followed by a `textarea` element, which requires a closing tag.
