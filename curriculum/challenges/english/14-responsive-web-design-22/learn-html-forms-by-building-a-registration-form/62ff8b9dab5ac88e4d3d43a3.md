...
Now, to follow accessiblity best practices, link the `input` elements and the `label` elements together using the `for` attribute.

Use these values in order for the `id`s: `first-name`, `last-name`, `email`, `new-password`
