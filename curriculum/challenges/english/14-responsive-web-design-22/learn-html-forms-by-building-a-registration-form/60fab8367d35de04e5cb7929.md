...
You should not have extra spaces around the text in the `a` element.

```js
var regex = /^\s+.*\s+$/;
assert.notMatch(document.querySelector('fieldset:nth-child(3) + label > input + a')?.textContent, regex);
```
