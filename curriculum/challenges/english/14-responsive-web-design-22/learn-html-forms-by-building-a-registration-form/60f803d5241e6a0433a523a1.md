...
As `label` elements are `inline` by default, all the labels appear on the same line and the text is all smushed together. To make them appear on separate lines, add `display: block` to the `label` element, and add a `margin` of `0.5rem 0`, to separate them from each other.
