...
The border of the last element looks a little out of place. You can select the last element of a specific type using the `last-of-type` CSS pseudo-class, like this: 
...
Your `fieldset:last-of-type` should have `border-bottom` set to `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type')?.borderBottom, 'none');
```
