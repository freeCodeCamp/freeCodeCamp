...
Now, to follow accessiblity best practices, link the form elements and their `label` elements together using the `for` attribute.

Use these values in order for the `id`s: `profile-picture`, `age`, `referrer`, `bio`
...
```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) select').matches('#bio'))
```
