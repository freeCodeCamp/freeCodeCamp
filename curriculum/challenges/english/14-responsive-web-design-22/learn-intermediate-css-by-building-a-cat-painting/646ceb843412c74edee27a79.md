...
Using a class selector, give the `.cat-right-ear` element `height` and `width` properties set to `100px`. Set the `background-color` to `white`. Set left and right border to `35px solid blue`. Set top and bottom border to `35px solid red`.
