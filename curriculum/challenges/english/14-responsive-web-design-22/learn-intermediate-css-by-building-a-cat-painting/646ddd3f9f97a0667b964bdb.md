...
Inside your `.cat-left-eye` element, create a `div` element with the class `cat-left-inner-eye`.

Similarly, inside your `.cat-right-eye` element, create a `div` element with the class `cat-right-inner-eye`.
...
The `div` element inside your `.cat-left-eye` should have the class `cat-left-inner-eye`.
...
The `div` element inside your `.cat-right-eye` should have the class `cat-right-inner-eye`.

```js
assert(document.querySelectorAll('.cat-right-eye div')[0]?.classList.contains('cat-right-inner-eye'));
```

Make sure you have closed each `div` element.

```js
assert.match(code, /<div\s+class\s*=\s*"\s*cat-right-eye\s*"\s*>\s*<div\s+class\s*=\s*"\s*cat-right-inner-eye\s*"\s*>\s*(<\/div>\s*){4}/);
```
