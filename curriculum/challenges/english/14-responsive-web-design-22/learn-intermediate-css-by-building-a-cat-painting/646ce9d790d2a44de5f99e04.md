...
Inside your `.cat-left-ear` element, create a `div` element with the class `cat-left-inner-ear`.

Similarly, inside your `.cat-right-ear` element, create a `div` element with the class `cat-right-inner-ear`.
...
The `div` element inside your `.cat-left-ear` should have the class `cat-left-inner-ear`.
...
The `div` element inside your `.cat-right-ear` should have the class `cat-right-inner-ear`.
...
```js
assert.match(code, /<div\s+class\s*=\s*"\s*cat-right-ear\s*"\s*>\s*<div\s+class\s*=\s*"\s*cat-right-inner-ear\s*"\s*>\s*(<\/div>\s*){4}/);
```
