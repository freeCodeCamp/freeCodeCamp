...
Create a `div` element with the class `cat-eyes`. Inside the `cat-eyes` `div`, create two `div` elements with the class `cat-left-eye` and another with the class `cat-right-eye`.
...
```js
const divs = document.querySelectorAll('.cat-eyes div')
console.log(divs)
