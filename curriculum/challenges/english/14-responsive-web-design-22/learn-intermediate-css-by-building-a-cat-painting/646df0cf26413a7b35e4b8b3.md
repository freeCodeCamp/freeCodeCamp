...
```js
assert.equals(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderTopLeftRadius, '50%');
```

Your `.cat-nose` selector should have a `border-bottom-right-radius` property set to `50%`.

```js
assert.equals(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderBottomRightRadius, '50%');
```

Your `.cat-nose` selector should have a `border-bottom-left-radius` property set to `50%`.

```js
assert.equals(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderBottomLeftRadius, '50%');
```

Your `.cat-nose` selector should have a `transform` property set to `rotate(180deg)`.

```js
assert.equals(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.transform, 'rotate(180deg)');
```
