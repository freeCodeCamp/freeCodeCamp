...
You should set the `transform` property of your `.left-ear` element to `rotate(-45deg)`. Don't forget to add a semi-colon.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-ear')?.transform === 'rotate(-45deg)')
```
