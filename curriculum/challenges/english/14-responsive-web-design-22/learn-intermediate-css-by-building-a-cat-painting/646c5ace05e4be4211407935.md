...
```
Set `.cat-head` background to a `linear-gradient` with `#5e5e5e` at `85%` and `#45454f` at `100%`. Double-check CSS syntax and formatting.

```js
assert.match(code, /background:\s*linear-gradient\(\s*#5e5e5e\s+85%,\s*#45454f\s+100%\s*\);?\s*}/);
```
