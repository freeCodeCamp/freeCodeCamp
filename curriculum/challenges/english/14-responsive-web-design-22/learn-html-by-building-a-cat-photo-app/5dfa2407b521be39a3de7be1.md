...
Add a `target` attribute with the value `_blank` to the second anchor (`a`) element's opening tag, so that the link opens in a new tab.
...
assert(
  anchor.length &&
    anchor[1].innerText.toLowerCase().replace(/\s+/g, ' ') === 'cat photos'
);
```

Your second anchor (`a`) element does not have a `target` attribute. Check that there is a space after the opening tag's name and/or there are spaces before all attribute names.

```js
const anchor = $('p > a');
assert(
  anchor.length &&
    anchor[1].hasAttribute('target')
);
```
...
assert(
  anchor.length &&
    anchor[1].getAttribute('target') === '_blank'
);
```
