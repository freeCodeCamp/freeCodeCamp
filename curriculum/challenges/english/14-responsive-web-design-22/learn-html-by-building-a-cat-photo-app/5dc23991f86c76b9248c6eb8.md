...
Your `main` element should be indented by 4 spaces. This was the default setting, and it might have been accidentally changed. Please correct the indentation to maintain code readability.

```js
assert(code.toLowerCase().match(/^\s{4}<main>[\s\S]*^\s{4}<\/main>/m));
```
