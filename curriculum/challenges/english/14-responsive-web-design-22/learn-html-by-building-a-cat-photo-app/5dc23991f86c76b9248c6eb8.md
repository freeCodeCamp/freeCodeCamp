...
In the previous step you nested the `h2` element, comment, and `p` element within the `main` element. A nested element is a child of its parent element and, for readability, should be indented two more spaces than its parent like this:

Indentation in code works like the list tag in html. For example
