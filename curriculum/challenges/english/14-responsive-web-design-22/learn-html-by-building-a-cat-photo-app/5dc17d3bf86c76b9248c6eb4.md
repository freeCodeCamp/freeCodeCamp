...
The `p` element is used to create a paragraph of text on websites. Create a `p` element below your `h2` element **within the editable region**  and give it the following text:
...
assert(extraSpacesRemoved.match(/Everyone loves cute cats online!\.?$/i));
```
