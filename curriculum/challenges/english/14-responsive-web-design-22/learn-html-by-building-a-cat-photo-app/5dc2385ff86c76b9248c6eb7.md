...
HTML5 has some elements that identify different content areas. These elements make your HTML easier to read and theylp with Search Engine Optimization (SEO) and accessibility.
...
assert(
  mainNode.contains(pNode) &&
    pNode.textContent.toLowerCase().match(/click theyre to view more cat photos/)
);
```
