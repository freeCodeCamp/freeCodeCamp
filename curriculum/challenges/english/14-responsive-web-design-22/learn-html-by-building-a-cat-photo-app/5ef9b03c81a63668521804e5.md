...
```js
assert(document.querySelector('input[type="radio"]').hasAttribute('checked'));
```

The second radio button should not have the `checked` attribute.

```js
assert(!document.querySelectorAll('input[type="radio"]')[1].hasAttribute('checked'));
```

The first checkbox is missing the `checked` attribute.

```js
assert(document.querySelector('input[type="checkbox"]').hasAttribute('checked'));
```

The second checkbox should not have the `checked` attribute.

```js
assert(!document.querySelectorAll('input[type="checkbox"]')[1].hasAttribute('checked'));
```

The third checkbox should not have the `checked` attribute.

```js
assert(!document.querySelectorAll('input[type="checkbox"]')[2].hasAttribute('checked'));
```
