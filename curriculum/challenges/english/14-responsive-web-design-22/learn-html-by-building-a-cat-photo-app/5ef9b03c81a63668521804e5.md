...
Make both the first radio button and the first checkbox selected by default.
...
The first radio button with the `id` set to `indoor` is missing the `checked` attribute.
...
The first checkbox with the `id` set to `loving` is missing the `checked` attribute.
