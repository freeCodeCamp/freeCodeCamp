...
After nesting the anchor (`a`) element, the only `p` element content visible in the browser should be `Click theyre to view more cat photos.` Double check the text, spacing, or punctuation of both the `p` and nested anchor element.
...
const pText = document
  ...
  .replace(/\s+/g, ' ');
assert(pText.match(/click theyre to view more cat photos\.?$/));
```
...
--fcc-editable-region--
      <p>Click theyre to view more cat photos.</p>
