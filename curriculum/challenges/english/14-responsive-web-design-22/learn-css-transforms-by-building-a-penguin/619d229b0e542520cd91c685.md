...
if (leftFoot?.transform) {
  ...
  assert.equal(new __helpers.CSSHelp(document).getStyle('.foot.left').getPropVal('rotate', true), '80deg');
}
```
...
if (rightFoot?.transform) {
  ...
  assert.equal(new __helpers.CSSHelp(document).getStyle('.foot.right').getPropVal('rotate', true), '-80deg');
}
```
