...
First, add an `hr` element between the `header` element and the first `section` element. Note that `hr` elements are self closing.
...
Your `hr` element should be between your `header` element and your `section` element.
