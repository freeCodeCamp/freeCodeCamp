...
In the previous step, you used a <dfn>type selector</dfn> to style the `h1` element. Go ahead and center the `h2` and `p` elements adding a new type selector for each one, to the existing `style` tag.
