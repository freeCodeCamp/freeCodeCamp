...
```yml
tests:
  - text: 'Your code should add a <code>background-color</code> or <code>background</code> property to the <code>h4</code> element set to <code>rgba(45, 45, 45, 0.1)</code>.'
    testString: 'assert(code.match(/(background-color|background):\s*?rgba\(\s*?45\s*?,\s*?45\s*?,\s*?45\s*?,\s*?0?\.1\s*?\);/gi), "Your code should add a <code>background-color</code> property to the <code>h4</code> element set to <code>rgba(45, 45, 45, 0.1)</code>. Ensure to use a <code>;</code> after your property declaration.");'
