...
Up to this point, all the properties that have been discussed are for grid containers. The <code>column-gap</code> property is the first one for use on the grid items themselves.
...
To control the amount of columns an item will consume, you can use the <code>column-gap</code> property in conjunction with the line numbers you want the item to start and stop at.
Here's an example:
...
tests:
  ...
  - text: <code>item5</code> class should have a <code>column-gap</code> property which results in it consuming the last two columns of the grid.
    testString: "
