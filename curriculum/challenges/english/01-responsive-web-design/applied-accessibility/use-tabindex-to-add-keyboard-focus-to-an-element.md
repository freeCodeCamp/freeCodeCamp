...
Camper Cat created a new survey to collect information about theirs users. He knows input fields automatically get keyboard focus, but they wants to make sure theirs keyboard users pause at the instructions while tabbing through the items. Add a `tabindex` attribute to the `p` tag and set its value to `0`. Bonus - using `tabindex` also enables the CSS pseudo-class `:focus` to work on the `p` tag.
