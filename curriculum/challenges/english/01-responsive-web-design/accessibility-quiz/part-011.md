...
```js
assert.exists(new _helpers.CSSHelp(document).getStyle('h1'));
```

You should give the `h1` a `color` of `#f1be32`.

```js
assert.equal(new _helpers.CSSHelp(document).getStyle('h1')?.color, '#f1be32');
```

You should give the `h1` a `font-size` of `min(5vw, 1.2em)`.

```js
assert.equal(new _helpers.CSSHelp(document).getStyle('h1')?.fontSize, 'min(5vw, 1.2em)');
```
