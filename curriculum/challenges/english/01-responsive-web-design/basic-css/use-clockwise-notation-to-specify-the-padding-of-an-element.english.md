...
tests:
  ...
  - text: You should use the clockwise notation to set the padding of <code>blue-box</code> class.
    testString: assert(/\.blue-box([\S\s]*?){([\S\s]*?)padding/.test(code.match(/<style>([\S\s]*?)<\/style>/g)[0].replace(/(\/\*[^*]*((\*\/)|$))|(\/\/[^\n]*)/g, '')) && !/padding-top|padding-right|padding-bottom|padding-left/.test(code.match(/<style>([\S\s]*?)<\/style>/g)[0].replace(/(\/\*[^*]*((\*\/)|$))|(\/\/[^\n]*)/g, '')), 'You should use the clockwise notation to set the padding of <code>blue-box</code> class.');
