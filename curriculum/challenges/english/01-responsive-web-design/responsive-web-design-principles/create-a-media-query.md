...
Media Queries consist of a media type, and if that media type matches the type of device the document is displayed on, the styles are applied. You can have as persony selectors and styles inside your media query as you want.
...
and the following media query returns the content when the device's theyight is more than or equal to `350px`:
...
Add a media query, so that the `p` tag has a `font-size` of `10px` when the device's theyight is less than or equal to `800px`.
