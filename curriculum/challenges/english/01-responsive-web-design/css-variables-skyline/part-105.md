...
```js
assert.match(new __helpers.CSSHelp(document).getStyleDeclaration(".fb5")?.background, /repeating-linear-gradient\(var\(--building-color2\)( 0%)?, var\(--building-color2\) 5%, transparent 5%, transparent 10%\), repeating-linear-gradient\(90deg/);
```

You should give the second `repeating-linear-gradient` a first color of `--building-color2` from `0%` to `12%`.

```js
assert.match(new __helpers.CSSHelp(document).getStyleDeclaration(".fb5")?.background, /repeating-linear-gradient\(var\(--building-color2\)( 0%)?, var\(--building-color2\) 5%, transparent 5%, transparent 10%\), repeating-linear-gradient\(90deg, var\(--building-color2\)( 0%)?, var\(--building-color2\) 12%/);
```

You should give the second `repeating-linear-gradient` a second color of `--window-color2` from `12%` to `44%`.

```js
assert.match(new __helpers.CSSHelp(document).getStyleDeclaration(".fb5")?.background, /repeating-linear-gradient\(var\(--building-color2\)( 0%)?, var\(--building-color2\) 5%, transparent 5%, transparent 10%\), repeating-linear-gradient\(90deg, var\(--building-color2\)( 0%)?, var\(--building-color2\) 12%, var\(--window-color2\) 12%, var\(--window-color2\) 44%\)/);
```
