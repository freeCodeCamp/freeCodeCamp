...
```js
assert.include(new __helpers.CSSHelp(document).getStyleDeclaration(".fb1c")?.getPropVal('background', true), "repeating-linear-gradient(90deg,var(--building-color4),var(--building-color)10%,transparent10%,transparent 15%)");
```

You should add a `repeating-linear-gradient` with a first color of `--building-color4` from `0%` to `10%`.

```js
assert.match(new __helpers.CSSHelp(document).getStyleDeclaration(".fb1c")?.getPropVal('background', true), /repeating-linear-gradient\(var\(--building-color4\)(0%)?,var\(--building-color4\)10%/);
```

You should use a second color of `--window-color4` from `10%` to `90%`.

```js
assert.match(new __helpers.CSSHelp(document).getStyleDeclaration(".fb1c")?.getPropVal('background', true), /repeating-linear-gradient\(90deg,var\(--building-color4\)(0%)?,var\(--building-color\)10%,transparent10%,transparent15%\),repeating-linear-gradient\(var\(--building-color4\)(0%)?,var\(--building-color4\)10%,var\(--window-color4\),var\(--window-color4\)90%\)/);
```
