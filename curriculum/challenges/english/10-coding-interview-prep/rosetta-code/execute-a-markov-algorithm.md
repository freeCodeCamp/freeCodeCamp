...
```js
assert.deepEqual(markov(rules[0], data[0]), outputs[0]);
```
...
```js
assert.deepEqual(markov(rules[1], data[1]), outputs[1]);
```
...
```js
assert.deepEqual(markov(rules[2], data[2]), outputs[2]);
```
...
```js
assert.deepEqual(markov(rules[3], data[3]), outputs[3]);
```
...
```js
assert.deepEqual(markov(rules[4], data[4]), outputs[4]);
```
...
let data=["I bought a B of As from T S.",
          "I bought a B of As from T S.",
          "I bought a B of As W my Bgage from T S.",
          "_1111*11111_",
          "000000A000000"];
