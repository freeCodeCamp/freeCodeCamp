...
Markov Concepts are used in machine learning, because of its simple approach in data manipulation. With a set number of `rules` you can manipulate given `test` to create a desired `output`.

We have added in the background:

The `rules` in the form of `nested array`, and the `test` in the form of `array`, too. And the desired `outputs`.
...
let test=["I bought a B of As from T S.",
...
Using Markov Algorithm, change the `test` into the desired `outputs` using the `rules` provided for you.
