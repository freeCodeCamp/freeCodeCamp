...
Adicione o elemento `label` após o terceiro `fieldset` e um elemento `input` com o atributo `type` com o valor de `checkbox` dentro do elemento `label` recém-adicionado. Torne este elemento `input` obrigatório (`required`), pois os usuários não devem se inscrever sem ler os termos e condições.
