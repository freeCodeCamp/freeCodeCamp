...
Para resolver isso, você pode adicionar o que é necessário usando um elemento `legend` com o texto `Tipo de conta (necessário)` antes do elemento `label` dentro do segundo campo `fieldset`. Adicione então o atributo `checked` ao campo de entrada `Personal` input para garantir que o formulário foi enviado com o que era necessário neste.
