...
Você quer adicionar o mesmo gradiente às próximas duas seções. Em vez de fazer isso, crie uma nova classe chamada `bb1-window` e mova as propriedades de valores de `height` e `background` de `.bb1a` para a nova classe.

# --hints--

Você deve criar uma declaração de classe chamada `bb1-window`.
