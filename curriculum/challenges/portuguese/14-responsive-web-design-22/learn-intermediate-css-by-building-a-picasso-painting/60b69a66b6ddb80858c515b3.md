...
Você deve ter um seletor `#orange-character`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character'));
```

O seletor `#orange-character` deve ter a propriedade `width` definida como `250px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.width === '250px');
```

O seletor `#orange-character` deve ter a propriedade `height` definida como `550px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.height === '550px');
```

O seletor `#orange-character` deve ter a propriedade `background-color` definida como `rgb(240, 78, 42)`.
