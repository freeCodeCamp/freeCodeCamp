...
O seletor `.triangle` deve ter a propriedade `border-style` definida como `solid`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.triangle')?.borderStyle === 'solid');
```

O seletor `.triangle` deve ter a propriedade `border-width` definida como `42px 45px 45px 0`.
