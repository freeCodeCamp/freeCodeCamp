...
Hora de usar alguns ícones FontAwesome.
...
Você deve ter quatro novos elementos `i` dentro do elemento `#white-paper`.

```js
assert(document.querySelectorAll('#white-paper > i')?.length === 4);
```

Todos os elementos `i` devem ter a `class` definida como `fas fa-music`.
