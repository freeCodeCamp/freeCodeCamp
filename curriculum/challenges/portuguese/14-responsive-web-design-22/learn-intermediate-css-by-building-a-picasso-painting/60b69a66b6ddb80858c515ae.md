...
O primeiro novo elemento `div` deve ter o `id` definido como `black-round-hat`.

```js
assert(document.querySelectorAll('#orange-character > div')?.[0]?.getAttribute('id') === 'black-round-hat');
```

O segundo novo elemento `div` deve ter o `id` definido como `eyes-div`.

```js
assert(document.querySelectorAll('#orange-character > div')?.[1]?.getAttribute('id') === 'eyes-div');
```

O terceiro novo elemento `div` deve ter para o atributo `id` o valor de `triangles`.

```js
assert(document.querySelectorAll('#orange-character > div')?.[2]?.getAttribute('id') === 'triangles');
```

O quarto novo elemento `div` deve ter para o atributo `id` o valor de `guitar`.
