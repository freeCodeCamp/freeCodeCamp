...
Você deve ter três novos elementos `div` dentro do elemento `#black-character`.

```js
assert(document.querySelectorAll('#black-character > div')?.length === 3);
```

O primeiro novo elemento `div` deve ter o `id` definido como `black-hat`.

```js
assert(document.querySelectorAll('#black-character > div')?.[0]?.getAttribute('id') === 'black-hat');
```

O segundo novo elemento `div` deve ter para o `id` definido como `gray-mask`.

```js
assert(document.querySelectorAll('#black-character > div')?.[1]?.getAttribute('id') === 'gray-mask');
```

O terceiro novo elemento `div` deve ter o `id` definido como `white-paper`.
