...
O seletor `#black-hat` deve ter a propriedade `position` definida como `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.position === 'absolute');
```

O seletor `#black-hat` deve ter a propriedade `top` definida como `-150px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.top === '-150px');
```

O seletor `#black-hat` deve ter a propriedade `left` definida como `0`.
