...
Coloque outro botão de opção com a opção `Outdoor` em um novo elemento `label`. O novo botão de opção deve ser colocado depois do primeiro. Além disso, defina o valor do atributo `id` como `outdoor`.
