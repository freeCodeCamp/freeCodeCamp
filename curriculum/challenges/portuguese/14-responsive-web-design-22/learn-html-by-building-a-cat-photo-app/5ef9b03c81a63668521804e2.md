...
Os formulários normalmente usam caixas de seleção para perguntas que tenham mais de uma resposta. The `input` element with a `type` attribute set to `checkbox` creates a checkbox.
