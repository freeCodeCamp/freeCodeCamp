...
Crie outra `@media` query para `only screen` com uma `max-width` de `600px`. Dentro, crie um seletor `.text-with-images` e dê a ele uma propriedade `grid-template-columns` de `1fr`.
