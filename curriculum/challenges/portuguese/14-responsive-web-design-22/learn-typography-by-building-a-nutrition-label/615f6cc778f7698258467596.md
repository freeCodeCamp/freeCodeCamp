...
Abaixo do elemento com o texto `Total Fat`, crie um elemento `p` com o texto `Saturated Fat 1g 5%`. Wrap the `5%` in a `span` with the `class` attribute set to `bold`. In this case this is enough to align the percentage to `5%`.
