...
Envolva o texto `2/3 cup (55g)` em um elemento `span` e dê a ele um atributo `class` definido como `right`.
...
O novo elemento `span` deve ter o atributo `class` definido como `right`.
