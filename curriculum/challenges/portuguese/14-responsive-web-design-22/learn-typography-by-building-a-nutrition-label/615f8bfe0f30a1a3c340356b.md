...
Você deve criar dois elementos `p` no final do elemento `.daily-value.sm-text`.
...
O primeiro elemento `span` deve ter o atributo `class` definido como `right`. Lembre-se, não o torne negrito.
...
O segundo elemento `span` deve ter o atributo `class` definido como `right`. Lembre-se, não o torne negrito.
