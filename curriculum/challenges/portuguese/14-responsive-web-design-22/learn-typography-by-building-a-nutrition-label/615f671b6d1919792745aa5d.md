...
Crie um elemento `div` abaixo do elemento `.md`. Dê a ele um atributo `class` definido como `daily-value sm-text`. Dentro dessa nova `div`, adicione um elemento `p` com o texto `% Daily Value *` e defina o atributo `class` como `right bold`.

# --hints--

Você deve criar um elemento `div` após o elemento `.md`.
...
A nova `div` deve ter o atributo `class` definido como `daily-value sm-text`.
...
O novo elemento `p` deve ter o atributo `class` definido como `right bold`.
