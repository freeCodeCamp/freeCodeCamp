...
Crie outro elemento `p` abaixo do divisor grande. Dê ao elemento `p` o texto `Vitamin D 2mcg 10%`. Use um `span` para fazer com que `10%` se alinhe à direita, mas não o deixe em negrito.
...
O elemento `span` deve ter a `class` definida como `right`. Lembre-se, não o torne negrito.
