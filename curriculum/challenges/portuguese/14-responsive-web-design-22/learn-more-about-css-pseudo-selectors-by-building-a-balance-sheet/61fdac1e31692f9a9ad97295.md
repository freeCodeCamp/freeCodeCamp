...
A propriedade `clip-path` determina a forma que a propriedade `clip` receberá. Defina as propriedades `clip-path` e `-webkit-clip-path` para que tenham o valor `inset(50%)`, deixando clip-path na forma de um retângulo dentro do elemento.
