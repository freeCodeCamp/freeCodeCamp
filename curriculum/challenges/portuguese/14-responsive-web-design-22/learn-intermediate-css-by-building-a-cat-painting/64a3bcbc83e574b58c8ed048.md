...
Como você fez na etapa anterior, use um seletor descendente que aponte para os três elementos `div` dentro do elemento `.cat-whiskers-right`. Dê a ele uma `width` de `40px`, uma `height` de `1px` e uma `background-color` de `#000`.
