...
Seu objeto `myFavoriteFootballTeam` deve ter uma propriedade `year`.

```js
assert.property(myFavoriteFootballTeam, 'year');
```

Sua propriedade `year` deve ser um número.

```js
assert.isNumber(myFavoriteFootballTeam.year);
```

Sua propriedade `year` deve ser definida como `1986`.

```js
assert.equal(myFavoriteFootballTeam.year, 1986);
```

Seu objeto `myFavoriteFootballTeam` deve ter uma propriedade `isWorldCupWinner`.

```js
assert.property(myFavoriteFootballTeam, 'isWorldCupWinner');
```

Sua propriedade `isWorldCupWinner` deve ser booleana.

```js
assert.isBoolean(myFavoriteFootballTeam.isWorldCupWinner);
```

Sua propriedade `isWorldCupWinner` deve ser definida como `true`.

```js
assert.equal(myFavoriteFootballTeam.isWorldCupWinner, true);
```

Seu objeto `myFavoriteFootballTeam` deve ter uma propriedade `headCoach`.

```js
assert.property(myFavoriteFootballTeam, 'headCoach');
```

Seu valor `headCoach` deve ser um objeto.

```js
assert.isObject(myFavoriteFootballTeam.headCoach)
```

Seu valor `headCoach` deve ser um objeto vazio.
