...
O método `.map()` deve ter uma função de callback que recebe um argumento de `el`.

```js
assert.match(calculate.toString(), /array\.map\(\(?\s*el\s*\)?\s*=>|array\.map\(function\s*\(?el\)\s*\{/)
```

A função de callback deve usar o construtor `Number` para converter `el` para um número.
