...
In your `goTown` function, change your `button` elements' `innerText` properties to be `Go to store`, `Go to cave`, and `Fight dragon`. Atualize as propriedades `onclick` para que sejam `goStore`, `goCave` e `fightDragon`, respectivamente.
