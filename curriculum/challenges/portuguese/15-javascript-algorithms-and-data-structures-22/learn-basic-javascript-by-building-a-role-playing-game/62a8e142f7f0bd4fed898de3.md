...
Adicione um novo objeto no final do array `locations`, seguindo as mesmas propriedades do resto dos objetos. Set `name` to `"fight"`, `"button text"` to an array with `"Attack"`, `"Dodge"`, and `"Run"`, `"button functions"` to an array with `attack`, `dodge`, and `goTown`, and `text` to `"You are fighting a monster."`.
