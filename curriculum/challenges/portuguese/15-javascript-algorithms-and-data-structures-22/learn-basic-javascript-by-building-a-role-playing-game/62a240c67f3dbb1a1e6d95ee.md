...
Dê ao `#game` uma largura máxima de `500px` e uma altura máxima de `400px`. Defina a `background-color` como `lightgray` e a `color` como `white`. Use margins to center it by setting the top and bottom margin to `0`, and the left and right margin to `auto`. Finally, give it `10px` of padding on all four sides.
