...
Usando uma lista de seletores (`selector1, selector2`) dê aos elementos `#controls` e `#stats` uma `border` de `1px solid #0a0a23`, uma cor de texto `#0a0a23` e `5px` de `padding`.

# --hints--

Você deve ter uma lista de seletores `#controls, #stats`.
...
O seletor `#controls, #stats` deve ter uma `border` de `1px solid #0a0a23`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#controls, #stats')?.border, '1px solid rgb(10, 10, 35)');
```

O seletor `#controls, #stats` deve ter uma `color` de `#0a0a23`.
