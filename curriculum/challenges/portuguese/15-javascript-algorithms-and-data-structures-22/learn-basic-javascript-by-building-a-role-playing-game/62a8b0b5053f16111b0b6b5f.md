...
A função `update` deve definir `button1.innerText` para `Go to store`.

```js
assert.match(update.toString(), /button1\.innerText\s*=\s*('|")Go to store\1/);
```

A função `update` deve definir `button2.innerText` para `Go to cave`.

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*('|")Go to cave\1/);
```

A função `update` deve definir `button3.innerText` para `Fight dragon`.
