...
O JavaScript oferece o utilitário `Math` integrado, que oferece várias ferramentas matemáticas. Uma dessas é `Math.random()`, que gera um número aleatório entre 0 e 1. Outra é `Math.floor()`, que arredonda um número qualquer para o inteiro mais próximo abaixo dele.
...
Seguindo este padrão, use o operador de adição (`+`) para adicionar um número aleatório entre `1` e o valor de `xp` à variável `monsterHealth` alterada.
