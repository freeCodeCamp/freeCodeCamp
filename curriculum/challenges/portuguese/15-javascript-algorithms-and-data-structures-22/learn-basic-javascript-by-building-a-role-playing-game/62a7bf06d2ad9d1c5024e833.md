...
Você também precisará atualizar as funções que são executadas quando os botões forem clicados. Atualize a propriedade `onclick` para cada botão para executar `buyHealth`, `buyWeapon` e `goTown`, respectivamente.
