...
You should add `"You sold a "` to `text.innerText`. O espaçamento é importante.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

You should add the value of `currentWeapon` to the `"You sold a "` string. Use o operador de concatenação para fazer isso na mesma linha.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a\s\1\s*\+\s*currentWeapon/);
```

You should add the string `"."` to the value of `currentWeapon`. Use o operador de concatenação para fazer isso na mesma linha.
