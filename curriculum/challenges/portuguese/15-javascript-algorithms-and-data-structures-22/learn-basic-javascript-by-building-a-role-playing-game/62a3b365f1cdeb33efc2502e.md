...
Você deve usar a palavra-chave `let` para declarar a variável `health`.

```js
assert.match(code, /let health/);
```

Você deve usar a palavra-chave `let` para declarar a variável `gold`.

```js
assert.match(code, /let gold/);
```

Você deve usar a palavra-chave `let` para declarar a variável `currentWeapon`.

```js
assert.match(code, /let currentWeapon/);
```

Você não deve alterar os valores das variáveis.
