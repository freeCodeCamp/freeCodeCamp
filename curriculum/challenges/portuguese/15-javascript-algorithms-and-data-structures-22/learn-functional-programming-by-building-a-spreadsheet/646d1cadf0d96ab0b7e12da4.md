...
Como `elemValue` retorna uma função, a função `addChars` retorna, no fim, um array de referências de função. Você quer que o método `.map()` execute a função interna à função `elemValue`, o que significa que você precisa chamar `elemValue` em vez de fazer referência a ela. Passe `num` como argumento para a função `elemValue`.
