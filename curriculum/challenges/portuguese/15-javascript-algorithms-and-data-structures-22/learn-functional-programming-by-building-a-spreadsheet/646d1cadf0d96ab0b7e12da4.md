...
Because `elemValue` returns a function, your `addCharacters` function ultimately returns an array of function references. Você quer que o método `.map()` execute a função interna à função `elemValue`, o que significa que você precisa chamar `elemValue` em vez de fazer referência a ela. Passe `num` como argumento para a função `elemValue`.
