...
Na função `elemValue`, declare uma função chamada `inner`, que recebe um parâmetro `character`.

Em seguida, retorne a função `inner`.

# --hints--

Você deve declarar uma variável `inner`.
...
Você deve usar `const` para declarar a variável `inner`.
...
A variável `inner` deve ser uma arrow function.
...
A função `inner` deve ter `character` como o único parâmetro.
...
A função `inner` deve estar vazia.
...
Você deve retornar explicitamente a função `inner`.
