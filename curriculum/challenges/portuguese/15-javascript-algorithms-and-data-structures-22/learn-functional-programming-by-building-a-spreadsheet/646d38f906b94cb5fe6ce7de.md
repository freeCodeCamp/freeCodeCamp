...
`addCharacters(char1)` também está retornando uma função, que retorna outra função. Você precisa fazer outra chamada de função para acessar essa referência da função mais interna para a função de callback de `.map()`. JavaScript allows you to immediately invoke returned functions:
