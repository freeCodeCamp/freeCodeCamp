...
`addCharacters(char1)` também está retornando uma função, que retorna outra função. Você precisa fazer outra chamada de função para acessar essa referência da função mais interna para a função de callback de `.map()`. O JavaScript permite a você encadear chamadas de função:

```js
myFunc(1)("hi");
```

Encadeie uma chamada de função à chamada de `addCharacters(char1)` e passe `char2` como o argumento.
