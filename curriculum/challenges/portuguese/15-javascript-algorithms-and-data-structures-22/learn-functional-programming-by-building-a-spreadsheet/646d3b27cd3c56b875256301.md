...
Declare uma variável `cellExpanded` e atribua a ela o valor da chamada de `.replace()` na variável `rangeExpanded`. Passe `cellRegex` para ela e uma função de callback vazia. A função de callback deve ter um parâmetro `match`.
