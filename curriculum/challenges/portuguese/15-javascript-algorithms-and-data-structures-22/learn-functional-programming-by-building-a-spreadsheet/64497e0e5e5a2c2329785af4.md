...
Atualize a condição `if` para também verificar se o primeiro caractere de `value` é `=`.

# --hints--

A condição `if` também deve verificar se o primeiro caractere de `value` é `=`. Você pode usar `[0]`, `.startsWith()` ou `.charAt(0)`.
