...
Agora, atualize a função mais interna na cadeia de `addCharacters` para que retorne implicitamente o resultado da chamada de `charRange()` com `character1` e `character2` como argumentos.

# --hints--

A função mais interna deve usar um retorno implícito.
...
A função mais interna deve retornar o resultado da chamada de `charRange()`.
...
Você deve passar `character1` como o primeiro argumento para a chamada de `charRange()`.
...
Você deve passar `character2` como o segundo argumento para a chamada de `charRange()`.
