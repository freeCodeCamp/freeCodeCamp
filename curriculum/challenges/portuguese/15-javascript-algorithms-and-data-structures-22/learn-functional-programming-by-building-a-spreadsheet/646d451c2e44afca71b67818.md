...
Declare uma função `apply`, que recebe os parâmetros `fn` e `args`.

# --hints--

Você deve declarar uma variável `apply`.
...
Você deve usar `const` para declarar a variável `apply`.
...
A variável `apply` deve receber uma arrow function.
...
A função `apply` deve ter `fn` como primeiro parâmetro.
...
A função `apply` deve ter `args` como segundo parâmetro.
...
A função `apply` deve estar vazia.
