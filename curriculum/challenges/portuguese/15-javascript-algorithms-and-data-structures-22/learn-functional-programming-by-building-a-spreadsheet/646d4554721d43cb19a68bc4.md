...
O parâmetro `fn` será o nome de uma função, como `SUM`. Atualize `apply` para que retorne implicitamente a função encontrada na propriedade `fn` do objeto `spreadsheetFunctions`.

Lembre-se de que `fn` não pode estar em minúsculas, então você precisará convertê-lo em uma string em letras minúsculas.
