...
Chame o método `.map()` na chamada de `rangeFromString()`, passando uma referência para `addCharacters` como a função de callback.

# --hints--

Você deve chamar o método `.map()` na chamada de `rangeFromString()`.
...
Você deve passar uma referência a `addCharacters` como a função de callback do método `.map()`.
