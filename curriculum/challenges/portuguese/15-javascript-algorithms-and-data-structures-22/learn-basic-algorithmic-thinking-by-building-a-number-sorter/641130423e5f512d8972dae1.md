...
Para corrigir isso, você pode passar uma função de callback para o método `.sort()`. The callback function takes two arguments - for yours, use `a` and `b`. Leave the function empty for now.
