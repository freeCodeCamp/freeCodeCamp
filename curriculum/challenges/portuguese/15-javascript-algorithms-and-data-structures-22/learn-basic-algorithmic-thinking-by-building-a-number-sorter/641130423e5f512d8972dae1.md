...
Para corrigir isso, você pode passar uma função de callback para o método `.sort()`. A função de callback recebe dois argumentos - para a sua, use `a` e `b`. Deixe a função em branco, por enquanto.

# --hints--

Você deve passar uma função de callback para o método `.sort()`. Lembre-se de usar a sintaxe das arrow functions.

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*\(?.*\)?\s*=>/);
```

A função de callback deve receber dois argumentos, `a` e `b`.
