...
Declare a `currValue` variable and assign it the value at `array[i]`. Em seguida, declare uma variável `j` e atribua a ela `i - 1`. A variável `j` deve ser atribuível novamente.
