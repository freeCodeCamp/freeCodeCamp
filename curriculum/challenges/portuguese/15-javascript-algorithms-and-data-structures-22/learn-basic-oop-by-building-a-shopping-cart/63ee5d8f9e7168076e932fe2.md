...
Após o elemento `h2`, crie dois elementos `p`. Dê ao primeiro a `class` `dessert-price` e o texto da variável `price` com um sinal de dólar na frente. Give the second a `class` of `product-category`, and the text `"Category: "` followed by the value of the `category` variable.
