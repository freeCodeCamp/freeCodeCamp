...
Por fim, atualize a propriedade `display` do objeto `style` da variável `cartContainer` para que tenha outro ternário que verifica se `isCartShowing` é verdadeiro. If it is, set the `display` property to `"block"`, otherwise set it to `"none"`.
