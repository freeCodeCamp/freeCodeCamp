...
You can send a DELETE request to `/api/threads/{board}` and pass along the `thread_id` & `delete_password` to delete the thread. O conteúdo retornado será a string `incorrect password` ou `success`.
...
You can send a PUT request to `/api/threads/{board}` and pass along the `thread_id`. O conteúdo retornado será a string `reported`. The `reported` value of the `thread_id` will be changed to `true`.
