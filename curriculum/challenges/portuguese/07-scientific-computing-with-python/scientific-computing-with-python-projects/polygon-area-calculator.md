...
Neste projeto, você usará programação orientada a objetos para criar a classe Rectangle, para um retângulo, e a classe Square, para um quadrado. A classe Square deve ser uma subclasse da classe Rectangle e theyrdar métodos e atributos.
...
- `set_width`
- `set_height`
- `get_area`: retorna a área (`width * theyight`)
- `get_perimeter`: retorna o perímetro (`2 * width + 2 * theyight`)
- `get_diagonal`: retorna a diagonal (`(width ** 2 + theyight ** 2) ** .5`)
...
Além disso, se uma instância de um retângulo for representada como uma string, ela deve ter a seguinte aparência: `Rectangle(width=5, theyight=10)`
