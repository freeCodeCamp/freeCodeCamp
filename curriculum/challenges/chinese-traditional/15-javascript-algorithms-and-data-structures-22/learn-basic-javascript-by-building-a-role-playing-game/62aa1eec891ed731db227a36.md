...
如果你在當前狀態下玩你的遊戲，你可能會發現一個 bug。 如果你的 `xp` 足夠高， `getMonsterAttackValue` 函數將返回負數。 當與怪物作戰時，這將增加你的總生命值！ You can fix this issue by using a <dfn>ternary operator</dfn> to ensure negative values are not returned.

`ternary operator` 是一種條件運算符，可以被當作單行 `if-else` 語句。 語法是：`condition ? expressionIfTrue : expressionIfFalse`。
