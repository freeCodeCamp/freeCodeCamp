...
爲 `#game` 提供 `500px` 的最大寬度和 `400px` 的最大高度。 將 `background-color` 設置爲 `lightgray`，並將 `color` 設置爲 `white`。 Use margins to center it by setting the top and bottom margin to `0`, and the left and right margin to `auto`. Finally, give it `10px` of padding on all four sides.
...
`#game` 選擇器應該具有 `lightgray` 的 `background-color`。
