...
在 `team` 屬性下方，新增三個新屬性鍵值對。
名為 `sport` 的鍵和一個名為 `"Football"` 的字串值。
鍵名為 `year`，數值為 `1986`。
名為 `isWorldCupWinner` 的鍵和設定為 `true` 的布林值。
一個名為 `headCoach` 的新鍵，其值為空物件。
...
您的 `myFavoriteFootballTeam` 物件應該有一個 `year` 屬性。
