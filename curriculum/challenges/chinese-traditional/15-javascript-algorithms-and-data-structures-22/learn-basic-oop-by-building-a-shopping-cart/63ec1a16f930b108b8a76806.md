...
接下來是獲取總數。 獲取 `#total-items`, `#subtotal`, `#taxes` 和 `#total` 元素。 將它們分別存儲在名爲 `totalNumberOfItems`, `cartSubTotal`, `cartTaxes` 和 `cartTotal` 的變量中。

# --hints--

你應該使用 `document.getElementById()` 來獲取 `#total-items` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)total-items\1\s*\)/);
```

你應該將 `#total-items` 元素分配給一個名爲 `totalNumberOfItems` 的變量。 請記住使用 `const` 聲明變量。

```js
assert.match(code, /const\s+totalNumberOfItems\s*=\s*document\.getElementById\(\s*('|"|`)total-items\1\s*\)/);
```

你應該使用 `document.getElementById()` 來獲取 `#subtotal` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)subtotal\1\s*\)/);
```

你應該將 `#subtotal` 元素分配給一個名爲 `cartSubTotal` 的變量。 請記住使用 `const` 聲明變量。

```js
assert.match(code, /const\s+cartSubTotal\s*=\s*document\.getElementById\(\s*('|"|`)subtotal\1\s*\)/);
```

你應該使用 `document.getElementById()` 來獲取 `#taxes` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)taxes\1\s*\)/);
```

你應該將 `#taxes` 元素分配給一個名爲 `cartTaxes` 的變量。 請記住使用 `const` 聲明變量。

```js
assert.match(code, /const\s+cartTaxes\s*=\s*document\.getElementById\(\s*('|"|`)taxes\1\s*\)/);
```

你應該使用 `document.getElementById()` 來獲取 `#total` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)total\1\s*\)/);
```

你應該將 `#total` 元素分配給一個名爲 `cartTotal` 的變量。 請記住使用 `const` 聲明變量。
