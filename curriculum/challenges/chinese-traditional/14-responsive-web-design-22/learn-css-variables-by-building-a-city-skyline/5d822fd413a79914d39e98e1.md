...
那個也使用了備用值？ 我現在明白問題是什麼了！ 你在 `.bb1` 中聲明的變量不會級聯到 `.bb2` 和 `.bb3` 兄弟元素。 這就是 CSS 的工作原理。 因此，變量通常在 `:root` 選擇器中聲明。 這是 CSS 中最高級別的選擇器；將變量放在那裏將使它們在任何地方都可用。 將 `:root` 選擇器添加到樣式表的頂部，並將所有變量聲明移到那裏。
