...
窗戶在該部分的左側堆疊在一起，在紫色建築的後面。 在 `.building-wrap` 下添加一個名爲 `window-wrap` 的新類。 使 `.window-wrap` 成爲一個 flexbox 容器，使用 `align-items` 和 `justify-content` 屬性將它的子元素垂直居中，並分別在它們的父元素中均勻排布。
