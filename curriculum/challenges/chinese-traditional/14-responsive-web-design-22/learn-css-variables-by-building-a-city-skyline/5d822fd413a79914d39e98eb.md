...
天際線正在匯合。 填充前景建築物的 `background-color` 屬性。 使用 `--building-color1` 變量填充 `.fb3` 和 `.fb4`，使用 `--building-color2` 填充 `.fb5`，使用 `--building-color3` 填充 `.fb2` 和 `.fb6`，以及使用 `--building-color4` 填充 `.fb1`。
