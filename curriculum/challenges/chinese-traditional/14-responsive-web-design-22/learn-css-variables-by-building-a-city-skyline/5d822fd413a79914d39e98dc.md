...
建築物相互堆疊，並超出了屏幕。讓我們解決這個問題。 將屬性 `display: flex;`, `align-items: flex-end;` 和 `justify-content: space-evenly;` 添加到 `background-buildings` 類。 這將再次使用 flexbox 將建築物均勻地分佈在元素的底部。
