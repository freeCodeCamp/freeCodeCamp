...
在天空漸變顏色列表的頂部，你可以在其中放置漸變方向；添加 `circle closest-corner at 15% 15%,`。 這會將漸變的起點移動到距離頂部和左側 `15%` 的位置。 漸變將在 `closest-corner` 處結束，形狀爲 `circle`。 這些是漸變中內置的一些關鍵字，用於描述其行爲方式。

# --hints--

你應該給 `.sky` `radial-gradient` 一個方向爲 `circle closest-corner at 15% 15%`。
