...
選擇 `.penguin-body` 元素，使其 `width` 爲 `53%`，`height` 爲 `45%`。 然後將 `background` 設置爲漸變軸爲 `45deg` 的線性漸變，起始點 `0%` 爲 `rgb(134, 133, 133)`，過渡到中間 `25%` 位置的 `rgb(234, 231, 231)`，在過渡到 `67%` 位置的 `white`。
