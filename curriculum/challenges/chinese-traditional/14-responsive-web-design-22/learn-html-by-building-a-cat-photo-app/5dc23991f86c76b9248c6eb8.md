...
在上一步中，你將 `h2` 元素、註釋和 `p` 元素嵌套在 `main` 元素中。 這叫作 *嵌套*。 嵌套的元素（子元素）的位置應該放在被他們嵌套的元素（父元素）中新的一行並且右側保留兩個空格的間距的後面。 這個間距被稱爲縮進，它被用來使 HTML 更容易讀。

`h2` 元素和註釋比下面代碼中的 `main` 元素多縮進了兩個空格。 使用鍵盤上的空格鍵在 `p` 元素前面再添加兩個空格，使其縮進得當。
