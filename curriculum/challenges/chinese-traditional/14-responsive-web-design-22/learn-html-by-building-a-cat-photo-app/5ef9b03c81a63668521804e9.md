...
你的錨（`a`）元素應該有一個結束標籤。 結束標籤在 `<` 字符之後有一個 `/`。
...
你的錨點（`a`）元素應該有一個 `href` 屬性，其值爲 `https://www.freecodecamp.org`。 你可能忽略了屬性/值，或者有拼寫錯誤。
...
嵌套錨（`a`）元素後，瀏覽器中唯一可見的 `p` 元素內容應該是 `No Copyright - freeCodeCamp.org`。 仔細檢查 `p` 和嵌套錨元素的文本、間距或標點符號。
