...
HTML <dfn>屬性</dfn>是寫在一個元素的開始標籤中的特殊詞彙，它們用於控制這個元素的行爲。 在 `img` 元素中的 `src` 屬性明確了一個圖片的 URL（圖片所在的位置）。 這是一個使用了 `src` 屬性的 `img` 元素：`<img src="https://www.your-image-source.com/your-image.jpg">`。
