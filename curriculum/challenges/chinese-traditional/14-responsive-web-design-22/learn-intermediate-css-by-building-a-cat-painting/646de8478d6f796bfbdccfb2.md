...
爲了使右眼看起來像一隻眼睛，給它設置一個 `60%` 的邊框半徑。 Also, using the `transform` property, rotate it at `-25` degrees.

# --hints--

你的 `.cat-right-eye` 選擇器應設置一個 `border-radius` 屬性爲 `60%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.borderRadius === '60%')
```

你的 `.cat-right-eye` 選擇器應設置一個 `transform` 屬性爲 `rotate(-25deg)`。
