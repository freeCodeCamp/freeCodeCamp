...
將內耳移到合適位置，將 `position` 屬性設置爲 `absolute`、`top` 設置爲 `22px`、`left` 設置爲 `-20px`。

# --hints--

你的 `.cat-left-inner-ear` 選擇器應該具有一個 `position` 屬性，其值設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.position === 'absolute')
```

你的 `.cat-left-inner-ear` 選擇器應該具有一個 `top` 屬性，其值設置爲 `22px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.top === '22px')
```

你的 `.cat-left-inner-ear` 選擇器應該具有一個 `left` 屬性，其值設置爲 `-20px`。
