...
在你的 `.cat-mouth` 元素內，創建一個帶有類 `cat-mouth-line-left` 的 `div` 元素和另一個帶有類 `cat-mouth-line-right` 的 `div` 元素。

# --hints--

你不應該更改現有的 `div` 元素，其類名爲 `cat-mouth`。

```js
assert(document.querySelectorAll('div.cat-mouth').length === 1);
```

你應該在 `.cat-mouth` 元素內有兩個 `div` 元素。

```js
assert(document.querySelectorAll('.cat-mouth div').length === 2);
```

在 `.cat-mouth` 元素內的第一個 `div` 元素應具有類 `cat-mouth-line-left`。

```js
assert(document.querySelectorAll('.cat-mouth div')[0]?.classList.contains('cat-mouth-line-left'));
```

在 `.cat-mouth` 元素內的第二個 `div` 元素應該有類 `cat-mouth-line-right`。
