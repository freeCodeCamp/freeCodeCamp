...
在 `.cat-whiskers-right` 元素內，創建 3 個 `div` 元素，類分別爲 `cat-whisker-right-top`、`cat-whisker-right-middle` 和 `cat-whisker-right-bottom`。

# --hints--

你不應該更改已有的 `div` 元素，類名爲 `cat-whiskers-right`。

```js
assert(document.querySelectorAll('div.cat-whiskers-right').length === 1)
```

你應該在你的 `.cat-whiskers-right` 元素內部創建三個 `div` 元素。

```js
assert(document.querySelectorAll('.cat-whiskers-right div').length === 3)
```

位於 `.cat-whiskers-right` 元素內的第一個 `div` 元素應具有 `cat-whisker-right-top` 類。

```js
assert(document.querySelectorAll('.cat-whiskers-right div')[0].classList.contains('cat-whisker-right-top'))
```

第二個 `div` 元素應位於 `.cat-whiskers-right` 元素內，並且應該有類 `cat-whisker-right-middle`。

```js
assert(document.querySelectorAll('.cat-whiskers-right div')[1].classList.contains('cat-whisker-right-middle'))
```

位於 `.cat-whiskers-right` 元素內部的第三個 `div` 元素應該有類 `cat-whisker-right-bottom`。
