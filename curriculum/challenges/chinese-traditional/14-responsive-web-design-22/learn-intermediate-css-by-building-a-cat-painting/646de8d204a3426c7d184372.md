...
這看起來很像眼睛了，但你仍然可以讓它們變得更好。 這就是爲什麼你創建了兩個內眼 `div` 元素。

使用類選擇器，給你的 `.cat-left-inner-eye` 元素設置寬度爲 `10px` 和高度爲 `20px`。 另外，給它一個背景顏色 `#fff`。
