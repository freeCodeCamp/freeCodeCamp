...
在 `.cat-whiskers-left` 元素內部，創建三個具有類 `cat-whisker-left-top`、`cat-whisker-left-middle` 和 `cat-whisker-left-bottom` 的 `div` 元素。

# --hints--

你不應該更改現有的 `div` 元素及其類 `cat-whiskers-left`。
