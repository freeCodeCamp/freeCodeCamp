...
通過設置 `999` 的 `z-index` 確保你的年份不會被隱藏。 然後，給它一個 `0 -2px` 的 `margin`，並將 `padding` 設置爲 `0.5rem calc(1.25rem + 2px) 0.5rem 0`。

# --hints--

`#years` 選擇器應該將 `z-index` 屬性設置爲 `999`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('z-index') === '999');
```

`#years` 選擇器應該有一個 `margin` 屬性設置爲 `0 -2px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('margin') === '0px -2px');
```

`#years` 選擇器應該有一個 `padding` 屬性設置爲 `0.5rem calc(1.25rem + 2px) 0.5rem 0`。
