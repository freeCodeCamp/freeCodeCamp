...
在第四個 `tr` 元素中，添加一個帶有文本 `Total Liabilities` 的 `th` 元素。 將文本 `Liabilities` 包裝在 `span` 元素中，並將 `class` 屬性設置爲 `sr-only`。
...
應該將文本 `Liabilities` 包裝在 `span` 元素中。
