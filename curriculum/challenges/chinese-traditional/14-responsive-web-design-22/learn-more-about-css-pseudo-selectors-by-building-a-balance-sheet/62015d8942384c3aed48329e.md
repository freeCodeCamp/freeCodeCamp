...
你之前將表格包裹在 section 元素中 - 現在你可以設置樣式以給表格添加邊框。 創建一個 `section` 選擇器，並將其 `max-width` 屬性設置爲 `40rem` 以進行響應式設計。 將 `margin` 屬性設置爲 `0 auto` 以使其居中，並將 `border` 屬性設置爲 `2px solid #d0d0d5`。
