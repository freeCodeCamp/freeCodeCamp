...
創建一個 `tr.data th .description` 選擇器以定位 `class` 爲 `description` 的元素，這些元素位於你的 `.data` 錶行中的 `th` 元素。 給它們一個塊顯示，使文本斜體並具有正常的粗細，並將它們定位爲 `padding` 設置爲 `1rem 0 0.75rem` 和右邊距爲 `-13.5rem`。
