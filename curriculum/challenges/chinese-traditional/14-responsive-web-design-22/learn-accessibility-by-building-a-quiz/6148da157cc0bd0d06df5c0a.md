...
要使 `input` 框彼此對齊，請將所有 `.info` 元素內 `input` 和 `label` 元素的 `display` 屬性設置爲 `inline-block`。
...
你應該給 `input` 和 `label` 元素添加 `text-align` 值爲 `right`。
...
你應該在 `.info input` 規則之前設置 `text-align: right` 屬性。
