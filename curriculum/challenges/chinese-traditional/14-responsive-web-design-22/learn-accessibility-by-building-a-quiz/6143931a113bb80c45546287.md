...
可以說，`D.O.B.` 的描述性不夠。 對於視力受損的用戶來說尤其如此。 解決此類問題的一種方法是添加只有屏幕閱讀器可以閱讀的文本，而無需向標籤添加可見文本。

將具有 `sr-only` 類的 `span` 元素添加到第三個 `label` 元素的當前文本內容。

# --hints--

你應該在第三個 `label` 元素中添加一個 `span` 元素。

```js
assert.exists(document.querySelector('.info:nth-of-type(3) > label > span'));
```

你應該給 `span` 元素添加 `sr-only` 類。

```js
assert.equal(document.querySelector('.info:nth-of-type(3) > label > span')?.className, 'sr-only');
```

你應該將 `span` 放在文本內容 `D.O.B.` 之後。
