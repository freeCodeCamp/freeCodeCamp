...
在 `.image-wrapper` 元素中，爲你的第一個 `img` 元素添加 `src` 值爲 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`，添加 `alt` 爲 `image of the quote machine project`，以及 `class` 爲 `image-1`，和值爲 `lazy` 的 `loading` 屬性、值爲 `600` 的 `width` 屬性、值爲 `400` 的 `height` 屬性。
