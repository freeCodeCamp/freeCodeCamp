...
創建第三個 `@media` 查詢，對應 `only screen` 且 `max-width` 爲 `550px`。 在其中，創建一個 `.hero-title` 選擇器，將 `font-size` 設置爲 `6rem`，一個 `.hero-subtitle, .author, .quote, .list-title` 選擇器，`font-size` 設置爲 `1.8rem`，一個 `.social-icons` 選擇器，`font-size` 設置爲 `2rem`，一個 `.text` 選擇器，`font-size` 設置爲 `1.6rem`。
