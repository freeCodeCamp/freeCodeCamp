...
在你的 `.image-wrapper` 元素中，給第三個 `img` 元素添加 `src` 爲 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`，添加 `alt` 爲 `four people working on code`，以及值爲 `lazy` 的 `loading` 屬性、值爲 `image-3` 的 `class`、值爲 `600` 的 `width` 屬性和值爲 `400` 的 `height` 屬性。
