...
在 `.image-quote` 元素中，嵌套一個 `hr` 元素、一個 `p` 元素和第二個 `hr` 元素。 給 `p` 元素一個 `class` 爲 `quote` 和文本 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`。
