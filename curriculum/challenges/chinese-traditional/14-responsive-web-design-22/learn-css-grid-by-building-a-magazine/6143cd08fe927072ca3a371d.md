...
在你的 `.image-wrapper` 元素中，給第二個 `img` 元素添加 `src` 爲 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`，添加 `alt` 爲 `image of a calculator project`，以及值爲 `lazy` 的 `loading` 屬性、值爲 `image-2` 的 `class`、值爲 `400` 的 `width` 屬性和值爲 `400` 的 `height` 屬性。
