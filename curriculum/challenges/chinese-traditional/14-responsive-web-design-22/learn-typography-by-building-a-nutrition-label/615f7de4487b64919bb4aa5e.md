...
在最後一個 `.divider` 之後，使用文本 `Cholesterol 0mg 0%` 創建一個新的 `p` 元素。 將文本 `Cholesterol` 包裹在 `span` 元素中，並將 `span` 元素的 `class` 屬性設置爲 `bold`。 將文本 `0%` 包裹在另一個 `span` 元素中，並將 `class` 設置爲 `bold`。 Finally, nest the `Cholesterol` `span` element and `0mg` inside an additional `span` element for alignment.
