...
現在將 `#eyes-div` 元素移動到位，將 `position` 設置爲 `absolute`，`top` 設置爲 `-40px`，並且 `left` 設置爲 `20px`。

# --hints--

你的 `#eyes-div` 選擇器應該有一個 `position` 屬性設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.position === 'absolute');
```

你的 `#eyes-div` 選擇器應該有一個 `top` 屬性設置爲 `-40px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.top === '-40px');
```

你的 `#eyes-div` 選擇器應該有一個 `left` 屬性設置爲 `20px`。
