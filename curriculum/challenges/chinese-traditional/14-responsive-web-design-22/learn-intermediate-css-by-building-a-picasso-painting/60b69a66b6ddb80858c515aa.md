...
現在將 `position` 設置爲 `absolute`，將 `top` 設置爲 `20%`，將 `left` 設置爲 `20%`。

# --hints--

你的 `#blue-left` 選擇器應該有一個 `position` 屬性設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-left')?.position === 'absolute');
```

你的 `#blue-left` 選擇器應該有一個 `top` 屬性設置爲 `20%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-left')?.top === '20%');
```

你的 `#blue-left` 選擇器應該有一個 `left` 屬性設置爲 `20%`。
