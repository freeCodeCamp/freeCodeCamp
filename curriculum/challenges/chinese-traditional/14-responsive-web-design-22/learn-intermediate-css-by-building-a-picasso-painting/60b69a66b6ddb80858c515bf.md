...
選擇值爲 `guitar-right` 的 `id`，將 `position` 設置爲 `absolute`。 這一次，將 `left` 設置爲 `100px`。

# --hints--

你應該創建一個新的 `#guitar-right` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-right'));
```

你的 `#guitar-right` 選擇器應該有一個 `position` 屬性設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-right')?.position === 'absolute');
```

你的 `#guitar-right` 選擇器應該有一個 `left` 屬性設置爲 `100px`。
