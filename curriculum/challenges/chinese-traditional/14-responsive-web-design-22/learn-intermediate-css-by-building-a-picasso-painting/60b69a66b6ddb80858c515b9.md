...
使用 id 選擇器爲 id 爲 `eyes-div` 的元素創建規則。 將 `width` 設置爲 `180px`，並將 `height` 設置爲 `50px`。

# --hints--

你應該創建一個 `#eyes-div` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div'));
```

你的 `#eyes-div` 選擇器應該有一個 `width` 屬性設置爲 `180px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.width === '180px');
```

你的 `#eyes-div` 選擇器應該將 `height` 屬性設置爲 `50px`。
