...
給 `#orange-character` 設置 `position` 爲 `absolute`，`top` 爲 `25%`，以及 `left` 爲 `40%`。

# --hints--

你的 `#orange-character` 選擇器應該有一個 `position` 屬性設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.position === 'absolute');
```

你的 `#orange-character` 選擇器應該有一個 `top` 屬性設置爲 `25%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.top === '25%');
```

你的 `#orange-character` 選擇器應該有一個 `left` 屬性設置爲 `40%`。
