...
現在移動 `#guitar-neck` 元素，`position` 爲 `absolute`，`top` 值爲 `45px`，以及 `left` 值爲 `200px`。

# --hints--

你的 `#guitar-neck` 選擇器應該有一個 `position` 屬性設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-neck')?.position === 'absolute');
```

你的 `#guitar-neck` 選擇器應該有一個 `top` 屬性設置爲 `45px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-neck')?.top === '45px');
```

你的 `#guitar-neck` 選擇器應該有一個 `left` 屬性設置爲 `200px`。
