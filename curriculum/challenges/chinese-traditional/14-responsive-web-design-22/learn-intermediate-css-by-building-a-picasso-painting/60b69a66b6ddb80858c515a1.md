...
你應該有一個 `#gray-mask` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask'));
```

你的 `#gray-mask` 選擇器應該將 `width` 屬性設置爲 `150px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.height === '150px');
```

你的 `#gray-mask` 選擇器應該將 `height` 屬性設置爲 `150px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.height === '150px')
```

你的 `#gray-mask` 選擇器應將 `background-color` 屬性設置爲 `rgb(167, 162, 117)`。
