...
現在你需要將條形圖標移動到位。 爲 `fa-bars` 類創建一個類選擇器。 將 `display` 設置爲 `block`，`margin-top` 設置爲 `30%`，`margin-left` 設置爲 `40%`。
