...
使用 id 選擇器爲 id `guitar-neck` 創建規則。 將 `width` 設置爲 `200px`，`height` 設置爲 `30px`，`background-color` 設置爲 `#D2691E`。

# --hints--

你應該創建一個 `#guitar-neck` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-neck'));
```

你的 `#guitar-neck` 選擇器應該有一個 `width` 屬性設置爲 `200px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-neck')?.width === '200px');
```

你的 `#guitar-neck` 選擇器應該有一個 `height` 屬性設置爲 `30px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-neck')?.height === '30px');
```

你的 `#guitar-neck` 選擇器應該有一個 `background-color` 屬性設置爲 `#D2691E`。
