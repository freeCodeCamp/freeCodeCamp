...
要生成連續的 ludic 數字，請創建一個從 2 開始遞增的整數數組。
...
<ul>
  ...
  <li>將數組中的第一項作爲下一個盧迪數，就是<span style='color:blue;font-weight:bold'>3</span>.</li>
  <li>從數組中刪除所有下標爲 <strong>3 的倍數</strong>的項。</li>
...
<ul>
  ...
  <li>從數組中刪除所有下標爲<strong>5的倍數</strong>的項 (包括第一個)。</li>
...
<ul>
  ...
  <li>從數組中刪除所有下標爲<strong>7的倍數</strong>的項 (包括第一個)。</li>
...
<ul>
  <li><big><b> ... </b></big></li>
  <li>將現有數組中的第一項作爲下一個盧迪數，就是<span style='color:blue;font-weight:bold'>L</span>.</li>
  <li>從數組中刪除所有下標爲<strong>L的倍數</strong>的項 (包括第一項)。</li>
  <li><big><b> ... </b></big></li>
</ul>

# --instructions--

編寫一個函數，該函數返回所有小於給定數字的盧迪數。
...
`ludic(5)` 返回數組 `[1, 2, 3, 5]`.

```js
assert.deepEqual(ludic(5), [1, 2, 3, 5]);
```

`ludic(20)` 返回數組 `[1, 2, 3, 5, 7, 11, 13, 17]`。

```js
assert.deepEqual(ludic(20), [1, 2, 3, 5, 7, 11, 13, 17]);
```

`ludic(26)`返回數組`[1, 2, 3, 5, 7, 11, 13, 17, 23, 25]`.
