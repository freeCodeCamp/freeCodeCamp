...
一個空子序列被認爲具有 \\( 0 \\)。因此，如果所有元素都是負數，則結果必須是空序列。

# --hints--

`maximumSubsequence` 應該是一個函數。

```js
assert(typeof maximumSubsequence == 'function');
```

`maximumSubsequence([ 1, 2, -1, 3, 10, -10 ])` 應該返回一個數組。

```js
assert(Array.isArray(maximumSubsequence([1, 2, -1, 3, 10, -10])));
```

`maximumSubsequence([ 1, 2, -1, 3, 10, -10 ])` 應該返回`[ 1, 2, -1, 3, 10 ]`。

```js
assert.deepEqual(maximumSubsequence([1, 2, -1, 3, 10, -10]), [1, 2, -1, 3, 10]);
```

`maximumSubsequence([ 0, 8, 10, -2, -4, -1, -5, -3 ])` 應該返回 `[ 0, 8, 10 ]`。
...
`maximumSubsequence([ 9, 9, -10, 1 ])` 應該返回 `[ 9, 9 ]`。

```js
assert.deepEqual(maximumSubsequence([9, 9, -10, 1]), [9, 9]);
```

`maximumSubsequence([ 7, 1, -5, -3, -8, 1 ])` 應該返回 `[ 7, 1 ]`。

```js
assert.deepEqual(maximumSubsequence([7, 1, -5, -3, -8, 1]), [7, 1]);
```

`maximumSubsequence([ -3, 6, -1, 4, -4, -6 ])` 應該返回 `[ 6, -1, 4 ]`。

```js
assert.deepEqual(maximumSubsequence([-3, 6, -1, 4, -4, -6]), [6, -1, 4]);
```

`maximumSubsequence([ -1, -2, 3, 5, 6, -2, -1, 4, -4, 2, -1 ])` 應該返回 `[ 3, 5, 6, -2, -1, 4 ]`。
