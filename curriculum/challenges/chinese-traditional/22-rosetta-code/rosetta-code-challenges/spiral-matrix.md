...
`spiralArray(3)` 應該返回一個數組。

```js
assert(Array.isArray(spiralArray(3)));
```

`spiralArray(3)` 應該返回 `[[0, 1, 2],[7, 8, 3],[6, 5, 4]]`。
...
`spiralArray(4)` 應該返回 `[[0, 1, 2, 3],[11, 12, 13, 4],[10, 15, 14, 5],[9, 8, 7, 6]]`。
...
`spiralArray(5)` 應該返回 `[[0, 1, 2, 3, 4],[15, 16, 17, 18, 5],[14, 23, 24, 19, 6],[13, 22, 21, 20, 7],[12, 11, 10, 9, 8]]`。
