...
`n` 以下有多少個數字可以表示爲質數平方、質數立方和質數四次方之和？

# --hints--

`primePowerTriples(50)` 應該返回一個數字。

```js
assert(typeof primePowerTriples(50) === 'number');
```

`primePowerTriples(50)` 應該返回 `4`。

```js
assert.strictEqual(primePowerTriples(50), 4);
```

`primePowerTriples(10035)` 應該返回 `684`。

```js
assert.strictEqual(primePowerTriples(10035), 684);
```

`primePowerTriples(500000)` 應該返回 `18899`。

```js
assert.strictEqual(primePowerTriples(500000), 18899);
```

`primePowerTriples(5000000)` 應該返回 `138932`。

```js
assert.strictEqual(primePowerTriples(5000000), 138932);
```

`primePowerTriples(50000000)` 應該返回 `1097343`。
