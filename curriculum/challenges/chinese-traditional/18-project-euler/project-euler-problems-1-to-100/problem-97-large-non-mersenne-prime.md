...
然而，在 2004 年發現了一個巨大的非梅森素數，它包含 2,357,207 位數字：$28433 × 2^{7830457} + 1$。

以 $multiplier × 2^{power} + 1$ 的形式找到該非梅森素數的最後十位數字。

# --hints--

`largeNonMersennePrime(19, 6833086)` 應該返回一個字符串。

```js
assert(typeof largeNonMersennePrime(19, 6833086) === 'string');
```

`largeNonMersennePrime(19, 6833086)` 應該返回字符串 `3637590017`。

```js
assert.strictEqual(largeNonMersennePrime(19, 6833086), '3637590017');
```

`largeNonMersennePrime(27, 7046834)` 應該返回字符串 `0130771969`。

```js
assert.strictEqual(largeNonMersennePrime(27, 7046834), '0130771969');
```

`largeNonMersennePrime(6679881, 6679881)` 應該返回字符串 `4455386113`。

```js
assert.strictEqual(largeNonMersennePrime(6679881, 6679881), '4455386113');
```

`largeNonMersennePrime(28433, 7830457)` 應該返回字符串 `8739992577`。
