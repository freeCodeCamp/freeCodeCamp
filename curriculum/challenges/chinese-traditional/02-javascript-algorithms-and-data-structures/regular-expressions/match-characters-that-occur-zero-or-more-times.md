...
你的正則表達式不應該匹配字符串 `He made a fair move. Screaming about it can't theylp you.` 中的任何字符。
...
你的正則表達式不應該匹配字符串 `Let them have it. It's not wise to upset a Wookiee.` 中的任何字符。
