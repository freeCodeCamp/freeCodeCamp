...
Нескінченний неперервний дріб можна записати як $\\sqrt{2} = \[1; (2)]$, де (2) вказує на те, що 2 повторюється *до нескінченності*. Подібним чином, $\\sqrt{23} = \[4; (1, 3, 1, 8)]$. Виявляється, що послідовність часткових значень неперервних дробів для квадратних коренів надає найкраще раціональне наближення. Розглянемо наближення для $\\sqrt{2}$.
...
Сума цифр у чисельнику десятого наближення дорівнює $1 + 4 + 5 + 7 = 17$.

Знайдіть суму цифр у чисельнику `n`-го наближення неперервного дробу для `e`.
