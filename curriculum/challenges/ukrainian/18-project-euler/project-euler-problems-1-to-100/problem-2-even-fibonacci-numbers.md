...
Кожне нове значення у послідовності Фібоначчі дорівнює сумі двох попередніх значень. Якщо почати з 1 та 2, то першими 10 значеннями послідовності будуть:
...
Ваша функція повинна повертати парне значення.

```js
assert.equal(fiboEvenSum(10) % 2 === 0, true);
```

Ваша функція має підсумовувати парні значення послідовності Фібоначчі: `fiboEvenSum(8)` має повернути 10.
