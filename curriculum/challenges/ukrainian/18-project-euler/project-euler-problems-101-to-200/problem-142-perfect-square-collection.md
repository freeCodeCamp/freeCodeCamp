---
id: 5900f3fa1000cf542c50ff0d
title: 'Завдання 142: колекція повних квадратів'
challengeType: 1
forumTopicId: 301771
dashedName: problem-142-perfect-square-collection
---

# --description--

Знайдіть найменше значення $x + y + z$ з такими цілими числами $x > y > z > 0$, щоб $x + y$, $x − y$, $x + z$, $x − z$, $y + z$, $y − z$ були повними квадратами.

# --hints--

`perfectSquareCollection()` має повернути `1006193`.

```js
assert.strictEqual(perfectSquareCollection(), 1006193);
```

# --seed--

## --seed-contents--

```js
function perfectSquareCollection() {

  return true;
}

perfectSquareCollection();
```

# --solutions--

```js
// solution required
```
