---
id: 5900f3db1000cf542c50feed
title: 'Завдання 110: обернені числа Діофанта ІІ'
challengeType: 1
forumTopicId: 301735
dashedName: problem-110-diophantine-reciprocals-ii
---

# --description--

У даному рівнянні x, y та n є натуральними числами.

$$\frac{1}{x} + \frac{1}{y} = \frac{1}{n}$$

Можна довести, що за умови `n` = 1260 існує 113 різних розв’язків, і це найменше значення `n`, за якого загальна кількість різних розв’язків перевищує сотню.

Яке найменше значення `n`, за якого кількість розв’язків перевищує чотири мільйони?

**Примітка:** це складніша версія завдання №108, оскільки для нього потрібно використати грамотне розумове рішення.

# --hints--

`diophantineTwo()` має повернути `9350130049860600`.

```js
assert.strictEqual(diophantineTwo(), 9350130049860600);
```

# --seed--

## --seed-contents--

```js
function diophantineTwo() {

  return true;
}

diophantineTwo();
```

# --solutions--

```js
// solution required
```
