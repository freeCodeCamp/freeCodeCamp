...
Спершу в методі `render()` відобразіть елементи `input`, `button`, та `ul`. Коли елемент `input` змінюється, він повинен запускати метод `handleChange()`. До того ж елемент `input` має відтворити значення `input` зі стану компонента. При натисканні елемент `button` повинен запускати метод `submitMessage()`.
