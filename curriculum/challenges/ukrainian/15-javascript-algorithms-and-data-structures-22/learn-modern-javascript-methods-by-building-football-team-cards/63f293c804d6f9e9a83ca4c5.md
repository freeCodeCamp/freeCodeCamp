...
`e.target.value` represents the `value` property from the `playersDropdownList` element. У подальших кроках ви використовуватимете це значення, щоб відображати картки гравців на основі позиції, яку вони грають.
