...
Below the `team` property, add three new property key-value pairs.
...
Ваш об’єкт `myFavoriteFootballTeam` повинен мати властивість `year`.

```js
assert.property(myFavoriteFootballTeam, 'year');
```

Ваша властивість `year` має бути числом.

```js
assert.isNumber(myFavoriteFootballTeam.year);
```

Для властивості `year` має бути встановлено `1986`.

```js
assert.equal(myFavoriteFootballTeam.year, 1986);
```

Ваш об’єкт myFavoriteFootballTeam має мати властивість isWorldCupWinner.

```js
assert.property(myFavoriteFootballTeam, 'isWorldCupWinner');
```

Ваша властивість `isWorldCupWinner` має бути логічним значенням.

```js
assert.isBoolean(myFavoriteFootballTeam.isWorldCupWinner);
```

Для вашої властивості `isWorldCupWinner` має бути встановлено значення `true`.

```js
assert.equal(myFavoriteFootballTeam.isWorldCupWinner, true);
```

Ваш об’єкт `myFavoriteFootballTeam` повинен мати властивість `headCoach`.

```js
assert.property(myFavoriteFootballTeam, 'headCoach');
```

Ваше значення `headCoach` має бути об’єктом.

```js
assert.isObject(myFavoriteFootballTeam.headCoach)
```

Ваше значення `headCoach` має бути порожнім об’єктом.
