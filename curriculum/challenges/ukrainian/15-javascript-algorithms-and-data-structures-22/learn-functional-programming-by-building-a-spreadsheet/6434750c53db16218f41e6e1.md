...
Ви повинні передати `1` як перший аргумент до виклику `range()`.

```js
assert.match(code, /range\(\s*1/);
```

Ви повинні передати `99` як другий аргумент до виклику `range()`.
...
Ви повинні передати функцію зворотного виклику до `.forEach()`, використовуючи стрілковий синтаксис.
