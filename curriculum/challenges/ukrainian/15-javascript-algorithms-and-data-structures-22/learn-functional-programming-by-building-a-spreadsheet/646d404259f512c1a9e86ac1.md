...
Ви повинні оголосити змінну `regex` у функції `highPrecedence`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*\(?\s*str\s*\)?\s*=>\s*{\s*(?:const|let|var)\s+regex/);
```

Ви повинні використати `const`, щоб оголосити змінну `regex`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*\(?\s*str\s*\)?\s*=>\s*{\s*const\s+regex/);
```

Змінна `regex` має бути регулярним виразом.
