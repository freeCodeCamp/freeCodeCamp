...
Ви повинні використати `const`, щоб оголосити змінну `input`.
...
Ви повинні передати рядок `input` до методу `.createElement()`.
...
Отримайте доступ до властивості `ariaLabel` елемента `input`.

```js
assert.match(code, /input\.ariaLabel/);
```

Встановіть атрибут `aria-label` елемента `input` на `letter + number`.
