...
Ви повинні використати `const`, щоб оголосити змінну `str2`.
...
Ви повинні передати `str` як перший аргумент до виклику `infixEval`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*\(?\s*str\s*\)?\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/;?\s*const\s+str2\s*=\s*infixEval\(\s*str/);
```

Ви повинні передати `regex` як другий аргумент до виклику `infixEval`.
