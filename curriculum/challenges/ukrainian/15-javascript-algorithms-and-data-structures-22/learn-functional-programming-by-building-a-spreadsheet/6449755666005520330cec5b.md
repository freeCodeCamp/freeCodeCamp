...
Ви повинні оголосити змінну `element` у функції `update`.

```js
assert.match(code, /const\s+update\s*=\s*\(?\s*event\s*\)?\s*=>\s*\{\s*(?:var|let|const)\s+element/);
```

Ви повинні використати `const`, щоб оголосити змінну `element`.

```js
assert.match(code, /const\s+update\s*=\s*\(?\s*event\s*\)?\s*=>\s*\{\s*const\s+element/);
```

Ви повинні присвоїти властивість `target` аргумента `event` до змінної `element`.
