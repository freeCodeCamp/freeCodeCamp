...
У своєму шаблонному літералі створіть елемент `div` із класом `dessert-card`. У цьому `div` створіть елемент `h2` та надайте йому текст змінної `name`.
...
Ваші елементи `div` повинні мати клас `dessert-card`.
...
Ваш елемент `h2` повинен мати текст змінної `name`.

```js
assert.equal(document.querySelectorAll('h2')[0]?.textContent, 'Vanilla Cupcakes (6 Pack)');
```

Ваш елемент `h2` повинен бути усередині елемента `div`.
