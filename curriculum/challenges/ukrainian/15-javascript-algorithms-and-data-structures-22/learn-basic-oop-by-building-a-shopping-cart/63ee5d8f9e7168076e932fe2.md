...
Після свого елемента `h2` створіть два елементи `p`. Дайте першому `class` типу `dessert-price` та текст змінної `price` зі знаком долара перед ним. Give the second a `class` of `product-category`, and the text `"Category: "` followed by the value of the `category` variable.
