...
Однак ви не зможете перевірити рівність, оскільки `NaN` не дорівнює саме собі. Натомість ви можете використати метод `Number.isNaN()`, який повертає `true`, якщо аргументом є `NaN`.

Додайте функцію зворотного виклику до методу `.filter()`, що повертає `true`, якщо елемент не `NaN`.

# --hints--

Метод `.filter()` повинен мати функцію зворотного виклику, яка приймає `el` як аргумент.

```js
assert.match(calculate.toString(), /numbers\.filter\(\(?\s*el\s*\)?\s*=>|numbers\.filter\(function\s*\(?el\)\s*\{/)
```

Функція зворотного виклику повинна використовувати `!` та `Number.isNaN()`, щоб перевірити, чи `el` НЕ `NaN`.
