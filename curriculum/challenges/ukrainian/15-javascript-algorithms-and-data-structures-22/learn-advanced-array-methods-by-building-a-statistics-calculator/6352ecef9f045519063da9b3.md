...
Ваша функція `getMode` повинна мати змінну `mode`.

```js
assert.match(getMode.toString(), /mode\s*=/);
```

Функція `getMode` повинна використовувати метод `Object.keys()`, щоб отримати ключі об’єкта `counts` та присвоїти їх до змінної `mode`.
