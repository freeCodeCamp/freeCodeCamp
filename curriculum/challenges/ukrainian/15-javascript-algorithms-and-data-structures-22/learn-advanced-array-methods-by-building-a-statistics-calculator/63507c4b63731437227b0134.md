...
Метод `.map()` повинен мати функцію зворотного виклику, яка приймає аргумент `el`.

```js
assert.match(calculate.toString(), /array\.map\(\(?\s*el\s*\)?\s*=>|array\.map\(function\s*\(?el\)\s*\{/)
```

Функція зворотного виклику повинна використовувати конструктор `Number`, щоб перетворити `el` у число.
