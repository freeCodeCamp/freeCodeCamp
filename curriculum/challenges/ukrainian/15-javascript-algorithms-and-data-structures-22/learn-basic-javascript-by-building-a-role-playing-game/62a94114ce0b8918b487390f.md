...
Ваше сьоме значення `locations` повинне мати масив `button text` із трьома рядками `REPLAY?`.

```js
assert.deepEqual(locations[6]["button text"], ['REPLAY?', 'REPLAY?', 'REPLAY?']);
```

Ваше сьоме значення `locations` повинне мати масив `button functions` із трьома змінними `restart`.
