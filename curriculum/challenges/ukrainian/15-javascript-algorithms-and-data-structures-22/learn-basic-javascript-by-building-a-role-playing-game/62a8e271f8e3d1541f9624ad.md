...
Ви повинні використати крапкову нотацію, щоб отримати доступ до властивості `innerText` об'єкта `monsterName`.
...
Ви повинні використати крапкову нотацію, щоб отримати доступ до властивості `innerText` об'єкта `monsterHealthText`.

```js
assert.match(goFight.toString(), /monsterHealthText\.innerText/);
```

Ви повинні встановити властивість `innerText` об'єкта `monsterHealthText` на значення `health` поточного монстра. You can get the current monster with `monsters[fighting]` or you can access this value using the `monsterHealth` variable that you previously assigned.
