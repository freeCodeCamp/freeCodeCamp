...
Ваш новий об’єкт повинен мати властивість `name` зі значенням `fight`.

```js
assert.equal(locations[3].name, "fight");
```

Новий об’єкт повинен мати властивість `button text` зі значенням масиву з рядками `Attack`, `Dodge` та `Run`.

```js
assert.deepEqual(locations[3]["button text"], ["Attack", "Dodge", "Run"]);
```

Новий об’єкт повинен мати властивість `button functions` зі значенням масиву зі змінними `attack`, `dodge` та `goTown`.

```js
assert.deepEqual(locations[3]["button functions"], [attack, dodge, goTown]);
```

Ваш новий об’єкт повинен мати властивість `text` зі значенням `You are fighting a monster.`.
