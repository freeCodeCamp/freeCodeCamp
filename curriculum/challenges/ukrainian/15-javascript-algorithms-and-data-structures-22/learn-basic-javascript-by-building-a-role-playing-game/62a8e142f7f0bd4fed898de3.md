...
Додайте новий об’єкт в кінець масиву `locations`, дотримуючись тих самих властивостей, що й в інших об’єктів. Set `name` to `"fight"`, `"button text"` to an array with `"Attack"`, `"Dodge"`, and `"Run"`, `"button functions"` to an array with `attack`, `dodge`, and `goTown`, and `text` to `"You are fighting a monster."`.
