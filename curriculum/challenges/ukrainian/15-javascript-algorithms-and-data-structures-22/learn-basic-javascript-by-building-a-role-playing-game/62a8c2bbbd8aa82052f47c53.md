...
Встановіть `name` на `cave`. Встановіть `button text` на масив з рядками `Fight slime`, `Fight fanged beast` та `Go to town square`. Встановіть `button functions` на масив зі змінними `fightSlime`, `fightBeast` та `goTown`. Встановіть властивість `text` на `You enter the cave. You see some monsters.`.
...
Ваш третій об’єкт `locations` повинен мати властивість `name` зі значенням `cave`.

```js
assert.equal(locations[2].name, "cave");
```

Ваш третій об’єкт `locations` повинен мати властивість `button text`, яка є масивом.

```js
assert.isArray(locations[2]["button text"]);
```

Ваша властивість `button text` повинна мати рядкові значення `Fight slime`, `Fight fanged beast` та `Go to town square`.
...
Ваш третій об’єкт `locations` повинен мати властивість `button functions`, яка є масивом.

```js
assert.isArray(locations[2]["button functions"]);
```

Ваша властивість `button functions` повинна мати значення функції `fightSlime`, `fightBeast` та `goTown`.
...
Ваш третій об'єкт `locations` повинен мати властивість `text` зі значенням `You enter the cave. You see some monsters.`.
