...
Так само як і масив `locations`, ваш масив `weapons` зберігатиме об'єкти. Додайте чотири об'єкти до масиву `weapons`, кожен з двома властивостями: `name` та `power`. The first should have the `name` set to `"stick"` and the `power` set to `5`. The second should be `"dagger"` and `30`. The third, `"claw hammer"` and `50`. The fourth, `"sword"` and `100`.
