...
Ваша функція `update` повинна встановити `button1.innerText` на `Go to store`.

```js
assert.match(update.toString(), /button1\.innerText\s*=\s*('|")Go to store\1/);
```

Ваша функція `update` повинна встановити `button2.innerText` на `Go to cave`.

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*('|")Go to cave\1/);
```

Ваша функція `update` повинна встановити `button3.innerText` на `Fight dragon`.

```js
assert.match(update.toString(), /button3\.innerText\s*=\s*('|")Fight dragon\1/);
```

Ваша функція `update` повинна встановити `button1.onclick` на `goStore`.

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*goStore/);
```

Ваша функція `update` повинна встановити `button2.onclick` на `goCave`.

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*goCave/);
```

Ваша функція `update` повинна встановити `button3.onclick` на `fightDragon`.
