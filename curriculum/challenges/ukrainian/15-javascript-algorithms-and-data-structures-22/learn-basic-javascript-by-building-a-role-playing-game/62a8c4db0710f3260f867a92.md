...
Що робити, якщо у гравця недостатньо золота, щоб купити здоров'я? You should use an `if` statement to check if the player has enough gold to buy health.
...
Ваша функція `buyHealth` повинна мати інструкцію `if`.
...
Весь ваш код `buyHealth` повинен бути в інструкції `if`.
