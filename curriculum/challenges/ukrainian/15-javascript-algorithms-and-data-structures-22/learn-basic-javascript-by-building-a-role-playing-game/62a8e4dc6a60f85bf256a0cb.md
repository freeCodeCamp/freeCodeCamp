...
The `Math` object in JavaScript contains static properties and methods for mathematical constants and functions. One of those is `Math.random()`, which generates a random number from `0` (inclusive) to `1` (exclusive). Іншим є `Math.floor()`, яке заокруглює задане число до найближчого цілого числа.

З їх використанням ви можете згенерувати випадкове число в межах діапазону. For example, this generates a random number between `1` and `5`: `Math.floor(Math.random() * 5) + 1;`.
