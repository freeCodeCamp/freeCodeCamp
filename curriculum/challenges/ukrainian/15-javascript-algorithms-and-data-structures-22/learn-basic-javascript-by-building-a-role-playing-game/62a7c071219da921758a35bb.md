...
In your `goTown` function, update your `button` elements' `innerText` properties to be `Go to store`, `Go to cave`, and `Fight dragon`. Оновіть значення `onclick` на `goStore`, `goCave` та `fightDragon`. Зрештою, оновіть властивість тексту `innerText` на `You are in the town square. You see a sign that says Store.`.
