...
In your `goTown` function, change your `button` elements' `innerText` properties to be `Go to store`, `Go to cave`, and `Fight dragon`. Оновіть значення `onclick` на `goStore`, `goCave` та `fightDragon`.
