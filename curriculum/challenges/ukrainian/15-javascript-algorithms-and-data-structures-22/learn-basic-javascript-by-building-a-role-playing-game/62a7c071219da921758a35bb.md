...
У своїй функції `goTown` оновіть властивості `innerText` елементів `button` на `Go to store`, `Go to cave` та `Fight dragon`. Оновіть значення `onclick` на `goStore`, `goCave` та `fightDragon`. Зрештою, оновіть властивість тексту `innerText` на `You are in the town square. You see a sign that says Store.`.
