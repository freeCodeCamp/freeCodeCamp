...
You should add `"You sold a "` to `text.innerText`. Інтервал важливий.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

You should add the value of `currentWeapon` to the `"You sold a "` string. Використайте оператор конкатенації, щоб зробити це в тому самому рядку.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a\s\1\s*\+\s*currentWeapon/);
```

You should add the string `"."` to the value of `currentWeapon`. Використайте оператор конкатенації, щоб зробити це в тому самому рядку.
