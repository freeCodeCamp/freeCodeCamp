...
Якщо гравець натискає на кнопку `Go to store`, потрібно, щоб змінювались кнопки та текст. Remove the code inside the `goStore` function and add a line that updates the text of `button1` to say `"Buy 10 health (10 gold)"`.
