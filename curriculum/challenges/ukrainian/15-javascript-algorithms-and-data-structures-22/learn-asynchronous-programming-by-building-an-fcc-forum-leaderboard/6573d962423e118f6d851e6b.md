...
Потім отримайте елемент `#posts-container`, використавши метод `getElementById()`. Призначте його до нової константи під назвою `postsContainer`.

# --hints--

Використайте `const`, щоб оголосити змінну `postsContainer`.

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

Використайте `document.getElementById()`, щоб отримати елемент `#posts-container`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

Призначте елемент `#posts-container` до змінної `postsContainer`.
