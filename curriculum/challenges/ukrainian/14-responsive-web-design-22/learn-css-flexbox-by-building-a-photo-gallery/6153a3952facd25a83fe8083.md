...
Створіть медіазапит для екранів, менших за `800px` завширшки. В цьому медіазапиті створіть правило `#gallery img` та встановіть властивість `width` на `50%`. Це перетворить вашу галерею в двоколонковий макет.
...
Ваш новий запит `@media` повинен мати `max-width` зі значенням `800px`, ось так: `@media (max-width: 800px)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === '(max-width: 800px)');
```

Ваш запит `@media` повинен мати правило `#gallery img`.
...
Ваше правило `#gallery img` повинне мати властивість `width` зі значенням `50%`.
