...
В межах другого `tr` додайте елемент `th` із текстом `Expenses Annual anticipated expenses, such as payroll.`. Обгорніть цей текст, окрім `Expenses`, в елемент `span`, що має `class` зі значенням `description`.
