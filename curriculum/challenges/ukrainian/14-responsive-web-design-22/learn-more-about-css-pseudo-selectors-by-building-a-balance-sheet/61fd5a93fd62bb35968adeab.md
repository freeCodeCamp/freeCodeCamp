...
Налаштуйте свій HTML з допомогою елементів `DOCTYPE`, `html`, `head` та `body`. Надайте елементу `head` відповідні елементи `meta` для `charset` та `viewport`, елемент `title` з відповідною назвою та елемент `link` для своєї таблиці стилів.
