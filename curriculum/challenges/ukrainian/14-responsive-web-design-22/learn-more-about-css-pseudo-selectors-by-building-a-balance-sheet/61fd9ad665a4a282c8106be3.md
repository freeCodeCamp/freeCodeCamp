...
Створіть елемент `tr` в `thead`. Всередину додайте `td` та три елементи `th`. В межах кожного елемента `th` додайте елемент `span`, що має `class` зі значенням `sr-only` та такий текст, за порядком: `2019`, `2020` та `2021`.
