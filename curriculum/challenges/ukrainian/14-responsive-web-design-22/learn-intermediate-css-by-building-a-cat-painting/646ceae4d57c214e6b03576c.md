...
Створіть ще один елемент `div` з класом `cat-right-inner-ear` всередині елемента `.cat-right-ear`.
...
Ви повинні мати елемент `div` всередині елемента `.cat-right-ear`.

```js
assert(document.querySelectorAll('.cat-right-ear div').length === 1);
```

Елемент `div` повинен мати клас `cat-right-inner-ear`.
