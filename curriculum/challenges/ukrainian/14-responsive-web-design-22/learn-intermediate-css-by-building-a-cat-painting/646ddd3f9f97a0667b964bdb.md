...
Створіть ще один елемент `div` з класом `cat-left-inner-eye` всередині елемента `.cat-left-eye`.
...
Ви повинні мати елемент `div` всередині елемента `.cat-left-eye`.

```js
assert(document.querySelectorAll('.cat-left-eye div').length === 1);
```

Елемент `div` повинен мати клас `cat-left-inner-eye`.
