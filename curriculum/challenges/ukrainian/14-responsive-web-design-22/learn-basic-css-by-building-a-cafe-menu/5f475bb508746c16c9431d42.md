...
Щоб зображення поводились як вбудовані елементи (на рівні блоків), створіть селектор типу `img`, використайте значення `block` для властивості `display` та використайте застосовані значення `margin-left` та `margin-right`, щоб зцентрувати горизонтально.
