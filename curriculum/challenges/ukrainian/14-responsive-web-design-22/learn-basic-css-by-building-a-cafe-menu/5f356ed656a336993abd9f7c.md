...
Next, you want to center the `#menu` horizontally. Це можна зробити, встановивши властивості `margin-left` та `margin-right` на `auto`. Думайте про поле як невидимий простір навколо елементу. Using these two margin properties, center the `#menu` element within the `body` element.
