...
Так ближче, але ціна не залишилась в правій частині. Це тому що елементи `inline-block` займають лише ширину свого вмісту. Щоб їх розтягнути, додайте властивість `width` до селекторів класу `flavor` та `price` зі значенням `50%` кожен.
