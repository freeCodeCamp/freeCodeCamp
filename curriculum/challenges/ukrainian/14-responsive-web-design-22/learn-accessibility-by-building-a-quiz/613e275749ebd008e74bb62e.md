...
Currently, the `img` is assuming its default size, which is too large. Належним способом виміряйте зображення, використовуючи `id` як селектор та встановлюючи `width` на `max(100px, 18vw)`.
