...
Створіть третій запит `@media` для `only screen` з `max-width` зі значенням `550px`. Всередині створіть селектор `.hero-title` з `font-size` зі значенням `6rem`, селектор `.hero-subtitle, .author, .quote, .list-title` з `font-size` зі значенням `1.8rem`, селектор `.social-icons` з `font-size` зі значенням `2rem` та селектор `.text` з `font-size` зі значенням `1.6rem`.
