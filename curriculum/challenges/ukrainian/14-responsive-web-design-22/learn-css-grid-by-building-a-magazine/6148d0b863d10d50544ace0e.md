...
Інший текст зміщено з місця. Перемістіть його на місце, надавши селектору `.first-paragraph::first-letter` властивість `float` зі значенням `left` та властивість `margin-right` зі значенням `1rem`.
