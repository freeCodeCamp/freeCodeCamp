...
Тепер у вас є щось, що нагадує будівлю. Ви готові створити свою першу змінну. Декларації змінних починаються з двох дефісів (`-`) та отримують ім’я і значення, ось так: `--variable-name: value;`. В класі `.bb1` створіть назву змінної `--building-color1` та надайте їй значення `#999`.
