...
Ваш селектор `#black-hat` повинен мати властивість `position` зі значенням `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.position === 'absolute');
```

Ваш селектор `#black-hat` повинен мати властивість `top` зі значенням `-150px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.top === '-150px');
```

Ваш селектор `#black-hat` повинен мати властивість `left` зі значенням `0`.
