...
Ви повинні мати два нових елементи `div` в межах елемента `.characters`.

```js
assert(document.querySelectorAll('.characters > div')?.length === 4);
```

Ваші два нових елементи `div` повинні мати `class` зі значенням `blue`.
...
Ваш перший новий `div` повинен мати `id` зі значенням `blue-left`.

```js
assert(document.querySelectorAll('.characters > div')?.[2]?.getAttribute('id') === 'blue-left');
```

Ваш другий новий `div` повинен мати `id` зі значенням `blue-right`.
