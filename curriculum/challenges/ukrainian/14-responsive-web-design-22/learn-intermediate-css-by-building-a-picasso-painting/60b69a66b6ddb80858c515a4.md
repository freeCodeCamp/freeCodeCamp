...
Ваш селектор `#white-paper` повинен мати властивість `position` зі значенням `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#white-paper')?.position === 'absolute');
```

Ваш селектор `#white-paper` повинен мати властивість `top` зі значенням `250px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#white-paper')?.top === '250px');
```

Ваш селектор `#white-paper` повинен мати властивість `left` зі значенням `-150px`.
