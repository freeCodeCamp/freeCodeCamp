...
Ви повинні мати два елементи `div` в межах елемента `#eyes-div`.

```js
assert(document.querySelectorAll('#eyes-div > div')?.length === 2);
```

Перший новий `div` повинен мати `class` зі значенням `eyes left`.
...
Другий новий `div` повинен мати `class` зі значенням `eyes right`.
