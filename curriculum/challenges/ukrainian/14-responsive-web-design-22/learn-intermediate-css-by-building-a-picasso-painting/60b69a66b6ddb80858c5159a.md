...
Ви повинні мати два елементи `div` в межах елемента `#gray-mask`.

```js
assert(document.querySelectorAll('#gray-mask > div')?.length === 2);
```

Ваш перший новий елемент `div` повинен мати `class` зі значенням `eyes left`.
...
Ваш другий новий елемент `div` повинен мати `class` зі значенням `eyes right`.
