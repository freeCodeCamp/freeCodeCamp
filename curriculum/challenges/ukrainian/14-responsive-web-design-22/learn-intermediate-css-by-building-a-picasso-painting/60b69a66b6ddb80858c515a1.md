...
Ви повинні мати селектор `#gray-mask`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask'));
```

Ваш селектор `#gray-mask` повинен мати властивість `width` зі значенням `150px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.height === '150px');
```

Ваш селектор `#gray-mask` повинен мати властивість `height` зі значенням `150px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.height === '150px')
```

Ваш селектор `#gray-mask` повинен мати властивість `background-color` зі значенням `rgb(167, 162, 117)`.
