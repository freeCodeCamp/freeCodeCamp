...
1. Крім того, навігаційна панель має містити елементи link (`a`) класу `nav-link`. Має бути по одному для кожного елемента класу `main-section`
1. Елемент `header` в `#navbar` має йти перед елементами посилання (`a`) у навігаційній панелі
1. Кожен елемент класу `nav-link` повинен містити текст, який відповідає тексту `header` у межах кожної `section` (наприклад, якщо у вас є секція/заголовок «Hello world», ваша навігаційна панель повинна містити елемент, який має текст «Hello world»)
1. Якщо натиснути на елемент навігаційної панелі, то сторінка має перейти до відповідної секції елемента `#main-doc` (наприклад, якщо ви натискаєте на елемент `.nav-link`, що містить текст «Hello world», то сторінка скеровує до елементу `section` з тим ідентифікатором, і містить відповідний заголовок)
...
Всі елементи `.nav-link` повинні бути елементами anchor (`a`).
...
Елемент `header` в `#navbar` повинен бути перед елементами link (`a`), також в `#navbar`.
