...
1. Можна побачити елемент `main` з відповідним ідентифікатором `id="main-doc"`, який має основний вміст сторінки (технічна документація)
1. В межах елементу `#main-doc` можна побачити декілька елементів `section`, кожен з яких має клас `main-section`. Їх має бути щонайменше п'ять
1. Перший елемент кожної `.main-section` повинен бути елементом `header`, який має текст, що описує тему секції.
1. Кожен елемент `section` класу `main-section` також має містити `id`, що відповідає тексту кожного `header`, вміщеного у ньому. Будь-які прогалини слід замінити знаками підкреслення (наприклад, розділ із заголовком «JavaScript and Java» має відповідний `id="JavaScript_and_Java"`)
...
1. Крім того, навігаційна панель має містити елементи посилання (`a`) класу `nav-link`. Має бути по одному для кожного елемента класу `main-section`
1. Елемент `header` в `#navbar` має йти перед елементами посилання (`a`) у навігаційній панелі
1. Кожен елемент класу `nav-link` повинен містити текст, який відповідає тексту заголовка `header` у межах кожної `section` (наприклад, якщо у вас є розділ/заголовок «Hello world», ваша навігаційна панель повинна містити елемент, який має текст «Hello world»)
1. Якщо натиснути на елемент навігаційної панелі, то сторінка має перейти до відповідного розділу елемента `#main-doc` (наприклад, якщо ви натискаєте на елемент `.nav-link`, що містить текст «Hello world», то сторінка скеровує до елементу `section` з тим ідентифікатором, і містить відповідний заголовок)
1. На пристроях звичайного розміру (портативний комп'ютер, настільний комп'ютер) елемент з `id="navbar"` повинен відображатись ліворуч на екрані та завжди бути видимим для користувача
...
Ви повинні мати елемент `main` з `main-doc` `id`.
...
Ви повинні мати щонайменше п'ять елементів `.main-section`, які наслідують `#main-doc`.
...
Жоден з елементів `header` не має бути порожнім.
...
Кожна `.main-section` повинна мати `id`, що відповідає тексту першого дочірнього елементу з заміною прогалин у тексті на знаки підкреслення (`_`) для ідентифікаторів.
...
Ви повинні мати щонайменше п'ять елементів `code`, що наслідують елементи `.main-section`.
...
Ви повинні мати щонайменше п'ять елементів `li`, що наслідують елементи `.main-section`.
...
Ви повинні мати елемент `nav` з `navbar` `id`.
...
Ваш `#navbar` повинен мати один елемент `header` у своїх межах.
...
Всі елементи `.nav-link` повинні бути елементами анкор (`a`).
...
Елемент `header` в `#navbar` повинен бути перед елементами посилання (`a`), також в `#navbar`.
...
Кожен `.nav-link` повинен мати атрибут `href`, що посилає на відповідну `.main-section` (наприклад, якщо натиснути на елемент `.nav-link`, що містить текст «Hello world», сторінка скерує на елемент `section` з таким ідентифікатором).
