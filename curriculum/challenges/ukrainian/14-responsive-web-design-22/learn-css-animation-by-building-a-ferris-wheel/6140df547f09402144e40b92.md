...
Властивість `animation-name` використовують, щоб прив’язати правило `@keyframes` з CSS-селектором. Значення цієї властивості повинне відповідати назві правила `@keyframes`. Надайте селектору `.wheel` властивість `animation-name` зі значенням `wheel`.
