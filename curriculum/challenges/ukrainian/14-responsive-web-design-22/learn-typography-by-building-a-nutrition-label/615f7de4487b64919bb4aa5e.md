...
Після останнього `.divider` створіть новий елемент `p` з текстом `Cholesterol 0mg 0%`. Обгорніть текст `Cholesterol` в елемент `span` та надайте цьому елементу `span` `class` зі значенням `bold`. Обгорніть текст `0%` в інший елемент `span` з `class` зі значенням `bold`. Наприкінці вкладіть елемент `span` (`Cholesterol`) та текст `0mg` у додатковий елемент `span` для вирівнювання.
