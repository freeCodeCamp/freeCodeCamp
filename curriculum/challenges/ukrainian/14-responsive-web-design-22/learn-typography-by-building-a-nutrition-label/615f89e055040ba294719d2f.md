...
Створіть інший елемент `p` під вашим великим роздільником. Надайте елементу `p` текст `Vitamin D 2mcg 10%`. Використайте `span`, щоб вирівняти `10%` за правим краєм, але не робіть його жирним.
...
Ваш елемент `span` повинен мати `class` зі значенням `right`. Пам’ятайте: не застосовуйте жирний шрифт.
