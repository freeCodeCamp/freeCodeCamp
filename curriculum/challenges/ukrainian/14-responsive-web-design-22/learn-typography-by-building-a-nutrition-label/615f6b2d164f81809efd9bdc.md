...
Після останнього елемента `.divider` створіть елемент `p` та надайте йому текст `Total Fat 8g 10%`. Wrap the text `Total Fat` in a `span` element with the `class` of `bold`. Wrap the text `10%` in another `span` element with the `class` of `bold`. Наприкінці вкладіть елемент `span` (`Total Fat`) та текст `8g` у додатковий елемент `span` для вирівнювання.
