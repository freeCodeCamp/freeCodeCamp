...
Стилізація `float` призводить до того, що новий елемент `p` знаходиться за кордонами етикетки. Використайте наявний елемент `.divider` як приклад, щоб додати новий роздільник після елемента `p`.
