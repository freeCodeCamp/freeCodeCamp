...
Під елементом `.dbl-indent` додайте новий елемент `p` з текстом `Includes 10g Added Sugars 20%`. Ваш новий елемент `p` також повинен мати подвійний відступ та не мати нижнього кордону. Використайте `span`, щоб зробити `20%` жирним та вирівняним за правим краєм.

Потім створіть ще один роздільник після цього елемента `p`.

# --hints--

Ви повинні створити нові елементи `p` та `div` в кінці елемента `.daily-value.sm-text`.
...
Ваш новий елемент `p` повинен мати атрибут `class` зі значенням `dbl-indent no-divider`.
...
Ваш елемент `span` повинен мати атрибут `class` зі значенням `bold right`.
