...
Створіть новий `div` під своїм елементом `.lg` та надайте йому атрибут `class` зі значенням `calories-info`.

# --hints--

Ви повинні створити інший елемент `div`.

```js
assert(document.querySelectorAll('div')?.length === 4);
```

Ваш новий `div` повинен мати атрибут `class` зі значенням `calories-info`.

```js
assert(document.querySelector('.label')?.lastElementChild?.classList?.contains('calories-info'));
```

Ваш новий `div` повинен йти після елемента `.lg`.
