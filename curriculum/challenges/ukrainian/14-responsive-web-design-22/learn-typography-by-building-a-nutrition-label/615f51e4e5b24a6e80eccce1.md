...
В елементі `.calories-info` створіть елемент `p`. Надайте цьому елементу `p` атрибут `class` зі значенням `bold sm-text` та текст `Amount per serving`.

# --hints--

Ви повинні створити новий елемент `p` в елементі `.calories-info`.

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.localName === 'p');
```

Ваш новий елемент `p` повинен мати атрибут `class` зі значенням `bold sm-text`.
...
Ваш новий елемент `p` повинен мати текст `Amount per serving`.
