...
Створіть ще один елемент `p` після останнього `.divider` та надайте йому текст `Total Sugars 12g`. Призначте цьому елементу `p` значення `class`, які необхідні для того, щоб зсунути його та видалити нижній край. Потім створіть ще один `.divider` під новим елементом `p`.
