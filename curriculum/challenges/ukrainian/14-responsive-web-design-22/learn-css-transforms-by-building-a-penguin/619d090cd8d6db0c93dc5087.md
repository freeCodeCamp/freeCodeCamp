...
Під елементом `.chin` додайте два елементи `div`, кожен з `class` зі значенням `eye`. Крім того, першому елементу `.eye` надайте `class` зі значенням `left`, а другому елементу `.eye` надайте `class` зі значенням `right`.
