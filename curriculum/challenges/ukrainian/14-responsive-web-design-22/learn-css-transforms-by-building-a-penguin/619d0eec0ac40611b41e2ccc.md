...
Під елементом `.eye.right` додайте два елементи `div`, кожен з `class` зі значенням `blush`. Крім того, першому елементу `.blush` надайте `class` зі значенням `left`, а другому елементу `.blush` надайте `class` зі значенням `right`.
