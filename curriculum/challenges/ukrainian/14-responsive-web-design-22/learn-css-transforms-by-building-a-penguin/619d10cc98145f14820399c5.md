...
Під елементом `.blush.right` додайте два елементи `div`, кожен з `class` зі значенням `beak`. Крім того, першому елементу `.beak` надайте `class` зі значенням `top`, а другому елементу `.beak` надайте `class` зі значенням `bottom`.
