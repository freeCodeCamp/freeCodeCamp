...
Вкладіть ще один цикл `for` в межах циклу, створеного в попередньому кроці, щоб ітерувати над множиною з трьох елементів, починаючи з `col_start`. Для створення множини знову скористайтеся функцією `range()`.
