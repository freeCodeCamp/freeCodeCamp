...
Якщо рекурсивний виклик `self.solver()` повертає `True`, то судоку розв’язано.

Якщо рекурсивний виклик повертає `True`, поверніть `True` з методу.

# --hints--

Внутрішній блок `if` повинен містити `if self.solver():`.
...
`if self.solver():` має містити `return True`.
