...
Створіть цикл `for`, який починається з `row_start` і закінчується одразу перед `row_start + 3`. Для створення множини можна використати функцію `range()`. Наприклад, якщо `row_start` дорівнює `3`, то цикл ітеруватиме над числами `3`, `4` та `5`.
