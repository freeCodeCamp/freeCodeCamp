...
Якщо `self.solver()` повертає `False`, то припущення (`guess`) призвело до нерозв’язного судоку.

Скасуйте припущення за межами внутрішнього блоку `if`, повернувши клітинці значення `0`.

# --hints--

За межами внутрішнього блоку `if` має бути `self.board[row][col] = 0`.
