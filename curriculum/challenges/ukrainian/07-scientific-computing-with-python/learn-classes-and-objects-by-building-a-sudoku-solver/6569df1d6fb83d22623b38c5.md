...
Наступним кроком потрібно перевірити, чи вказане число (`num`) вже присутнє в поточній клітинці квадрата 3×3.

Створіть інструкцію `if` в межах внутрішнього циклу `for`, яка перевіряє, чи поточна клітинка в `self.board` дорівнює `num`.
