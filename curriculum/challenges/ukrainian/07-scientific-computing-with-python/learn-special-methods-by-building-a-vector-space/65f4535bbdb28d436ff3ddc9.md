...
Як ви можете бачити з вихідних даних, `__dict__` містить значення атрибутів екземпляра. Instead of explicitly adding the squares of `self.x` and `self.y`, you are going to iterate over the values stored in `__dict__` to calculate the value of the norm.
...
Поверніть `sum(val**2 for val in self.__dict__.values())**0.5` з методу `norm()`.
