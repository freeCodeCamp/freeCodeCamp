...
The `__ne__` method is called under the hood when the `!=` operator is used. Визначте метод `__ne__` з двома параметрами: `self` та `other`. From your new method, return the opposite of `self == other`.
