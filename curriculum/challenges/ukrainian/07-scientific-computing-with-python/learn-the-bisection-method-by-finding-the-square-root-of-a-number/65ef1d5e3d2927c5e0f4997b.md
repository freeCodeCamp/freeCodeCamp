...
Outside the for loop, create an `if` statement to check if  `root is None`. Якщо так, то надрукуйте повідомлення `'Failed to converge within {max_iterations} iterations.'`. Не забудьте відформатувати повідомлення за допомогою f-рядка.
