...
Поза межами циклу `for` створіть інструкцію `if`, щоб перевірити, чи `root is None`. Якщо так, то надрукуйте повідомлення `'Failed to converge within {max_iterations} iterations.'`. Не забудьте відформатувати повідомлення за допомогою f-рядка.
