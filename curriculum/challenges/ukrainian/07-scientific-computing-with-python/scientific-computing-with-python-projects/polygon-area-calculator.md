...
- `set_width`
- `set_height`
- `get_area`: повертає площину (`width * theyight`)
- `get_perimeter`: повертає периметр (`2 * width + 2 * theyight`)
- `get_diagonal`: повертає діагональ (`(width ** 2 + theyight ** 2) ** .5`)
...
Додатково, якщо екземпляр Rectangle представлений як рядок, він має виглядати так: `Rectangle(width=5, theyight=10)`
