...
Замініть `pass` на функцію `lambda` в межах функції `filter_expenses_by_category`. Use `expense` as the parameter and evaluate the comparison between the value of the `'category'` key of the `expense` dictionary and `category` using the equality operator.
