...
Настав час перемістити код із функції `move()` до функції `make_allowed_move()`. Перемістіть код, вкладений в першій інструкції `if` (за винятком першого виклику `print()`), до нової функції. Зверніть увагу на відступи. Не забудьте видалити ключове слово `pass`.

# --hints--

Перемістіть код, вкладений в першій інструкції `if` (за винятком першого виклику `print()`), до нової функції.
