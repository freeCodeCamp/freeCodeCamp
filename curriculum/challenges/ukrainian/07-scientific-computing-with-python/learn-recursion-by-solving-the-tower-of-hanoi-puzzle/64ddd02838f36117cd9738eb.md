...
Додайте коментар `display our progress` поза блоком `else` та надрукуйте вміст списків, щоб перевірити, чи все працює.

# --hints--

Використайте наданий текст, щоб додати коментар поза блоком `else`.

```js
({ test: () => assert.match(code, /rods\s*\[\s*source\s*\]\s*\.append\s*\(\s*rods\s*\[\s*target\s*\]\s*\.pop\s*\(\s*\)\s*\)\s+#\s*display\sour\sprogress/) })
```

Надрукуйте об’єкт `rods` поза блоком `else` після коментаря.
