...
Інший випадок, коли потрібно перемістити диск з `source` до `target`: список джерела **непорожній**, а верхній диск в `source` менший за верхній диск в `target`.

Додайте інструкцію `elif`, щоб перевірити цю умову. Потім встановіть змінну `forward` на `True`, якщо умова виконана.

# --hints--

Ви повинні мати інструкцію `elif`, яка перевіряє, чи `rods[source]` непорожній та `rods[source][-1] < rods[target][-1]`.
