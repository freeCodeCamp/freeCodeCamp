...
Перш ніж додати останній елемент до цільового стрижня, додайте коментар `move the nth disk from source to target`.

# --hints--

Додайте коментар з наданим текстом перед `rods[target].append(rods[source].pop())`.
