...
Замініть `print(char == ' ')` на інструкцію `if`, яка запускається, якщо `char == ' '`. Do not use parentheses to enclose the `if` condition and remember to include the final colon.
