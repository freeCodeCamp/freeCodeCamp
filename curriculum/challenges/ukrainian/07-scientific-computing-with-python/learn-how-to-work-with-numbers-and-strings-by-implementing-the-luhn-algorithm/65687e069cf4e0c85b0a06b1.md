...
Додайте `number` до `sum_of_even_digits` поза межами інструкції `if`. Також видаліть виклик `print`.

# --hints--

Цикл `for` повинен містити `sum_of_even_digits += number`.
...
Цикл `for` не повинен містити `print(number)`.
...
Інструкція `if` не повинна містити `sum_of_even_digits += number`.
