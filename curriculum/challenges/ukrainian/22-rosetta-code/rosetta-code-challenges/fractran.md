...
FRACTRAN — це повна за Тюрінгом езотерична мова програмування, яку винайшов математик Джон Гортон Конвей.

Програма на FRACTRAN — це впорядкований список додатних дробів $P = (f_1, f_2, \\ldots, f_m)$ з початковим введенням цілого числа $n$.

Програма запускається шляхом оновлення цілого числа $n$ наступним чином:

<ul>
  <li>для першого дробу $f_i$ в списку, для якого $nf_i$ є цілим числом, замініть $n$ на $nf_i$ ;</li>
  <li>повторюйте це правило, поки жоден дріб в списку не видасть цілого числа при множенні на $n$, потім припиніть.</li>
</ul>

Конвей надав програму на FRACTRAN для цілих чисел:
...
Починаючи з $n=2$, ця програма FRACTRAN змінить $n$ на $15=2\\times (\\frac{15}{2})$, тоді $825=15\\times (\\frac{55}{1})$, генеруючи наступну послідовність цілих чисел:
...
Напишіть функцію, яка приймає програму на fractran як рядковий параметр та повертає перші 10 чисел програми як масив. Якщо результат не містить 10 чисел, поверніть числа як є.
