...
Алгоритм Лемпеля-Зіва-Велча (LZW) забезпечує стиснення даних без втрат.

# --instructions--

Напишіть функцію, яка приймає два параметри. Першим параметром є булеве значення, де `true` позначає стиснення, а `false` позначає розширення. Другим параметром є рядок або масив, який потрібно обробити. Якщо це рядок для стиснення, поверніть масив чисел. Якщо це масив чисел для розширення, поверніть рядок.
