...
Для малих значень $n$ інколи використовуються Грецькі цифрові префікси для окремої назви кожної серії.
...
Союзні послідовності можна створити там, де початкові значення змінюються: Послідовність Люка сумує два попередніх значення, так само як серія Фібоначчі за умови $n= 2$, але використовує рядок $\[2, 1]$ як початкові значення.

# --instructions--

Напишіть функцію для створення послідовностей $n$-крокових чисел Фібоначчі та послідовностей Люка. Першим параметром буде $n$. Другим параметром буде кількість елементів, які потрібно повернути. Третій параметр вказуватиме, що виводити: послідовність Фібоначчі чи послідовність Люка. If the parameter is `"f"` then return the Fibonacci sequence and if it is `"l"`, then return the Lucas sequence. Послідовності необхідно повернути у вигляді масиву.
