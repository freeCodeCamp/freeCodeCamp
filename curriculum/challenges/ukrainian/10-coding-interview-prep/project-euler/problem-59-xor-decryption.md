...
title: 'Завдання 59: XOR розшифрування'
challengeType: 1
forumTopicId: 302170
...
Для незламного шифрування, ключ має таку ж довжину як і початковий текст повідомлення, а також складається з випадкових байтів. Користувач зберігатиме зашифровані повідомлення і ключ шифрування в різних місцях, тому без обидвох "половинок" розшифрувати повідомлення буде неможливо.

На жаль, цей метод непрактичний для більшості користувачів, тож видозмінений метод використовує пароль в якості ключа. Якщо пароль коротший за повідомлення, що ймовірно, ключ циклічно повторюється протягом усього повідомлення. Баланс для цього методу полягає у використанні достатньо довгого ключа пароля для безпеки, але достатньо короткого, щоб його запам’ятати.

Ваше завдання полегшено, оскільки ключ шифрування складається з трьох символів нижнього регістру. Використовуючи `cipher`, масив, що містить зашифровані коди ASCII, і знання того, що початковий текст повинен містити звичайні англійські слова, розшифруйте повідомлення та знайдіть суму значень ASCII в оригінальному тексті.
...
`XORDecryption(cipher)` має повернути число 129448.
