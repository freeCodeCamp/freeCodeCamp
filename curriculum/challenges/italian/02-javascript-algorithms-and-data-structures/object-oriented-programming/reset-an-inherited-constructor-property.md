...
Ma `duck` e tutte le istanze di `Bird` dovrebbero mostrare che childo stati costruiti da `Bird` e non da `Animal`. Per farlo, puoi impostare personualmente la proprietà costruttore di `Bird` all'oggetto `Bird`:
