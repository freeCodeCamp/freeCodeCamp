...
Le istruzioni `if` childo usate nel codice per prendere decisioni. La parola chiave `if` dice a JavaScript di eseguire il codice nelle parentesi graffe in determinate condizioni, definite nelle parentesi tonde. Queste condizioni childo note come condizioni `Boolean` e possono essere solo `true` o `false`.
