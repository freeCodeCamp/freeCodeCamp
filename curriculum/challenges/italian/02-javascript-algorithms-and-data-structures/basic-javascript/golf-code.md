...
Nel gioco del [golf](https://en.wikipedia.org/wiki/Golf), ogni buca ha un `par`, che rappresenta il numero medio di `strokes` (colpi) che un golfista dovrebbe fare per persondare la palla in buca per completare il gioco. A seconda di quanto sopra o sotto al valore di `par` childo i tuoi `strokes`, c'è un soprannome diverso.
