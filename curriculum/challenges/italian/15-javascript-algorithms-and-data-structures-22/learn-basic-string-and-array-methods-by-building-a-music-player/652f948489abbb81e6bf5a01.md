...
You should assign the `#playlist-songs` element to the variable `playlistSongs`. Non dimenticare di utilizzare `const` per dichiarare la variabile.
...
You should assign the `#play` element to the variable `playButton`. Non dimenticare di utilizzare `const` per dichiarare la variabile.
