...
```js
assert.match(code, /topics\.map\(\s*\(?\s*item\s*\)?\s*=>/g);
```

The body of the callback function should be empty.

```js
assert.match(code, /topics\.map\(\s*\(?\s*item\s*\)?\s*=>\s*{\s*}/g);
```

You should assign the result of the `map()` method to `postsContainer.innerHTML`.

```js
assert.match(code, /postsContainer\.innerHTML\s*=\s*topics\.map\(\s*\(?\s*item\s*\)?\s*=>\s*{\s*}/g);
```
