...
Infine, aggiorna la proprietà `display` dell'oggetto `style` della variabile `cartContainer` in un altro ternario che controlla se `isCartShowing` è vero. If it is, set the `display` property to `"block"`, otherwise set it to `"none"`.
