...
Nel template literal, crea un elemento `div` con la classe `dessert-card`. In quel `div`, crea un elemento `h2` e dagli il testo della variabile `name`.
...
L'elemento `div` dovrebbe avere la classe `dessert-card`.
...
L'elemento `h2` dovrebbe avere il testo della variabile `name`.

```js
assert.equal(document.querySelectorAll('h2')[0]?.textContent, 'Vanilla Cupcakes (6 Pack)');
```

L'elemento `h2` dovrebbe essere all'interno dell'elemento `div`.
