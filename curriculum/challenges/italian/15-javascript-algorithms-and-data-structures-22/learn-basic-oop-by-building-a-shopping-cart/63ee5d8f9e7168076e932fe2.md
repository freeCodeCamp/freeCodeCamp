...
Dopo l'elemento `h2`, crea due elementi `p`. Dai al primo una `class` con il valore `dessert-price` e il testo della variabile `price` con il segno del dollaro all'inizio. Give the second a `class` of `product-category`, and the text `"Category: "` followed by the value of the `category` variable.
