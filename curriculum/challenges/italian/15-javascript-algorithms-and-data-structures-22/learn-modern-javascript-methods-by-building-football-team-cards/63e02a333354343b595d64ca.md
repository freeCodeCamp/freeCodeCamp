...
Il prossimo passo è accedere alla chiave chiamata `sport` dell'oggetto `myFavoriteFootballTeam` e assegnarla a una nuova variabile dichiarata con `const` chiamata `sport`.
...
Dovresti assegnare il valore della chiave `sport` dell'oggetto `myFavoriteFootballTeam` alla variabile `sport`.
