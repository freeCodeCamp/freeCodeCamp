...
Un'istruzione `switch` è una struttura di controllo che verifica il valore di un'espressione rispetto a più clausole `case` e può includere una clausola `default` opzionale.
...
L'istruzione `switch` dovrebbe usare `e.target.value` come espressione.

```js
assert.match(code, /switch\s*\(\s*e\.target\.value\s*\)/);
```

L'istruzione `switch` dovrebbe essere vuota.
