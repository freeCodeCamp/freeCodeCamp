...
Il parametro `list` sarà il risultato di un selettore, che restituirà un `NodeList`. Un `NodeList` è un oggetto array-like che contiene gli elementi che corrispondono al selettore.

Dovrai iterare su questi elementi, quindi inizia creando un loop `for`. L'iteratore `i` dovrebbe partire da `0`, continuare finché è inferiore alla lunghezza della lista, e incrementare di `1` a ogni iterazione.

# --hints--

La funzione `getCaloriesFromInputs` dovrebbe avere un loop `for`.
...
Il loop `for` dovrebbe avere una condizione che verifica se `i` è minore della lunghezza di `list`.
