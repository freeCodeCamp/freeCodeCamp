...
`Math.abs()` è un metodo integrato di JavaScript che restituisce il valore assoluto di un numero. Nel testo dello `span` racchiudi il riferimento a `remainingCalories` in `Math.abs()` per assicurarti che il valore sia positivo.
