...
Il parametro `fn` sarà il nome di una funzione, come `SUM`. Aggiorna `apply` per restituire implicitamente la funzione trovata nella proprietà `fn` dell'oggetto `spreadsheetFunctions`.

Ricorda che `fn` potrebbe non essere in minuscolo, quindi dovrai convertirla in una stringa minuscola.
