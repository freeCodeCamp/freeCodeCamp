...
Dichiara una funzione `apply` che prende dei parametri `fn` e `args`.
...
Dovresti usare `const` per dichiarare la variabile `apply`.
...
Alla variabile `apply` dovrebbe essere assegnata una funzione freccia.
...
La funzione `apply` dovrebbe avere `fn` come primo parametro.
...
La funzione `apply` dovrebbe avere `args` come secondo parametro.
...
La funzione `apply` dovrebbe essere vuota.
