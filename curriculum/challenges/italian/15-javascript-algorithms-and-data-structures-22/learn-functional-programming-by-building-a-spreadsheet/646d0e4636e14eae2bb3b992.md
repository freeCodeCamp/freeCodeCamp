...
Ora aggiorna la funzione più interna nella catena `addCharacters` per restituire implicitamente il risultato della chiamata `charRange()` con `character1` e `character2` come argomenti.

# --hints--

La funzione più interna dovrebbe usare un return implicito.
...
La funzione più interna dovrebbe restituire il risultato della chiamata `charRange()`.
