...
La funzione callback ha bisogno di quattro parametri. `match`, `arg1`, `operator` e `arg2`.

Non utilizzerai il parametro `match`, quindi ricordati di prefissarlo.

# --hints--

La funzione callback dovrebbe avere `match` come primo parametro.

```js
assert.match(code, /const\s+infixEval\s*=\s*\(\s*str\s*,\s*regex\s*\)\s*=>\s*str\.replace\(\s*regex\s*,\s*\(\s*_?match/);
```

Il parametro `match` dovrebbe essere prefissato con un trattino basso.

```js
assert.match(code, /const\s+infixEval\s*=\s*\(\s*str\s*,\s*regex\s*\)\s*=>\s*str\.replace\(\s*regex\s*,\s*\(\s*_match/);
```

La funzione callback dovrebbe avere `arg1` come secondo parametro.

```js
assert.match(code, /const\s+infixEval\s*=\s*\(\s*str\s*,\s*regex\s*\)\s*=>\s*str\.replace\(\s*regex\s*,\s*\(\s*_match\s*,\s*arg1/);
```

La funzione callback dovrebbe avere `operator` come terzo parametro.

```js
assert.match(code, /const\s+infixEval\s*=\s*\(\s*str\s*,\s*regex\s*\)\s*=>\s*str\.replace\(\s*regex\s*,\s*\(\s*_match\s*,\s*arg1\s*,\s*operator/);
```

La funzione callback dovrebbe avere `arg2` come quarto parametro.
...
La funzione callback dovrebbe essere ancora vuota.
