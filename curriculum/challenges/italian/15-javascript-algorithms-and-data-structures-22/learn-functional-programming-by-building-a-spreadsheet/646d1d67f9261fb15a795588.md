...
Dichiara una variabile `rangeExpanded` e assegnale il risultato della chiamata del metodo `.replace()` sul parametro `x`. Passa la variabile `rangeRegex` come argomento.
...
Dovresti usare `const` per dichiarare la variabile `rangeExpanded`.
...
Dovresti assegnare il risultato della chiamata `.replace()` su `x` alla variabile `rangeExpanded`.
