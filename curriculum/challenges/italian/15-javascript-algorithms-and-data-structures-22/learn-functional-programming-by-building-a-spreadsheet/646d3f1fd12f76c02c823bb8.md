...
Hai un piccolo bug. `arg1` e `arg2` sono stringhe, non numeri. `infixToFunction['+']("1", "2")` restituisce `12`, che non è matematicamente corretto.

Racchiudi ogni argomento di `infixToFunction[operator]` in una chiamata `parseFloat()`.

# --hints--

Dovresti inserire `arg1` in una chiamata `parseFloat()`.
...
Dovresti inserire `arg2` in una chiamata `parseFloat()`.
