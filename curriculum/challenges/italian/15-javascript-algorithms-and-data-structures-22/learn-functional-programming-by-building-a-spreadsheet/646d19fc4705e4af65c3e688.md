...
La funzione `addCharacters` alla fine restituisce un intervallo di caratteri. Vuoi che restituisca un array di id delle celle. Concatena il metodo `.map()` alla chiamata `charRange()`. Non passare ancora una funzione callback.

# --hints--

Dovresti concatenare il metodo `.map()` alla chiamata `charRange()`.
...
Non dovresti passare nulla alla chiamata `.map()`.
