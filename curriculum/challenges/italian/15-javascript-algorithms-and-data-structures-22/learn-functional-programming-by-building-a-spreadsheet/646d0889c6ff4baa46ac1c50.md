...
Nella funzione `elemValue`, dichiara una funzione chiamata `inner` che prende un parametro `character`.

Quindi, restituisci la funzione `inner`.
...
Dovresti usare `const` per dichiarare la variabile `inner`.
...
La variabile `inner` dovrebbe essere una funzione freccia.
...
La funzione `inner` dovrebbe avere `character` come unico parametro.
...
La funzione `inner` dovrebbe essere vuota.
...
Dovresti restituire esplicitamente la funzione `inner`.
