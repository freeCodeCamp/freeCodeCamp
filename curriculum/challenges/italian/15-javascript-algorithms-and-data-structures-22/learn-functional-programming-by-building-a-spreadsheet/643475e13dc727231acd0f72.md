...
Per l'accessibilità, imposta l'attributo `aria-label` sullo stesso valore dell'attributo `id`.
...
Dovresti accedere alla proprietà `ariaLabel` dell'elemento `input`.

```js
assert.match(code, /input\.ariaLabel/);
```

Dovresti impostare l'attributo `aria-label` dell'elemento `input` su `letter + number`.
