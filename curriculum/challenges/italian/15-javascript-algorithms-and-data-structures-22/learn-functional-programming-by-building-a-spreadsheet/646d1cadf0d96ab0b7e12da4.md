...
Because `elemValue` returns a function, your `addCharacters` function ultimately returns an array of function references. Vuoi che il metodo `.map()` esegua la funzione interna della funzione `elemValue`, il che significa che devi chiamare `elemValue` invece di farvi riferimento. Passa `num` come argomento alla funzione `elemValue`.

<!-- TODO: Explain further? -->

# --hints--

Dovresti chiamare `elemValue()` nel metodo `.map()`.
...
Dovresti passare `num` alla chiamata `elemValue()`.
