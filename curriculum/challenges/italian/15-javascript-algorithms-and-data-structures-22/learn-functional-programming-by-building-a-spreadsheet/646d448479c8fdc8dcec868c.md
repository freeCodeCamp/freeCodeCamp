...
Dichiara una variabile `functionCall` e assegnale questa espressione regolare: `/([a-z]*)\(([0-9., ]*)\)(?!.*\()/i`

Questa espressione cercherà chiamate di funzioni come `sum(1, 4)`.
...
Dovresti usare `const` per dichiarare la variabile `functionCall`.
