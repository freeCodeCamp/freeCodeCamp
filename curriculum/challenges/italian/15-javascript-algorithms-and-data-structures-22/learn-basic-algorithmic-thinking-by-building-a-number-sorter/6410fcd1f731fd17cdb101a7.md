...
All'interno del loop `for` annidato, aggiungi una chiamata di `console.log()` che controlla i valori di `array`, `array[j]` e `array[minIndex]` a ogni iterazione. Puoi fare clic sul pulsante `Sort` per vedere come l'algoritmo sta percorrendo l'array.

Then write an `if` statement that checks if the value at `array[j]` is smaller than the value at `array[minIndex]`. Se lo è, imposta `minIndex` su `j`.

# --hints--

Dovresti avere una chiamata di `console.log()` all'interno del loop `for` annidato.
...
Dovresti avere un'istruzione `if` dopo la chiamata di `console.log()`.
...
Dovresti avere una condizione nell'istruzione `if` che verifica se `array[j]` è minore di `array[minIndex]`.
...
L'istruzione `if` dovrebbe impostare `minIndex` su `j`.
