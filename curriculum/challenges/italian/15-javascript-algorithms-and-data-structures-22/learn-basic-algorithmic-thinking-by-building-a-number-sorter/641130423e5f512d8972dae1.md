...
Per risolvere questo problema, puoi passare una funzione callback al metodo `.sort()`. La funzione callback richiede due argomenti - per la tua, usa `a` e `b`. Lascia la funzione vuota per ora.

# --hints--

Dovresti passare una funzione callback al metodo `.sort()`. Ricorda di utilizzare la sintassi freccia.

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*\(?.*\)?\s*=>/);
```

La funzione callback dovrebbe prendere due argomenti, `a` e `b`.
