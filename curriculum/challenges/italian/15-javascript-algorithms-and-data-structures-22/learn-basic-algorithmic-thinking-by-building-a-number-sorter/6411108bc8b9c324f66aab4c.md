...
Dichiara una variabile `currValue` e assegnale il valore a `i`. Quindi, dichiara una variabile `j` e assegnale `i - 1`. La variabile `j` dovrebbe essere riassegnabile.
...
Dovresti assegnare a `currValue` il valore a `i`.
