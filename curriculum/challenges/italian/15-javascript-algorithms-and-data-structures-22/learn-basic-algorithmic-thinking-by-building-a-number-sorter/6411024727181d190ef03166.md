...
Dopo il loop `for` annidato, hai trovato il valore più piccolo. Devi scambiarlo con il valore corrente.

Proprio come hai fatto con bubble sort, usa una variabile `temp` per estrarre il valore a `i`, poi scambia i valori a `i` e `minIndex`.

# --hints--

Dopo il loop `for` annidato, dovresti dichiarare una variabile `temp`.
