...
Per ottenere il risultato corretto, devi verificare se l'elemento corrente è più grande dell'elemento successivo. Puoi farlo accedendo ad `array` con `j` e `j+1`.

Crea una condizione `if` che controlla se l'elemento corrente è più grande dell'elemento successivo.

# --hints--

Non dovresti rimuovere l'istruzione `console.log`.
...
Dovresti usare un'istruzione `if` nel loop annidato.
...
L'istruzione `if` dovrebbe verificare se `array[j]` è maggiore di `array[j+1]`.
