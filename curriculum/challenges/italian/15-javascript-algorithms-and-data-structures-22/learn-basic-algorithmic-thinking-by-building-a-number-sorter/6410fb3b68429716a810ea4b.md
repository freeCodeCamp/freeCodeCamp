...
Selection sort si basa sul tracciamento dell'indice del valore più piccolo nell'array. Dichiara una variabile `minIndex` e impostala su `i` - questo assicura che se il valore attuale è il più piccolo, sarà scambiato con sé stesso e non sarà spostato. Dovrai essere in grado di riassegnare il valore di `minIndex` mentre iteri sull'array.

Poi, scrivi un altro loop `for` usando `j` come iteratore. Questo loop deve iniziare dall'indice dopo `i` e iterare sul resto dell'array.
...
Il loop `for` dovrebbe inizializzare `j` a `i + 1`.
...
Il loop `for` dovrebbe iterare sul resto dell'array.
...
Il loop `for` dovrebbe incrementare `j`.
