...
L'algoritmo insertion sort avvia l'ordinamento all'inizio della lista, il che significa che il primo elemento è già ordinato. Tenendo questo aspetto a mente, crea un loop `for` che inizia dal secondo elemento dell'array - dovrebbe comunque iterare sul resto dell'array.
...
Il loop `for` dovrebbe inizializzare `i` a `1`.
