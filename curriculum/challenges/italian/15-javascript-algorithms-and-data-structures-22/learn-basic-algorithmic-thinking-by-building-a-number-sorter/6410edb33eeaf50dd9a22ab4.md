...
Quando la condizione di `if` è vera, devi scambiare i due elementi, spostando l'elemento più grande verso la fine dell'array.

Per farlo, dichiara una variabile `temp` e assegnale il valore di `array[j]`. Quindi assegna a `array[j]` il valore di `array[j + 1]`. Infine, assegna a `array[j + 1]` il valore di `temp`.

# --hints--

All'interno dell'istruzione `if` dovresti dichiarare una variabile `temp`.
