...
Ora hai un piccolo bug. Quando provi ad accedere a una proprietà di un oggetto e la proprietà non esiste, ottieni `undefined`. Questo significa che se il dessert non è già presente nell'oggetto `totalCountPerProduct`, finisci per cercare di aggiungere `1` a `undefined`, il cui risultato è `NaN`.

Per risolvere questo problema, puoi usare l'operatore `||` per impostare il valore a `0` se non esiste. Racchiudi tra parentesi il `totalCountPerProduct[data.id]` di destra e aggiungi `|| 0` alla fine dell'espressione.

# --hints--

Dovresti racchiudere tra parentesi il `totalCountPerProduct[data.id]` di destra.
...
Dovresti usare `0` come valore di ripiego.
...
Dovresti comunque aggiungere `1` al valore.
