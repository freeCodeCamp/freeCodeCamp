...
In your `goTown` function, update your `button` elements' `innerText` properties to be `Go to store`, `Go to cave`, and `Fight dragon`. Aggiorna le proprietà `onclick` rispettivamente in `goStore`, `goCave` e `fightDragon`. Infine, aggiorna il testo della proprietà `innerText` in `You are in the town square. You see a sign that says Store.`.
