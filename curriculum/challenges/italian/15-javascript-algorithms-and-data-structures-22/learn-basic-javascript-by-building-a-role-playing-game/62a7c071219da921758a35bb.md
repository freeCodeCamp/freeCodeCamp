...
Nella funzione `goTown`, aggiorna le proprietà `innerText` degli elementi `button` in `Go to store`, `Go to cave` e `Fight dragon`. Aggiorna le proprietà `onclick` rispettivamente in `goStore`, `goCave` e `fightDragon`. Infine, aggiorna il testo della proprietà `innerText` in `You are in the town square. You see a sign that says Store.`.
