...
In your `goTown` function, change your `button` elements' `innerText` properties to be `Go to store`, `Go to cave`, and `Fight dragon`. Aggiorna le proprietà `onclick` rispettivamente in `goStore`, `goCave` e `fightDragon`.
