...
Aggiungi un nuovo oggetto alla fine dell'array `locations`, seguendo la stessa struttura degli altri oggetti. Set `name` to `"kill monster"`, set `"button text"` to an array with three `"Go to town square"` strings, set `"button functions"` to an array with three `goTown` variables, and set `text` to `"The monster screams Arg! as it dies. You gain experience points and find gold."`.
