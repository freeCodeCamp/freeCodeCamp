...
Il giocatore dovrebbe colpire se `Math.random() > .2` **o** se la vita del giocatore è minore di 20.

Alla fine dell'istruzione `return`, usa l'operatore <dfn>logico or</dfn>, `||`, e verifica se `health` è minore di `20`.

L'operatore logico or userà il primo valore nel caso sia truthy – ovvero qualsiasi cosa eccetto `NaN`, `null`, `0`, `-0`, `0n`, `""` e `false`. Altrimenti, userà il secondo valore.
