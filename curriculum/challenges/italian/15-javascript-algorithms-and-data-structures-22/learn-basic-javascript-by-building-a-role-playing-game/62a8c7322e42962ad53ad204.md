...
Proprio come l'array `locations`, l'array `weapons` conterrà oggetti. Aggiungi quattro oggetti all'array `weapons`, ognuno con due proprietà: `name` e `power`. The first should have the `name` set to `"stick"` and the `power` set to `5`. The second should be `"dagger"` and `30`. The third, `"claw hammer"` and `50`. The fourth, `"sword"` and `100`.
