...
Le <dfn>proprietà</dfn> degli oggetti sono scritte come coppie `key: value`, dove `key` è il nome della proprietà (o chiave), e `value` è il valore che la proprietà contiene. For example, here is an object with a key of `name` set to `"Quincy Larson"`.
