...
Le <dfn>proprietà</dfn> degli oggetti sono scritte come coppie `key: value`, dove `key` è il nome della proprietà (o chiave), e `value` è il valore che la proprietà contiene. Ad esempio, ecco un oggetto con una chiave `name` impostata sul valore `Quincy Larson`.
