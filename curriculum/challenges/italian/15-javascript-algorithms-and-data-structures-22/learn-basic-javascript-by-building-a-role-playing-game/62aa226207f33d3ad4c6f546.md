...
Usa l'operatore `+=` per aggiungere `Your [weapon] breaks.`, con una spazio prima di `Your`, alla fine di `text.innerText`. Sostituisci `[weapon]` con l'ultimo elemento dell'array `inventory` usando `inventory.pop()`, che rimuoverà l'ultimo elemento dell'array E lo restituirà così da apparire nella stringa.
...
Dovresti aggiungere `Your`, con una spazio prima e dopo, a `text.innerText`.
...
Dovresti aggiungere `breaks.`, con uno spazio davanti, alla fine della stringa.
