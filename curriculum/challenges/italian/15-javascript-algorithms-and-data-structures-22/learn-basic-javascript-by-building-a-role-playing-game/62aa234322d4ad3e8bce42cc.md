...
Non vogliamo che l'unica arma di un giocatore si rompa. The logical AND operator checks if two statements are true.

Use the <dfn>logical AND</dfn> operator `&&` to add a second condition to your `if` statement. L'arma del giocatore dovrebbe rompersi solo se `inventory.length` non è uguale (`!==`) a uno.
