...
Use `querySelector()` to get the other two `button` elements using their `id`s: `button2` and `button3`. Conservali in variabili chiamate `button2` e `button3`. Ricorda di usare `const`.
...
La variabile `button2` dovrebbe avere il valore dell'elemento `#button2`.
...
La variabile `button3` dovrebbe avere il valore dell'elemento `#button3`.
