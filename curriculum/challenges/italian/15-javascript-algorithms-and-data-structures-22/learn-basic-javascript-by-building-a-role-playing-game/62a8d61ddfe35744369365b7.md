...
You should add `"You sold a "` to `text.innerText`. La spaziatura è importante.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

You should add the value of `currentWeapon` to the `"You sold a "` string. Usa l'operatore di concatenazione per farlo sulla stessa riga.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a\s\1\s*\+\s*currentWeapon/);
```

You should add the string `"."` to the value of `currentWeapon`. Usa l'operatore di concatenazione per farlo sulla stessa riga.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1\s*\+\s*currentWeapon\s+\+\s+('|")\.\2/);
```

`text.innerText` dovrebbe essere aggiornato con la stringa appropriata.
