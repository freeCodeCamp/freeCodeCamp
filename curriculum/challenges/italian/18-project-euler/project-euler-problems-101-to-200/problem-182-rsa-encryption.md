...
Generare due primi distinti `p` e `q`. Calcolare `n=p*q` e `φ=(p-1)(q-1)`. Trovare un numero intero `e`, `1 < e < φ`, in modo che `MCD(e,φ) = 1`
...
Per decifrare il testo, è necessaria la seguente procedura: calcolare `d` in modo che `ed=1 mod φ`, quindi per ogni messaggio cifrato `c`, calcolare m=c<sup>d</sup> mod n.
