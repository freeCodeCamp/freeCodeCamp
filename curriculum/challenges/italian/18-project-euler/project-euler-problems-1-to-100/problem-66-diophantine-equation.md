...
Considera l'equazione quadratica diofantina nella forma:
...
Trova il valore di D ≤ `n` in soluzioni minimali di `x` per cui viene ottenuto il valore più grande di `x`.
