...
6174 è un numero notevole; se mettiamo le sue cifre in ordine crescente e sottraiamo quel numero dal numero che si ottiene quando si ordinano le cifre in ordine decrescente, otteniamo $7641 - 1467 = 6174$.
...
Possiamo considerare la routine di Kaprekar per altre basi e numeri di cifre. Purtroppo, non è garantito che una costante di Kaprekar esista in tutti i casi; inoltre la routine può finire in un ciclo per alcuni numeri di input o la costante a cui la routine arriva può essere diversa per numeri di input diversi. Tuttavia, può essere dimostrato che per 5 cifre e una base $b = 6t + 3 ≠ 9$, esiste una costante di Kaprekar.
...
Nota che possiamo definire $sb(i)$ per tutti gli interi $i &lt; b^5$. Se $i$ scritto in base $b$ richiede meno di 5 cifre, il numero è riempito con zeri iniziali fino a quando non abbiamo 5 cifre prima di applicare la routine Kaprekar.
