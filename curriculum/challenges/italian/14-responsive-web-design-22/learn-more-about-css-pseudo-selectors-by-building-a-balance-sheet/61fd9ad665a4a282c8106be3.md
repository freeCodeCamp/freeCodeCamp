...
All'interno dell'elemento `thead`, crea un elemento `tr`. Aggiungi al suo interno un elemento `td` e tre elementi `th`. Dentro ogni elemento `th`, aggiungi un elemento `span` con l'attributo `class` impostato su `sr-only` e il seguente testo, in ordine: `2019`, `2020` e `2021`.
