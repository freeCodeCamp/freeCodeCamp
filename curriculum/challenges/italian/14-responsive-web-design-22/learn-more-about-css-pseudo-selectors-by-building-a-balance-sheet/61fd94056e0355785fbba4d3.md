...
All'interno del secondo `tr`, aggiungi un elemento `th` con il testo `Expenses Annual anticipated expenses, such as payroll.`. Racchiudi il testo, eccetto `Expenses`, all'interno di un elemento `span` con l'attributo `class` impostato su `description`.
