...
Il selettore `.cat-right-eye` dovrebbe avere una proprietà `border-radius` impostata al `60%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.borderRadius === '60%')
```

Il selettore `.cat-right-eye` dovrebbe avere una proprietà `transform` con il valore `rotate(-25deg)`.
