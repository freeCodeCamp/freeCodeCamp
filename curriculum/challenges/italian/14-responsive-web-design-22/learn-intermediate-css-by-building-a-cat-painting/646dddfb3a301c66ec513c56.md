...
All'interno dell'elemento `.cat-right-eye` crea un altro elemento `div` con la classe `cat-right-inner-eye`.
...
Dovresti avere un elemento `div` all'interno dell'elemento `.cat-right-eye`.

```js
assert(document.querySelectorAll('.cat-right-eye div').length === 1);
```

L'elemento `div` dovrebbe avere la classe `cat-right-inner-eye`.
