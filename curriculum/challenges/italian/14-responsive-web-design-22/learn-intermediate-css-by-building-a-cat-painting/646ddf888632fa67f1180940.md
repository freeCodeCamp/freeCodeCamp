...
Il selettore `.cat-left-eye` dovrebbe avere una proprietà `position` con il valore `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.position === 'absolute')
```

Il selettore `.cat-left-eye` dovrebbe avere una proprietà `top` impostata su `54px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.top === '54px')
```

Il selettore `.cat-left-eye` dovrebbe avere una proprietà `left` impostata su `39px`.
