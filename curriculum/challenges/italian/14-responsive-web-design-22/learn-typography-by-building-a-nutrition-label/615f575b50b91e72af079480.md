...
Crea un nuovo selettore `.left-container p` impostando i margini superiore e inferiore a `-5px` e i margini di sinistra e di destra a `-2px`. Inoltre imposta `font-size` a `2em` e `font-weight` a `700`.
