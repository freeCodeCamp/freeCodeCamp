...
Crea un nuovo elemento `div` sotto il tuo elemento `header` e dagli un attributo `class` impostato a `divider lg`.
...
Il tuo nuovo elemento `div` dovrebbe venire dopo l'elemento `header`.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'div');
```

Il nuovo elemento `div` dovrebbe avere l'attributo `class` con valore di `divider lg`.
