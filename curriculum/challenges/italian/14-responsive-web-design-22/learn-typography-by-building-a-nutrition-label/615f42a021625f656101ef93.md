...
Inserisci il testo `2/3 cup (55g)` in un elemento `span` e assegnagli un attributo `class` con il valore `right`.
...
Il nuovo elemento `span` dovrebbe avere l'attributo `class` con il valore `right`.

```js
assert(document.querySelector('span')?.classList?.contains('right'));
```

L'elemento `.right` dovrebbe avere il testo `2/3 cup (55g)`.
