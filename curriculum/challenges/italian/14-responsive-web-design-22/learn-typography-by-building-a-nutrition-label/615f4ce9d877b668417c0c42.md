...
Inserisci tutto ciò che c'è all'interno dell'elemento `.label` in un nuovo elemento `header`.
...
L'elemento `header` dovrebbe essere all'interno dell'elemento `.label`.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

Gli elementi `h1`, `div` e `p` dovrebbero essere dentro il nuovo elemento `header`.
