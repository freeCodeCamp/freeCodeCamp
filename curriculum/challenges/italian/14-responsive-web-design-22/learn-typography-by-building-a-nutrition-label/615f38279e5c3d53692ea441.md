...
Annida gli elementi `h1` e `p` in un elemento `div`. Assegna a quel `div` un attributo `class` impostato su `label`.
...
Il nuovo elemento `div` dovrebbe avere l'attributo `class` impostato su `label`.
