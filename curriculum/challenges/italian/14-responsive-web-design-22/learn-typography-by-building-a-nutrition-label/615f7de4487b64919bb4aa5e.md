...
Il primo elemento `span` `Cholesterol` dovrebbe avere l'attributo `class` con il valore `bold`.
...
Il primo elemento `span` dovrebbe racchiudere il testo `Cholesterol 0mg`.
...
Il secondo elemento `span` dovrebbe avere l'attributo `class` con il valore `bold`.
