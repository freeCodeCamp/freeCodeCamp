...
All’interno dell’elemento `.image-wrapper`, per il primo elemento `img` imposta `src` su `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, `alt` su `image of the quote machine project`, `class` su `image-1`, `loading` su `lazy`, `width` a `600` e `height` a `400`.
