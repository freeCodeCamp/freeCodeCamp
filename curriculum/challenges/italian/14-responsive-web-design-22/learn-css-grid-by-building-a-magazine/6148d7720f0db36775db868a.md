...
Avrai bisogno di una colonna per il testo e di una colonna per le immagini. Assegna al selettore `.text-with-images` una proprietà `grid-template-columns` impostata su `1fr 2fr`. Imposta anche la proprietà `column-gap` su `3rem` per creare più spazio tra le colonne.
