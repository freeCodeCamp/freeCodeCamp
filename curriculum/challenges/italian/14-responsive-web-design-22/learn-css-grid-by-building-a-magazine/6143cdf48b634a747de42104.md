...
All’interno dell’elemento `.image-wrapper`, per il terzo elemento `img` imposta `src` su `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`, `alt` su `four people working on code`, `loading` su `lazy`, `class` su `image-3`, `width` a `600` e `height` a `400`.
