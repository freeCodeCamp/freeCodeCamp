...
È il momento di impostare lo stile del terzo elemento `section`. Nota che ha i valori `text` e `text-with-images` per l'attributo `class`, il che significa che sta già ereditando gli stili dalla regola `.text`.
