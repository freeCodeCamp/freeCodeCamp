...
L'altro testo ora è fuori posto. Mettilo in posizione dando al selettore `.first-paragraph::first-letter` la proprietà `float` impostata su `left` e una proprietà `margin-right` impostata su `1rem`.
