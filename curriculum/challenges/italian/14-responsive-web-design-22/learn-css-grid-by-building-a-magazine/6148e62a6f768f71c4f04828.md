...
La proprietà `place-items` può essere utilizzata per impostare contemporaneamente i valori `align-items` e `justify-items`. La proprietà `place-items` accetta uno o due valori. Se viene fornito un solo valore, viene utilizzato per entrambe le proprietà `align-items` e `justify-items`. Se sono forniti due valori, il primo valore è utilizzato per la proprietà `align-items` e il secondo valore è utilizzato per la proprietà `justify-items`.
