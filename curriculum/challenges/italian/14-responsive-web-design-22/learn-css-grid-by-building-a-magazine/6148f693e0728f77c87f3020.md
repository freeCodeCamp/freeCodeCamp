...
La nuova regola `@media` dovrebbe avere un selettore `.hero-title`, un selettore `.hero-subtitle, .author, .quote, .list-header`, un selettore `.social-icons` e un selettore `.text`. I selettori dovrebbero essere in questo ordine.
...
Il selettore `.hero-subtitle, .author, .quote, .list-header` dovrebbe avere una proprietà `font-size` impostata a `1.8rem`.
