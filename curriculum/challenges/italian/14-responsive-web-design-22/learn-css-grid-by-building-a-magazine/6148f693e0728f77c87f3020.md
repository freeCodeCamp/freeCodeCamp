...
Crea una terza `@media` query per `only screen` con una `max-width` di `550px`. All'interno, crea un selettore `.hero-title` impostando `font-size` a `6rem`, un selettore `.hero-subtitle, .author, .quote, .list-title` impostando `font-size` a `1.8rem`, un selettore `.social-icons` impostando `font-size` a `2rem` e un selettore `.text` impostando `font-size` a `1.6rem`.
