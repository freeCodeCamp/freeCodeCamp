...
A differenza di Flexbox, se necessario, le impostazioni predefinite di CSS Grid creeranno righe aggiuntive. Dai al selettore `.social-icons` una proprietà `grid-template-columns` impostata su `repeat(5, 1fr)` per organizzare le icone in una singola riga.
