...
`gap` è una proprietà shorthand per impostare il valore di `column-gap` e `row-gap` allo stesso tempo. Se ha un solo valore, imposta `column-gap` e `row-gap` entrambi a quel valore. Se le vengono dati due valori, imposta `row-gap` come primo valore e `column-gap` come secondo.
