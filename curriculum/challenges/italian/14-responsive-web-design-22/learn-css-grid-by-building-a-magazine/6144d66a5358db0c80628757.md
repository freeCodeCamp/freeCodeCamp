...
Crea un selettore `html` e dagli una proprietà `font-size` impostata al `62.5%`. In questo modo, la dimensione predefinita del carattere per la pagina web sarà 10px (il valore predefinito del browser è 16px).

Questo ti renderà più facile lavorare con le unità `rem` più tardi, dato che `2rem` sarebbero 20px.

Imposta anche la proprietà `box-sizing` su `border-box`.
...
Il selettore `html` dovrebbe avere una proprietà `box-sizing` impostata su `border-box`.
