...
All'interno dell'elemento `.image-quote`, annida un elemento `hr`, un elemento `p` e un secondo elemento `hr`. Dai all'elemento `p` un attributo `class` impostato su `quote` e il testo `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
