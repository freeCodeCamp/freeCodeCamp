...
Crea un'altra `@media` query per `only screen` con una `max-width` di `600px`. All'interno, crea una regola `.text-with-images` e dalle una proprietà `grid-template-columns` di `1fr`.
