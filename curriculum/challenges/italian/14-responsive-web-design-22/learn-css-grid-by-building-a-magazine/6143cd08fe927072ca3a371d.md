...
All’interno dell’elemento `.image-wrapper`, per il secondo elemento `img` imposta `src` su `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`, `alt` su `image of a calculator project`, `loading` su `lazy`, `class` su `image-2`, `width` a `400` e `height` a `400`.
