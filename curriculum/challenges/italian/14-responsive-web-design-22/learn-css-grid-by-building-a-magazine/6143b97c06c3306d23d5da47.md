...
All'interno dell'elemento `article`, crea un elemento `h3` con `class` impostato su `list-title` e il testo `A Brief History`. Sotto questo, crea un elemento `p` con il testo `Of the Curriculum`. Poi crea un elemento `ul` con la classe `lists`.
