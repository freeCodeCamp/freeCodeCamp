...
Per iniziare a impostare lo stile, normalizza le regole CSS selezionando tutti gli elementi con un `*`, includendo anche gli pseudo-selettori `::before` e `::after`. Imposta entrambe le proprietà `padding` e `margin` a `0`.
