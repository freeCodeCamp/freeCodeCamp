...
Crea un'ultima `@media` query per `only screen` con una `max-width` di `420px`. All'interno, crea un selettore `.hero-title` con una proprietà `font-size` impostata su `4.5rem`.
