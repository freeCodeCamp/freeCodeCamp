...
Sotto l'elemento `.blush.right`, aggiungi due elementi `div` ciascuno con un valore `class` di `beak`. Inoltre, dai al primo elemento `.beak` un valore `class` di `top` e al secondo elemento `.beak` un valore `class` di `bottom`.
