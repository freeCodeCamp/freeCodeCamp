...
Sotto l'elemento `.chin`, aggiungi due elementi `div` ciascuno con un valore `class` di `eye`. Inoltre, dai al primo elemento `.eye` un valore `class` di `left` e al secondo elemento `.eye` un valore `class` di `right`.
