...
Seleziona l'elemento `.penguin-body`, dagli una proprietà `width` del `53%` e una proprietà `height` del `45%`. Quindi, imposta il `background` su un gradiente lineare a `45deg`, `rgb(134, 133, 133)` dallo `0%`, `rgb(234, 231, 231)` dal `25%` e `white` dal `67%`.
