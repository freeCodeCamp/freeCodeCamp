...
Dove rilevante, sostituisci i valori delle proprietà con la variabile `--penguin-face`.

# --hints--

Dovresti dare a `.face` una proprietà `background-color` con valore di `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Dovresti dare a `.chin` una proprietà `background-color` con valore di `var(--penguin-face)`.
