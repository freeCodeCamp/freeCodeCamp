...
Dai al tuo selettore `.wheel` una proprietà `max-height` e `max-width` entrambe impostate a `500px`.
...
Il tuo selettore `.wheel` dovrebbe avere una proprietà `max-width` impostata a `500px`.
