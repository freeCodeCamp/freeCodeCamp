...
Crea un'altra regola `@keyframes` con il nome `cabins`. Usa le stesse proprietà del tuo `@keyframes wheel`, copiando entrambe le regole `0%` e `100%`, ma imposta la prorpietà `transform` del selettore `100%` a `rotate(-360deg)`.
...
La tua nuova regola `@keyframes` dovrebbe essere nominata `cabins`.
...
La tua nuova regola `@keyframes` dovrebbe venire dopo la regola `@keyframes wheel`.
...
Il tuo selettore `0%` dovrebbe avere una proprietà `transform` con valore di `rotate(0deg)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[0]?.style?.transform === 'rotate(0deg)');
```

La regola della ruota `@keyframes cabins` dovrebbe avere un selettore `100%`.
...
Il tuo selettore `100%` dovrebbe avere una proprietà `transform` impostata a `rotate(-360deg)`.
