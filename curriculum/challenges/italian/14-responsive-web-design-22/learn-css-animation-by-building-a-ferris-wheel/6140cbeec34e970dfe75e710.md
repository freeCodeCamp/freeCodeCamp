...
Crea un selettore per il tuo elemento `.wheel`. Inizia impostando `border` a `2px solid black`, `border-radius` a `50%` e `margin-left` a `50px`.
...
Il selettore `.wheel` dovrebbe avere una proprietà `border` impostata a `2px solid black`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.wheel')?.border === '2px solid black');
```

Il selettore `.wheel` dovrebbe avere una proprietà `border-radius` impostata a `50%`.
