...
È ora di posizionare le cabine intorno alla ruota. Seleziona il primo elemento `.cabin`. Imposta la proprietà `right` a `-8.5%` e la proprietà `top` a `50%`.
...
Il selettore `.cabin:nth-of-type(1)` dovrebbe avere una proprietà `top` impostata a `50%`.
