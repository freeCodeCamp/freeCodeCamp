...
Dovresti avere due nuovi elementi `div` all'interno dell'elemento `.characters`.

```js
assert(document.querySelectorAll('.characters > div')?.length === 4);
```

I due nuovi elementi `div` dovrebbero avere l'attributo `class` impostato su `blue`.
...
Il primo `div` dovrebbe avere un `id` di `blue-left`.

```js
assert(document.querySelectorAll('.characters > div')?.[2]?.getAttribute('id') === 'blue-left');
```

Il secondo `div` dovrebbe avere un `id` di `blue-right`.
