...
Dovresti avere un selettore `#black-hat`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat'));
```

Il selettore `#black-hat` dovrebbe avere una proprietà `width` impostata a `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.width === '0px');
```

Il selettore `#black-hat` dovrebbe avere una proprietà `height` impostata a `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.height === '0px');
```

Il selettore `#black-hat` dovrebbe avere una proprietà `border-style` impostata su `solid`.
