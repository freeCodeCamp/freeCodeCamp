...
Dovresti dare ad `address` un `text-align` di `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Dovresti dare a `address` un `padding-top` di almeno `1px`.
...
Dovresti dare a `address` un `padding-right` di almeno `1px`.
...
Dovresti dare a `address` un `padding-bottom` di almeno `1px`.
...
Dovresti dare a `address` un `padding-left` di almeno `1px`.
