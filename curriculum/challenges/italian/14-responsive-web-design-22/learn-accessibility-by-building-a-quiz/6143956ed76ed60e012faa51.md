...
Il testo `.sr-only` è ancora visibile. C'è un modello comune per nascondere visivamente il testo dedicato ai lettori di schermo.

Questo modello è quello di usare le seguenti proprietà CSS:
...
Usa quanto sopra per definire la classe `sr-only`.
