...
Il testo dell'elemento `.sr-only` è ancora visibile. C'è un modello comune per nascondere visivamente il testo dedicato ai lettori di schermo.
...
Usa il modello qui sopra per definire la classe `sr-only`.
