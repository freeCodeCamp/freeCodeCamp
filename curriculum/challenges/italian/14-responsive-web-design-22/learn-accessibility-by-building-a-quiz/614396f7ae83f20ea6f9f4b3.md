...
Dentro il secondo elemento `section`, aggiungi due elementi `div` con la classe di `question-block`.

Poi, dentro ogni elemento `div.question-block`, aggiungi un elemento `p` con un testo contenente numeri crescenti, iniziando da `1`, e un elemento `fieldset` con la classe di `question`.
