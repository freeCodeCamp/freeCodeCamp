...
Dovresti assegnare al primo elemento `ul` un attributo `class` del valore `answers-list`.

```js
assert.equal(document.querySelectorAll('fieldset > ul')?.[0]?.className, 'answers-list');
```

Dovresti assegnare al secondo elemento `ul` un attributo `class` del valore `answers-list`.
