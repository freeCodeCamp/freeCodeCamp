...
Completando il contenuto del quiz, sotto `#student-info`, aggiungi tre elementi `div` con una `class` di `info` >.

Poi, all'interno di ogni `div` un elemento `label`, e un elemento `input`.
...
Dovresti annidare un elemento `input` dentro il primo `div`, dopo `label`.
...
Dovresti annidare un elemento `label` dentro il secondo `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.querySelectorAll('label')?.length, 1);
```

Dovresti annidare un elemento `input` dentro il secondo `div`, dopo `label`.
...
Dovresti annidare un elemento `label` dentro il terzo `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.querySelectorAll('label')?.length, 1);
```

Dovresti annidare un elemento `input` dentro il terzo `div`, dopo `label`.
