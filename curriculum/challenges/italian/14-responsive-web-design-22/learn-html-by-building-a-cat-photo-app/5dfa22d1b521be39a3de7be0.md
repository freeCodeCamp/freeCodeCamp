...
Trasforma le parole `cat photos` all'interno dell'elemento `p` in un link sostituendo le parole con l'elemento di ancoraggio aggiunto precedentemente. L'elemento `p` dovrebbe mostrare lo stesso testo nel browser, ma adesso le parole `cat photos` dovrebbero essere un link. Dovrebbe esserci un solo link visibile nell'app.
