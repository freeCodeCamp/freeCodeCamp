...
Identifica la sezione principale di questa pagina aggiungendo un tag di apertura `<main>` dopo l'elemento `h1` e un tag di chiusura `</main>` dopo l'elemento `p`.
...
Il tag di apertura dell'elemento `main` dovrebbe essere al di sotto dell'elemento `h1`. Sono nell'ordine sbagliato.
...
Il tag di apertura dell'elemento `main` dovrebbe essere sopra l'elemento `h2`. Sono nell'ordine sbagliato.
