...
Aggiungi un altro `repeating-linear-gradient` sotto quello che hai appena aggiunto. Dagli una direzione di `90deg`, utilizza il colore dell'edificio dalo `0%` al `12%` e il colore delle finestre dal `12%` al `44%`. Creerai un sacco di finestre rettangolari.
