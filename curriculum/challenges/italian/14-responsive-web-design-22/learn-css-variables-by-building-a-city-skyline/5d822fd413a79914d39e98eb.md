...
Il profilo degli edifici si sta unendo. Imposta la proprietà `background-color` per gli edifici in primo piano. Usa la variabile `--building-color1` per `.fb3` e `.fb4`, `--building-color2` per `.fb5`,`--building-color3` per `.fb2` e `.fb6`, e `--building-color4` per `.fb1`.
