...
Ora, rimuovi `width` e `height` da `.bb2a` e cambia `border-left` e `border-right` usando `5vw` invece di `1vw`. L'elemento ora non avrà dimensione e i bordi si uniranno nel mezzo.
