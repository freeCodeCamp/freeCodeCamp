...
Aggiungi un elemento `hr` tra l'elemento `p` con la classe `established` e il primo elemento `section`. Nota che gli elementi `hr` sono autoconcludenti.
...
Non dovresti cambiare l'elemento `p` con la classe `established` esistente.
...
L'elemento `hr` dovrebbe essere tra l'elemento `p` e l'elemento `section`.
