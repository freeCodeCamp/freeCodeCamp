...
1. 你应该有一个 `input` 元素，它的 `id` 为 `cash`
1. 您应该有一个 `div` 元素，它的`id` 为 `change-due`
1. 您应该有一个 `button` 元素，它的`id` 为 `purchase-btn`
1. 当 `#cash` 元素中的值小于 `price` ，应该显示一个警告框，搭配文本 `Customer does not have enough money to purchase the item`
