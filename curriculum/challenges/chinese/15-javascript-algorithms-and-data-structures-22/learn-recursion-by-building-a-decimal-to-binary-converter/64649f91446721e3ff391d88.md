...
assert.match(
  code,
  /animationContainer\.innerHTML\s*\+=\s*`\s*<p[\s\S]+>\s*decimalToBinary\s*\(\s*\$\{\s*obj\.inputVal\s*\}\s*\)\s*<\s*\/\s*p\s*>\s*`|animationContainer\.innerHTML\s*\+=\s*`\s*<p[\s\S]+>\s*decimalToBinary\s*\(\s*\$\{\s*obj\[\s*('|"|`)\s*inputVal\s*\1\s*\]\s*\}\s*\)\s*<\s*\/\s*p\s*>\s*`/
);
```
