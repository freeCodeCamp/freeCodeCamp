...
在之前的项目中，你使用了 `fetch()` 和 `.then()` 方法在 Promise 解析后执行逻辑。 现在你将使用 `await` 关键字来处理 `fetch()` 方法的异步特性。

`await` 关键字等待一个 promise 解析并返回结果。
...
在 `try` 块中，创建一个名为 `res` 的常量，并将 `await fetch()` 赋值给它。 对于 `fetch` 调用，传入 `forumLatest` 变量。

# --hints--

你应该在你的 `try` 块中有一个名为 `res` 的常量。

```js
assert.match(code, /\s*const\s+res/)
```

你应该将 `await fetch()` 赋值给你的 `res` 变量。

```js
assert.match(code, /\s*const\s+res\s*=\s*await\s+fetch\(\s*(.*)\s*\)/);
```

你应该将 `forumLatest` 常量传递给 `fetch` 调用。
