...
首先创建一个名为 `showLatestPosts` 的箭头函数，它接收一个单一的 `data` 参数。

# --hints--

你应该有一个名为 `showLatestPosts` 的箭头函数。

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

你的 `showLatestPosts` 函数应该有一个 `data` 参数。

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

应该定义 `showLatestPosts`。

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts` 应该是一个函数。
