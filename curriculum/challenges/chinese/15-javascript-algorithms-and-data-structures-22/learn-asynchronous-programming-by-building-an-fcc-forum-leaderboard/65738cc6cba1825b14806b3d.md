...
接下来，创建一个 `const` 变量，名为 `forumCategoryUrl`，并将其赋值为字符串 `https://forum.freecodecamp.org/c/`。

在其下方，创建另一个 `const` 变量，名为 `avatarUrl`，并将其赋值为字符串 `https://sea1.discourse-cdn.com/freecodecamp`。
...
你应该将 `https://forum.freecodecamp.org/c/` 分配给你的 `forumCategoryUrl` 变量。
...
你应该将 `https://sea1.discourse-cdn.com/freecodecamp` 分配给你的 `avatarUrl` 变量。
