...
在 `team` 属性下方，添加三个新属性键值对。
名为 `sport` 的键和一个名为 `"Football"` 的字符串值。
键名为 `year`，数值为 `1986`。
名为 `isWorldCupWinner` 的键和设置为 `true` 的布尔值。
一个名为 `headCoach` 的新键，其值为空对象。
...
您的 `myFavoriteFootballTeam` 对象应该有一个 `year` 属性。
