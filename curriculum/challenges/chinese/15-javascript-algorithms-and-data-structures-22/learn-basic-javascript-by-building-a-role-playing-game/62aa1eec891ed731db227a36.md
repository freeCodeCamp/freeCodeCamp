...
如果你在当前状态下玩你的游戏，你可能会发现一个 bug。 如果你的 `xp` 足够高， `getMonsterAttackValue` 函数将返回负数。 当与怪物作战时，这将增加你的总生命值！ You can fix this issue by using a <dfn>ternary operator</dfn> to ensure negative values are not returned.

`ternary operator` 是一种条件运算符，可以被当作单行 `if-else` 语句。 语法是：`condition ? expressionIfTrue : expressionIfFalse`。
