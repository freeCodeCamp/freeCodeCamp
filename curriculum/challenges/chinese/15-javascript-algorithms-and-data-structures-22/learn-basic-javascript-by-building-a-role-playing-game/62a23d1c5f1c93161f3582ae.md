...
与 `#stats` 元素类似，`#monsterStats` 元素需要两个 `span` 元素。 给它们类 `stat`，并给第一个元素文本 `Monster Name:`，给第二个元素文本 `Health:`。 在每个文本之后，添加一个 `strong` 元素和一个空的嵌套 `span` 元素。 Give the first inner `span` element an `id` of `monsterName` and the second inner `span` element an `id` of `monsterHealth`.
