...
Initially, each flock consists of $n$ sheep. Each sheep (regardless of color) is equally likely to be the next sheep to bleat. 在一只绵羊咩咩叫，另一只羊的羊群已经越过之后，Peredur可能会移走一些白羊，以便最大化预期的最终黑羊数量。 Let $E(n)$ be the expected final number of black sheep if Peredur uses an optimal strategy.
