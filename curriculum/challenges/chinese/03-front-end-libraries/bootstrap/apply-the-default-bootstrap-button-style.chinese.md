...
videoUrl: ''
localeTitle: Apply the Default Bootstrap Button Style
---
...
tests:
  - text: 将 <code>btn</code> class 添加到所有的 <code>button</code> 元素中。
    testString: assert($(".btn").length > 5, '将 <code>btn</code> class 添加到所有的 <code>button</code> 元素中。');
  - text: 将 <code>btn-default</code> class 添加到每一个 <code>button</code> 元素中。
    testString: assert($(".btn-default").length > 5, '将 <code>btn-default</code> class 添加到每一个 <code>button</code> 元素中。');
