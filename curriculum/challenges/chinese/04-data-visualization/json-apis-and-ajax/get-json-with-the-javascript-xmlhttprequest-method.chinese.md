...
tests:
  ...
  - text: 您的代码应该有一个设置为函数的<code>onload</code>事件处理程序。
    testString: assert(code.match(/\.onload\s*=\s*function\(\s*?\)\s*?{/g));
  - text: 您的代码应该使用<code>JSON.parse</code>方法来解析<code>responseText</code> 。
    testString: assert(code.match(/JSON\.parse\(.*\.responseText\)/g));
  - text: 您的代码应该获取带有类<code>message</code>的元素，并将其内部HTML更改为JSON数据字符串。
    testString: assert(code.match(/document\.getElementsByClassName\(\s*?("|")message\1\s*?\)\[0\]\.innerHTML\s*?=\s*?JSON\.stringify\(.+?\)/g));
