...
现在将 `#eyes-div` 元素移动到位，将 `position` 设置为 `absolute`，`top` 设置为 `-40px`，并且 `left` 设置为 `20px`。

# --hints--

你的 `#eyes-div` 选择器应该有一个 `position` 属性设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.position === 'absolute');
```

你的 `#eyes-div` 选择器应该有一个 `top` 属性设置为 `-40px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.top === '-40px');
```

你的 `#eyes-div` 选择器应该有一个 `left` 属性设置为 `20px`。
