...
使用 id 选择器选中 id 为 `triangles` 的元素。 将 `width` 设置为 `250px`，`height` 设置为 `550px`。

# --hints--

你应该添加一个 `#triangles` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('#triangles'));
```

你的 `#triangles` 选择器应该有一个 `width` 属性设置为 `250px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#triangles')?.width === '250px');
```

你的 `#triangles` 选择器应该将 `height` 属性设置为 `550px`。
