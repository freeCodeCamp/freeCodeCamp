...
FontAwesome 图标带有自己的样式来定义图标。 但是，你仍然可以自己设置样式，以更改颜色和大小等内容。 现在，使用类选择器来选中具有 `fa-music` 类的图标。 将 `display` 设置为 `inline-block`，将 `margin-top` 设置为 `8%`，并将 `margin-left` 设置为 `13%`。
