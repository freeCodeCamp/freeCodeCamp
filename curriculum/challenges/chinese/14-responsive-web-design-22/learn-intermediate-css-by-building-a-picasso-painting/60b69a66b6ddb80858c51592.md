...
使用类选择器为具有 `black-dot` 类的元素创建规则。 将 `width` 设置为 `10px`，`height` 设置为 `10px`，`background-color` 设置为 `rgb(45, 31, 19)`。
