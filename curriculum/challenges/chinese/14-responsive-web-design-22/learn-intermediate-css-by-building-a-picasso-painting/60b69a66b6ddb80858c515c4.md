...
是时候用 `eyes` 类设置元素的样式了。 使用类选择器将 `width` 设置为 `35px`，`height` 设置为 `20px`，`background-color` 设置为 `#8B4513`，`border-radius` 设置为 `20px 50%`。
