...
将表格移至适当位置，为此，给它一个 `position` 值为 `absolute`、一个 `top` 值为 `275px`，和一个 `left` 值为 `15px`。

# --hints--

`#tan-table` 选择器应该有一个 `position` 属性设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#tan-table')?.position === 'absolute');
```

`#tan-table` 选择器应该有一个 `top` 属性设置为 `275px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#tan-table')?.top === '275px');
```

`#tan-table` 选择器应该有一个 `left` 属性设置为 `15px`。
