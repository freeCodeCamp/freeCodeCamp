...
现在你需要将条形图标移动到位。 为 `fa-bars` 类创建一个类选择器。 将 `display` 设置为 `block`，`margin-top` 设置为 `30%`，`margin-left` 设置为 `40%`。
