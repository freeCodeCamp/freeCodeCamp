...
为你的 `.triangle` 元素的边框设置样式。 将 `border-style` 设置为 `solid`，`border-width` 设置为 `42px 45px 45px 0`。

# --hints--

你的 `.triangle` 选择器应该有一个 `border-style` 属性设置为 `solid`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.triangle')?.borderStyle === 'solid');
```

你的 `.triangle` 选择器应该有一个 `border-width` 属性设置为 `42px 45px 45px 0`。
