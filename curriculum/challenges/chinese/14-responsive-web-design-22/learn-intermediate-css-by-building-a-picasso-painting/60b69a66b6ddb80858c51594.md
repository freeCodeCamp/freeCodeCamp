...
将圆点移至适当位置，为此，将 `display` 设置为 `block`、将 `margin` 设置为 `auto`，并将 `margin-top` 设置为 `65%`。

# --hints--

`.black-dot` 选择器应该有一个 `display` 属性设置为 `block`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.black-dot')?.display === 'block');
```

`.black-dot` 选择器应该有一个 `margin` 属性设置为 `auto`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.black-dot')?.margin?.includes('auto'));
```

`.black-dot` 选择器应该有一个 `margin-top` 属性设置为 `65%`。
