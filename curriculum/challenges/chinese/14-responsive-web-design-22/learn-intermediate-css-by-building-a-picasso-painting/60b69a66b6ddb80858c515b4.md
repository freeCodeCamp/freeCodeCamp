...
给 `#orange-character` 设置 `position` 为 `absolute`，`top` 为 `25%`，以及 `left` 为 `40%`。

# --hints--

你的 `#orange-character` 选择器应该有一个 `position` 属性设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.position === 'absolute');
```

你的 `#orange-character` 选择器应该有一个 `top` 属性设置为 `25%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.top === '25%');
```

你的 `#orange-character` 选择器应该有一个 `left` 属性设置为 `40%`。
