...
现在将 `position` 设置为 `absolute`，将 `top` 设置为 `20%`，将 `left` 设置为 `20%`。

# --hints--

你的 `#blue-left` 选择器应该有一个 `position` 属性设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-left')?.position === 'absolute');
```

你的 `#blue-left` 选择器应该有一个 `top` 属性设置为 `20%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-left')?.top === '20%');
```

你的 `#blue-left` 选择器应该有一个 `left` 属性设置为 `20%`。
