...
接着，使用 id 选择器选中 id 为 `blue-right` 的元素。 将 `width` 设置为 `400px`，将 `height` 设置为 `300px`。

# --hints--

你应该有一个 `#blue-right` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-right'));
```

你的 `#blue-right` 选择器应该有一个 `width` 属性设置为 `400px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-right')?.width === '400px');
```

你的 `#blue-right` 选择器应该有一个 `height` 属性设置为 `300px`。
