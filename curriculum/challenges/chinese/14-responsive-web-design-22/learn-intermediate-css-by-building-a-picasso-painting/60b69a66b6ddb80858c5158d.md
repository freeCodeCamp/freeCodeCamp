...
为面罩提供 `position` 值为 `absolute`，以及 `top` 和 `left` 的值均为 `0`。

# --hints--

`#black-mask` 选择器应该有一个 `position` 属性设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-mask')?.position === 'absolute');
```

`#black-mask` 选择器应该有一个 `top` 属性设置为 `0`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-mask')?.top === '0px');
```

`#black-mask` 选择器应该有一个 `left` 属性设置为 `0`。
