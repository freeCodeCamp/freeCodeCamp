...
使用 id 选择器为 id 为 `eyes-div` 的元素创建规则。 将 `width` 设置为 `180px`，并将 `height` 设置为 `50px`。

# --hints--

你应该创建一个 `#eyes-div` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div'));
```

你的 `#eyes-div` 选择器应该有一个 `width` 属性设置为 `180px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.width === '180px');
```

你的 `#eyes-div` 选择器应该将 `height` 属性设置为 `50px`。
