...
使用 id 选择器，为 id 为 `white-paper` 的元素创建规则。 将 `width` 设置为 `400px`、`height` 设置为 `100px`、`background-color` 设置为 `GhostWhite`。

# --hints--

你应该有一个 `#white-paper` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('#white-paper'));
```

你的 `#white-paper` 选择器应该将 `width` 属性设置为 `400px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#white-paper')?.width === '400px');
```

你的 `#white-paper` 选择器应该将 `height` 属性设置为 `100px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#white-paper')?.height === '100px');
```

你的 `#white-paper` 选择器应该将 `background-color` 属性设置为 `GhostWhite`。
