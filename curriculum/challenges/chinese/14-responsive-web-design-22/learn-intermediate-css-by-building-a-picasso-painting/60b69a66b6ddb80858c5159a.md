...
面具需要眼睛。 在你的 `#gray-mask` 元素中，添加两个 `div` 元素。 第一个应该将 `class` 设置为 `eyes left`，第二个应该将 `class` 设置为 `eyes right`。

# --hints--

`#gray-mask` 元素中应该有两个 `div` 元素。

```js
assert(document.querySelectorAll('#gray-mask > div')?.length === 2);
```

第一个新 `div` 元素应该将 `class` 设置为 `eyes left`。
...
第二个新 `div` 元素应该将 `class` 设置为 `eyes right`。
