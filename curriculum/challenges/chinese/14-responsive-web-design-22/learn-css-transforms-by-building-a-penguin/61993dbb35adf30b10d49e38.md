...
选择 `.penguin-body` 元素，使其 `width` 为 `53%`，`height` 为 `45%`。 然后将 `background` 设置为渐变轴为 `45deg` 的线性渐变，起始点 `0%` 为 `rgb(134, 133, 133)`，过渡到中间 `25%` 位置的 `rgb(234, 231, 231)`，在过渡到 `67%` 位置的 `white`。
