...
要使 `input` 框彼此对齐，请将所有 `.info` 元素内 `input` 和 `label` 元素的 `display` 属性设置为 `inline-block`。
...
你应该给 `input` 和 `label` 元素添加 `text-align` 值为 `right`。
...
你应该在 `.info input` 规则之前设置 `text-align: right` 属性。
