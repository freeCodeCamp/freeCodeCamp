...
HTML <dfn>属性</dfn>是写在一个元素的开始标签中的特殊词汇，它们用于控制这个元素的行为。 在 `img` 元素中的 `src` 属性明确了一个图片的 URL（图片所在的位置）。 这是一个使用了 `src` 属性的 `img` 元素：`<img src="https://www.your-image-source.com/your-image.jpg">`。
