...
在你的 `.image-wrapper` 元素中，给第二个 `img` 元素添加 `src` 为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`，添加 `alt` 为 `image of a calculator project`，以及值为 `lazy` 的 `loading` 属性、值为 `image-2` 的 `class`、值为 `400` 的 `width` 属性和值为 `400` 的 `height` 属性。
