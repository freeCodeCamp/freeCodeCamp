...
在你的 `.image-wrapper` 元素中，给第三个 `img` 元素添加 `src` 为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`，添加 `alt` 为 `four people working on code`，以及值为 `lazy` 的 `loading` 属性、值为 `image-3` 的 `class`、值为 `600` 的 `width` 属性和值为 `400` 的 `height` 属性。
