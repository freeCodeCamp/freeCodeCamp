...
在 `.image-wrapper` 元素中，为你的第一个 `img` 元素添加 `src` 值为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`，添加 `alt` 为 `image of the quote machine project`，以及 `class` 为 `image-1`，和值为 `lazy` 的 `loading` 属性、值为 `600` 的 `width` 属性、值为 `400` 的 `height` 属性。
