...
在 `.image-quote` 元素中，嵌套一个 `hr` 元素、一个 `p` 元素和第二个 `hr` 元素。 给 `p` 元素一个 `class` 为 `quote` 和文本 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`。
