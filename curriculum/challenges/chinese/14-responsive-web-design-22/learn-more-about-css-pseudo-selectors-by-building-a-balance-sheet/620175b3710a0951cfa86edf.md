...
通过设置 `999` 的 `z-index` 确保你的年份不会被隐藏。 然后，给它一个 `0 -2px` 的 `margin`，并将 `padding` 设置为 `0.5rem calc(1.25rem + 2px) 0.5rem 0`。

# --hints--

`#years` 选择器应该将 `z-index` 属性设置为 `999`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('z-index') === '999');
```

`#years` 选择器应该有一个 `margin` 属性设置为 `0 -2px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('margin') === '0px -2px');
```

`#years` 选择器应该有一个 `padding` 属性设置为 `0.5rem calc(1.25rem + 2px) 0.5rem 0`。
