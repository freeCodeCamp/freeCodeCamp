...
在最后一个 `.divider` 之后，使用文本 `Cholesterol 0mg 0%` 创建一个新的 `p` 元素。 将文本 `Cholesterol` 包裹在 `span` 元素中，并将 `span` 元素的 `class` 属性设置为 `bold`。 将文本 `0%` 包裹在另一个 `span` 元素中，并将 `class` 设置为 `bold right`。
