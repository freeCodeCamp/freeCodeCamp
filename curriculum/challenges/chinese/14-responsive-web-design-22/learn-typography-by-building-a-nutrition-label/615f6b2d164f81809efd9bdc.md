...
在最后一个 `.divider` 元素之后，创建一个 `p` 元素并为其指定文本 `Total Fat 8g 10%`。 将 `Total Fat` 包裹在 `span` 元素中，并将 `class` 设置为 `bold`。 将文本 `10%` 包裹在另一个 `span` 元素中，并将 `class` 设置为 `bold`。 最后：嵌套 `Total Fat` `span` 元素和文本 `8g` 在另一个 `span` 元素中，以实现对齐。
