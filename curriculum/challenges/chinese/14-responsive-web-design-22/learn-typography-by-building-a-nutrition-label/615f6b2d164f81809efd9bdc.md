...
在最后一个 `.divider` 元素之后，创建一个 `p` 元素并为其指定文本 `Total Fat 8g 10%`。 Wrap the text `Total Fat` in a `span` element with the `class` of `bold`. Wrap the text `10%` in another `span` element with the `class` of `bold`. 最后：嵌套 `Total Fat` `span` 元素和文本 `8g` 在另一个 `span` 元素中，以实现对齐。
