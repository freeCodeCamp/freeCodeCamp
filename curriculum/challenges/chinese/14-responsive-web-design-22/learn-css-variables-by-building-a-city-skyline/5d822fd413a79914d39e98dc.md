...
建筑物相互堆叠，并超出了屏幕。 让我们解决这个问题。 将属性 `display: flex;`, `align-items: flex-end;` 和 `justify-content: space-evenly;` 添加到 `background-buildings` 类。 这将再次使用 flexbox 将建筑物均匀地分布在元素的底部。
