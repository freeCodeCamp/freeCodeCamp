...
我看到一些可以优化的代码。 将 `position` 和 `top` 属性和值从 `.foreground-buildings` 移动到 `.background-buildings`。 然后在此处同时选择 `.background-buildings` 和 `.foreground-buildings`，有效地将这些样式应用于这两个元素。 你可以使用逗号（`,`）分隔选择器，例如：`selector1, selector2`。
