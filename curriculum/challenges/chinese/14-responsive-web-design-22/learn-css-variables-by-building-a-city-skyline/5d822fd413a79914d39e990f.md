...
窗户在该部分的左侧堆叠在一起，在紫色建筑的后面。 在 `.building-wrap` 下添加一个名为 `window-wrap` 的新类。 使 `.window-wrap` 成为一个 flexbox 容器，使用 `align-items` 和 `justify-content` 属性将它的子元素垂直居中，并分别在它们的父元素中均匀排布。
