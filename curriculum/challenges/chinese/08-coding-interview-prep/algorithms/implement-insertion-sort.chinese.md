...
有许多不同的排序方式。当选择排序运行时，数组开头的子数组被排序，但最后的子数组不是。选择排序扫描未排序的子数组，以便在排序的子数组中包含下一个元素。
这是考虑排序的另一种方式。想象一下，你正在玩纸牌游戏。你拿着卡片，这些卡片已经分类了。经销商只给你一张新卡。你必须把它放到正确的地方，这样你所持有的牌仍然是有序的。在选择排序中，添加到已排序子数组的每个元素都不小于已排序子数组中已有的元素。但在我们的卡片示例中，新卡可能比您已经持有的一些卡片要小，所以您可以将新卡片与手中的每张卡片进行比较，直到找到放置它的地方为止。 。您将新卡插入正确的位置，然后您的手再次握住完全分类的卡。然后经销商给你另一张卡，你重复相同的程序。然后另一张卡，另一张卡，依此类推，直到经销商停止给你卡。
