...
Unda sheria nyingine ya `@keyframes` kwa jina `cabins`. Tumia sifa sawa na `@keyframes wheel`, unakili sheerriai za `0%` na `100%`, lakini weka sifa ya `transform` ya kiteuzi cha `100%` kuwa `rotate(-360deg)`.
