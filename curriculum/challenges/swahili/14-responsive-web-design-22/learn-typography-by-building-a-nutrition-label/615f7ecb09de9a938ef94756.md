...
Below your last `p` element, add another `p` element with the text `Total Carbohydrate 37g 13%`. Kama hapo awali, tumia vipengee vya `span` ili kufanya maandishi `Total Carbohydrate` na `13%` kuwa nzito. Also add an additional `span` element around the `Total Carbohydrate 37g` text to have it aligned to the left and `13%` to the right.
