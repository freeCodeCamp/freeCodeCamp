...
Tumia kichaguzi cha `>` kulenga vipengele vya orodha ambavyo havijapangwa ndani ya vipengele vya `nav`, na utumie _Flexbox_ kusawazisha nafasi kwa watoto.
...
Unapaswa kuvipa vipengele vya `nav > ul` `display` ya `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('nav > ul')?.display, 'flex');
```

Unapaswa kuvipa vipengele vya `nav > ul` `justify-content` ya `space-evenly`.
