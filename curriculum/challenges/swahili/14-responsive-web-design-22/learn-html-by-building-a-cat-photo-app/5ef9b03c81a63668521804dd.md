...
Vipengele vya `label` hutumika kusaidia kuhusisha maandishi ya kipengele cha `input` na kipengele chenyewe cha `input` (hasa kwa teknolojia saidizi kama vile visoma skrini). Kwa mfano, `<label><input type="radio"> cat</label>` hufanya hivyo kubofya neno `cat` pia kuchagua kitufe cha redio sambamba.
