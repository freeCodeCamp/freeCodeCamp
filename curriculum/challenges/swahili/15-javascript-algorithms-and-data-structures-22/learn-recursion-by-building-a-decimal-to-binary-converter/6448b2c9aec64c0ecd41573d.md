...
```js
assert.notMatch(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\);?\s*return\;?\s*\}\s*console\.log\((\s|.)*\)/);
```

You should call the `decimalToBinary` function after the `if` statement within the body of your `checkUserInput` function.

```js
assert.match(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\);?\s*return\;?\s*\}\s*decimalToBinary\(/);
```
