...
```js
assert.notMatch(code, /('|"|`)keydown\1\s*,\s*\(\s*e\s*\)\s*=>\s*{\s*.*console\.log\(/);
```

Your `if` statement should check if `e.key` is equal to the string `Enter`.

```js
assert.match(code, /if\s*\(\s*e\s*\.\s*key\s*===?\s*('|"|`)Enter\1\s*\)\s*\{/);
```

The body of your `if` statement should be empty.

```js
assert.match(code, /if\s*\(\s*e\s*\.\s*key\s*===?\s*('|"|`)Enter\1\s*\)\s*\{\s*\}/);
```
