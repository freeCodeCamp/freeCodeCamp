...
```js
assert.match(code, /<a\s+(?:class=['"]post-title['"]\s+target=['"]_blank['"]|target=['"]_blank['"]\s+class=['"]post-title['"])(?:.|\n)*?<\/a>/);
```

Your opening anchor tag should have a `href` attribute set to `${forumTopicUrl}${slug}/${id}` after the `target` attribute.

```js
assert.match(code, /<a\s+class=['"]post-title['"]\s+target=['"]_blank['"]\s+href=['"]\$\{forumTopicUrl\}\$\{slug\}\/\$\{id\}['"]\s*>([\s\S]*?)<\/a>/);
```
