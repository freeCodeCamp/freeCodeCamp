...
窓がセクションの左端、紫色のビルの後ろで互いに積み重なっています。 Add a new class below `.building-wrap` called `window-wrap`. Make `.window-wrap` a flexbox container, and use the `align-items` and `justify-content` properties to center its child elements vertically and evenly space them in their parent, respectively.
