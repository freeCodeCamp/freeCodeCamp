...
前景のビル群 (foreground buildings) のコンテナーが背景のビル群の要素の上にぴったり重なるようにしましょう。 このコンテナーの `width` と `height` を `100%` に、`position` を `absolute` に、`top` を `0` に設定してください。 これでコンテナーのサイズが body と同じになり、開始位置が左上の角に移動されます。
