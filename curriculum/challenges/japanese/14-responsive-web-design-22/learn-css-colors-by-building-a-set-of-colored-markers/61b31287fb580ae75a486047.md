...
`.red` CSS ルール内の `box-shadow` プロパティの値を変更して、`offsetX` を `0`、`offsetY` を `0`、`blurRadius` を `20px`、`spreadRadius` を `0`、`color` を `red` にしてください。 Remember that you don't need to add units to a zero value.
