...
`input` ボックスの位置を揃えるため、`.info` 要素内のすべての `input` と `label` 要素を対象に、`display` プロパティを `inline-block` に設定してください。

さらに、テキストを右側にそろえてください。
...
`input` と `label` 要素の `text-align` を `right` に設定する必要があります。
...
`text-align: right` プロパティの設定は `.info input` ルールより前で行う必要があります。
