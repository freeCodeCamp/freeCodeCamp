...
`scroll-behavior` を `smooth` に設定することはほとんどのユーザーに好まれます。 ただ、この動きが遅過ぎると感じ、一瞬でスクロールする動作を好むユーザーもいます。

ユーザーのブラウザー設定に基づいて CSS を設定してくれるメディアルールがあります。 このメディアルールは `prefers-reduced-motion` と呼ばれ、以下の値のどちらか 1 つを受け付けます:

- `reduce`
- `no-preference`

適切なルールを `prefers-reduced-motion` メディアルールで囲み、ユーザーのブラウザー設定が `no-preference` である場合にのみ `scroll-behavior` が `smooth` に設定されるようにしてください。
