...
`#orange-character` の要素について、`position` を `absolute`、`top` を `25%`、`left` を `40%` に設定してください。

# --hints--

`#orange-character` セレクターの `position` プロパティを `absolute` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.position === 'absolute');
```

`#orange-character` セレクターの `top` プロパティを `25%` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.top === '25%');
```

`#orange-character` セレクターの `left` プロパティを `40%` に設定する必要があります。
