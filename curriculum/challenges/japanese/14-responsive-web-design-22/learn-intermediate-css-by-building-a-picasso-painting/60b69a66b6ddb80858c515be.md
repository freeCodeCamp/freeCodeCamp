...
`guitar-left` という値の `id` を選択し、`position` を `absolute` に、`left` を `0px` に設定してください。

# --hints--

`#guitar-left` セレクターを新しく作成する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-left'));
```

`#guitar-left` セレクターの `position` プロパティを `absolute` に設定する必要があります。
