...
`guitar-right` という値の `id` を選択し、`position` を `absolute` に設定してください。 こちらは `left` を `100px` に設定してください。

# --hints--

`#guitar-right` セレクターを新しく作成する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-right'));
```

`#guitar-right` セレクターの `position` プロパティを `absolute` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-right')?.position === 'absolute');
```

`#guitar-right` セレクターの `left` プロパティを `100px` に設定する必要があります。
