...
`#white-paper` セレクターの `position` プロパティを `absolute` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#white-paper')?.position === 'absolute');
```

`#white-paper` セレクターの `top` プロパティを `250px` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#white-paper')?.top === '250px');
```

`#white-paper` セレクターの `left` プロパティを `-150px` に設定する必要があります。
