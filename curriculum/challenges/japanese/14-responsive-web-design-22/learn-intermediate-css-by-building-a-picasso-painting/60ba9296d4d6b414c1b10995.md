...
同じ `#guitar` セレクター内で、`position` を `absolute` に、`top` を `120px` に、`left` を `0px` に設定してください。

# --hints--

`#guitar` セレクターの `position` プロパティを `absolute` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar')?.position === 'absolute');
```

`#guitar` セレクターの `top` プロパティを `120px` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar')?.top === '120px');
```

`#guitar` セレクターの `left` プロパティを `0px` に設定する必要があります。
