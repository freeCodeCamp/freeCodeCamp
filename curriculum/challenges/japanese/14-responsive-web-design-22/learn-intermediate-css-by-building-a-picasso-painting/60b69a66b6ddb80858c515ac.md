...
`#blue-right` の要素を正しい位置に移動するため、`position` を `absolute`、`top` を `50%`、`left` を `40%` に設定してください。

# --hints--

`#blue-right` セレクターの `position` プロパティを `absolute` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-right')?.position === 'absolute');
```

`#blue-right` セレクターの `top` プロパティを `50%` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-right')?.top === '50%');
```

`#blue-right` セレクターの `left` プロパティを `40%` に設定する必要があります。
