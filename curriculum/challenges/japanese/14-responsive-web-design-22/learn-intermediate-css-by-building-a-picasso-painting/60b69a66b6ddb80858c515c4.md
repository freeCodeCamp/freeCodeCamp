...
では `eyes` クラスの要素のスタイルを設定しましょう。 クラスセレクターを使用して、`width` を `35px`、`height` を `20px`、`background-color` を `#8B4513`、`border-radius` を `20px 50%` に設定してください。

# --hints--

`.eyes` セレクターを作成する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.eyes'));
```

`.eyes` セレクターの `width` プロパティを `35px` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.eyes')?.width === '35px');
```

`.eyes` セレクターの `height` プロパティを `20px` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.eyes')?.height === '20px');
```

`.eyes` セレクターの `background-color` プロパティを `#8B4513` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.eyes')?.backgroundColor === 'rgb(139, 69, 19)');
```

`.eyes` セレクターの `border-radius` プロパティを `20px 50%` に設定する必要があります。
