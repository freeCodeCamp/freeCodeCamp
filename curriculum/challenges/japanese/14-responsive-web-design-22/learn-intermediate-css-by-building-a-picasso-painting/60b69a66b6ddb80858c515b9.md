...
ID セレクターを使用して、`eyes-div` という ID を対象とするルールを作成してください。 `width` を `180px` に、`height` を `50px` に設定してください。

# --hints--

`#eyes-div` セレクターを作成する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div'));
```

`#eyes-div` セレクターの `width` プロパティを `180px` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.width === '180px');
```

`#eyes-div` セレクターの `height` プロパティを `50px` に設定する必要があります。
