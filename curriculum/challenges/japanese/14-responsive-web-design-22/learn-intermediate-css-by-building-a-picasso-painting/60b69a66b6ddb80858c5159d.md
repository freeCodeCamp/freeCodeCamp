...
`#black-character` セレクターの `position` プロパティを `absolute` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-character')?.position === 'absolute');
```

`#black-character` セレクターの `top` プロパティを `30%` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-character')?.top === '30%');
```

`#black-character` セレクターの `left` プロパティを `59%` に設定する必要があります。
