...
`#orange-character` セレクターが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character'));
```

`#orange-character` セレクターの `width` プロパティを `250px` に設定する必要があります。
...
`#orange-character` セレクターの `background-color` プロパティを `rgb(240, 78, 42)` に設定する必要があります。
