...
点を所定の位置に移動するため、`display` を `block` に、`margin` を `auto` に、`margin-top` を `65%` に設定してください。

# --hints--

`.black-dot` セレクターの `display` プロパティを `block` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.black-dot')?.display === 'block');
```

`.black-dot` セレクターの `margin` プロパティを `auto` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.black-dot')?.margin?.includes('auto'));
```

`.black-dot` セレクターの `margin-top` プロパティを `65%` に設定する必要があります。
