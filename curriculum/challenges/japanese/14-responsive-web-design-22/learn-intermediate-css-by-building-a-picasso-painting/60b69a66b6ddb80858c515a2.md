...
`#gray-mask` の要素の位置を調整するために、`position` を `absolute` に、`top` を `-10px` に、`left` を `70px` に設定してください。

# --hints--

`#gray-mask` セレクターの `position` プロパティを `absolute` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.position === 'absolute');
```

`#gray-mask` セレクターの `top` プロパティを `-10px` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.top === '-10px');
```

`#gray-mask` セレクターの `left` プロパティを `70px` に設定する必要があります。
