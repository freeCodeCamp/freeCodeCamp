...
次に `position` を `absolute` に、`top` を `20%` に、`left` を `20%` に設定してください。

# --hints--

`#blue-left` セレクターの `position` プロパティを `absolute` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-left')?.position === 'absolute');
```

`#blue-left` セレクターの `top` プロパティを `20%` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-left')?.top === '20%');
```

`#blue-left` セレクターの `left` プロパティを `20%` に設定する必要があります。
