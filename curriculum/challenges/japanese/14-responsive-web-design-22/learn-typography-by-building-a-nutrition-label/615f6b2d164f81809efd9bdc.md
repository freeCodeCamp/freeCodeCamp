...
最後の `.divider` 要素の下に、`p` 要素を作成してテキストを `Total Fat 8g 10%` にしてください。 テキストの `Total Fat` を、`bold` という `class` を持つ `span` 要素で囲んでください。 テキストの `10%` は別の `span` 要素で囲み、そちらも `bold` という `class` を持つようにしてください。 最期に、`Total Fat` の `span` 要素と `8g` のテキストを、もう 1 つの `span` 要素で囲んで位置を揃えましょう。
