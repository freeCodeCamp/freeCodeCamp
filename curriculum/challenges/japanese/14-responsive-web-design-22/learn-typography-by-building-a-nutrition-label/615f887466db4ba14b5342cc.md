...
最後の仕切り線の後に `p` 要素をもう 1 つ作成し、テキストを `Protein 3g` にしてください。 適切なクラスを使用して下端の境界線を取り除き、`span` を使用して `Protein` の部分を太字にしてください。 Then wrap the text `Protein 3g` including the new `span` element, in a new `span` element.
