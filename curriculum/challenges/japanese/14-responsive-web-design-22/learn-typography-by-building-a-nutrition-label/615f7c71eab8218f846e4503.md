...
最後の `.divider` の後に `p` 要素をもう 1 つ作成し、テキストを `Trans Fat 0g` にしてください。 `Trans` という単語を `i` 要素で囲んでイタリック体にしてください。 新しい `p` 要素の `class` 属性は `indent no-divider` に設定してください。 Wrap `Trans Fat 0g` in a `span` element for alignment.
