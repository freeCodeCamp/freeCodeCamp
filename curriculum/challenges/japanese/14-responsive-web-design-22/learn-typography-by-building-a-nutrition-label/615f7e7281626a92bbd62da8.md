...
最後の `p` 要素の下に `p` 要素をもう 1 つ作成し、テキストを `Sodium 160mg 7%` にしてください。 `Sodium` というテキストを `span` 要素で囲み、その `class` 属性を `bold` に設定してください。 Wrap the `7%` text in another `span` element with the `class` set to `bold`. Also add an additional `span` element around `Sodium 160mg` for aligning it correctly.
