...
`.face` 要素の `width` を `60%` に、`height` を `70%` に、`background-color` を `white` に設定してください。

# --hints--

`.face` セレクターを使用する必要があります。

```js
assert.match(code, /\.face\s*\{/);
```

`.face` には `width` プロパティが必要です。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.width);
```

`.face` の `width` を `--fcc-expected--` に設定する必要がありますが、`--fcc-actual--` が設定されています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.width, '60%');
```

`.face` には `height` プロパティが必要です。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.height);
```

`.face` の `height` を `--fcc-expected--` に設定する必要がありますが、`--fcc-actual--` が設定されています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.height, '70%');
```

`.face` には `background-color` プロパティが必要です。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.backgroundColor);
```

`.face` の `background-color` を `--fcc-expected--` に設定する必要がありますが、`--fcc-actual--` が設定されています。
