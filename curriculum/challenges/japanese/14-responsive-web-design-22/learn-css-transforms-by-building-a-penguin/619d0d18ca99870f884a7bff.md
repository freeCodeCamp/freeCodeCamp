...
各 `.eye` 要素内に、`class` が `eye-lid` の `div` を 1 つずつ追加してください。

# --hints--

`.eye.left` 内に `div` 要素を 1 つ追加する必要がありますが、 `--fcc-actual--` 個が見つかりました。

```js
assert.equal(document.querySelectorAll('.eye.left > div')?.length ?? 0, 1);
```

`.eye.right` 内に `div` 要素を 1 つ追加する必要がありますが、 `--fcc-actual--` 個が見つかりました。
