...
ここまでのコードを見てみると、`.face` 要素と `.chin` 要素の `background-color` は同じです。

`--penguin-face` という名前のカスタム CSS プロパティ (変数) を作成し、その値を `white` に設定してください。
...
`:root` 内に `--penguin-face` プロパティが必要です。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--penguin-face'));
```

`:root` の `--penguin-face` を `--fcc-expected--` に設定する必要がありますが、`--fcc-actual--` が設定されています。
