...
```js
assert(/return\s*`\s*<tr>\s*<td>\s*\S*\s*<p[^>]*class\s*=\s*['"]\s*post-title\s*['"][^>]*>.*<\/p>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
...
```js
assert(/return\s*`\s*<tr>\s*<td>\s*\S*\s*<p[^>]*class\s*=\s*['"]\s*post-title\s*['"][^>]*>\${title}\s*<\/p>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```
