...
- `price`: el precio del artículo como un número de punto flotante.
- `cash`: el monto de dinero proporcionado por el cliente para el artículo, que se proporciona a través de un elemento `input` dentro de la página.
- `cid`: cajón de efectivo, un arreglo 2D listando las monedas disponibles en el efectivo.
...
- `Status: INSUFFICIENT_FUNDS`: si `cash-in-drawer` es menor que el cambio esperado, ó si no puede devolver el cambio exacto.
