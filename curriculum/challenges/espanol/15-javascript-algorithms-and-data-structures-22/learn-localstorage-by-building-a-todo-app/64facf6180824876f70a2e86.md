...
```js
assert.match(code, /if\s*\(\s*formInputsContainValues\s*\)\s*\{\s*confirmCloseDialog\.showModal\(\s*\);?/)
```

Your `if` statement should have an `else` block.

```js
assert.match(code, /if\s*\(\s*formInputsContainValues\s*\)\s*\{\s*confirmCloseDialog\.showModal\(\s*\);?\s*\}\s*else\s*\{/)
```

You should call the `reset()` function in the `else` block of your `if` statement.

```js
assert.match(code, /if\s*\(\s*formInputsContainValues\s*\)\s*\{\s*confirmCloseDialog\.showModal\(\s*\);?\s*\}\s*else\s*\{\s*reset\(\s*\);?\s*\}/)
```
