...
Un elemento `label` es como una etiqueta que ayuda a asociar un texto a un elemento `input` (Especialmente para tecnologías de asistencia como lectores de pantalla). Por ejemplo, `<label><input type="radio"> cat</label>` hace que al hacer click en la palabra `cat` se seleccione el radio button correspondiente.
