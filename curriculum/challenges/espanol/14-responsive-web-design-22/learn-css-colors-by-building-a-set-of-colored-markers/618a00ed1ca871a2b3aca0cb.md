...
Primero, en las reglas `.one`, `.two`, y `.three`, ajusta los valores de la función `rgb` de manera que la propiedad `background-color` de cada elemento se establezca en negro puro. Recuérdese que la función `rgb` utiliza el modelo aditivo, en el que los colores comienzan como negro y cambian a medida que los valores de rojo, verde y azul aumentan.
