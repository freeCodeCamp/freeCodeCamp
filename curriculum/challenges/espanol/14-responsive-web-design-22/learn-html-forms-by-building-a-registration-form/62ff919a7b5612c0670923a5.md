...
El primer elemento `label` debe tener un atributo `for` con el valor `profile-picture`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[0]?.matches('label[for="profile-picture"]'))
```

El segundo elemento `label` debe tener un atributo `for` con el valor `age`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[1]?.matches('label[for="age"]'))
```

El tercer elemento `label` debe tener un atributo `for` con el valor `referrer`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[2]?.matches('label[for="referrer"]'))
```

El cuarto elemento `label` debe tener un atributo `for` con el valor `bio`.
