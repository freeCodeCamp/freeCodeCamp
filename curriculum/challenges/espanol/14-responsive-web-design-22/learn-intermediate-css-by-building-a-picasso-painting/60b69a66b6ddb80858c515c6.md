...
Debes crear un nuevo selector `.left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left'));
```

Tu selector `.left` debe tener una propiedad `position` con el valor `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left')?.position === 'absolute');
```

Tu selector `.left` debe tener una propiedad `top` con el valor `15px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left')?.top === '15px');
```

Tu selector `.left` debe tener una propiedad `left` con el valor `30px`.
