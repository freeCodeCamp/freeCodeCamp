...
Podrías seguir probando diferentes porcentajes para la propiedad width (ancho). En cambio, usa la tecla de retroceso (borrar) en tu teclado para mover el elemento `p` con la clase `price` hasta colocarlo al lado del elemento `p` con la clase `flavor` para que estén en la misma línea en el editor. Asegúrate de que no haya espacio entre ellos.
