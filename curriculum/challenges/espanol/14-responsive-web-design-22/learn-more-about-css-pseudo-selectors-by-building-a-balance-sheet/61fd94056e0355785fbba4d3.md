...
Dentro del segundo `tr`, agrega un elemento `th` con el texto `Expenses Annual anticipated expenses, such as payroll.`. Envuelve el texto, excepto por `Expenses`, dentro de un elemento `span` con el `class` establecido en `description`.
