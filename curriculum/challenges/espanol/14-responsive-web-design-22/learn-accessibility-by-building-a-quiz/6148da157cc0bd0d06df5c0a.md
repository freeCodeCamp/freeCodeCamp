...
Para alinear las cajas `input` entre sí, establece la propiedad `display` a `inline-block` para todos los elementos `input` y `label` dentro de los elementos `.info`.

Además, alinee el texto a la derecha.
...
Debes dar a los elementos `input` y `label` un `text-align` de `right`.
...
Debes establecer la propiedad `text-align: right` antes de la regla `.info input`.
