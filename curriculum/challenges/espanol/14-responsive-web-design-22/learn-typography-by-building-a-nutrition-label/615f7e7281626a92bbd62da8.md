...
Debajo de su último elemento `p`, cree otro elemento `p` con el texto `Sodium 160mg 7%`. Envuelva el texto `Sodium` en un elemento `span` con un atributo `class` establecido en `bold`. Wrap the `7%` text in another `span` element with the `class` set to `bold`. Also add an additional `span` element around `Sodium 160mg` for aligning it correctly.
