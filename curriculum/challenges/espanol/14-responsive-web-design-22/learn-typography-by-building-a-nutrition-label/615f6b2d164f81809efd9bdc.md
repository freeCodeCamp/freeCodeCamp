...
Después de su último elemento `.divider`, cree un elemento `p` y asígnele el texto `Total Fat 8g 10%`. Envuelva `Total Fat` en un elemento `span` con la `class` configurada en `bold`. Wrap `10%` in another `span` element with the `class` set to `bold`. Finally, nest these two `span` elements in an additional `span` element for alignment.
