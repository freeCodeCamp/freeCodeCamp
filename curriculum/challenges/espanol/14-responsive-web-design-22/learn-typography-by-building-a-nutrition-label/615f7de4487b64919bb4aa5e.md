...
Tu elemento `p` debe tener tres elementos `span`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelectorAll('span')?.length === 3);
```

Su primer elemento `span` `Cholesterol` deben tener el atributo `class` establecido en `bold`.
...
Tu primer elemento `span` debe envolver el texto `Cholesterol 0mg`.
...
Su segundo elemento `span` debe tener el atributo `class` establecido en `bold`.
