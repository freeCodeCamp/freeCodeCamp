...
Puedes añadir parámetros `from`, `to` y `limit` a una petición `GET /api/users/:_id/logs` para recuperar parte del log de cualquier usuario. `from` y `to` child fechas en formato `yyyy-mm-dd`. `limit` es un número entero de cuántos logs hay que devolver.
