...
ماذا لو لم يكن لدى اللاعب ما يكفي من الذهب (gold) لشراء الصحة (health)؟ عندما تريد تشغيل الكود بشروط معيَّنة، يمكنك استخدام تعبير `if`. ضع كل الكود في وظيفة `buyHealth` داخل تعبير `if`. على سبيل المثال:
...
في الوقت الحالي، اتبع هذا النمط لاستخدام المقطع النصي `condition` داخل تعبير `if`.
