...
مثل Flexbox، مع CSS Grid يمكنك محاذاة محتوى عناصر الـ grid باستخدام `align-items` و `justify-items`. `align-items` ستعمل على محاذاة العناصر الفرعية (child elements) على طول محور العمود (column)، و `justify-items` ستعمل على محاذاة العناصر الفرعية على طول محور الصف (row).
