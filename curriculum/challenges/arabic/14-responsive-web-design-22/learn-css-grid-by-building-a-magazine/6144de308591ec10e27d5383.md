...
قم بانشاء منتقي `h1` و تعيين خاصية `font-family` الي `Anton` مع fallback بقيمة `sans-serif`.
...
منتقي `h1` يجب أن يكون لديه خاصية `font-family` بقيمة `Anton`، مع fallback بقيمة `sans-serif`.
