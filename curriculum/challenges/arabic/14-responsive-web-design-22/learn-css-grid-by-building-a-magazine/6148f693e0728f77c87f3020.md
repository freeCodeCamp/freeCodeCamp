...
قم بإنشاء query ثالث للـ `@media` لـ `only screen` مع `max-width` من `550px`. ضمنها، أنشئ منتقي `.hero-title` مع خاصية `font-size` بقيمة `6rem`, ومنتقي `.hero-subtitle, .author, .quote, .list-title` مع خاصية `font-size` بقيمة `1.8rem`, ومنتقي `.social-icons` مع خاصية `font-size` بقيمة `2rem`, ومنتقي `.text` مع خاصية `font-size` بقيمة `1.6rem`.
