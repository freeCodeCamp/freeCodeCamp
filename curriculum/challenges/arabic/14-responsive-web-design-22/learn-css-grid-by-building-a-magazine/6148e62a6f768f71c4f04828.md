...
يمكن استخدام خاصية `place-items` لتعيين `align-items` و `justify-items` في نفس الوقت. تأخذ خاصية `place-items` قيمة أو قيمتين. إذا تم توفير قيمة واحدة، فإنه يتم استخدامها لكل من خصائص `align-items` و `justify-items`. إذا تم توفير قيمتين، القيمة الأولى تستخدم لخاصية `align-items` والقيمة الثانية تستخدم لـ `justify-items`.
