...
ضمن العنصر الخاص بك `article`، قم بإنشاء عنصر `h3` مع `class` بقيمة `list-title` والنص `A Brief History`. أسفل ذلك، أنشئ عنصر `p` مع النص `Of the Curriculum`. ثم قم بإنشاء عنصر `ul` مع class يدعي `lists`.
