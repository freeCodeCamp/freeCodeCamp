...
داخل عنصر `.image-wrapper` الثالث، اعطي ثاني عنصر `img` سمة `src` بقيمة `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg` و `alt` بقيمة `four people working on code` و سمة `loading` بقيمة `lazy` و سمة `class` بقيمة `image-3`و سمة `width` بقيمة `600`و سمة `height` بقيمة `400`.
