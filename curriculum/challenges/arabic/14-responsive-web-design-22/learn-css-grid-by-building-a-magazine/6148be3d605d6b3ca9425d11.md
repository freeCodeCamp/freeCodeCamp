...
ويحتاج العنوان الفرعي (subtitle) أيضا إلى أن يكون styled. قم إنشاء منتقي `.hero-subtitle` وإعطائه خاصية `font-size` تم تعيينها إلى `2.4rem` و الخاصية `color` التي تم تعيينها إلى `orangered` و خاصية `text-align` تم تعيينها إلى `center`.
