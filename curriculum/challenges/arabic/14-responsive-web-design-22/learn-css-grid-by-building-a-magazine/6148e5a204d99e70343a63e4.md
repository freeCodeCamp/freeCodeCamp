...
خاصية `gap` هي طريقة مختصرة لتعيين قيمة `column-gap` و `row-gap` في نفس الوقت. إذا أعطيت قيمة واحدة، فإنها تعين `column-gap` و `row-gap` الي هذه القيمة. إذا أعطيت قيمتين، فإنها تعين `row-gap` للقيمة الأولى و `column-gap` للثانية.
