...
داخل عنصر `.image-wrapper` الثاني، اعطي ثاني عنصر `img` سمة `src` بقيمة `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png` و `alt` بقيمة `image of a calculator project` و سمة `loading` بقيمة `lazy` و سمة `class` بقيمة `image-2`و سمة `width` بقيمة `400`و سمة `height` بقيمة `400`.
