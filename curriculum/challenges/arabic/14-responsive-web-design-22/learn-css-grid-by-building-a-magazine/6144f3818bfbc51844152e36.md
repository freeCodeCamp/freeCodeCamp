...
استخدم دالة `minmax` لجعل الأعمدة (columns) الخاصة بك مستجيبة (responsive) على أي جهاز. تتطلب دالة `minmax` حجتين اي 2 arguments، الأولى هي الحد الأدنى للقيمة والثانية هي الحد الأقصى. يمكن أن تكون هذه القيم طولا أو نسبة مئوية أو `fr` أو حتى keyword مثل `max-content`.
