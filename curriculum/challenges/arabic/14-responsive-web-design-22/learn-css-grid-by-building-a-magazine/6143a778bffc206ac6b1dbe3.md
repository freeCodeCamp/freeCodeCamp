...
أضف أخر عنصر `p` نهاية عنصر `.text` الخاص بك، وقم بإعطائه النص التالي:
...
يجب عليك إنشاء عنصر `p` سادس بداخل عنصر `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 7)
```

يجب أن يحتوي العنصر `p` السادس على النص المطلوب.
