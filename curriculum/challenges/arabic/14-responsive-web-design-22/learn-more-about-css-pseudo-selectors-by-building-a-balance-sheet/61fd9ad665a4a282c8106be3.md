...
داخل `thead`، أنشئ عنصر `tr`. في داخلها، أضف عنصر `td` و ثلاث عناصر `th`. بداخل كل عناصر `th` ضف عنصر `span` مع تعيين `class` إلى `sr-only` والنص التالي. بالترتيب: `2019`، و`2020`، و `2021`.
