...
في عنصر `tr` الرابع ، أضف عنصر `th` مع النص `Total Liabilities`. قم بتغليف النص `Liabilities` في `span` مع سمة `class` تم تعيينها إلى `sr-only`.

بعد ذلك، أضف ثلاثة عناصر `td` مع النصوص التالية (بالترتيب): `$750` و `$600` و `$475`. اعطي عنصر `td` الثالث السمة `class` التي تم تعيينها إلى `current`.
