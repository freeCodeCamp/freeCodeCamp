...
في `tr` الثاني، قم بإضافة عنصر `th` مع النص `Expenses Annual anticipated expenses, such as payroll.`. غلف ذلك النص، باستثناء `Expenses`، ضمن عنصر `span` مع `class` تم تعيينه إلى `description`.
