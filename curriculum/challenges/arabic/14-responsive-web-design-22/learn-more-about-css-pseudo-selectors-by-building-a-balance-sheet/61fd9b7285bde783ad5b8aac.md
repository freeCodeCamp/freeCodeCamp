...
بداخل عنصر `tbody`، ضف `tr` مع `class` يسمي `total`. في ذلك، أضف `th` مع النص `Total Net Worth`، وغلف `Net Worth` في `span` مع `class` يدعى `sr-only`.

ثم أضف ثلاث عناصر `td`، و اعطي الثالث `class` تم تعيينه إلى `current`، ويعطى كل منهم نص على النحو التالي: `$-171`, و`$136`, و`$334`.

# --hints--

يجب أن يحتوي عنصر `tbody` الخاص بك على عنصر `tr` واحد.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('tbody')?.querySelectorAll('tr')?.length === 1);
```

عنصر `tr` الخاص بك يجب أن يحتوي على `class` يسمي `total`.
...
يجب أن يحتوي عنصر `th` الخاص بك على النص `Total Net Worth`.
...
يجب أن يغلف النص `Net Worth` في `span`.
