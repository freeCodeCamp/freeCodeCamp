...
في المبنى التالي! أدخل عنصرين `div` ضمن `.fb2` ومنحهم فئات `fb2a` و `fb2b`، بهذا الترتيب.

# --hints--

يجب عليك إضافة عنصرين `div` إلى `.fb2`.

```js
assert.equal(document.querySelectorAll("div.fb2 > div")?.length, 2);
```

يجب عليك إعطاء أول `div` جديد فئة `fb2a`.
...
يجب عليك إعطاء ثاني `div` جديد فئة `fb2b`.
