...
تريد إضافة نفس معامل التدرج إلى القسمين التاليين. بدلاً من ذلك، أنشئ منتقي لفئة جديدة يسمى `bb1-window`، وانقل الخصائص `height` و `background` وقيمهم من `.bb1a` إلى منتقي الفئة الجديد.
