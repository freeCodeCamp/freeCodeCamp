...
المساحات في CSS هي طريقة للانتقال بين الألوان عبر مسافة العنصر. يتم تطبيقها على خاصية `background` وبناء الجملة يبدو مثل:
...
يجب عليك إعطاء `background` بنوع `linear-gradient`.
