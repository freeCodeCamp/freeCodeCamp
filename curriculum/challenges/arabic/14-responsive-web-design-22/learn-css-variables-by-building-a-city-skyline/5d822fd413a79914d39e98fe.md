...
بعد إضافة هذه, يمكنك أن ترى كيف أن الحدود العنصر السميكة تعطيك بعض الزوايا حيث يلتقي جانبانها. ستستخدم تلك الحدود السفلى كسطح المبنى.

# --hints--

يجب عليك إعطاء `.bb2a` خاصية `margin` من `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.margin, "auto");
```

يجب عليك إعطاء `.bb2a` خاصية `width` بقيمة `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.width, "5vw");
```

يجب عليك إعطاء `.bb2a` خاصية `height` بقيمة `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.height, "5vw");
```

يجب عليك إعطاء `.bb2a` خاصية `border-top` بقيمة `1vw solid #000`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderTop, "1vw solid rgb(0, 0, 0)");
```

يجب عليك إعطاء `.bb2a` خاصية `border-bottom` بقيمة `1vw solid #000`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderBottom, "1vw solid rgb(0, 0, 0)");
```

يجب عليك إعطاء `.bb2a` خاصية `border-left` بقيمة `1vw solid #999`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderLeft, "1vw solid rgb(153, 153, 153)");
```

يجب عليك إعطاء `.bb2a` خاصية `border-right` بقيمة `1vw solid #999`.
