...
عنصر `input` الخاص بك يجب أن يحتوي على سمة `type` بقيمة `radio`. إما أنك حذفت القيمة أو لديك خطأ إملائي. تذكر أن قيم السمة يجب أن تكون محاطة بعلامات اقتباس.
...
على الرغم من أنك قمت بتعيين عنصر `input` سمة `type` إلى `radio`، يوصى بأن تحيط دائما قيمة السمة بعلامات اقتباس.

```js
assert(!/\<\s*input\s+type\s*=\s*radio/i.test(code));
```

يجب أن يتواجد النص `Indoor` لزر الـ `radio`، بعد الرز وليس قبله.
...
النص `Indoor` يجب أن يكون مباشرة على يمين زر الـ `radio`. إما أنك حذفت النص أو لديك خطأ إملائي.
