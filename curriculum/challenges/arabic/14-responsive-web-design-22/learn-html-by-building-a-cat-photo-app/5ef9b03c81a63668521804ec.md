...
لاحظ أن محتويات الصفحة بأكملها مدمجة داخل عنصر `html`. The `html` element is the root element of an HTML page and wraps all content on the page.

You can also specify the language of your page by adding the `lang` attribute to the `html` element.

أضف السمة `lang` مع القيمة `en` إلى opening tag الـ `html` لتحديد أن لغة الصفحة هي الإنجليزية.

# --hints--

إما أنك قمت بحذف عنصر `html` أو أنه يفتقد علامة فتح أو إغلاق.
