...
ضمن query الـ `@media` ، أضف منتقي `.keys` وقم بتعيين `width` إلى `318px`.

# --hints--


قاعدة `@media` الخاصة بك يجب أن تحتوي على منتقى `.keys`.
...
يجب أن يكون لمنتقي `.keys` الجديد الخاص بك `width` من `318px`.
