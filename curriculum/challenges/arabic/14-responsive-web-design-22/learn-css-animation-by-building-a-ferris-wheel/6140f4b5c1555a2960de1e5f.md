...
قم بإنشاء قاعدة أخرى `@keyframes` باسم `cabins`. استخدم نفس الخصائص كـ `@keyframes wheel`، بنسخ كل من قواعد `0%` و `100%` ولكن مع تعيين خاصية `transform` لمنتقي `100%` إلى `rotate(-360deg)`.
