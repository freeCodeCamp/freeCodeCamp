...
استهدف الـ `class` بقيمة `right` وعين `position` إلى `absolute` و `top` إلى `15px`و `right` إلى `30px`.
...
يجب أن يكون لمنتقي `.right` الخاص بك الخاصية `position` بقيمة `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.right')?.position === 'absolute');
```

يجب أن يكون لمنتقي `.right` الخاص بك الخاصية `top` بقيمة `15px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.right')?.top === '15px');
```

يجب أن يكون لمنتقي `.right` الخاص بك الخاصية `right` بقيمة `30px`.
