...
يجب أن يكون لمنتقي `#black-round-hat` الخاص بك خاصية `position` بقيمة `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-round-hat')?.position === 'absolute');
```

يجب أن يحتوي منتقي `#black-round-hat` على خاصية `top` بقيمة `-100px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-round-hat')?.top === '-100px');
```

يجب أن يحتوي منتقي `#black-round-hat` على خاصية `left` بقيمة `5px`.
