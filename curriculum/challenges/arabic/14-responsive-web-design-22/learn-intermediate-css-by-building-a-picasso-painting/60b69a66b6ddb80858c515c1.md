...
يجب عليك إنشاء منتقي `#guitar-neck`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-neck'));
```

يجب أن يكون لدى منتقي `#guitar-neck` خاصية `width` تم تعيينها إلى `200px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-neck')?.width === '200px');
```

يجب أن يكون لدى منتقي `#guitar-neck` خاصية `height` تم تعيينها إلى `30px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#guitar-neck')?.height === '30px');
```

يجب أن يكون لمنتقي `#guitar-neck` الخاص بك خاصية `background-color` بقيمة `#D2691E`.
