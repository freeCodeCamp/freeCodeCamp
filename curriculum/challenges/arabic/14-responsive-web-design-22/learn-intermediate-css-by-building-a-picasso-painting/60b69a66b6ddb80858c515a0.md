...
يجب أن يكون لمنتقيك `#black-hat` خاصية `position` بقيمة `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.position === 'absolute');
```

يجب يحمل منتقيك `#black-hat` الخاصية `top` بقيمة `-150px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.top === '-150px');
```

يجب أن يحتوي منتقيك `#black-hat` على خاصية `left` بقيمة `0`.
