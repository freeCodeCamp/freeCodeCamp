...
يجب أن يكون لديك منتقي `#blue-right`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-right'));
```

يجب أن يكون لمنتقي `#blue-right` الخاص بك الخاصية `width` بقيمة `400px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#blue-right')?.width === '400px');
```

يجب أن يكون لمنتقي `#blue-right` الخاص بك الخاصية `height` بقيمة `300px`.
