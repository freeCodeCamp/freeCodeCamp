...
يجب عليك إنشاء منتقي `.guitar`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.guitar'));
```

يجب أن يكون لمنتقي `.guitar` الخاص بك الخاصية `width` بقيمة `150px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.guitar')?.width === '150px');
```

يجب أن يكون لمنتقي `.guitar` الخاص بك الخاصية `height` بقيمة `120px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.guitar')?.height === '120px');
```

يجب أن يكون لمنتقي `.guitar` الخاص بك خاصية `background-color` محددة إلى `Goldenrod`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.guitar')?.backgroundColor === 'goldenrod');
```

يجب أن يكون لمنتقي `.guitar` الخاص بك الخاصية `border-radius` بقيمة `50%`.
