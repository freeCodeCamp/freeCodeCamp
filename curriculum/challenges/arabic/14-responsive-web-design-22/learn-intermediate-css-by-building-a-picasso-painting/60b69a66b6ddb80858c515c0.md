...
يجب عليك إنشاء منتقي `.fa-bars`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.fa-bars'));
```

يجب أن يحتوي منتقي `.fa-bars` على خاصية `display` بقيمة `block`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.fa-bars')?.display === 'block');
```

يجب أن يكون لمنتقي `.fa-bars` الخاص بك خاصية `margin-top` بقيمة `30%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.fa-bars')?.marginTop === '30%');
```

يجب أن يكون لمنتقي `.fa-bars` الخاص بك خاصية `margin-left` بقيمة `40%`.
