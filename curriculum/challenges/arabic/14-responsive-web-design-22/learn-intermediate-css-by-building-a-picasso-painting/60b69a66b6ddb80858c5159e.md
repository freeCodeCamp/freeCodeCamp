...
يجب أن يكون لديك منتقي `#black-hat`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat'));
```

يجب أن يحتوي منتقيك `#black-hat` على خاصية `width` بقيمة `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.width === '0px');
```

يجب أن يحتوي منتقيك `#black-hat` على خاصية `height` بقيمة `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.height === '0px');
```

يجب أن يحتوي منتقيك `#black-hat` على خاصية `border-style` بقيمة `solid`.
