...
يجب أن يكون لمنتقي `#orange-character` الخاص بك خاصية `position` بقيمة `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.position === 'absolute');
```

يجب أن يحتوي منتقي `#orange-character` على خاصية `top` بقيمة `25%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#orange-character')?.top === '25%');
```

يجب أن يحتوي منتقي `#orange-character` على خاصية `left` بقيمة `40%`.
