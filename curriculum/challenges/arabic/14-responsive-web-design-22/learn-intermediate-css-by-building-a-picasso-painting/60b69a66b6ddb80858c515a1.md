...
يجب أن يكون لديك منتقي `#gray-mask`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask'));
```

يجب أن يحتوي منتقي `#gray-mask` على خاصية `width` بقيمة `150px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.height === '150px');
```

يجب أن يحتوي منتقي `#gray-mask` على خاصية `height` بقيمة `150px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.height === '150px')
```

يجب أن يكون لدي منتقي `#gray-mask` خاصية `background-color` بقيمة `rgb(167, 162, 117)`.
