...
يجب أن يكون لمنتقي `#gray-mask` الخاص بك خاصية `position` بقيمة `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.position === 'absolute');
```

يجب أن يحتوي منتقي `#gray-mask` على خاصية `top` بقيمة `-10px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#gray-mask')?.top === '-10px');
```

يجب أن يحتوي منتقي `#gray-mask` على خاصية `left` بقيمة `70px`.
