...
يجب أن يكون لمنتقيك `#eyes-div` خاصية `position` بقيمة `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.position === 'absolute');
```

يجب أن يحتوي منتقيك `#eyes-div` على خاصية `top` بقيمة `-40px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#eyes-div')?.top === '-40px');
```

يجب أن يكون لمنتقيك `#eyes-div` الخاصية `left` بقيمة `20px`.
