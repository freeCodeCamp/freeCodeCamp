...
يجب عليك إنشاء منتقي `.triangle` جديد.

```js
assert(new __helpers.CSSHelp(document).getStyle('.triangle'));
```

يجب أن يكون لمنتقي `.triangle` الخاص بك الخاصية `width` بقيمة `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.triangle')?.width === '0px');
```

يجب أن يكون لمنتقي `.triangle` الخاص بك الخاصية `height` بقيمة `0`.
