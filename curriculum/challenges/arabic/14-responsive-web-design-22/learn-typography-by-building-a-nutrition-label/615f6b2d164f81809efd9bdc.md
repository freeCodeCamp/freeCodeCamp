...
بعد آخر عنصر `.divider`، أنشئ عنصر `p` وأعطيه النص `Total Fat 8g 10%`. ادخل `Total Fat` في `span` مع `class` بقيمة `bold`. غلف نص `10%` داخل `span` أخرى، مع `class` بقيمة `bold`. أخيراً، أدخل عنصر `span` الذي يحتوي على نص `Total Fat` مع نص `8g` في عنصر `span` إضافي للمحاذاة.
