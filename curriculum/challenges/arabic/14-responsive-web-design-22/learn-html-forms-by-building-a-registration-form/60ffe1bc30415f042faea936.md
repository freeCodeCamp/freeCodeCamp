...
لجعل عناصر `input` و `textarea` تنسجم مع سمة الخلفية، قم بتعيين `background-color` إلى `#0a0a23`. ثم أعطيهم `1px` و `solid` للـ border بلون `#0a0a23`.

# --hints--

يجب عليك استخدام منتقي عناصر مفصول بفواصل لتحديد عناصر `input` و `textarea`.
...
يجب عليك إعطاء عناصر `input` و `textarea` الـ `background-color` بقيمة `#0a0a23`.
...
يجب عليك إعطاء عناصر `input` و `textarea` الـ border بقيمة `1px` و `solid` بلون `#0a0a23`.
