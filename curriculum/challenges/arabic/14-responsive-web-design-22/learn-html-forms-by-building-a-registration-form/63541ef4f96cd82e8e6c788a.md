...
تحدد سمة `method` كيفية إرسال بيانات النموذج إلى عنوان URL المحدد في سمة `action`. يمكن إرسال بيانات الاستمارة عبر طلب `GET` كحجج عنوان URL (بواسطة `method="get"`) أو عبر طلب `POST` كبيانات في هيئة الطلب (بواسطة `method="post"`).
