...
استهدف عناصر الـ unordered list داخل عناصر `nav`، واستخدم _Flexbox_ لتوزيع المسافات بين الأطفال بشكل متساوٍ.

# --hints--

يجب عليك استخدام منتقي `nav > ul`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('nav > ul'));
```

يجب عليك إعطاء عناصر `nav > ul` الـ `display` بقيمة `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('nav > ul')?.display, 'flex');
```

يجب عليك إعطاء عناصر `nav > ul` الـ `justify-content` بقيمة `space-evenly`.
