...
بهذه الخطوة الأخيرة، اكتملت الآن لوحة Rothko الخاصة بك.

# --hints--

يجب عليك تعيين الخاصية `transform` إلى `rotate(-0.2deg)`.
...
يجب أن يكون لعنصر `.three` الخاص بك `transform` بقيمة `rotate(-0.2deg)`.
