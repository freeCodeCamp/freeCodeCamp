...
استهدف عنصر `.back-mountain` وعيّن `width` و `height` إلى `300px`. ثم عيّن `background` إلى التدريج الخطي (linear gradient) يبدأ في `rgb(203, 241, 228)`, وينتهي في `rgb(47, 170, 255)`.
