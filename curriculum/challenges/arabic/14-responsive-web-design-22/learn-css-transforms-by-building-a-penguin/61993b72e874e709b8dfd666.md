...
استهدف عنصر `.penguin-head` وعيّن `width` بقيمة نصف قيمة العنصر الحاوي, و `height` إلى `45%`. ثم عيّن `background` إلى متدرج خطي (linear gradient) بدارجة `45deg` يبدأ في `gray` وينتهي في `rgb(239, 240, 228)`.
