...
في هذا المثال، يتم محاكاة طلب غير متزامن مع مكالمة `setTimeout()`. من الشائع إرسال إجراء قبل الشروع في أي سلوك غير متزامن, حتى يتعرف حالة التطبيق الخاص بك على بعض البيانات مطلوبة (هذه الحالة يمكن أن تعرض أيقونة التحميل, مثلاً). بعد ذلك، بمجرد تلقي البيانات، ارسل إجراء آخر يحمل البيانات كحمولة مربوطة بالمعلومات التي تم إكمالها.
...
إرسال `requestingData` يجب أن يقوم منشئ الإجراءات بتحديث خاصية `state` المتجر لجلب `true`.
