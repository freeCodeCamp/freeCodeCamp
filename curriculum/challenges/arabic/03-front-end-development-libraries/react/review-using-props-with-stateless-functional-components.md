...
title: مراجعة باستخدام مِيزات Props مع المكونات الوظيفية عديمة الحالة (Stateless Functional Components)
challengeType: 6
forumTopicId: 301411
...
باستثناء التحدي الأخير، كنت تمرير المِيزات (props) إلى مكونات وظيفية عديمة الجنسية (tateless functional components). هذه المكونات تعمل مثل وظائف خالصة (pure functions). فهم يقبلون الميزات مثل مدخلات وينتجون نفس النظرة في كل مرة تمرر فيها نفس الميزات. قد تتساءل ما هي الحالة (state)، التحدي التالي سيغطيها بمزيد من التفصيل. وقبل ذلك، هنا استعراض لمصطلحات المكونات.

*مكون وظيفي عديم الحالة (stateless functional component)* هو وظيفية تكتبها تقبل الميزات (props) و تنتج JSX. *مكون عديم الحالة (stateless component)*، من ناحية أخرى، هو فئة تمدد `React.Component`، ولكنه لا يستخدم الحالة الداخلية (internal state) (المشمولة في التحدي التالي). وأخيرا، فإن *مكون حالة (stateful component)* هو مكون فئة يحتفظ بحالته الداخلية. قد ترى المكونات الحالة المشار إليها ببساطة مثل مكونات أو مكونات React.

ومن الأنماط الشائعة محاولة التقليل إلى أدنى حد من الوضع الحالة واستحداث عناصر وظيفية عديمة الحالة كلما أمكن ذلك. هذا يساعدك في احتواء إدارة الحالة الخاصة بك إلى منطقة معينة في تطبيقك. يؤدي هذا بدوره إلى تحسين تطوير وصيانة التطبيق الخاص بك, عن طريق تيسير متابعة كيفية تأثير التغييرات في الحالة على سلوكها.
