...
في التحديات السابقة، تعلمت كيفية حقن تعليمات JavaScript البرمجية في تعليمات JSX البرمجية باستخدام الأقواس المنحنية، `{ }`، للمهام مثل الوصول إلى مِيزات، وتمرير مِيزات، والوصول إلى الحالة، وإدراج التعليقات في الكود الخاص بك، ومؤخراً تصميم المكونات الخاصة بك. هذا كلها حالات استخدام شائعة لوضع JavaScript في JSX، لكنهم ليسوا الطريقة الوحيدة التي يمكنك بها استخدام تعلميات JavaScript البرمجية في مكونات React الخاصة بك.

يمكنك أيضًا كتابة JavaScript قاصدًا في طرق `render` الخاصة بك، قبل بيان `return`، و***دون*** إدراجه داخل أقواس منحنية (curly braces). هذا لأنه ليس ضمن تعلميات JSX البرمجية. عندما تريد استخدام متغير في وقت لاحق في تعليمات JSX البرمجية *داخل* بيان `return`، ضع اسم المتغير داخل الأقواس المنحنية (curly braces).

# --instructions--

في التعليمات البرمجية المقدمة، طريقة `render` تحتوي على مصفوفة التي تحتوي على 20 عبارة لتمثيل الإجابات الموجودة في لُعْبَة Magic Eight Ball toy بالثمانينيات. أن حدث النقر فوق الزر مرتبط بطريقة `ask` لذلك في كل مرة يتم النقر على الزر سيتم إنشاء رَقَم عشوائي وتخزينه كمؤشر في الحالة `randomIndex`. في السطر 52، حذف string بقيمة `change me!` وإعادة تعيين `answer` حتى تصل التعليمات البرمجية الخاصة بك عشوائيا إلى ترتيب مختلف في مصفوفة `possibleAnswers` كل مرة يتم فيها تحديث المكون. أخيرا، أدخل ثابت `answer` داخل العلامات `p`.
...
يجب تهيئة حالة `MagicEightBall` بخاصية `userInput` وخاصية `randomIndex` وكلاهما مجموعة إلى قيمة string فارغة.
