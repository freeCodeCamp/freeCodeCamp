...
Füge unter dem `.blush.right`-Element zwei `div`-Elemente mit einer `class` von `beak` hinzu. Gib ebenfalls dem ersten `.beak`-Element eine `class` von `top` und dem zweiten `.beak`-Element eine `class` von `bottom`.
