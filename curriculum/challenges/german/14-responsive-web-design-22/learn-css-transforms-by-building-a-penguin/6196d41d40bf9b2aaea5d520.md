...
Du solltest `.sun` eine `top`-Eigenschaft mit dem Wert `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.top, '-75px');
```

Du solltest `.sun` eine `right`-Eigenschaft mit dem Wert `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.
