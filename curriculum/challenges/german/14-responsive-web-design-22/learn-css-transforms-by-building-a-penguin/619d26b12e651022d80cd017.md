...
Wähle die `.arm`-Elemente aus und gib ihnen eine `width` mit dem Wert `30%`, eine `height` mit dem Wert `60%` und einen `background` in Form eines linearen Farbverlaufs mit `90deg` im Uhrzeigersinn, der bei `gray` beginnt und bei `rgb(209, 210, 199)` endet.
