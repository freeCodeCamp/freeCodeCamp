...
Visiere das `.penguin-head`-Element an und gib ihm die Hälfte der `width` seines übergeordneten Elements und eine `height` von `45%`. Setze dann den `background` auf einen linearen Farbverlauf bei `45deg`, der mit `gray` beginnt und mit `rgb(239, 240, 228)` endet.
