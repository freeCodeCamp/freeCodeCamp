...
Wähle das `.left-mountain`-Element aus und setze seine `width` und `height` auf `300px`. Stelle dann den `background` auf einen linearen Farbverlauf ein, der mit `rgb(203, 241, 228)` beginnt und mit `rgb(80, 183, 255)` endet.
...
Du solltest `.left-mountain` eine `width` von `300px` geben. `--fcc-actual--` wird erwartet `--fcc-expected--` zu sein.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.width, '300px');
```

Du solltest `.left-mountain` eine `height` von `300px` geben. `--fcc-actual--` wird erwartet `--fcc-expected--` zu sein.
