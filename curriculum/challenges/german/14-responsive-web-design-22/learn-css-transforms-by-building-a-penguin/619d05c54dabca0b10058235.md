...
Bislang haben die `.face`- und `.chin`-Elemente die gleiche `background-color`.

Erstelle eine benutzerdefinierte CSS-Eigenschaft namens `--penguin-face` und setze sie auf `white`.

# --hints--

Du solltest den `:root`-Selektor verwenden.

```js
assert.match(code, /:root\s*\{/);
```

Du solltest `:root` eine `--penguin-face`-Eigenschaft geben.
