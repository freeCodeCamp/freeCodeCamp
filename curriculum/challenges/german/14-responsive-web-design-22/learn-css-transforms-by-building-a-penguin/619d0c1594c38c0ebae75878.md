...
Du solltest den `.eye.left`-Selektor verwenden.

```js
assert.match(code, /\.eye\.left\s*\{/);
```

Du solltest `.eye.left` einen `left` von `--fcc-expected--` geben, aber hast `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye.left')?.left, '25%');
```

Du solltest den `.eye.right`-Selektor verwenden.

```js
assert.match(code, /\.eye\.right\s*\{/);
```

Du solltest `.eye.right` einen `right` von `--fcc-expected--` geben, aber hast `--fcc-actual--` gefunden.
