...
Füge in dein viertes `tr`-Element ein `th`-Element mit dem Text `Total Assets` hinzu. Bette den Text `Assets` in einem `span`-Element ein, welches das `class`-Attribut mit dem Wert `sr-only` enthält.
