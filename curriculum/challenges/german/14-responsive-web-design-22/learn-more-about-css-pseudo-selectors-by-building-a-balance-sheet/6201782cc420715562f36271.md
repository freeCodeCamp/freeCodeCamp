...
Gestalte den Text innerhalb deines `#years`-Elements, indem du einen `#years span[class]`-Selektor erstellst. The `span[class]` syntax will target any `span` element that has a `class` attribute set, regardless of the attribute's value.
