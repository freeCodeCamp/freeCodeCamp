...
Füge in dein viertes `tr`-Element ein `th`-Element mit dem Text `Total Liabilities` hinzu. Wrap the text `Liabilities` in a `span` element with the `class` attribute set to `sr-only`.

Füge daraufhin drei `td`-Elemente mit dem folgenden Text hinzu (in dieser Reihenfolge): `$750`, `$600`, `$475`. Give the third `td` element a `class` attribute set to `current`.
