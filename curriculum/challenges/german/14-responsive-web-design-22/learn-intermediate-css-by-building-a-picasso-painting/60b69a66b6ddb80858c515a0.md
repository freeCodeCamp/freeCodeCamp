...
Dein `#black-hat`-Selektor sollte eine `position`-Eigenschaft auf `absolute` gesetzt haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-hat')?.position === 'absolute');
```

Dein `#black-hat`-Selektor sollte eine `top`-Eigenschaft auf `-150px` gesetzt haben.
