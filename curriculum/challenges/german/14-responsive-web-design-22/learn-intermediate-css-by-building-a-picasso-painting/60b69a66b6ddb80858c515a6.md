...
FontAwesome icons come with their own styling to define the icon. However, you can still set the styling yourself as well, to change things like the color and size. Verwende zunächst einen Klassenselektor, um die Symbole mit der Klasse `fa-music` auszuwählen. Set the `display` to `inline-block`, the `margin-top` to `8%`, and the `margin-left` to `13%`.
