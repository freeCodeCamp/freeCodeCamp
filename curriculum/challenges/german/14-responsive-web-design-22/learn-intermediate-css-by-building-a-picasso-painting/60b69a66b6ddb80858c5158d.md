...
Dein `#black-mask`-Selektor sollte eine `position`-Eigenschaft von `absolute` haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-mask')?.position === 'absolute');
```

Dein `#black-mask`-Selektor sollte eine `top`-Eigenschaft von `0` enthalten.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-mask')?.top === '0px');
```

Dein `#black-mask`-Selektor sollte eine `left`-Eigenschaft von `0` haben.
