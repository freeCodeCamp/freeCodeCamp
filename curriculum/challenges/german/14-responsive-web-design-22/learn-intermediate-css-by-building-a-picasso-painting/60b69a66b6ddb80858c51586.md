...
Du solltest den `#offwhite-character`-Selektor verwenden.

```js
assert(new __helpers.CSSHelp(document).getStyle('#offwhite-character'));
```

Dein `#offwhite-character` sollte eine `width`-Eigenschaft von `300px` enthalten.

```js
assert(new __helpers.CSSHelp(document).getStyle('#offwhite-character')?.width === '300px');
```

Dein `#offwhite-character` sollte eine `height`-Eigenschaft von `550px` enthalten.

```js
assert(new __helpers.CSSHelp(document).getStyle('#offwhite-character')?.height === '550px');
```

Dein `#offwhite-character` sollte eine `background-color`-Eigenschaft von `GhostWhite` enthalten.
