...
Dein zweites neues `div`-Element sollte die `id` `black-mask` enthalten.

```js
assert(document.querySelectorAll('#offwhite-character div')[1]?.getAttribute('id') === 'black-mask');
```

Dein drittes neues `div`-Element sollte die `id` `gray-instrument` enthalten.

```js
assert(document.querySelectorAll('#offwhite-character div')[2]?.getAttribute('id') === 'gray-instrument');
```

Dein viertes neues `div`-Element sollte die `id` `tan-table` enthalten.
