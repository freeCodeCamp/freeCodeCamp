...
Select the `id` with value `guitar-right`, and also set `position` to `absolute`. Setze dieses Mal `left` auf `100px`.
...
Dein `#guitar-right`-Selektor sollte eine `position`-Eigenschaft auf `absolute` gesetzt haben.
