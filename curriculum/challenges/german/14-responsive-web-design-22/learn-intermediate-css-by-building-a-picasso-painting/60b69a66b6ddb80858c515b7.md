...
Bringe das `#black-round-hat`-Element an seinen Platz mit einer `position` von `absolute`, einem `top` von `-100px` und einem `left` von `5px`.
...
Dein `#black-round-hat`-Selektor sollte eine `top`-Eigenschaft von `-100px` enthalten.

```js
assert(new __helpers.CSSHelp(document).getStyle('#black-round-hat')?.top === '-100px');
```

Dein `#black-round-hat`-Selektor sollte eine `left`-Eigenschaft von `5px` haben.
