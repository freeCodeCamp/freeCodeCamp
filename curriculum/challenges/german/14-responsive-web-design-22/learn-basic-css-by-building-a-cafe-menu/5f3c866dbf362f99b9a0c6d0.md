...
Die `p`-Elemente sind innerhalb eines `article`-Elements mit dem Klassenattribut `item` verschachtelt. You can style all the `p` elements nested anywhere in elements with a class named `item` like this:
