...
Als Nächstes möchtest du das `#menu` horizontal zentrieren. Dies kannst du tun, indem du `margin-left`- und `margin-right`-Eigenschaften auf `auto` einstellst. Stelle dir den Rand als einen unsichtbaren Raum um ein Element herum vor. Mit diesen beiden Margin-Eigenschaften zentrierst du das `#menu`-Element innerhalb des `body`-Elements.
