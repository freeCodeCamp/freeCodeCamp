...
Nun ist es einfach zu sehen, dass der Text im `div` Element zentriert ist. Derzeit ist die Breite des `div` Elements in Pixeln angegeben (`px`). Change the `width` property's value to be `80%`, to make it 80% the width of its parent element (`body`).
