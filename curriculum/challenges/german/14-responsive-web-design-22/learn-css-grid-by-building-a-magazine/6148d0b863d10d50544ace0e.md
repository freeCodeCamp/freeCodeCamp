...
Der andere Text wurde an einen anderen Ort verschoben. Verschiebe ihn in Position, indem du dem `.first-paragraph::first-letter`-Selektor eine `float`-Eigenschaft mit dem Wert `left` und eine `margin-right`-Eigenschaft mit dem Wert `1rem` gibst.
