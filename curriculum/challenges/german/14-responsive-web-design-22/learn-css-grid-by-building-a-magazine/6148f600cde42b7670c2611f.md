...
Erstelle eine weitere `@media`-Anfrage für `only screen` mit einer `max-width` von `600px`. Erstelle innen eine `.text-with-images`-Regel und gib ihr eine `grid-template-columns`-Eigenschaft mit dem Wert `1fr`.
