...
Create one final `@media` query for `only screen` with a `max-width` of `420px`. Erstelle innerhalb der Media Query einen `.hero-title`-Selektor mit einer `font-size`-Eigenschaft mit dem Wert `4.5rem`.
