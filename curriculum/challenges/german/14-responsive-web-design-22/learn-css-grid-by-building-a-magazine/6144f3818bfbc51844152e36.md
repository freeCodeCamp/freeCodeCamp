...
Verwende die `minmax`-Funktion, um deine Spalten auf jedem Gerät reaktionsfähig zu machen. Die `minmax`-Funktion nimmt zwei Argumente, zum einen den Mindestwert und zum anderen das Maximum. Diese Werte können eine Länge, Prozentsatz, `fr` oder sogar ein Schlüsselwort wie `max-content` sein.
