...
Der Untertitel sollte ebenfalls gestaltet werden. Erstelle einen `.hero-subtitle`-Selektor und gib ihm eine `font-size`-Eigenschaft mit dem Wert `2.4rem`, eine `color`-Eigenschaft mit dem Wert `orangered` und eine `text-align`-Eigenschaft mit dem Wert `center`.
