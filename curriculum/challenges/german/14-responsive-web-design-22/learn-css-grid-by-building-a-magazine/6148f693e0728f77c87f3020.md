...
Erstelle eine dritte `@media`-Query für `only screen` mit einer `max-width` von `550px`. Erstelle innerhalb einen `.hero-title`-Selektor eine `font-size` mit dem Wert `6rem`, einen `.hero-subtitle, .author, .quote, .list-title`-Selektor mit der `font-size` von `1.8rem`, einen `.social-icons`-Selektor mit einer `font-size` von `2rem` und einem `.text`-Selektor mit einer `font-size` von `1.6rem`.
