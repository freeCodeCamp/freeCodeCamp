...
Du solltest innerhalb des `.image-quote`-Elements zwei `hr`-Elemente hinzufügen.

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

Du solltest innerhalb des `.image-quote`-Elements ein `p`-Element hinzufügen.

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

Die Elemente, die dem `.image-quote`-Element untergeordnet sind, sollten in der richtigen Reihenfolge stehen.
...
Dein neues `p`-Element sollte eine `class`-Eigenschaft mit dem Wert `quote` enthalten.

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Dein neues `p`-Element sollte den Text `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.` enthalten.
