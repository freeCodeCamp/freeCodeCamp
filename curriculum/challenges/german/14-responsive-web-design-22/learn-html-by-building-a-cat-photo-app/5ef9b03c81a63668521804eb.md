...
Das `title`-Element bestimmt, welche Browser in der Titelleiste oder im Tab der Seite angezeigt werden.
...
Dein `title`-Element sollte ein schließendes Tag haben. Schließende Tags haben ein `/` direkt nach dem `<` Zeichen.

```js
assert(code.match(/\<\/title\>/));
```

Your `title` element's text should be `CatPhotoApp`. Du hast entweder den Text weggelassen oder einen Tippfehler gemacht.
