...
Your new checkbox should be after the second one. Sie sind in falscher Reihenfolge.
...
The new `label` should have a `for` attribute with the same value as the `id` attribute of the new checkbox. Du hast entweder den Wert weggelassen oder einen Tippfehler gemacht.
