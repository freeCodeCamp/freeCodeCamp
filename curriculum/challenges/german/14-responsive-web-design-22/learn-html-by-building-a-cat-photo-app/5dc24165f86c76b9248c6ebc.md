...
Alle `img`-Elemente sollten ein `alt`-Attribut haben. Der Text des `alt`-Attributs wird für Screenreader verwendet, um die Zugänglichkeit zu verbessern und wird angezeigt, wenn das Bild nicht geladen wird. Zum Beispiel hat `<img src="cat.jpg" alt="A cat">` ein `alt`-Attribut mit dem Text `A cat`.
