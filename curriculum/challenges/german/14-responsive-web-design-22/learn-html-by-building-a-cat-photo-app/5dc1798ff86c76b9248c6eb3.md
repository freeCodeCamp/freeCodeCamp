...
Die `h1`- bis `h6`-Überschriftenelemente werden verwendet, um die Wichtigkeit der Inhalte unter ihnen zu signalisieren. Je kleiner die Zahl, desto höher die Wichtigkeit, so dass `h2`-Elemente weniger von Bedeutung sind als `h1`-Elemente. Benutze nur ein `h1`-Element pro Seite und platziere die Überschriften mit geringer Bedeutung unterhalb der Überschriften mit hoher Bedeutung.
