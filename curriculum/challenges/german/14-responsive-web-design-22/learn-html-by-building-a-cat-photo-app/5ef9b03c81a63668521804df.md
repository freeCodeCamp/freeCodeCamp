...
Your radio button should have an `id` attribute. Überprüfe, ob ein Leerzeichen nach dem Namen des öffnenden Tags und/oder Leerzeichen vor allen Attributnamen vorhanden sind.

```js
assert($('input')[0].hasAttribute('id'));
```

Your radio button should have a `type` attribute. Überprüfe, dass ein Leerzeichen nach dem Namen des öffnendes Tags und/oder Leerzeichen vor allen Attributnamen vorhanden sind.

```js
assert($('input')[0].hasAttribute('type'));
```

Your radio button should have an `id` attribute with the value `indoor`. Du hast entweder den Wert weggelasssen oder einen Tippfehler gemacht. Denke daran, dass Attributwerte zwischen Anführungszeichen stehen sollten.
