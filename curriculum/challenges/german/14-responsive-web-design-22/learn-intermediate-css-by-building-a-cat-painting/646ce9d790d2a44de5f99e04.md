...
Erstelle innerhalb deines `.cat-left-ear`-Elements ein weiteres `div`-Element mit der Klasse `cat-left-inner-ear`.
...
Du solltest ein `div`-Element innerhalb deines `.cat-left-ear`-Elements haben.

```js
assert(document.querySelectorAll('.cat-left-ear div').length === 1);
```

Dein `div`-Element sollte die Klasse `cat-left-inner-ear` enthalten.
