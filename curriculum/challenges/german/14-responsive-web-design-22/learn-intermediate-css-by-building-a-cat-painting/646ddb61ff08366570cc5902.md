...
Erstelle ein `div`-Element mit der Klasse `cat-eyes`. Erstelle innerhalb des `cat-eyes`-Elements zwei `div`-Elemente mit der Klasse `cat-left-eye` bzw. `cat-right-eye`.
