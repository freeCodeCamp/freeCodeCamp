...
Beginne mit der grundlegenden HTML-Struktur. Add a `DOCTYPE` reference of `html` and an `html` element with its `lang` attribute set to `en`. Füge außerdem ein `head`- und `body`-Element innerhalb des `html`-Elements hinzu.
