...
Erstelle innerhalb des `.cat-right-eye`-Elements ein weiteres `div`-Element mit der Klasse `cat-right-inner-eye`.
...
Du solltest ein `div`-Element innerhalb deines `.cat-right-eye`-Elements haben.

```js
assert(document.querySelectorAll('.cat-right-eye div').length === 1);
```

Dein `div`-Element sollte die Klasse `cat-right-inner-eye` enthalten.
