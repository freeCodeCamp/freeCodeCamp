...
Drehe `.three` um 0,2 Grad gegen den Uhrzeigersinn.

Mit diesem letzten Schritt ist dein Rothko-Bild nun fertig.

# --hints--

Du solltest die `transform`-Eigenschaft auf `rotate(-0.2deg)` setzen.
...
Dein `.three`-Element sollte einen `transform`-Wert von `rotate(-0.2deg)` haben.
