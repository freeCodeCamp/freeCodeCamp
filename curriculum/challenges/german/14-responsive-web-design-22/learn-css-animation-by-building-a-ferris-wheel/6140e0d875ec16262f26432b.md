...
Die `animation-iteration-count`-Eigenschaft legt fest, wie oft deine Animation wiederholt werden soll. Das kann eine Zahl oder auch `infinite` sein, wenn man die Animation unendlich wiederholen möchte. Dein Ferris-Wheel sollte nie anhalten, deshalb setzt du die `animation-iteration-count` deines `.wheel`-Selektors auf `infinite`.
