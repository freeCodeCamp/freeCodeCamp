...
Erstelle eine weitere `@keyframes`-Regel mit dem Namen `cabins`. Verwende die gleichen Eigenschaften wie dein `@keyframes wheel`, kopiere sowohl die `0%` und `100%`-Regeln, aber setze die `transform`-Eigenschaft des `100%`-Selektors auf `rotate(-360deg)`.
...
Deine neue `@keyframes`-Regel sollte `cabins` heißen.
...
Deine neue `@keyframes`-Regel sollte nach deiner `@keyframes wheel`-Regel stehen.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.name === 'cabins');
```

Du solltest den Namen deiner `@keyframes wheel`-Regel nicht ändern.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[0]?.name === 'wheel');
```

Deine `@keyframes cabins`-Regel sollte einen `0%`-Selektor haben.
...
Deine `@keyframes cabins`-Regel sollte einen `100%`-Selektor haben.
...
Dein `100%`-Selektor sollte eine `transform`-Eigenschaft von `rotate(-360deg)` haben.
