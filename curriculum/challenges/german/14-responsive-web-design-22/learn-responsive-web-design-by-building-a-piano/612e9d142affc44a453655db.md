...
Es ist an der Zeit, die Tasten selbst zu gestalten. Create a `class` selector for the `.key` elements. Setze die `background-color` auf den Wert `#ffffff`, die `position`-Eigenschaft auf `relative`, die `width`-Eigenschaft auf `41px` und die `height`-Eigenschaft auf `175px`.
