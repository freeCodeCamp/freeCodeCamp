...
Füge ein weiteres `p`-Element mit dem Text `Total Carbohydrate 37g 13%` hinzu. Verwende wie zuvor `span`-Elemente, um den Text `Total Carbohydrate` und `13%` fettgedruckt zu machen. Füge außerdem ein zusätzliches `span`-Element hinzu, um den `Total Carbohydrate 37g`-Text mit einem span-Element zu umschließen., damit der Text links und `13%` rechts ausgerichtet ist.
