...
Bette alles in das `.label`-Element in einem neuen `header`-Element ein.

# --hints--

Du solltest ein neues `header`-Element erstellen.

```js
assert(document.querySelector('header'));
```

Dein `header`-Element sollte sich innerhalb deines `.label`-Elements befinden.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

Deine `h1`-, `div`- und `p`-Elemente sollten sich innerhalb deines neuen `header`-Elements befinden.
