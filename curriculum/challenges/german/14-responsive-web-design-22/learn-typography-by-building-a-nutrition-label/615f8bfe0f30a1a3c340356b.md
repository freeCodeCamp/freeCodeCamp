...
Erstelle ein weiteres `p`-Element und weise ihm den Text `Calcium 260mg 20%` zu. Align `20%` to the right. Below that, create a `p` element with the text `Iron 8mg 45%`, aligning the `45%` to the right.
