...
Erstelle einen neuen `.calories-info`-Selektor und gib ihm eine `display`-Eigenschaft von `flex`. Also give it a `justify-content` property set to `space-between` and `align-items` property set to `flex-end`.
