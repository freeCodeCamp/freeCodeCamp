...
Erstelle einen neuen `.left-container p`-Selektor und setze damit die obere und untere Margin auf `-5px` sowie die linke und rechte Margin auf `-2px`. Setze auch die `font-size` auf `2em` und `font-weight` auf `700`.
