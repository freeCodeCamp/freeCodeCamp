...
Dein neues `p`-Element sollte drei `span`-Elemente enthalten.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelectorAll('span')?.length === 3);
```

Dein erstes `span` `Cholesterol`-Element sollte `class` auf `bold` gesetzt haben.
...
Dein erstes `span`-Element sollte den Text `Cholesterol 0mg` umschließen.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.firstElementChild?.innerText?.match(/Cholesterol[\s|\n]+0mg/));
```

Dein erstes `span`-Element sollte den Text `Cholesterol` umschließen.
...
Dein zweites `span`-Element sollte das `class`-Attribut auf `bold` gesetzt haben.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.lastElementChild?.classList?.contains('bold'));
```

Dein zweites `span`-Element sollte den Text `0%` umschließen.
