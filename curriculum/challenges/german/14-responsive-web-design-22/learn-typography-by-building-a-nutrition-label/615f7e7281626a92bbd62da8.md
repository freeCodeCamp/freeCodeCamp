...
Erstelle unter deinem letzen `p`-Element ein weiteres `p`-Element mit dem Text `Sodium 160mg 7%`. Umschließe den Text `Sodium` mit einem `span`-Element mit einem `class`-Attribut, das auf `bold` gesetzt ist. Umschließe den Text `7%` mit einem weiteren `span`-Element mit der `class`, die auf `bold` gesetzt ist. Füge außerdem ein zusätzliches `span`-Element um `Sodium 160mg` hinzu, um es richtig ausrichten.
...
Ein `span`-Element sollte den Text `Sodium 360mg` umschließen.
