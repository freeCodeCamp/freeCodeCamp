...
Um die `input`- und `textarea`-Elemente mit dem Hintergrundmotiv zu verschmelzen, setze ihre `background-color` auf `#0a0a23`. Gib ihnen dann einen `1px`, `solid`-Rand mit der Farbe `#0a0a23`.

# --hints--

Du solltest einen Elementselektor verwenden, der durch Kommas getrennt wird, um die `input`- und `textarea`-Elemente auszuwählen.
...
Du solltest den `input`- und `textarea`-Elementen eine `background-color` von `#0a0a23` geben.
...
Du solltest den `input`- und `textarea`-Elementen einen `1px`, `solid`-Rand mit der Farbe `#0a0a23` geben.
