...
Next, add a `min` attribute to the `input` with a value of `13` because users under the age of 13 should not register. Außerdem werden die Benutzer wahrscheinlich nicht älter als 120 Jahre sein; füge ein `max`-Attribut mit dem Wert `120` hinzu.
