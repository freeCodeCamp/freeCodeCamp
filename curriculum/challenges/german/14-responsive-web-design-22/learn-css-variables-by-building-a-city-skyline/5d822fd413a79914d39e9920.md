...
Du solltest `.fb2a` einen `border-bottom` von `10vh solid var(--building-color3)` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderBottom.trim(), "10vh solid var(--building-color3)");
```

Du solltest `.fb2a` einen `border-left` von `1vw solid transparent` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderLeft, "1vw solid transparent");
```

Du solltest `.fb2a` einen `border-right` von `1vw solid transparent` geben.
