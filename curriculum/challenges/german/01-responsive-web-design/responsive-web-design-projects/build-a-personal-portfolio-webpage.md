...
title: Erstelle eine persönliche Portfolio-Webseite
challengeType: 14
forumTopicId: 301143
...
1. Der Begrüßungsbereich sollte ein `h1` Element besitzen, das einen Text enthält
...
1. Dein Portfolio sollte eine Navigationsleiste mit einer Id der `navbar` besitzen
...
1. Dein Portfolio sollte einen Link mit der Id von `profile-link` besitzen, der dein GitHub oder das freeCodeCamp Profil in einem neuen Tab öffnet
1. Dein Portfolio sollte mindestens eine Media Query (Medienabfrage) enthalten
1. Die Höhe des Begrüßungsabschnitts sollte der Höhe des Viewports entsprechen
...
Dein Portfolio sollte einen Begrüßungsbereich mit einer `id` von `welcome-section` besitzen.
...
Dein `#navbar` Element sollte mindestens ein `a` Element, dessen `href` Attribut mit `#` beginnt, besitzen.
...
Dein Portfolio sollte ein `a` Element mit einer `id` von `profile-link` besitzen.
...
Dein `#profile-link` Element sollte ein `target` Attribut von `_blank` besitzen.
...
Dein Portfolio sollte mindestens eine Medienabfrage (Media Query) benutzen.
