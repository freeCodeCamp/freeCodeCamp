...
Da `elemValue` eine Funktion zurückgibt, gibt deine `addCharacters`-Funktion letztendlich ein Array von Funktionsreferenzen zurück. Du möchtest, dass die `.map()`-Methode, die innere Funktion deiner `elemValue`-Funktion ausführt, was bedeutet, dass du `elemValue` aufrufst, anstatt darauf zu verweisen. Übergib deiner `elemValue`-Funktion `num` als Argument.

<!-- TODO: Explain further? -->

# --hints--

Du solltest `elemValue()` in deiner `.map()`-Methode aufrufen.
...
Du solltest deinem `elemValue()`-Aufruf `num` übergeben.
