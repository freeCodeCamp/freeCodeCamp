...
Rufe die `.map()`-Methode bei deinem `rangeFromString()`-Aufruf auf und übergebe einen Verweis auf `addCharacters` als Callback-Funktion.

# --hints--

Du solltest die `.map()`-Methode für deinen `rangeFromString()`-Aufruf aufrufen.
...
Du solltest einen Verweis auf `addCharacters` als Callback an deine `.map()`-Methode übergeben.
