...
Aktualisiere nun deine innerste Funktion in der `addCharacters`-Kette so, dass sie implizit das Ergebnis des Aufrufs von `charRange()` mit `character1` und `character2` als Argumente zurückgibt.

# --hints--

Deine innerste Funktion sollte eine implizite Rückgabe verwenden.
...
Deine innerste Funktion sollte das Ergebnis des Aufrufs von `charRange()` zurückgeben.
...
Du solltest `character1` als erstes Argument an deinen `charRange()`-Aufruf übergeben.
...
Du solltest `character2` als zweites Argument an deinen `charRange()`-Aufruf übergeben.
