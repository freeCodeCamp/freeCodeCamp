...
Gib in deiner `inner`-Funktion das Ergebnis des Aufrufs von `idToText` mit `character + num` als Argument zurück.

# --hints--

Deine `inner`-Funktion sollte eine explizite Rückgabe verwenden.
...
Deine `inner`-Funktion sollte das Ergebnis des Aufrufs deiner `idToText`-Funktion zurückgeben.
...
Du solltest `character + num` als Argument an deine `idToText`-Funktion übergeben.
