...
The `.map()` method here will call the `myFunc` function, passing the same arguments that a `.map()` callback takes. Das erste Argument ist der Wert des Arrays bei der aktuellen Iteration, `newArray` wäre also `[value: 1, value: 2, value: 3]`.
