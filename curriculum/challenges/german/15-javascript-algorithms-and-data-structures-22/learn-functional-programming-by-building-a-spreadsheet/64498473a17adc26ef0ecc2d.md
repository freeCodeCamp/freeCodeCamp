...
Deine `idToText`-Funktion gibt derzeit ein `input`-Element zurück. Aktualisiere sie so, dass sie den `value` dieses `input`-Elements zurückgibt.

# --hints--

Du solltest die `value`-Eigenschaft des Rückgabewerts der `.find()`-Methode zurückgeben.
