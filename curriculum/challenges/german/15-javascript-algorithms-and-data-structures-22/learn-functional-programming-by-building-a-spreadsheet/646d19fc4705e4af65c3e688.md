...
Deine `addCharacters`-Funktion gibt schließlich einen Bereich von Zeichen zurück. Du willst, dass es ein Array von Zell-IDs zurückgibt. Verkette die `.map()`-Methode an deinen `charRange()`-Aufruf. Übergib noch keine Callback-Funktion.

# --hints--

Du solltest `.map()` mit deinem `charRange()`-Aufruf verketten.
...
Du solltest deinem `.map()`-Aufruf nichts übergeben.
