...
Dein `addCharacters(char1)` gibt auch eine Funktion zurück, die eine weitere Funktion zurückgibt. Du musst einen weiteren Funktionsaufruf tätigen, um auf die innerste Funktionsreferenz für den `.map()`-Callback zuzugreifen. JavaScript allows you to immediately invoke returned functions:
...
Du solltest einen Funktionsaufruf mit deinem `addCharacters(char1)`-Aufruf verketten.
...
Du solltest `char2` als Argument an deinen verketteten Funktionsaufruf übergeben.
