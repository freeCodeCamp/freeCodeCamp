...
Findet `infixEval` keine Übereinstimmungen, gibt er den `str`-Wert so zurück, wie er ist. Überprüfe mit Hilfe eines ternären Ausdrucks, ob `str2` gleich `str` ist. If it is, return `str`, otherwise return the result of calling `highPrecedence()` on `str2`.
