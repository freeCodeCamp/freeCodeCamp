...
Das zweite Argument der `.replace()`-Methode muss nicht unbedingt ein String sein. Du kannst stattdessen eine Callback-Funktion übergeben, um eine komplexere Logik auf den übereinstimmenden String anzuwenden.

Die Callback-Funktion verwendet ein paar Parameter. Der erste ist der übereinstimmende String. Übergib deinem `.replace()`-Aufruf eine leere Callback-Funktion und weise ihm einen `match`-Parameter zu.

# --hints--

Du solltest deiner `.replace()`-Methode eine Arrow-Funktion als zweites Argument übergeben.
...
Deine Arrow-Funktion sollte einen `match`-Parameter verwenden.
