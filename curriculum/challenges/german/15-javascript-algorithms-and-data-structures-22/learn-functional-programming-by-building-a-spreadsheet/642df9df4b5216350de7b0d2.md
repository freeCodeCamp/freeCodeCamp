...
Declare an empty `range` function which takes a `start` and `end` parameter. Verwende den `Array()`-Konstruktor und gib implizit ein leeres Array zurück.
...
Deine `range`-Funktion sollte eine implizite Rückgabe verwenden. Remember that this means you will not use curly brackets.

```js
assert.notMatch(code, /const\s+range\s*=\s*\(\s*start\s*,\s*end\s*\)\s*=>\s*{/);
```

Your `range` function should use the `Array()` constructor. Primitive Konstruktoren benötigen kein `new`-Schlüsselwort.
