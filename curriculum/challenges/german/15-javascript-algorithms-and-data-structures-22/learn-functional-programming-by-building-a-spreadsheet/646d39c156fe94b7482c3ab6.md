...
Deklariere eine `cellRegex`-Variable, um mit Zellreferenzen übereinzustimmen. Es solte mit einem Buchstaben von `A` bis `J` übereinstimmen, gefolgt von einer Ziffer von `1` bis `9` und eine optionale Ziffer von `0` bis `9`. Make the regular expression case-insensitive and global.
...
Du solltest deiner `cellRegex`-Variablen einen regulären Ausdruck zuweisen.
