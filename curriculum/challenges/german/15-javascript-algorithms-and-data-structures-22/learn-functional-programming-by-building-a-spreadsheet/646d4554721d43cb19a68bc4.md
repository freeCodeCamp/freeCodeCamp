...
Der `fn`-Parameter wird der Name einer Funktion sein, wie z. B. `SUM`. Aktualisiere `apply`, um implizit die Funktion zurückzugeben, die in der `fn`-Eigenschaft deines `spreadsheetFunctions`-Objekts gefunden wurde.

Denke daran, dass `fn` möglicherweise nicht kleingeschrieben ist, daher musst du es in einen String mit Kleinbuchstaben umwandeln.
