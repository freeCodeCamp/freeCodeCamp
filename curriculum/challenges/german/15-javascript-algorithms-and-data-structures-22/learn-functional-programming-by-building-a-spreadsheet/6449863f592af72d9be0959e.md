...
Nachdem deine `rangeRegex` den `:` gefunden hat, muss sie nach demselben Buchstaben- und Zahlenmuster suchen wie zuvor.

Kopiere deine zwei vorhandenen Capture-Gruppen und füge sie nach dem Doppelpunkt ein.

# --hints--

Du solltest deiner `rangeRegex` eine dritte Capture-Gruppe nach dem Doppelpunkt hinzufügen.
...
Deine dritte Capture-Gruppe sollte mit den Zeichen `A` bis `J` übereinstimmen.
...
Du solltest deiner `rangeRegex` eine vierte Capture-Gruppe hinzufügen.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(.*\)/);
```

Deine vierte Capture-Gruppe sollte mit ein oder zwei Ziffern übereinstimmen.
