...
Your final step is to make your clear button functional. Add a `click` event listener to the `clearCartBtn`. Für den Callback kannst du `cart.clearCart` direkt übergeben.

However, doing so will not work, because the context of `this` will be the `clearCartBtn` element. Du musst die `clearCart`-Methode an das `cart`-Objekt binden.
