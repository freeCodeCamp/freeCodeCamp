...
Erstelle nach dem gleichen Muster ein `fieldset` für `dinner` mit den gleichen verschachtelten Elementen.

# --hints--

Du solltest ein drittes `fieldset`-Element in deiner `form` erstellen.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]);
```

Dein drittes `fieldset`-Element sollte nach deinem zweiten `fieldset`-Element stehen.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.previousElementSibling?.tagName, "FIELDSET");
```

Dein drittes `fieldset`-Element sollte eine `id` auf `dinner` gesetzt haben.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.id, "dinner");
```

Dein drittes `fieldset`-Element sollte ein `legend`-Element enthalten.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]?.querySelector('legend'));
```

Dein neues `legend`-Element sollte den Text `Dinner` haben.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.querySelector('legend')?.innerText, "Dinner");
```

Dein drittes `fieldset`-Element sollte ein `div`-Element enthalten.
