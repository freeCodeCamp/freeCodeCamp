...
Deklariere eine `surplusOrDeficit`-Variable. Then use a ternary operator to set `surplusOrDeficit` to the string `"Surplus"` or `"Deficit"` depending on whether `remainingCalories` is less than `0`. If it is less than `0`, then `surplusOrDeficit` should be `"Surplus"`. Otherwise, it should be `"Deficit"`.
