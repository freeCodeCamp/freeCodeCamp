...
Use a ternary operator to set `surplusOrDeficit` to the string `Surplus` or `Deficit` depending on whether `remainingCalories` is greater than or equal to `0`. Wenn er größer oder gleich `0` ist, dann sollte `surplusOrDeficit` `Surplus` sein. Otherwise, it should be `Deficit`.
