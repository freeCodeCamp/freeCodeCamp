...
Die `.split()`-Methode nimmt einen String und zerlegt ihn in einen Array von Strings. Du kannst ihr einen String oder einen regulären Ausdruck übergeben, das als Trennzeichen verwendet werden soll. Zum Beispiel würde `string.split(",")` den String an jedem Komma trennen und ein Array von Strings zurückgeben.

Use the `/,\s*/g` regex to split the `value` string by commas. You can tweak it based on the number of spaces separating your values. Speichere das Array in einer `array`-Vabriablen.
