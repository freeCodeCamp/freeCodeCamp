...
Array methods can often be chained together to perform multiple operations at once. Ein Beispiel:

```js
array.map().filter();
```

Die `.map()`-Methode wird mit dem Array aufgerufen, und dann wird die `.filter()`-Methode mit dem Ergebnis der `.map()`-Methode aufgerufen. This is called method chaining.
