...
Wenn die `array.length` eine gerade Zahl ist, sollte deine `median`-Variable die `getMean`-Funktion verwenden, um den Mittelwert der beiden mittleren Zahlen zu berechnen. Your first argument should be the value of `sorted` at `array.length / 2`, and the second at `array.length / 2 - 1`.
