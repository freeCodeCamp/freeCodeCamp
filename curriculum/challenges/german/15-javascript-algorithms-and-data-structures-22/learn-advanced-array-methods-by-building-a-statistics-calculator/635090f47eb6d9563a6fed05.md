...
Ob eine Zahl gerade oder ungerade ist, kannst du mit dem <dfn>Modulo-Operator</dfn> prüfen, welcher durch das `%`-Symbol dargestellt wird. Dieser Operator gibt den Rest der Division von zwei Zahlen zurück. Wenn der Rest `0` ist, ist die Zahl gerade. Wenn der Rest `1` ist, ist die Zahl ungerade:
...
Wenn die `array.length` eine ungerade Zahl ist, sollte deine `median`-Variable der `median`-Variablen die mittlere Zahl zuweisen. Die mittlere Zahl hat den Wert von `sorted` bei `Math.floor(array.length / 2)`.
