...
Die Callback-Funktion muss einen booleschen Wert zurückgeben, der angibt, ob das Element in das neue Array aufgenommen werden soll. In this case, you want to return `true` if the element is not `NaN` (not a number).
...
Deine Callback-Funktion sollte `!` und `Number.isNaN()` verwenden, um zu überprüfen, ob `el` NICHT `NaN` ist.
