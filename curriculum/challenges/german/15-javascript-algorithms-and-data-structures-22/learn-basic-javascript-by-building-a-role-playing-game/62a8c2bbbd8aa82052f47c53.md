...
Setze `name` auf `cave`. Setze `button text` auf ein Array mit den Strings `"Fight slime"`, `"Fight fanged beast"`, und `"Go to town square"`. Set the `"button functions"` to an array with the variables `fightSlime`, `fightBeast`, and `goTown`. Setze die `text`-Eigenschaft auf `"You enter the cave. You see some monsters."`.
