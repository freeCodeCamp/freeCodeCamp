...
Ähnlich wie dein `#stats`-Element benötigt dein `#monsterStats`-Element zwei `span`-Elemente. Gib ihnen die Klasse `stat` und gib dem ersten Element den Text `Monster Name:` und dem zweiten den Text `Health:`. Füge nach dem Text in jedem ein `strong`-Element mit einem leeren eingebetteten `span`-Element hinzu. Give the first inner `span` element an `id` of `monsterName` and the second inner `span` element an `id` of `monsterHealth`.
