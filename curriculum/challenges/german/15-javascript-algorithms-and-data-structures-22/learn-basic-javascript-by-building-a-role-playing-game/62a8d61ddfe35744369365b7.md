...
You should add `"You sold a "` to `text.innerText`. Die Abstände sind wichtig.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

You should add the value of `currentWeapon` to the `"You sold a "` string. Verwende den Verkettungsoperator, um dies in derselben Zeile zu tun.
