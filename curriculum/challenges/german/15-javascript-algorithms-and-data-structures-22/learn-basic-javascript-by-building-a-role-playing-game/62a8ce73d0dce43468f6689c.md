...
You can insert variables into a string with the <dfn>concatenation operator</dfn> `+`. Update the `You now have a new weapon.` string to say `You now have a` and the name of the new weapon. Denke daran, den Satz mit einem Punkt zu beenden.

Hier ist ein Beispiel, das den String `Hello, our name is freeCodeCamp.` erzeugt:
