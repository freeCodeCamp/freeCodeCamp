...
Erstelle eine `button1`-Variable und verwende `querySelector()`, um sie deinem Element mit der `id` "`button1`" zuzuweisen. Remember that CSS `id` selectors are prefixed with a `#`.
