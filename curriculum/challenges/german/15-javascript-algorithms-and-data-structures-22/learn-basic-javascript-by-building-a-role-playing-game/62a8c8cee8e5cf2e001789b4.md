...
Der Wert deiner `currentWeapon`-Variable entspricht einem Index im `weapons`-Array. The player starts with a `"stick"`, since `currentWeapon` starts at `0` and `weapons[0]` is the `"stick"` weapon.
...
Du solltest eine zusammengesetzte Zuweisung verwenden, um eine zu `currentWeapon` hinzuzufügen.
