...
Genau wie dein `locations`-Array wird auch dein `weapons`-Array Objekte enthalten. Füge dem `weapons`-Array vier Objekte hinzu, jedes mit zwei Eigenschaften: `name` und `power`. The first should have the `name` set to `"stick"` and the `power` set to `5`. The second should be `"dagger"` and `30`. The third, `"claw hammer"` and `50`. The fourth, `"sword"` and `100`.
