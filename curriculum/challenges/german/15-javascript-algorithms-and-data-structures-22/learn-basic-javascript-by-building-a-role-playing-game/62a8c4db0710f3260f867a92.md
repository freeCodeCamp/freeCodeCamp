...
Was passiert, wenn der Spieler nicht genügend Gold hat, um Gesundheit zu kaufen? Wenn du den Code unter bestimmten Bedingungen ausführen möchtest, kannst du die `if`-Anweisung verwenden. Füge den gesamten Code deiner `buyHealth`-Funktion in eine `if`-Anweisung ein. Zum Beispiel:
