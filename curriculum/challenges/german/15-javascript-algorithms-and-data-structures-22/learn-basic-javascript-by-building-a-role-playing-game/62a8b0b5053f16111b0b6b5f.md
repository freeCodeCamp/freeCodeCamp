...
Jetzt kannst du einen Teil deines Codes konsolidieren. Start by copying the code from inside the `goTown` function and paste it into your `update` function. Then, remove all the code from inside the `goTown` and `goStore` functions.
...
Deine `update`-Funktion sollte `text.innerText` auf `You are in the town square. You see a sign that says \"Store\".` setzen.
