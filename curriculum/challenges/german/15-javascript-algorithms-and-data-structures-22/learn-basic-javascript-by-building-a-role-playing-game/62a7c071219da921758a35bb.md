...
Verändere in deiner `goTown`-Funktion die `innerText`-Eigenschaften deiner `button`-Elemente, so dass sie `Go to store`, `Go to cave` und `Fight dragon` ergeben. Aktualisiere deine `onclick` Eigenschaften, so dass sie `goStore`, `goCave` bzw. `fightDragon` sind.

Aktualisiere schließlich die `innerText`-Eigenschaft deines Textes, so dass sie `You are in the town square. You see a sign that says Store.` als Wert enthält.
