...
Bei jedem Angriff sollte die Möglichkeit bestehen, dass die Waffe des Spielers zerbricht. At the end of the `attack` function, add an empty `if` statement with the condition `Math.random() <= .1`.
