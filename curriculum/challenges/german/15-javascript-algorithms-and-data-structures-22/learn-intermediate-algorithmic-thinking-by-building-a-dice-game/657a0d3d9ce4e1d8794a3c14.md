...
Du solltest `const` verwenden, um eine Variable namens `rulesContainer` zu deklarieren.
...
```js
assert.match(code, /const\s+rulesBtn\s*=\s*document\.getElementById\s*\(\s*('|")\s*rules-btn\s*\1\s*\)/)
```
