...
Beggine, indem du eine Arrow-Funktion namens `resetRadioOption` erstellst.

# --hints--

Du solltest eine `resetRadioOption`-Funktion haben.

```js
assert.isFunction(resetRadioOption);
```

Du solltest die Arrow-Syntax für deine `resetRadioOption`-Funktion verwenden.
