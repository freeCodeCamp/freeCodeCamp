...
Ihre Eigenschaft `year` sollte eine Zahl sein.

```js
assert.isNumber(myFavoriteFootballTeam.year);
```

Ihre Eigenschaft `year` sollte auf `1986` eingestellt sein.
...
Ihre Eigenschaft `isWorldCupWinner` sollte ein boolescher Wert sein.

```js
assert.isBoolean(myFavoriteFootballTeam.isWorldCupWinner);
```

Ihre Eigenschaft `isWorldCupWinner` sollte auf `true` gesetzt sein.
