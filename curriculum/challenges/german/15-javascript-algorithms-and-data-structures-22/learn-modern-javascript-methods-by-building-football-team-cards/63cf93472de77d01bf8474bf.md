...
Füge unterhalb der `isWorldCupWinner`-Eigenschaft einen neuen Schlüssel namens `headCoach` mit dem Wert eines leeres Objekts hinzu. Inside that object, add a property with a key of `coachName` and a string value of `"Carlos Bilardo"`. Below that property, add another key called `matches` with a number value of `7`.
...
Deine `matches`-Eigenschaft sollte auf `7` eingerichtet sein.
