...
Before the `${name}` expression, add a new embedded expression. Verwende innerhalb dieses Ausdrucks einen ternären Operator, um zu prüfen, ob `isCaptain` wahr (true) ist. If so, return `"(Captain)"` otherwise return an empty string.
