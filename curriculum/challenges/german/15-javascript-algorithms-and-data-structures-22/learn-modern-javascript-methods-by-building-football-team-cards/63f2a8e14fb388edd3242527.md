...
Du solltest `players.filter()` an deinen `setPlayerCards`-Aufruf übergeben.

```js
assert.match(code, /\s*case\s*('|"|`)\s*defender\s*\1\s*:\s*setPlayerCards\(\s*players\.filter\(/)
```

Du solltest eine Arrow-Callback-Funktion an deinen `players.filter()`-Aufruf übergeben.
