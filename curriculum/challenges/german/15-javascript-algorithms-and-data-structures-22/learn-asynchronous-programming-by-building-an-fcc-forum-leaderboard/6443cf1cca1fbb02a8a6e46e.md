...
Du solltest eine Konstante mit dem Namen `daysAgo` deklarieren.

```js
assert.match(code, /const\s+daysAgo\s*=/);
```

You should assign `Math.floor(hoursAgo / 24)` to your `daysAgo` variable.
