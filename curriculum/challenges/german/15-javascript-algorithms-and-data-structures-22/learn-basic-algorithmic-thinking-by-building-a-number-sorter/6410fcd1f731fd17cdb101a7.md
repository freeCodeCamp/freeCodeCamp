...
Inside your nested `for` loop, add a `console.log()` call to check the values of `array`, `array[j]`, and `array[minIndex]` at each iteration. Du kannst auf den `Sort`-Button klicken, um zu sehen, wie dein Algorithmus das Array durchläuft.

Schreibe anschließend eine `if`-Anweisung, die prüft, ob der Wert bei `j` kleiner ist als der Wert bei `minIndex`. Ist dies der Fall, dann setze `minIndex` auf `j`.
