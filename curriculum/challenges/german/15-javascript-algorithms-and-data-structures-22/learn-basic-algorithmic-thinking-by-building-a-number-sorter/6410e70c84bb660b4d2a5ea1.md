...
To achieve the "bubble up" result, you need to check if the current element is larger than the next element. Dies kannst du tun, indem du auf das `array` bei `j` und `j+1` zugreifst.
