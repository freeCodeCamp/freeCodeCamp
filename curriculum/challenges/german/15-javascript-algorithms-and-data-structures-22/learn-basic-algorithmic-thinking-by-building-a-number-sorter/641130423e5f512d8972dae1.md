...
Um dies zu beheben, kannst du der `.sort()`-Methode eine Callback-Funktion übergeben. Die Callback-Funktion benötigt zwei Argumente - verwende für deine Funktion `a` und `b`. Lasse die Funktion vorerst leer.
...
Die Callback-Funktion sollte zwei Argumente verwenden, `a` und `b`.
