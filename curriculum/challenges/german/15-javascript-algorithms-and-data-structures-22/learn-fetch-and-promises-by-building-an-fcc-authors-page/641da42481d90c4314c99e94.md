...
Chain the `.then()` method to your `fetch` call. Inside the `.then()` method, add a callback function with `res` as a parameter, then log the `res` to the console so you can see the `Response` object. Öffne deine Konsole und erweitere das `Response`-Objekt, um zu sehen, was es enthält.
