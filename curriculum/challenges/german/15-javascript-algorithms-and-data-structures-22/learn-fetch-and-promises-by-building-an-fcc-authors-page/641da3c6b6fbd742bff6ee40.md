...
The <dfn>Fetch API</dfn> is a built-in JavaScript interface to make network requests to a server. Sie verfügt über eine `fetch()`-Methode, mit der du `GET`-, `POST`-, `PUT`- oder `PATCH`-Anfragen stellen kannst. In diesem Projekt wirst du eine `GET`-Anfrage an eine URL für eine JSON-Datei mit Informationen über Autoren auf freeCodeCamp-News stellen.
