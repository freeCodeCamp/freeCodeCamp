...
Der wahrheitsgemäße Ausdruck deiner Ternäre sollte die `textContent`-Eigenschaft des `result`-Elements auf `Oh no! This looks like a spam message.` setzen.
...
Der fehlerhafte Ausdruck deiner Ternäre sollte die `textContent`-Eigenschaft des `result`-Elements auf `This message does not seem to contain any spam.` setzen.
