...
A <dfn>character class</dfn> is defined by square brackets, and matches any character within the brackets. Zum Beispiel stimmt `[aeiou]` mit jedem Zeichen in der `aeiou`-Liste überein. You can also define a range of characters to match using a hyphen. Beispielsweise stimmt `[a-z]` mit jedem Zeichen von `a` bis `z` überein.
...
Dein `dollarRegex` sollte mit `1 dollars` übereinstimmen.
