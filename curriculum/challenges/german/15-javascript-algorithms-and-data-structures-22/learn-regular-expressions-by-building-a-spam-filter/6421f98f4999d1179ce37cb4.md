...
Denke daran, dass Arrays über eine `.some()`-Methode verfügen. Benutze die `.some()`-Methode, um zu überprüfen, ob der Test von `msg` auf einen der regulären Ausdrücke von `denyList` `true` zurückgibt.
