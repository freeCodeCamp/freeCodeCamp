...
To create a non-capturing group in a regular expression, you can add `?:` after the opening parenthesis of a group. Beispielsweise wird `(?:a|b)` entweder mit `a` oder mit `b` übereinstimmen, aber er wird nicht das Ergebnis erfassen.

Update your regular expression to use a non-capturing group.

# --hints--

Dein `dollarRegex` sollte `?:` verwenden.
