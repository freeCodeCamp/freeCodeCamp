...
- `Status: INSUFFICIENT_FUNDS`: wenn `cash-in-drawer` weniger als das anfallende Wechselgeld oder, wenn du nicht das genaue Wechselgeld zurückgeben kannst.
- `Status: CLOSED`: wenn `cash-in-drawer` gleich dem anfallenden Wechselgeld ist.
- `Status: OPEN`: wenn `cash-in-drawer` größer ist als das anfallende Wechselgeld, kannst du das Wechselgeld zurückgeben, wobei das Wechselgeld in Münzen und Scheinen in der Reihefolge vom höchsten zum niedrigsten Wert sortiert wird.
...
**Aufgabe:** Erstelle eine Applikation, die eine ähnliche Funktionalität wie <a href="https://cash-register.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://cash-register.freecodecamp.rocks</a> aufweist

**User Stories:**

1. You should have an `input` element with an `id` of `cash`
1. Du solltest ein `div`-Element mit einer `id` von `change-due` haben
1. You should have a `button` element with an `id` of `purchase-btn`
1. Wenn der Wert in dem `#cash`-Element kleiner ist als `price`, sollte eine Warnmeldung mit dem Text `Customer does not have enough money to purchase the item` erscheinen
1. Wenn der Wert in dem `#cash`-Element gleich `price` ist, sollte der Wert in dem `#change-due`-Element `No change due - customer paid with exact cash` sein
1. Wenn `price` `19.5` ist, der Wert im `#cash`-Element `20` ist, `cid` `[["PENNY", 1.01], ["NICKEL", 2.05], ["DIME", 3.1], ["QUARTER", 4.25], ["ONE", 90], ["FIVE", 55], ["TEN", 20], ["TWENTY", 60], ["ONE HUNDRED", 100]]` ist und das `#purchase-btn`-Element angeklickt wird, sollte der Wert in dem `#change-due`-Element sollte `Status: OPEN QUARTER: $0.5` sein
