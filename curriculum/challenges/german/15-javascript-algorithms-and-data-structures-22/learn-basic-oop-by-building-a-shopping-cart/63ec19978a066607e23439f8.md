...
Jetzt musst du deine beiden Buttons holen. Weiter geht es mit den `#cart-btn`- und `#clear-cart-btn`-Elementen. Speichere sie in Variablen namens `cartBtn` und `clearCartBtn`.
...
Du solltest `document.getElementById()` verwenden, um das `#clear-cart-btn`-Element zu erhalten.
