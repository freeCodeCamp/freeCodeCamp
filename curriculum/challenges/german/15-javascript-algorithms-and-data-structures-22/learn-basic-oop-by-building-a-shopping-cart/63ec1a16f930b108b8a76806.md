...
Als Nächstes musst du deine Gesamtbeträge ermitteln. Rufe die `#total-items`-, `#subtotal`-, `#taxes`- und `#total`-Elemente ab. Speichere sie in den Variablen namens `totalNumberOfItems`, `cartSubTotal`, `cartTaxes` und `cartTotal`.
