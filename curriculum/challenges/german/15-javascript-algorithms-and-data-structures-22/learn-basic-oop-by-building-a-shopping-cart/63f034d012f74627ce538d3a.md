...
Abschließend aktualisiere die `display`-Eigenschaft des `style`-Objekts der `cartContainer`-Variablen auf eine andere Ternary, die überprüft, ob `isCartShowing` wahr ist. If it is, set the `display` property to `"block"`, otherwise set it to `"none"`.
