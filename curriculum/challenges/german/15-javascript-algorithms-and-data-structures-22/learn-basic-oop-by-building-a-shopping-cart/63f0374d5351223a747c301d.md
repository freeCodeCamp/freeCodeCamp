...
Du solltest auf die `textContent`-Eigenschaft des `cartTaxes`-Elements zugreifen.

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

Du solltest die `.toFixed()`-Methode für die `tax`-Variable aufrufen und `2` als Argument übergeben.
...
Du solltest den `textContent` des `cartTaxes`-Elements auf den `tax.toFixed`-Template-String setzen.
...
Du solltest auf die `textContent`-Eigenschaft des `cartTotal`-Elements zugreifen.

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

Du solltest die `.toFixed()`-Methode für die `total`-Variable aufrufen und `2` als Argument übergeben. Denke daran, das `this`-Schlüsselwort zu verwenden.
...
Du solltest den `textContent` des `cartTotal`-Elements auf den `total.toFixed`-Template-String setzen.
