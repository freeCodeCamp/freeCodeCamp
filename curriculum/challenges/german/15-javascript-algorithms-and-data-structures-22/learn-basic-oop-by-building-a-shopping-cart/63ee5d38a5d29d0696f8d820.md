...
Erstelle in deinem Template-Literal ein `div`-Element mit der Klasse `dessert-card`. In diesem `div` erstellst du ein `h2`-Element und gibst ihm den Text der `name`-Variablen.
...
Dein `h2`-Element sollte den Text der `name`-Variable haben.

```js
assert.equal(document.querySelectorAll('h2')[0]?.textContent, 'Vanilla Cupcakes (6 Pack)');
```

Dein `h2`-Element sollte sich innerhalb des `div`-Elements befinden.
