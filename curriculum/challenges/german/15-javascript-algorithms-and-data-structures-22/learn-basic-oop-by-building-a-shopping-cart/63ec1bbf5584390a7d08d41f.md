...
Verwende dann `let`, um eine Variable mit dem Namen `isCartShowing` zu deklarieren und setze sie auf `false`.

# --hints--

Du solltest `document.getElementById()` verwenden, um das `#show-hide-cart`-Element zu erhalten.
...
Du solltest `let` verwenden, um eine Variable mit dem Namen `isCartShowing` zu deklarieren.

```js
assert.match(code, /let\s+isCartShowing/);
```

Du solltest die `isCartShowing`-Variable auf `false` setzen.
