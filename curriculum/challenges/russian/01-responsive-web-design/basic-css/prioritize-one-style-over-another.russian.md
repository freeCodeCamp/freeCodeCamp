...
## Description
<section id="description"> Иногда ваши HTML-элементы получат несколько стилей, которые конфликтуют друг с другом. Например, ваш элемент <code>h1</code> не может быть одновременно зеленым и розовым. Давайте посмотрим, что произойдет, когда мы создадим класс, который делает текст розовым, а затем применим его к элементу. <em>Переопределит</em> ли наш класс <code>color: green;</code> свойство CSS элемента <code>body</code>? </section>

## Instructions
<section id="instructions"> Создайте класс CSS <code>pink-text</code>, который задает элементу розовый цвет. Задайте вашему элементу <code>h1</code> класс <code>pink-text</code> . </section>
