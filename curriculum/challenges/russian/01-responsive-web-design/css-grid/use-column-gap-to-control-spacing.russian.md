...
```yml
tests:
  - text: <code>item5</code> class should have a <code>column-gap</code> property.
    testString: assert($('style').text().replace(/\s/g, '').match(/\.item5{.*column-gap:.*}/g));
  - text: <code>item5</code> class should have a <code>column-gap</code> property which results in the <code>div</code> with the <code>item5</code> consuming the last two columns of the grid.
