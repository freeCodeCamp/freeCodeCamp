...
```yml
tests:
  - text: <code>hound</code> должна быть создана с использованием конструктора <code>Dog</code> .
    testString: 'assert(hound instanceof Dog, "<code>hound</code> should be created using the <code>Dog</code> constructor.");'
  - text: Ваш код должен использовать <code>new</code> оператор для создания <code>instance</code> <code>Dog</code> .
