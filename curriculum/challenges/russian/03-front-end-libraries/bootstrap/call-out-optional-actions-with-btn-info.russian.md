...
challengeType: 0
videoUrl: ''
localeTitle: Вызовите дополнительные действия с помощью btn-info
---

## Description
<section id="description"> Bootstrap поставляется с несколькими заранее определенными цветами для кнопок. Класс <code>btn-info</code> используется для привлечения внимания к необязательным действиям, которые пользователь может выполнить. Создайте новую кнопку Bootstrap на уровне блока под кнопкой « <code>btn-info</code> нравится» с текстом «Информация» и добавьте к нему <code>btn-info</code> и <code>btn-block</code> Bootstrap. Обратите внимание, что этим кнопкам по-прежнему нужны <code>btn</code> и <code>btn-block</code> . </section>
...
```yml
tests:
  - text: Создайте новый элемент <code>button</code> с текстом «Информация».
    testString: 'assert(new RegExp("info","gi").test($("button").text()), "Create a new <code>button</code> element with the text "Info".");'
  - text: Обе ваши кнопки Bootstrap должны иметь <code>btn</code> и <code>btn-block</code> .
  ...
  - text: Ваша новая кнопка должна иметь класс <code>btn-info</code> .
