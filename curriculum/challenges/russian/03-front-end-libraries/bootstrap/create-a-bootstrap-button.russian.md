...
challengeType: 0
videoUrl: ''
localeTitle: Создать кнопку загрузки
---

## Description
<section id="description"> У Bootstrap есть свои стили для элементов <code>button</code> , которые выглядят намного лучше, чем простые HTML-версии. Создайте новый элемент <code>button</code> под фотографией большого котенка. Дайте ему <code>btn</code> и <code>btn-default</code> , а также текст «Like». </section>
...
```yml
tests:
  - text: Создайте новый элемент <code>button</code> с текстом «Like».
  ...
  - text: 'Ваша новая кнопка должна иметь два класса: <code>btn</code> и <code>btn-default</code> .'
