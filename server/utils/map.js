...
export function mapChallengeToLang({ translations = {}, ...challenge }, lang) {
...
export function getMapForLang(lang) {
  return ({ entities: { challenge: challengeMap, ...entities }, result }) => {
    entities.challenge = Object.keys(challengeMap).reduce(
...
export function getChallengeById(map, id) {
  return Observable.if(
    ...
    map.map(addNameIdMap).map(map => {
      const { entities: { challenge: challengeMap, challengeIdToName } } = map;
      let finalChallenge;
...
export function getChallenge(challengeDashedName, blockDashedName, map, lang) {
