...
export default function settingsController(app) {
  ...
  function refetchChallengeMap(req, res, next) {
    ...
    return user
      .requestChallengeMap()
      .subscribe(challengeMap => res.json({ challengeMap }), next);
  }
  ...
  function updateMyEmail(req, res, next) {
    ...
    return user
      ...
      .subscribe(message => res.sendFlash(alertTypes.info, message), next);
  }
  ...
  function updateMyLang(req, res, next) {
    ...
    return user.update$(update).subscribe(
      () =>
        res.json({
  ...
  function updateMyCurrentChallenge(req, res, next) {
    ...
    return user.update$({ currentChallengeId }).subscribe(
      () =>
        res.json({
          message:
            'your current challenge has been updated to ' +
            `${currentChallengeId}`
        }),
      next
    );
  ...
  function updateMyProjects(req, res, next) {
    const { user, body: { projects: project } } = req;
    return user
  ...
  api.post('/refetch-user-challenge-map', ifNoUser401, refetchChallengeMap);
  ...
  api.post('/update-my-lang', ifNoUser401, updateMyLang);
