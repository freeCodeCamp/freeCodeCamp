...
- [x] rename challenge files by their ids, otherwise it's a pain to look them up.
- [ ] validate that challenge filenames match their ids
...
- [ ] consider regularizing the curriculum structure. i.e. `[{ name: "<superblockName>" blocks: [ { name: "<blockName>", challenges: []}]  }]`. So, we'd not need the various order properties. First, check that we can sort the entire curriculum like we currently do.
- [x] rename the files to \<challengeId\>.md
- [ ] only build each block once if they appear in multiple superblocks (memoize?)
