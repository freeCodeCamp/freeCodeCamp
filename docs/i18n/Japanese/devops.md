...
> [!TIP|label:見積もり] 通常、ビルドの実行には20~25分ほどかかり、その後はクライアントにとって15~20分ほどかかるリリースランが続きます。 APIがライブ配信されるまで5~10分ほどかかります。 コードプッシュからステージングプラットフォーム上でのライブまで、全体のプロセスは **~35~45分** かかります。
