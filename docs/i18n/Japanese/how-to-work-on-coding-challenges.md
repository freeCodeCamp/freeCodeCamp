...
あなたのIDを取得したら、上部の `id` フィールドとしてマークダウンファイルに入れてください。例えば、
...
各チャレンジには`Get a Hint`ボタンがあります。 したがって、ユーザーは挑戦のために作成されたヒント/ソリューションにアクセスすることができます。 カリキュラムヒント/ソリューションのトピックは、format@@0(https://forum.freecodecamp.org/c/ガイド)の「ガイド」カテゴリにあります。
...
カリキュラムチャレンジ関連の解決策を提案する場合 ガイドのトピック. 完全なコードを追加しなければならない これには、すべての元のシードコードと、すべてのチャレンジテストに合格するために必要な変更が含まれます。 次のテンプレートは、新しいヒント/ソリューションのトピックを作成するときに使用する必要があります:
