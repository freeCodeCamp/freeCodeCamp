...
Наші [Умови користування](https://freecodecamp.org/terms) вимагають, щоб користувачі freeCodeCamp досягли 13-річного віку. If a user reveals that they are under the age of 13, send them the message below, suspend their account, then email the link of their forum account to `support[at]freecodecamp.org` for their freeCodeCamp /learn and forum accounts removal.
...
2. **Якщо користувач безсумнівно та навмисно порушує [Кодекс поведінки](https://code-of-conduct.freecodecamp.org), модератор робить наступне:**
   ...
   Усі модераційні команди приймають параметр `reason`, що вимагає короткого пояснення вжитих дій. Moderation actions done with the bot will be logged in `#mod-actions`, which allows us all to stay on the same page. Вбудовані інструменти дискорду не записуються, тому їх потрібно уникати.
...
4. **Видалення повідомлень**
   ...
   Our moderation bot is configured to log deleted messages automatically in the `#mod-actions` channel. Якщо повідомлення не відповідає нашому Кодексу поведінки або є неприпустимим для нашої спільноти, його можна видалити.
